...
def main():
    ...
    health = HealthLayout.model_validate(
        yaml.safe_load(
            (root / "profiles/classic-1074-health-candidate.yaml").read_text()
        )
    )
    ...
    for name, candidate in candidates:
        with MemorySession(candidate.identity["pid"], CLIENT_SHA256) as session:
            ...
            adapter = SimpleNamespace(
                ...
                request=lambda op, body: operations.dispatch(op, body),
            )
        ...
        path = root / Path(
            state_path(f".runtime/merchant-diagnostic-{name.lower()}.json")
        )
        if path.exists():
            raise ValueError(
                "Diagnostic connection file already exists; check its worker first"
            )
    threads = []
    for name, candidate in candidates:
        path = root / Path(
        ...
        thread = threading.Thread(
            target=serve,
            args=(candidate.identity["pid"], candidate.hwnd, CLIENT_SHA256, path),
            kwargs={"lifetime": 14400, "read_only": True},
            daemon=False,
        )
        thread.start()
...
if __name__ == "__main__":
    ...
    with (Path(state_path("reports/merchants/diagnostics.log"))).open(
        "w", encoding="utf-8"
    ) as log:
