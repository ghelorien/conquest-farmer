...
body = {'health_profile':yaml.safe_load(Path('profiles/classic-1074-health-candidate.yaml').read_text()),
        'character':'Parasite','point':[width//2+58,height-42],'expected_size':[width,height],
        'move_settle_seconds':.2,'position_cursor':args.position_cursor,'expires_at':time.time()+4}
try:
    result = request(info,'background-click',body)
except Exception as error:
    result = {'qualified':False,'error':str(error),'delivery_may_be_partial':True}
report = Path('reports/embedded-positioned-status.json' if args.position_cursor else 'reports/embedded-background-status.json')
