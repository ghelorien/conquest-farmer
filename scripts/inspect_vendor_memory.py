...
header = sample_fields(
    session, [(collection + offset, "u64") for offset in (0x58, 0x60, 0x68)]
)
begin, end, capacity = header
if not (
    0 < begin <= end <= capacity
    and (end - begin) % 16 == 0
    and (capacity - begin) // 16 <= 4096
):
...
for obj, vt in zip(objects, vtables):
    ...
    if (
        rva == layout.monster_vtable_rva
        and struct.unpack_from("<I", data, 0x80)[0] in layout.monster_type_ids
    ):
        continue
    strings = [
        (hex(m.start()), m.group().decode(errors="replace"))
        for m in re.finditer(rb"[A-Za-z][A-Za-z0-9_ -]{3,}", data)
    ]
    rows.append(
        {
            "object": obj,
            ...
            "strings": strings,
            "u32": {
                hex(i): struct.unpack_from("<I", data, i)[0]
                for i in range(0x20, 0x180, 4)
            },
        }
    )
    if len(rows) >= 128:
...
result = {
    ...
    "vtable_counts": dict(collections.Counter(hex(v - base) for v in vtables)),
    "candidates": rows,
}
Path("reports/vendor-memory-candidates.json").write_text(
