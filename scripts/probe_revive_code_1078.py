...
class BridgeSession:
    ...
    def __init__(self, info_path, pid, created, profile_id):
        self.info_path, self.pid, self.created, self.profile_id = info_path, pid, created, profile_id
        self.identity = None
        self.modules = None
    ...
    def __enter__(self):
        ...
        if (health.get("worker") != "live" or health.get("expected_sha256") != CLIENT_SHA256_1078
                ...
                or health.get("embedded_controls", {}).get("control", {}).get("enabled") is not False):
            raise ValueError("Exact stopped farmer worker, client or process identity changed")
    ...
    def assert_identity(self):
        health = request(self.info_path, "health")
        if (health.get("worker") != "live" or health.get("expected_sha256") != self.expected_sha256
                or health.get("profile_id") != self.profile_id
                or health.get("target") != self.identity or health.get("modules") != self.modules
                or health.get("embedded_controls", {}).get("control", {}).get("enabled") is not False):
            raise ValueError("Farmer worker, process, module or control state changed")

    def read(self, address, size):
        if not 0 < size <= 12 * 1024 * 1024 or self.total_bytes + size > 32 * 1024 * 1024:
        ...
        for offset in range(0, size, 65536):
            ...
            result = request(self.info_path, "read-block", {"address": hex(address+offset), "size": length})
            if result.get("address") != hex(address+offset) or result.get("size") != length:
...
def inspect(info_path, pid, created, profile_id):
    with BridgeSession(info_path, pid, created, profile_id) as session:
        ...
        functions = [struct.unpack_from("<II", pdata, pos)
        ...
        for index in range(count):
            ...
            raw = b"".join(session.read(base+rva+offset, min(1048576, size-offset))
        ...
        for name, rva, raw in sections:
            ...
            for label in labels:
                for encoding in ("ascii", "utf-16le"):
                    needle = label.encode(encoding) + (b"\0\0" if encoding == "utf-16le" else b"\0")
                    start = 0
                    while (at := raw.find(needle, start)) >= 0:
                        strings[base+rva+at] = {"rva": hex(rva+at), "label": label,
                                                 "encoding": encoding}
                        start = at + 1
        ...
        for name, rva, raw in sections:
            ...
            for address, size, mnemonic, operand in cs.disasm_lite(raw, base+rva):
                ...
                if target not in strings:
                    continue
                reference = {"rva": hex(address-base), "instruction": mnemonic+" "+operand,
        ...
        for start, end in sorted(selected):
            ...
            disassembly.append({"start_rva": hex(start), "end_rva": hex(end),
                                "instructions": [f"{address-base:x}: {mnemonic} {operand}"
                                                 for address, _, mnemonic, operand
                                                 in cs.disasm_lite(raw, base+start)]})
        session.assert_identity()
        return {"read_only": True, "input_qualified": False, "identity": session.identity,
                "module_sha256": CLIENT_SHA256_1078, "strings": list(strings.values()),
                "references": references, "functions": disassembly}
...
if __name__ == "__main__":
    ...
    print(json.dumps({"output": str(args.output), "strings": len(result["strings"]),
                      "references": len(result["references"]), "functions": len(result["functions"])}))
