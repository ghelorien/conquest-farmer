...
def main():
    ...
    fields = [
        {"name": "position", "address": hex(addresses["position"]), "kind": "xy_u32"},
    ...
    result = {
        ...
        "player_object": addresses["object"],
        "window_before": health["window"],
        "control_before": health["embedded_controls"]["control"],
        "requested_hz": args.hz,
        "samples": [],
        "errors": [],
    }
    print(
        json.dumps({"watch_started": str(output), "app_pid": state["pid"]}), flush=True
    )
    ...
    while time.monotonic() - start < args.seconds:
        ...
        try:
            ...
            result["errors"].append(
                {"elapsed": round(time.monotonic() - start, 3), "error": str(error)}
            )
            break
    ...
    result["mapping_source"] = (
        "C:/Program Files/Classic Conquer 2.0/ini/ActionSound.ini"
    )
    output.write_text(json.dumps(result, indent=2), encoding="utf-8")
    print(
        json.dumps(
            {
                ...
                "jump_motion_observed": result["jump_motion_observed"],
                "positions": list(
                    dict.fromkeys(tuple(row["position"]) for row in result["samples"])
                ),
