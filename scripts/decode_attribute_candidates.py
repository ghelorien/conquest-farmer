...
player=u64(shared)
if u64(player)!=base+0x5cef60:raise ValueError('Player type changed')
...
if not 1<=count<=1024 or mode not in range(4):raise ValueError(f'Invalid attribute bounds: {mode},{count}')
data=read(table,count*4)
values=[]
for i in range(count):
    if mode==0: j=i
    elif mode==1:j=count-1-i
    elif mode==2:j=i//2 + (count//2+1 if count&1 else 0)
    else:j=i//2 + (count//2 if not count&1 else 0)
    if j>=count:break
...
if u64(player+0x968)!=attrs or read(attrs,24)!=header or read(table,count*4)!=data:
    raise ValueError('Attributes changed during sample')
if u64(base+0x69c730)!=shared or u64(shared)!=player:raise ValueError('Player changed')
if request(info,'health')['target']!=health['target']:raise ValueError('Process changed')
result={'qualified':False,'process_identity':health['target'],'player':hex(player),'attributes':hex(attrs),
    'mode':mode,'count':count,'values':values}
(root/'reports/attribute-candidates-current.json').write_text(json.dumps(result,indent=2))
