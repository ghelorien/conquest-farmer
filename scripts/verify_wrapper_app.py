...
def fixture(args):
    ...
    entry.bind(
        "<KeyRelease>",
        lambda event: Path(args.stop_file + ".typing").write_text(entry.get()),
    )
    ...
    def draw():
        ...
        canvas.create_text(
            width / 2,
            height / 2 - 20,
            text="Wrapper verification · no game connection",
            fill="#8aa6c0",
            font=("Segoe UI", 13),
        )
        x = 50 + (time.monotonic() - started) * 90 % max(width - 100, 1)
        canvas.create_oval(
            x - 12, height / 2 + 35, x + 12, height / 2 + 59, fill="#3ed4a0", outline=""
        )
        canvas.create_text(
            width / 2,
            height - 45,
            text=f"{width} × {height} · rendering live",
            fill="#8aa6c0",
            font=("Segoe UI", 12),
        )
        root.after(40, draw)
...
def verify(args):
    ...
    app.detail_text.set(
        "Isolated wrapper test. Conquer is not connected to this preview."
    )
    result = {
        "live_conquer_tested": False,
        "scope": "actual DesktopApp with native fixture",
    }
    started = time.monotonic()
    ...
    def step():
        ...
        try:
            now = time.monotonic()
            if now - started > (
                180 if args.keyboard_preview else args.preview_seconds + 18
            ):
                raise TimeoutError(
                    f"Wrapper verification timed out at {stage}: {app.state_text.get()}"
                )
            if stage == "launch" and app.host.saved:
                ...
                deadline, stage = now + max(args.preview_seconds, 0.6), "resize"
                print("Wrapper preview ready", flush=True)
            elif stage == "resize" and now >= deadline:
                assert win32gui.GetClientRect(saved.hwnd)[2:] == (
                    app.pane.winfo_width(),
                    app.pane.winfo_height(),
                )
                result["resized"] = True
                assert app.show_game(), app.state_text.get()
                assert app.host.api.contains(
                    saved.hwnd, app.host.api.thread_info(saved.hwnd)[1].hwndFocus
                )
                result["keyboard_focus_handed_to_client"] = True
                if args.keyboard_preview:
                    stage = "keyboard_preview"
                    print(
                        "Keyboard preview ready: click the test entry and type focusworks",
                        flush=True,
                    )
                else:
                    app.release()
                    deadline, stage = now + 0.3, "released"
