...
def main():
    ...
    parser.add_argument(
        "--watch",
        action="store_true",
        help="Run until stopped; requires verified live rollout. Default only collects prices.",
    )
    ...
    with lock_path.open("a+b") as lock:
        ...
        try:
            if not args.watch:
                data = collect_market()
                print(
                    f"Collected {data['total']} America listings. No game operation requested."
                )
                return
            verify_rollout()
            while True:
                try:
                    result = run_cycle()
                    write_json(
                        state_path("reports/merchants/script-worker.json"),
