...
def main():
    ...
    try:
        ...
        while time.monotonic()<deadline and win32gui.GetClientRect(hwnd)[2:]!=(300,180):
            time.sleep(.02)
        assert win32gui.GetParent(hwnd)==pane.winfo_id()
        assert win32gui.GetClientRect(hwnd)[2:]==(300,180)
        host.resize(280,160)
        deadline = time.monotonic()+2
        while time.monotonic()<deadline and win32gui.GetClientRect(hwnd)[2:]!=(280,160):
            time.sleep(.02)
        ...
        result = {'scope':'temporary Tk window, separate process',
            'embedded':True,'resized':True,'restored':True,
            'live_conquer_tested':False,'time':time.time()}
