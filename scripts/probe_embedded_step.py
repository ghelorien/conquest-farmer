...
result = {'qualified':False,'generated_at':generated_at.isoformat(),
          'process_identity':health['target'],'before_position':before,
          'baseline_positions':baseline,
          'expected_position':[before[0]+args.dx,before[1]+args.dy],
          'point':point,'position_cursor':args.position_cursor}
try:
    result['input'] = request(info,'background-click',{
        'health_profile':layout.model_dump(mode='json'),'character':'Parasite',
        'point':point,'expected_size':[width,height],'position_cursor':args.position_cursor,
    ...
    result['expected_position_stable']=all(p==result['expected_position'] for p in samples[-3:])
    original=result['input']['before']
    result['sampled_foreground_unchanged']=all(w['foreground']==original['foreground'] for w in windows)
    result['sampled_cursor_unchanged']=all(w['cursor']==original['cursor'] for w in windows)
...
Path(f'reports/{name}-{generated_at.strftime("%Y%m%dT%H%M%S%fZ")}.json').write_text(rendered,encoding='utf-8')
