...
with sqlite3.connect(path.as_uri()+'?mode=ro',uri=True) as db:
    records=[(t,json.loads(payload)['count']) for t,payload in db.execute(
        'select time,payload from events where time>? and event=? order by time',
        (now-900,'kill_verified'))]
windows={str(seconds):{'kills':sum(n for t,n in records if t>now-seconds)}
         for seconds in (60,300,900)}
for seconds,window in windows.items():window['kills_per_minute']=window['kills']*60/int(seconds)
