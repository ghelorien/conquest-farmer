...
def main():
    ...
    parser.add_argument(
        "--report", default="reports/performance/resolution-endurance-monitor.json"
    )
    args = parser.parse_args()
    if not 1 <= args.seconds <= 3600:
        raise ValueError("Observation duration must be 1–3600 seconds")
    ...
    if not 0 < started <= time.time():
        raise ValueError("Run start must be a past timestamp")
    app = read_json("reports/desktop-farming/app-state.json")
    info = app.get("worker_info_path")
    if not info:
        raise ValueError("No initial worker connection available")
    report = args.report
    ...
    rows = db.execute(
        "select rowid,time,event,payload from events order by rowid desc limit 50000"
    ).fetchall()
    cursor = max((r[0] for r in rows), default=0)
    events.extend(
        (t, e, json.loads(p)) for _, t, e, p in reversed(rows) if t >= started
    )
    try:
        while time.monotonic() < deadline:
            ...
            sample = {
                ...
                "max_hp": life.get("max_hp"),
                "dead": life.get("dead_candidate"),
                "flying": bool(life.get("status", 0) & 0x8000000),
                "enabled": data["control"]["enabled"],
                "execution": data["control"]["execution_state"],
                "manual_mouse": data.get("manual_mouse"),
            }
            samples.append(sample)
            if now - last_publish >= 30:
                for rowid, t, event, payload in db.execute(
                    "select rowid,time,event,payload from events where rowid>? order by rowid",
                    (cursor,),
                ):
                    cursor = rowid
                    if t >= started:
                        events.append((t, event, json.loads(payload)))
                elapsed = now - started
                kills = sum(
                    p.get("count", 0) for _, e, p in events if e == "kill_verified"
                )
                ...
                summary = {
                    ...
                    "route": {
                        ...
                        if k in ("phase", "activity", "updated_at", "pid", "detail")
                    },
                }
