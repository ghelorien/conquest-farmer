...
def main():
    ...
    with MemorySession(args.pid, CLIENT_SHA256_1078) as session:
        ...
        for rva, expected_hex in _PROJECTION_CODE.items():
            ...
            if first != second:
                raise ValueError(
                    f"Loaded code changed during observation at RVA 0x{rva:x}"
                )
            difference = next(
                (
                    i
                    for i, (left, right) in enumerate(zip(expected, first))
                    if left != right
                ),
                None,
            )
        ...
        if args.collect:
            ...
            if (
                ...
                raise ValueError(
                    "Merchant ownership identity changed before booth preflight"
                )
            actor = _owned_scene_actor(
                session,
                MemoryEntityReader(session, entity_reader_layout(session)),
                snapshot,
                module,
            )
            kind_reads = {
                offset: session.read(actor["address"] + offset, 4)
                for offset in (0x2B0, 0x2C0, 0x2D0)
            }
            if any(
                session.read(actor["address"] + offset, 4) != raw
                for offset, raw in kind_reads.items()
            ):
                raise ValueError(
                    "Owned booth hit-test fields changed during observation"
                )
            actor_kind = {
                "actor_address": hex(actor["address"]),
                "kind_candidates": {
                    hex(offset): struct.unpack("<I", raw)[0]
                    for offset, raw in kind_reads.items()
                },
            }
