...
def main():
    ...
    parser.add_argument('--input-mode',choices=('background','foreground'),default='background')
    ...
    report={'before':asdict(before),'window_before':window,'revival_verified':False,'samples':[]}
    if not args.send:
    ...
    checkpoint={'identity':session.identity,'map_id':before.map_id,
        'death_position':list(before.position),'phase':'waiting_for_revive','report':str(output)}
    checkpoint_path=Path('.runtime/death-return.json')
    if checkpoint_path.exists():
        previous=json.loads(checkpoint_path.read_text())
        if previous['identity']==session.identity and previous['phase']!='completed':
            checkpoint['death_position']=previous['death_position']
    checkpoint_path.write_text(json.dumps(checkpoint,indent=2),encoding='utf-8')
    output.write_text(json.dumps(report,indent=2),encoding='utf-8')
    try:
        report['input']=request(info,'revive-click',{'health_profile':layout.model_dump(mode='json'),
            'character':'Parasite','expected_size':[1036,793],'expires_at':time.time()+4,
        ...
        for _ in range(20):
            ...
            recovered=(not life.ghost_candidate and life.current_hp>=life.max_hp*.4)
            consecutive=consecutive+1 if recovered else 0
            if consecutive>=3:
                report['revival_verified']=True
                checkpoint.update(phase='return_pending',revival_position=list(life.position))
                break
            time.sleep(.25)
        ...
        output.write_text(json.dumps(report,indent=2),encoding='utf-8')
        checkpoint_path.write_text(json.dumps(checkpoint,indent=2),encoding='utf-8')
        print(json.dumps({'report':str(output),'revival_verified':report['revival_verified'],
            'last':report['samples'][-1] if report['samples'] else None,'error':report.get('error')}))
