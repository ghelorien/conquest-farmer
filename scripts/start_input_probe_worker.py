...
def main():
    ...
    if info.exists():
        ...
        if (
            health.get("expected_sha256") == expected
            and health.get("read_only") is False
            and health.get("input_revision", 0) >= 7
        ):
            return 0
        raise ValueError(
            "Existing input worker is incompatible; inspect it before replacement"
        )
    ...
    if report.gate == "blocked":
        raise ValueError(
            "; ".join(c.detail for c in report.checks if c.status == "failed")
        )
