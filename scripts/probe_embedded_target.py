...
def main():
    ...
    if args.send and (args.target_id is None or args.point is None):
        raise ValueError(
            "--send requires one exact target ID and inspected sprite point"
        )
    ...
    player = PlayerLayout.model_validate(
        yaml.safe_load(Path("profiles/classic-1074-player-candidate.yaml").read_text())
    )
    hp_layout = HealthLayout.model_validate(
    ...
    base = next(
        ...
        if m["name"].casefold() == player.module.casefold()
    )
    ...
    def player_fields():
        ...
        return {
            "position": list(
                struct.unpack("<II", session.read_block(addresses["position"], 8))
            ),
            "level": struct.unpack("<I", session.read_block(addresses["level"], 4))[0],
            "map_id": struct.unpack("<I", session.read_block(addresses["map"], 4))[0],
            "kill_counter": struct.unpack(
                "<I", session.read_block(addresses["kill_counter"], 4)
            )[0],
        }

    player_before = player_fields()
    if player_before["map_id"] != 1002 or health["window"]["client_size"] != [
        1036,
        793,
    ]:
            ...
            rows.append(
                {
                    ...
                    "projected_feet": projected,
                    "draw_position": candidate["draw_position"],
    ...
    result["health_before"] = asdict(
        MemoryHealthReader(session, hp_layout, "Parasite").read()
    )
    ...
    if (
        target["hp_candidate"] <= 0
        or result["inventory_before"]["equipped_ammo"] is None
    ):
    ...
    if not (
        50 <= point[0] <= 986
        and 60 <= point[1] <= 653
        and abs(point[0] - draw[0]) <= 24
        and draw[1] - 48 <= point[1] <= draw[1] + 8
    ):
        raise ValueError(
            "Point is outside the selected actor sprite region or inspected scene"
        )
    ...
    if fresh != target or player_fields() != player_before:
        raise ValueError(
            "Player or selected target changed before click; no input sent"
        )
    result["point"] = point
    try:
        result["input"] = request(
            ...
            "background-click",
            {
                "health_profile": hp_layout.model_dump(mode="json"),
                "character": "Parasite",
                "point": point,
        ...
        for _ in range(15):
            ...
            try:
                ...
                sample["equipped_ammo"] = (
                    asdict(current.equipped_ammo) if current.equipped_ammo else None
                )
            except (OSError, ValueError) as error:
                sample["ammo_error"] = str(error)
            samples.append(sample)
            time.sleep(0.15)
        result["health_after"] = asdict(
            MemoryHealthReader(session, hp_layout, "Parasite").read()
        )
