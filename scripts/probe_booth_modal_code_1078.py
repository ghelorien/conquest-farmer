...
def main(args):
    ...
    with MemorySession(args.pid, CLIENT_SHA256_1078) as session:
        ...
        functions = [
            struct.unpack_from("<III", pdata, pos)[:2]
        ...
        for index in range(count):
            ...
            if header[pos : pos + 8].rstrip(b"\0") in (b".text", b".rdata"):
                ...
                raw = b"".join(
                    session.read(base + rva + off, min(1048576, size - off))
        ...
        for start, end in sorted(selected):
            ...
            for address, size, mnemonic, operand in cs.disasm_lite(raw, start):
                ...
                if (
                    value
                    and all(32 <= byte < 127 for byte in value)
                    and len(value) < 128
                ):
        ...
        if (
            any(before[key] != after[key] for key in stable)
            or reader._model(25, reader.booth_vtable_rva) != model
        ):
        ...
        result = {
            ...
            "model_hex": model_raw.hex(),
            "vtable": vtable,
            "label_references": refs,
            "rip_strings": rip_strings,
...
if __name__ == "__main__":
    ...
    try:
        ...
        with arguments.output.with_suffix(".error.txt").open(
            "x", encoding="utf-8"
        ) as stream:
