...
parser.add_argument('--destination',nargs=2,type=int,help='An inspected stop on the same map')
...
report={'route':route.id,'phase':args.phase,'destination':destination,'steps':[],'arrived':False}
...
try:
    while time.monotonic()<deadline:
        ...
        if health['embedded_controls']['control']['enabled']:
            raise ValueError('The standalone route cannot share input with the farm executor')
        if life['dead_candidate'] or life['current_hp']<life['max_hp']*.4:
            time.sleep(.1)
            continue
        ...
        try:
            ...
            if str(error) in ('Player left the planned route segment','Player moved from the planned starting tile','Life state changed during observation','Health fields or pointer topology changed during sampling'):
                time.sleep(.1)
                continue
            raise
        report['steps'].append(result)
        print(json.dumps({'source':source,'destination':target,'reached':result['reached'],
                          'movement':result.get('input',{}).get('movement')}),flush=True)
        if not result['reached']:
            if len(avoided)>=6:
                break
            latest=stepper.observe().position
            dx,dy=target[0]-source[0],target[1]-source[1]
            blocked=(latest[0]+(1 if dx>0 else -1 if dx<0 else 0),
    ...
    print(json.dumps({'arrived':report['arrived'],'destination':destination}),flush=True)
