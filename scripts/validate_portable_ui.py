...
def main():
    with tempfile.TemporaryDirectory(prefix='conquest-ui-check-') as temporary:
        ...
        registry.add('Parasite');registry.add('Spiritual',role='Merchant')
        registry.add('Dutch',role='Merchant');registry.add('Farmer',role='Merchant')
        ...
        config=select_profile(repo,SimpleNamespace(profile_id=active.id,manage_profiles=False),Path(temporary))
        ...
        with patch('conquest.mouse_priority.install',return_value=SimpleNamespace(active=lambda:False)), \
            ...
            ui=UnifiedUI(app);app.unified=ui
            ...
            assert ui.dispatch({'action':'profiles'})['profiles'][0]['profile_id']==active.id
            ui.close();app.stop_observer();root.destroy()
            print('PASS: dynamic tabs, reserved-name isolation, duplicate-server profile, paused startup, no client input')
