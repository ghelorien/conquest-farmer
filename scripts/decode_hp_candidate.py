...
if struct.unpack_from('<Q',data)[0] != base+0x5cef60: raise ValueError('Player type mismatch')
result={'qualified':False,'process_identity':health['target'],
    'root_rva':'0x69c730','pointer_offsets':[0,0],
    'object':hex(player),'name':data[0x94:0xd4].split(b'\0')[0].decode(),
...
result['candidate_current_hp']=sum(w<<(8*i) for i,w in enumerate(words)) if all(0<=w<=255 for w in words) else None
if struct.unpack('<Q',read(base+0x69c730,8))[0]!=shared or struct.unpack('<Q',read(shared,8))[0]!=player:
