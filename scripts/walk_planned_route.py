...
parser.add_argument('--phase',choices=('outbound','return','patrol'),default='outbound')
...
stepper=BridgeSceneStepper(state['worker_info_path'],'profiles/classic-1074-player-candidate.yaml',
    'profiles/classic-1074-health-candidate.yaml','Parasite',expected_map=1002,
    allowed_bounds=(400,370,710,710),max_delta=4)
...
if args.route:
    ...
    report={'source':source,'destination':args.destination,'tile_steps':len(path)-1,
            'terrain_sha256':terrain.source_sha256,'waypoints':straight_waypoints(path,4)}
...
try:
    for destination in waypoints[1:args.maximum_segments+1]:
        ...
        print(json.dumps({'destination':destination,'reached':result['reached'],'error':result.get('error')}),flush=True)
    ...
    output.write_text(json.dumps(report,indent=2),encoding='utf-8')
    print(json.dumps({'report':str(output),'arrived':report['arrived'],
                      'last_verified_position':source,'error':report.get('error')}),flush=True)
