...
def main():
    ...
    try:
        ...
        if not kernel.GetProcessTimes(handle, ctypes.byref(created), ctypes.byref(exited),
            ...
            raise OSError(ctypes.get_last_error(), "Cannot read controller creation time")
        ...
        if not user.GetWindowThreadProcessId(args.hwnd, ctypes.byref(owner)) or owner.value != args.pid:
        ...
        if not user.PostMessageW(args.hwnd, 0x0010, 0, 0):
            raise OSError(ctypes.get_last_error(), "Normal controller close was refused")
