...
if not stop.get("shop_memory_qualified") or not stop.get("npc_entity_id"):
    raise ValueError(
        "Memory NPC identity and open-shop item reader must be connected before buying; visual fallback is disabled"
    )
...
inventory = MemoryInventoryReader(
    session,
    layout,
...
try:
    for _ in range(args.count):
        ...
        if (
            health["embedded_controls"]["control"]["enabled"]
            or life["map_id"] != 1002
            or life["position"] != [466, 333]
            or life["dead_candidate"]
        ):
            raise ValueError(
                "Restock requires the inspected living character at the Pharmacist, with farming Off"
            )
        ...
        while True:
            ...
            if (
                after.count(1000020) == before.count(1000020) + 1
                and after.silver == before.silver - 60
            ):
                break
