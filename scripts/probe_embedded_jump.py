...
def main():
    ...
    if (
        health["input_revision"] < 9
        or health["embedded_controls"]["control"]["enabled"]
    ):
        raise ValueError(
            "The current worker must support Control messages with farming Off"
        )
    ...
    if (
        before.ghost_candidate
        or before.current_hp < before.max_hp * 0.8
        or before.position != (430, 380)
        or before.map_id != 1002
    ):
        raise ValueError("Healthy, recovered town baseline changed; no input sent")
    window = health["window"]
    if (
        window["foreground"] == window["root_hwnd"]
        or window["minimized"]
    ...
    def sample():
        ...
        return {
            "position": list(struct.unpack_from("<II", data)),
            "motion_candidate_words": {
                hex(0xD8 + i): struct.unpack_from("<I", data, i)[0]
    ...
    baseline = sample()
    time.sleep(0.2)
    if sample()["position"] != baseline["position"] or baseline["position"] != list(
        before.position
    ):
    ...
    result = {
        ...
        "baseline": baseline,
        "expected_position": list(destination),
        "terrain_sha256": terrain.source_sha256,
        "checked_segment": segment,
        "samples": [],
        "note": "Control messages and arrival do not alone distinguish jumping from walking.",
    }
    ...
    try:
        ...
        result["arrival_verified"] = all(
            row["position"] == list(destination) for row in result["samples"][-3:]
        )
    ...
    print(
        json.dumps(
            {
                "report": str(output),
                "arrival_verified": result["arrival_verified"],
                "jump_qualified": False,
                "dispatch_elapsed": result.get("dispatch_elapsed"),
                "positions": [
                    (round(row["elapsed"], 3), row["position"])
