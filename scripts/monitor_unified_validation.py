...
def source_integrity(config):
    ...
    try:
        ...
        if hashlib.sha256(raw).hexdigest() != config["release_manifest_sha256"]:
            return False
        ...
        for relative, digest in value["files"].items():
            ...
            if path.is_absolute() or not path.parts or ".." in path.parts:
                return False
            allowed = (
                len(path.parts) > 1 and path.parts[0] in RELEASE_DIRECTORIES
...
def qualification(config, now, kills, app, route, previous):
    ...
    class ReadOnlyJournal:
        ...
        def db(self):
            path = Path(
                state_path("reports/banking/merchant-deliveries.sqlite3")
            ).resolve()
            with closing(
    ...
    if (
        not source_checked
        or now - source_checked >= 60
        or now >= config["started_at"] + config["duration_seconds"]
    ):
        source_checked = now
        if not source_integrity(config):
            interruptions.append(
                "Release source manifest is missing or changed during qualification"
            )
    for name, actual in (("app_pid", app.get("pid")), ("route_pid", route.get("pid"))):
        if not config.get(name):
            interruptions.append("Qualification lacks baseline " + name)
        elif actual != config[name]:
            interruptions.append(name + " changed during qualification")
    ...
    source = ReadOnlyJournal()
    deliveries = []
    with source.db() as db:
        rows = db.execute(
            "SELECT id,created,updated,before_json,character,phase FROM transactions WHERE kind='farmer_delivery' AND created>=?",
            (config["started_at"],),
        ).fetchall()
    ...
    for row in rows:
        ...
        merchant = before.get("merchant", {})
        farmer = before.get("farmer", {})
        record = database_character(row["character"])
        deliveries.append(
            {
                **status(source, row["id"]),
                "started_at": row["created"],
                "verified_at": row["updated"] if row["phase"] == "verified" else None,
                "merchant_identity": {
                    "record_id": row["character"],
                    "profile_id": getattr(record, "profile_id", None),
                    "character": merchant.get("character"),
                    "character_uid": merchant.get("character_uid"),
                },
                "farmer_identity": {
                    "profile_id": before.get("farmer_profile_id"),
                    "character": farmer.get("character"),
                    "character_uid": farmer.get("character_uid"),
                },
            }
        )
    with closing(
        sqlite3.connect(
            Path(state_path("reports/merchants/journal.sqlite3")).resolve().as_uri()
            + "?mode=ro",
            uri=True,
            timeout=1,
        )
        ...
        refills = [
            ...
            for event_id, character, at, raw in db.execute(
                "SELECT id,character,timestamp,payload FROM events WHERE event='refill_checked' AND timestamp BETWEEN ? AND ?",
                (
                    config["started_at"],
                    min(now, config["started_at"] + config["duration_seconds"]),
                ),
            )
        ]
        pending_merchants = [
            ...
            for row in db.execute(
                "SELECT id FROM transactions WHERE created>=? AND phase NOT IN ('verified','aborted')",
                (config["started_at"],),
            )
        ]
    ...
    pending_notes = (
        ["New merchant transactions remain unresolved"] if pending_merchants else []
    )
    ...
    result = evaluate(
        ...
        interruptions=interruptions + pending_notes,
    )
    return {
        **result,
        "target_met": result["qualified"],
        "interruptions": interruptions,
        "source_checked_at": source_checked,
        "unresolved_merchant_operations": pending_merchants,
        "refill_results": refills,
...
def main():
    ...
    cursor = (
        old.get("cursor", config["baseline_event_id"])
        if same
        else config["baseline_event_id"]
    )
    ...
    while True:
        ...
        try:
            with sqlite3.connect(
                dbpath.as_uri() + "?mode=ro", uri=True, timeout=1
            ) as db:
                rows = db.execute(
                    "select rowid,time,event,payload from events where rowid>? order by rowid limit 50000",
                    (cursor,),
                ).fetchall()
            for rid, at, event, raw in rows:
                ...
                if event == "kill_verified":
                    kills.append([at, json.loads(raw)["count"]])
            ...
            windows = {
                str(n): {
                    "complete": elapsed >= n,
                    "kills": sum(k for t, k in kills if now - n < t <= now),
                    "kills_per_minute": round(
                        sum(k for t, k in kills if now - n < t <= now) * 60 / n, 2
                    ),
                }
                for n in (60, 300, 900)
            }
            report = {
                ...
                "observed_at": now,
                "cursor": cursor,
                "phase": "complete" if now >= end else "monitoring",
                "elapsed_seconds": round(elapsed, 1),
                "total_kills": sum(k for t, k in kills),
                "overall_kills_per_minute": round(
                    sum(k for t, k in kills) * 60 / elapsed, 2
                ),
                "windows": windows,
                ...
                "route_activity": route.get("activity"),
                "route_error": route.get("detail"),
                "route_cycles": route.get("cycles"),
                "target_met": False,
                "observer_is_read_only": True,
            }
            report.update(
                qualification(config, now, kills, app, route, old if same else {})
            )
            ...
            if not same:
                old = {}
                same = True
            old.setdefault("interruptions", []).append(
                "Qualification telemetry was unavailable during observation"
            )
            if now >= end:
                raise
        if now >= end:
