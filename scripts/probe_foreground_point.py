...
layout = HealthLayout.model_validate(yaml.safe_load((root/'profiles/classic-1074-health-candidate.yaml').read_text()))
...
result = request(info, 'foreground-click', {'point':[826,751], 'expected_size':[1536,793],
    'require_foreground':True, 'guard':{'name_address':hex(addresses['name']),
        'name':'Parasite', 'hp_address':hex(addresses['max_hp']), 'max_hp':hp.max_hp}})
(root/'reports/foreground-status-scaled.json').write_text(json.dumps(result, indent=2))
