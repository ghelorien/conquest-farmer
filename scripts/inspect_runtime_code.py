...
def read(address, size):
    ...
    for offset in range(0, size, 65536):
        ...
        if len(data) != n or int(r["address"], 0) != address + offset:
...
refs = json.loads((root / "reports/player-reference-scan.json").read_text())[
    "candidates"
]["player_pointer"]["addresses"]
...
result = {
    ...
    "root_links": links,
    "player_vtable_functions": [hex(v - base) for v in struct.unpack("<64Q", vtable)],
    "max_hp_references": matches,
}
