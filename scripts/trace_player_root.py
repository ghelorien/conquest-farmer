...
def read(address, size):
    ...
    for offset in range(0, size, 512):
        fields = [{'name': str(a), 'address': hex(a), 'kind': 'u64'}
                  for a in range(address+offset, address+min(offset+512,size),8)]
        result.extend(b''.join(struct.pack('<Q', f['value'][0])
...
if len(matches)!=1: raise ValueError('Expected one player object')
player=matches[0]
...
for offset in range(0,len(module)-7,8):
    ...
    if value==player: links.append(hex(0x690000+offset))
result={'qualified':False,'process_identity':health['target'],'object':hex(player),
    'direct_player_root_rvas':links,
    'hp_213_u32_offsets':[hex(i) for i in range(len(data)-3) if struct.unpack_from('<I',data,i)[0]==213],
    'hp_213_f32_offsets':[hex(i) for i in range(len(data)-3) if struct.unpack_from('<f',data,i)[0]==213]}
