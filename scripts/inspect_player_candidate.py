...
def sample(addresses):
    return request(info, 'sample', {'fields': [
        {'name': hex(a), 'address': hex(a), 'kind': 'u64'} for a in addresses]})['fields']
...
for offset in range(0, 4096, 512):
    data.extend(b''.join(struct.pack('<Q', f['value'][0]) for f in sample(
        list(range(player + offset, player + offset + 512, 8)))))
...
result = {'qualified': False, 'first': hex(first), 'object': hex(player),
          ...
          'fields': {hex(o): struct.unpack_from('<I', data, o)[0]
