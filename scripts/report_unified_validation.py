    ...
    for row in sorted(timeline, key=lambda r: r["time"]):
        ...
        if event == "kill_verified":
            ...
            if pending and pending.get("arrived_at"):
                completed.append({**pending, "hunting_resumed_at": at})
                pending = None
        elif event == "return_required":
            counts = row.get("supplies", {})
            required = (
                row.get("reason")
                in ("inventory_full", "ammo_unavailable", "potions_exhausted")
                or counts.get("arrows", 3) < 3
                or counts.get("potions", 1) <= 0
                or counts.get("free_slots", 1) <= 0
            )
            pending = (
                {"return_started_at": at, "storage_receipts": []}
                if hunting and required and not row.get("validation_cycle")
                else None
            )
        elif (
            event == "valuable_stored"
            and pending
            and not pending.get("restock_complete_at")
        ):
            ...
            counts = row.get("supplies", {})
            if (
                pending["storage_receipts"]
                and counts.get("arrows", 0) >= 3
                and counts.get("potions", 0) > 0
                and counts.get("free_slots", 0) > 0
            ):
                pending["restock_complete_at"] = at
        elif (
            event == "farming_area_reached"
            and pending
            and pending.get("restock_complete_at")
        ):
...
def report(root=ROOT, *, now=None, extend=False):
    ...
    observed = now if extend else min(now, end)
    elapsed = max(observed - start, 1)
    database = root / "reports/desktop-farming/trial.sqlite3"
    with sqlite3.connect(database.as_uri() + "?mode=ro", uri=True, timeout=2) as db:
        rows = db.execute(
            "select time,event,payload from events where rowid>? and time>=? and time<=? order by rowid",
            (config["baseline_event_id"], start, observed),
        ).fetchall()
    counts = Counter()
    kills = []
    delays = []
        ...
        with path.open(encoding="utf-8") as source:
            for line in source:
                try:
                    ...
                    continue
                if not start <= r.get("time", 0) <= observed:
                    continue
                route_counts[r["event"]] += 1
                timeline.append(r)
                if r["event"] == "runback_finished":
                    travels.append(r)
    windows = {
        str(n): {
            "complete": elapsed >= n,
            ...
            "kills_per_minute": round(
                sum(k for at, k in kills if observed - n < at <= observed) * 60 / n, 2
            ),
        }
        for n in (60, 300, 900, 3600)
    }
    ...
    result = {
        ...
        "verified_kills": total,
        "overall_kills_per_minute": round(total * 60 / elapsed, 2),
        "windows": windows,
        "events": dict(counts),
        "route_events": dict(route_counts),
        "travel_receipts": travels,
        "minimum_observed_hp_percent": None
        if minimum_hp is None
        else round(minimum_hp * 100, 1),
        "jump_to_scatter": {
            "samples": len(delays),
            "median_seconds": round(statistics.median(delays), 3) if delays else None,
            "p95_seconds": round(
                sorted(delays)[min(len(delays) - 1, int(len(delays) * 0.95))], 3
            )
            if delays
            else None,
        ...
        "natural_restock_storage_return_observed": bool(cycles),
        "completed_cycles": cycles,
        "malformed_kill_events": malformed,
        "rate_target_met": now >= end and total * 60 / elapsed >= 40,
        "all_downtime_retained": True,
        "farming_not_stopped_by_report": True,
    }
    result.update(original_deadline=end, extended_observation=extend)
    output = (
        "unified-validation-extended-report.json"
        if extend
        else "unified-validation-report.json"
    )
        ...
        json.dumps(
            {
                ...
                if k not in ("travel_receipts", "events", "route_events")
            },
            indent=2,
        )
    )
