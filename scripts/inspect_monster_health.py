...
for monster in health['embedded_controls']['monsters'][:8]:
    ...
    row = {'id':monster['entity_id'],'type':monster['type_id'],'name':monster['name'],'max_hp':monster['max_hp']}
    try:
        record = session.read_block(obj,0x988)
        if struct.unpack_from('<Q',record)[0]!=base+0x5c5e20 or struct.unpack_from('<I',record,0x78)[0]!=monster['entity_id']:
        ...
        row.update(candidate_hp=hp,in_bounds=0<=hp<=monster['max_hp'],mode=mode,count=count)
    except (ValueError,OSError) as error:
        row['error'] = str(error)
    rows.append(row)
result = {'qualified':False,'process_identity':health['target'],'rows':rows}
Path('reports/monster-health-candidates.json').write_text(json.dumps(result,indent=2),encoding='utf-8')
