...
XP skill: the pinned client HUD at RVA 0x994d0 reads actor+0x3cc and renders XP out of 100. Status bit 0x10 enables the ready XP popup; 0x08000000 is flying. The popup renderer iterates actor+0x1998, independently of the learned skill list. The runner locates one enabled Fly (8002) entry with self-target flag 2 in the XP vector, including when ArrowRain is also learned. It requires a stable memory-identified popup with 40x40 buttons, 8-pixel spacing/padding, and dimensions 56 + 48*(entry_count-1) by 56; it clicks only the matched Fly button through guarded foreground input. Disabled neighbors still occupy their vector-order slot. Duplicate identities, changed entries, reordered vectors, and changed geometry are rejected. Flight is only reported as verified after the status bit changes. Live activation verified on September 8: at full XP the ordinary popup click activated Fly; fresh memory showed status 0x08000000 and charge 0, while the route continued. Evidence: reports/fly-auto-validation.json.
...
September 13 multi-skill validation: Kilhiam's ready XP vector contained enabled Fly (8002) and ArrowRain (8030); the live popup was 104x56 at (976,1056). The updated app selected Fly at (1004,1084), then read charge 0 and flying=true and emitted xp_fly_verified while farming continued. App PID 51292; local evidence is characters/dd592fb1-9bfe-4987-8249-6f7d301688f0/diagnostics/fly-multi-popup-validation.json under the Conquest data root. One live activation cycle was verified; both button orders, disabled neighbors and changed/ambiguous layouts are covered by regression tests.
