...
def activate_native_caption(hwnd, identity, api):
    ...
    if (gui.GetAncestor(hwnd, 2) != hwnd or gui.GetWindow(hwnd, 4)
            or not gui.IsWindowVisible(hwnd) or gui.IsIconic(hwnd)
            or gui.GetClassName(hwnd) != 'ImGuiShell'
            or not (title == '[ClassicConquer]' or
    ...
    with physical_coordinates():
        ...
        try:
            ...
            def caption():
                check_input();api.assert_owner(hwnd, identity)
                if (key_state(0x7B) & 0x8000 or gui.GetAncestor(hwnd, 2) != hwnd
                        or gui.GetWindow(hwnd, 4) or not gui.IsWindowVisible(hwnd)
                        or gui.IsIconic(hwnd) or gui.GetClassName(hwnd) != 'ImGuiShell'
                        or gui.GetWindowText(hwnd).strip() != title
                        or gui.GetWindowRect(hwnd) != rect):
                    raise CaptureUnavailable('Native Conquest title bar changed or Stop was pressed; no button pressed')
                if gui.WindowFromPoint(point) != hwnd:
                    raise CaptureUnavailable('Native Conquest title bar is covered; no button pressed')
                result = c.c_size_t()
                coords = c.c_int32(((point[1]&65535)<<16)|(point[0]&65535)).value
                if not hit(hwnd, 0x84, 0, coords, 2, 200, c.byref(result)) or result.value != 2:
                    raise CaptureUnavailable('Native Conquest title bar hit test is not HTCAPTION; no button pressed')
            caption()
            metrics = bind(api.backend.user, 'GetSystemMetrics', [c.c_int], c.c_int)
            x,y,width,height = [metrics(n) for n in (76,77,78,79)]
            if min(width,height) <= 1:
                raise CaptureUnavailable('Native desktop geometry is invalid; no button pressed')
            send = guarded_send(bind(api.backend.user, 'SendInput',
                                     [w.UINT,c.POINTER(Input),c.c_int],w.UINT))
            def mouse(flags,dx=0,dy=0):
                event = Input(type=0,data=InputUnion(mi=MouseInput(dx,dy,0,flags,0,0)))
                if send(1,c.byref(event),c.sizeof(event)) != 1:
                    raise OSError('Native caption focus input was not delivered')
            mouse(0xC001,round((point[0]-x)*65535/(width-1)),
                  round((point[1]-y)*65535/(height-1)))
            caption()
            if gui.GetCursorPos() != point:
                raise CaptureUnavailable('Pointer did not reach native title bar; no button pressed')
            try:mouse(2)
            finally:mouse(4)
        finally:
        ...
        if gui.GetForegroundWindow() != hwnd:
            raise CaptureUnavailable('Native title-bar click did not transfer foreground ownership')
        return True
