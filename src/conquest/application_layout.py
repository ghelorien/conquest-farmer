...
def application_root(explicit=None, *, verify=True):
    ...
    if configured:
        ...
        if module.parent.name != "conquest" or module.parent.parent.name != "src":
            raise ValueError(
                "Installed Conquest requires an explicit CONQUEST_APP_ROOT"
            )
    ...
    if pin or (root / "release-manifest.json").exists():
        ...
        if (
            ...
            or hashlib.sha256(manifest.read_bytes()).hexdigest() != pin
        ):
    ...
    def resolve(cls, explicit=None, *, verify=True):
        ...
        if immutable:
            if state_root is None:
                raise ValueError(
                    "Immutable release requires an absolute managed state root"
                )
            if (
                state_root == root
                or root in state_root.parents
                or state_root in root.parents
            ):
                raise ValueError(
                    "Managed state and immutable application roots must be separate"
                )
    ...
    def python(self, *, windowed=False):
        if self.immutable:
            name = (
                ...
                else ("Scripts/python.exe" if os.name == "nt" else "bin/python")
            )
