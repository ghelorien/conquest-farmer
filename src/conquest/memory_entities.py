...
class EntityLayout(BaseModel):
    ...
    monster_type_ids: tuple[Annotated[int, Field(strict=True, ge=1, le=65535)], ...] = (
        Field(default=(), max_length=1024)
    )
    id_offset: Offset
    kind_offset: Offset
    name_offset: Offset
...
def sample_fields(session, fields):
    ...
    if not hasattr(session, "request"):
        ...
        for address, kind in fields:
            if kind == "utf8":
                values.append(
                    reader(checked_address(address), 64)
                    .split(b"\0", 1)[0]
                ...
                values.append(
                    struct.unpack(
                        formats[kind],
                        reader(
                            checked_address(address), struct.calcsize(formats[kind])
                        ),
                    )
                )
                if kind != "xy_u32":
                ...
                raise ValueError("Unsupported direct entity sample kind")
        return values
    values = []
...
def record_values(session, actors, specs, *, packed=False):
    if not packed:
        return sample_fields(
            session, [(obj + offset, kind) for obj in actors for offset, kind in specs]
        )
    ...
    values = []
    for obj in actors:
        data = session.read_block(
            checked_address(obj + begin, end - begin), end - begin
        )
        if len(data) != end - begin:
            raise ValueError("Incomplete monster record")
        for offset, kind in specs:
            relative = offset - begin
            if kind == "utf8":
                value = (
                    data[relative : relative + 64]
                    .split(b"\0", 1)[0]
                    .decode("utf-8", errors="replace")
                )
            else:
...
class MemoryEntityReader:
    ...
    def _resolve(self):
        ...
        if any(
            rva + 8 > module["size"]
            for rva in (p.root_rva, p.collection_vtable_rva, p.monster_vtable_rva)
        ):
        ...
        if (
            sample_fields(s, [(address, "u64")])[0]
            != module["base"] + p.collection_vtable_rva
        ):
    ...
    def read(self, *, selected=None, packed=False):
        ...
        typed = [
            obj
            for obj, vtable in zip(inspected, vtables)
            if vtable == module + p.monster_vtable_rva
        ]
        ...
        if (
            ...
            or sample_fields(s, [(collection, "u64")])[0]
            != module + p.collection_vtable_rva
        ):
        ...
        for i, obj in enumerate(actors):
            uid, kind, name, position, draw_x, draw_y, max_hp, level = values[
                i * 8 : i * 8 + 8
            ]
        ...
        if selected is not None and (
            len(monsters) != 1 or monsters[0].entity_id != selected[0]
        ):
    ...
    def report(self):
        ...
        return {
            "schema_version": 1,
            "stage": "memory_entity_candidate_sample",
