...
class SessionKills:
    def __init__(self, output, *, clock=time.time):
        ...
        try:
            if self.path.exists():
                ...
                if (type(value.get('kills')) is not int or value['kills'] < 0
                        or type(value.get('cursor')) is not int or value['cursor'] < 0
    ...
    def _connect(self):
        return sqlite3.connect(self.database.resolve().as_uri() + '?mode=ro',
                               uri=True, timeout=.05)
    ...
    def begin(self):
        ...
        self.state = dict(started_at=self.clock(), stopped_at=None, cursor=cursor, kills=0)
        self._save()
    ...
    def refresh(self):
        ...
        try:
            ...
            if self.database.exists():
                with closing(self._connect()) as db:
                    db.execute('BEGIN')
                    end = db.execute('SELECT COALESCE(MAX(rowid),0) FROM events').fetchone()[0]
                    if end < self.state['cursor']:
                        raise ValueError('Kill log was truncated')
                    rows = db.execute("SELECT time,payload FROM events WHERE rowid>? AND rowid<=? "
                                      "AND event='kill_verified'", (self.state['cursor'], end))
                    increment = 0
                    for observed_at, payload in rows:
                        if observed_at < self.state['started_at']:
                            continue
                        if self.state['stopped_at'] is not None and observed_at > self.state['stopped_at']:
                            continue
            ...
            if changed or self.error:self._save()
            self.error = None
    ...
    def snapshot(self):
        ...
        return dict(kills=count, kills_per_hour=(None if self.error else
                    count * 3600 / elapsed if active and elapsed > 0 else 0),
                    kill_session_active=active, kill_session_started_at=state['started_at'] if state else None,
