...
import os
import time
from conquest.character_profiles import (
    ProfileRegistry,
    data_root,
    write_json,
    context_for,
...
def app_owner(root, timeout=15):
    ...
    with managed_root_owner(root, timeout=timeout):
        ...
        with _owner_file(
            root / "app.lock", timeout, "Conquest is already running on this PC"
        ):
            yield
...
def initialize(repo, argv):
    ...
    if args.data_root:
        os.environ["CONQUEST_DATA_ROOT"] = str(Path(args.data_root).resolve())
    destination = data_root()
    if (
        not destination.exists()
        and not args.migrate_from
        and (Path(repo) / "reports/merchants/journal.sqlite3").exists()
    ):
        ...
        dialog.withdraw()
        try:
            migrate = messagebox.askyesnocancel(
                "Import existing Conquest setup",
                ...
                "Choose No to create fresh paused profiles.",
                parent=dialog,
            )
        finally:
            dialog.destroy()
        if migrate is None:
            raise SystemExit(0)
        if migrate:
            args.migrate_from = str(Path(repo).resolve())
...
def select_profile(repo, args, destination):
    ...
    if not config.exists():
        ...
        base = yaml.safe_load(
            (Path(repo) / "profiles/desktop-foreground.example.yaml").read_text()
        )
...
def offline_edit_ready(root):
    ...
    readiness = ProfileReadiness(root)
    return all(
        readiness.transaction_idle(profile.id) for profile in registry.profiles()
    )
