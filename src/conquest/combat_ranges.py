...
def _read_combat_ranges(s, life, layout, *, require_scatter=True):
    ...
    if life.dead_candidate:
        raise ValueError("Living archer required for combat ranges")
    ...
    if (
        struct.unpack_from("<Q", bow)[0] != base + layout.item_vtable_rva
        or bow_type // 1000 != 500
        or not 1 <= bow_range <= 32
    ):
    ...
    if (
        not start <= end <= capacity
        or not 0 <= end - start <= capacity - start <= 128 * 16
        or (end - start) % 16
        or (capacity - start) % 16
        or (start == 0 and capacity != 0)
    ):
        raise ValueError("Learned skill vector is invalid")
    if start:
        checked_address(start, max(1, capacity - start))
    entries = (
        s.read_block(checked_address(start, end - start), end - start)
        if end > start
        else b""
    )
    matches = []
    for offset in range(0, len(entries), 16):
        ...
        if struct.unpack_from("<Q", raw)[0] != base + layout.skill_vtable_rva:
            raise ValueError("Learned skill object type changed")
        if struct.unpack_from("<I", raw, 0x10)[0] != 8001:
            continue
        ...
        matches.append(
            {"type_id": 8001, "level": level, "range": radius, "distance": distance}
        )
    if len(matches) > 1 or (require_scatter and not matches):
        raise ValueError("Exactly one learned Scatter is required")
    fresh_bow = s.read_block(bow_address, 0x74)
    if (
        fresh_bow[:0x14] != bow[:0x14]
        or fresh_bow[0x70:0x74] != bow[0x70:0x74]
    ...
    s.assert_identity()
    return {
        "bow": {"type_id": bow_type, "range": bow_range},
        "scatter": matches[0] if matches else None,
...
def route_combat_settings(route, ranges):
    ...
    return {
        "attack_button": "right" if scatter else "left",
        "adaptive_scatter": bool(scatter),
        "single_isolated_targets": True,
        "single_attack_range_tiles": min(
            route.attack_range_tiles, ranges["bow"]["range"]
        ),
        "attack_range_tiles": min(
            route.attack_range_tiles, (scatter or ranges["bow"])["range"]
        ),
