...
class SceneProjection:
    ...
    def point(self, source, destination, max_delta=1):
        ...
        point = (
            self.anchor[0] + (dx - dy) * self.half_tile[0],
    ...
    def __init__(
        ...
        *,
        ...
        if allowed_bounds is not None:
            if (
                len(allowed_bounds) != 4
                or any(type(v) is not int for v in allowed_bounds)
                or allowed_bounds[0] > allowed_bounds[2]
                or allowed_bounds[1] > allowed_bounds[3]
            ):
    ...
    def _in_bounds(self, position):
        if self.allowed_bounds is not None:
            ...
            if not (x0 <= position[0] <= x1 and y0 <= position[1] <= y1):
                raise ValueError(
                    "Player or waypoint is outside the permitted map bounds"
                )

    def _valid(self, reading):
        window = reading.window
        if not 0 <= self.clock() - reading.observed_at <= 0.5:
            raise ValueError("Scene observation expired")
        if window["minimized"] or (
            self.require_unfocused
            and window["foreground"] == window.get("root_hwnd", window["hwnd"])
        ):
        ...
        if reading.hp is not None and (
            reading.max_hp is None
            or reading.max_hp <= 0
            or not 0 < reading.hp <= reading.max_hp
            or reading.hp / reading.max_hp < 0.4
        ):
        ...
        try:
            ...
            for _ in range(25):
                ...
                report["observations"].append(
                    {
                        "position": list(reading.position),
                        "window": reading.window,
                ...
                changes = {
                    key: {
                        "before": before.window.get(key),
                        "after": reading.window.get(key),
                    }
                ...
                if not (
                    min(source[0], destination[0])
                    <= reading.position[0]
                    <= max(source[0], destination[0])
                    and min(source[1], destination[1])
                    <= reading.position[1]
                    <= max(source[1], destination[1])
                ):
                    raise ValueError(
                        "Player moved to an unexpected tile outside the waypoint segment"
                    )
...
class BridgeSceneStepper:
    ...
    def __init__(
        ...
        *,
        ...
        self.stepper = BackgroundSceneStepper(
            self.observe,
            self._click,
            expected_map=expected_map,
            allowed_bounds=allowed_bounds,
            max_delta=max_delta,
            require_unfocused=require_unfocused,
        )
    ...
    def observe(self):
        ...
        position = tuple(
            struct.unpack("<II", self.session.read_block(addresses["position"], 8))
        )
        ...
        if (
            struct.unpack("<I", self.session.read_block(addresses["map"], 4))[0]
            != map_id
        ):
    ...
    def _click(self, point, viewport):
        return self.session.request(
            "background-click",
            {
                "health_profile": self.health.model_dump(mode="json"),
                "character": self.character,
                "point": list(point),
                "expected_size": list(viewport),
...
def memory_player_anchor(observer, life, *, layout=None):
    ...
    if (
        layout is not None
        and observer.adapter.expected_sha256 != layout.expected_sha256
    ):
    ...
    if position != tuple(life.position) or not (
...
def memory_player_anchor_for_session(session, character):
    ...
    if provider is not None:
        adapter = SimpleNamespace(
            expected_sha256=adapter.expected_sha256,
            modules=adapter.modules,
            identity=adapter.identity,
            read=adapter.read,
            read_block=adapter.read_block,
            assert_identity=adapter.assert_identity,
            viewport_size=provider,
        )
    anchor = memory_player_anchor(
        SimpleNamespace(adapter=adapter), life, layout=read_build_layout(reader.session)
    )
    latest = MemoryLifeReader.for_session(session, character).read()
    if (
        latest.object_address != life.object_address
        or latest.position != life.position
...
def clear_route_point(point, bounds=(80, 140, 956, 667)):
    ...
    return (
        left < x < right
        and top < y < bottom
        and clear_scene(point, (right + 80, bottom + 126))
    )
...
def visible_route_delta(delta, anchor, bounds=(80, 140, 956, 667)):
    ...
    for steps in range(distance, 0, -1):
        ...
        if clear_route_point((px, py), bounds):
            return x, y
    return None
