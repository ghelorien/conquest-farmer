...
def resolve_player(session, layout: PlayerLayout):
    ...
    modules = [module for module in session.modules if module["name"].casefold() == layout.module.casefold()]
    if len(modules) != 1:
    ...
    session.assert_identity()
    result = {"object": address, "name": checked_address(address + layout.name_offset, 64),
            "max_hp": checked_address(address + layout.max_hp_offset, 4),
            "position": checked_address(address + layout.position_offset, 8)}
    for name, offset in (("kill_counter", layout.kill_counter_offset), ("level", layout.level_offset)):
...
class WorkerPointerSession:
    ...
    def __init__(self, info_path, expected_sha256):
        ...
        if health.get("protocol_version") != 2 or health.get("expected_sha256") != expected_sha256:
            raise ValueError("Worker protocol or pinned executable fingerprint differs from player layout")
    ...
    def read(self, address, size):
        ...
        sample = self.request("sample", {"fields": [{"name": "pointer", "address": hex(address), "kind": "u64"}]})
...
def sample_player(info_path, layout):
    ...
    sample = session.request("sample", {"fields": [
        ...
        for name, kind in (("name", "utf8"), ("max_hp", "u32"), ("position", "xy_u32"))]})
    if resolve_player(session, layout) != addresses:
        raise ValueError("Player object changed during sampling")
    return {"schema_version": 1, "stage": "player_candidate_sample", "process_identity": session.identity,
            "addresses": {key: hex(value) for key, value in addresses.items()}, "sample": sample,
            "qualified": False, "autonomous_actions_enabled": False}
