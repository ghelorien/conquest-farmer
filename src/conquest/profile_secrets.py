...
def encrypt(path,plain,label):
    ...
    temporary.write_bytes(win32crypt.CryptProtectData(plain,label,None,None,None,0))
...
def save_login(context,username,password):
    ...
    encrypt(context.credentials,json.dumps({'username':username,'password':password}).encode(),'Conquest account')


def save_notification_webhook(context,value):
    url=urlsplit(value.strip())
    if (url.scheme!='https' or url.netloc not in ('discord.com','discordapp.com')
            or not re.fullmatch(r'/api/(?:v\d+/)?webhooks/\d+/[A-Za-z0-9_-]+',url.path)):
        raise ValueError('Enter a valid Discord webhook URL')
    path=(context.root/'machine-state/.runtime/merchants/shops-webhook.dpapi' if context.profile.role=='Merchant'
