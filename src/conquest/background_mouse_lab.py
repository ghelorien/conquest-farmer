...
def model_frame(queue, position=INVALID_POSITION):
    ...
    for event in queue:
        ...
        used += 1
    return {'position': list(position), 'processed': used,
            'remaining': queue[used:], 'inserted_characters': inserted}
...
def _child(pairs, alternate=False, capture=False):
    ...
    hwnd = None
    registered = False
    baseline = {'foreground': win32gui.GetForegroundWindow(), 'cursor': list(win32api.GetCursorPos())}
    owner_pid = w.DWORD()
    foreground_thread = user.GetWindowThreadProcessId(baseline['foreground'], ctypes.byref(owner_pid))
    ...
    def callback(window, message, wp, lp):
        ...
        try:
            if capture and message == win32con.WM_LBUTTONDOWN:
                ...
                held = True
                native.append({'message': 'WM_LBUTTONDOWN', 'capture_before': before,
            ...
            if capture and message == win32con.WM_LBUTTONUP:
                ...
                native.append({'message': 'WM_LBUTTONUP', 'capture_after': win32gui.GetCapture()})
                return 0
            if message == win32con.WM_MOUSEMOVE:
                point = [ctypes.c_short(lp & 0xffff).value, ctypes.c_short((lp >> 16) & 0xffff).value]
            ...
            if message == win32con.WM_MOUSELEAVE:
                ...
                queue.append({'type': 'mouse_position', 'position': list(INVALID_POSITION)})
                return 0
    ...
    try:
        ...
        registered = True
        hwnd = win32gui.CreateWindowEx(0, class_name, 'Disposable mouse dummy',
            win32con.WS_POPUP, -32000, -32000, 200, 200, 0, 0, cls.hInstance, None)
        if win32gui.IsWindowVisible(hwnd) or win32gui.GetParent(hwnd) or win32gui.GetWindow(hwnd, win32con.GW_OWNER):
        ...
        while time.monotonic() < deadline:
            ...
            current = {'foreground': win32gui.GetForegroundWindow(), 'cursor': list(win32api.GetCursorPos())}
            samples.append(current)
            if current != baseline:
                raise RuntimeError('Desktop focus or cursor changed during dummy probe')
            if callbacks_failed:
                raise RuntimeError('Dummy callback failed: ' + ', '.join(callbacks_failed))
            if queue:
        ...
        if capture:
            def capture_observation():
                ...
                if not user.GetGUIThreadInfo(foreground_thread, ctypes.byref(info)):
                    raise ctypes.WinError(ctypes.get_last_error())
                desktop = {'foreground': win32gui.GetForegroundWindow(), 'cursor': list(win32api.GetCursorPos())}
                samples.append(desktop)
                if desktop != baseline:
                    raise RuntimeError('Desktop changed during dummy capture probe')
                return {'foreground_capture': int(info.hwndCapture or 0),
                        'foreground_focus': int(info.hwndFocus or 0),
                        'dummy_capture': win32gui.GetCapture(), 'dummy_button_held': held}
            ...
            capture_result = {'before': before, 'down': down, 'during': during, 'after': after,
                'foreground_capture_unchanged': all(row['foreground_capture'] == before['foreground_capture']
                    for row in (down, during, after)),
                'foreground_focus_unchanged': all(row['foreground_focus'] == before['foreground_focus']
                    for row in (down, during, after)),
                'dummy_capture_cleared': after['dummy_capture'] == 0 and not after['dummy_button_held'],
        ...
        return {'schema_version': 1, 'dummy_only': True, 'gameplay_qualified': False,
                'pairs': pairs, 'alternate_adjacent_positions': alternate,
                'native_messages': native, 'modeled_frames': frames,
                'generated_leave_observed': any(row['message'] == 'WM_MOUSELEAVE' for row in native),
                'modeled_requested_position_frames': sum(frame['position'] in ([60, 80], [61, 80]) for frame in frames),
                'inserted_characters': inserted, 'queue_drained': not queue,
                'desktop_unchanged': all(row == baseline for row in samples),
                'desktop_samples': len(samples), 'callback_errors': callbacks_failed,
...
def run_lab(*, pairs=12, alternate=False, capture=False):
    ...
    completed = subprocess.run([sys.executable, '-m', 'conquest.background_mouse_lab', '--child', str(pairs), str(int(alternate)), str(int(capture))],
...
if __name__ == '__main__':
    if len(sys.argv) == 5 and sys.argv[1] == '--child' and all(value in ('0', '1') for value in sys.argv[3:]):
        print(json.dumps(_child(int(sys.argv[2]), bool(int(sys.argv[3])), bool(int(sys.argv[4])))))
    elif len(sys.argv) == 1:
