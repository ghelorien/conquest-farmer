...
def read_definition(session, type_id):
    ...
    vtable, initialized, _, sentinel, count, buckets, end, capacity, mask, size = (
        struct.unpack("<10Q", header)
    )
    if (
        vtable != base + 0x5CF590
        or initialized != 1
        or not 1 <= count <= 100000
        or not 16 <= size <= 131072
        or size & (size - 1)
        or mask != size - 1
        or end - buckets != size * 16
        or capacity != end
    ):
    ...
    while node != sentinel:
        ...
        if key == type_id:
            ...
            if (
                ...
                or not 1 <= length <= 128
                or not max(15, length) <= allocated <= 4096
            ):
                raise ValueError("Invalid item definition identity/name")
            name_bytes = (
                raw[0x20 : 0x20 + length]
                if allocated <= 15
            ...
            if (
                allocated > 15
                and session.read_block(struct.unpack_from("<Q", raw, 0x20)[0], length)
                != name_bytes
            ):
    ...
    if (
        session.read_block(address, 0x50) != header
        or session.read_block(bucket, 16) != bounds
