...
def require_viewport(width, height, minimum=(1036, 793)):
    if width < minimum[0] or height < minimum[1]:
        raise ViewportTooSmall(
            f"Game pane is {int(width)}×{int(height)}; the supported viewport needs {minimum[0]}×{minimum[1]}. Enlarge the app, hide farmer controls, or use a separate game window."
        )
...
class AttachmentStatus:
    ...
    def enter(self, stage, **evidence):
        if stage not in STAGES:
            raise ValueError("Unknown attachment stage")
        self.stage = stage
        self.error = None
    ...
    def fail(self, error):
        ...
        messages = {
            "discovery": "Select a running client.",
            "identity": "Verify the character, server and client build.",
        ...
        self.error = {
            ...
            "message": str(error)
            if isinstance(error, (ViewportTooSmall, UnsupportedClientBuildError))
            else messages[self.stage],
        }
        import traceback
...
def find_installation(executable):
    ...
    if not path.is_file() or path.name.casefold() != "imconquer.exe":
        raise ValueError("Select a verified ImConquer executable")
    for parent in path.parents:
        if (parent / "ini").is_dir() and (parent / "map").is_dir():
            return parent
    raise ValueError(
        "Game map and ini directories were not found beside the client installation"
    )
...
def remember_installation(context, executable):
    ...
    if context:
        path = context.root / "machine.json"
...
def fit_geometry(width, height, x, y, work_areas):
    ...
    if not work_areas:
        raise ValueError("No monitor work area is available")
    area = max(
        work_areas,
        key=lambda a: (
            max(0, min(x + width, a[2]) - max(x, a[0]))
            * max(0, min(y + height, a[3]) - max(y, a[1]))
        ),
    )
    left, top, right, bottom = area
    width = min(max(width, 640), right - left)
    height = min(max(height, 480), bottom - top)
    return (
        width,
        height,
