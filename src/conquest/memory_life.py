...
def read_life(session, health_layout, character, *, clock=time.monotonic):
    ...
    return MemoryLifeReader(
        session, health_layout, player, character, clock=clock
    ).read()
...
class MemoryLifeReader:
    def __init__(
        ...
        *,
        ...
        if layout is None and any(
            fingerprint != CLIENT_SHA256
            for fingerprint in (
                session.expected_sha256,
                health_layout.player.expected_sha256,
                player_layout.expected_sha256,
            )
        ):
            raise ValueError(
        ...
        if layout is not None and any(
            fingerprint != layout.expected_sha256
            for fingerprint in (
                session.expected_sha256,
                health_layout.player.expected_sha256,
                player_layout.expected_sha256,
        ...
        self.session, self.health_layout, self.player_layout = (
            session,
            health_layout,
            player_layout,
        )
    ...
    def for_session(cls, session, character, *, clock=time.monotonic):
        ...
        health = health_reader_layout(session)
    ...
    def read(self):
        ...
        if actual["position"] != player["position"]:
            raise ValueError(
                "Player and health profiles resolve different position fields"
            )
        build = self.layout
        status_offset = build.life_status_offset if build is not None else STATUS_OFFSET
        appearance_offset = (
            build.life_appearance_offset if build is not None else APPEARANCE_OFFSET
        )
        revive_offset = (
            build.life_revive_gate_offset if build is not None else REVIVE_GATE_OFFSET
        )
        ...
        if build and build.conservative_life_block:
            ...
            try:
                ...
                revive_supported = False
        fields = (
            (actual["object"] + status_offset, 8),
        ...
        if (
            resolve_player(session, self.health_layout.player) != actual
        ...
        status = (
            struct.unpack_from("<I", before[0])[0]
            if build and build.conservative_life_block
