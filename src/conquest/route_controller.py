...
def ensure_running(route_id, *, root=None, fresh_start=False):
    ...
    if locks:
        ...
        if not matching_scroll_recovery(
            locks, path=root / state_path("reports/banking/merchant-journey.json")
        ):
            return False
    if read_json(root / state_path(".runtime/storage-halt.json")).get("active"):
        return False
    now = time.time()
    ...
    if status.get("phase") == "needs_attention" and not fresh_start:
        return False
    if (
        status.get("phase") in ACTIVE
        and 0 <= now - status.get("updated_at", 0) < 15
    ...
    with (root / state_path(".runtime/route-controller-error.log")).open("ab") as error:
        process = subprocess.Popen(
            [str(python), "-B", str(script), "--route", route_id],
            cwd=root,
    ...
    write_json(
        launch,
        {"time": now, "pid": process.pid, "identity": identity, "route": route_id},
    )
    return True
