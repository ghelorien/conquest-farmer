...
def ensure_running(route_id,*,root=None):
    ...
    if read_json(root/'.runtime/storage-halt.json').get('active'):return False
    now=time.time()
    status=read_json(root/'reports/overnight/status.json')
    if (status.get('phase') in ACTIVE and 0<=now-status.get('updated_at',0)<15
            and process_alive(status.get('pid')) is not False):
        return False
    launch=root/'.runtime/route-controller-launch.json'
    ...
    (root/'.runtime/overnight.stop').unlink(missing_ok=True)
    launch.parent.mkdir(parents=True,exist_ok=True)
    python=Path(sys.executable).with_name('pythonw.exe')
    if not python.exists():python=Path(sys.executable)
    with (root/'.runtime/route-controller-error.log').open('ab') as error:
...
@contextmanager
def controller_guard(path=Path('.runtime/route-controller.lock')):
    import msvcrt
