...
def catalog():
    ...
    for source in (seed, observed):
        for city, vendors in (
            source.get("cities", {}).items()
            if isinstance(source.get("cities"), dict)
            else ()
        ):
            if isinstance(vendors, dict):
                cities[str(city)] = {**cities.get(str(city), {}), **vendors}
...
def record(map_id, vendor, products, *, observed_at):
    ...
    if not isinstance(vendors, dict):
        vendors = {}
        cities[str(map_id)] = vendors
    vendors[str(vendor)] = {"observed_at": observed_at, "products": products}
