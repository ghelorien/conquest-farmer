...
def profile_choice_rows(registry):
    ...
    return [
        (profile.id, (profile.label or profile.name, profile.server, profile.role))
...
def new_character_role(value):
    ...
    if role not in ROLES:
        raise ValueError("Role must be Farmer or Merchant")
    return role
...
def toggle_advanced_section(section, button):
    if section.winfo_manager():
        section.pack_forget()
        button.configure(text="Show character settings")
    else:
        section.pack(fill="x", pady=(6, 0))
        button.configure(text="Hide character settings")
...
def manage_profiles(registry, selected=None):
    ...
    root = tk.Tk()
    root.title("Conquest — characters on this PC")
    root.geometry("900x670")
    root.minsize(640, 480)
    from conquest.sidebar import ScrollableSidebar

    scroll = ScrollableSidebar(root)
    scroll.pack(fill="both", expand=True)
    frame = scroll.content
    ttk.Label(
        frame, text="Choose a character to open", font=("Segoe UI", 18, "bold")
    ).pack(anchor="w")
    chooser_note = tk.StringVar(value="Select a character below, then open it.")
    ttk.Label(frame, textvariable=chooser_note, wraplength=780).pack(
        fill="x", pady=(2, 8)
    )
    tree = ttk.Treeview(
        frame, columns=("name", "server", "role"), show="headings", height=7
    )
    for key in ("name", "server", "role"):
        tree.heading(key, text=key.title())
        tree.column(key, width=170)
    tree.pack(fill="x")
    result = [None]
    quick_actions = ttk.Frame(frame)
    quick_actions.pack(fill="x", pady=(8, 4))
    open_button = ttk.Button(quick_actions, text=OPEN_SELECTED_CHARACTER)
    open_button.pack(fill="x")
    quick_secondary = ttk.Frame(quick_actions)
    quick_secondary.pack(fill="x", pady=(4, 0))
    ttk.Button(
        quick_secondary, text="Add character", command=lambda: guarded(add)
    ).pack(side="left", expand=True, fill="x", padx=(0, 2))
    ttk.Button(
        quick_secondary,
        text="Import settings",
        command=lambda: guarded(import_settings),
    ).pack(side="left", expand=True, fill="x", padx=2)
    advanced_button = ttk.Button(
        quick_secondary,
        text="Show character settings",
        command=lambda: toggle_advanced_section(advanced, advanced_button),
    )
    advanced_button.pack(side="left", expand=True, fill="x", padx=(2, 0))

    advanced = ttk.LabelFrame(
        frame, text="Character settings and advanced options", padding=8
    )
    label = tk.StringVar()
    role = tk.StringVar(value="Farmer")
    enabled = tk.BooleanVar(value=True)
    ttk.Label(advanced, text="Select a character above to edit its settings.").pack(
        anchor="w", pady=(0, 6)
    )
    line = ttk.Frame(advanced)
    line.pack(fill="x", pady=6)
    ttk.Label(line, text="Tab label").pack(side="left")
    ttk.Entry(line, textvariable=label, width=28).pack(side="left", padx=8)
    ttk.Combobox(
        line,
        textvariable=role,
        values=("Farmer", "Merchant"),
        state="readonly",
        width=12,
    ).pack(side="left")
    ttk.Checkbutton(line, text="Enabled on this PC", variable=enabled).pack(
        side="left", padx=8
    )
    ttk.Label(
        advanced,
        text="Overrides (JSON) — {} keeps automatic defaults. Allowed: "
        + ", ".join(SETTING_TYPES),
        wraplength=800,
    ).pack(fill="x")
    settings = tk.Text(advanced, height=5, wrap="word")
    settings.pack(fill="x", pady=4)
    ttk.Label(
        advanced,
        text="Trusted delivery sources (JSON): name, server, verified character_uid.",
        wraplength=800,
    ).pack(fill="x")
    trust = tk.Text(advanced, height=3, wrap="word")
    trust.pack(fill="x", pady=4)
    ttk.Label(
        advanced,
        text="Allowed manual visitors (exact profile-local permissions)",
        wraplength=800,
    ).pack(fill="x", pady=(6, 2))
    visitors = ttk.Treeview(
        advanced,
        columns=("name", "server", "uid"),
        show="headings",
        height=3,
        selectmode="extended",
    )
    for key, title, width in (
        ("name", "Visitor", 240),
        ("server", "Server", 160),
        ("uid", "Verified UID", 160),
    ):
        visitors.heading(key, text=title)
        visitors.column(key, width=width)
    ...
    def load(event=None):
        ...
        label.set(p.label)
        role.set(p.role)
        enabled.set(p.local_enabled)
        chooser_note.set(
            f"Selected: {p.label or p.name}. Choose “{OPEN_SELECTED_CHARACTER}” when ready."
        )
        settings.delete("1.0", "end")
        settings.insert("1.0", json.dumps(registry.effective(p), indent=2))
        trust.delete("1.0", "end")
        trust.insert("1.0", json.dumps(p.trusted_sources, indent=2))
        visitors.delete(*visitors.get_children())
        visitor_rows.clear()
        for index, row in enumerate(registry.list_visitors(p.id)):
            ...
            visitors.insert(
                ...
                "end",
                iid=key,
                values=(row["visitor_name"], row["visitor_server"], row["visitor_uid"]),
            )
            visitor_rows[key] = {
                name: row[name]
                for name in (
    ...
    def refresh(profile_id=None):
        ...
        for profile_id_value, values in rows:
            tree.insert("", "end", iid=profile_id_value, values=values)
        ...
        if target:
            ...
            chooser_note.set(
                "No characters are saved on this PC. Add or import a character to continue."
            )
    ...
    def guarded(action):
        try:
            ...
            messagebox.showerror("Character settings", str(error), parent=root)

    def add():
        name = simpledialog.askstring(
            "New character", "Exact in-game name:", parent=root
        )
        if not name:
            return
        ...
        if not server:
            return
        requested_role = simpledialog.askstring(
            "New character role",
            "Role for this character: Farmer or Merchant",
            initialvalue="Farmer",
            parent=root,
        )
    ...
    def save():
        ...
        registry.update(
            p.id,
            {
                "label": label.get(),
                "role": role.get(),
                "local_enabled": enabled.get(),
                "template": "automatic",
                "overrides": json.loads(settings.get("1.0", "end")),
                "trusted_sources": json.loads(trust.get("1.0", "end")),
            },
        )
        refresh(p.id)
        note.set("Saved. No running behavior was changed.")

    def revoke_visitors():
        p = chosen()
        selected = list(visitors.selection())
        exact = [dict(visitor_rows[key]) for key in selected]
        if not exact:
            raise ValueError(
                "Select one or more exact manual visitor permissions to revoke"
            )
        registry.revoke_visitors(p.id, exact, operator="profile UI")
        load()
        note.set(
            "Selected exact manual visitor permission(s) revoked. Trusted delivery sources were unchanged."
        )

    def export():
        p = chosen()
        path = filedialog.asksaveasfilename(
            parent=root, defaultextension=".json", filetypes=[("Preferences", "*.json")]
        )
        if path:
            write_json(path, registry.export(p.id))
            note.set(
                "Exported saved preferences only; credentials and trust identities excluded."
            )

    def import_settings():
        path = filedialog.askopenfilename(
            parent=root, filetypes=[("Preferences", "*.json")]
        )
        if not path:
            return
        name = simpledialog.askstring(
            "Import preferences",
            "Exact in-game name for the new character:",
            parent=root,
        )
        if not name:
            return
        server = simpledialog.askstring(
            "Server", "Server:", initialvalue="America", parent=root
        )
        if server:
            refresh(
                registry.import_preferences(
                    json.loads(Path(path).read_text()), name, server
                ).id
            )
    ...
    def installation():
        ...
        path = filedialog.askopenfilename(
            parent=root,
            title="Select this PC’s ImConquer.exe",
            filetypes=[("Game client", "ImConquer.exe")],
        )
        if path:
    ...
    def start():
        ...
        if not p.local_enabled:
            raise ValueError("Choose an enabled profile")
        result[0] = p.id
        root.destroy()
    ...
    def save_template():
        ...
        title = simpledialog.askstring(
            "Settings template", "Template name:", parent=root
        )
        if not title:
            return
    ...
    def login():
        ...
        username = simpledialog.askstring(
            "Local account", "Account login (stored encrypted on this PC):", parent=root
        )
        if username is None:
            return
        password = simpledialog.askstring(
            "Local account", "Password:", show="*", parent=root
        )
        if password is None:
            return
        ...
        note.set(
            "Login saved encrypted for the selected profile. It is never exported."
        )
    ...
    def notification():
        ...
        value = simpledialog.askstring(
            "Discord destination",
            "Webhook URL (merchants share the shops channel on this PC):",
            show="*",
            parent=root,
        )
        if value:
    ...
    tree.bind(
        "<Double-1>",
        lambda event: open_on_profile_double_click(event, tree, lambda: guarded(start)),
    )
    ...
    for index, (text, action) in enumerate(
        [
            ("Save changes", save),
            ("Revoke selected manual visitor", revoke_visitors),
            ("Export settings", export),
            ("Game installation", installation),
            ("Save as template", save_template),
            ("Account login", login),
            ("Discord destination", notification),
        ]
    ):
        ttk.Button(row, text=text, command=lambda f=action: guarded(f)).grid(
        ...
        def configure_labels(parent):
            for widget in parent.winfo_children():
                if (
                    ...
                    and int(widget.cget("wraplength") or 0) != width
                ):
