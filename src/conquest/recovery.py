...
class RecoveryConfig(BaseModel):
    ...
    def validate_calibration(self):
        if bool(self.revive_template) != (self.revive_region is not None):
            raise ValueError(
                "Revive template and its observed region must be provided together"
            )
        if self.revive_region:
            l, t, r, b = self.revive_region
            if not 0 <= l < r <= 1584 or not 0 <= t < b <= 861:
                raise ValueError(
                    "Revive search region is outside the calibrated client"
                )
...
def revive_button(frame, template, region):
    ...
    x, y = point
    scores[
        max(0, y - h // 2) : y + h // 2 + 1, max(0, x - w // 2) : x + w // 2 + 1
    ] = -1
...
class DeathRecovery:
    def __init__(self, config, death_position, started, boundary):
        self.config, self.death_position, self.started, self.boundary = (
            config,
            death_position,
            started,
            boundary,
    ...
    def decide(self, *, health, position, map_id, timestamp, now, button=None):
        ...
        if self.phase in (RecoveryPhase.WAITING, RecoveryPhase.REVIVING):
            if health > 0:
                ...
                if (
                    math.dist(position, self.death_position) <= 3
                    and l <= position[0] <= r
                    and t <= position[1] <= b
                ):
                    ...
                    return None
                if (
                    math.dist(position, self.config.town_position)
                    > self.config.town_tolerance
                ):
                ...
                return RecoveryAction("revive", button, now + 0.3)
        if health <= 0:
        ...
        if self.moving:
            ...
            self.movement_failures = (
                self.movement_failures + 1 if math.dist(before, position) < 0.5 else 0
            )
            self.moving = None
            if self.movement_failures >= 3:
                return self.fail("return_movement_failure_limit")
        while (
            self.waypoint < len(self.config.return_route)
            and math.dist(position, self.config.return_route[self.waypoint]) <= 2
        ):
            self.waypoint += 1
        if self.waypoint == len(self.config.return_route):
            ...
            return None
        dx, dy = (
            v - p for v, p in zip(self.config.return_route[self.waypoint], position)
        )
        scale = min(1, 6 / max(abs(dx), abs(dy), 1))
        dx, dy = dx * scale, dy * scale
        if self.movement_failures == 1:
            dx += 1
            dy -= 1
        if self.movement_failures == 2:
            dx -= 1
            dy += 1
        scale = min(
            1, 280 / max(abs((dx - dy) * 32), 1), 110 / max(abs((dx + dy) * 16), 1)
        )
        point = (
            round(792 + (dx - dy) * scale * 32),
            round(432 + (dx + dy) * scale * 16),
        )
        if not (100 < point[0] < 1100 and 170 < point[1] < 550):
            return self.fail("return_point_obscured")
        self.moving = (position, now)
        return RecoveryAction("return_walk", point, now + 0.3)
