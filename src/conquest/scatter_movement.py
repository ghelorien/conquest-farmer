...
def clear_jump(terrain, source, destination):
    ...
    previous = source
    if not terrain.walkable(source) or not steps:
        return False
    for i in range(1, steps + 1):
        point = (round(source[0] + dx * i / steps), round(source[1] + dy * i / steps))
        if not terrain.walkable(point):
            return False
        if point[0] != previous[0] and point[1] != previous[1]:
            if not terrain.walkable((point[0], previous[1])) or not terrain.walkable(
                (previous[0], point[1])
            ):
                return False
        previous = point
...
def remember_scatter(supervisor, targets):
    supervisor.last_scatter_group = (
        time.monotonic(),
        {
            (t.entity_id, t.object_address): t.current_hp
            for t in targets
            if getattr(t, "entity_id", None)
            and getattr(t, "object_address", None)
...
def wounded_group_in_range(supervisor, targets, position, radius):
    ...
    if not 0 <= time.monotonic() - stamp <= 3:
        return False
...
def scatter_landing(
    ...
    *,
    ...
    if hunting_boundary is not None and getattr(
        getattr(supervisor, "combat_speed", None), "cross_region_scatter", False
    ):
    ...
    live = [
        ...
        and type(t.current_hp) is int
        and t.current_hp > 0
        and left <= t.world_position[0] <= right
        and top <= t.world_position[1] <= bottom
    ]
    if not live:
        return None
    groups = []
    local_count = sum(
        max(abs(a - b) for a, b in zip(p, position)) <= radius for p in live
    )
    if (
        getattr(getattr(supervisor, "combat_speed", None), "cluster_lookahead", False)
        and local_count <= 2
    ):
        ...
        centers = sorted(
            set(live), key=lambda p: max(abs(a - b) for a, b in zip(p, position))
        )[:64]
        for center in centers:
            separation = max(abs(a - b) for a, b in zip(center, position))
            if not radius < separation <= 48:
                continue
            count = sum(
                max(abs(a - b) for a, b in zip(p, center)) <= max(2, radius - 2)
                for p in live
            )
            ...
            if count >= max(4, local_count + 3) and clear_jump(
                terrain, position, center
            ):
                groups.append((center, count, separation))
    recent = getattr(supervisor, "scatter_landings", [])
    now = time.monotonic()
    recent = [(p, at) for p, at in recent if now - at < 8]
    blocked = {
        p
        for (world, p), until in getattr(
    ...
    for dx in range(-12, 13):
        for dy in range(-12, 13):
            ...
            if not clear_scene((px, py), viewport):
                continue
            if any(
                abs(px - t.x) <= 24 and -40 <= py - t.y <= 10
                for t in observed
                if hasattr(t, "x") and hasattr(t, "y")
            ):
                continue
            steps = max(abs(dx), abs(dy)) * 4
            if (blocked or not fast) and any(
                (round(x + dx * i / steps), round(y + dy * i / steps)) in blocked
                for i in range(1, steps + 1)
            ):
                continue
            count = sum(
                max(abs(p[0] - point[0]), abs(p[1] - point[1])) <= radius for p in live
            )
            ...
            if (count < minimum_count and future < max(2, minimum_count)) or (
            ...
            centrality = -sum(
                max(abs(p[0] - point[0]), abs(p[1] - point[1]))
                for p in live
    ...
    if fast:
        ...
        winner = next(
            (
                ...
                if clear_jump(terrain, position, row[1])
            ),
            None,
        )
        if winner is None:
    ...
    landing_contact = sum(
        max(abs(p[0] - destination[0]), abs(p[1] - destination[1])) <= 3 for p in live
    )
    landing_close = sum(
