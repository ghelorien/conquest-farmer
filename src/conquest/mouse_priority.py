...
def desktop_access_denied(error):
    ...
    return getattr(error, "winerror", None) == 5 or (
        getattr(error, "winerror", None) is None and error.args and error.args[0] == 5
    )
    ...
    def active(self):
        with self.lock:
            try:
                ...
                if not desktop_access_denied(error):
                    raise
    ...
    if _guard is None:
        ...
        def sample():
            ...
            try:
                ...
                buttons = sum(
                    bit
                    for key, bit in ((1, 1), (2, 2), (4, 4), (5, 8), (6, 16))
    ...
    def wrapped(count, pointer, size):
        ...
        for index in range(count):
            ...
            if event.type == 0:
                ...
                release = (
                    release
                    and bool(flags & (4 | 16 | 64))
                    and not bool(flags & (1 | 2 | 8 | 32))
                )
            else:
                key = (
                    event.data.ki.wVk,
                    event.data.ki.wScan,
                    event.data.ki.dwFlags & ~2,
                )
        ...
        with nullcontext() if release else input_scope():
            if _guard is None:
                return send(count, pointer, size)
            with _guard.lock:
                if release and not (
                    up & _guard.owned_buttons or key_up & _guard.owned_keys
                ):
                    return (
                        count  # Never release a physical button/key we did not press.
                    )
                result = _guard.send(
                    lambda: send(count, pointer, size),
                    release=release,
                    moving=moving,
                    down=down,
                    up=up,
