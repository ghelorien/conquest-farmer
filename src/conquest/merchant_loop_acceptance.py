...
def state():
    ...
    with closing(
        sqlite3.connect(STATE.resolve().as_uri() + "?mode=ro", uri=True)
    ) as db:
...
def update(event, change):
    ...
    with closing(sqlite3.connect(STATE, timeout=5)) as db, db:
        ...
        db.execute(
            "CREATE TABLE IF NOT EXISTS events(id INTEGER PRIMARY KEY, at REAL, event TEXT, value TEXT)"
        )
        db.execute("BEGIN IMMEDIATE")
        row = db.execute("SELECT value FROM state WHERE id=1").fetchone()
        old = (
            json.loads(row[0]) if row else {"enabled": False, "phase": "not_configured"}
        )
        ...
        db.execute(
            "INSERT INTO events(at,event,value) VALUES(?,?,?)",
            (time.time(), event, encoded),
        )
...
def source_checked(source, saved=None):
    now = time.time()
    if (
        source.get("character") != farmer_name()
        or source.get("server") != "America"
        or type(source.get("character_uid")) is not int
        or source["character_uid"] <= 0
        or type(source.get("timestamp")) not in (int, float)
        or not 0 <= now - source["timestamp"] <= 5
        or source.get("hp", 0) <= 0
        or source.get("trade")
        or source.get("request")
    ):
    ...
    if saved and (
        saved["farmer_profile_id"] != profile_id()
        or any(
            source.get(key) != saved["farmer"].get(key)
...
def deliverable_item(item):
    ...
    return (
        eligible(item)
        and item.get("type_id") not in DRAGONBALL_TYPES
...
def merchant_live(name, status, snapshot, *, require_capacity=True):
    ...
    return bool(
        ...
        and status.get("refill", {}).get("enabled") is True
        and all(
            status.get("qualification", {}).get(capability) is True
            for capability in MERCHANT_CAPABILITIES
        )
        and isinstance(status.get("profile_id"), str)
        and status["profile_id"]
        and not status.get("pending")
        and not status.get("manual_input_fence")
        and not status.get("input_active")
        and not status.get("error")
        and not status.get("needs_attention")
        and snapshot.get("character") == name
        and snapshot.get("server") == "America"
        and type(snapshot.get("character_uid")) is int
        and snapshot["character_uid"] > 0
        and snapshot.get("hp", 0) > 0
        and snapshot.get("map_id") == 1036
        and snapshot.get("booth_open") is True
        and type(snapshot.get("own_booth_uid")) is int
        and snapshot["own_booth_uid"] > 0
        and not snapshot.get("trade")
        and not snapshot.get("request")
        and 0 <= time.time() - snapshot.get("timestamp", 0) <= 5
        and (
            available_slots(snapshot) > 0
            if require_capacity
            else available_slots(snapshot) >= 0
        )
    )
...
def merchant_qualification(ui, name, snapshot):
    ...
    if (
        snapshot["identity"] != adapter.identity
        or not isinstance(adapter.expected_sha256, str)
        or not adapter.expected_sha256
    ):
        raise ValueError(
            "Acceptance merchant process or build does not match its attached driver"
        )
    for capability in MERCHANT_CAPABILITIES:
        driver.require_qualified(capability)
    path = Path(driver.qualification)
    encoded = path.read_bytes()
    data = json.loads(encoded)
    if (
        data.get("client_sha256") != adapter.expected_sha256
        or data.get("character") != name
        or data.get("server") != "America"
        or not data.get("evidence")
        or not all(
            data.get("capabilities", {}).get(capability) is True
            for capability in MERCHANT_CAPABILITIES
        )
    ):
...
def qualification_unchanged(evidence):
    try:
        return bool(
            evidence.get("client_sha256")
            and hashlib.sha256(Path(evidence["path"]).read_bytes()).hexdigest()
            == evidence["sha256"]
        )
...
def configure(ui, body):
    ...
    if (
        ...
        or body.get("farmer_profile_id") != profile_id()
    ):
    ...
    if not body["enabled"]:
        ...
        def disable(old):
            ...
            if old.get("active"):
                raise ValueError(
                    "Acceptance has an unresolved cycle; Farming Off always stops input, but reconcile its native delivery/refill/return before disabling this evidence hold"
                )
    ...
    key = body["request_id"]
    if (
        type(body.get("cycles")) is not int
        or body["cycles"] != 3
    ...
    old = state()
    if old.get("request_id") == key:
        return old  # Lost enable ack never resets a baseline.
    if old.get("enabled"):
    ...
    if (
        ...
        raise ValueError(
            "Acceptance must be armed with Farming Off and no input or pause holds"
        )
    ...
    for name, status in ui.runtime.status().items():
        ...
        if merchant_live(name, status, snap):
            ...
            merchants[str(name)] = {
                ...
                "character_uid": snap["character_uid"],
                "own_booth_uid": snap["own_booth_uid"],
                "qualification": evidence,
            }
    if not merchants:
        raise ValueError(
            "Acceptance requires an exact ready merchant with trading and refill enabled, qualified booth input, and no manual or transaction holds"
        )
    route = ui.app.selected_route
    if route is None:
        raise ValueError("Choose a native farmer route before acceptance")
    ...
    travel = (
        read_json(TRAVEL_POLICY).get("origins", {}).get(str(route.restock_map_id), {})
    )
    outbound, inbound = travel.get("outbound", {}), travel.get("return", {})
    if (
        not bank_policy.get("enabled")
        or not bank_policy.get("deposit_after_shopping", True)
        or not outbound.get("verified")
        or not inbound.get("verified")
        or (
            outbound.get("source_map"),
            outbound.get("destination_map"),
            inbound.get("source_map"),
            inbound.get("destination_map"),
        )
        != (route.restock_map_id, 1036, 1036, route.restock_map_id)
    ):
        raise ValueError(
            "Acceptance requires enabled native banking and a verified round-trip merchant route"
        )
    from conquest.town_visit import TownVisit

    if TownVisit().active_id():
        raise ValueError(
            "Complete the current required town return before arming acceptance"
        )

    def enable(previous):
        if previous.get("request_id") == key:
            return None
        if previous.get("enabled"):
            raise ValueError("Acceptance changed while arming; recheck")
        return {
            "schema_version": 1,
            "enabled": True,
            "phase": "armed",
            ...
            "target_cycles": 3,
            "farmer_profile_id": profile_id(),
            "farmer": source,
            "route_id": route.id,
            "hunt_map_id": route.map_id,
            "qualification": qualification_evidence,
            "rollout_promoted": False,
            "merchants": merchants,
            "cycle_baseline_at": time.time(),
            "baseline_uids": sorted(inventory),
...
def journey_scope_completed(scope):
    ...
    return bool(
        ...
        and cycle.get("phase") == "awaiting_hunt"
        and cycle.get("refill")
        and delivery.get("outcome") == "transferred"
        and delivery.get("proof_digest")
    )
...
def trial_permitted(loop=None):
    ...
    if (
        not row.get("enabled")
        or not cycle
        or cycle["phase"] not in ("town", "delivered")
        or row.get("farmer_profile_id") != profile_id()
        or len(row["cycles"]) >= row["target_cycles"]
    ):
        return False
    if loop is not None and (
        loop.route.id != row["route_id"]
    ...
    try:
        return (
            hashlib.sha256(Path(qualification["path"]).read_bytes()).hexdigest()
...
def trial_delivery_permitted(ui, key, character, uids, origin):
    ...
    return bool(
        admission
        and cycle["phase"] == "town"
        and merchant_allowed(character, merchant, merchant.get("snapshot") or {})
        and character == admission["merchant"]
        and uids == [cycle["item"]["uid"]]
        ...
        and ui.app.observer.adapter.expected_sha256
        == row["qualification"]["client_sha256"]
    )
...
def merchant_allowed(name, status, snapshot):
    ...
    try:
        return bool(
            saved
            and merchant_live(
                name, status, snapshot, require_capacity=cycle["phase"] == "town"
            )
...
def hunt_provenance(row, item):
    ...
    if (
        not checkpoint.get("available")
        or not kill
        or not baseline <= kill["time"] <= now
    ):
        return None
    try:
    ...
    pickup = next(
        (
            ...
            if p.get("inventory_uid") == item["uid"]
            and p.get("type_id") == item["type_id"]
            and p.get("plus") == item.get("plus")
            and p.get("increase", 0) > 0
            and p.get("map_id") == row["hunt_map_id"]
            and baseline <= p.get("timestamp", 0) <= now
            and p.get("source") in (None, "ground_pickup", "inventory_gain")
        ),
        None,
    )
    return (
        {"pickup": pickup, "verified_kill": kill, "kill_checkpoint": checkpoint}
        if pickup and kill["time"] <= pickup["timestamp"]
        else None
    )
...
def _live_health(row, health):
    ...
    return (
        ...
        and data.get("manual_mouse") is False
        and data.get("manual_input_fence") is False
...
def observe_hunting(loop, health, *, send=None):
    ...
    if not _live_health(row, health):
        return False
    ...
    try:
        ...
        same_run = (
            current.get("enabled")
            and current.get("run_id") == row.get("run_id")
    ...
    if active and active["phase"] == "awaiting_hunt":
        ...
        if (
            ...
            and visit.get("town_visit_id") == active["town_visit_id"]
            and visit.get("farmer_profile_id") == row["farmer_profile_id"]
            and visit.get("hunt_map_id") == row["hunt_map_id"]
            and process(visit.get("return_target"))
            == process(row["farmer"]["identity"])
            and (visit.get("first_verified_resume_kill") or {}).get("time", 0)
            >= active["service_finished_at"]
        ):

            def complete(current):
                if (
                    not current.get("enabled")
                    or current.get("active", {}).get("cycle_id") != active["cycle_id"]
                ):
                    return None
                cycle = current["active"]
                if cycle["phase"] != "awaiting_hunt":
                    return None
                cycle.update(
                    phase="completed", town_return=visit, completed_at=time.time()
                )
                current["cycles"].append(cycle)
                current["active"] = None
                current["baseline_uids"] = sorted(
                    set(current["baseline_uids"]) | set(inventory)
                )
            ...
            loop.record(
                "merchant_acceptance_cycle_completed",
                run_id=finished["run_id"],
                completed_cycles=len(finished["cycles"]),
                target_cycles=finished["target_cycles"],
            )
        return False
    if active:
        return active["phase"] in ("triggered", "town", "delivered")
    candidates = [
        i
        for i in source["inventory"]
        if i["uid"] not in row["baseline_uids"] and deliverable_item(i)
    ]
    if not candidates:
        return False
    selected = sorted(
        candidates,
        key=lambda i: (-(i["plus"] if type(i.get("plus")) is int else 0), i["uid"]),
    )[0]
    ...
    def trigger(current):
        if (
            not current.get("enabled")
            or current.get("run_id") != row["run_id"]
            or current.get("active")
        ):
            return None
        current["active"] = {
...
def admitted(active):
    def save(row):
        cycle = row.get("active")
        if not row.get("enabled") or not cycle or cycle["phase"] != "town":
            return None
        if (
            active.get("town_visit_id") != cycle.get("town_visit_id")
        ...
        previous = next(
            (a for a in cycle["admissions"] if a["request_id"] == active["request_id"]),
            None,
        )
        if previous:
            if previous != active:
                raise ValueError("Acceptance delivery admission changed")
            return None
        cycle["admissions"].append(deepcopy(active))
        return row
...
def settled(receipt):
    def save(row):
        ...
        admitted = next(
            (
                a
                for a in cycle["admissions"]
                if a["request_id"] == receipt.get("request_id")
            ),
            None,
        )
        if not admitted:
            return None
        if (
            receipt.get("outcome") != "transferred"
            or not receipt.get("proof_digest")
            or receipt.get("next_action") != "release_route"
            or receipt.get("cleanup_pending")
            ...
            return None
        cycle.update(phase="delivered", delivery=deepcopy(receipt))
        return row
...
def reconcile_route_receipts():
    if not cycle_pending():
        return
    from conquest.discord_notify import read_json
    from conquest.merchants.delivery_route import STATE as ROUTE_STATE

    for receipt in read_json(ROUTE_STATE).get("receipts", []):
        settled(receipt)
...
def pending_delivery_matches(native):
    ...
    return bool(
        row.get("enabled")
        and row.get("farmer_profile_id") == profile_id()
...
def refill_observed(statuses):
    def save(row):
        cycle = row.get("active")
        if not row.get("enabled") or not cycle or cycle["phase"] != "delivered":
            return None
        receipt = cycle["delivery"]
        merchant = statuses.get(receipt["merchant"], {})
        refill = merchant.get("refill") or {}
        snapshot = merchant.get("snapshot") or {}
        admission = next(
            a for a in cycle["admissions"] if a["request_id"] == receipt["request_id"]
        )
        if (
            not merchant_allowed(receipt["merchant"], merchant, snapshot)
            or refill.get("pending") is not False
            or refill.get("status") not in ("completed", "no_stock", "booth_full")
            ...
            or merchant.get("pending")
            or merchant.get("manual_input_fence")
        ):
            return None
        item = cycle["item"]
        wanted = exact_items([item])[item["uid"]]
        bag, booth = (
            exact_items(snapshot["inventory"]),
            exact_items(snapshot.get("booth", [])),
        )
        if item["uid"] in bag and item["uid"] in booth:
            return None
        disposition = (
            "listed"
            if booth.get(item["uid"]) == wanted
            else "queued"
            if bag.get(item["uid"]) == wanted
            else None
        )
...
def refill_source(key, character):
    ...
    return (
        key
        if row.get("enabled")
        and cycle.get("phase") == "delivered"
        and cycle.get("delivery", {}).get("request_id") == key
        and cycle["delivery"].get("merchant") == character
        else None
    )
...
def finish_town(loop, *, send=None):
    ...
    def finish(row):
        ...
        if cycle["phase"] != "delivered" or not cycle.get("refill"):
            raise ValueError(
                "Acceptance cycle lacks exact delivery and completed merchant refill evidence; review before continuing"
            )
        cycle.update(phase="awaiting_hunt", service_finished_at=time.time())
        return row
...
def verify_carried_or_delivered(source):
    ...
    if inventory.get(item["uid"]) != exact_items([item])[item["uid"]]:
        raise ValueError(
            "Acceptance item is not carried and has no matching bilateral receipt; no withdrawal or trade replay allowed"
        )
    return True
...
def abort(ui, body):
    ...
    if (
        set(body) != {"action", "farmer_profile_id", "run_id"}
        or body["farmer_profile_id"] != profile_id()
    ):
        raise ValueError("Acceptance abort requires this farmer and exact run ID")
    row = state()
    if row.get("run_id") != body["run_id"]:
        raise ValueError("Acceptance run changed; recheck before aborting")
    if row.get("phase") == "aborted":
        return row
    cycle = row.get("active")
    pre_input_town = bool(
        cycle
        and cycle["phase"] == "town"
        and not cycle.get("admissions")
        and not cycle.get("native_town_input_maybe_started_at")
    )
    if (
        not row.get("enabled")
        or not cycle
        or (cycle["phase"] not in ("triggered", "awaiting_hunt") and not pre_input_town)
    ):
        raise ValueError(
            "Only a pre-input trigger or fully settled service can be aborted safely"
        )
    ...
    if cycle["phase"] == "triggered" or pre_input_town:
        if cycle.get("admissions") or (
            cycle["phase"] == "triggered" and cycle.get("town_visit_id")
        ):
            raise ValueError("Triggered acceptance already has native input admission")
        item = cycle["item"]
        if inventory.get(item["uid"]) != exact_items([item])[item["uid"]]:
            raise ValueError(
                "Pre-input acceptance item ownership changed; no abort inferred"
            )
...
def restoration_checks(ui):
    ...
    if (
        ...
        raise ValueError(
            "Stop Farming and wait for all native input to release before aborting acceptance"
        )
    from conquest.merchants.delivery_operation import guard_reload
    from conquest.merchants.delivery_journey import pending as journey_pending
    from conquest.meteor_banking import pending as meteor_pending
    ...
    statuses = ui.runtime.status()
    if any(
        s.get("pending") or s.get("refill", {}).get("pending") or s.get("input_active")
...
def operator_override(ui, body):
    ...
    if not preview:
        allowed |= {"operator_confirmed", "incident_digest"}
    if set(body) != allowed or body.get("farmer_profile_id") != profile_id():
        raise ValueError(
            "Acceptance override requires an exact farmer, run and preview digest"
        )
    row = state()
    if row.get("run_id") != body["run_id"]:
        raise ValueError("Acceptance run changed")
    if not preview and row.get("phase") == "operator_overridden":
        if body.get("operator_confirmed") is True and row["operator_override"][
            "incident_digest"
    ...
    try:
        ...
        warehouse = {
            "available": True,
            "items": exact_items(stored["items"]),
            "capacity": stored["capacity"],
            "ownership_inference": "read_only_current_stock",
        }
    except (OSError, ValueError):
        pass  # No panel is opened to make evidence available.
    ownership = {
        "farmer": {
            key: source[key]
            for key in ("character", "character_uid", "identity", "silver")
        },
        "inventory": exact_items(source["inventory"]),
        "warehouse": warehouse,
    }
    incident = _digest(
        {
            ...
            if key not in ("updated_at", "override_preview")
        }
    )
    evidence = {
        "incident_digest": incident,
        "ownership_digest": _digest(ownership),
        "observed_at": time.time(),
        "ownership": ownership,
        "historical_outcome": "unknown_or_deferred",
        "native_journals_unchanged": True,
    }
    if preview:
                ...
                _digest(
                    {
                        ...
                        if k not in ("updated_at", "override_preview")
                    }
                )
                != incident
            ):
                raise ValueError("Acceptance changed during preview")
            current["override_preview"] = evidence
            return current
    ...
    if (
        body.get("operator_confirmed") is not True
        or body.get("incident_digest") != incident
