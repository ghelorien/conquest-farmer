...
def _blocked(code, note=None):
    return {
        ...
        "blocker": code,
        "note": note or code,
        "handoff_mode": "existing_route_grant",
        "focus_mode": "exact_native_owner",
    }
...
def _settle_cursor(journal, character, request_id):
    ...
    with journal.db() as db:
        ...
        state = db.execute(
            "SELECT value FROM state WHERE character=? AND name='refill'", (character,)
        ).fetchone()
        ...
        state.update(
            ...
            cursor=[
                uid for uid in state.get("cursor", []) if uid != request["item_uid"]
            ],
        )
        db.execute(
            "UPDATE state SET value=? WHERE character=? AND name='refill'",
...
def pending_cancel(runtime, character, snapshot, *, preflight=False):
    ...
    request = state.get("listing1078_request")
    if (
        not runtime.refill_enabled(character)
        or state.get("listing1078_engine") != 1
        or not request
    ):
    ...
    ownership_complete = all(
        key in old for key in OWNERSHIP_FIELDS if key not in ("trade", "request")
    )
    original_closed = all(
        old.get(key) is None
        if key in old
        else (
            old.get("closed_modal") is True
            and old.get("trade_open") is False
            and old.get("request_open") is False
        )
        for key in ("trade", "request")
    )
    if (
        before.get("request") != request
        or before.get("profile_id") != profile.id
        ...
        or not ownership_complete
        or not original_closed
    ):
    ...
    if preflight:
        if snapshot.get("closed_modal") is not True:
            raise ValueError(
                "Trade/request windows must be closed before refill cleanup"
            )
        modal = (snapshot.get("listing_preflight") or {}).get("price_modal") or {}
        digits = modal.get("candidate_price_buffer_text")
        if (
            modal.get("observed") is not True
            or modal.get("candidate_selected_item_uid") != request["item_uid"]
            or any(
                modal.get(key) is not True
                for key in (
                    "selected_item_uid_verified",
                    "price_buffer_binding_verified",
    ...
    snapshot = {
        ...
        **{
            ...
            for collection in ("inventory", "booth")
        },
    }
...
def step(ui, character, snapshot):
    ...
    try:
            ...
            if row and row["phase"] == "aborted":
                ...
                if row["phase"] in ("prepared", "uncertain"):
                    try:
                        ...
                        return _blocked(
                            "listing_receipt_needs_reconciliation", str(error)
                        )
        ...
        if len(snapshot["booth"]) >= 32 or not snapshot["inventory"]:
            ...
            schedule.complete(
                "booth_full" if snapshot["inventory"] else "no_stock",
        ...
        _policy(
            ...
            merchant_intent=_merchant_intent(runtime, character),
        ...
        if native["root_hwnd"] != observer.hwnd:
            return _blocked(
                "native_owner_surface_unavailable",
        ...
        schedule.start()
        schedule.checkpoint(
            [row["uid"] for row in queue], deferred=len(queue) - len(eligible)
        )
        state = schedule.state()
