...
def parent_visit():
    ...
    for name in ("merchant-journey", "meteor-consolidation", "overflow"):
        ...
        if row.get("started_at") and row.get("phase") not in (
            None,
            "completed",
            "complete",
            "failed",
...
def validate_grant(ui, body):
    ...
    if (
        ...
        or row.get("visit_id") != body.get("visit_id")
    ...
    with observer.lock:
        source = source_memory(observer).read()
    if (
        source.get("map_id") != 1036
...
class MarketVisit:
    def __init__(self, path=None, *, clock=time.time):
        self.path = Path(
            path or state_path("reports/banking/merchant-service-visit.json")
        )
        self.clock = clock

    def begin(self, *, parent=None, profile=None):
        now = self.clock()
        profile = profile or farmer_id()
        ...
        row = {
            ...
            "farmer_profile_id": profile,
            "parent_visit_id": parent,
            "phase": "active",
            "started_at": now,
        ...
        required = TownVisit().active_id()
        if required and required == parent:
            row["town_visit_id"] = required
    ...
    def attempt(self, merchant, position, outcome):
        row = read_json(self.path)
        if row.get("phase") != "active":
            raise ValueError("No active Market visit")
