...
class Recovery:
    def __init__(self, character, journal, *, clock=time.time, limit=3):
        self.character, self.journal, self.clock, self.limit = (
            character,
            journal,
            clock,
            limit,
    ...
    def state(self):
        return self.journal.get(
            self.character,
            "recovery",
            {"attempts": 0, "next_attempt": 0, "state": "connected"},
        )
    ...
    def retry(self):
        self.journal.set(
            self.character,
            "recovery",
            {"attempts": 0, "next_attempt": 0, "state": "retry_requested"},
        )
    ...
    def attempt(self, action):
        ...
        try:
            ...
            raise ValueError(
                "Recovery failed; check local credentials and qualified login state"
            ) from None
        return True
    ...
    def verified(self):
        ...
        if state["state"] != "connected":
            self.journal.event(
                self.character, "recovery_verified", attempts=state["attempts"]
            )
        self.journal.set(
            self.character,
            "recovery",
