...
def allowed(runtime, character, snapshot):
    ...
    if state.get("phase") != "needs_attention" or not runtime.refill_enabled(character):
        return False
    if (
        ...
        return False
    observer = runtime.observers.get(character)
    if observer is None or observer.adapter.identity != snapshot.get("identity"):
        return False
    ...
    if evidence.get("identity") != identity or evidence.get("booth_uid") != booth:
        ...
        return False
    if snapshot["timestamp"] <= evidence["first_seen"]:
        return False
    if not evidence.get("qualified"):
        ...
        evidence.update(
            qualified=True,
            verified_at=snapshot["timestamp"],
            current_snapshot=snapshot,
            missing_uids=sorted(previous.keys() - current.keys()),
            new_uids=sorted(current.keys() - previous.keys()),
            incident_unresolved=True,
            original_return_state=state,
        )
        runtime.journal.set(character, "held_stock_refill", evidence)
        runtime.journal.event(
            character,
            "held_stock_refill_qualified",
            missing_uids=evidence["missing_uids"],
            new_uids=evidence["new_uids"],
            incident_unresolved=True,
        )
    return True
