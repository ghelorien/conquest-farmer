...
class TradeObservationReader1078:
    ...
    def __init__(self, session, character: str):
        if getattr(session, "expected_sha256", None) != CLIENT_SHA256_1078:
            raise ObservationUnavailable1078("1078 reader requires its exact executable fingerprint")
        ...
        for rva in (self.root_rva, self.registry_rva, self.server_rva,
                    self.actual_vtable_rva, self.wrapper_vtable_rva, self.item_vtable_rva,
                    self.trade_vtable_rva, self.confirm_vtable_rva, self.booth_vtable_rva,
                    self.map_rva):
            if not 0 <= rva < self.module["size"]:
                raise ObservationUnavailable1078("1078 candidate RVA is outside the loaded module")
    ...
    def _string(self, address, maximum=128):
        ...
        value = (raw[:length] if capacity <= 15 else
                 self._read(struct.unpack_from("<Q", raw)[0], length) if length else b"")
    ...
    def _actual_and_wrapper(self):
        ...
        if self._read(actual + 0x94, 64).split(b"\0", 1)[0].decode("utf-8") != self.character:
            raise ObservationUnavailable1078("Selected character differs from 1078 actor")
        if self._read(wrapper + 0xA4, 64).split(b"\0", 1)[0].decode("utf-8") != self.character:
            raise ObservationUnavailable1078("Selected character differs from 1078 wrapper")
        return holder, actual, wrapper

    def _assert_actor_stable(self, holder, actual, wrapper):
        if (self._u64(self.base + self.root_rva) != holder or self._u64(holder) != actual
            ...
            raise ObservationUnavailable1078("1078 actor identity changed during observation")
    ...
    def _model(self, key, expected_vtable):
        ...
        while node != head:
            ...
            if found == key:
                ...
                if (self._u64(model) != self.base + expected_vtable
                        or self._read(node, 56) != raw
                        or self._read(self.base + self.registry_rva, 16) != header):
                    raise ObservationUnavailable1078("1078 model changed during observation")
                return model
    ...
    def _deque(self, address, limit):
        ...
        for index in range(count):
            slot = struct.unpack_from("<Q", entries, ((start + index) % capacity) * 8)[0]
            shared = self._read(slot, 16)
            pointers.append(struct.unpack_from("<Q", shared)[0])
            references.append((slot, shared))
        if (len(set(pointers)) != len(pointers) or self._read(address, 32) != header
                or self._read(table, capacity * 8) != entries
                or any(self._read(slot, 16) != raw for slot, raw in references)):
            raise ObservationUnavailable1078("1078 item deque changed during observation")
    ...
    def _item(self, pointer, slot, *, booth=False):
        ...
        uid, type_id = struct.unpack_from("<I", raw, 8)[0], struct.unpack_from("<I", raw, 0x10)[0]
        length, capacity = struct.unpack_from("<QQ", raw, 0x28)
        if not uid or not 0 < type_id < 100000000 or not 1 <= length <= 127 or not length <= capacity <= 1024:
            raise ObservationUnavailable1078("Invalid 1078 item identity")
        name_bytes = raw[0x18:0x28] if capacity <= 15 else self._read(struct.unpack_from("<Q", raw, 0x18)[0], length)
        try:
        ...
        return StockItem1078(uid, type_id, name, plus, raw[0x67], raw[0x68],
                             bool(raw[0x44] & 1), quantity, slot, price)
    ...
    def _inventory(self, wrapper):
        ...
        if independent_count != len(pointers):
            raise ObservationUnavailable1078("1078 inventory count disagrees with its deque")
        stock = [self._item(pointer, slot) for slot, pointer in enumerate(pointers)]
        silver = self._u32(wrapper + 0xAB8)
        if self._read(wrapper + 0xBB0, 32) != header or self._u64(wrapper + 0xBE0) != independent_count:
            raise ObservationUnavailable1078("1078 inventory changed during observation")
        return stock, silver
    ...
    def _booth(self, actual):
        ...
        if open_ and (not owner or struct.unpack_from("<I", raw, 0x4C)[0] != owner):
            raise ObservationUnavailable1078("1078 displayed booth is not the actor's booth")
        if not owner and pointers:
            raise ObservationUnavailable1078("1078 booth items have no owned booth")
        items = [self._item(pointer, slot, booth=True) for slot, pointer in enumerate(pointers)]
        if (self._read(actual + 0x34B0, 32) != header or self._read(model, 0x50) != raw
    ...
    def _health(self, actual):
        ...
        if not 1 <= maximum <= 1000000 or mode not in (0, 1, 2, 3) or not 1 <= count <= 1024:
        ...
        if (not 0 <= current <= maximum or self._u64(actual + 0x978) != attributes
                or self._read(attributes, 24) != header or self._read(table_address, count * 4) != table
    ...
    def _request(self, actual):
        ...
        if not raw[12]:
            return None
        title, participant = self._string(model + 0x48, 63), self._string(actual + 0x1000, 63)
        message, participant_uid = self._string(model + 0x68, 256), self._u32(actual + 0xFF8)
        if (title != "Trade###Confirm" or not participant or not participant_uid
                or message != f"{participant} wishes to trade with you."):
            raise ObservationUnavailable1078("1078 incoming trade request disagrees with confirmation")
        if self._read(model, 0xC0) != raw:
            raise ObservationUnavailable1078("1078 incoming request changed during observation")
        return {"participant": participant, "participant_uid": participant_uid, "message": message}
    ...
    def read_manual_ownership(self):
        ...
        if server.split(b"\0", 1)[0] != b"Classic_US":
            raise ObservationUnavailable1078("1078 server field is not the qualified America value")
        ...
        result["canonical_manual_ownership"] = result["trade"] is None and result["request"] is None
        ...
        if bool(self._u8(self._model(14, self.trade_vtable_rva) + 12)) != (result["trade"] is not None):
            raise ObservationUnavailable1078("1078 trade state changed during observation")
        if bool(self._u8(self._model(15, self.confirm_vtable_rva) + 12)) != (result["request"] is not None):
            raise ObservationUnavailable1078("1078 request state changed during observation")
        latest_inventory, latest_silver = self._inventory(wrapper)
        latest_booth, latest_owner, latest_open = self._booth(actual)
        if (latest_inventory != inventory or latest_silver != silver or latest_booth != booth
                or (latest_owner, latest_open) != (own_booth_uid, booth_open)):
            raise ObservationUnavailable1078("1078 ownership changed during observation")
        self._assert_actor_stable(holder, actual, wrapper)
        if (self._u32(actual + 0x68) != character_uid
                or self._u32(self.base + self.map_rva) != map_id
                or self._read(actual + 0xD8, 8) != struct.pack("<2I", *position)
                or self._read(actual + 0x94, 64).split(b"\0", 1)[0].decode("utf-8") != self.character
                or self._read(wrapper + 0xA4, 64).split(b"\0", 1)[0].decode("utf-8") != self.character):
            raise ObservationUnavailable1078("1078 actor evidence changed during observation")
        if self._read(self.base + self.server_rva, 64) != server:
            raise ObservationUnavailable1078("1078 server field changed during observation")
        self.session.assert_identity()
