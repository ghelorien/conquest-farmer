...
def _read():
    ...
    try:
        ...
        raise ValueError(
            "Trade qualification prep evidence is unreadable; reconcile before input"
        ) from error
...
def _archive(state):
    ...
    def verify_and_sync():
        with path.open("r+b") as stream:
            if stream.read() != raw:
                raise ValueError(
                    "Trade qualification prep archive differs from its receipt"
                )
            stream.flush()
            os.fsync(stream.fileno())
...
def _selected(source, uid):
    ...
    scroll = (
        ...
        and kind == 720027
        and all(
            type(item.get(key)) is int and item[key] == value
            for key, value in (("plus", 0), ("gem1", 0), ("gem2", 0), ("quantity", 1))
        )
    )
    if (
        not eligible(item)
        or not (equipment or scroll)
        or item.get("gem1") != 0
        or item.get("gem2") != 0
        or item.get("quantity") != 1
    ):
        raise ValueError(
            "Qualification requires one unbound, unsocketed, non-Super +1 equipment item or one exact MeteorScroll"
        )
    return item
...
def _same_participant(current, saved, role):
    if (
        current.get("character") != saved.get("character")
        ...
        raise ValueError(
            f"{role} process or character identity changed; prep cannot resume"
        )
...
def _payload_is_safe(source, state):
    ...
    if extras:
        raise ValueError(
            "More than one protected or delivery-eligible item remains carried"
        )
    return selected
...
def _market_outbound(state):
    ...
    if (
        ...
        or type(outbound.get("source_map")) is not int
        or outbound["source_map"] != 1036
        or type(outbound.get("destination_map")) is not int
        or outbound["destination_map"] != 1036
        or type(outbound.get("fare")) is not int
        or outbound["fare"] != 0
    ):
        raise ValueError(
            "Market-origin prep requires the exact verified zero-fare route"
        )
    return outbound
...
def _market_no_banking_body(state, source, merchant):
    ...
    if (
        not isinstance(saved_farmer, dict)
        or type(saved_farmer.get("map_id")) is not int
        or saved_farmer["map_id"] != 1036
        or not isinstance(saved_merchant, dict)
        or type(saved_merchant.get("map_id")) is not int
        or saved_merchant["map_id"] != 1036
    ):
    ...
    if state.get("deposit") is not None or state.get("withdraw") is not None:
        raise ValueError(
            "Pending deposit or withdrawal evidence blocks the Market fast path"
        )
    _same_participant(source, state["farmer"], "Farmer")
    ...
    return {
        "schema_version": 1,
        "kind": "market_no_banking_required",
        "origin": 1036,
        "selected_uid": state["selected_uid"],
        "farmer": {
            key: source[key]
            for key in ("character", "character_uid", "server", "identity")
        },
        "merchant": {
            key: merchant[key]
            for key in ("character", "character_uid", "server", "identity")
...
def _new_market_no_banking_proof(state, source, merchant):
    ...
    return {
        **_market_no_banking_body(state, source, merchant),
...
def _validate_market_no_banking_proof(state, source, merchant):
    ...
    if set(proof) != {*expected, "observed_at"} or any(
...
def _candidate_profile(observer):
    ...
    if (
        ...
        or any(
            type(recipient.get(key)) is not int
            for key in (
                "vtable_rva",
                "uid_offset",
                "name_offset",
        ...
        or not all(
            isinstance(value, str) and value for value in profile["evidence_reports"]
        )
...
def _projection_observation(observer, profile, farmer, merchant):
    ...
    try:
        actionability = recipient_actionability(
            observer, profile, merchant, farmer=farmer
        )
...
def target_projection(ui, character):
    ...
    if (
        not state
        or state.get("phase") not in ("market", "approaching")
    ...
    if (
        len(size) != 2
        or len(gui) != 2
        or any(type(value) is not int or value <= 0 for value in size + gui)
    ):
    ...
    if fresh_farmer.get("position") != farmer.get("position") or fresh_merchant.get(
        "position"
    ) != merchant.get("position"):
        raise ValueError("Prep participants moved during target projection")
    if (
        fresh_farmer.get("trade")
        or fresh_farmer.get("request")
        or fresh_merchant.get("trade")
        or fresh_merchant.get("request")
    ):
    ...
    if (
        projection_binding(first) != projection_binding(second)
        or first_anchor != second_anchor
    ):
        raise ValueError(
            "Trade recipient actionability changed during target projection"
        )

    current = _read()
    current_incident = hashlib.sha256(
    ...
    if second is None:
        return {
            ...
            "character": farmer["character"],
            "farmer_position": farmer["position"],
    ...
    reason = (
        second["reason"]
        if in_range or not second["actionable"]
        else "recipient_out_of_range"
    )
    return {
        "schema_version": 1,
        **second,
...
def _validate_request(ui, character, uid):
    ...
    if control.get("enabled") or control.get("paused"):
        raise ValueError(
            "Trade qualification prep requires Farming Off and no pause hold"
        )
    if (
        getattr(ui.app, "thread", None)
        and ui.app.thread.is_alive()
        or getattr(ui, "delivery_probe_thread", None)
        and ui.delivery_probe_thread.is_alive()
        or any(
    ...
    if (
        ...
        raise ValueError(
            "Reconcile existing automated transactions before supervised prep"
        )
...
class PrepLoop:
    ...
    def __init__(self, ui, state):
        ...
        self.terrain = read_terrain(
            installation_path(r"C:\Program Files\Classic Conquer 2.0"), state["origin"]
        )
        self.deadline = None
    ...
    travel = lambda self, destination, **fields: self._base.travel(
        self, destination, **fields
    )
    _travel = lambda self, destination, **fields: self._base._travel(
        self, destination, **fields
    )
    ...
    def check_stop(self):
        ...
        if (
            self.ui.closed
            or self.ui.app.closing
            or self.ui.coordinator.stopped
            or control.get("enabled")
            ...
            raise OvernightStopped(
                "Trade qualification prep stopped by control or input ownership change"
            )
...
def _release_window(ui, key):
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            raise ValueError(
                "Farmer input did not release; prep work window remains active"
            )
        time.sleep(0.05)
...
def _recovery_available(ui):
    if (
        getattr(ui, "trade_qualification_prep_thread", None)
    ...
    if (
        getattr(ui, "delivery_probe_thread", None)
        and ui.delivery_probe_thread.is_alive()
        or any(
            worker.is_alive() for worker in getattr(ui, "delivery_workers", {}).values()
        )
...
def _fresh_recovery_evidence(ui, state):
    ...
    proof = {
        "farmer": canonical_ownership(source, require_closed=False),
        "merchant": canonical_ownership(merchant, require_closed=False),
        "farmer_map": source["map_id"],
        "merchant_map": merchant["map_id"],
    }
    warehouse = {"available": False, "money_available": False}
    observer = ui.app.observer
    with observer.lock:
        try:
            money_before = observer.town_trade({"action": "warehouse-money"})
        except (ValueError, OSError, KeyError, TypeError) as error:
            money_before = None
            warehouse["money_reason"] = type(error).__name__
        try:
            snapshot = observer.town_trade({"action": "warehouse-items", "rich": True})
        except (ValueError, OSError, KeyError, TypeError) as error:
            snapshot = None
            warehouse["reason"] = type(error).__name__
        try:
            money_after = observer.town_trade({"action": "warehouse-money"})
        except (ValueError, OSError, KeyError, TypeError) as error:
            money_after = None
            warehouse["money_reason"] = type(error).__name__
    ...
    return {
        ...
        "merchant": merchant,
        "proof": proof,
        "ownership_digest": evidence_digest(proof),
        "farmer_identity_matches": source.get("identity")
        == state.get("farmer", {}).get("identity"),
        "merchant_identity_matches": merchant.get("identity")
        == state.get("merchant", {}).get("identity"),
    }
...
def recheck(ui):
    ...
    preview = {
        ...
        "evidence_digest": fresh["ownership_digest"],
...
def operator_override(
    ...
    *,
    ...
    _recovery_available(ui)
    if (
        operator_confirmed is not True
        or not isinstance(incident_digest, str)
        or not incident_digest
        or confirmation_reference != incident_digest
    ):
    ...
    if state and state.get("phase") == "operator_overridden":
        ...
        if (
            prior.get("confirmation_reference") == confirmation_reference
        ...
        raise ValueError(
            "Trade prep was already overridden with different confirmation"
        )
    if (
        not state
        or state.get("phase") not in PENDING
        or evidence_digest(state) != incident_digest
    ):
    ...
    if (
        preview.get("incident_digest") != incident_digest
        or not 0
        <= time.time() - preview.get("fresh_evidence", {}).get("observed_at", 0)
        <= 30
    ):
    ...
    if fresh["ownership_digest"] != preview.get("evidence_digest"):
        raise ValueError(
            "Current ownership changed; recheck trade prep before overriding"
        )
    result = close(
        JOURNAL,
        ...
        operator=operator,
        fresh_evidence=fresh,
        incident="trade-qualification-prep",
    )
...
def _reconcile_deposit(loop, state, *, send=request):
    ...
    if (
        type(uid) is not int
        or not isinstance(before_bag, list)
    ...
    new_bag, new_bank = (
        exact_items(source["inventory"]),
        exact_items(warehouse["items"]),
    )
    if (
        uid not in old_bag
        or uid in old_bank
        or old_bag[uid] != exact_items([item])[uid]
    ):
        raise ValueError(
            "Pending deposit baseline does not own the exact selected item"
        )
    expected_bag = dict(old_bag)
    del expected_bag[uid]
    expected_bank = dict(old_bank)
    expected_bank[uid] = old_bag[uid]
    ...
    if new_bag == old_bag and new_bank == old_bank:
        return False
    raise ValueError(
        "Pending warehouse deposit changed ownership unexpectedly; no input repeated"
    )
...
def _bank_nonselected(loop, state, *, send=request):
    ...
    if money["silver"] < fare:
        ...
        if any(unchanged.get(key) != value for key, value in before.items()):
            raise ValueError(
                "Warehouse money changed before withdrawal; no input issued"
            )
        _save(state, "withdraw_submitted")
        transfer(loop, "withdraw", amount)
        fresh = loop.town("warehouse-money")
        if (
            fresh["silver"] != before["silver"] + amount
...
def _reconcile_withdraw(loop, state):
    ...
    if (
        current["silver"] == before["silver"] + amount
        and current["stored_silver"] == before["stored_silver"] - amount
    ):
        _save(state, "warehouse_opening", withdraw=None)
        return
    if (
        current["silver"] == before["silver"]
        and current["stored_silver"] == before["stored_silver"]
    ):
        raise ValueError(
            "Fare withdrawal outcome needs attention; no transfer repeated"
        )
    ...
    try:
        ...
        if phase in ("withdraw_pending", "withdraw_submitted"):
            if world != state["origin"]:
                raise ValueError(
                    "Fare withdrawal location changed; no transfer repeated"
                )
        ...
        if phase in ("prepared", "warehouse_opening", "deposit_pending"):
            if world != state["origin"]:
                raise ValueError(
                    "Banking phase left its original town; no input repeated"
                )
        ...
        if phase == "banked":
            ...
            if state["origin"] == 1036:
                ...
                if source["map_id"] != state["origin"]:
                    raise ValueError(
                        "Banked prep changed maps before the saved fare; no input issued"
                    )
                _payload_is_safe(source, state)
                from conquest.meteor_banking import trip

                def before_submit():
                    source, merchant = pair(
                        ui, state["character"], farmer_preflight=True
                    )
                    _same_participant(merchant, state["merchant"], "Merchant")
                    if source["map_id"] != state["origin"]:
                        raise ValueError(
                            "Farmer left the fare origin before submission"
                        )
                    _payload_is_safe(source, state)
        ...
        if phase in ("market", "approaching"):
            ...
            def prep_send(body):
                if body.get("action") == "delivery-target" and set(body) == {
                    "action",
                    "character",
                }:
                    return send(
            ...
            target = prep_send(
                {"action": "delivery-target", "character": state["character"]}
            )
            plan = {
                "merchant": state["character"],
                "position": target["merchant_position"],
            }
            ...
            if not approach_merchant(loop, plan, prep_send, deadline=time.time() + 30):
                raise ValueError(
                    "Could not reach a memory-actionable position near the merchant"
                )
            source, merchant = pair(ui, state["character"])
            if state.get("origin") == 1036:
                _validate_market_no_banking_proof(state, source, merchant)
            ...
            if (
                target.get("farmer_position") != source.get("position")
                or target.get("merchant_position") != merchant.get("position")
                or not within_delivery_probe_range(
                    source.get("position"), merchant.get("position")
                )
            ):
                raise ValueError(
                    "Trade-probe distance changed after final target projection"
                )
            _save(
                state,
                "completed",
                completed_at=time.time(),
                farmer_position=source["position"],
                merchant_position=merchant["position"],
            )
    finally:
...
def start(ui, character, *, selected_uid, send=request):
    ...
    with _START_LOCK:
        ...
        if old and old.get("phase") not in TERMINAL:
            if (
                old.get("character") != character
                or old.get("selected_uid") != selected_uid
            ):
                raise ValueError(
                    "Resume or reconcile the existing trade qualification prep first"
                )
            ...
            if (
                not route
                or not route.get("outbound", {}).get("verified")
                or route["outbound"].get("source_map") != source["map_id"]
                or route["outbound"].get("destination_map") != 1036
            ):
                raise ValueError(
                    "No verified memory-qualified route from this town to Market"
                )
            _archive(old)
            ...
            state = {
                "phase": "prepared",
                "request_id": "trade-prep:" + uuid.uuid4().hex,
        ...
        thread = threading.Thread(
            target=work, daemon=True, name="trade-qualification-prep"
        )
