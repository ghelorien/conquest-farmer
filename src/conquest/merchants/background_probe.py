...
def code_evidence(adapter, base):
    ...
    matches = {
        hex(start): adapter.read_block(base + start, size) == data[start : start + size]
    ...
    while stack:
        ...
        if ptr == head:
            continue
        if ptr in seen or len(seen) >= count:
            raise ValueError("Unstable window-handler registry")
        seen.add(ptr)
        row = adapter.read_block(ptr, 56)
        left, right = (
            struct.unpack_from("<Q", row)[0],
            struct.unpack_from("<Q", row, 16)[0],
        )
        key, model = (
            struct.unpack_from("<I", row, 32)[0],
            struct.unpack_from("<Q", row, 40)[0],
        )
        ...
        handlers.append(
            {
                "key": key,
                "vtable_rva": hex(vtable - base),
                "handler_rva": hex(handler - base),
            }
        )
        rva = handler - base
        ...
        matches[hex(rva)] = (
            adapter.read_block(handler, 0x800) == data[rva : rva + 0x800]
        )
        stack.extend((left, right))
    if len(seen) != count or adapter.read_block(base + 0x6986C0, 16) != struct.pack(
        "<QQ", head, count
    ):
        raise ValueError("Window-handler registry changed during observation")
    return {
        "code_matches": matches,
        "game_state": struct.unpack("<I", adapter.read_block(base + 0x663B4C, 4))[0],
        ...
        if attempted and not report.get("ctrl_release_verified"):
            ...
            try:
                ...
                if process is not None and process.poll() is None:
                    raise ValueError(
                        "Ctrl helper still running; release table state is unknown"
                    )
...
def settle_input(
    reader, *, include_keys=False, timeout=2, clock=time.monotonic, sleep=time.sleep
):
    ...
    while clock() < deadline:
        ...
        previous = frame
        neutral = (
            not state["queue"]["size"]
            and not state["backend"]["buttons_down"]
            and not any(state["mouse_down"])
            and not state["active"]["id"]
            and not any(state["modifiers"].values())
            and not state.get("key_mods", 0)
        )
        if include_keys:
...
def reconciliation_read(driver, *, timeout=0.5, clock=time.monotonic, sleep=time.sleep):
    ...
    while True:
        ...
        try:
            ...
            transient = (
                isinstance(error, GuiObservationChanged)
                or str(error) == "Invalid GUI geometry"
            )
...
def cleanup_probe(ui, character, report, reader, driver, before):
    ...
    sent = any(
        report.get(k)
        for k in ("input_messages_sent", "button_messages_sent", "key_messages_sent")
    )
    if sent:
        try:
            report["input_cleanup"] = settle_input(
                reader, include_keys=bool(report.get("key_messages_sent"))
            )
    ...
    try:
        ...
        report["final_reconciliation"] = {
            ...
            "stock_unchanged": report["stock_unchanged"],
            "position_unchanged": report["position_unchanged"],
        }
    except Exception as error:
    ...
    changed = (
        report["stock_unchanged"] is not True
        or report["position_unchanged"] is not True
    )
    if unsettled or changed:
        note = (
            unsettled
            or "Final background diagnostic stock/position verification requires attention"
        )
        ...
        try:
            ui.runtime.journal.set(
                character,
                "attention",
                {"kind": "background_input_cleanup", "note": note},
            )
        except Exception:
            report["attention_persistence_error"] = True
    elif sent:
        try:
            if (ui.runtime.journal.get(character, "attention") or {}).get(
                "kind"
            ) == "background_input_cleanup":
...
def run_probe(ui, character, mode, cancel):
    ...
    with (
        ...
        windows = [
            w for w in before["windows"] if w["name"] == "Inventory/##ItemGrid_A800F95C"
        ]
        if len(windows) != 1:
        ...
        if (
            not left + 2 < logical[0] < right - 2
            or not top + 2 < logical[1] < bottom - 2
        ):
            raise ValueError("Inventory hover target is clipped")
        point = tuple(
            round(v * n / g)
            for v, n, g in zip(logical, original["client_size"], viewport)
        )
        if not all(
            0 <= p < min(n, 32768) for p, n in zip(point, original["client_size"])
        ):
        ...
        def check():
            ...
            if (
                current["client_size"] != original["client_size"]
                or current["minimized"]
                or observer.adapter.identity != identity
                or gui.GetWindow(target.hwnd, 4) != owner
            ):
                raise ValueError("Diagnostic target changed")
            if current["foreground"] in (target.hwnd, owner, conquest_hwnd):
                raise ValueError(
                    "User activated the game or Conquest; background diagnostic stopped"
                )
            if (
                gui.GetAncestor(gui.WindowFromPoint(tuple(current["cursor"])), 2)
                == target.hwnd
            ):
                raise ValueError(
                    "Physical pointer reached the target game; diagnostic stopped"
                )
        ...
        def sample(stage):
            ...
            ui.background_probe.update(
                {
                    "stage": stage,
                    "frame": observed["frame"],
                    "button_down": any(observed["mouse_down"]),
                    "active_control": observed["active"]["id"],
                }
            )
        ...
        report = {
            ...
            "mode": mode,
            "identity": identity,
            "initial_stock": stock(before),
            "initial_position": before["position"],
            "viewport": viewport,
            "point": point,
            "target_window": window["address"],
            "samples": samples,
            "qualified": False,
            "button_messages_sent": False,
        }
        ...
        report["code_evidence"] = code_evidence(
            observer.adapter, driver.memory.gui.base
        )
        report["click_code_audit"] = {
            hex(rva): observer.adapter.read_block(
                driver.memory.gui.base + rva, size
            ).hex()
        ...
        try:
            ...
            if input_mode == "ctrl":
                ...
                if (
                    ...
                    raise ValueError(
                        "Ctrl probe requires idle target keyboard, buttons and controls"
                    )
                if (
                    code_evidence(observer.adapter, driver.memory.gui.base)
                ...
                isolated_ctrl_probe(
                    ...
                    samples,
                    report,
                    scope_factory=BackgroundControlScope,
                )
            elif input_mode in (
                "barrier-hover",
                "barrier-burst-hover",
                ...
                if (
                    ...
                    raise ValueError(
                        "Input handlers or game state changed during baseline"
                    )
                allowed = {
                    0x62D00,
                    0xFA2E0,
                ...
                if report["code_evidence"]["game_state"] != 1010 or any(
                    int(row["handler_rva"], 16) not in allowed
                    for row in report["code_evidence"]["window_handlers"]
                ):
                    raise ValueError(
                        "Character barrier is not qualified for these message handlers"
                    )
                state = reader.snapshot()
                ctx = state["context"]
                ...
                if (
                    ...
                    raise ValueError(
                        "Character barrier requires idle game keys, buttons, controls and queue"
                    )
                if any(w["name"] == "Add Item to Booth" for w in before["windows"]):
                    raise ValueError(
                        "Finish the existing price dialog before a character barrier test"
                    )
                pairs = (
                    12 if input_mode in ("barrier-burst-hover", "barrier-click") else 1
                )
                if input_mode in ("drag-cancel", "price-cancel"):
                    pairs = 0
                ...
                if input_mode == "barrier-click":
                    ...
                    while time.monotonic() < deadline:
                        ...
                        state = samples[-1]["gui"]
                        if (
                            state["hover"]["window"] == window["address"]
                            and state["hover"]["id"]
                            and state["want_capture_mouse"]
                            and not state["active"]["id"]
                        ):
                    ...
                    if expected_id is None:
                        raise ValueError(
                            "Inventory control did not acknowledge hover before click"
                        )
                    fresh = driver.read()
                    if (
                    ...
                    if (
                        state["hover"]
                        != {"window": window["address"], "id": expected_id}
                    ...
                    try:
                        ...
                        while time.monotonic() < deadline:
                            ...
                            state = samples[-1]["gui"]
                            if (
                                state["active"]["id"] == expected_id
                                and state["mouse_down"][0]
                            ):
                        ...
                        if not report.get("press_verified"):
                            raise ValueError(
                                "Background inventory press was not acknowledged"
                            )
                        check()
                        ...
                        while time.monotonic() < deadline:
                            state = reader.snapshot()
                            if not state["backend"]["buttons_down"] and not any(
                                state["mouse_down"]
                            ):
                                report["release_verified"] = True
                                break
                        ...
                        if not report.get("release_verified"):
                            raise ValueError(
                                "Background button release requires attention"
                            )
                ...
                report["input_messages_sent"] = True
                if not send(
                    target.hwnd, 0x200, 0, packed, 0x2 | 0x20, 300, ctypes.byref(reply)
                ):
                ...
                if input_mode == "cancel-hover":
                    ...
                    track = bind(
                        target.backend.user,
                        "TrackMouseEvent",
                        [ctypes.POINTER(Tracking)],
                        wintypes.BOOL,
                    )
                    operation = Tracking(
                        ctypes.sizeof(Tracking), 0x80000002, target.hwnd, 0
                    )
                    report["tracking_cancel_accepted"] = bool(
                        track(ctypes.byref(operation))
                    )
            ...
            report["stock_unchanged"] = stock(before) == stock(after)
            report["position_unchanged"] = before["position"] == after["position"]
            report["hover_frames"] = sorted(
                {
                    s["gui"]["frame"]
                    for s in samples
                    if s["stage"] == "after"
                    and s["gui"]["hover"]["window"] == window["address"]
                    and s["gui"]["hover"]["id"]
                }
            )
