...
def project(point, matrices, viewport):
    ...
    for matrix in matrices:
        ...
        values = [sum(values[i] * matrix[i * 4 + j] for i in range(4)) for j in range(4)]
    ...
    return (x + (values[0] / values[3] + 1) * width / 2,
...
def flag_target(observer, flag):
    ...
    a = flag['address']
    if (flag.get('name') != 'ShopFlag' or flag.get('model') != 1086 or flag.get('type_id') != 0
    ...
    if (not all(math.isfinite(v) and abs(v) < 1000 for v in bounds)
            or any(not bounds[i] < bounds[i + 3] for i in range(3))):
        raise ValueError('Invalid flag collision bounds')
    matrices = [read(model + 0xc, '<16f'), read(base + 0x676750, '<16f'), read(base + 0x676790, '<16f')]
    ...
    if (read(base + 0x6545a0, '<I')[0] != 1 or viewport[:2] != (0, 0)
            or viewport[4:] != (0., 1.) or (width, height) != (camera_width, camera_height)
            or not 300 <= min(width, height) <= max(width, height) <= 16384
            or not 1 <= zoom <= 1024 or not 1 <= percent <= 400):
        raise ValueError('Unsupported flag viewport or camera')
    origin = project((0., 0., 0.), matrices, viewport)
    draw = read(a + 0xf8, '<2i')
    if list(draw) != flag['draw_position'] or any(abs(p - q) > 1 for p, q in zip(origin, draw)):
    ...
    point = tuple(round((v - d // 2) * scale + d // 2) for v, d in zip(unscaled, (width, height)))
