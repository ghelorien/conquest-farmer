...
def partial_offer(intent, farmer, merchant):
    ...
    if not trade:
        raise ValueError("Farmer trade is not open")
...
def _recipient_record(observer, profile, merchant, *, targeting=False):
    ...
    offsets = [
        spec[k] for k in ("uid_offset", "name_offset", "position_offset", "draw_offset")
    ]
    ...
    headers = [
        (collection + o, "u64")
        for o in (p.begin_offset, p.end_offset, p.capacity_offset)
    ]
    ...
    objects = [
        struct.unpack_from("<Q", entries, i + 8)[0] for i in range(0, len(entries), 16)
    ]
    if len(set(objects)) != len(objects):
        raise ValueError("Ambiguous scene object ownership")
    ...
    for obj, kind in zip(objects, types):
        ...
        if struct.unpack_from("<Q", raw)[0] != vtable:
            raise ValueError("Receiver object type changed")
        ...
        ranges = [
            ...
            (offsets[1], offsets[1] + length),
            (offsets[2], offsets[2] + 8),
            (offsets[3], offsets[3] + 8),
        ]
    ...
    if (
        sample_fields(s, headers) != header
        or (end > begin and s.read_block(begin, end - begin) != entries)
        or any(
            s.read_block(obj, span)[offsets[2] : offsets[2] + 8]
            != raw[offsets[2] : offsets[2] + 8]
            for obj, raw in occupied_blocks
        )
    ...
    if time.monotonic() - started > 0.5:
        raise CaptureUnavailable("Receiver observation expired")
    if not matches:
        raise RecipientAbsent(
            "Receiver UID is absent from the farmer scene",
...
def recipient_actionability(
    observer, profile, merchant, *, farmer=None, targeting=False
):
    ...
    result = target_actionability(
        record["point"],
        profile["gui_size"],
        profile.get("client_size", profile["gui_size"]),
...
def recipient_record(observer, profile, merchant, *, farmer=None, targeting=False):
    result = recipient_actionability(
        observer, profile, merchant, farmer=farmer, targeting=targeting
    )
...
def recipient_binding(record):
    ...
    if (
        any(
            ...
            for key in ("address", "uid")
        )
        or not isinstance(record["name"], str)
        or not record["name"]
        or any(
            not isinstance(record[key], (list, tuple))
            or len(record[key]) != 2
...
class FarmerTradeDriver:
    ...
    def _before_action(self, stage):
        if self.operation is not None:
            self.operation.before_action(stage)

    def _action_observed(self, stage, evidence=None):
        if self.operation is not None:
            self.operation.action_observed(stage, evidence=evidence)

    def report(self, activity, state="running"):
        self.ui.app.messages.put(
            (
                "automation_work",
                {
                    "state": state,
    ...
    def require_qualified(self, capability="farmer_delivery"):
        ...
        for key in ("recipient", "target_mode", "gui_size", "controls"):
            if not profile.get(key):
                raise ValueError("Farmer delivery layout qualification is incomplete")
        for control in (
            "start_trade",
            "open_inventory",
            "inventory_item",
            ...
            if control not in profile["controls"]:
                raise ValueError("Farmer trade control is not qualified")
        if profile.get("native_trade_layout_revision") == 1:
            profile = {
                **profile,
                "client_size": self.driver.target.snapshot()["client_size"],
    ...
    def check(self):
        ...
        if grant and grant["expires_at"] <= time.time():
            raise CaptureUnavailable(
                "Merchant delivery work window expired; reconcile before further input"
            )
        if self.ui.closed:
            raise CaptureUnavailable(
                "Farmer delivery input permission changed or expired: ui_closed"
            )
        if state["enabled"]:
            raise CaptureUnavailable(
                "Farmer delivery input permission changed or expired: farming_enabled"
            )
        if state.get("paused"):
            raise CaptureUnavailable(
                "Farmer delivery input permission changed or expired: farming_paused"
            )
        if state["revision"] != self.revision:
            raise CaptureUnavailable(
                "Farmer delivery input permission changed or expired: control_revision_changed"
            )
        if not self.ui.safe_to_yield():
            raise CaptureUnavailable(
                "Farmer delivery input permission changed or expired: safe_handoff_unavailable"
            )
        ...
        with ExitStack() as stack:
            ...
            while True:
                try:
                    self.check()
                    stack.enter_context(
                        self.ui.coordinator.lease("Farmer", purpose="farmer_delivery")
                    )
                    break
                    ...
                    if (
                        str(error)
                        not in (
                            "Another character owns game input",
                            "Waiting for input owner",
                            "Waiting for the current input action",
                            "Another app owns merchant input",
                        )
            ...
            from conquest.merchants.delivery_farmer_surface import (
                prepare_delivery,
                verify_stage_pair,
            )

            presentation = prepare_delivery(
                self, intent, stage=stage, deadline=time.monotonic() + 3
            )
            f, m = self.read_pair(intent["merchant"]["character"])
            verify_stage_pair(intent, f, m, stage=stage)
            presentation()
            self.check()
            from conquest.focus_recovery import activate_client

            if not activate_client(
                self.driver.target.hwnd, self.driver.observer.adapter.identity
            ):
                raise CaptureUnavailable(
                    "Farmer focus unavailable; no delivery input sent"
                )
            yield
    ...
    def button(self, intent, control, guard, *, stage=None):
        ...
        profile = self.require_qualified()
        snapshot = self.driver.read()
        point = self.driver.point(snapshot, control)
        spec = profile["controls"][control]
        if not spec.get("label"):
            raise ValueError("Trade button hover identity is not qualified")
        layout = self.driver.layout_revision()
        layout_revision = layout.stable()

        def before():
            self.check()
            f, m = self.read_pair(intent["merchant"]["character"])
            guard(f, m)
            if self.driver.point(f, control) != point:
                raise ValueError("Trade control moved")
            if spec.get("mode") == "native_trade_confirm":
                ...
                seeds = (
                    [0x02A99238] if spec.get("mode") == "native_items_trade" else None
                )
                self.driver.memory.gui.assert_hovered(
                    window, spec["label"], seeds=seeds
                )
            layout.assert_current(layout_revision)
            if stage:
                self._before_action(stage)

        foreground_click(
            self.driver.target,
            *point,
            tuple(profile["client_size"]),
    ...
    def target_status(self, merchant):
        ...
        profile = self.require_qualified()
        farmer, receiver = self.read_pair(merchant)
        try:
            result = recipient_actionability(
                self.driver.observer, profile, receiver, farmer=farmer
            )
        except RecipientAbsent as error:
            result = None
            absent_occupied = error.occupied_tiles
        ...
        life = MemoryLifeReader.for_session(
            self.driver.observer.adapter, self.driver.observer.character
        ).read()
        ...
        if result is None:
            return {
                ...
                "merchant": receiver["character"],
                "merchant_position": receiver["position"],
                "point": None,
                "viewport": list(profile["gui_size"]),
                ...
                "occupied_tiles": list(
                    map(
                        list,
                        dict.fromkeys(
                            map(tuple, [farmer["position"], *absent_occupied])
                        ),
                    )
                ),
            }
        ...
        return {
            "schema_version": 1,
            "ready": result["actionable"],
            "reason": result["reason"],
            "character": farmer["character"],
            "farmer_position": farmer["position"],
            "merchant": receiver["character"],
            "merchant_position": receiver["position"],
            "point": result["recipient"]["point"],
            "viewport": list(profile["gui_size"]),
            "client_size": list(profile["client_size"]),
            "anchor": list(anchor),
    ...
    def open_trade(self, intent):
        ...
        def unchanged(f, m):
            ...
            for role, current in (("farmer", f), ("merchant", m)):
                if (
                    current["identity"] != intent[role]["identity"]
                    or current["character_uid"] != intent[role]["character_uid"]
                    or exact_items(current["inventory"])
                    != exact_items(intent[role]["inventory"])
                ):
                    raise ValueError(
                        "Delivery participants or stock changed before request"
                    )
        ...
        with self.action(intent, stage="open"):
            ...
            f, m = self.read_pair(intent["merchant"]["character"])
            unchanged(f, m)
            recipient_record(self.driver.observer, profile, m, farmer=f)
            ...
            while True:
                self.check()
                targeting_f, targeting_m = self.read_pair(m["character"])
            ...
            point = tuple(
                round(v * p / g)
                for v, p, g in zip(
                    recipient["point"], profile["client_size"], profile["gui_size"]
                )
            )
            layout = self.driver.layout_revision()
            layout_revision = layout.stable()
            ...
            def before():
                ...
                while True:
                    self.check()
                    fresh_f, fresh_m = self.read_pair(m["character"])
                    unchanged(fresh_f, fresh_m)
                    try:
                        fresh_recipient = recipient_record(
                            self.driver.observer,
                            profile,
                            fresh_m,
                            farmer=fresh_f,
    ...
    def place_item(self, intent, item):
        ...
        with self.action(intent, stage="place"):
            f, m = self.read_pair(intent["merchant"]["character"])
            placed = partial_offer(intent, f, m)
            if item["uid"] not in exact_items(intent["items"]):
                raise ValueError("Item is outside reserved batch")
            if item["uid"] in exact_items(placed):
                raise ValueError("Item already offered; no repeat drag")
            spec = self.require_qualified()["controls"]["inventory_item"]
            if not any(w["name"] == spec["window"] for w in f["windows"]):
                self.button(
                    intent, "open_inventory", lambda f, m: partial_offer(intent, f, m)
                )
                f, m = self.read_pair(m["character"])
                partial_offer(intent, f, m)
            current = next((i for i in f["inventory"] if i["uid"] == item["uid"]), None)
            if not current or exact_items([current]) != exact_items([item]):
                raise ValueError("Reserved item changed")
            source = self.driver.point(f, "inventory_item", current["slot"])
            destination = self.driver.point(f, "trade_drop")
            layout = self.driver.layout_revision()
            layout_revision = layout.stable()

            def before():
                self.check()
                fresh, receiver = self.read_pair(m["character"])
                if exact_items(partial_offer(intent, fresh, receiver)) != exact_items(
                    placed
                ):
                    raise ValueError("Partial offer changed before drag")
                same = next(
                    (i for i in fresh["inventory"] if i["uid"] == current["uid"]), None
                )
                if (
                    not same
                    or exact_items([same]) != exact_items([current])
                    or same["slot"] != current["slot"]
                    or self.driver.point(fresh, "inventory_item", same["slot"])
                    != source
                ...
                window = next(
                    w for w in fresh["windows"] if w["name"] == spec["window"]
                )
            ...
            foreground_drag(
                ...
                source,
                destination,
                tuple(self.require_qualified()["client_size"]),
                before_press=lambda: wait_hover_validation(before, self.check),
                layout_guard=lambda: layout.assert_current(layout_revision),
            )
        self.wait_until(
            m["character"],
            lambda f, m: item["uid"] in exact_items(partial_offer(intent, f, m)),
        )
        self._action_observed(
            "offer_item:" + str(item["uid"]), {"offered_uid": item["uid"]}
        )

    def confirm(self, intent):
        self.report(
            "Confirming the exact item transfer to " + intent["merchant"]["character"]
        )
        with self.action(intent, stage="confirm"):
            f, m = self.read_pair(intent["merchant"]["character"])
            validate_offers(intent, f, m)
            self.button(
                intent,
                "confirm_trade",
                lambda f, m: validate_offers(intent, f, m),
                stage="farmer_confirm",
            )
    ...
    def wait_pair(self, merchant):
        ...
        return self.wait_until(
            ...
            lambda f, m: (
                not f.get("trade")
                and not m.get("trade")
                and not f.get("request")
                and not m.get("request")
            ),
        )
