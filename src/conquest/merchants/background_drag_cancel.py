...
def _geometry(driver, snapshot, item, client_size, viewport):
    control, table = grid_control(driver.memory.gui, snapshot, GRID, '##ItemTable', (20, 20))
    win = window(snapshot, GRID)
    if control['stride'] != [40., 40.]:
    ...
    for px, py in logical:
        if (not left + 2 < px < right - 2 or not top + 2 < py < bottom - 2
                or not wx + 2 < px < wx + width - 2 or not wy + 2 < py < wy + height - 2):
            raise ValueError('Drag source cell is clipped')
    points = [tuple(round(v * n / g) for v, n, g in zip(p, client_size, viewport))
              for p in logical]
    if any(not 0 <= p < min(n, 32768) for point in points for p, n in zip(point, client_size)):
        raise ValueError('Invalid background drag coordinates')
    return {'window': win['address'], 'geometry': list(win['geometry']),
            'scroll': list(win['scroll']), 'table': table, 'points': points}
...
def run_drag_cancel(driver, before, reader, report, sample, check):
    ...
    if before.get('trade') or before.get('request') or any(
            w['name'] == 'Add Item to Booth' for w in before['windows']):
    ...
    for candidate in before['inventory']:
        ...
        break
    if item is None:
        raise ValueError('No visible verified unbound inventory item for drag diagnostic')
    origin, displaced = geometry['points']
    report['drag_source'] = {'uid': item['uid'], 'slot': item['slot'],
                             'window': geometry['window'], 'origin': origin, 'displaced': displaced}
    attempted = False
    ...
    def fresh():
        ...
        if (len(matches) != 1 or matches[0] != item or stock(current) != original_stock
                or current['position'] != before['position'] or current.get('trade') or current.get('request')
    ...
    burst(origin, 0)
    hovered = wait('drag-prime', lambda s: s['hover']['window'] == geometry['window']
                   and s['hover']['id'] and s['want_capture_mouse'] and not s['active']['id'])
    expected_id = hovered['hover']['id']
    report['drag_source']['control_id'] = expected_id
    fresh()
    state = reader.snapshot()
    if (state['hover'] != {'window': geometry['window'], 'id': expected_id}
    ...
    try:
        ...
        wait('drag-press', lambda s: s['active']['id'] == expected_id and s['mouse_down'][0])
        report['drag_press_verified'] = True
        fresh()
        burst(displaced, 1)
        state = wait('drag-held', lambda s: s['drag']['active'])
        if (state['drag']['payload_type'] != 'CQITEM' or state['drag']['source_item_uid'] != item['uid']
                or state['drag']['source_id'] != expected_id or not state['mouse_down'][0]
                or state['drag']['delivery']):
            raise ValueError('Drag payload did not match the exact source inventory item')
        ...
        def returned(s):
            ...
            logical_origin = [v * g / n for v, g, n in zip(origin, viewport, client_size)]
            return (s['drag']['active'] and s['drag']['source_item_uid'] == item['uid']
                    and not s['drag']['delivery'] and s['mouse_down'][0]
        ...
        attempted = False
        wait('drag-release', lambda s: not s['drag']['active'] and not s['active']['id']
             and not s['backend']['buttons_down'] and not any(s['mouse_down']), timeout=1.2)
