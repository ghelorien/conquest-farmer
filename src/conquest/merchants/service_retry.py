...
def route_retry(loop, visit, send):
    ...
    loop.check_stop()
    loop.record(
        "merchant_service_retry_check",
        activity="Checking the unsubmitted acceptance delivery before retry",
    )
    result = send(
        {
            ...
            "route_pid": loop.state["pid"],
            "route_started_at": loop.state["started_at"],
        }
    )
    ...
    loop.record(
        "merchant_service_retry",
        visit_id=fresh["visit_id"],
        previous_visit_id=visit["visit_id"],
...
def _deferred_attempts(visit):
    return isinstance(visit, dict) and all(
        isinstance(attempt, dict) and attempt.get("outcome") == "deferred_before_input"
...
def _same_origin(origin, visit, *, visit_ids=()):
    return bool(
        ...
        and (
            origin.get("visit_id") in set(visit_ids) | {visit["visit_id"]}
            or visit.get("town_visit_id")
            and origin.get("town_visit_id") == visit["town_visit_id"]
        )
    )
...
def _unadmitted_journals(ui, visit, *, visit_ids=()):
    ...
    if (
        route.get("active")
        or route.get("cleanup_pending")
        or any(
            _same_origin(r, visit, visit_ids=visit_ids)
            for r in route.get("operations", []) + route.get("receipts", [])
        )
    ):
    ...
    if delivery_operation.JOURNAL.exists():
        with closing(
            ...
            admissions = list(
                db.execute("SELECT phase,origin_json FROM delivery_admissions")
            )
            ...
            transactions = list(
                db.execute(
                    "SELECT phase,before_json FROM transactions WHERE kind='farmer_delivery'"
                )
            )
            if any(
                r[0] not in ("verified", "aborted", "operator_overridden")
    ...
    with ui.runtime.journal.db() as db:
        reservations = [
            json.loads(r[0])
            for r in db.execute("SELECT state FROM delivery_reservations")
        ]
    if any(
        r.get("phase") not in delivery_reservation.TERMINAL
        or _same_origin(r.get("intent", {}), visit, visit_ids=visit_ids)
...
def _idle(
    ui, visit, body, row, *, visit_ids=(), window_request_id=None, cleared_record=None
):
    ...
    if (
        ROUTE_STOP.exists()
        or route.get("phase") != "restocking"
        or route.get("route") != row["route_id"]
        or route.get("town_visit_id") != visit["town_visit_id"]
        or route.get("pid") != body["route_pid"]
        or route.get("started_at") != body["route_started_at"]
        or not 0 <= time.time() - route.get("updated_at", 0) <= 5
        or process_alive(route.get("pid")) is not True
    ):
        raise ValueError(
            "Market service retry requires the exact active native town controller"
        )
    ...
    if cleared_record is not None:
        if (
            not isinstance(cleared_record, dict)
            ...
            raise ValueError(
                "Market service stale clear does not match its exact old window"
            )
    elif window_request_id is None and (
        window.get("visit_id") in set(visit_ids) | {visit["visit_id"]}
    ...
    if any(
        s.get("pending")
        or s.get("input_active")
        or s.get("manual_input_fence")
...
def _terminal_idle(ui, visit, body, row, request_id):
    ...
    window = WorkWindows().state()
    if (
        window.get("request_id") != request_id
        or window.get("visit_id") != visit["visit_id"]
        or window.get("town_visit_id") not in (None, visit.get("town_visit_id"))
        or window.get("deadline") != visit.get("deadline")
        or (
            window.get("phase") in ("preparing", "working")
            and (
                not isinstance(window.get("deadline"), (int, float))
                or time.time() < window["deadline"]
            )
        )
        or (
            window.get("phase") not in ("preparing", "working")
            and not isinstance(window.get("finished_at"), (int, float))
        )
    ):
        raise ValueError(
            "Terminal delivery retry requires its exact closed Market window"
        )
    return statuses
...
def _canonical_terminal(ui, visit, cycle):
    ...
    if (
        ...
        or not admissions
        or any(not isinstance(row, dict) for row in admissions)
    ):
    ...
    route_history = [
        record
        for record in route.get("operations", []) + route.get("receipts", [])
        if isinstance(record, dict) and _same_origin(record, visit)
    ]
    if any(
        record.get("request_id") not in request_ids for record in route_history
    ) or len({record.get("request_id") for record in route_history}) != len(
        route_history
    ):
        raise ValueError("Acceptance route history is ambiguous")
    if not route_history or route_history[-1].get("request_id") != admissions[-1].get(
        "request_id"
    ):
    ...
    with source_journal.db() as db:
        ...
        admissions_history = [
            row[0] for row in db.execute("SELECT phase FROM delivery_admissions")
        ]
    ...
    if any(
        row["id"] not in request_ids
        or row["phase"] not in ("aborted", "verified", "operator_overridden")
        for row in same_source
    ) or len({row["id"] for row in same_source}) != len(same_source):
    ...
    if any(
        record.get("phase") not in delivery_reservation.TERMINAL
        for record in receiver_history
    ):
    ...
    if any(
        record.get("request_id") not in request_ids
        or record.get("phase")
        not in ("no_transfer_reconciled", "partial_aborted_reconciled", "verified")
        for record in same_receiver
    ) or len({record.get("request_id") for record in same_receiver}) != len(
        same_receiver
    ):
    ...
    for admission in admissions:
        ...
        operation = next(
            (
                record
                for record in route.get("operations", [])
                if isinstance(record, dict) and record.get("request_id") == key
            ),
            None,
        )
        if (
            ...
            or receipt.get("outcome") != "no_transfer"
            or receipt.get("next_action") != "retry_delivery"
            or receipt.get("cleanup_pending")
            or operation.get("outcome") != "no_transfer"
            or operation.get("next_action") != "retry_delivery"
            or operation.get("items")
            ...
            or any(
                receipt.get(name) != admission.get(name)
                for name in ("visit_id", "town_visit_id", "farmer_profile_id")
            )
        ):
            raise ValueError(
                "Acceptance delivery lacks a canonical no-transfer receipt"
            )
        with ui.runtime.journal.db() as db:
            row = db.execute(
                "SELECT state FROM delivery_reservations WHERE character=? AND request_id=?",
                (receipt["character"], key),
            ).fetchone()
        receiver = json.loads(row[0]) if row else {}
        if (
            receiver.get("phase") != "no_transfer_reconciled"
            or (receiver.get("disposition") or {}).get("outcome") != "no_transfer"
            or (receiver.get("disposition") or {}).get("proof_digest")
            != receipt["proof_digest"]
        ):
            raise ValueError(
                "Receiver no-transfer receipt differs from the source operation"
            )
    ...
    if admission.get("merchant") != receipt.get("character"):
        raise ValueError(
            "Latest acceptance admission differs from its source operation"
        )
...
def _terminal_retry(ui, body, row, cycle, visit, visit_store):
    ...
    admission_controller = admission.get("controller")
    operation_controller = operation.get("controller")
    if admission_controller != operation_controller:
        raise ValueError(
            "Latest failed admission controller differs from its route operation"
        )
    if admission_controller is not None:
        ...
        if provenance and (
            ...
            raise ValueError(
                "Latest failed admission retry provenance differs from its route controller"
            )
        prior_controller = admission_controller
    else:
        if len(provenance) != 1 or not isinstance(
            provenance[0].get("controller"), dict
        ):
    ...
    if prior_controller == controller:
        raise ValueError(
            "Terminal delivery retry requires a fresh native town controller"
        )
    previous = next(
        (
            record
            for record in _terminal_retry_records(cycle)
            if record.get("controller") == controller
            and record.get("previous_visit", {}).get("visit_id") == body["visit_id"]
        ),
        None,
    )
    if previous:
        ...
        raise ValueError(
            "Terminal service retry acknowledgement conflicts with the sealed visit"
        )
    ...
    if any(
        record.get("source_request_id") == receipt["request_id"]
        for record in _terminal_retry_records(cycle)
    ):
        raise ValueError(
            "This canonical no-transfer operation already used its retry budget"
        )
    if (
        visit.get("phase") != "active"
        or visit.get("visit_id") != body["visit_id"]
        or type(visit.get("deadline")) not in (int, float)
        or time.time() < visit["deadline"]
    ):
        raise ValueError(
            "Terminal delivery retry requires its exact expired Market visit"
        )
    ...
    if (
        ...
        raise ValueError(
            "Terminal delivery retry requires unchanged acceptance journey ownership"
        )
    ...
    wanted = exact_items([cycle["item"]])
    if source.get("map_id") != 1036 or any(
        inventory.get(uid) != value for uid, value in wanted.items()
    ):
        raise ValueError(
            "Terminal delivery retry requires the exact carried item in fresh Market memory"
        )
    merchant = statuses.get(admission["merchant"], {})
    snapshot = merchant.get("snapshot") or {}
    if (
        not acceptance.merchant_allowed(admission["merchant"], merchant, snapshot)
        ...
        raise ValueError(
            "Terminal delivery retry requires an idle original acceptance merchant"
        )
    if ui.app.control.snapshot() != control:
        raise ValueError(
            "Farmer control changed while checking the terminal service retry"
        )
    from conquest.merchants.handoff import WorkWindows

    windows = WorkWindows()
    windows.finish("terminal_no_transfer_reconciled")
    closed = windows.state()
    if (
        closed.get("request_id") != receipt["request_id"]
        or closed.get("visit_id") != visit["visit_id"]
    ...
    retry = {
        ...
        "source_proof_digest": receipt["proof_digest"],
        "previous_visit": deepcopy(visit),
    ...
    retry["visit"] = {
        **visit,
        ...
        "attempts": [],
        "retry_of": visit["visit_id"],
        "retry_of_operation_id": receipt["request_id"],
        "retry_of_proof_digest": receipt["proof_digest"],
    }
    retry["visit"]["deadline"] = (
        retry["visit"]["started_at"] + service_visit.MARKET_SECONDS
    )

    def seal(current):
        if current != row:
            raise ValueError(
                "Acceptance changed while sealing its terminal service retry"
            )
        current["active"]["terminal_service_retries"] = [
            *_terminal_retry_records(cycle),
            deepcopy(retry),
        ]
        return current
    ...
    if read_json(visit_store.path) != visit:
        raise ValueError(
            "Market visit changed while sealing its terminal service retry"
        )
...
def dispatch(ui, body):
    if (
        set(body)
        != {"action", "visit_id", "run_id", "cycle_id", "route_pid", "route_started_at"}
        or body["action"] != "delivery-service-retry"
    ):
...
def _retry(ui, body):
    ...
    row = acceptance.state()
    cycle = row.get("active") or {}
    visit_store = service_visit.MarketVisit()
    visit = read_json(visit_store.path)
    saved = cycle.get("service_retry")
    if (
        not acceptance.trial_permitted()
        or cycle.get("phase") != "town"
        or row.get("run_id") != body["run_id"]
        or cycle.get("cycle_id") != body["cycle_id"]
        ...
        raise ValueError(
            "Market service retry requires the exact unadmitted acceptance cycle"
        )
    ...
    if saved and saved.get("previous_visit", {}).get("visit_id") == body["visit_id"]:
        ...
        raise ValueError(
            "Market service retry acknowledgement conflicts with the sealed visit"
        )
    history = cycle.get("service_retry_history", [])
    ...
    if (
        ...
        or time.time() < visit["deadline"]
        or any(
            a.get("outcome") != "deferred_before_input"
            for a in visit.get("attempts", [])
        )
        or saved
        and not _retry_visit_matches(saved.get("visit"), visit)
        or any(
            not _deferred_attempts(record.get("previous_visit"))
            or not _deferred_attempts(record.get("visit"))
            for record in records
        )
    ):
    ...
    if (
        ...
        raise ValueError(
            "Market service retry requires unchanged acceptance journey ownership"
        )
    from conquest.merchants.handoff import WorkWindows
    ...
    clear = next(
        (
            ...
            and window.get("pre_admission_stale_clear") == record
        ),
        None,
    )
    ...
    if source.get("map_id") != 1036 or any(
        ...
        raise ValueError(
            "Market service retry requires the exact carried item in fresh Market memory"
        )
    if not any(
        acceptance.merchant_allowed(name, status, status.get("snapshot") or {})
        for name, status in statuses.items()
    ):
        raise ValueError(
            "Market service retry requires a currently qualified acceptance merchant"
        )
    if ui.app.control.snapshot() != control:
    ...
    retry = {
        "previous_visit": deepcopy(visit),
        "farmer_identity": source["identity"],
        "item": deepcopy(cycle["item"]),
        "run_id": row["run_id"],
        "cycle_id": cycle["cycle_id"],
        "controller": controller,
