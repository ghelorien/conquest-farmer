...
def operator_override(
    ...
    *,
    ...
    with runtime.journal.db() as db:
        ...
        row = db.execute(
            "SELECT value FROM state WHERE character=? AND name='connect_market'",
            (character,),
        ).fetchone()
        ...
        if state.get("phase") == "operator_overridden":
            ...
            return state
        if not state or (
            state.get("phase") in TERMINAL
            and not state.get("fare_pending")
            and not state.get("launch_unresolved")
        ):
        ...
        if row:
            db.execute(
                "UPDATE state SET value=? WHERE character=? AND name='connect_market'",
                (encoded, character),
            )
        else:
            db.execute(
                "INSERT INTO state(character,name,value) VALUES(?,?,?)",
                (character, "connect_market", encoded),
            )
    runtime.journal.event(
        character,
        "connect_market_operator_overridden",
        original_evidence_digest=digest,
        confirmation_reference=confirmation_reference.strip(),
    )
    return state
...
def geometry(driver):
    ...
    if observer.adapter.expected_sha256 != CLIENT_SHA256:
        raise ValueError("Client build changed")
    ...
    if life.dead_candidate or life.map_id not in (1002, 1036):
        raise ValueError(
            "Market travel requires a living merchant in Twin City or Market"
        )
    anchor = memory_player_anchor(observer, life)
    gui = driver.memory.gui.viewport_size()
    with physical_coordinates():
        size = list(driver.target.snapshot()["client_size"])
    if (
        len(gui) != 2
        or min(gui) < 300
        or min(size) < 300
        or memory_player_anchor(observer, life) != anchor
        or driver.memory.gui.viewport_size() != gui
    ):
...
def record_capability(driver, capability, evidence, *, dimensions=None):
    ...
    if (
        old.get("client_sha256") != driver.observer.adapter.expected_sha256
        or old.get("character") != driver.observer.character
    ):
        old = {}
    old.update(
        client_sha256=driver.observer.adapter.expected_sha256,
    ...
    if not isinstance(old.get("evidence", {}), dict):
        old["evidence"] = {"previous": old["evidence"]}
...
def qualify_move(driver, travel, before, destination, check):
    ...
    def verify():
        check()
        if geometry(driver) != candidate:
            raise ValueError("Movement trial viewport changed")
        return candidate

    original = travel.qualify_movement
    travel.qualify_movement = verify
    try:
        after = travel.move(before, destination, check)
    finally:
        travel.qualify_movement = original
    ...
    record_capability(
        ...
        "market_return",
        {
            ...
            "source": "pinned actor projection and verified movement",
        },
...
def approach_vacant_flag(driver, travel, check, preferred=None):
    ...
    if initial.get("own_booth_uid"):
        return None
    if (
        initial["map_id"] != 1036
        or initial.get("trade")
        or initial.get("request")
        or initial["booth_open"]
    ):
    ...
    for flag in vacant_flags(driver.observer, spec):
        try:
            distance, target = stall_approach(terrain, initial["position"], flag)
        except ValueError:
            continue
        choices.append((distance, flag["uid"], target))
    if not choices:
        if (
            not isinstance(preferred, (list, tuple))
            or len(preferred) != 2
        ...
        if (
            ...
            or stock(fresh) != stock(initial)
            or fresh.get("own_booth_uid")
            or fresh.get("trade")
        ...
        raise CaptureUnavailable(
            "Approaching saved shop area; recheck vacancy before claiming"
        )
    ...
    for _ in range(8):
        ...
        flag = next(
            (f for f in vacant_flags(driver.observer, spec) if f["uid"] == uid), None
        )
        if flag is None:
            raise CaptureUnavailable("Selected flag is now occupied; approach deferred")
        if max(abs(a - b) for a, b in zip(fresh["position"], target)) <= 1:
            return uid
...
def select_client(ui, character, pid):
    ...
    if character in ui.runtime.observers:
        raise ValueError("This merchant already has an attached client")
    return candidate
...
def run(
    ...
    selected=None,
    market_trial=False,
    stall_inspection=False,
):
    runtime = ui.runtime
    guard = ui.coordinator
    ...
    def allowed():
        return (
            not cancel.is_set()
            and not guard.stopped
            and not ui.closed
            and (not stall_inspection or time.monotonic() - started < 15)
            and ui.app.control.snapshot()["revision"] == revision
            and ui.safe_to_yield()
        )
    ...
    def check():
        ...
        if any(
            ...
            cancel.set()
        if not allowed():
            raise CaptureUnavailable("Market connection cancelled by manual control")
        guard.check()
        if time.monotonic() - started > 600:
            raise ValueError("Market connection exceeded its bounded work time")
    ...
    try:
        ...
        if selected:
            ...
            try:
                if not login_screen(selected.hwnd):
            ...
            save(
                ...
                "client_found",
                launch_unresolved=False,
                client_identity=selected.identity,
                client_hwnd=selected.hwnd,
            )
        if character not in runtime.observers:
            try:
                runtime.attach(character)
            except ValueError:
                if market_trial or stall_inspection:
                    raise ValueError(
                        "Market qualification requires an already connected merchant"
                    )
                ...
                with guard.lease(character, purpose="connect_launch"):
                    check()
                    save(runtime, character, "launching")
                    watch.start()
                    save(
                        runtime,
                        character,
                        "launching",
                        launcher_pid=watch.process.pid,
                        launch_before=[list(key) for key in watch.before],
                        launch_unresolved=True,
                    )
                while watch.pending:
                    check()
                    ...
                    if candidate:
                        runtime.bind(
                            character, runtime.observer_factory(candidate, character)
                        )
                        save(
                            runtime,
                            character,
                            "client_found",
                            launch_unresolved=False,
                            client_identity=candidate.identity,
                            client_hwnd=candidate.hwnd,
                        )
                        break
                    time.sleep(0.2)
                if character not in runtime.observers:
                    raise ValueError("Launcher did not produce one unique client")
        ...
        if (market_trial or stall_inspection) and submitted:
            raise ValueError("Movement qualification cannot submit a login")
        if submitted:
            with guard.lease(character, purpose="connect"):
                ...
                while time.monotonic() < until:
                    if not allowed():
                        raise CaptureUnavailable("Login cancelled by manual control")
                    if guard.manual_active():
                        quiet = None
                    elif quiet is None:
                    ...
                    raise CaptureUnavailable("Login deferred while the mouse is in use")
                check()
                save(runtime, character, "login_pending")
                try:
                    submit_login(
                        driver.target,
                    ...
                    raise ValueError(
                        "Login submission failed; credentials were not logged"
                    ) from None
            until = time.monotonic() + 30
            while login_screen(driver.target.hwnd) and time.monotonic() < until:
                check()
                time.sleep(0.2)
            if login_screen(driver.target.hwnd):
                raise ValueError("Login did not reach the game; no repeated submission")
        from conquest.merchants.transit_life import stable_life as read_life
        ...
        while True:
            ...
            try:
                life = read_life(
                    driver.observer.adapter, driver.observer.health_layout, character
                )
                break
            except ValueError:
                if time.monotonic() >= until:
                    raise
                time.sleep(0.2)
        if (market_trial or stall_inspection) and (
            life.map_id != 1036 or life.dead_candidate
        ):
            raise ValueError(
                "Movement qualification is restricted to a living merchant in Market"
            )
        if submitted:
            record_capability(
                driver,
                ...
                {
                    "verified_at": time.time(),
                    "identity": driver.observer.adapter.identity,
                    "character": life.character,
                    "map_id": life.map_id,
                    "source": "memory-verified character after saved login",
                },
            )
        save(
            runtime,
        ...
        travel = ReturnDriver(driver, travel_only=True)
        stalls = 0
        for _ in range(300):
            check()
            ...
            if before["map_id"] == 1036:
                if stall_inspection:
                    ...
                    with guard.lease(character, purpose="connect"):
                        ...
                        def stationary_geometry():
                            check()
                            if geometry(driver) != dimensions:
                                raise ValueError("Stall inspection viewport changed")
                            return dimensions
                        ...
                        try:
                            ...
                            selected_flag = (
                                None
                                if pending.get("phase") == "submitted"
                                else approach_vacant_flag(
                                    driver,
                                    travel,
                                    check,
                                    preferred=(
                                        runtime.journal.get(character, "shop_home", {})
                                        or {}
                                    ).get("position"),
                                )
                            )
                        ...
                        if (
                            result.get("operation") == "open_owned_panel"
                            and result.get("booth_open")
                            and result.get("control") == CONTROL
                            and result.get("own_booth_uid_before")
                            == result.get("own_booth_uid")
                            == result.get("displayed_booth_uid")
                            and result.get("inventory_unchanged")
                        ):
                            record_capability(
                                driver, "booth_panel", result, dimensions=dimensions
                            )
                            qualified = read_json(driver.qualification)
                            qualified["booth_panel"] = dict(CONTROL)
                            write_json(driver.qualification, qualified)
                        from conquest.merchants.flag_target import (
                            CONTROL as FLAG_CONTROL,
                        )

                        if (
                            result.get("operation") == "inspect_vacant_flag"
                            and (
                                result.get("booth_open") or result.get("claim_verified")
                            )
                            and result.get("control") == FLAG_CONTROL
                            and not result.get("own_booth_uid_before")
                            and result.get("own_booth_uid")
                            and (
                                result.get("claim_verified")
                                or result["own_booth_uid"]
                                == result.get("displayed_booth_uid")
                            )
                            and result.get("inventory_unchanged")
                            and not result.get("dialog")
                        ):
                            record_capability(
                                driver, "booth_setup", result, dimensions=dimensions
                            )
                            qualified = read_json(driver.qualification)
                            from conquest.merchants.booth_confirmation import (
                                CONTROL as CONFIRM_CONTROL,
                            )

                            qualified.setdefault("shop_setup", {}).update(
                                claim_mode="native_confirm"
                                if result.get("native_confirmation")
                                else "direct",
                                confirmation=CONFIRM_CONTROL
                                if result.get("native_confirmation")
                                else None,
                                target=FLAG_CONTROL,
                            )
                ...
                if market_trial:
                    ...
                    if (
                        ...
                        raise ValueError(
                            "Movement qualification requires a closed booth and no trade"
                        )
                    from conquest.character_context import merchant_installation
                    ...
                    for flag in scene_flags(driver.observer):
                        ...
                        try:
                            distance, destination = stall_approach(
                                terrain, before["position"], flag
                            )
                        except ValueError:
                            continue
                        if distance > 1:
                            candidates.append((distance, flag["uid"], destination))
                    if not candidates:
                        raise ValueError("No checked Market approach is available")
                    ...
                    with guard.lease(character, purpose="connect"):
                        ...
                        try:
                            before = qualify_move(
                                driver, travel, before, destination, check
                            )
                        finally:
                            save(
                                runtime,
                                character,
                                "movement_trial",
                                movement_evidence=getattr(travel, "last_move", {}),
                            )
                        final = driver.memory.read()
                        if stock(final) != stock(initial):
                            raise ValueError(
                                "Stock changed during Market movement qualification"
                            )
                ...
                runtime.journal.event(
                    character, "connect_market_verified", position=before["position"]
                )
                return
            if before["map_id"] != 1002:
                raise ValueError("Merchant is on an unsupported arrival map")
            if before.get("trade") or before.get("request"):
                raise ValueError("Open trade prevents Market travel")
            with guard.lease(character, purpose="connect"):
                check()
                before = travel.read()
                if max(abs(a - b) for a, b in zip(before["position"], (438, 444))) > 2:
                    ...
                    try:
                        ...
                        after = qualify_move(driver, travel, before, (438, 444), check)
                    else:
                        after = travel.move(before, (438, 444), check)
                    stalls = (
                        stalls + 1 if after["position"] == before["position"] else 0
                    )
                    if stalls >= 3:
                        raise ValueError(
                            "Market approach stalled; no unbounded movement retry"
                        )
                else:
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Qualify a movement step before Conductress input"
                        )
                    ...
                    while time.monotonic() < until:
                        ...
                        if after["map_id"] == 1036:
                            if before["silver"] - after["silver"] != 100:
                                raise ValueError(
                                    "Market fare needs reconciliation; no repeat payment"
                                )
                            save(runtime, character, "arrived", fare_pending=False)
                            break
                        time.sleep(0.2)
                    else:
                        raise ValueError("Market arrival uncertain; no repeated fare")
            time.sleep(0.15)
        ...
        note = (
            ...
            else "Unexpected Market connection failure"
        )
        save(runtime, character, "failed", note=note)
        runtime.journal.event(character, "connect_market_failed", note=note)
    finally:
        runtime.connecting.pop(character, None)
        runtime.connect_checks.pop(character, None)


def start(
    ui, character, *, client_pid=None, market_trial=False, stall_inspection=False
):
    character = character_name(character)
    runtime = ui.runtime
    if not credential_path(character).exists():
        raise ValueError("Save this merchant login through the app first")
    if runtime.connecting:
        raise ValueError("Finish the current merchant connection first")
    if not ui.safe_to_yield() or ui.coordinator.owner or ui.coordinator.stopped:
        raise ValueError(
            "Stop the farmer at a safe location before connecting merchants"
        )
    if runtime.journal.pending(character):
        raise ValueError("Reconcile pending merchant transactions first")
    ...
    selected = (
        select_client(ui, character, client_pid) if client_pid is not None else None
    )
    if previous.get("launch_unresolved") and selected is None:
        raise ValueError(
            "Use the already-open launcher; reconcile its client before launching another"
        )
    cancel = threading.Event()
    runtime.connect_cancel[character] = cancel
    ...
    thread = threading.Thread(
        ...
        args=(
            ...
            character,
            cancel,
            ui.app.control.snapshot()["revision"],
            selected,
            market_trial,
            stall_inspection,
        ),
        daemon=True,
        name="connect-market-" + character,
    )
    ui.connect_threads[character] = thread
    thread.start()
