...
class PriceHistory:
    ...
    def remember(self, market):
        ...
        for raw,row in market.entries:
            ...
            exact = replace(row,key=replace(row.key,category=f'{row.key.category}:{raw["name"]}'))
            groups[exact.key].append(exact)
        quotes = []
        for key,rows in groups.items():
            decision = price_item(key,rows)
            if decision.reference is not None:
                quotes.append((json.dumps(asdict(key),sort_keys=True),decision.reference,at))
        with sqlite3.connect(self.path,timeout=5) as db:
            db.execute('BEGIN IMMEDIATE')
            previous = db.execute('SELECT data,observed_at FROM catalog WHERE id=1').fetchone()
            ...
            rows = {(r['name'],r['category'],r['quality']):r for r in catalog.get('rows',[])}
            rows.update({(r['name'],r['category'],r['quality']):{k:r[k] for k in ('name','category','quality')}
            ...
            catalog = dict(equipment_categories=mappings,rows=list(rows.values()),
                           ambiguous_equipment_types=sorted(ambiguous))
            db.executemany('''INSERT INTO quotes VALUES(?,?,?) ON CONFLICT(key) DO UPDATE
                SET unit_price=excluded.unit_price, observed_at=excluded.observed_at
                WHERE excluded.observed_at > quotes.observed_at''',quotes)
            db.execute('INSERT OR REPLACE INTO catalog VALUES(1,?,?)',(json.dumps(catalog),at))

    def quotes(self):
        with sqlite3.connect(self.path,timeout=5) as db:
            rows = db.execute('SELECT key,unit_price,observed_at FROM quotes').fetchall()
        result = {}
        for encoded,price,at in rows:
            key = json.loads(encoded);key['sockets'] = tuple(key['sockets'])
