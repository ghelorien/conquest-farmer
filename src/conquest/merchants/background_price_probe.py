...
class _PriceProbe:
    def __init__(self, driver, before, reader, report, sample, check):
        self.driver, self.before, self.reader, self.report = (
            driver,
            before,
            reader,
            report,
    ...
    def guard(self):
        ...
        if (
            ...
            or tuple(self.target.snapshot()["client_size"]) != self.size
    ...
    def fresh(self):
        ...
        if (
            stock(current) != self.initial_stock
            or current["position"] != self.before["position"]
            or current.get("request")
            or current.get("trade")
        ):
            raise ValueError(
                "Background price diagnostic stock, position or trade changed"
            )
        return current
    ...
    def point(self, logical):
        point = tuple(
            round(v * n / g) for v, n, g in zip(logical, self.size, self.viewport)
        )
    ...
    def wait(self, stage, predicate, timeout=0.8):
    ...
    def click_modal(self, control, label):
        ...
        point = self.point(
            tuple(a + b for a, b in zip(win["geometry"][:2], spec["offset"]))
        )
        ...
        def hovered(state):
            return (
                state["hover"] == {"window": win["address"], "id": expected}
        ...
        if (
            fresh_win != win
            or fresh_controls != controls
            or not hovered(self.reader.snapshot())
        ):
            raise ValueError("Price dialog control moved or hover expired")
        self.burst(point, 1, 0x201)
        self.wait(
            control + "-press",
            lambda s: s["active"]["id"] == expected and s["mouse_down"][0],
        )
        self.dialog()
        self.burst(point, 0, 0x202)
        self.wait(
            control + "-release",
            lambda s: not any(s["mouse_down"]) and not s["backend"]["buttons_down"],
        )
        return expected
    ...
    def cancel(self):
        ...
        def closed(_):
            current = self.fresh()
            return (
                not any(w["name"] == DIALOG for w in current["windows"])
                and self.read_model()[0] == 0
            )
    ...
    def release(self):
        if self.held:
            ...
            if self.session.identity != self.identity:
                raise ValueError(
                    "Price probe release refused after process identity changed"
                )
...
def run_price_probe(driver, before, reader, report, sample, check):
    ...
    try:
        current = probe.fresh()
        if (
            len(current["booth"]) >= 32
            or not current.get("booth_open")
            or any(w["name"] == DIALOG for w in current["windows"])
            or probe.read_model()[0]
        ):
            raise ValueError(
                "Price diagnostic requires an open own booth with space and no existing dialog"
            )
        source = None
        ...
        def source_fresh():
            ...
            if (
                items != [probe.item]
                or _geometry(driver, fresh, probe.item, probe.size, probe.viewport)
                != source
                or window(fresh, "Booth") != booth
                or len(fresh["booth"]) >= 32
            ):
        ...
        state = reader.snapshot()
        if (
            state["hover"] != {"window": source["window"], "id": expected}
        ...
        def payload(s):
            return (
                s["drag"]["active"]
                and s["drag"]["payload_type"] == "CQITEM"
                and s["drag"]["source_item_uid"] == probe.item["uid"]
                and s["drag"]["source_id"] == expected
        ...
        logical_destination = [
            v * g / n for v, g, n in zip(destination, probe.viewport, probe.size)
        ]
        drop = probe.wait(
            "price-drop-hover",
            lambda s: (
                ...
                and all(
                    abs(a - b) <= 2
                    for a, b in zip(s["mouse_position"], logical_destination)
                )
            ),
        )
        ...
        if (
            not payload(state)
            or state["hover"]["window"] not in drop_windows
            or not state["want_capture_mouse"]
            or not all(
                abs(a - b) <= 2
                for a, b in zip(state["mouse_position"], logical_destination)
            )
        ):
