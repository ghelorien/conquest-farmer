...
CODE_BLOCKS = (
    (
        0x113740,
        4912,
        "ce09814f3180a2aca6a676ba1f7249a684b99761fa812619c7250a8535d8f070",
    ),
    (
        0x114A70,
        1168,
        "0b900e1653f054d24670287c9b66576ee861fe4ab597db83deb357841ffd8b91",
    ),
...
def assert_trade_code(session):
    if session.expected_sha256 != CLIENT_SHA256_1078:
        raise ObservationUnavailable1078(
            "1078 trade requires its exact executable fingerprint"
        )
    ...
    for rva, size, digest in CODE_BLOCKS:
        if (
            rva + size > module["size"]
            or hashlib.sha256(read(module["base"] + rva, size)).hexdigest() != digest
        ):
            raise ObservationUnavailable1078(
                "1078 loaded trade renderer differs from its field proof"
            )
    session.assert_identity()
    return module["base"]
...
class TradeMemory1078(TradeObservationReader1078):
    ...
    def _trade(self, actual):
        ...
        if (
            ...
            or counterpart_accepted not in (0, 1)
            or raw[0x98] not in (0, 1)
            or raw[0x54] not in (0, 1)
        ):
            raise ObservationUnavailable1078(
                "Invalid 1078 trade participant or acceptance flags"
            )
        ...
        result = {
            ...
            "own_silver": _silver(
                own_text, accepted=bool(raw[0x98]), locked=bool(raw[0x54])
            ),
            "other_silver": _silver(other_text),
            "accepted": bool(raw[0x98]),
            "other_accepted": bool(counterpart_accepted),
        }
        if (
            self._read(model, 0x9A) != raw
            ...
            raise ObservationUnavailable1078(
                "1078 trade changed during canonical observation"
            )
        return result
    ...
    def read(self, *, max_seconds=3, recovery=False, farmer_preflight=False):
        ...
        if farmer_preflight:
            if self.character != farmer_name():
                raise ObservationUnavailable1078(
                    "Town trade preflight is restricted to the farmer"
                )
            maps = (1002, 1011, 1036)
        if life.dead_candidate or life.current_hp <= 0 or life.map_id not in maps:
            raise ObservationUnavailable1078(
                "1078 trade observation needs a living character in its allowed town"
            )
        ...
        if (
            self._trade(life.object_address) != snapshot["trade"]
            or self._request(life.object_address) != snapshot["request"]
        ):
            raise ObservationUnavailable1078(
                "1078 trade changed while reading rendered windows"
            )
        ...
        if (
            ...
            raise ObservationUnavailable1078(
                "1078 trade observation expired or changed identity/location"
            )
        stock = snapshot["inventory"] + snapshot["booth"]
        free = snapshot["capacity"] - len(stock)
        if free < 0:
            raise ObservationUnavailable1078(
                "1078 combined owned stock exceeds capacity"
            )
        for item in stock:
