...
def run(ui, state):
    ...
    def lease():
        with ui.coordinator.lock:
            ...
            if not ui.runtime.reconcile_probe_pair(character, f, m):
                raise CaptureUnavailable(
                    "Delivery placement needs fresh bilateral probe reconciliation"
                )
            manual_fence()
            with (
                ui.coordinator.lease("Farmer", purpose="delivery_offer_probe"),
    ...
    with lease():
        ...
        check()
        f, m = pair(ui, character)
        offered = partial_offer(intent, f, m)
        ...
        presentation()
        if not activate_client(target.hwnd, f["identity"]):
            raise ValueError("Farmer focus unavailable; no drag sent")
        ...
        for wanted in intent["items"]:
            ...
            f, m = pair(ui, character)
            offered = partial_offer(intent, f, m)
            if wanted["uid"] in exact_items(offered):
                continue
            if deadline - time.monotonic() < 1.5:
                save(
                    "trade_open_verified",
                    offered_uids=list(exact_items(offered)),
                    needs_more=True,
                )
                return
            if any(
            ...
            if not item or exact_items([item]) != exact_items([wanted]):
                raise ValueError("Reserved item changed")
            inventory, trade, source, destination = endpoints(
                memory.gui, f, item, offered
            )

            def before():
                check()
                a, b = pair(ui, character)
                if exact_items(partial_offer(intent, a, b)) != exact_items(offered):
                    raise ValueError("Offer changed before drag")
                if any(
                ...
                current = next(
                    (i for i in a["inventory"] if i["uid"] == wanted["uid"]), None
                )
                if (
                    not current
                    or exact_items([current]) != exact_items([item])
                    or current["slot"] != item["slot"]
                    or endpoints(memory.gui, a, current, offered)
                    != (inventory, trade, source, destination)
                ):
                    raise ValueError("Item or grid changed before drag")
                context = unpack(
                    observer.adapter, memory.gui.base + memory.gui.context_rva, "<Q"
                )[0]
                if (
                    unpack(observer.adapter, context + 0x3EC0, "<Q")[0]
                    != inventory["address"]
                ):
            ...
            while True:
                try:
                    a, b = pair(ui, character)
                    received = partial_offer(intent, a, b)
                    if item["uid"] in exact_items(received):
                        break
                ...
                if time.monotonic() >= until:
                    raise ValueError(
                        "Placement unverified; reconcile before another drag"
                    )
                time.sleep(0.05)
            f, m = a, b
            offered = received
            save(
                "trade_open_verified",
                offered_uids=list(exact_items(received)),
                needs_more=True,
            )
        f, m = pair(ui, character)
