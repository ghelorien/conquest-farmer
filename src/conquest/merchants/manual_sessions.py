...
PROBE_HISTORY_CHECKS = frozenset(
    (
        "transaction",
        "ownership",
        "saved_boundary",
...
def canonical_ownership(snapshot, *, require_closed=True):
    ...
    if require_closed and (
        snapshot["request"] is not None or snapshot["trade"] is not None
    ):
        raise ManualSessionError("Request and trade windows must be absent")
    for field in ("request", "trade"):
        if snapshot[field] is not None and (
            not isinstance(snapshot[field], dict) or not snapshot[field]
        ):
    ...
    return {
        ...
        "character_uid": _integer(
            snapshot.get("character_uid"), "Target character UID", 1
        ),
...
def _ownership_delta(before, after):
    ...
    def located(proof):
        return {
            item["uid"]: {"location": location, "item": item}
            for location in ("inventory", "booth")
            for item in proof[location]
        }
    ...
    for uid in sorted(old.keys() | current.keys()):
        ...
        if previous is None:
            ...
            fields = sorted(
                field
                for field in previous["item"].keys() | final["item"].keys()
                if previous["item"].get(field) != final["item"].get(field)
            )
            category = (
                "moved" if previous["location"] != final["location"] else "changed"
            )
    ...
    return {
        ...
        **{
            ...
            for field in ("silver", "capacity")
        },
        "booth_state": {
            "before": {field: before[field] for field in booth_fields},
...
class ManualSessionStore:
    ...
    def __init__(self, path=None, *, on_settlement=None):
        self.path = Path(
            path
            if path is not None
            else state_path("reports/merchants/journal.sqlite3")
        )
    ...
    def _audit(self, db, session_id, event, payload, now):
        previous = db.execute(
            "SELECT digest FROM manual_audit ORDER BY id DESC LIMIT 1"
        ).fetchone()
        ...
        db.execute(
            "INSERT INTO manual_audit(session_id,event,at,payload_json,previous_digest,digest) VALUES(?,?,?,?,?,?)",
    ...
    def _evidence(self, db, session_id, snapshot, now, proof=None, error=None):
        ...
        cursor = db.execute(
            "INSERT INTO manual_evidence(session_id,observed_at,recorded_at,digest,ownership_digest,snapshot_json,error) VALUES(?,?,?,?,?,?,?)",
            (
                session_id,
                observed_at,
                now,
                _digest(snapshot),
                _digest(proof) if proof is not None else None,
                encoded,
                error,
            ),
        )
    ...
    def _row(self, db, session_id):
        row = db.execute(
            "SELECT * FROM manual_sessions WHERE id=?", (session_id,)
        ).fetchone()
        if row is None:
    ...
    def _view(self, db, row):
        ...
        request = db.execute(
            "SELECT * FROM manual_requests WHERE id=?", (row["current_request_id"],)
        ).fetchone()
        result["approval_binding"] = (
            json.loads(request["binding_json"]) if request else None
        )
    ...
    def active(self, target_profile_id):
        ...
        with self.db() as db:
            row = db.execute(
                "SELECT * FROM manual_sessions WHERE target_profile_id=? AND phase NOT IN ('completed','request_withdrawn','declined_verified','operator_overridden')",
                (target_profile_id,),
            ).fetchone()
        ...
        with self.db() as db:
            return [
                ...
                for row in db.execute(
                    "SELECT * FROM visitor_permissions"
                    + (
    ...
    def _allowed(self, db, visitor):
        row = db.execute(
            "SELECT allowed FROM visitor_permissions WHERE target_profile_id=? AND visitor_name=? AND visitor_server=? AND visitor_uid=?",
            tuple(asdict(visitor).values()),
        ).fetchone()
    ...
    def revoke(self, visitor, *, operator, now=None):
        ...
        with self.db() as db:
            ...
            db.execute(
                "INSERT INTO visitor_permissions VALUES(?,?,?,?,0,?) ON CONFLICT(target_profile_id,visitor_name,visitor_server,visitor_uid) DO UPDATE SET allowed=0,updated_at=excluded.updated_at",
            ...
            self._audit(
                ...
                None,
                "visitor_revoked",
                {"visitor": asdict(visitor), "operator": operator},
                now,
            )
    ...
    def revoke_many(self, visitors, *, operator, now=None, require_allowed=False):
        ...
        with self.db() as db:
            ...
            if require_allowed and any(
                not self._allowed(db, visitor) for visitor in keys
            ):
                raise ManualSessionError(
                    "An exact allowed visitor permission changed before revocation"
                )
            for visitor in keys:
                db.execute(
                    "INSERT INTO visitor_permissions VALUES(?,?,?,?,0,?) ON CONFLICT(target_profile_id,visitor_name,visitor_server,visitor_uid) DO UPDATE SET allowed=0,updated_at=excluded.updated_at",
    ...
    def _new_session(
        ...
        *,
        ...
        target_profile_id = (
            visitor.target_profile_id
            if visitor is not None
            else _text(target_profile_id, "Target profile ID")
        )
        visitor_data = asdict(visitor) if visitor is not None else None
        character = {
            field: proof[field] for field in ("character", "character_uid", "server")
        }
        db.execute(
            "INSERT INTO manual_sessions(id,target_profile_id,visitor_json,process_json,character_json,phase,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)",
            (
                session_id,
                target_profile_id,
                _json(visitor_data),
                _json(proof["identity"]),
                _json(character),
                phase,
                now,
                now,
        ...
        self._audit(
            ...
            session_id,
            "session_started",
            {"visitor": visitor_data, "phase": phase},
            now,
        )
    ...
    def begin_request(
        ...
        *,
        session_id=None,
        timeout_seconds=APPROVAL_SECONDS,
        now=None,
    ):
        ...
        visitor = VisitorKey(
            target_profile_id,
            snapshot["request"]["participant"],
            snapshot["server"],
            snapshot["request"]["participant_uid"],
        )
        with self.db() as db:
            db.execute("BEGIN IMMEDIATE")
            row = db.execute(
            ...
            if row:
                ...
                if (
                    row["visitor_json"] != _json(asdict(visitor))
                    or row["phase"] == "needs_attention"
                ):
                    raise BindingMismatch("Reconcile the existing manual session first")
                prior = db.execute(
                    "SELECT * FROM manual_requests WHERE id=?",
                    (row["current_request_id"],),
                ).fetchone()
                if prior and prior["state"] != "closed":
                    if prior["fingerprint"] != fingerprint:
                        raise BindingMismatch(
                            "Existing request changed without a closed-window observation"
                        )
            ...
            db.execute(
                "INSERT INTO manual_requests VALUES(?,?,?,?,?,?,?,?)",
                (
                    request_id,
                    row["id"],
                    fingerprint,
            ...
            db.execute(
                "UPDATE manual_sessions SET current_request_id=?,phase='approval_pending',stable_digest=NULL,stable_since=NULL,stable_evidence_id=NULL,reason=NULL,updated_at=?,last_observed_at=? WHERE id=?",
            ...
            self._audit(
                ...
                row["id"],
                "approval_pending",
                {"binding": binding, "evidence_id": evidence_id},
                now,
            )
    ...
    def _bound_request(self, db, binding, snapshot, now):
        ...
        self._same_owner(row, proof)
        if (
            snapshot["trade"] is not None
            or request_fingerprint(snapshot) != request["fingerprint"]
        ):
    ...
    def allow_and_activate(self, binding, live_snapshot, *, operator, now=None):
        ...
        return self._activate(
            binding, live_snapshot, operator=operator, grant=True, now=now
        )
    ...
    def activate_allowed(self, binding, live_snapshot, *, now=None):
        ...
        return self._activate(
            binding, live_snapshot, operator=None, grant=False, now=now
        )
    ...
    def _activate(self, binding, snapshot, *, operator, grant, now):
        ...
        with self.db() as db:
            ...
            if grant:
                db.execute(
                    "INSERT INTO visitor_permissions VALUES(?,?,?,?,1,?) ON CONFLICT(target_profile_id,visitor_name,visitor_server,visitor_uid) DO UPDATE SET allowed=1,updated_at=excluded.updated_at",
                    (*asdict(visitor).values(), now),
                )
            ...
            evidence_id = self._evidence(db, row["id"], snapshot, now, proof)
            db.execute(
                "UPDATE manual_requests SET state='approved' WHERE id=?",
                (request["id"],),
            )
            db.execute(
                "UPDATE manual_sessions SET phase='manual_active',ever_approved=1,stable_digest=NULL,stable_since=NULL,stable_evidence_id=NULL,updated_at=?,last_observed_at=? WHERE id=?",
    ...
    def _decline(self, db, row, request, reason, now):
        if request["state"] != "pending" or row["phase"] != "approval_pending":
            return False
        db.execute(
            "INSERT INTO manual_declines VALUES(?,?,?,?,?)",
            (request["id"], row["id"], reason, now, request["binding_json"]),
        )
        db.execute(
            "UPDATE manual_requests SET state='decline_pending' WHERE id=?",
            (request["id"],),
        )
        db.execute(
            "UPDATE manual_sessions SET reason=?,updated_at=? WHERE id=?",
            (reason, now, row["id"]),
        )
        self._audit(
            db,
        ...
        with self.db() as db:
            ...
            if self._decline(db, row, request, "rejected", now):
                self._audit(
                    ...
                    row["id"],
                    "operator_rejected",
                    {"operator": operator, "request_id": request["id"]},
                    now,
                )
            return self._view(db, self._row(db, row["id"]))
    ...
    def claim_decline(self, session_id, live_snapshot, *, now=None):
        ...
        with self.db() as db:
            ...
            db.execute(
                "INSERT INTO manual_decline_claims VALUES(?,?,?)",
                (request["id"], now, evidence_id),
            )
            db.execute(
                "UPDATE manual_requests SET state='decline_claimed' WHERE id=?",
                (request["id"],),
            )
            self._audit(
                db,
                session_id,
                "decline_claimed",
                {"request_id": request["id"], "evidence_id": evidence_id},
                now,
            )
            return {
                "session_id": session_id,
    ...
    def _attention(self, db, row, reason, now, evidence_id):
        db.execute(
            "UPDATE manual_sessions SET phase='needs_attention',reason=?,updated_at=?,stable_digest=NULL,stable_since=NULL,stable_evidence_id=NULL WHERE id=?",
            (reason, now, row["id"]),
        )
        self._audit(
            db,
    ...
    def _recover_reader_gap(self, db, row, request, snapshot, proof, evidence_id, now):
        ...
        if (
            row["reason"] != "Request/trade window evidence is unavailable"
            or row["ever_approved"]
            or request is None
            or request["state"] not in ("pending", "decline_pending")
            or db.execute(
                "SELECT 1 FROM manual_decline_claims c JOIN manual_requests r "
                "ON r.id=c.request_id WHERE r.session_id=?",
                (row["id"],),
            ).fetchone()
            or db.execute(
                "SELECT COUNT(*) FROM manual_requests WHERE session_id=? AND state!='closed'",
                (row["id"],),
            ).fetchone()[0]
            != 1
        ):
            return False
        decline = db.execute(
            "SELECT reason FROM manual_declines WHERE request_id=?", (request["id"],)
        ).fetchone()
        if (request["state"] == "pending" and decline is not None) or (
            request["state"] == "decline_pending"
        ...
        if (
            binding.get("visitor") != visitor
            or binding.get("session_id") != row["id"]
        ...
        for audit in db.execute(
            "SELECT event,payload_json FROM manual_audit WHERE session_id=?",
            (row["id"],),
        ):
            ...
            if (
                audit["event"] == "needs_attention"
                and payload.get("reason")
                != "Request/trade window evidence is unavailable"
            ):
                return False
            if audit["event"] == "decline_requested" and (
                payload.get("request_id") != request["id"]
                or payload.get("reason") != "timeout"
            ):
                return False
        for evidence in db.execute(
            "SELECT snapshot_json,ownership_digest FROM manual_evidence WHERE session_id=?",
            (row["id"],),
        ):
            ...
            try:
                ...
                if (
                    prior_proof["request"] is not None
            ...
            if _holdings(prior_proof) != baseline:
                return False
        db.execute(
            "UPDATE manual_sessions SET phase='approval_pending',reason=NULL,updated_at=? WHERE id=?",
            (now, row["id"]),
        )
        self._audit(
            db, row["id"], "reader_gap_recovered", {"evidence_id": evidence_id}, now
        )
        return True
        ...
        with self.db() as db:
            ...
            try:
                ...
                evidence_id = self._evidence(
                    db, session_id, snapshot, now, error=str(exc)
                )
            ...
            if row["phase"] == "needs_attention":
                if not self._recover_reader_gap(
                    db, row, request, snapshot, proof, evidence_id, now
                ):
            ...
            if (
                ...
                self._audit(
                    ...
                    session_id,
                    "observation_ignored",
                    {"reason": "non_increasing_time", "evidence_id": evidence_id},
                    now,
                )
                return self._view(db, row)
            ...
            if trade is not None:
                if (
                    not approved
                    or trade.get("participant") != visitor["visitor_name"]
                    or trade.get("participant_uid") != visitor["visitor_uid"]
                    or trade.get("server", snapshot["server"])
                    != visitor["visitor_server"]
                ):
            ...
            if problem:
                ...
                db.execute(
                    "UPDATE manual_sessions SET phase=?,stable_digest=NULL,stable_since=NULL,stable_evidence_id=NULL,last_observed_at=?,updated_at=? WHERE id=?",
                    (
                        "manual_active" if approved else "approval_pending",
                        snapshot["timestamp"],
                        now,
                        session_id,
                    ),
                ...
                claimed = bool(
                    request
                    and db.execute(
                        "SELECT 1 FROM manual_decline_claims WHERE request_id=?",
                        (request["id"],),
                    ).fetchone()
                )
                if (
                    not row["ever_approved"]
                    and request
                    and _digest(_holdings(json.loads(request["before_json"]))) != digest
                ):
                    self._attention(
                        db, row, "Unapproved ownership changed", now, evidence_id
                    )
                elif (
                    row["stable_digest"] == digest
                    and snapshot["timestamp"] - row["stable_since"]
                    >= SETTLEMENT_SECONDS
                ):
                    phase = (
                        "completed"
                        if row["ever_approved"]
                        else "declined_verified"
                        if claimed
                    ...
                    baseline = db.execute(
                        "SELECT * FROM manual_requests WHERE session_id=? ORDER BY rowid LIMIT 1",
                        (session_id,),
                    ).fetchone()
                    if baseline is None:
                        raise ManualSessionError(
                            "Manual ownership baseline is unavailable"
                        )
                    baseline_binding = json.loads(baseline["binding_json"])
                    receipt = {
                        "phase": phase,
                        "ownership_digest": digest,
                        "first_evidence_id": row["stable_evidence_id"],
                        ...
                        "ownership_delta": _ownership_delta(
                            json.loads(baseline["before_json"]), proof
                        ),
                    }
                    db.execute(
                        "UPDATE manual_sessions SET phase=?,terminal_json=?,last_observed_at=?,updated_at=? WHERE id=?",
                    ...
                    if self.on_settlement is not None:
                        ...
                        self.on_settlement(
                            db,
                            self._view(db, self._row(db, session_id)),
                            snapshot,
                            receipt,
                        )
                else:
                    ...
                    db.execute(
                        "UPDATE manual_sessions SET phase='settlement_observed',stable_digest=?,stable_since=?,stable_evidence_id=?,last_observed_at=?,updated_at=? WHERE id=?",
                        (
                            digest,
                            row["stable_since"] if same else snapshot["timestamp"],
                            row["stable_evidence_id"] if same else evidence_id,
                            snapshot["timestamp"],
                            now,
                            session_id,
                        ),
                    ...
                    self._audit(
                        ...
                        session_id,
                        "settlement_observed",
                        {"evidence_id": evidence_id, "ownership_digest": digest},
                        now,
                    )
                if request:
                    db.execute(
                        "UPDATE manual_requests SET state='closed' WHERE id=?",
                        (request["id"],),
                    )
            return self._view(db, self._row(db, session_id))
    ...
    def retract_probe_admission(
        self, session_id, snapshot, state, farmer, *, farmer_profile_id, now=None
    ):
        ...
        with self.db() as db:
            ...
            request_owned_at = (
                ...
                if state.get("phase") == "request_verified"
                else None
            )
            if type(request_owned_at) not in (int, float) or not math.isfinite(
                request_owned_at
            ):
                raise BindingMismatch(
                    "Durable verified request ownership boundary is unavailable"
                )
            if (
                row["phase"] not in ("approval_pending", "needs_attention")
                or row["ever_approved"]
                or proof["modal"] != "request"
                or row["created_at"] < request_owned_at
                or db.execute(
                    "SELECT 1 FROM manual_decline_claims c JOIN manual_requests r "
                    "ON r.id=c.request_id WHERE r.session_id=?",
                    (session_id,),
                ...
                raise BindingMismatch(
                    "Manual interval cannot be retracted as a bot-owned request"
                )
            requests = db.execute(
                "SELECT * FROM manual_requests WHERE session_id=?", (session_id,)
            ).fetchall()
            if (
                len(requests) != 1
                or requests[0]["state"] not in ("pending", "decline_pending")
                or requests[0]["id"] != row["current_request_id"]
            ):
                raise BindingMismatch(
                    "Manual request has prior activity or uncertain input"
                )
            ...
            if json.loads(request["before_json"]) != current:
                raise BindingMismatch(
                    "Original manual evidence differs from the exact bot-owned request"
                )
            original = db.execute(
                "SELECT snapshot_json FROM manual_evidence WHERE session_id=? AND digest=?",
                (session_id, binding["evidence_digest"]),
            ).fetchone()
            original = json.loads(original[0]) if original else None
            if (
                not original
                or original["timestamp"] < request_owned_at
                or any(
                    original.get(key) != snapshot.get(key)
                    for key in ("position", "map_id")
                )
                or any(
                    {item["uid"]: item.get("slot") for item in original[field]}
                    != {item["uid"]: item.get("slot") for item in snapshot[field]}
                    for field in ("inventory", "booth")
                )
            ):
                raise BindingMismatch(
                    "Original manual location/chronology differs from the probe"
                )
            ...
            receipt = {
                **proof,
                "phase": "request_withdrawn",
                "disposition": "manual_admission_retracted_bot_owned",
                "request_still_visible": True,
                ...
                "original_request_state": request["state"],
                "evidence_id": evidence_id,
                "probe": state,
                "farmer_evidence": farmer,
                "at": now,
            }
            db.execute(
                "UPDATE manual_sessions SET phase='request_withdrawn',terminal_json=?,reason=?,"
                "last_observed_at=?,updated_at=? WHERE id=?",
                (_json(receipt), reason, snapshot["timestamp"], now, session_id),
            )
            self._audit(
                db, session_id, "manual_admission_retracted_bot_owned", receipt, now
            )
            # No settlement callback: this reclassification proves no transfer,
            # new stock, sale, disappeared request or successful decline.
            return self._view(db, self._row(db, session_id))

    def retract_probe_pair(
        self,
        state,
        farmer,
        merchant,
    ...
    def _probe_pair(
        ...
        *,
        ...
        """
        from conquest.merchants.delivery_probe_ownership import (
            ownership,
            historical_local_trade,
            _saved_boundary,
            _context,
        )
        ...
        with self._probe_connection(read_only) as db:
            ...
            if proof["modal"] != "trade":
                raise BindingMismatch("An exact open bot trade is required")
            intent, originals, items = _context(
                state,
                merchant["character"],
                target_profile_id,
                farmer_profile_id,
            ...
            if (
                type(accepted_at) not in (int, float)
            ...
            pending = []
            for role, target, snapshot, peer in (
                ("farmer", farmer_profile_id, farmer, merchant),
                ("merchant", target_profile_id, merchant, farmer),
            ):
                history_check = "session_row"
                index = None
                row = db.execute(
                    "SELECT * FROM manual_sessions WHERE target_profile_id=? AND phase NOT IN "
                    "('completed','request_withdrawn','declined_verified','operator_overridden')",
                    (target,),
                ).fetchone()
                if row is None:
                ...
                if (
                    row["phase"] not in ("needs_attention", "approval_pending")
                    or row["ever_approved"]
                    or row["created_at"] < accepted_at
                    or row["stable_digest"] is not None
                    or row["terminal_json"] is not None
                ):
                    raise BindingMismatch(
                        "Manual interval predates or exceeds bot admission"
                    )
                ...
                visitor = VisitorKey(
                    target, peer["character"], peer["server"], peer["character_uid"]
                )
                if row["visitor_json"] != _json(asdict(visitor)):
                    raise BindingMismatch(
                        "Manual visitor differs from the exact bot peer"
                    )
                history_check = "visitor_permission"
                if self._allowed(db, visitor):
                    raise BindingMismatch(
                        "Manual visitor permission prevents bot retraction"
                    )
                ...
                if (
                    db.execute(
                        "SELECT 1 FROM manual_declines WHERE session_id=?",
                        (session_id,),
                    ).fetchone()
                    or db.execute(
                        "SELECT 1 FROM manual_decline_claims c JOIN manual_requests r "
                ...
                if requests:
                    if (
                        role != "merchant"
                        or len(requests) != 1
                        or requests[0]["state"] != "pending"
                        or requests[0]["id"] != row["current_request_id"]
                    ):
                        raise BindingMismatch(
                            "Manual request history is not a pristine stale admission"
                        )
                    ...
                    raise BindingMismatch(
                        "Only a pristine merchant request may remain approval-pending"
                    )
                history_check = "audit_events"
                events = db.execute(
                    "SELECT event FROM manual_audit WHERE session_id=?", (session_id,)
                ).fetchall()
                if any(
                    event[0]
                    not in (
                ...
                evidence = db.execute(
                    "SELECT * FROM manual_evidence WHERE session_id=? ORDER BY id",
                    (session_id,),
                ).fetchall()
                if not evidence:
                    raise BindingMismatch("Original admission evidence is unavailable")
                gaps = []
                gap = None
                ...
                for index, record in enumerate(evidence):
                    ...
                    try:
                        ...
                        raise BindingMismatch(
                            "Historical evidence JSON is malformed"
                        ) from error
                    ...
                    if (
                        ...
                        or not previous_at <= at <= now
                        or record["digest"] != _digest(original)
                    ):
                        raise BindingMismatch(
                            "Historical evidence chronology or digest changed"
                        )
                    previous_at = at
                    if record["error"]:
                        ...
                        allowed_error = {
                            ...
                            "merchant": "Attached memory reader is unavailable",
                        }[role]
                        if (
                            requests
                            or index == 0
                            or row["stable_since"] is not None
                            or row["stable_evidence_id"] is not None
                            or record["error"]
                            != "Request/trade window evidence is unavailable"
                            or record["ownership_digest"] is not None
                            or record["observed_at"] is not None
                            or not isinstance(original, dict)
                            or set(original) != {"reader_error"}
                            or original["reader_error"] != allowed_error
                        ):
                            raise BindingMismatch(
                                "Admission has missing or uncertain historical evidence"
                            )
                        if gap is None:
                            gap = {
                                "before_evidence_id": evidence[index - 1]["id"],
                                "first_error_id": record["id"],
                                "before_digest": evidence[index - 1]["digest"],
                                "before_ownership_digest": evidence[index - 1][
                                    "ownership_digest"
                                ],
                                "before_observed_at": evidence[index - 1][
                                    "observed_at"
                                ],
                                "before_recorded_at": evidence[index - 1][
                                    "recorded_at"
                                ],
                        ...
                        gap.update(
                            last_error_id=record["id"],
                            last_recorded_at=at,
                            error_count=gap["error_count"] + 1,
                        )
                        continue
                    try:
                        ...
                        if (
                            ...
                            or type(original.get("timestamp")) not in (int, float)
                            or not math.isfinite(original["timestamp"])
                            or original["timestamp"] > at
                            or record["observed_at"] != original["timestamp"]
                        ):
                            raise BindingMismatch(
                                "Historical observation time is invalid"
                            )
                        historical = historical_local_trade(
                            state,
                            merchant["character"],
                            target_profile_id,
                            farmer_profile_id,
                        ...
                        raise BindingMismatch(
                            "Historical bot ownership is unverified"
                        ) from error
                    ...
                    if gap is not None:
                        gaps.append(
                            {
                                **gap,
                                "after_evidence_id": record["id"],
                                "after_digest": record["digest"],
                                "after_ownership_digest": record["ownership_digest"],
                                "after_observed_at": record["observed_at"],
                    ...
                    if index == 0:
                        ...
                        if requests:
                            ...
                            binding = json.loads(request["binding_json"])
                            if (
                                original.get("request") is None
                                or request["before_json"] != _json(historical)
                                or binding.get("evidence_digest") != record["digest"]
                                or binding.get("ownership_digest")
                                != _digest(historical)
                                or binding.get("session_id") != session_id
                                or binding.get("request_id") != request["id"]
                                or binding.get("target_profile_id") != target
                                or binding.get("visitor") != asdict(visitor)
                                or binding.get("game_process_identity")
                                != historical["identity"]
                                or binding.get("character_identity")
                                ...
                                raise BindingMismatch(
                                    "Original stale request binding differs"
                                )
                        elif original.get("trade") is None:
                            raise BindingMismatch(
                                "Original admission was not an open trade"
                            )
                if gap is not None:
                    ...
                    if (
                        snapshot["timestamp"] < gap["last_recorded_at"]
                        or snapshot["timestamp"] < gap["before_observed_at"]
                    ):
                        raise BindingMismatch(
                            "Current evidence does not follow the reader outage"
                        )
                    boundary = historical_local_trade(
                        state,
                        merchant["character"],
                        target_profile_id,
                        farmer_profile_id,
                    ...
                    if _digest(boundary) != _digest(current):
                        raise BindingMismatch(
                            "Current outage boundary ownership differs"
                        )
                    gaps.append(
                        {
                            **gap,
                            "after_evidence_id": None,
                            "after_digest": _digest(snapshot),
                            "after_ownership_digest": _digest(current),
                            "after_observed_at": snapshot["timestamp"],
                            "after_recorded_at": now,
                            "after_source": "fresh_bilateral_reconciliation",
                        }
                    )
            ...
            if read_only:
                return {
                    "eligible_sessions": len(pending),
                    "outage_intervals": sum(len(row[4]) for row in pending),
                    "pending_current_boundaries": sum(
                        gap["after_evidence_id"] is None
                        for row in pending
                        for gap in row[4]
                    ),
                }
            ...
            for row, snapshot, current, original_id, gaps in pending:
                ...
                if any(gap["after_evidence_id"] is None for gap in gaps):
                    written = db.execute(
                        "SELECT * FROM manual_evidence WHERE id=?", (evidence_id,)
                    ).fetchone()
                    for gap in gaps:
                        if gap["after_evidence_id"] is not None:
                            continue
                        if any(
                            gap["after_" + key] != written[key]
                            for key in (
                                "digest",
                                "ownership_digest",
                                "observed_at",
                            ...
                            raise BindingMismatch(
                                "Persisted outage boundary differs from the exact current pair"
                            )
                ...
                receipt = {
                    **proof,
                    "phase": "request_withdrawn",
                    "disposition": "manual_admission_retracted_bot_owned",
                    "trade_still_visible": True,
                    "request_still_visible": False,
                    "gameplay_input": False,
                    "sales_receipt": False,
                    "reason": reason,
                    "original_phase": row["phase"],
                ...
                self._audit(
                    db, row["id"], "manual_admission_retracted_bot_owned", receipt, now
                )
                results.append(self._view(db, self._row(db, row["id"])))
            history_check = "commit_recheck"
            if evidence_digest(current_probe()) != proof["probe_digest"]:
                raise BindingMismatch(
                    "Probe changed before bilateral retraction committed"
                )
            return results
    ...
    def observe_target(self, target_profile_id, snapshot, *, now=None):
        ...
        with self.db() as db:
            ...
            if row is None:
                ...
                evidence_id = self._evidence(
                    db, row["id"], snapshot, now, full_proof, error
                )
                reason = "Unapproved open trade without a manual session"
                self._attention(
                    db, row, reason + (": " + error if error else ""), now, evidence_id
                )
            return self._view(db, self._row(db, row["id"]))

    def operator_override(
        self, session_id, *, confirmation_reference, operator, reason, now=None
    ):
        ...
        with self.db() as db:
            ...
            if row["phase"] in TERMINAL_PHASES:
                ...
                if (
                    row["phase"] != "operator_overridden"
                    or receipt.get("confirmation_reference") != confirmation_reference
                ):
            ...
            db.execute(
                "UPDATE manual_sessions SET phase='operator_overridden',terminal_json=?,updated_at=? WHERE id=?",
                (_json(receipt), now, session_id),
            )
    ...
    def overlaps(self, target_profile_id, start, end):
        ...
        """
        _number(start, "Interval start")
        _number(end, "Interval end")
        if end < start:
            raise ManualSessionError("Interval end precedes start")
        with self.db() as db:
            rows = db.execute(
                "SELECT * FROM manual_sessions WHERE target_profile_id=? AND created_at<=? AND (phase NOT IN ('completed','request_withdrawn','declined_verified','operator_overridden') OR updated_at>=?) ORDER BY created_at,id",
