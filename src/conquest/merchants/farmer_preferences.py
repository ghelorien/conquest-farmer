...
def enabled(character=None):
    ...
    return (
        read_json(state_path(PATH))
        .get("farmers", {})
        .get(character or farmer_name(), False)
        is True
    )
...
def rollout_enabled(character=None, *, policy=None):
    ...
    settings = read_json(state_path(PATH))
    if settings.get("farmers", {}).get(character, False) is not True:
        return False
    override = settings.get("rollout", {}).get(character)
    if type(override) is bool:
        return override
    policy = (
        read_json("profiles/merchant-deliveries.json") if policy is None else policy
    )
...
def rollout_source(character=None):
    override = (
        read_json(state_path(PATH)).get("rollout", {}).get(character or farmer_name())
    )
...
def permits_new_delivery(character=None):
    character = character or farmer_name()
    if not enabled(character):
        raise ValueError("Merchant transfers are Off for " + character)
