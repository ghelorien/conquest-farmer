...
class ManualHandoffStore:
    ...
    def _audit(db, session_id, event, now, **payload):
        db.execute(
            "INSERT INTO manual_handoff_audit(session_id,event,at,payload) VALUES(?,?,?,?)",
    ...
    def active(self):
        with self.journal.db() as db:
            row = db.execute(
                "SELECT * FROM manual_handoffs WHERE phase!='completed' ORDER BY created_at DESC LIMIT 1"
            ).fetchone()
    ...
    def participant_identities(self, session_id):
        ...
        with self.journal.db() as db:
            rows = db.execute(
                "SELECT target_profile_id,process_json FROM manual_handoff_participants WHERE session_id=?",
                (session_id,),
            )
            return {
                row["target_profile_id"]: json.loads(row["process_json"])
                if row["process_json"]
                else None
                for row in rows
            }
    ...
    def bind_participant_identities(self, session_id, identities):
        ...
        with self.journal.db() as db:
            ...
            row = db.execute(
                "SELECT * FROM manual_handoffs WHERE id=?", (session_id,)
            ).fetchone()
            if row is None or row["phase"] == "completed":
                raise ValueError("Manual handoff is not active")
            for target, identity in identities.items():
                encoded = json.dumps(identity, sort_keys=True)
                current = db.execute(
                    "SELECT process_json FROM manual_handoff_participants WHERE session_id=? AND target_profile_id=?",
                    (session_id, target),
                ).fetchone()
                if current is None:
                    raise ValueError(
                        "Manual handoff participant changed during discovery"
                    )
                if current["process_json"] not in (None, encoded):
                    raise ValueError("Manual handoff process changed during discovery")
                db.execute(
                    "UPDATE manual_handoff_participants SET process_json=? WHERE session_id=? AND target_profile_id=?",
    ...
    def identity_changed(self, target, reason, *, now=None):
        ...
        with self.journal.db() as db:
            ...
            if not part["process_json"]:
                return self._view(db, row)
            db.execute(
                "UPDATE manual_handoffs SET phase='needs_attention',updated_at=?,reason=? WHERE id=?",
            ...
            return self._view(
                db,
                db.execute(
                    "SELECT * FROM manual_handoffs WHERE id=?", (row["id"],)
                ).fetchone(),
            )
    ...
    def _view(self, db, row):
        ...
        result["participants"] = [
            ...
            for item in db.execute(
                "SELECT target_profile_id,role,baseline_at,last_at,saw_window FROM manual_handoff_participants WHERE session_id=? ORDER BY role,target_profile_id",
                (row["id"],),
            )
        ]
        return result
    ...
    def start(self, participants, *, operator, identities=None, now=None):
        ...
        if not participants or any(
            not isinstance(k, str) or not k for k in participants
        ):
            raise ValueError("Attached Farmer and merchant profiles are required")
        if len(set(participants)) != len(participants):
            raise ValueError("Duplicate handoff participant")
        if identities is not None and set(identities) != set(participants):
            raise ValueError(
                "Manual handoff discovery identities do not match participants"
            )
        key = "operator-handoff:" + uuid.uuid4().hex
        with self.journal.db() as db:
            db.execute("BEGIN IMMEDIATE")
            existing = db.execute(
                "SELECT id FROM manual_handoffs WHERE phase!='completed' LIMIT 1"
            ).fetchone()
            if existing:
                raise ValueError("A global manual handoff is already active")
            db.execute(
                "INSERT INTO manual_handoffs(id,phase,operator,created_at,updated_at) VALUES(?,'preparing',?,?,?)",
                (key, operator, now, now),
            )
            for target, role in participants.items():
                process = (
                    json.dumps(identities[target], sort_keys=True)
                    if identities is not None
                    else None
                )
                db.execute(
                    "INSERT INTO manual_handoff_participants(session_id,target_profile_id,role,process_json) VALUES(?,?,?,?)",
                    (key, target, role, process),
                )
            self._audit(
                db, key, "started", now, operator=operator, participants=participants
            )
            return self._view(
                db,
                db.execute(
                    "SELECT * FROM manual_handoffs WHERE id=?", (key,)
                ).fetchone(),
            )
    ...
    def end(self, session_id, *, operator, now=None):
        ...
        with self.journal.db() as db:
            ...
            if row is None:
                raise ValueError("Manual handoff not found")
            if row["phase"] == "completed":
                return self._view(db, row)
            db.execute(
                "UPDATE manual_handoffs SET phase=CASE WHEN phase='needs_attention' THEN phase ELSE 'ending' END,end_requested=1,updated_at=? WHERE id=?",
                (now, session_id),
            )
    ...
    def unavailable(self, target, reason, *, now=None):
        ...
        with self.journal.db() as db:
            ...
            participant = db.execute(
                "SELECT 1 FROM manual_handoff_participants WHERE session_id=? AND target_profile_id=?",
                (row["id"], target),
            ).fetchone()
            if not participant:
                return self._view(db, row)
            ...
            db.execute(
                "UPDATE manual_handoffs SET updated_at=?,reason=? WHERE id=?",
                (
                    now,
                    "Waiting for fresh "
                    + target
            ...
            db.execute(
                "UPDATE manual_handoff_participants SET stable_digest=NULL,stable_since=NULL WHERE session_id=?",
                (row["id"],),
            )
            last = db.execute(
                "SELECT 1 FROM manual_handoff_audit WHERE session_id=? AND event='reader_gap' AND json_extract(payload,'$.target')=? LIMIT 1",
                (row["id"], target),
            ).fetchone()
            if not last:
    ...
    def observe(self, target, snapshot, *, bot_busy=False, mouse_idle=True, now=None):
        ...
        with self.journal.db() as db:
            ...
            if not session:
                return None
            part = db.execute(
                "SELECT * FROM manual_handoff_participants WHERE session_id=? AND target_profile_id=?",
                (session["id"], target),
            ).fetchone()
            if not part:
            ...
            try:
                ...
                if not 0 <= now - snapshot["timestamp"] <= 2:
                    raise ValueError("Fresh memory observation is required")
                ...
                character = json.dumps(
                    {k: proof[k] for k in ("character", "character_uid", "server")},
                    sort_keys=True,
                )
                ...
                if not str(session["reason"] or "").startswith(
                    "Waiting for fresh " + target + " observation:"
                ):
                    self._audit(db, session["id"], "reader_gap", now, target=target)
                return self._view(
                    db,
                    db.execute(
                        "SELECT * FROM manual_handoffs WHERE id=?", (session["id"],)
                    ).fetchone(),
                )
            if part["process_json"] and (
                part["process_json"] != process
                or part["character_json"]
                and part["character_json"] != character
            ):
                db.execute(
                    "UPDATE manual_handoffs SET phase='needs_attention',updated_at=?,reason=? WHERE id=?",
                    (now, "Game-process identity changed for " + target, session["id"]),
                )
            ...
            if bot_busy:
                db.execute(
                    ...
                    (
                        now,
                        "Waiting for active bot transaction/input to settle",
                        session["id"],
                    ),
                )
                return self._view(
            ...
            if baseline is None:
                if not closed:
                    db.execute(
                        "UPDATE manual_handoff_participants SET process_json=?,character_json=?,last_json=?,last_at=?,saw_window=1 WHERE session_id=? AND target_profile_id=?",
                        (
                            process,
                            character,
                            json.dumps(snapshot),
                    ...
                    db.execute(
                        "UPDATE manual_handoff_participants SET process_json=?,character_json=?,baseline_json=?,baseline_at=?,last_json=?,last_at=?,stable_digest=?,stable_since=? WHERE session_id=? AND target_profile_id=?",
                        (
                            process,
                            character,
                            json.dumps(snapshot),
            ...
            if not closed:
                ...
                db.execute(
                    "UPDATE manual_handoff_participants SET last_json=?,last_at=?,stable_digest=NULL,stable_since=NULL,saw_window=1 WHERE session_id=? AND target_profile_id=?",
                    (
                        json.dumps(snapshot),
                        snapshot["timestamp"],
                        session["id"],
            ...
            since = (
                part["stable_since"]
                if part["stable_digest"] == digest
                else snapshot["timestamp"]
            )
            db.execute(
                "UPDATE manual_handoff_participants SET last_json=?,last_at=?,stable_digest=?,stable_since=? WHERE session_id=? AND target_profile_id=?",
                (
                    json.dumps(snapshot),
                    snapshot["timestamp"],
                    digest,
    ...
    def _refresh(self, db, session_id, now, *, mouse_idle=True):
        ...
        rows = list(
            db.execute(
                "SELECT * FROM manual_handoff_participants WHERE session_id=?",
                (session_id,),
            )
        )
        ...
        if any(row["baseline_json"] is None for row in rows):
            db.execute(
                "UPDATE manual_handoffs SET phase='preparing',updated_at=?,reason='Waiting for closed-window baseline' WHERE id=?",
                (now, session_id),
            )
            return self._view(
                db,
        ...
        if session["phase"] == "preparing":
            db.execute(
                "UPDATE manual_handoffs SET phase='ready',updated_at=?,reason=NULL WHERE id=?",
                (now, session_id),
            )
        ...
        finish = bool(session["end_requested"]) or any(
            row["saw_window"] for row in rows
        )
        stable = all(
            row["stable_since"] is not None
            and row["last_at"] - row["stable_since"] >= 5
            and 0 <= now - row["last_at"] <= 2
            for row in rows
        )
        if finish and stable and mouse_idle:
            ...
            db.execute(
                ...
                (
                    now,
                    "Waiting for mouse to be idle before handoff settlement",
                    session_id,
                ),
            )
        elif session["reason"] and not (
            str(session["reason"]).startswith("Waiting for fresh ")
            and any(row["last_at"] is None or now - row["last_at"] > 2 for row in rows)
        ):
            db.execute(
                "UPDATE manual_handoffs SET updated_at=?,reason=NULL WHERE id=?",
                (now, session_id),
            )
        return self._view(
            db,
    ...
    def _complete(self, db, session, rows, now):
        ...
        for row in rows:
            current = json.loads(row["last_json"])
                    ...
                    json.dumps(
                        {
                            ...
                            "to": at,
                            "reason": "operator_manual_handoff",
            ...
            db.execute(
                ...
                (target, json.dumps(baseline), original[0] if original else at),
            )
            before_items = {
                item["uid"]: tuple(
                    item[k]
            ...
            if added and row["role"] == "Merchant":
                db.execute(
                    "INSERT OR REPLACE INTO state VALUES(?,?,?)",
                    (target, "new_stock", "true"),
                )
            db.execute(
            ...
            self._audit(
                ...
                ownership_delta=_ownership_delta(
                    canonical_ownership(before), canonical_ownership(current)
                ),
            )
        db.execute(
            "UPDATE manual_handoffs SET phase='completed',completed_at=?,updated_at=?,reason=NULL WHERE id=?",
            (now, now, session["id"]),
        )
