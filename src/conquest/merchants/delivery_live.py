...
def start(ui, stage):
    if stage == "cancel-reserved-request":
        ...
        return start(ui)
    if stage not in ("accept", "offer", "confirm", "cancel"):
        raise ValueError("Unknown delivery qualification stage")
    if (
    ...
    if not ui.safe_to_yield():
        raise ValueError("Farmer input has not been released")
    ...
    state = read_probe()
    phases = {
        "accept": {"request_verified"},
        "cancel": {"request_verified"},
        "offer": {"trade_open_verified"},
        "confirm": {"offer_verified", "farmer_confirm_verified"},
    }
    ...
    def work():
        try:
            if stage == "confirm":
                module.run(ui, state, revision=revision)
            else:
                module.run(ui, state)
    ...
    ui.delivery_probe_thread = threading.Thread(
        target=work, name="delivery-" + stage + "-probe", daemon=True
    )
