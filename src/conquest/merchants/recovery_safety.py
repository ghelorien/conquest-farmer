...
def operator_override(
    ...
    *,
    ...
    with runtime.journal.db() as db:
        ...
        if row:
            db.execute(
                "UPDATE state SET value=? WHERE character=? AND name=?",
                (encoded, character, KEY),
            )
        else:
        ...
        matching_attention = (
            ...
            and attention_value.get("kind") == "recovery_stalled"
        )
    ...
    runtime.journal.event(
        character,
        "recovery_safety_operator_overridden",
        original_evidence_digest=digest,
        confirmation_reference=confirmation_reference.strip(),
    )
    return state
...
def arm(runtime, character, *, now=None):
    ...
    runtime.journal.set(
        ...
        {
            "active": True,
            "source": "unexpected_connection_loss",
...
def submitted(runtime, character, *, now=None):
    ...
    if state.get("active") and state.get("last_progress") is None:
        state.update(
            phase="logging_in", last_progress=time.time() if now is None else now
        )
...
def observe(runtime, character, identity, life=None, *, now=None, close=None):
    ...
    if life is not None:
        ...
        if (
            ...
            state.update(
                best_distance=distance, last_progress=now, phase="returning_to_market"
            )
    if state.get("last_progress") is None:
        state["last_progress"] = now
    runtime.journal.set(character, KEY, state)
    if now - state["last_progress"] < 5:
        return True
    ...
    if cancel:
        cancel.set()
    state.update(
        phase="disconnect_pending",
        reason="Recovery made no verified progress for five seconds",
        identity=identity,
        disconnected=False,
    )
    runtime.journal.set(character, KEY, state)
    ...
    state.update(
        active=not state["disconnected"],
        phase="disconnected" if state["disconnected"] else "disconnect_pending",
    )
    runtime.journal.set(character, KEY, state)
    if state["disconnected"]:
        ...
        runtime.journal.event(
            character, "recovery_safety_disconnect", reason=state["reason"]
        )
    return True
