...
def reconcile(journal, character, before, after, *, sources, apply=False):
    ...
    with journal.db() as db:
        ...
        rows = list(
            db.execute(
                "SELECT * FROM sales WHERE character=? AND observed_at>? AND observed_at<=? ORDER BY id",
                (character, start, end),
            )
        )
        if not rows:
        ...
        key = hashlib.sha256(
            json.dumps([character, [r["id"] for r in rows]]).encode()
        ).hexdigest()
        existing = db.execute(
            "SELECT evidence FROM sales_reconciliations WHERE id=?", (key,)
        ).fetchone()
        if existing:
        ...
        baseline = db.execute(
            "SELECT started_at FROM sales_baseline WHERE character=?", (character,)
        ).fetchone()
        if not baseline or any(r["observed_at"] < baseline[0] for r in rows):
            raise ValueError("Departures predate tracking")
        tx = list(
            db.execute(
                "SELECT * FROM transactions WHERE character=? AND created<=? AND updated>=?",
                (character, end, start),
            )
        )
        if any(
            t["kind"] != "listing"
            or t["phase"] not in ("verified", "aborted", "operator_overridden")
            for t in tx
        ):
        ...
        if len({i["uid"] for i in items}) != len(items) or any(
            (i["uid"], i["price"]) not in listed or i["uid"] in present for i in items
        ):
        ...
        if not low <= gain <= high:
            raise ValueError(
                "Balance gain does not reconcile with net listing proceeds"
            )
        evidence = dict(
            character=character,
            ...
            items=sum(i["quantity"] for i in items),
        )
        if apply:
            db.execute(
                "INSERT INTO sales_reconciliations VALUES(?,?,?,?,?,?,?)",
                (
                    key,
                    character,
            ...
            for r in rows:
                db.execute(
                    "UPDATE sales SET phase='reconciled',note=? WHERE id=?",
                    ("Net proceeds in historical reconciliation " + key, r["id"]),
                )
            db.execute(
                "INSERT INTO events(character,event,payload,timestamp) VALUES(?,?,?,?)",
                (
                    character,
                    "sales_history_reconciled",
                    json.dumps(
                        {
                            "receipt": key,
                            "silver": gain,
