...
def reserve(journal,request_id,farmer,merchant,items,*,origin=None,now=None):
    ...
    if origin and (origin.get('operation_id')!=request_id
            or not isinstance(origin.get('farmer_profile_id'),str)
            or not origin['farmer_profile_id']
            or any(origin.get(name) is not None and not isinstance(origin[name],str)
    ...
    if row:
        ...
        if (exact_items(previous['intent']['items'])!=exact_items(items)
                ...
                or any(previous['intent'].get(name)!=origin.get(name) for name in
...
def finish(journal,character,request_id,farmer,merchant,*,now=None):
    ...
    try:outcome=reconciliation_outcome(state['intent'],farmer,merchant,
                                      sale_receipts=sales,now=now)['outcome']
...
def disposition(journal,character,request_id,farmer,merchant,*,trace,intent=None,now=None):
    ...
    if not row:
        ...
        result=reconciliation_outcome(intent,farmer,merchant,trace=trace,
                                      sale_receipts=sales,now=now)
        actions=[step for step in trace if step.get('status')=='before_action']
        if result['outcome']!='no_transfer' or actions:
            raise ReconciliationBlocked('Missing reservation can only prove a no-input no-transfer')
        current={'request_id':request_id,'phase':'no_transfer_reconciled',
                 'created_at':intent.get('created_at',time.time() if now is None else now),
                 'intent':intent,'disposition':result}
        save(journal,character,current,creating=True)
        journal.event(character,'delivery_no_transfer_reconciled',request_id=request_id,
                      delivered=[],remaining=[i['uid'] for i in result['remaining']],
                      proof_digest=result['proof_digest'],reservation_missing=True)
        return result
    ...
    result=reconciliation_outcome(current['intent'],farmer,merchant,trace=trace,
                                  sale_receipts=sales,now=now)
    if result['outcome'] not in ('no_transfer','partial_transfer'):
        raise ReconciliationBlocked('Delivery completed and requires the verified finish path')
    phase='no_transfer_reconciled' if result['outcome']=='no_transfer' else 'partial_aborted_reconciled'
    current.update(phase=phase,disposition=result)
    save(journal,character,current)
    journal.event(character,'delivery_'+result['outcome']+'_reconciled',request_id=request_id,
