...
def control(driver, snapshot):
    ...
    constants = {
        0x5D10DC: b"#CLOSE\0",
        0x5E6880: b"Trade##TradeWindow\0",
    ...
    if any(
        struct.unpack("<Q", read(base + TRADE_VTABLE + slot, 8))[0] != base + rva
    ...
    if (
        width <= 0
        or height <= 0
        or not x < point[0] < x + width
        or not y < point[1] < y + height
        or not 0 <= point[0] < viewport[0]
        or not 0 <= point[1] < viewport[1]
    ):
