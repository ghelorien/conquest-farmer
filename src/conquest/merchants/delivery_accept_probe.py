...
def lease_authorized(ui, character):
    ...
    try:
        ...
        if (
            coordinator.purpose != "delivery_accept_probe"
            or worker is None
            or worker.ident != threading.get_ident()
        ...
        if (
            ...
            or state.get("phase") not in ("request_verified", "accept_submitted")
        ...
        if (
            ...
            or merchant.get("character") != str(character)
        ...
        if (
            observer is None
            or observer.adapter.identity != merchant["identity"]
        ...
        return not active(runtime.journal, profile) and not runtime.journal.pending(
            profile
        )
...
def control(driver, snapshot):
    ...
    if s.read_block(model + 12, 1) != b"\x01" or [
        string(s, model + o) for o in (0x48, 0x68, 0x88, 0xA8)
    ] != [
        "Trade###Confirm",
        f"{farmer_name()} wishes to trade with you.",
        "Accept",
        "Cancel",
    ]:
    ...
    if (
        not all(
            type(value) in (int, float) and math.isfinite(value) for value in geometry
        )
...
def exact_incoming_request(intent, merchant):
    ...
    if not isinstance(farmer, dict) or not isinstance(request, dict):
        return False
    return (
...
def run(ui, state):
    ...
    if profiles:
        try:
            ...
            raise ValueError(
                "Delivery acceptance merchant profile is unavailable"
            ) from error
        if (
            resolved.id != profile_id
    ...
    def manual_fence():
        if any(
            ui.coordinator.manual_session_blocked(
                owner, purpose="delivery_accept_probe"
            )
            for owner in (
                state.get("farmer_profile_id", "Farmer"),
                profile_id or character,
            )
        ):
    ...
    def fresh():
        ...
        if (
            ...
            or m.get("trade")
            or f.get("request")
            or not exact_incoming_request(intent, m)
        ):
        ...
        for role, snapshot in (("farmer", f), ("merchant", m)):
            ...
            if any(
                snapshot[k] != old[k]
                for k in ("identity", "character_uid", "position", "silver")
            ) or exact_items(snapshot["inventory"]) != exact_items(old["inventory"]):
                raise ValueError(
                    "Participants or inventory changed before request acceptance"
                )
        current = read_probe()
        if (
            not isinstance(current, dict)
            or current.get("phase") not in ("request_verified", "accept_submitted")
        ...
        ownership(
            current,
            str(character),
            profile_id or str(character),
            current.get("farmer_profile_id", "Farmer"),
            f,
            m,
            now=time.time(),
        )
    ...
    try:
        ...
        def lease():
            with ui.coordinator.lock:
                ...
                reconciled = ui.runtime.reconcile_probe_owned(
                    character, _farmer, merchant
                )
                if not reconciled:
                    raise CaptureUnavailable(
                        "Delivery acceptance needs fresh bilateral probe reconciliation"
                    )
                manual_fence()
                with (
                    ui.coordinator.lease(character, purpose="delivery_accept_probe"),
        ...
        with lease():
            ...
            binding = control_binding(w, point)
            size = driver.target.snapshot()["client_size"]
            if size != driver.memory.gui.viewport_size():
                raise ValueError("Native and GUI dimensions differ")

            def before():
                f, m = fresh()
                if native1078:
                    native_foreground(driver, m["identity"])
                current, now_point = control(driver, m)
                if control_binding(current, now_point) != binding:
                    raise ValueError("Accept control moved")
