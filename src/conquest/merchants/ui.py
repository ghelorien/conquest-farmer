...
def wait_for_merchant_surface(
    host, others, check, *, clock=time.monotonic, sleep=time.sleep
):
    ...
    while True:
        ...
        if (
            gui.IsWindowVisible(host.parent)
            and gui.IsWindowVisible(host.saved.hwnd)
            and not gui.IsIconic(host.saved.hwnd)
            and host.api.is_above(host.saved.hwnd, gui.GetAncestor(host.parent, 2))
            and gui.GetWindowRect(host.saved.hwnd)
            == (x, y, x + right - left, y + bottom - top)
            and all(
                not other.api.gui.IsWindowVisible(other.saved.hwnd)
                for other in others
                if other.saved
            )
        ):
        ...
        if clock() >= deadline:
            raise ValueError(
                "Waiting for the selected merchant window to become visible; no game input sent"
            )
        sleep(0.025)
...
def client_tab_character(notebook, frames, detail_tabs, client_tabs):
    ...
    for character in CHARACTERS:
        if selected == str(frames[character]) and str(
            detail_tabs[character].select()
        ) == str(client_tabs[character]):
            return character
    return None
...
def refresh_permission_menu(menu, entries, state):
    ...
    menu.entryconfigure(
        manage,
        label=("Pause" if state["enabled"] else "Enable") + " trading & repricing",
    )
    menu.entryconfigure(
        refill,
        label=("Pause" if state["refill"]["enabled"] else "Enable")
        + " automatic refill",
    )
...
class UnifiedUI:
    def __init__(self, app):
        ...
        self.input_revision_marker = (
            app.control.snapshot()["revision"],
            bool(app.mouse_priority.active()),
        )
        ...
        self.header_labels = [
            ...
            ttk.Label(
                self.header,
                textvariable=self.silver_text,
                anchor="w",
                font=("Segoe UI", 10, "bold"),
            ),
            ttk.Label(
                self.header,
                textvariable=self.on_sale_text,
                anchor="w",
                font=("Segoe UI", 10, "bold"),
            ),
        ]
        ...
        self.notebook.bind(
            "<<NotebookTabChanged>>", lambda event: self.on_tab_changed()
        )
        self.root.bind(
            "<Configure>",
            lambda event: (
                self.schedule_visibility() if event.widget == self.root else None
            ),
            add="+",
        )
        ...
        self.coordinator = InputCoordinator(
            self.safe_to_yield, app.mouse_priority.active
        )
        self.runtime = MerchantRuntime(app.catalog, self.coordinator)
        self.released_clients = {
            c for c in CHARACTERS if self.runtime.journal.get(c, "host_released", False)
        }
        from conquest.merchants.refill_1078 import step as refill1078_step

        self.runtime.refill1078_step = lambda character, snapshot: refill1078_step(
            self, character, snapshot
        )
        from conquest.merchants.listing_handoff_1078 import (
            farmer_safe as native_farmer_safe,
        )
        ...
        self.runtime.configure_manual_farmer(
            lambda: getattr(app, "observer", None), app.control.snapshot
        )
        ...
        def owner_allowed(character):
            if (self.grant or {}).get("scope") == "merchant_host":
                return (
                    is_farmer_owner(character)
                    and self.coordinator.purpose == "merchant_host"
                )
            if (self.grant or {}).get("scope") == "delivery_empty_recovery":
                ...
                return (
                    self.coordinator.purpose == "delivery_empty_recovery"
                    and lease_authorized(self, character)
                )
            if (self.grant or {}).get("scope") == "listing_1078":
                if character != self.grant.get(
                    "character"
                ) or self.coordinator.purpose not in (
            ...
            if (
                self.coordinator.purpose == "delivery_confirm_probe"
                and self.runtime.read_only_1078(character)
            ):
                return self.coordinator.native_trade1078_authorized(character)
            return self.runtime.input_allowed(character) or (
                not getattr(self.runtime, "delivery_window", None)
                and not getattr(self.runtime, "refill_window", None)
                and character in self.calibrating
                and not self.calibration_cancel[character].is_set()
            )
        ...
        self.manual_displayed, self.manual_texts, self.manual_buttons = (
            ProfileMap(),
            ProfileMap(),
            ProfileMap(),
        )
        ...
        write_json(
            state_path(".runtime/merchants/app-lifecycle.json"),
    ...
    def safe_to_yield(self):
        ...
        if self.grant:
            ...
            if fence and (
                fence.active is None
                or fence.active.request_id != self.grant["request_id"]
            ):
                return False
            if (
                self.grant["expires_at"] > time.time()
                and self.grant["revision"] == control["revision"]
                and not control["enabled"]
        ...
        if (
            route.get("phase") not in (None, "stopped", "completed", "failed")
            and process_alive(route.get("pid")) is not False
        ):
            return False
        return True
    ...
    def dispatch(self, body):
        ...
        if body == {"action": "profiles"}:
            return {"profiles": profile_status()}
        ...
        if action == "show-merchant-view" and set(body) == {"action", "character"}:
            ...
            if observer is None:
                raise ValueError(
                    "Merchant view requires an already identified exact client"
                )
            expected = (host, observer, observer.hwnd, dict(observer.adapter.identity))
            done, result = threading.Event(), {}

            def show():
                result.update(present_user_view(self, character, expected=expected))

            self.ui_requests.put((show, done, result))
            if not done.wait(5):
                result["expired"] = True
                raise ValueError(
                    "Merchant display request expired without a confirmed view"
                )
            if result.get("error"):
                raise ValueError(result["error"])
            return result
        if action in (
            "merchant-owned-panel-prepare-1078",
            "merchant-owned-panel-status-1078",
        ...
        if action in (
            "merchant-booth-list-once-1078",
            "merchant-booth-list-once-status-1078",
            "merchant-booth-list-once-reconcile-1078",
            "merchant-booth-list-once-cancel-1078",
        ):
        ...
        if action == "merchant-booth-confirm-diagnostic-1078":
            if set(body) != {"action", "character"}:
                raise ValueError(
                    "1078 booth confirm diagnostic requires exactly one merchant"
                )
            from conquest.merchants.booth_confirm_diagnostic_1078 import collect

            return collect(self.runtime, body["character"])
        if action in (
            "merchant-observe-1078",
            "merchant-listing-preflight-1078",
            "merchant-booth-target-preflight-1078",
        ):
            if set(body) != {"action", "character"}:
                raise ValueError(
                    "Read-only merchant observation requires exactly one character"
                )
        ...
        if action == "merchant-restoration-preview-1078":
            if set(body) != {"action", "character"}:
                raise ValueError(
                    "Read-only restoration preview requires exactly one merchant"
                )
        ...
        if action == "merchant-refill-preview-1078":
            if set(body) != {"action", "character"}:
                raise ValueError(
                    "Read-only refill preview requires exactly one merchant"
                )
        ...
        if action == "merchant-refill-cursor-reconcile-1078":
            if set(body) != {"action", "character"}:
                raise ValueError(
                    "1078 refill cursor reconciliation requires exactly one merchant"
                )
        ...
        if action == "merchant-market-arrival-1078":
            if set(body) != {"action", "character"}:
                raise ValueError(
                    "Market arrival settlement requires exactly one merchant"
                )
        ...
        if action in (
            "delivery-stale-pre-admission-preview",
            "delivery-stale-pre-admission-clear",
        ):
            from conquest.merchants.pre_admission_clear import dispatch

            return dispatch(self, body)
        if action in (
            "farmer-loop-acceptance",
            "farmer-loop-acceptance-status",
            "farmer-loop-acceptance-abort",
            "farmer-loop-acceptance-override-preview",
            "farmer-loop-acceptance-override",
        ):
            from conquest.merchant_loop_acceptance import configure

            return configure(self, body)
        if action == "notification-workers-restart" and set(body) == {
            "action",
            "worker",
        }:
        ...
        if action == "trade-qualification-prep-target" and set(body) == {
            "action",
            "character",
        }:
        ...
        if action in ("recovery-status", "recovery-recheck", "recovery-override"):
            allowed = (
                ...
                else {"action", "character", "incident_id"}
            )
            ...
            if set(body) != allowed:
                raise ValueError("Unsupported recovery command arguments")
            character = character_name(body["character"])
            incidents = self._merchant_recovery_incidents(character)
            incident = (
                next(
                    (row for row in incidents if row["id"] == body.get("incident_id")),
                    None,
                )
                if action != "recovery-status"
                else None
            ...
            if action == "recovery-status":
                return {"character": character, "incidents": incidents}
            if incident is None:
            ...
            if action == "recovery-recheck":
                ...
                return {
                    "incident": fresh,
                    "incident_digest": fresh.get("digest"),
                    "rechecked": True,
                }
            if body.get("operator_confirmed") is not True or body.get(
                "confirmation_reference"
            ) != body.get("incident_digest"):
            ...
            return self._apply_merchant_override(
                character,
                incident,
                body["incident_digest"],
                operator=body.get("operator"),
            )
        if action == "farmer-view-height" and set(body) == {"action", "scale"}:
            scale = float(body["scale"])
            if not 1 <= scale <= 1.15:
                raise ValueError("Height scale must be between 1 and 1.15")
            if self.app.control.snapshot()["enabled"]:
                raise ValueError("Stop farming before resizing")
            from conquest.discord_notify import write_json
        ...
        if action == "cancel-empty-delivery" and set(body) == {"action", "character"}:
            ...
            return start(self, body["character"])
        if action == "cancel-empty-delivery-reconcile" and set(body) == {
            "action",
            "character",
        }:
        ...
        if action == "probe-delivery-request" and set(body) == {
            "action",
            "character",
            "uids",
        }:
        ...
        if action == "probe-delivery-promote":
            if set(body) != {"action"}:
                raise ValueError("Unsupported delivery promotion arguments")
        ...
        if action == "probe-delivery-reconciliation-diagnostic":
            if set(body) != {"action"}:
                raise ValueError("Unsupported reconciliation diagnostic arguments")
            ...
            with self.coordinator.lock:
                last = probe_attempt_projection(
                    getattr(self.runtime, "last_probe_reconciliation", None)
                )
                state = read_probe(read_only=True)
                if not state:
                    raise ValueError("No supervised delivery probe exists")
                character = character_name(state["character"])
                farmer, merchant = pair(self, character)
                return {
                    **self.runtime.inspect_probe_reconciliation(
                        character, farmer, merchant
                    ),
                    "last_attempt": last,
                }
        ...
        if action == "probe-delivery-override":
            ...
            if "operator" in body:
                allowed.add("operator")
            if set(body) != allowed:
                raise ValueError("Unsupported trade-probe override arguments")
            ...
            return operator_override(
                ...
                incident_digest=body["incident_digest"],
                operator=body.get("operator"),
            )
        ...
        if action == "prepare-trade-qualification" and set(body) == {
            "action",
            "character",
            "selected_uid",
        }:
        ...
        if action == "trade-qualification-prep-override":
            ...
            if "operator" in body:
                allowed.add("operator")
            if set(body) != allowed:
                raise ValueError("Unsupported trade-prep override arguments")
        ...
        if action == "reconcile-stall-inspection" and set(body) == {
            "action",
            "character",
        }:
            character = character_name(body["character"])
            ...
            with observer.lock:
                return reconcile_interrupted_probe(
                    self.runtime.controllers[character].driver, self.runtime.journal
                )
        ...
        if action == "peer-identity-evidence" and set(body) == {
            ...
            character = character_name(body["character"])
            peer = character_name(body["peer"])
            if character == peer:
                raise ValueError("Choose two different merchant clients")
            from conquest.merchants.identity_evidence import peer_evidence

            source = self.runtime.observers.get(character)
            other = self.runtime.observers.get(peer)
            ...
            with first.lock, second.lock:
                ...
                if any(
                    before[k] != after[k]
                    for k in ("identity", "character_uid", "position", "map_id")
                ):
                    raise ValueError("Peer identity changed across client observations")
                return evidence
        if action == "connect-market" and set(body) in (
            {"action", "character"},
            {"action", "character", "client_pid"},
        ):
            from conquest.merchants.connect_market import start
        ...
        if action == "delivery-window" and set(body) == {"action", "request_id"}:
            ...
            try:
                ...
                current = getattr(self.runtime, "delivery_window", None) or getattr(
                ...
                if current:
                    ...
                    return {
                        "requested": key
                    }  # A retry cannot restart timers or revert refill to trade.
        ...
        if action == "delivery-refill" and set(body) == {"action", "request_id"}:
            key = body["request_id"]
            if (
                getattr(self.runtime, "delivery_window", None) != key
                or self.runtime.handoff != key
                or not self.grant
                or self.grant["request_id"] != key
                or not self.safe_to_yield()
            ):
                raise ValueError(
                    "Delivery refill requires the original unexpired safe window"
                )
            ...
            if (
                not receipt
                or receipt["phase"] != "verified"
                or worker
                and worker.is_alive()
                or getattr(self, "delivery_errors", {}).get(key)
            ):
                raise ValueError(
                    "Both delivery participants must reconcile before refill"
                )
            held = self.runtime.journal.get(
                receipt["character"], "delivery_reservation", {}
            )
            ...
            try:
                ...
                if self.coordinator.owner:
                    raise ValueError("Merchant still owns delivery input")
                ...
                for character in CHARACTERS:
                    if self.runtime.refill_enabled(character) and (
                        ...
                        start_refill(
                            self.runtime,
                            character,
                            visit_id=self.grant.get("visit_id"),
                            town_visit_id=self.grant.get("town_visit_id"),
                            operation_id=key,
        ...
        if action == "empty-delivery-sales-promote":
            if set(body) != {"action", "request_id", "sale_ids", "confirmation"}:
                raise ValueError(
                    "Exact incident, sale IDs and operator statement are required"
                )
        ...
        if action == "focus-farmer" and set(body) == {"action"}:
            ...
            if result.get("error"):
                raise ValueError(result["error"])
            return result
        if action == "manual-status" and set(body) in (
            {"action"},
            {"action", "character"},
        ):
            ...
            if character is not None:
                character = (
                    "Farmer" if character == "Farmer" else character_name(character)
                )
            return {
                "sessions": self.runtime.manual_status(character),
        ...
        if action == "manual-handoff-start" and set(body) in (
            {"action"},
            {"action", "operator"},
        ):
            return self.runtime.start_manual_handoff(
                operator=body.get("operator", "bridge operator")
            )
        if action == "manual-handoff-end" and set(body) in (
            {"action", "session_id"},
            {"action", "session_id", "operator"},
        ):
            return self.runtime.end_manual_handoff(
                body["session_id"], operator=body.get("operator", "bridge operator")
            )
        if action in ("manual-approve", "manual-reject"):
            allowed = {"action", "binding"} | (
                {"operator"} if "operator" in body else set()
            )
            if set(body) != allowed:
                raise ValueError("Unsupported manual decision arguments")
            binding = body.get("binding")
            if not isinstance(binding, dict):
                raise ValueError("Exact displayed approval binding is required")
            operation = (
                self.runtime.approve_manual
                if action == "manual-approve"
                else self.runtime.reject_manual
            )
        ...
        if action == "manual-override":
            ...
            if set(body) != allowed:
                raise ValueError("Unsupported manual override arguments")
            return self.runtime.override_manual(
                body["session_id"],
                confirmation_reference=body["confirmation_reference"],
        ...
        if action == "status" and set(body) == {"action"}:
            ...
            return {
                "characters": self.runtime.status(),
                "input_owner": self.coordinator.owner,
                ...
                "ui_health": {
                    **self.ui_health,
                    "tick_age_ms": round((time.monotonic() - self.last_ui_tick) * 1000),
                },
        ...
        if action == "background-probe" and set(body) == {
            "action",
            "character",
            "mode",
        }:
        ...
        if action == "background-item-status" and set(body) == {
            "action",
            "character",
            "uid",
        }:
        ...
        if action == "background-minimize" and set(body) == {"action"}:
            if probe_busy(self):
                raise ValueError(
                    "Wait for the active diagnostic before minimizing Conquest"
                )
        ...
        if action == "market-refresh" and set(body) <= {
            "action",
            "request_id",
            "character",
        }:
            ...
            return {
                c: self.runtime.market_worker.request(c, body["request_id"])
                for c in targets
            }
        ...
        if (
            action == "merchant-enabled"
            and set(body) == {"action", "character", "enabled"}
        ...
        if (
            action == "window-state"
            and set(body) == {"action", "maximized"}
            and type(body["maximized"]) is bool
        ):
        ...
        if action == "return-route-status" and set(body) == {"action", "character"}:
            ...
            with observer.lock:
                snapshot = self.runtime.controllers[character].driver.memory.read(
                    recovery=True
                )
            ...
            return {
                ...
                "flags": flags,
                "return_state": self.runtime.journal.get(character, "shop_return"),
            }
        if action == "reload-app" and set(body) == {"action"}:
            if (
                probe_busy(self)
                or not self.safe_to_yield()
                or self.coordinator.owner
                or self.calibrating
                or self.runtime.refilling
                or any(
                    self.runtime.enabled(c) or self.runtime.journal.pending(c)
                    for c in CHARACTERS
                )
            ):
                raise ValueError(
                    "Pause merchants and finish active work before reloading"
                )
            self.ui_requests.put((self.app.restart, None, {}))
            return {"requested": True}
        if (
            action == "refill-enabled"
            and set(body) == {"action", "character", "enabled"}
            and type(body["enabled"]) is bool
        ):
            character = character_name(body["character"])
            if body["enabled"]:
                self.resume_refill(character)
            else:
                self.runtime.set_refill_enabled(character, False)
            return {
                "character": character,
                "enabled": self.runtime.refill_enabled(character),
            }
        if action == "attach-farmer-client" and set(body) == {
            "action",
            "client_pid",
            "client_started",
        }:
            ...
            if not done.wait(3):
                result["expired"] = True
                raise ValueError(
                    "Farmer attachment pending; inspect current state before retrying"
                )
            if result.get("error"):
                raise ValueError(result["error"])
            return result
        ...
        if action == "verify-booth" and set(body) == {"action", "character"}:
            character = character_name(body["character"])
            self.ui_requests.put(
                (lambda: self.start_qualification(character), None, {})
            )
            return {"requested": character, "listing_submitted": False}
        if action == "scan" and set(body) <= {"action", "request_id", "character"}:
            ...
            return {
                c: self.runtime.journal.request_scan(c, body["request_id"])
                for c in targets
            }
        ...
        if action == "handoff-grant" and set(body) in (
            {"action", "request_id", "revision", "expires_at", "safe"},
            ...
            if (
                "scope" in body
                and not (market or listing or hosting)
                or market
                and "visit_id" not in body
                or listing
                and "character" not in body
            ):
                raise ValueError("Unknown handoff scope or binding")
            if (
                body["request_id"] != (host_request or {}).get("request_id")
                if hosting
                else body["request_id"] != self.runtime.handoff
            ):
                raise ValueError("Handoff request changed")
            if (
                body["safe"] is not True
                or body["revision"] != control["revision"]
                or control["enabled"]
                or control.get("paused")
                or type(body["expires_at"]) not in (int, float)
                or not 0
                < body["expires_at"] - time.time()
                <= (60 if market else 45 if listing else 15)
            ):
                raise ValueError(
                    "Farmer must explicitly grant a current bounded safe handoff"
                )
            visit = None
            if market:
            ...
            if hosting:
                if (
                    ...
                    raise ValueError(
                        "Release current merchant work before host attachment"
                    )
                    ...
                    for refill in getattr(self.runtime, "refills", {}).values():
                        if refill.state().get("pending"):
                            refill.start(
                                ...
                                town_visit_id=visit.get("town_visit_id"),
        ...
        if action == "handoff-release" and set(body) == {"action", "request_id"}:
            ...
            previous_handoff = (
                (self.grant or {}).get("previous_handoff") if hosting else None
            )
            if hosting:
                self.host_release_pending = (body["request_id"], previous_handoff)
            saved_host_release = getattr(self, "host_release_pending", None)
            if saved_host_release and saved_host_release[0] == body["request_id"]:
                hosting = True
                previous_handoff = saved_host_release[1]
            fence = getattr(self, "grant_fence", None)
    ...
    def build_overview(self):
        ...
        ttk.Label(frame, text="Conquest", font=("Segoe UI", 22, "bold")).pack(
            anchor="w", padx=20, pady=15
        )
        ttk.Label(frame, text="Characters and coordinated input on this PC").pack(
            anchor="w", padx=20
        )
        for name in ("Farmer", *CHARACTERS):
            box = ttk.LabelFrame(frame, text=name, padding=15)
            ...
            ttk.Label(box, textvariable=manual, wraplength=900, justify="left").pack(
                anchor="w", fill="x"
            )
            ...
            override = ttk.Button(
                actions,
                text="Resolve attention hold…",
                state="disabled",
                command=lambda target=name: self.override_manual_displayed(target),
            )
            approve.pack(side="left")
            reject.pack(side="left", padx=6)
            override.pack(side="left")
            self.manual_buttons[name] = {
                "approve": approve,
                "reject": reject,
                "override": override,
            }
        ...
        ttk.Button(row, text="Update all shops now", command=self.list_once).pack(
            side="left"
        )
        ttk.Button(row, text="How shop controls work", command=self.shop_help).pack(
            side="left", padx=8
        )
        ttk.Button(
            row, text="Configure Discord #shops", command=self.configure_shops
        ).pack(side="left", padx=8)
        ttk.Button(
            row, text="Stop all (including farmer)", command=self.global_stop
        ).pack(side="right")
        ...
        ttk.Label(
            handoff_row,
            textvariable=self.manual_handoff_text,
            wraplength=860,
            justify="left",
        ).pack(anchor="w")
        self.manual_handoff_button = ttk.Button(
            handoff_row, text="Start manual handoff", command=self.toggle_manual_handoff
        )
        self.manual_handoff_button.pack(anchor="w", pady=(5, 0))
        self.discord_note = tk.StringVar(
            value="Checking Discord notification services..."
        )
        ttk.Label(frame, textvariable=self.discord_note, wraplength=900).pack(
            anchor="w", padx=20, pady=6
        )
        self.input_note = tk.StringVar()
        ttk.Label(frame, textvariable=self.input_note, wraplength=900).pack(
    ...
    def refresh_manual_operator(self, statuses, control, *, now=None):
        ...
        from conquest.merchants.manual_operator import (
            displayed,
            status_text,
            action_state,
        )
        ...
        for target in ("Farmer", *CHARACTERS):
            row = (
                farmer.get("session")
                if target == "Farmer"
                else self.runtime.manual_status(target)
            )
            shown = displayed(row)
            ...
            intent = (
                ...
                if target == "Farmer"
                    ...
                    "refill_enabled": bool(
                        statuses.get(target, {}).get("refill", {}).get("enabled")
                    ),
                }
            )
            self.manual_texts[target].set(
                status_text(target, shown, farmer_status=farmer, intent=intent, now=now)
            )
        ...
        if hasattr(self, "manual_handoff_text"):
            if handoff:
                ...
                note = handoff.get("reason") or (
                    "Waiting for stable closed windows."
                    if phase in ("preparing", "ending")
                    else "User controls all game actions; automation remains fenced."
                )
                self.manual_handoff_text.set(
                    f"{phase.title()} · {count} participant(s) · {note}"
                )
                self.manual_handoff_button.configure(text="End manual handoff")
            else:
                if getattr(
                    getattr(self.runtime, "manual_1078_registry", None),
                    "read_only_build",
                    False,
                ):
                    self.manual_handoff_text.set(
                        "1078 manual observation ready; farming and merchant automation are unavailable. Start, then wait for Ready."
                    )
                else:
                    self.manual_handoff_text.set(
                        "No global handoff active. Start fences attached Farmer/merchant automation before you act."
                    )
    ...
    def _displayed_manual(self, target, *, binding=False):
        ...
        if row is None:
            raise ValueError("The displayed manual session is no longer available")
    ...
    def approve_manual_displayed(self, target):
        ...
        try:
            row = self._displayed_manual(target, binding=True)
        except ValueError as error:
            messagebox.showerror("Manual visitor", str(error), parent=self.root)
            return None
        ...
        if not messagebox.askyesno(
            "Approve exact manual visitor", prompt, parent=self.root
        ):
            return None
        try:
            return self.runtime.approve_manual(
                row["approval_binding"], operator="local UI"
            )
        except (ValueError, OSError) as error:
            messagebox.showerror("Manual visitor", str(error), parent=self.root)
            return None
    ...
    def reject_manual_displayed(self, target):
        ...
        if not messagebox.askyesno(
            "Reject exact manual request", prompt, parent=self.root
        ):
            return None
        try:
            return self.runtime.reject_manual(
                row["approval_binding"], operator="local UI"
            )
        except (ValueError, OSError) as error:
            messagebox.showerror("Manual visitor", str(error), parent=self.root)
    ...
    def override_manual_displayed(self, target):
        ...
        try:
            row = self._displayed_manual(target)
        except ValueError as error:
            messagebox.showerror("Manual visitor", str(error), parent=self.root)
            return None
        if row.get("phase") != "needs_attention" and not row.get("rebaseline"):
            messagebox.showerror(
                "Manual visitor",
                "Only a displayed attention/rebaseline hold can be overridden.",
                parent=self.root,
            )
            return None
        reason = simpledialog.askstring(
            "Manual visitor hold",
            "Reason for the explicit disposition/retry:",
            parent=self.root,
        )
        if not reason:
            return None
        ...
        if not reference:
            return None
        if not messagebox.askyesno(
            "Confirm manual visitor disposition",
            f"Target: {target}\nExact session: {row.get('id')}\nReason: {reason}\n\n"
            "This does not prove a transfer or enable automation. Fresh stable memory is still required.",
            parent=self.root,
        ):
            return None
        try:
    ...
    def configure_shops(self):
        ...
        value = simpledialog.askstring(
            "Discord #shops",
            "Paste the webhook URL created in #shops. It is encrypted locally.",
            show="*",
            parent=self.root,
        )
        if value:
            try:
                ...
                messagebox.showinfo(
                    "Discord #shops",
                    "Saved locally. Scheduled sales reports will use this webhook.",
                    parent=self.root,
                )
            except ValueError:
                messagebox.showerror(
                    "Discord #shops",
                    "Enter a valid Discord channel webhook URL.",
                    parent=self.root,
                )
    ...
    def build_merchant(self, character):
        ...
        text = tk.StringVar(value="Connecting…")
        self.labels[character] = text
        status = ttk.Frame(frame)
        status.pack(fill="x", padx=12, pady=(8, 4))
        status_label = ttk.Label(
            status, textvariable=text, wraplength=950, anchor="nw", justify="left"
        )
        status_label.pack(fill="x")
        status.bind(
            "<Configure>",
            lambda event: status_label.configure(wraplength=max(200, event.width)),
        )
        ...
        primary = ttk.Button(
            controls,
            text="Pause merchant",
            command=lambda: self.toggle_merchant(character),
        )
        primary.grid(row=0, column=0, sticky="ew", padx=(0, 6), pady=2)
        self.merchant_buttons[character] = primary
        update = ttk.Button(
            controls, text="Update shop now", command=lambda: self.list_once(character)
        )
        update.grid(row=0, column=1, sticky="ew", padx=(0, 6), pady=2)
        self.batch_buttons[character] = update
        ttk.Button(
            controls,
            text="Show manual view",
            command=lambda c=character: self.request_manual_handoff_surface(c),
        ).grid(row=0, column=2, sticky="ew", padx=(0, 6), pady=2)
        ...
        menu.add_command(
            label="Full status details",
            command=lambda: self.show_merchant_details(character),
        )
        menu.add_command(
            label="Recovery status",
            command=lambda: self.show_recovery_status(character),
        )
        menu.add_command(
            label="Recheck recovery",
            command=lambda: self.recheck_merchant_recovery(character),
        )
        menu.add_command(
            label="Override recovery & resume",
            command=lambda: self.override_merchant_recovery(character),
        )
        menu.add_separator()
        menu.add_command(
            label="Toggle trading & repricing permission",
            command=lambda: self.toggle_manage(character),
        )
        manage_entry = menu.index("end")
        menu.add_command(
            label="Toggle automatic refill permission",
            command=lambda: self.toggle_refill(character),
        )
        ...
        menu.add_command(
            label="Copy attachment diagnostics",
            command=lambda: copy_diagnostics(self, character),
        )
        menu.add_command(
            label="Download prices only (no shop changes)",
            command=lambda: self.scan(character),
        )
        menu.add_separator()
        ...
        ttk.Button(
            controls, text="Stop all (including farmer)", command=self.global_stop
        ).grid(row=0, column=4, pady=2)
        ...
        ttk.Label(recovery, textvariable=recovery_text, width=55).pack(
            side="left", fill="x", expand=True
        )
        ttk.Button(
            recovery,
            text="Recheck",
            command=lambda c=character: self.recheck_merchant_recovery(c),
        ).pack(side="left", padx=(6, 0))
        ttk.Button(
            recovery,
            text="Override & resume",
            command=lambda c=character: self.override_merchant_recovery(c),
        ).pack(side="left", padx=(6, 0))
        help_text = ttk.Label(
            frame,
            text="Pause merchant stops both activities without closing the game. Settings keeps separate permissions.",
            wraplength=950,
        )
        ...
        for label, columns in (
            ...
            holder = ttk.Frame(tabs)
            tabs.add(holder, text=label)
            if label == "Waiting items":
                ttk.Label(
                    holder,
                    text="Items left unchanged until a safe price, shop space or safe input is available. "
                    "The reason for each item is below.",
                    wraplength=950,
            ...
            for col in columns:
                ...
                tree.column(
                    col, width=130 if col != "Details" and col != "Reason" else 440
                )
            tables[label] = tree
    ...
    def _merchant_recovery_incidents(self, character):
        ...
        try:
            ...
            for row in journal.pending(character):
                if row.get("kind") not in ("listing", "delivery"):
                    continue
                try:
                ...
                incidents.append(
                    {
                        ...
                        or ([before.get("item")] if before.get("item") else []),
                        "digest": journal.original_evidence_digest(row["id"]),
                        "note": "Listing confirmation needs reconciliation"
                        if row["kind"] == "listing"
        ...
        try:
            ...
            delivery_journal = (
                Journal(delivery_operation.JOURNAL)
                if Path(delivery_operation.JOURNAL).exists()
                else None
            )
            for row in delivery_journal.pending(character) if delivery_journal else []:
                if row.get("kind") != "farmer_delivery":
                    continue
                request_id = row.get("id")
                try:
                    receipt = (
                        delivery_operation.status(delivery_journal, request_id) or {}
                    )
                ...
                incidents.append(
                    {
                        "id": f"farmer-delivery:{request_id}",
                        "kind": "farmer-delivery",
                        "request_id": request_id,
                        "phase": receipt.get("phase"),
                        "state": receipt,
                        "items": receipt.get("items") or [],
                        "digest": digest,
                        "note": receipt.get("reason")
                        or "Merchant delivery needs reconciliation",
                    }
                )
        ...
        if state.get("phase") not in (None, "complete", "operator_overridden"):
            incidents.append(
                {
                    "id": f"shop-return:{character}",
                    "kind": "shop-return",
                    "phase": state.get("phase"),
                    "state": state,
                    "items": state.get("wanted")
                    or state.get("before", {}).get("inventory", []),
                    "digest": self._recovery_digest(state),
                    "note": state.get("note")
                    or state.get("reason")
                    or "Shop return needs attention",
                }
            )
        ...
        for key, kind, phases, note in (
            ...
            if kind == "connect-market":
                ...
                if not state or state.get("phase") == "operator_overridden":
                    continue
                if state.get("phase") in TERMINAL and not (
                    state.get("fare_pending") or state.get("launch_unresolved")
                ):
                    continue
            elif state.get("phase") not in phases:
                continue
            incidents.append(
                {
                    "id": f"{kind}:{character}",
                    ...
                    "phase": state.get("phase"),
                    "state": state,
                    "items": state.get("inventory_before") or [],
                    "digest": self._recovery_digest(state),
                    "note": state.get("note") or note,
                }
            )
        ...
        if (market_safety and market_safety.get("phase") != "operator_overridden") or (
            ...
            incidents.append(
                {
                    ...
                    or "Market safety hold requires review",
                }
            )
        elif safety.get("active") or (
            self.runtime.journal.get(character, "connect_hold", False)
            and safety.get("phase") not in ("operator_overridden", None)
        ):
            state = {
                "safety": safety,
                "attention": attention,
                "connect_hold": self.runtime.journal.get(
                    character, "connect_hold", False
                ),
            }
            incidents.append(
                {
                    "id": f"recovery-safety:{character}",
                    "kind": "recovery-safety",
                    "phase": safety.get("phase") or "connect_hold",
                    "state": state,
                    "items": [],
                    "digest": self._recovery_digest(safety),
                    "note": attention.get("note")
                    or "Protective recovery stop requires review",
                }
            )
        elif attention:
            ...
            incidents.append(
                {
                    "id": f"attention:{character}",
                    "kind": "attention",
                    "phase": attention.get("kind", "needs_attention"),
                    "state": attention,
                    "items": [],
    ...
    def _choose_merchant_recovery(self, character):
        ...
        dialog.title("Choose recovery incident")
        dialog.transient(self.root)
        ttk.Label(
            dialog,
            text="Select exactly one incident to inspect or override:",
            wraplength=600,
        ).pack(padx=12, pady=(12, 6))
        choices = tk.Listbox(
            dialog, width=100, height=min(10, len(incidents)), exportselection=False
        )
        choices.pack(padx=12, fill="both", expand=True)
        for incident in incidents:
            choices.insert(
                "end", f"{incident['id']} · {incident['phase']} · {incident['note']}"
            )
        selected = []
        ...
        def choose():
            ...
            if index:
                selected.append(incidents[index[0]])
                dialog.destroy()
    ...
    def _merchant_fresh_recheck(self, character, incident):
        if incident["kind"] == "farmer-delivery":
            ...
            try:
                result = self.dispatch(
                    {"action": "delivery-recheck", "request_id": incident["request_id"]}
                )
        ...
        if incident["kind"] in ("listing", "delivery"):
            if self.coordinator.owner:
                return dict(
                    incident, rechecked={"recheck_unavailable": "Merchant input active"}
                )
        ...
        if incident["kind"] == "connect-market":
            ...
            incident = dict(incident)
            try:
                snapshot = recheck(self.runtime, character)
            except (OSError, ValueError, KeyError, TypeError):
                snapshot = {"recheck_unavailable": "Merchant memory unavailable"}
            incident["rechecked"] = {"snapshot": snapshot}
            return incident
        if incident["kind"] == "stall-probe":
            ...
            if controller is not None:
                try:
                    snapshot = controller.driver.memory.read(recovery=True)
                except (OSError, ValueError, KeyError, TypeError):
                    pass
            incident = dict(incident)
            incident["rechecked"] = {"snapshot": snapshot}
        ...
        return dict(
            incident,
            rechecked={"snapshot": snapshot, "state": incident.get("state", {})},
        )
    ...
    def recheck_merchant_recovery(self, character):
        ...
        if not incident:
            ...
            if textvar is not None:
                textvar.set("No unresolved recovery holds")
            return None
        try:
            incident = self._merchant_fresh_recheck(character, incident)
        except (OSError, ValueError, KeyError, TypeError) as error:
            messagebox.showerror("Recovery recheck", str(error), parent=self.root)
            return None
    ...
    def _apply_merchant_override(self, character, incident, digest, *, operator=None):
        ...
        try:
            ...
            fresh = self._merchant_fresh_recheck(character, incident).get(
                "rechecked", {}
            )
            if kind in ("listing", "delivery"):
                ...
                with self.runtime.journal.db() as db:
                    row = db.execute(
                        "SELECT phase FROM transactions WHERE id=? AND character=?",
                        (key, character),
                    ).fetchone()
                if row and row["phase"] in ("verified", "aborted"):
                    return {"reconciled": True, "request_id": key}
                result = self.runtime.journal.operator_override(
                    key, fresh_evidence=fresh, **options
                )
                self.runtime.journal.set(character, "new_stock", True)
                return result
            if kind == "shop-return":
                return self.runtime.returns[character].operator_override(
                    fresh.get("snapshot", {}), **options
                )
            if kind == "recovery-safety":
                from conquest.merchants.recovery_safety import operator_override

                return operator_override(
                    self.runtime, character, fresh_evidence=fresh, **options
                )
            ...
            if kind == "stall-probe":
                ...
                return operator_override(
                    ...
                    dict(fresh.get("snapshot", {}), character=character),
                    character=character,
                    **options,
                )
            if kind == "attention":
                ...
                with journal.db() as db:
                    ...
                    row = db.execute(
                        "SELECT value FROM state WHERE character=? AND name='attention'",
                        (character,),
                    ).fetchone()
                    original = json.loads(row[0]) if row else {}
                    if self._recovery_digest(original) != digest:
                        raise ValueError(
                            "Incident evidence changed; recheck before overriding"
                        )
                    audit = {
                    ...
                    db.execute(
                        ...
                        (
                            character,
                            "attention_operator_overridden",
                            json.dumps(audit),
                            time.time(),
                        ),
                    )
                    db.execute(
                        "UPDATE state SET value='null' WHERE character=? AND name='attention'",
                        (character,),
                    )
                return audit
            raise ValueError("Unknown recovery incident")
    ...
    def override_merchant_recovery(self, character):
        ...
        permissions = (
            self.runtime.enabled(character),
            self.runtime.refill_enabled(character),
        )
        if not self._show_merchant_preview(incident, action="override"):
            return None
        try:
            result = self._apply_merchant_override(
                character, incident, incident["digest"]
            )
        except (OSError, ValueError, KeyError, TypeError) as error:
            messagebox.showerror("Recovery override", str(error), parent=self.root)
            return None
        self.recovery_texts[character].set(
            "Hold cleared; queued for a fresh stock check"
        )
        self.runtime.journal.set(character, "new_stock", True)
    ...
    def shop_help(self):
        messagebox.showinfo(
            ...
            "Stop all (including farmer)\nStops farming, merchant actions and auto-refill.",
            parent=self.root,
        )
    ...
    def show_recovery_status(self, character):
        ...
        messagebox.showinfo(
            character + " recovery",
            text.get() if text else "Recovery status is unavailable.",
            parent=self.root,
        )
    ...
    def show_merchant_details(self, character):
        ...
        messagebox.showinfo(
            ...
            merchant_text(
                ...
                now=time.time(),
                waiting_items=data["tables"][character]["deferred"],
    ...
    def toggle_manage(self, character):
        if self.runtime.enabled(character):
            self.pause(character)
        else:
            self.resume(character)
    ...
    def schedule_resize(self, character):
        ...
        self.resize_jobs[character] = self.root.after(
            150, lambda: self.finish_resize(character)
        )
    ...
    def on_tab_changed(self):
        ...
        if len(self.client_tabs) == len(CHARACTERS):
    ...
    def apply_client_compact_layout(self):
        ...
        if len(self.client_tabs) != len(CHARACTERS):
            return None
        character = client_tab_character(
            self.notebook, self.frames, self.detail_tabs, self.client_tabs
        )
    ...
    def auto_show_selected(self):
        ...
        character = next(
            (c for c in CHARACTERS if selected == str(self.frames[c])), None
        )
        if (
            not character
            or character in self.released_clients
            or not self.client_panes[character].winfo_ismapped()
        ...
        if (
            ...
            and host.saved
            and getattr(
                self.runtime.observers.get(character),
        ...
        if self.runtime.manual_handoff_status() is not None:
            try:
                self.show_manual_handoff_surface(character, selected=True)
            except (OSError, ValueError):
                self.calibration_results[character] = {
                    "verified": False,
    ...
    def show_manual_handoff_surface(self, character, *, selected=False):
        ...
        host = self.hosts.get(character)
        if getattr(
            self.runtime.observers.get(character), "merchant_observation_only", False
        ):
            from conquest.merchants.restore_hosts import (
                present_existing,
                present_user_view,
            )

            if selected:
                return present_existing(self, character)
            return present_user_view(self, character)
        handoff = self.runtime.manual_handoff_status()
        if handoff is None:
            raise ValueError("Start Manual handoff before using manual game view")
        if handoff.get("phase") not in ("ready", "ending"):
            raise ValueError("Wait for Manual handoff Ready before showing a game view")
        if self.closed or self.app.closing or probe_busy(self):
            raise ValueError("Manual game view is unavailable now")
        observer = self.runtime.observers.get(character)
        if observer is None:
            raise ValueError("Selected merchant has no verified attached process")
        observer.adapter.assert_identity()
        if not selected:
        ...
        hwnd = (
            observer.hwnd
            if getattr(observer, "merchant_observation_only", False)
            else observer.operations.target.hwnd
        )
        if not host.saved:
            if host.api.gui.IsIconic(hwnd):
                ...
                raise ValueError(
                    "Merchant is minimized; restore it yourself before manual view"
                )
        ...
        self.layout_status.setdefault(character, {}).update(
            manual_handoff_view=True, native_visible=True, selected=True
        )

    def request_manual_handoff_surface(self, character):
        try:
            self.show_manual_handoff_surface(character)
    ...
    def finish_resize(self, character):
        ...
        if getattr(
            ...
            if (
                ...
                and host.saved
                and observer
                and not pane.winfo_ismapped()
                and not self.coordinator.owner
                and self.coordinator.lock.acquire(blocking=False)
            ):
                try:
                    if (
        ...
        if getattr(getattr(self, "coordinator", None), "owner", None) or getattr(
            self.runtime, "delivery_window", None
        ):
            return
        try:
            ...
            if host and host.saved and not host.is_alive():
                ...
                self.layout_status[character] = {
                    "native_visible": False,
                    "selected": False,
                }
                self.calibration_results[character] = {
                    "verified": False,
                    "note": "Client closed; waiting for reconnect",
                }
                return
            ...
            if self.runtime.manual_handoff_status() is not None or getattr(
                self.runtime.observers.get(character),
                "merchant_observation_only",
                False,
            ):
            ...
            self.calibration_results[character] = {
                "verified": False,
                "note": "Client layout unavailable; re-embed before continuing",
            }
    ...
    def resize_merchant(self, character, *, automatic=False):
        ...
        if registry() and pane.winfo_ismapped() and min(size) > 1:
            ...
            try:
                ...
                if character in manual_blocks:
                    self.coordinator.surface_blocks[character] = manual_blocks.pop(
                        character
                    )
            except ViewportTooSmall:
                if self.runtime.manual_handoff_status() is not None:
                    if character not in manual_blocks:
                        manual_blocks[character] = bool(
                            self.coordinator.surface_blocks.get(character)
                        )
                ...
                self.calibration_results[character] = {
                    "verified": False,
                    "note": "Full viewport does not fit. Use More / help → Open game in separate window.",
                }
                raise
        ...
        if pane.winfo_ismapped() and min(size) > 1 and previous != size:
            ...
            if not automatic and (
                getattr(getattr(self, "coordinator", None), "owner", None)
                or getattr(self.runtime, "delivery_window", None)
            ):
                return
    ...
    def embed_client(self, character, *, queued_at=None, automatic=False):
        ...
        try:
            ...
            self.layout_status.setdefault(character, {}).update(
                ...
                queue_ms=round((started - queued_at) * 1000)
    ...
    def embed_merchant(self, character, *, automatic=False):
        if probe_busy(self):
            raise ValueError(
                "Background diagnostic owns the client; wait for restoration"
            )
        observer = self.runtime.observers.get(character)
        if observer and getattr(observer, "merchant_observation_only", False):
            ...
            if not restore_readonly(self, character):
                raise ValueError(
                    "Waiting for a fresh safe farmer handoff before showing this merchant"
                )
            return observer
        if (
            not self.safe_to_yield()
            or self.runtime.enabled(character)
            and not automatic
            or self.calibrating
            and character not in self.calibrating
        ):
            raise ValueError(
                "Pause the merchant and wait for a safe farmer handoff before embedding"
            )
        observer = self.runtime.observers.get(character)
        if not observer:
        ...
        if getattr(observer, "merchant_observation_only", False):
            ...
            if not _readonly_market_hosting_safe(self):
                raise ValueError(
                    "Park the farmer alive in Market with Farming Off, no active input "
        ...
        if status:
            status.enter(
                "attachment", pane_size=[pane.winfo_width(), pane.winfo_height()]
            )
        if registry():
        ...
        if status:
            ...
            status.enter("memory" if read_only else "behavior")
            if read_only:
    ...
    def prepare_native_host_input(self, character):
        ...
        expected = (
            ...
            observer,
            host.saved.hwnd,
            dict(host.saved.identity),
            self.coordinator.purpose,
        )
        done, result = threading.Event(), {}
        ...
        if result.get("error"):
            raise ValueError(result["error"])
        wait_for_merchant_surface(
            host,
            [h for h in self.hosts.values() if h is not host],
            self.coordinator.check,
        )
    ...
    def prepare_input(self, character):
        ...
        if self.coordinator.purpose == "connect":
            ...
            if observer and login_screen(observer.operations.target.hwnd):
                ...
                if not activate_client(
                    observer.operations.target.hwnd, observer.adapter.identity
                ):
                    ...
                    if farmer and self.safe_to_yield():
                        ...
                        HostApi().activate_owned_caption(
                            farmer.operations.target.hwnd, farmer.adapter.identity
                        )
                    if not activate_client(
                        observer.operations.target.hwnd, observer.adapter.identity
                    ):
                        raise ValueError(
                            "Activate the selected login client before continuing"
                        )
                return
        done, result = threading.Event(), {}
    ...
    def prepare_delivery_accept_surface(self, character):
        ...
        observer, target, host, identity, profile, profile_id = (
            self.delivery_accept_binding(character)
        )
    ...
    def delivery_accept_binding(self, character, expected=None):
        ...
        if (
            ...
            or state.get("phase") != "request_verified"
            or state.get("character") != character
        ):
            raise ValueError(
                "Trade acceptance probe is no longer request-verified; no client action sent"
            )
        intent = state.get("intent")
        merchant = intent.get("merchant") if isinstance(intent, dict) else None
        if (
            not isinstance(merchant, dict)
            or merchant.get("character") != character
            or merchant.get("server") != "America"
            or not isinstance(merchant.get("identity"), dict)
        ):
            raise ValueError(
                "Trade acceptance merchant binding is unreadable; no client action sent"
            )
        ...
        if not isinstance(profile_id, str) or not profile_id:
            raise ValueError(
                "Trade acceptance merchant profile binding is unreadable; no client action sent"
            )
        if profiles:
            try:
                resolved = profiles.resolve(
                    profile_id, role="Merchant", server=merchant["server"]
                )
            except ValueError as error:
                raise ValueError(
                    "Trade acceptance merchant profile is unavailable; no client action sent"
                ) from error
            ...
            if (
                ...
                or resolved.name != character
                or resolved.name != merchant["character"]
                or resolved.server != merchant["server"]
            ):
                raise ValueError(
                    "Trade acceptance merchant profile changed; no client action sent"
                )
        ...
        if (
            type(receipt_identity.get("pid")) is not int
            or receipt_identity["pid"] <= 0
            ...
            or not receipt_identity["path"]
        ):
            raise ValueError(
                "Trade acceptance merchant identity is incomplete; no client action sent"
            )
        observer = self.runtime.observers.get(profile)
        if not observer or observer.adapter.identity != receipt_identity:
            raise ValueError(
                "Waiting for the exact merchant process before trade acceptance"
            )
        observer.adapter.assert_identity()
        target = observer.operations.target
        if type(getattr(target, "hwnd", None)) is not int or target.hwnd <= 0:
            raise ValueError(
                "Trade acceptance merchant window is unavailable; no client action sent"
            )
        if (
            expected is not None
            and (target.hwnd, receipt_identity, profile_id) != expected
        ):
            raise ValueError(
                "Trade acceptance merchant mapping changed; no client action sent"
            )
        ...
        if host and host.saved:
            ...
            host.api.assert_owner(host.saved.hwnd, host.saved.identity)
            if (
                host.saved.hwnd != target.hwnd
                or host.saved.identity != receipt_identity
            ):
                raise ValueError(
                    "Selected merchant host changed; re-embed was not attempted"
                )
            if host.mode != "owned":
                raise ValueError(
                    "Selected merchant is not an owned host; no client action sent"
                )
    ...
    def embed_delivery_accept_merchant(self, character, expected):
        ...
        if probe_busy(self):
            raise ValueError(
                "Background diagnostic owns the client; no client action sent"
            )
        observer, target, host, identity, profile, profile_id = (
            self.delivery_accept_binding(character, expected)
        )
        ...
        observer, target, current, identity, profile, profile_id = (
            self.delivery_accept_binding(character, expected)
        )
    ...
    def show_delivery_accept_merchant(self, character, expected):
        ...
        if not host or not host.saved:
            raise ValueError(
                "Selected merchant host is unavailable; no client action sent"
            )
        foreground = host.api.gui.GetForegroundWindow()
        bookmark = {"tab": self.notebook.select(), "hwnd": foreground, "identity": None}
        if foreground:
    ...
    def restore_input(self, character):
        ...
        if (
            ...
            or self.closed
            or self.coordinator.owner
            or self.app.mouse_priority.active()
        ...
        if bookmark["tab"] in self.notebook.tabs():
            ...
            self.root.update_idletasks()
            self.resize_merchant(character)
        if bookmark["identity"] and host.api.owns_window(
            bookmark["hwnd"], bookmark["identity"]
        ):
    ...
    def poll_ui_requests(self):
        ...
        self.ui_health = {
            "last_gap_ms": gap,
            "max_gap_ms": max(gap, self.ui_health["max_gap_ms"]),
        }
    ...
    def focus_clicked_merchant(self):
        ...
        for character, host in self.hosts.items():
            ...
            if host.api.activate_owned_click(host.saved, host.parent):
                self.layout_status.setdefault(character, {})["click_focus_ms"] = round(
    ...
    def start_qualification(self, character):
        ...
        if self.calibrating:
            self.calibration_results[character] = {
                "verified": False,
                "note": "Another booth verification is active",
            }
            return
        try:
            ...
            self.calibration_results[character] = {
                "verified": False,
                "note": "Waiting for mouse idle, then verifying booth controls",
            }
            self.coordinator.resume()
            ...
            def work():
                try:
                    ...
                    wait_for_calibration_idle(
                        self.coordinator, cancel, lambda: self.closed
                    )
                    with (
                        self.coordinator.lease(character),
                        observer.lock,
                        physical_coordinates(),
                    ):
                        if self.runtime.journal.pending(character):
                            raise ValueError(
                                "Reconcile pending transactions before panel verification"
                            )
                        from conquest.merchants.inventory_panel import (
                            verify_inventory_panel,
                        )

                        driver = self.runtime.controllers[character].driver
                        if any(
                            w["name"] == "Add Item to Booth"
                            for w in driver.read()["windows"]
                        ):
                        ...
                        result = verify_booth_controls(
                            driver, self.runtime.journal, check
                        )
                    self.calibration_results[character] = result
                except Exception as error:
                    failure = calibration_failure(error)
                    self.calibration_results[character] = failure
                    if not failure["note"].startswith(
                        (
                            "Mouse remained active",
                            "Automation stopped or manual input active",
                            "Booth verification stopped",
                        )
                    ):
                        self.runtime.journal.set(
                            character,
                            "attention",
                            {"kind": "calibration", "note": failure["note"]},
                        )
                    self.runtime.journal.event(
                        character, "booth_calibration_failed", **failure
                    )
                finally:
                    self.calibrating.discard(character)

            threading.Thread(
                target=work, daemon=True, name=f"qualify-{character}"
            ).start()
        except (ValueError, OSError) as error:
            self.calibration_results[character] = {
                "verified": False,
    ...
    def release_merchant(self, character):
        ...
        if self.coordinator.owner == character or character in self.calibrating:
            self.calibration_results[character] = {
                "verified": False,
                "note": "Waiting for input release; press Release client again",
            }
            return
        host = self.hosts.get(character)
        from conquest.merchants.restore_hosts import released
        ...
        try:
            for c in [character] if character else CHARACTERS:
                ...
                if existing.get("pending"):
                    if not self.runtime.enabled(c):
                        self.dispatch({"action": "resume-batch", "character": c})
                else:
                    self.dispatch(
                        {
                            "action": "list-once",
                            "request_id": f"once:{uuid.uuid4().hex}",
                            "character": c,
                        }
                    )
        except (ValueError, OSError) as error:
    ...
    def credentials(self, character):
        ...
        dialog = tk.Toplevel(self.root)
        dialog.title(f"{character} — local encrypted login")
        dialog.transient(self.root)
        ttk.Label(
            dialog, text="Stored with Windows encryption for this character only."
        ...
        for label, value in (("Username", username), ("Password", password)):
            ...
            ttk.Entry(
                dialog, textvariable=value, show="*" if label == "Password" else ""
        ...
        def save():
            try:
                ...
                messagebox.showerror(
                    "Login",
                    "Could not save credentials. Enter both fields and check local file access.",
                    parent=dialog,
                )
                return
            username.set("")
    ...
    def update_header(self, statuses):
        ...
        now = time.time()
        data = self.presentation.latest
        self.header_status = header_text(
            data["sales"], data["reporting"], statuses, now=now
        )
        ...
        if hasattr(self, "discord_note"):
            self.discord_note.set(
                data.get(
                    "notification_health", "Checking Discord notification services..."
                )
            )
    ...
    def poll(self):
        ...
        try:
            ...
            self.rows["Farmer"].set(
                f"{self.app.state_text.get()} · {self.app.activity_text.get()}\n{self.app.stats_text.get()}"
            )
            self.input_note.set(
                f"Input owner: {self.coordinator.owner or 'none'}. "
                + (
                    "Farmer handoff available."
                    if self.safe_to_yield()
                    else "Waiting for the farmer to stop or explicitly grant a safe handoff."
                )
            )
            events = data["events"]
            for character, state in statuses.items():
                ...
                if (
                    character in self.background_surfaces
                    and self.background_probe.get("state") == "restoration_required"
                ):
                    note += (
                        "\nClient restoration needs attention: "
                        + str(
                            self.background_probe.get("restoration_error")
                            or self.background_probe.get("error")
                            or "native window restore failed"
                        )
                        + ". Embed client retries restoration."
                    )
                active_batch = state["scan"].get("pending")
                self.batch_buttons[character].configure(
                    text="Updating shop…"
                    if active_batch and state["enabled"]
                    else "Resume shop update"
                    if active_batch
                    else "Update shop now",
                    state="disabled" if active_batch and state["enabled"] else "normal",
                )
                self.merchant_buttons[character].configure(
                    text="Pause merchant"
                    if state["enabled"] or state["refill"]["enabled"]
                    else "Resume merchant"
                )
                ...
                if entries and character in getattr(self, "permission_menus", {}):
                    refresh_permission_menu(
                        self.permission_menus[character], entries, state
                    )
                from conquest.merchants.simple_controls import summary

                note = summary(
                    state, now=time.time(), global_stopped=self.coordinator.stopped
                )
                recovery_reader = getattr(self, "_merchant_recovery_incidents", None)
                holds = (
                    recovery_reader(character)
                ...
                if character in getattr(self, "recovery_texts", {}):
                    self.recovery_texts[character].set(
                        ...
                        if holds
                        else "No unresolved recovery holds"
                    )
                host = self.hosts.get(character)
                if host and host.saved:
                    ...
                    self.fill(
                        ...
                        [
                            (
                                ...
                                i["plus"],
                                f"{i['gem1']}/{i['gem2']}",
                                i["price"] if key == "booth" else i["quantity"],
                            )
                            for i in snapshot.get(key, [])
                        ],
                    )
                for label, key in (
                    ("Comparisons", "comparisons"),
                    ("Waiting items", "deferred"),
                ):
                    if selected_detail != label:
                        continue
                    self.fill(
                        ...
                        [
                            ...
                            for p in data["tables"][character][key]
                        ],
                    )
                own = [e for e in events if e["character"] == character]
                for label, kind in (
                    ("Trades", "delivery_verified"),
                    ("Price history", "listing_verified"),
                    ("Sales", "sale_verified"),
                    ("Logs", None),
                ):
                    if selected_detail != label:
                    ...
                    self.fill(
                        ...
                        [
                            (
                                time.strftime(
                                    "%H:%M:%S", time.localtime(e["timestamp"])
                                ),
                                e["event"],
                                e["payload"],
                            )
                            ...
                            or e["event"] == kind
                            or label == "Sales"
                            and e["event"] == "sales_history_reconciled"
                        ],
                    )
        finally:
            if not self.closed:
    ...
    def defer_background_action(self, callback):
        ...
        if self.background_after_job is None:
            self.background_after_job = self.root.after(
                100, self.finish_background_action
            )
    ...
    def close(self, *, reason="closed"):
        if probe_busy(self):
            self.defer_background_action(
                self.app.restart if reason == "restarting" else self.app.close
            )
            self.app.state_text.set(
                "Waiting for background client restoration; Conquest remains open"
            )
            return False
        if not self.closed:
            self.closed = True
            ...
            write_json(
                state_path(".runtime/merchants/app-lifecycle.json"),
                {"state": reason, "at": time.time()},
            )
            write_json(
                state_path(".runtime/merchants/window.json"),
                {"geometry": self.root.geometry(), "state": self.root.state()},
            )
