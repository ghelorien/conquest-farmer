...
class MarketRefreshWorker:
    ...
    def request(self, character, request_id):
        ...
        with self.lock:
            ...
            if old.get('pending') or old.get('request_id')==request_id:return old
            state={'request_id':request_id,'pending':True,'phase':'queued','requested_at':self.clock(),'retry_at':0}
            self.journal.set(character,'market_refresh',state)
            return state

    def snapshot(self):
        try:return MarketSnapshot(json.loads(self.path.read_text(encoding='utf-8')),now=self.clock())
    ...
    def step(self):
        ...
        for character in CHARACTERS:
            ...
            if scan.get('pending') and market is None:
                self.request(character,'scan:'+scan['request_id']+':'+str(int(self.clock()//120)))
        with self.lock:
            waiting={c:self.state(c) for c in CHARACTERS if self.state(c).get('pending')}
            if not waiting or any(v.get('retry_at',0)>self.clock() for v in waiting.values()):return
            for c,state in waiting.items():
                state={**state,'phase':'fetching','started_at':self.clock(),'error':None}
                waiting[c]=state;self.journal.set(c,'market_refresh',state)
        try:
            ...
            import traceback
            diagnostic={'type':type(error).__name__,'frames':[{'file':Path(f.filename).name,'function':f.name,'line':f.lineno}
                for f in traceback.extract_tb(error.__traceback__)]}
            note=str(error) if isinstance(error,ValueError) else 'Market download failed.'
            with self.lock:
                for c,state in waiting.items():
                    self.journal.set(c,'market_refresh',{**state,'phase':'failed',
                        'error':note+' Retrying in 60 seconds; prices unchanged.',
                        'retry_at':self.clock()+60,'diagnostic':diagnostic})
                    self.journal.event(c,'market_refresh_failed',note='Market download failed; retry scheduled')
            return
        with self.lock:
            for c,state in waiting.items():
                self.journal.set(c,'market_refresh',{**state,'pending':False,'phase':'ready',
                    'completed_at':self.clock(),'observed_at':data['observed_at'],'items':data['total'],'retry_at':0})
