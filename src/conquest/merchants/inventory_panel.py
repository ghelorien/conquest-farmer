...
def verify_inventory_panel(driver,check):
    ...
    def unchanged():
        check();fresh=driver.read()
        if (stock(fresh)!=stock(before) or not fresh['booth_open']
    ...
    for expected in transitions:
        state=unchanged();point=inventory_button(gui)
        def guard():
            current=unchanged()
        ...
        while visible(unchanged())!=expected:
            if time.monotonic()>=deadline:
                raise ValueError('Inventory toggle result was not verified; no repeat input')
            time.sleep(.05)
    evidence={'verified_at':time.time(),'identity':before['identity'],
              'closed_to_open_verified':True,'stock_unchanged':True,
              'own_booth_uid':before['own_booth_uid'],'source':'pinned Items table control'}
