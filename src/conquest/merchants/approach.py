...
def positions(terrain,probe,*,used=(),deadline=None):
    ...
    for dx in range(-DELIVERY_PROBE_MAX_AXIS_DELTA, DELIVERY_PROBE_MAX_AXIS_DELTA+1):
        for dy in range(-DELIVERY_PROBE_MAX_AXIS_DELTA, DELIVERY_PROBE_MAX_AXIS_DELTA+1):
            ...
            if p in candidates_used or not terrain.walkable(p):continue
    ...
    for _,p in sorted(candidates)[:24]:
        ...
        try:path=terrain.travel_path(source,p,avoid=occupied)
...
def bounded_position(terrain,probe,destination,*,used=(),deadline=None):
    ...
    if deadline is not None and time.time()>=deadline:return None
    avoid=({tuple(probe['merchant_position'])}|set(map(tuple,probe.get('occupied_tiles',[]))))-{source}
    try:path=terrain.travel_path(source,destination,avoid=avoid)
    except ValueError:return None
