...
class DeliveryPeer:
    ...
    def command(self,action,key,intent,phase):
        body={'action':action,'character':intent['merchant']['character'],'request_id':key}
        uids=[item['uid'] for item in intent['items']]
        if action=='delivery-reserve':body['uids']=uids
        result=self.send(body)
        if (result.get('request_id')!=key or result.get('phase')!=phase
