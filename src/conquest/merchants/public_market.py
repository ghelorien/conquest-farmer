...
def socket_label(value):
    ...
    if not isinstance(value, str) or not re.fullmatch(
        r"(Normal|Refined|Super)(Phoenix|Dragon|Fury|Rainbow|Kylin|Violet|Moon|Tortoise)Gem",
        value,
    ):
...
def normalize_pages(pages, definitions, *, observed_at):
    if not pages:
        raise ValueError("Empty public market response")
    first = pages[0]
    ...
    if (
        ...
        or len(versions) != 1
        or versions[0].get("server") != 0
        or versions[0].get("isAvailable") is not True
        or not versions[0].get("updatedAtUtc")
    ):
        raise ValueError("America market is unavailable")
    if len(pages) != count:
        raise ValueError("Public market pages missing")
    rows = []
    seen = set()
    for number, page in enumerate(pages, 1):
        if (
            ...
            or page.get("totalCount") != total
            or page.get("totalPages") != count
            or page.get("servers") != versions
        ):
            raise ValueError("Public market changed during collection")
        items = page.get("items")
        if not isinstance(items, list) or len(items) != min(
            100, total - (number - 1) * 100
        ):
        ...
        for item in items:
            ...
            if (
                type(item.get("price")) is not int
                or item["price"] <= 0
                or type(item.get("additionLevel")) is not int
            ):
                raise ValueError("Invalid public market price or plus")
            rows.append(
                [
                    item["attributeName"] + "\n" + item["itemMinorClass"],
                    item.get("qualityName") or "—",
                    str(item["additionLevel"]),
                    socket_label(item.get("gem1"))
                    + "\n"
    ...
    data = browser_pages(
        dict(
            ...
            initial_total=total,
            final_total=total,
            initial_change=versions,
            final_change=versions,
            last_page=True,
            pages=[
                {"page": i // 50 + 1, "rows": rows[i : i + 50]}
...
def collect_public(request, definitions, *, check=lambda: None):
    ...
    for number in range(2, count + 1):
        ...
        if current.get("servers") != first.get("servers") or current.get(
            "totalCount"
