...
class RefillController(MerchantController):
    ...
    def active(self):
        return (
            self.journal.get(self.character, "refill_enabled", True)
            and not self.coordinator.stopped
        )

    def check(self):
        if not self.active():
            raise CaptureUnavailable("Inventory refill is paused")
...
class RefillSchedule:
    ...
    def state(self):
        ...
        if state is None:
            ...
            if base is None:
                base = state.get("next_check", self.clock() + 300) - 300
    ...
    def start(
        ...
        *,
        ...
        town_visit_id=None,
        operation_id=None,
        source_delivery_operation_id=None,
    ):
        state = self.state()
        now = self.clock()
        if not state.get("pending"):
            state.update(
                original_due_at=state["next_check"],
                cursor=[],
                listed=0,
                deferred=0,
        ...
        if source_delivery_operation_id is not None:
            if state.get("source_delivery_operation_id") not in (
                None,
                source_delivery_operation_id,
            ):
        ...
        if town_visit_id is not None:
            state["town_visit_id"] = town_visit_id
    ...
    def pause_budget(self):
        ...
        self.journal.event(
            ...
            "refill_deferred",
            status="paused_budget",
            listed=state.get("listed", 0),
            remaining=len(state.get("cursor", [])),
            visit_id=state.get("visit_id"),
            town_visit_id=state.get("town_visit_id"),
        )
