...
def owned_booth_target(observer, booth):
    ...
    if (
        read(graphics, "<Q")[0] != base + 0x5D49D0
    ...
    if (
        not 300 <= width <= 16384
        or not 300 <= height <= 16384
        or not 0.25 <= scale <= 4
        or not 0 <= x < map_size[0]
        or not 0 <= y < map_size[1]
    ):
        raise ValueError("Booth camera dimensions or position are invalid")
    unscaled = ((x - y) * 32 + origin_x - scroll_x, (x + y) * 16 + origin_y - scroll_y)
    point = tuple(
        round((v - dim // 2) * scale + dim // 2)
    ...
    px, py = (
        math.trunc((v - dim // 2) / scale) + dim // 2
        for v, dim in zip(point, (width, height))
    )
    px += scroll_x - origin_x
    py += scroll_y - origin_y
    tile = (math.floor(py / 32 + px / 64 + 0.5), math.floor(py / 32 - px / 64 + 0.5))
