...
class FarmerJournal:
    ...
    def get(self,character,name,default=None):return self.runtime._manual_get('Farmer',name,default)
    def set(self,character,name,value):self.runtime._manual_set('Farmer',name,value)
    def pending(self,character):return [{'kind':'farmer_delivery'}] if self.runtime.farmer_bot_owned() else []
...
def controller(runtime,observer):
    ...
    def active():
        ...
        return bool(intent.get('enabled') and not intent.get('paused') and not runtime.coordinator.stopped)
    def check():
        if not active():raise CaptureUnavailable('Farmer is paused')
        import ctypes
    ...
    return SimpleNamespace(character='Farmer',manual_farmer=True,journal=FarmerJournal(runtime),
                           driver=driver,coordinator=runtime.coordinator,active=active,check=check)


def observe(runtime,observer=None):
    configured=runtime.manual_farmer_provider()
    if observer is not None and observer is not configured:return False
    observer=configured
    if observer is None:
        runtime.manual_farmer_observation={'available':False,'reason':'Farmer has no attached memory observer'}
        return runtime.manual_unavailable('Farmer','Farmer has no attached memory observer')
    ...
    if target is not None and getattr(target,'hwnd',None) is not None:
        ...
        if login_screen(target.hwnd):
            runtime.manual_farmer_observation={'available':False,'reason':'Farmer is disconnected; trade memory is unavailable'}
            return runtime.manual_unavailable('Farmer','Farmer disconnected during manual session')
    if runtime.farmer_bot_owned():
        runtime.manual_farmer_observation={'available':True,'bot_owned':True,
    ...
    try:
        ...
        context=current()
        if registry() is not None and (context is None or context.profile.role!='Farmer'):
    ...
    intent=runtime.manual_farmer_control()
    try:
        runtime.process_manual('Farmer',snapshot,decline_enabled=bool(
            runtime.manual_farmer_controller and intent.get('enabled') and not intent.get('paused')
