...
from conquest.merchants.booth_listing_once_1078 import _profile, _row, _farmer_journals_clear

KIND = 'owned_booth_panel_1078'
PENDING = 'owned_booth_panel_1078_pending'
CAPABILITY = 'owned_booth_panel_1078_capability'
ASSETS = ('identity','character','character_uid','server','map_id','position',
          'silver','capacity','inventory','booth','own_booth_uid')
WORKERS = {}
LOCK = threading.RLock()


def _closed(snapshot, request, profile):
    if (any(type(request.get(key)) is not int or request[key] <= 0
            ...
            or not snapshot['own_booth_uid'] or snapshot['booth_open']
        ...
        raise ValueError('Owned panel request identity, ownership or closed state changed')


def _unchanged(snapshot, baseline):
    if any(snapshot[key] != baseline[key] for key in ASSETS):
        raise ValueError('Owned panel assets or actor changed; read-only reconciliation required')
    if (snapshot['hp'] <= 0 or snapshot.get('trade') is not None
            or snapshot.get('request') is not None or snapshot.get('closed_modal') is False
            or not 0 <= time.time()-snapshot['timestamp'] <= 1):
...
def _markers(journal, key):
    with journal.db() as db:
        return db.execute("SELECT payload FROM transaction_steps WHERE transaction_id=? "
                          "AND stage='panel_click' AND status='before_action'",(key,)).fetchall()
...
def status(journal, character, key):
    ...
    return {'request_id':key,'phase':row['phase'],
...
def require_receipt(runtime, character, snapshot):
    ...
    if not row or row['kind'] != KIND or row['character'] != character or row['phase'] != 'verified':
...
def prepare(ui, body):
    fields = {'action','character','request_id','expected_identity',
              'expected_character_uid','expected_own_booth_uid'}
    if set(body) != fields or not re.fullmatch(r'booth-open1078-[A-Za-z0-9_-]{8,80}',str(body['request_id'])):
    ...
    if getattr(ui,'grant',None):
        raise ValueError('Release the existing handoff before a new owned panel request')
    _idle(ui, character)
    ...
    profile = _profile(character)
    _closed(first,body,profile);_closed(second,body,profile);_unchanged(second,first)
    ...
    before = {'request':dict(body),'profile_id':profile.id,'snapshot':second,
              'target_candidate':first['booth_target_preflight'],'hwnd':observer.hwnd,
    ...
    return {**status(journal,character,key),'handoff_request_id':handoff,
            'scope':'listing_1078','mode':'open_panel'}


def _idle(ui, character, *, key=None):
    r,c = ui.runtime,ui.coordinator
    if (ui.closed or ui.app.closing or r.stop_event.is_set() or c.stopped
            or not r.refill_enabled(character) or c.manual_active()
            or r.manual_handoff_status() is not None
            or c.manual_session_blocked(character) or c.manual_session_blocked('Farmer')
            or r.connecting or r.refilling or ui.calibrating
            or getattr(r,'delivery_window',None)):
        raise CaptureUnavailable('Owned panel opening stopped or held by manual/merchant work')
    ...
    with r.journal.db() as db:
        rows = db.execute("SELECT id,kind,phase FROM transactions WHERE phase NOT IN "
                          "('verified','aborted','operator_overridden')").fetchall()
    if any(row['id'] != key or row['kind'] != KIND or row['phase'] != 'prepared' for row in rows):
...
def validate_grant(ui, character):
    ...
    if not row or row['phase'] != 'prepared':raise ValueError('No unsent owned panel request')
    if _markers(ui.runtime.journal,row['id']):
        raise ValueError('Owned panel click was already marked; reconcile without input')
    _idle(ui,character,key=row['id'])
    before = json.loads(row['before_json'])
    from conquest.merchants.observe_1078 import observe
    snapshot = observe(ui.runtime,character,booth_target_preflight=True)
    _closed(snapshot,before['request'],_profile(character));_unchanged(snapshot,before['snapshot'])
    return {'mode':'open_panel','profile_id':before['profile_id'],'panel_request_id':row['id']}


def _verify_open(snapshot, baseline, *, fresh=True):
    if fresh:_unchanged(snapshot,baseline)
    elif any(snapshot[key] != baseline[key] for key in ASSETS):raise ValueError('Panel receipt assets changed')
    preflight = snapshot.get('listing_preflight') or {}
    owned = preflight.get('owned_booth') or {}
    if (snapshot['booth_open'] is not True or snapshot.get('closed_modal') is not True
            or preflight.get('layout_observed') is not True
            or not preflight.get('inventory_grid') or not preflight.get('booth_grid')
            or owned.get('model_key') != 25 or owned.get('model_owner_verified') is not True
...
def reconcile(ui, character, key):
    ...
    row = _row(journal,key)
    if not row or row['character'] != character or row['kind'] != KIND:raise ValueError('Panel request changed')
    if row['phase'] in ('verified','aborted','operator_overridden'):return status(journal,character,key)
    with journal.db() as db:
        markers = db.execute("SELECT payload FROM transaction_steps WHERE transaction_id=? "
                             "AND stage='panel_click' AND status='before_action'",(key,)).fetchall()
    if not markers:return status(journal,character,key)
    if len(markers) != 1:raise ValueError('Ambiguous owned panel click marker')
    before = json.loads(row['before_json'])
    from conquest.merchants.observe_1078 import observe
    ...
    if second['timestamp'] <= first['timestamp']:raise ValueError('Panel verification needs distinct fresh samples')
    from conquest.merchants.listing_capability_1078 import _digest
    evidence = {'input':json.loads(markers[0][0]),'observations':[first,second]}
    journal.transition(key,'verified',{'closed_to_open_verified':True,'assets_unchanged':True,'evidence':evidence})
    journal.set(character,CAPABILITY,{'request_id':key,'digest':_digest(evidence)})
    attention = journal.get(character,'attention') or {}
    if attention.get('transaction_id') == key:journal.set(character,'attention',None)
    return status(journal,character,key)
...
def step(ui, character, snapshot):
    ...
    with LOCK:
        ...
        if (grant.get('scope') != 'listing_1078' or grant.get('character') != character
                or authority.get('mode') != 'open_panel' or authority.get('panel_request_id') != key):
            from conquest.merchants.listing_handoff_1078 import request_handoff
            return {'state':'waiting_panel_handoff','request_id':key,
                    'handoff_request_id':request_handoff(ui.runtime,character)}
        if not ui.runtime.can_start_work(12):raise CaptureUnavailable('Owned panel opening needs twelve grant seconds')
        before = json.loads(row['before_json'])
        token = ui.coordinator.fence.capture()
        worker = threading.Thread(target=_run,args=(ui,character,before,dict(grant),token),daemon=True,name=KIND)
        WORKERS[key]=worker;worker.start()
...
def _run(ui, character, before, grant, token):
    ...
    try:
        ...
        with MemorySession(target.pid,CLIENT_SHA256_1078) as session, physical_coordinates():
            if session.identity != before['snapshot']['identity']:raise ValueError('Merchant process changed')
            gui=GuiReader.for_session(session);reader=open_read_only_1078(session,character)
            base=qualify(session)
            def policy():
                _idle(ui,character,key=key)
                if (time.monotonic() >= deadline or ui.grant != grant
                        or ui.app.control.snapshot() != control or not ui.safe_to_yield()
                        or not panel_scope_allows(ui,character,key)):
                    raise CaptureUnavailable('Exact owned panel handoff or control revision changed')
                farmer_safe(ui,grant['listing_authority']['farmer_target'],deadline=deadline)
                session.assert_identity()
                state=target.snapshot()
                if state['root_hwnd'] != target.hwnd or focus and state['foreground'] != target.hwnd:
                    raise CaptureUnavailable('Owned panel native foreground changed')
            coordinator=ui.coordinator
            with coordinator.fence.bind_worker(token), coordinator.owned_panel_scope(character,policy):
                with coordinator.lease(character,purpose=KIND):
                    if not activate_client(target.hwnd,session.identity):raise CaptureUnavailable('Native focus unavailable')
                    focus=True;policy()
                    layout=SharedLayoutRevision(target,windows=gui.windows,gui_size=gui.viewport_size)
                    revision=layout.stable()
                    def fresh():
                        policy();layout.assert_current(revision)
                        snap=reader.read_manual_ownership()
                        _closed(snap,before['request'],_profile(character));_unchanged(snap,before['snapshot'])
                        candidate=collect(session,snap)
                        point=tuple(candidate['projection_candidate']['candidate_point'])
                    ...
                    def guard():
                        ...
                        if current != candidate or actual != point:raise ValueError('Owned booth target moved')
                    ...
                    foreground_click(target,*point,expected_size=revision.client_size,
                        expected_origin=revision.client_origin,require_foreground=True,
                        before_press=hover,before_mouse_down=mark,
                        layout_guard=lambda:(policy(),layout.assert_current(revision)))
                    journal.transition(key,'submitted',{'click_attempted':True})
            until=time.monotonic()+3
            while True:
                try:
                    reconcile(ui,character,key);break
                except ValueError:
                    if time.monotonic()>=until:raise
                    time.sleep(.05)
        ...
        if row and row['phase'] not in ('verified','aborted','operator_overridden'):
            if row['phase'] != 'uncertain':
                journal.transition(key,'uncertain' if attempted else 'aborted',
                                   {'reason':str(error),'click_attempted':attempted})
            journal.step(key,'panel_failed','error',{'reason':str(error),'click_attempted':attempted})
            journal.set(character,'attention',{'kind':KIND,'transaction_id':key,'reason':str(error)})


def panel_scope_allows(ui, character, key):
    grant=getattr(ui,'grant',None) or {};authority=grant.get('listing_authority') or {}
    return (grant.get('scope')=='listing_1078' and grant.get('character')==character
            and authority.get('mode')=='open_panel' and authority.get('panel_request_id')==key
            and authority.get('profile_id')==_profile(character).id)


def dispatch(ui, body):
    if body['action']=='merchant-owned-panel-prepare-1078':return prepare(ui,body)
    if set(body)!={'action','character','request_id'}:raise ValueError('Exact panel request ID required')
    if body['action']=='merchant-owned-panel-reconcile-1078':return reconcile(ui,body['character'],body['request_id'])
    if body['action']=='merchant-owned-panel-status-1078':return status(ui.runtime.journal,body['character'],body['request_id'])
