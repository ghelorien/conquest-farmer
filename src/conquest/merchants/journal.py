...
LEGAL_TRANSITIONS = {
    "prepared": frozenset(
        ("submitted", "uncertain", "verified", "aborted", "operator_overridden")
    ),
...
def canonical_delivery_items(items):
    ...
    exact_items(items)
    return sorted(
        ({name: item.get(name) for name in DELIVERY_ITEM_FIELDS} for item in items),
...
def profile_row(cursor, values):
    values = tuple(
        database_character(value) if column[0] == "character" else value
...
class Journal:
    ...
    def set(self, character, name, value):
        ...
        with self.db() as db:
            db.execute(
                ...
                (character, name, json.dumps(value)),
            )
    ...
    def begin(self, key, character, kind, before, *, admission=False):
        ...
        with self.db() as db:
            ...
            if old:
                if (old["character"], old["kind"], old["before_json"]) != (
                    character,
                    kind,
                    encoded,
                ):
                ...
                return False
            pending = db.execute(
                "SELECT 1 FROM transactions WHERE character=? AND phase NOT IN ('verified','aborted','operator_overridden')",
                (character,),
            ).fetchone()
            if pending:
            ...
            db.execute(
                ...
                (key, character, kind, "prepared", encoded, now, now),
            )
            db.execute(
            ...
            if admission:
                row = db.execute(
                    "SELECT * FROM delivery_admissions WHERE request_id=?", (key,)
                ).fetchone()
                items = json.dumps(
                    canonical_delivery_items(before["items"]), sort_keys=True
                )
                if (
                    not row
                    or row["character"] != character
                    or json.loads(row["uids_json"])
                    != sorted(item["uid"] for item in before["items"])
                    or row["items_json"] not in (None, items)
                ):
                    raise ValueError(
                        "Delivery admission changed before transaction persistence"
                    )
                db.execute(
                    "UPDATE delivery_admissions SET phase='transaction_started',items_json=?,reason=NULL,updated=? WHERE request_id=? AND phase='admitted'",
                    (items, now, key),
                )
    ...
    def transition(self, key, phase, result=None, *, expected=None):
        ...
        with self.db() as db:
            ...
            if phase not in LEGAL_TRANSITIONS.get(current, ()):
                raise ValueError(
                    f"Illegal transaction transition: {current} -> {phase}"
                )
            now = time.time()
            encoded = json.dumps(result)
            changed = db.execute(
                "UPDATE transactions SET phase=?,result_json=?,updated=? WHERE id=? AND phase=?",
                (phase, encoded, now, key, current),
            ).rowcount
            if changed != 1:
                raise ValueError("Transaction phase changed before transition")
            db.execute(
            ...
            if phase == "verified":
                db.execute(
                    ...
                    (
                        row["character"],
                        row["kind"] + "_verified",
                        json.dumps({"transaction_id": key, "result": result}),
                        now,
                    ),
                )
    ...
    def operator_override(
        ...
        *,
        ...
        with self.db() as db:
            ...
            original_steps = [
                step for step in steps if step["stage"] != "operator_recheck"
            ]
            original = {
                "id": key,
            ...
            unobserved_cleanup = any(
                step["stage"] in ("cleanup", "cleanup_trade")
                and step["status"] == "before_action"
                and not any(
                    later["stage"] == step["stage"]
                    and later["status"] == "observed"
                    and later["timestamp"] >= step["timestamp"]
                    for later in steps
                )
                for step in steps
            )
            ...
            changed = db.execute(
                "UPDATE transactions SET phase=?,result_json=?,updated=? "
                "WHERE id=? AND phase=?",
                ("operator_overridden", encoded, now, key, row["phase"]),
            ).rowcount
            if changed != 1:
                raise ValueError("Transaction changed before operator override")
            db.execute(
                "INSERT INTO transaction_steps(transaction_id,stage,status,payload,timestamp) "
                "VALUES(?,?,?,?,?)",
            ...
            db.execute(
                ...
                (
                    row["character"],
                    "farmer_delivery_operator_overridden",
                    json.dumps(
                        {
                            "transaction_id": key,
                            "original_evidence_digest": original_digest,
    ...
    def original_evidence_digest(self, key):
        ...
        original = {
            ...
            if row["phase"] != "operator_overridden"
                ...
                json.loads(row["result_json"] or "{}").get("operator_override") or {}
            ).get("original_phase", row["phase"]),
            "before_json": row["before_json"],
            "result_json": None
            if row["phase"] == "operator_overridden"
    ...
    def step(self, key, stage, status, payload=None, *, terminal=False):
        if (
            ...
            or not 1 <= len(stage) <= 100
            or not isinstance(status, str)
            or not 1 <= len(status) <= 40
        ):
        ...
        with self.db() as db:
            db.execute("BEGIN IMMEDIATE")
            row = db.execute(
                "SELECT phase FROM transactions WHERE id=?", (key,)
            ).fetchone()
            if not row:
                raise ValueError("Transaction not found")
            if row["phase"] in TERMINAL_PHASES:
                if not terminal or stage not in (
                    "cleanup",
                    "cleanup_trade",
                    "receiver_receipt",
                ):
        ...
        with self.db() as db:
            return [
                ...
                for row in db.execute(
                    "SELECT id,stage,status,payload,timestamp FROM transaction_steps WHERE transaction_id=? ORDER BY id",
                    (key,),
                )
            ]

    def admit_delivery(self, key, character, uids, origin, items=None):
        encoded_uids = json.dumps(sorted(uids))
        encoded_origin = json.dumps(origin, sort_keys=True)
        encoded_items = (
            json.dumps(canonical_delivery_items(items), sort_keys=True)
        ...
        with self.db() as db:
            ...
            if row:
                if (
                    ...
                    or row["uids_json"] != encoded_uids
                    or row["origin_json"] != encoded_origin
                    or encoded_items is not None
                    and row["items_json"] not in (None, encoded_items)
                ):
    ...
    def update_delivery_admission(self, key, phase, *, items=None, reason=None):
        ...
        with self.db() as db:
            ...
            if not row:
                raise ValueError("Delivery admission not found")
            ...
            db.execute(
                "UPDATE delivery_admissions SET phase=?,items_json=?,reason=?,updated=? WHERE request_id=?",
    ...
    def pending(self, character):
        with self.db() as db:
            rows = db.execute(
                "SELECT * FROM transactions WHERE character=? AND phase NOT IN ('verified','aborted','operator_overridden') ORDER BY created",
    ...
    def request_scan(self, character, request_id, now=None):
        ...
        with self.db() as db:
            db.execute("BEGIN IMMEDIATE")
            seen = db.execute(
                "SELECT state FROM scan_requests WHERE character=? AND request_id=?",
            ...
            if state.get("request_id") == request_id or state.get("pending"):
                db.execute(
                    "INSERT OR IGNORE INTO scan_requests VALUES(?,?,?)",
                    (character, request_id, json.dumps(state)),
                )
                return state
            state = {
            ...
            db.execute(
                "INSERT OR REPLACE INTO state VALUES(?,?,?)",
                (character, "scan", json.dumps(state)),
            )
            db.execute(
    ...
    def complete_scan(self, character, request_id, *, changed, deferred, now=None):
        ...
        with self.db() as db:
            ...
            for request in db.execute(
                "SELECT request_id,state FROM scan_requests WHERE character=?",
                (character,),
            ).fetchall():
                if json.loads(request["state"]).get("request_id") == request_id:
                    db.execute(
                        "UPDATE scan_requests SET state=? WHERE character=? AND request_id=?",
    ...
    def request_once(self, character, request_id, now=None):
        ...
        with self.db() as db:
            ...
            if old and json.loads(old[0]).get("pending"):
                raise ValueError(
                    "Finish or pause the existing scan before requesting a different one-time batch"
                )
            if db.execute(
                "SELECT 1 FROM transactions WHERE character=? AND phase NOT IN ('verified','aborted','operator_overridden')",
                (character,),
                ...
                raise ValueError(
                    "Reconcile the unfinished transaction before listing once"
                )
            state = {
                "request_id": request_id,
                "pending": True,
    ...
    def resume_batch(self, character):
        ...
        with self.db() as db:
            ...
            if not state.get("pending"):
                raise ValueError("No paused batch to resume")
            if db.execute(
                "SELECT 1 FROM transactions WHERE character=? AND phase NOT IN ('verified','aborted','operator_overridden')",
                (character,),
            ).fetchone():
                raise ValueError(
                    "Reconcile the interrupted transaction before resuming"
                )
            state["one_time"] = True
            db.execute(
                "INSERT OR REPLACE INTO state VALUES(?,?,?)",
