...
def trade_grid(gui, snapshot):
    ...
    if gui.session.expected_sha256 == CLIENT_SHA256_1078:
        ...
        for rva, code in (
            (0x10F44D, "488d0d34bd4b00"),
            (0x10F606, "488d0d93314b00"),
            (0x10F623, "e848fa0a00"),
        ):
            if gui.session.read_block(
                gui.base + rva, len(bytes.fromhex(code))
            ) != bytes.fromhex(code):
...
def cell(table, slot, offset=(20, 20)):
    ...
    if type(slot) is not int or slot < 0:
        raise ValueError("Invalid grid slot")
    x = columns[slot % len(columns)]["content_x"] + offset[0]
    y = table["outer"][1] + slot // len(columns) * table["row_height"] + offset[1]
    l, t, r, b = table["clip"]
    if not l + 2 < x < r - 2 or not t + 2 < y < b - 2:
        raise ValueError("Item cell is clipped")
...
def endpoints(gui, snapshot, item, offered):
    ...
    if len(offered) >= 20:
        raise ValueError("Trade grid is full")
