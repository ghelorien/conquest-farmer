...
def peer_evidence(observer, peer):
    ...
    pointers = [
        struct.unpack_from("<Q", entries, i + p.entry_object_offset)[0]
        for i in range(0, len(entries), p.entry_stride)
    ]
    if len(set(pointers)) != len(pointers):
        raise ValueError("Peer evidence scene is ambiguous")
    matches = []
    ...
    for address in pointers:
        ...
        for uid_offset, name_offset, position_offset, draw_offset in (
            ...
            if uid != peer["character_uid"]:
                continue
            name = raw[name_offset : name_offset + 32].split(b"\0")[0]
            if name != peer["character"].encode("utf-8"):
                continue
            position = list(struct.unpack_from("<2I", raw, position_offset))
            if position != peer["position"]:
                raise ValueError("Peer position differs between clients")
            ...
            matches.append(
                {
                    ...
                    "draw_format": "i32",
                    "draw_i32": list(struct.unpack_from("<2i", raw, draw_offset)),
                    "draw_bytes": raw[draw_offset : draw_offset + 8].hex(),
                }
            )
    if len(matches) != 1:
        raise ValueError("One matching peer identity was not found in the scene")
    if (
        sample_fields(s, fields) != header
