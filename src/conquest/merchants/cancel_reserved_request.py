...
from conquest.merchants.delivery_operation import (
    JOURNAL,
    clear_settled_attention,
    status,
)
...
def _origin(intent):
    return {
        ...
        for name in ("operation_id", "town_visit_id", "visit_id", "farmer_profile_id")
    }
...
def _same_participants(intent, active):
    ...
    return (
        active.get("merchant") == merchant["character"]
        ...
        and all(
            active.get(name) == intent.get(name)
            for name in ("town_visit_id", "visit_id", "farmer_profile_id")
        )
    )
...
def _source(ui):
    ...
    if not isinstance(key, str) or not key:
        raise ValueError(
            "No active farmer route delivery is available for cancellation"
        )
    ...
    if (
        not admission
        or admission["character"] != character
        or admission["phase"] != "transaction_started"
        or json.loads(admission["uids_json"])
        != sorted(item["uid"] for item in intent["items"])
    ...
    if (
        ...
        or not _same_items(
            reservation.get("intent", {}).get("items", []), intent["items"]
        )
        or any(
            reservation.get("intent", {}).get(role, {}).get(field)
            != intent[role].get(field)
...
def _idle(ui, character):
    ...
    if (
        ...
        or character in ui.runtime.refilling
    ):
        raise ValueError(
            "Cancellation requires Farming Off, a paused merchant and idle native input"
        )
    ...
    payload = json.dumps(
        {
            "request_id": key,
            "character": character,
            "intent_operation_id": intent.get("operation_id"),
        },
        sort_keys=True,
    )
    with journal.db() as db:
        ...
        exists = db.execute(
            ...
            "AND stage='cancel_request' AND status='before_action'",
            (key,),
        ).fetchone()
        if exists:
            raise ValueError(
                "Cancellation input was already submitted; re-entry is read-only"
            )
        db.execute(
...
def _archive_terminal(output, state):
    ...
    archive = (
        output.parent
        / "reserved-request-cancellation-audit"
        / (hashlib.sha256(encoded.encode()).hexdigest() + ".json")
    )
    if archive.exists():
        if read_json(archive) != state:
            raise ValueError(
                "Historical cancellation audit differs from its terminal receipt"
            )
        return True
...
def _terminal(ui, journal, key, intent, farmer, merchant):
    ...
    if terminal:
        ...
        if (
            payload.get("outcome") != "cancelled_unaccepted"
...
def _recover(ui, journal, key, character, intent, state):
    ...
    if not cancelled(intent, farmer, merchant):
        raise ValueError(
            "Cancellation remains unverified; only read-only reconciliation is available"
        )
    digest = _terminal(ui, journal, key, intent, farmer, merchant)
    state.update(
        phase="cancel_verified",
        farmer_after=farmer,
        merchant_after=merchant,
        ownership_digest=digest,
    ...
    transaction = DeliveryTransaction(
        journal,
        SimpleNamespace(read_pair=lambda name: pair(ui, name)),
        sale_receipts=lambda planned, source, destination: qualified_delivery_receipts(
            ui.runtime.journal, planned, destination
        ),
    )
...
def start(ui):
    ...
    if previous and previous.get("request_id") not in (None, key):
        if not _archive_terminal(output, previous):
            raise ValueError(
                "Another reserved-request cancellation needs reconciliation"
            )
    if _cancel_before_actions(journal, key):
        ...
        state = (
            previous
            if previous
            else {"request_id": key, "character": character, "intent": intent}
        )
        return {
            "request_id": key,
            "running": False,
            "receipt": _recover(ui, journal, key, character, intent, state),
            "reentered_read_only": True,
        }
    _eligible(ui, journal, key, character, intent)
    state = {
        "phase": "request_verified",
        "kind": "cancel_reserved_request",
        "request_id": key,
        "character": character,
        "intent": intent,
    ...
    ui.delivery_probe_thread = threading.Thread(
        target=work, daemon=True, name="cancel-reserved-request"
    )
...
def prove_request_replaced(intent, farmer, merchant):
    ...
    return {
        "outcome": "original_request_absent_no_transfer",
        "farmer": farmer,
        "merchant": merchant,
        "verified_at": time.time(),
    }
