...
def close_point(driver, snapshot, *, display_only=False):
    ...
    if (
        unpack(s, model + 0xC, "<B")[0] != 1
        or (
            not display_only
            and unpack(s, model + 0x4C, "<I")[0] != snapshot.get("own_booth_uid")
        )
...
def close_owned_panel(driver, travel, journal, check):
    ...
    record = dict(
        phase="close_prepared", before=before, point=point, started_at=time.time()
    )
        ...
        while True:
            ...
            try:
                ...
                if time.monotonic() >= deadline:
                    raise ValueError(
                        "Booth close hover was not verified; no button pressed"
                    )
                time.sleep(0.01)
                continue
            if time.monotonic() >= deadline:
    ...
    while True:
        ...
        if not fresh["booth_open"]:
            record.update(
                ...
                own_booth_uid=fresh["own_booth_uid"],
