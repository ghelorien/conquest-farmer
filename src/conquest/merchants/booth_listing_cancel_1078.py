...
def _pending(journal, request_id, character, *, allow_cancel=False):
    ...
    if (not row or row['kind'] != KIND or row['character'] != character
        ...
        raise ValueError('Cancellation requires the exact unresolved pre-confirmation listing')
    trace = journal.trace(request_id)
    if any(step['stage'] == 'confirm_press' for step in trace):
        raise ValueError('Listing OK may have been submitted; cancellation is forbidden')
    if not allow_cancel and any(step['stage'] == 'cancel_press' for step in trace):
        raise ValueError('Cancellation was already attempted; observe its result without replay')
    return json.loads(row['before_json'])


def _unchanged(snapshot, before, profile):
    from conquest.merchants.booth_listing_once_1078 import OWNERSHIP_FIELDS, _validate_snapshot
    _validate_snapshot(snapshot, profile, before['request'])
    old = before['snapshot']
    if any(snapshot[key] != old[key] for key in OWNERSHIP_FIELDS if key in old):
        raise ValueError('Original listing ownership changed; cancellation is unavailable')
...
def _cancel_marker(journal, request_id, character, payload):
    ...
    with journal.db() as db:
        ...
        if (not row or row['kind'] != KIND or row['character'] != character
                or row['phase'] not in ('prepared', 'uncertain')
                or db.execute("SELECT 1 FROM transaction_steps WHERE transaction_id=? "
                              "AND stage IN ('confirm_press','cancel_press')", (request_id,)).fetchone()):
            raise ValueError('Listing/cancellation journal changed before Cancel mouse-down')
        db.execute('INSERT INTO transaction_steps(transaction_id,stage,status,payload,timestamp) VALUES(?,?,?,?,?)',
                   (request_id, 'cancel_press', 'before_mouse_down', json.dumps(payload, sort_keys=True), time.time()))


def reconcile_cancel(ui, request_id, character):
    from conquest.merchants.booth_listing_once_1078 import (
        _profile, _status, _row, KIND, OWNERSHIP_FIELDS,
    )
    ...
    with MemorySession(identity['pid'], CLIENT_SHA256_1078) as session:
        ...
        def cleared():
            return (not any(w['name'] == 'Add Item to Booth' for w in gui.windows())
        ...
        journal.transition(request_id, 'aborted', {
            'uid': before['request']['item_uid'], 'cancel_verified': True,
            'stock_unchanged': True, 'listing_submitted': False,
            'confirmation_attempted': False, 'cancellation_attempted': True,
            'first': first, 'second': second, 'replay_allowed': False,
...
def dispatch_cancel(ui, request_id, character):
    from conquest.merchants.booth_listing_once_1078 import (
        WORKER_LOCK, WORKERS, _profile, _status, _row, _policy,
        _merchant_intent, _farmer_safe_market,
    )
    journal = ui.runtime.journal
    with WORKER_LOCK:
        worker = WORKERS.get(request_id)
        if worker and worker.is_alive():
            raise ValueError('Wait for the exact listing input worker to release before cancellation')
        row = _row(journal, request_id)
        ...
        _policy(ui, character, profile, control, request_id=request_id,
                farmer_target=farmer, merchant_intent=intent,
                phases=('prepared', 'uncertain'), explicit_cleanup=True)
        if not ui.runtime.can_start_work(6):
            raise CaptureUnavailable('A fresh safe handoff with six seconds remaining is required to cancel')
        token = ui.coordinator.fence.capture() if ui.coordinator.fence else None
        journal.step(request_id, 'cancel_requested', 'prepared',
                     {'control': control, 'farmer_target': farmer,
                      'identity': before['request']['expected_identity'],
                      'item_uid': before['request']['item_uid']})
        worker = threading.Thread(target=_run_cancel,
            args=(ui, character, profile, request_id, before, control, farmer, intent, token),
            daemon=True, name='booth-1078-cancel-once')
        WORKERS[request_id] = worker
        worker.start()
    return _status(journal, request_id, character)


def _run_cancel(ui, character, profile, request_id, before, control, farmer, intent, token):
    from conquest.merchants.booth_listing_once_1078 import (
        KIND, WORKER_LOCK, WORKERS, _policy, _modal,
    )
    ...
    try:
        with MemorySession(request['expected_identity']['pid'], CLIENT_SHA256_1078) as session, physical_coordinates():
            ...
            def policy():
                _policy(ui, character, profile, control, request_id=request_id,
                        deadline=deadline, farmer_target=farmer, merchant_intent=intent,
                        phases=('prepared', 'uncertain'), explicit_cleanup=True)
                session.assert_identity()
                native = target.snapshot()
                if native['root_hwnd'] != target.hwnd or focused and native['foreground'] != target.hwnd:
                    raise CaptureUnavailable('Exact native merchant foreground changed during cancellation')
            ...
            stage = 'acquire_input'
            with (fence.bind_worker(token) if fence else nullcontext()), coordinator.booth_listing_once_scope(character, policy):
                with coordinator.lease(character, purpose=KIND):
                    ...
                    if not activate_client(target.hwnd, session.identity):
                        raise CaptureUnavailable('Native merchant focus unavailable for exact cancellation')
                    focused = True
                    check()
                    stage = 'stable_layout'
                    ...
                    window, points, buffer = _modal(gui, model, baseline, request['item_uid'])
                    digits = buffer.replace(',', '')
                    if digits and (not digits.isascii() or not digits.isdigit()
                                   or not str(request['price']).startswith(digits)):
                        raise ValueError('Native partial price is not a prefix of this exact listing request')
                    binding = (window, points, buffer)
                    point = tuple(round(value*physical/logical) for value, physical, logical in
                                  zip(points['Cancel'], revision.client_size, revision.gui_size))
                    if any(not 1 < value < maximum-2 for value, maximum in zip(point, revision.client_size)):
                        raise ValueError('Native Cancel point is outside the exact client')
                    ...
                    def fresh():
                        ...
                        if (gui.model(25, booth_vtable) != model
                                or unpack(gui.session, model+0x4c, '<I')[0] != request['expected_own_booth_uid']
                                or _modal(gui, model, snapshot, request['item_uid']) != binding):
                            raise ValueError('Exact native price dialog changed before cancellation')
                    ...
                    def press():
                        ...
                        hover()
                        _cancel_marker(journal, request_id, character, {
                            'uid': request['item_uid'], 'price_buffer': buffer,
                            'identity': session.identity, 'owned_booth_uid': request['expected_own_booth_uid'],
                            'window': {'name': window['name'], 'address': window['address']},
                            'control': 'Cancel'})
                        marked = True

                    stage = 'native_cancel_hover_and_press'
                    foreground_click(target, *point, revision.client_size,
                                     require_foreground=True, before_press=hover,
                    ...
                    while time.monotonic() < until:
                        ...
                        if reconcile_cancel(ui, request_id, character)['phase'] == 'aborted':
                            return
                        session.assert_identity()
                        time.sleep(.1)
                    raise CaptureUnavailable('Cancel result uncertain; only read-only reconciliation is allowed')
    except BaseException as error:
        failure = {'confirmation_attempted': False, 'cancellation_attempted': marked,
                   'replay_allowed': False, 'reason': str(error),
                   'error_type': type(error).__name__, 'failure_stage': stage}
        try:
            ...
            journal.step(request_id, 'cancel_failed', 'uncertain' if marked else 'before_press', failure)
            from conquest.merchants.booth_listing_once_1078 import _row
            row = _row(journal, request_id)
            if row and row['phase'] == 'prepared':
                journal.transition(request_id, 'uncertain', failure, expected='prepared')
        ...
        try:
            ...
            journal.set(character, 'attention', {'kind': KIND, 'transaction_id': request_id,
                'note': 'Exact pre-confirmation listing cleanup failed at '+stage+': '+str(error),
