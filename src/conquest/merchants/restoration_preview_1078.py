...
def _verified_listing_receipts(db, profile_id):
    ...
    for request_id, before_json, result_json in db.execute(
        ...
        if not proof or result.get("exact_memory_listing_verified") is not True:
            continue
        steps = [
            dict(zip(("stage", "status", "payload"), row))
            for row in db.execute(
                "SELECT stage,status,payload FROM transaction_steps WHERE transaction_id=? ORDER BY id",
                (request_id,),
            )
        ]
        if before.get("profile_id") != profile_id or proof != _proof(
            before, steps, proof["first"], proof["second"]
        ):
            raise ValueError("New booth stock listing evidence changed")
        request = before["request"]
        item = next(
            item
            for item in proof["second"]["booth"]
            if item["uid"] == request["item_uid"]
        )
        receipts.append(
            {
                ...
                "character_uid": proof["character_uid"],
                "own_booth_uid": proof["own_booth_uid"],
                "item": item,
    ...
    for sale_id, at, encoded, silver in db.execute(
        ...
        try:
            ...
            if (
                not items
                or any(
                    type(i.get("price")) is not int
                    or not 1 <= i["price"] <= 2_147_483_647
                    for i in items
                )
                or type(silver) is not int
            ...
            for event_id, payload in db.execute(
                ...
                "AND event='sale_verified' AND timestamp=?",
                (profile_id, at),
            ):
                event = json.loads(payload)
                started = event.get("from")
                if (
                    event.get("items") == items
                    and event.get("silver") == silver
...
def _journal_image(path, profile_id):
    ...
    with sqlite3.connect(uri, uri=True, timeout=2) as db:
        ...
        values["verified_listing_receipts_1078"] = _verified_listing_receipts(
            db, profile_id
        )
        ...
        admissions = list(
            db.execute(
                ...
                "AND phase IN ('admitted','transaction_started')",
                (profile_id,),
            )
        )
        reservations = [
            (key, json.loads(state).get("phase"))
    ...
    if (
        transactions
        or admissions
        or any(phase not in _TERMINAL_RESERVATIONS for _, phase in reservations)
    ):
        raise ValueError(
            "An unresolved merchant transaction or delivery reservation needs reconciliation"
        )
    reservation = values.get("delivery_reservation") or {}
    if reservation and reservation.get("phase") not in _TERMINAL_RESERVATIONS:
        raise ValueError(
            "An unresolved merchant delivery reservation needs reconciliation"
        )
    return values
...
def _preview(snapshot, state):
    ...
    if not snapshot["closed_modal"]:
        raise ValueError(
            "Close the live merchant trade/request before restoration preview"
        )
    if (
    ...
    changed = {
        uid
        for uid in set(historical) & set(current)
    ...
    for uid in sorted(missing):
        ...
        for receipt in state.get("verified_sale_receipts", []):
            if (
                receipt.get("profile_id") != snapshot["profile_id"]
                or not max(
                    before.get("timestamp") or 0, incident.get("started_at") or 0
                )
                <= receipt["from"]
                < receipt["observed_at"]
                <= snapshot["timestamp"]
            ...
            listings = [
                listing
                for listing in state.get("verified_listing_receipts_1078", [])
            ...
            if listings:
                matches.append(
                    {
                        "sale": receipt,
                        "listing": max(listings, key=lambda r: r["observed_at"]),
                    }
                )
        if len(matches) == 1:
            sold.append(
                {
                    ...
                    **(
                        {"listing_receipt": matches[0]["listing"]}
                        if "listing" in matches[0]
                        else {}
                    ),
                }
            )
    if missing - {row["uid"] for row in sold} or changed:
        raise ValueError(
            "Pre-disconnect merchant ownership differs from current stock; reconcile before restoring"
        )
        ...
        wanted.append(
            {
                ...
                "prior_listing_price": price,
                "prior_total_price": price,
    ...
    new_inventory = [
        {
            "uid": uid,
            "name": current_inventory[uid]["name"],
            "quantity": current_inventory[uid]["quantity"],
            "price": None,
            "origin": "new_or_unattributed_inventory",
        }
    ...
    for uid in new_booth:
        ...
        matches = [
            receipt
            for receipt in state.get("verified_listing_receipts_1078", [])
            ...
            and all(
                receipt["item"][key] == actual[key] for key in (*_ITEM_FIELDS, "price")
            )
        ]
        if not matches:
            raise ValueError(
                "Unattributed newly listed booth stock requires reconciliation: UID "
                + str(uid)
            )
        attributed_booth.append(
            {
                "uid": uid,
                "price": actual["price"],
    ...
    cursor_rows = [
        {
            ...
            "origin": (
                "prior_booth"
                if uid in old_booth
                else "prior_inventory"
                if uid in old_inventory
                else "new_inventory"
                if uid in new_uids
                else "not_currently_owned"
            ),
        }
        for uid in cursor
    ]
    ...
    if any(
        item["current_location"] == "booth" and not item["already_restored"]
        for item in wanted
    ):
    ...
    return {
        ...
        "closed_modal": True,
        "incident_phase": incident["phase"],
        "incident_started_at": incident.get("started_at"),
        "recovery_safety_phase": safety.get("phase"),
        "restore_prior_listings": wanted,
        "new_inventory_without_prior_listing": new_inventory,
        ...
        "unchanged_prior_inventory_count": (
            len(old_inventory)
            - len(prior_inventory_now_listed)
            - sum(row["origin"] == "prior_inventory" for row in sold)
        ),
        "refill": {
            "pending": bool(refill.get("pending")),
            "status": refill.get("status"),
            "last_checked": refill.get("last_checked"),
            "cursor": cursor_rows,
...
def preview(runtime, character):
    ...
    if snapshot["profile_id"] != profile_id or not snapshot["profile_uid_verified"]:
        raise ValueError(
            "Current merchant identity is not the configured profile identity"
        )
