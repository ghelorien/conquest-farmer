...
def binding(ui, character, capability, expected=None):
    ...
    if (capability is None or coordinator._probe_abort_capability is not capability
            or coordinator.thread!=capability[0] or coordinator.purpose!=abort.PURPOSE
    ...
    saved=abort.read();state=abort.probe.read_probe()
    if (state.get('phase')!='offer_verified' or abort.digest(state)!=saved['probe_digest']
            or saved['phase'] not in ('abort_prepared','cancel_submitted')
            or state['character']!=str(character) or state['target_profile_id'] not in targets):
        raise ValueError('Abort presentation receipt changed')
    merchant=state['intent']['merchant'];profile_id=state['target_profile_id']
    ...
    if profiles:
        ...
        if (resolved.id!=profile_id or not resolved.local_enabled or resolved.name!=str(character)
                or resolved.server!=merchant['server']):raise ValueError('Abort merchant profile changed')
        profile=ProfileName(resolved.name,resolved.id)
    else:
        if profile_id!=str(character):raise ValueError('Abort merchant profile changed')
        profile=character
    identity=merchant['identity'];observer=ui.runtime.observers.get(profile)
    controller=ui.runtime.controllers.get(profile)
    if (merchant['character']!=str(character) or merchant['server']!='America'
            or observer is None or controller is None or observer.adapter.identity!=identity
    ...
    if (type(getattr(target,'hwnd',None)) is not int or target.hwnd<=0
            ...
            or expected is not None and (target.hwnd,identity,profile_id)!=expected):
    ...
    if (host is None or not host.saved or host.mode!='owned'
            or host.saved.hwnd!=target.hwnd or host.saved.identity!=identity):
...
def prepare(ui, character, capability):
    ...
    ui.notebook.select(ui.frames[profile]);ui.detail_tabs[profile].select(0)
    layout=getattr(ui,'apply_client_compact_layout',None)
    if layout:layout()
    ui.root.update_idletasks()
    target,host,identity,profile,profile_id=binding(ui,character,capability,expected)
