...
def _context(character, build):
    ...
    if context is None:
        if registry() is not None:
            raise ValueError('Select a farmer profile before using delivery qualification')
        return None
    if (context.profile.role != 'Farmer' or context.profile.name != character
            or context.profile.server != 'America' or not context.profile.local_enabled):
        raise ValueError('Delivery qualification does not belong to the selected farmer')
...
def _path(context, build):
    ...
    return context.state_dir / '.runtime' / 'delivery-q' / (build[:16].lower() + '.json')


def _matches(data, context, build, *, legacy=False):
    profile = context.profile
    return (data.get('character') == profile.name and data.get('server') == profile.server
            ...
            and data.get('character_uid') in ((None, profile.character_uid) if legacy
                                              else (profile.character_uid,)))
...
def _migration_receipt_matches(data, context):
    ...
    if (state.get('phase') != 'delivery_verified' or not state.get('verified_at')
            or not intent.get('items') or any(
    ...
    return reconcile(intent, after, merchant,
...
def qualification_path(observer, *, migrate=True):
    ...
    if destination.exists():
        if not _matches(read_json(destination), context, build):
            raise ValueError('Stored delivery qualification has a different profile or build identity')
        return destination
    old_path = Path(state_path(LEGACY))
    old = read_json(old_path) if migrate else {}
    if old and _matches(old, context, build, legacy=True) and _migration_receipt_matches(old, context):
        migrated = {**old, 'profile_id': context.profile.id,
                    'character_uid': context.profile.character_uid,
                    'qualification_scope_version': 1, 'migrated_from': str(old_path)}
...
def promotion_destination(requested_path, data, farmer):
    ...
    if (context.profile.character_uid is None
        ...
        raise ValueError('Bind the exact farmer UID before promoting delivery qualification')
    return _path(context, data['client_sha256']), {
        **data, 'profile_id': context.profile.id, 'character_uid': context.profile.character_uid,
