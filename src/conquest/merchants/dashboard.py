...
def merchant_text(state, *, now, waiting_items=None, global_stopped=False):
    ...
    pending=[r for r in state.get('pending',[]) if r.get('phase')=='uncertain' or not state.get('input_active')]
    error=state.get('error') or {}
    title='Connected' if state.get('connected') else 'Waiting for client'
    stock=(f"Inventory {len(snapshot.get('inventory',[]))}/{snapshot.get('capacity','?')} · Shop {len(snapshot.get('booth',[]))}/32"
           if snapshot else 'Inventory and shop counts unavailable')
    action=''
    if pending:
        ...
        detail=json.loads(pending[0].get('result_json') or '{}').get('note','Result needs reconciliation')
        action='Reason: '+detail+' Check Logs; verify the item and silver before retrying.'
    elif market.get('pending'):
        if market.get('phase')=='failed':
            batch='Market download failed · retry in '+countdown(market.get('retry_at',now),now)
            action=market.get('error','Prices unchanged; retry is automatic.')
        else:
            batch='Downloading America market prices…'
            action='Shop update starts after the download.' if scan.get('pending') else 'Downloading price data only. Your shop listings stay unchanged.'
    elif scan.get('pending'):
        if not state.get('enabled'):
            batch='Shop update paused';action='Click Resume shop update to finish this request.'
        elif error:
            batch='Shop update waiting: '+error['note']
            action='Continues when your mouse is idle and the farmer can safely share control.' if 'input' in error['note'].lower() or 'handoff' in error['note'].lower() else 'The update stays queued until this problem is resolved.'
        elif progress.get('request_id') and progress['request_id']==scan.get('request_id'):
            batch=f"Updating shop: {progress.get('changed',0)} done · {progress.get('remaining',0)} remaining"
            action='Current item: '+progress['item'] if progress.get('item') else 'Preparing the next item.'
        else:
        ...
        if waiting_items:
            ...
            reasons=[]
            if unpriced:reasons.append(f'{unpriced} need a safe price')
            if len(waiting_items)>unpriced:reasons.append(f'{len(waiting_items)-unpriced} need shop space or safe input')
            action='Waiting items: '+', '.join(reasons)+'. Open the Waiting items tab for reasons.'
...
def on_sale_text(characters, *, now):
    ...
    for character in CHARACTERS:
        ...
        if (state.get('connected') and snapshot.get('booth_open')
                and 0 <= now-snapshot.get('timestamp', 0) <= 10
                and isinstance(booth, list)
                and all(type(item.get('price')) is int and item['price'] > 0 for item in booth)):
    ...
    parts = [f'Currently on sale: {total}' + (' (partial)' if totals and not complete else '')]
    parts.extend(f'{c}: {totals[c]:,}' if c in totals else f'{c}: unavailable' for c in CHARACTERS)
    return '     |     '.join(parts)
...
def header_text(sales, reporting, characters, *, now):
    ...
    timers = f'Discord #shops (4h): {report}     |     Reprice (12h) — ' + ' · '.join(scans)
    ...
    for character in CHARACTERS:
        ...
        amount = f'{row["total"]["silver"]:,}' if row.get('started_at') is not None else 'unavailable'
    ...
    if len(tracked) != len(CHARACTERS) or any(
            r.get('gaps') or r.get('unconfirmed') or now-(r.get('last_observed_at') or 0)>10
            for r in tracked):
        parts.append('Coverage incomplete')
    return {'timers': timers, 'silver': '     |     '.join(parts),
