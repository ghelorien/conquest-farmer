...
def _reconcile_pending(controller, snapshot, pending):
    ...
    if snapshot.get("request") or snapshot.get("trade"):
        raise CaptureUnavailable(
            "Unrelated request decline awaits read-only reconciliation"
        )
    ...
    if (
        ...
        raise CaptureUnavailable(
            "Unrelated request decline changed client, stock, booth, or silver"
        )
    controller.journal.set(
        controller.character,
        "unrelated_request_decline",
...
def _require_no_owned_work(controller):
    ...
    if reserved(controller.journal, controller.character) or controller.journal.pending(
        controller.character
    ):
        raise CaptureUnavailable(
            "Incoming request waits for transaction reconciliation"
        )
...
def _shared_profile(observer):
    ...
    for path in paths:
        try:
            ...
            continue
        if (
            data.get("client_sha256") == build
            and data.get("server") == "America"
            and data.get("recipient")
            and data.get("evidence")
        ):
            matches.append(data)
    if not matches:
        raise ValueError(
            "Remote-player identity layout is not qualified for this build"
        )
...
def requester_identity(observer, name):
    ...
    profile = _shared_profile(observer)
    ...
    if (
        any(type(o) is not int or not 8 <= o <= 2048 for o in offsets)
    ...
    if len(set(objects)) != len(objects):
        raise ValueError("Ambiguous remote-player ownership")
    vtable = base + spec["vtable_rva"]
    types = sample_fields(s, [(checked_address(o), "u64") for o in objects])
    matches = []
    for obj, kind in zip(objects, types):
        ...
        if any(
            raw[o : o + n] != fresh[o : o + n]
            for o, n in ((0, 8), (offsets[0], 4), (offsets[1], length), (offsets[2], 8))
        ):
    ...
    if sample_fields(s, headers) != header or (
        end > begin and s.read_block(begin, end - begin) != entries
    ):
...
def _cancel_control(driver, snapshot, name):
    ...
    if [string(s, model + o) for o in (0x48, 0x68, 0x88, 0xA8)] != [
        "Trade###Confirm",
        f"{name} wishes to trade with you.",
        "Accept",
        "Cancel",
    ]:
    ...
    if len(windows) != 1:
        raise ValueError("Incoming request dialog is absent or ambiguous")
    w = windows[0]
...
def decline_unrelated_request(
    controller, snapshot, *, operations_enabled, manual_session=None
):
    """Return True only after one unrelated request is durably cleared."""
    if not operations_enabled or not controller.active():
        return False
    allowed_maps = (
        (1002, 1011, 1036) if getattr(controller, "manual_farmer", False) else (1036,)
    )
    if (
        snapshot.get("map_id") not in allowed_maps
        or type(snapshot.get("hp")) is not int
        or snapshot["hp"] <= 0
    ):
        return False
    ...
    if (
        ...
        or not name
        or request.get("message") != f"{name} wishes to trade with you."
    ):
    ...
    scope = (
        input_scope(purpose=purpose)
        if getattr(controller, "manual_farmer", False)
        else controller.coordinator.lease(controller.character, purpose=purpose)
    )
    with scope, physical_coordinates():
        ...
        if not activate_client(driver.target.hwnd, fresh["identity"]):
            raise CaptureUnavailable(
                "Merchant focus unavailable; no decline input sent"
            )
        layout = driver.layout_revision()
        ...
        point = tuple(
            round(value * native / logical)
            for value, native, logical in zip(logical_point, size, gui_size)
        )
        ...
        def before():
            ...
            current = driver.read()
            if (
                current["identity"] != fresh["identity"]
            ...
            controller.journal.set(
                ...
                key,
                {
                    ...
                    "inventory": _exact_inventory(fresh["inventory"]),
                    "booth": _exact_booth(fresh.get("booth", [])),
                    "silver": fresh["silver"],
                    "at": time.time(),
                },
            )
    ...
    after = driver.wait_for(
        lambda s: not s.get("request") and not s.get("trade"),
        controller.check,
        seconds=3,
    )
    pending = controller.journal.get(controller.character, key)
    return _reconcile_pending(
        controller, after, controller.journal.get(controller.character, key)
    )
