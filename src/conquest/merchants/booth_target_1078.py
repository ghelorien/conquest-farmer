...
_PROJECTION_CODE = {
    ...
    0x13FE10: (
        "40534883ec40442b4148498bd92b51440f297424300f297c2420"
...
def _projection_candidate(session, booth, module, actor_vtable, accessor, candidates):
    ...
    if (
        _read(session, base + 0x5F0828, "<d")[0] != 1 / 64
    ...
    selected = [
        ...
        for c in candidates
        if (
            c["actor_offset"] == 0x308
            and c["graphics_vtable_rva"] == 0x5F04D0
            and c["orientation_candidate"] == 6
            and any(
                m["slot"] == 0x40 and m["method_rva"] == 0x274CD0 for m in c["methods"]
            )
        )
    ]
    if len(selected) != 1:
    ...
    camera_fields = tuple(
        (camera + offset, session.read(camera + offset, size))
    ...
    x, y = booth["position"]
    if (
        not 0 < x < map_width <= 4096
        or not 0 < y < map_height <= 4096
        or (width, height) != viewport
        or not 640 <= width <= 7680
        or not 480 <= height <= 4320
        or not 64 <= scale_numerator <= 1024
    ):
        raise ValueError("1078 booth camera or viewport is outside supported bounds")
    draw = ((x - y) * 32 + origin_x - scroll_x, (x + y) * 16 + origin_y - scroll_y)
    ...
    candidate = tuple(
        round((value - dimension // 2) * scale + dimension // 2)
    ...
    unscaled = tuple(
        math.trunc((value - dimension // 2) / scale) + dimension // 2
        for value, dimension in zip(candidate, viewport)
    )
    local_x = unscaled[0] + scroll_x - origin_x
    local_y = unscaled[1] + scroll_y - origin_y
    tile = (
        math.floor(local_y / 32 + local_x / 64 + 0.5),
        math.floor(local_y / 32 - local_x / 64 + 0.5),
    )
    if tile != (x, y):
        raise ValueError(
            "1078 booth candidate does not round-trip through native tile math"
        )
    if (
        any(session.read(address, len(raw)) != raw for address, raw in camera_fields)
    ...
    return {
        "candidate_point": list(candidate),
        "roundtrip_tile": list(tile),
        "viewport": list(viewport),
        "native_footprint": [list(v) for v in footprint],
        "camera_rva": 0x6B9B40,
        "zoom_numerator": scale_numerator,
        "candidate_only": True,
...
def _read(session, address, fmt):
    return struct.unpack(
        fmt, session.read(checked_address(address), struct.calcsize(fmt))
    )
...
def _graphics_methods(session, address, module):
    ...
    for slot in (0x20, 0x28, 0x40):
        ...
        if (
            session.read(method, 24) != prefix
            or _read(session, vtable + slot, "<Q")[0] != method
        ):
            raise ValueError(
                "Booth graphics method changed during read-only observation"
            )
        methods.append(
...
def _owned_scene_actor(session, entities, snapshot, module):
    ...
    def members():
        begin, end, capacity = sample_fields(
            session,
            [
                (collection + offset, "u64")
                for offset in (p.begin_offset, p.end_offset, p.capacity_offset)
            ],
        )
        if (
    ...
    matches = [a for a, uid in zip(typed, ids) if uid == snapshot["own_booth_uid"]]
    if len(matches) != 1:
        raise ValueError("Expected exactly one 1078 owned booth scene actor")
    address = matches[0]
    fields = [
        (address, "u64"),
        (address + p.id_offset, "u32"),
        (address + 0x7C, "u32"),
        (address + 0x84, "u32"),
    ...
    if (
        vtable != base + p.monster_vtable_rva
        or uid != snapshot["own_booth_uid"]
        or type_id != 0
        or model != 406
        or species != 0
    ...
    if (
        sample_fields(session, fields) != values
        or members() != objects
...
def collect(session, snapshot):
    ...
    if (
        _read(session, address, "<Q")[0] != module["base"] + layout.monster_vtable_rva
        or _read(session, address + layout.id_offset, "<I")[0]
        != snapshot["own_booth_uid"]
    ):
        raise ValueError("1078 owned booth scene actor changed")
    actor_vtable = _read(session, address + 0x10, "<Q")[0]
    accessor = (
        _read(session, actor_vtable + 0x28, "<Q")[0]
        if _module_rva(actor_vtable, module) is not None
        else 0
    )
    ...
    for offset, graphics in zip(offsets, pointers):
        ...
        if method is not None:
            candidates.append(
                {
                    "actor_offset": offset,
                    "orientation_candidate": orientation
                    if 0 <= orientation < 8
                    else None,
                    **method,
                }
    ...
    if (
        tuple(_read(session, address + offset, "<Q")[0] for offset in offsets)
        != pointers
    ):
    ...
    if _read(session, address + 0x10, "<Q")[0] != actor_vtable or (
        _module_rva(actor_vtable, module) is not None
