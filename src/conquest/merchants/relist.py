    ...
    *,
    ...
    for character in CHARACTERS:
        ...
        if (
            ...
            or scan.get("one_time")
            and scan.get("pending")
        ):
            continue
    ...
    try:
        MarketSnapshot(
            json.loads(Path(market_path).read_text(encoding="utf-8")),
            now=now,
            max_age=120,
        )
    ...
    for character in due:
        ...
        if (
            state.get("enabled") is not True
            or state.get("scan", {}).get("one_time")
            and state["scan"].get("pending")
        ):
            continue
        bridge(
            {
                "action": "scan",
                "character": character,
                "request_id": f"12h:{int(now // 43200)}",
            }
        )
