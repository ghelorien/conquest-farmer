...
class GrantFence:
    ...
    def capture(self):
        ...
        with self.lock:
            return (
                self._bound()
                or self.active
                or GrantToken(self.epoch, self.generation, None, None, None, "idle")
            )
        ...
        *,
        ...
        with self.lock:
            ...
            if previous:
                ...
                if (
                    not previous["revoked"]
                    and token == self.active
                    and (
                        token.revision,
                        token.expires_at,
                        token.scope,
                        token.farmer_profile_id,
                    )
            ...
            if self.actions or any(
                not self._quiescence(row["token"])["released"]
    ...
    def _resolve(self, token_or_request_id):
        key = (
            token_or_request_id.request_id
            if isinstance(token_or_request_id, GrantToken)
            else token_or_request_id
        )
        row = self.requests.get(key)
        if row is None or (
            isinstance(token_or_request_id, GrantToken)
    ...
    def _quiescence(self, token):
        ...
        workers = sum(
            worker["token"] == token
            and worker["action_capable"]
            and not worker["acknowledged"]
            for worker in self.workers.values()
        )
        revoked = self.requests[token.request_id]["revoked"]
        return {
            "request_id": token.request_id,
            "released": revoked and not actions and not workers,
            "revoked": revoked,
