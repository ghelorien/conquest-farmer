...
def _buttons(gui):
    ...
    if (
        hashlib.sha256(read(base + HUD_RVA, HUD_SIZE)).hexdigest() != HUD_SHA256
    ...
    if (
        not 0 <= frame - last_frame <= 3
        or columns != 6
        or owners != (window.address, window.address)
        or not all(
            math.isfinite(v)
            for v in (left, top, right, bottom, *clip, row_height, x1, x2)
        )
        or bottom - top != 40
        or row_height != 40
        or x2 - x1 != 71
        or not left <= x1 < x2 <= right
        or not clip[0] <= x1 < x2 <= clip[2]
        ...
        or not 0 <= (x1 + x2) / 2 < viewport[0]
        or not 0 < top + 30 < viewport[1]
    ):
        raise ValueError("1078 Items/Trade table geometry changed")
    current = read(address, 536)
    if (
        read(context + 0x4338, 16) != header
