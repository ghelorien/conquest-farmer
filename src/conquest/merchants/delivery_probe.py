...
def read_probe(*, read_only=False):
    ...
    try:
        ...
        raise ValueError(
            "Trade probe evidence is unreadable; reconcile before input"
        ) from error
    ...
    if intent_path.exists():
        if read_only:
            raise ValueError("Trade probe disposition requires separate recovery")
        try:
            override = json.loads(intent_path.read_text(encoding="utf-8"))
            if (
                not isinstance(override, dict)
                or not isinstance(override.get("record"), dict)
                or override.get("terminal_phase") != "operator_overridden"
                or not isinstance(override.get("audit"), str)
                or not override["audit"]
            ):
                raise ValueError("Invalid operator disposition intent")
        except (OSError, ValueError) as error:
            raise ValueError(
                "Trade probe override evidence is unreadable; no input is authorized"
            ) from error
...
def archive_probe(state):
    ...
    def verify_and_sync():
        ...
        with path.open("r+b") as stream:
            if stream.read() != raw:
                raise ValueError(
                    "Trade probe archive differs from its historical receipt"
                )
            stream.flush()
            os.fsync(stream.fileno())
    ...
    try:
        with os.fdopen(fd, "wb") as out:
            if out.write(raw) != len(raw):
                raise OSError("Incomplete trade probe archive write")
            out.flush()
            os.fsync(out.fileno())
...
def recovery_available(ui):
    if (
        ...
        raise ValueError(
            "Wait for delivery input to finish before rechecking its evidence"
        )
...
def recovery_evidence(ui, state):
    ...
    for role, snapshot in (("farmer", farmer), ("merchant", merchant)):
        ...
        if (
            any(
                snapshot.get(key) != old.get(key)
                for key in ("character", "character_uid", "server")
            )
            or not 0 <= now - snapshot["timestamp"] <= 5
            or snapshot.get("hp", 0) <= 0
        ):
            raise ValueError(
                "Recheck requires fresh memory of the original named characters"
            )
    return {
        "observed_at": now,
        "farmer": farmer,
...
def recheck(ui):
    ...
    if not state or state.get("phase") not in RECOVERABLE:
        raise ValueError(
            "No known unfinished trade probe is available for operator review"
        )
...
def operator_override(
    ...
    *,
    ...
    if state and state.get("phase") == "operator_overridden":
        ...
        if (
            previous.get("confirmation_reference") == confirmation_reference
        ...
        raise ValueError(
            "Trade probe was already overridden with a different confirmation"
        )
    if (
        not state
        or state.get("phase") not in RECOVERABLE
    ...
    result = close(
        JOURNAL,
        pending_phases=RECOVERABLE,
        operator_confirmed=True,
        confirmation_reference=confirmation_reference,
        incident_digest=incident_digest,
        operator=operator,
        fresh_evidence=fresh,
        incident="delivery-request-probe",
    )
...
def selected_intent(farmer, merchant, uids):
    ...
    if (
        ...
        or len(uids) != 1
        or type(uids[0]) is not int
        or uids[0] <= 0
    ):
        raise ValueError(
            "Select exactly one carried +1 equipment or MeteorScroll UID for qualification"
        )
...
def unchanged(intent, farmer, merchant):
    ...
    for role, current in (("farmer", farmer), ("merchant", merchant)):
        ...
        if (
            ...
            or current["character_uid"] != old["character_uid"]
            or current["position"] != old["position"]
            or current["silver"] != old["silver"]
...
def prepared_resume(ui, state, character, uids, farmer, merchant):
    ...
    if (
        ...
        or state.get("selected_uids") != uids
        ...
        raise ValueError(
            "Existing probe is not an exact input-free preparation; reconcile before input"
        )
    intent = state["intent"]
    ...
    if targeting_state(ui.app.observer.adapter)["current"] != 16:
        raise ValueError(
            "Prepared probe requires native idle targeting before resuming"
        )
    return intent
...
def _start(ui, character, *, uids=None):
    ...
    intent = (
        prepared_resume(ui, old, character, uids, f, m)
        if resuming
    ...
    if not within_delivery_probe_range(f["position"], m["position"]):
        raise ValueError(
            "Approach the memory-identified merchant before the trade probe"
        )
...
def run(ui, intent, revision, state):
    ...
    with ExitStack() as stack:
        ...
        while True:
            ...
            try:
                stack.enter_context(
                    ui.coordinator.lease("Farmer", purpose="delivery_request_probe")
                )
                break
            except CaptureUnavailable as error:
                if (
                    str(error) != "Waiting for input owner"
                    or time.monotonic() >= acquire_deadline
                ):
                    raise
                time.sleep(0.03)
        ...
        presentation = present(
            ui,
            state,
            purpose="delivery_request_probe",
            revision=revision,
            deadline=deadline,
        )
        f, m = pair(ui, character)
        verify_stage_pair(intent, f, m, stage="open")
        presentation()
        ...
        if size != gui:
            raise ValueError("Trade probe requires matching native and GUI dimensions")
        ...
        while not targeting_state(observer.adapter)["targeting_trade"]:
            ...
            if time.monotonic() >= until:
                raise ValueError("Trade targeting transition not verified")
            time.sleep(0.03)
        ...
        def before_peer():
            ...
            fresh_recipient = recipient_record(
                observer, profile, m, farmer=f, targeting=True
            )
