...
def validate_receipt(state):
    ...
    if not isinstance(state,dict):raise ValueError('1078 trade receipt is unavailable')
    intent=state.get('intent') or {}
    farmer=state.get('farmer_after') or {};merchant=state.get('merchant_after') or {}
    before_farmer=intent.get('farmer') or {};before_merchant=intent.get('merchant') or {}
    if (state.get('phase')!='delivery_verified' or not state.get('verified_at')
            or not intent.get('items') or not state.get('accept_point') or not state.get('confirm_point')
            or any(s.get('client_sha256')!=CLIENT_SHA256_1078
            ...
            or not reconcile(intent,farmer,merchant,
                             now=max(farmer.get('timestamp',0),merchant.get('timestamp',0)))):
        raise ValueError('1078 trade qualification lacks a verified bilateral native receipt')
    return state
...
def validate_qualification(profile,capability,character):
    ...
    if role=='farmer' and profile.get('target_mode')!={'rva':TRADE_MODE_RVA,'value':TRADE_MODE_VALUE}:
...
def targeting_state(session):
    ...
    return {'rva':TRADE_MODE_RVA,'value':TRADE_MODE_VALUE,'current':mode,
...
def require_recipient_scale(session):
    ...
    if (zoom!=256 or percent!=100 or not 640<=width<=7680 or not 480<=height<=4320
            or read(address,16)!=raw):
        raise ValueError('1078 recipient targeting requires unchanged native 100 percent zoom')
    session.assert_identity()
...
def _request_control(driver,snapshot):
    ...
    gui=driver.memory.gui;s=gui.session
    model=gui.model(15,0x5E0148)
    request=snapshot.get('request')
    if (not isinstance(request,dict) or snapshot.get('trade') is not None
            or type(request.get('participant_uid')) is not int or request['participant_uid']<=0
            or not request.get('participant')
            or request.get('message')!=f"{request['participant']} wishes to trade with you."
    ...
    matches=[w for w in snapshot['windows'] if str(w.get('name','')).endswith('###Confirm')]
    if len(matches)!=1:raise ValueError('1078 incoming confirmation is absent or ambiguous')
...
def locate(driver,snapshot,mode):
    ...
    if mode=='native_trade_request':return _request_control(driver,snapshot)
    trade=snapshot.get('trade')
    if not trade:raise ValueError('1078 native Trade window is not open')
    w=window(snapshot,'Trade##TradeWindow')
    outer=gui.table(w,'##TradeWindowGrid')
    if len(outer['columns'])!=3:raise ValueError('1078 trade layout changed')
    if mode=='native_trade_drop':
        if trade['accepted'] or trade['other_accepted']:
            raise ValueError('1078 trade is already confirmed; no item placement allowed')
        ...
        if count>=20:raise ValueError('1078 trade grid is full')
        return w,cell(table,count),None,None
    if mode=='native_trade_confirm':
        if trade['accepted']:raise ValueError('1078 own trade was already confirmed')
        ...
        l,t,r,b=bar['outer'];point=(round((l+r)/2),round(b+13))
...
def hover(driver,snapshot,mode):
    ...
    if not label:raise ValueError('1078 trade drop target is not a button')
...
def native_foreground(driver,identity,*,activate=False):
    ...
    observer=driver.observer;target=driver.target
    observer.adapter.assert_identity()
    before=target.snapshot()
    if (observer.adapter.identity!=identity or before['pid']!=identity['pid']
    ...
    observer.adapter.assert_identity()
    if (after['pid']!=identity['pid'] or after['root_hwnd']!=target.hwnd
            or after['foreground']!=target.hwnd or after['minimized']):
