...
def observe(observer):
    ...
    if (
        ...
        or life.map_id != snapshot["map_id"]
        ...
        raise ValueError(
            "Open Booth cleanup requires a living Market Farmer with no trade"
        )
...
def _owned(snapshot):
    return {key: snapshot[key] for key in OWNED}


def cleanup(loop):
    """Native route hook; retain existing behavior for other client builds."""
    health = getattr(loop, "health", None)
    if health is None or health().get("expected_sha256") != CLIENT_SHA256_1078:
        return None
...
def _settle(record, after):
    ...
    record.update(
        ...
        after=after,
        outcome="closed_unchanged",
        cancel_verified=True,
    )
    _save(record)
    return {
        "closed_panel": "Open Booth###Confirm",
        "cancel_verified": True,
...
def cancel(trade, check=lambda: None):
    ...
    if visible not in (0, 1):
        raise ValueError("Invalid confirmation visibility")
    record = _load()
    if not visible and record.get("phase") not in ("prepared", "submitted"):
        return None
    ...
    with input_scope(), physical_coordinates():
        ...
        if record.get("phase") in ("prepared", "submitted"):
            if _owned(before) != _owned(record["before"]):
                raise ValueError(
                    "Open Booth cancellation ownership changed; reconciliation required"
                )
            if before["confirmation"] is None:
                return _settle(record, before)
            if record["phase"] == "submitted":
                raise ValueError(
                    "Open Booth cancellation already submitted; no repeat input"
                )
            ...
            if history:
                history[-1].pop("history", None)
            record = {
                "version": 1,
                "request_id": "open-booth-cancel-" + uuid.uuid4().hex,
                "phase": "prepared",
        ...
        def guard():
            ...
            if (
                _owned(fresh) != _owned(before)
                or fresh["confirmation"] != before["confirmation"]
                or control.locate(gui, fresh["confirmation"]) != (window, logical)
            ):
                raise ValueError(
                    "Open Booth ownership or control changed before cancellation"
                )
        ...
        try:
            trade.click(
                point,
                before_press=lambda: wait_hover_validation(guard, allowed),
            ...
            while True:
                ...
                if after["confirmation"] is None:
                    return _settle(record, after)
                if _owned(after) != _owned(before):
                    raise ValueError(
                        "Ownership changed while reconciling Open Booth cancellation"
                    )
                if time.monotonic() >= deadline:
                    raise ValueError(
                        "Open Booth cancellation remains uncertain; no replay"
                    )
                time.sleep(0.05)
