...
def describe(ui, farmer_driver):
    ...
    if not enabled(ui_character(ui)):
        blockers.append("farmer_transfers_off")
    rollout = rollout_enabled(ui_character(ui), policy=policy)
    if not rollout:
        blockers.append("delivery_rollout_disabled")
    if not farmer_qualified:
        blockers.append("farmer_trade_controls_unqualified")
    for character, state in merchants.items():
        if not state["credentials_present"]:
            blockers.append(f"{character}:credentials_missing")
        blockers.extend(
            f"{character}:{capability}"
            for capability in state["missing_qualifications"]
        )
        if not state["trading_enabled"]:
            blockers.append(f"{character}:trading_paused")
    ...
    return {
        "qualified": farmer_qualified,
        "qualification_scope": "farmer_trade_controls",
        "configured_prerequisites_met": not blockers,
        "blockers": blockers,
        "merchants": merchants,
        "delivery_enabled": rollout,
        "parity_verified": policy.get("parity_verified") is True,
