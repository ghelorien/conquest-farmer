...
def scene_stalls(observer):
    ...
    for attempt in range(6):
        try:
            return _scene_stalls(observer)
        except ValueError as error:
            if (
                str(error) != "Shop flag changed while reading"
                or attempt == 5
...
def _scene_stalls(observer):
    ...
    if len(set(objects)) != len(objects):
        raise ValueError("Shop flag scene membership is ambiguous")
    vtables = sample_fields(s, [(a, "u64") for a in objects])
    candidates = [
        a for a, v in zip(objects, vtables) if v == base + p.monster_vtable_rva
    ]
    labels = sample_fields(
        s,
        [
            f
            for a in candidates
            for f in ((a + p.name_offset, "utf8"), (a + 0x84, "u32"))
        ],
    )
    flags = [
        a
        for i, a in enumerate(candidates)
        if labels[i * 2] == "ShopFlag" or labels[i * 2 + 1] == 406
    ]
    fields = []
    for a in flags:
        fields.extend(
    ...
    for i, a in enumerate(flags):
        vtable, kind, model, name, position, uid, species, dx, dy = values[
            i * 9 : (i + 1) * 9
        ]
        if (
    ...
    if (
        sample_fields(s, [(a, "u64") for a, _ in trace]) != [v for _, v in trace]
...
def owned_booth(observer, snapshot):
    ...
    if (
        len(matches) != 1
        or matches[0]["name"] != observer.character
        or (matches[0]["type_id"], matches[0]["model"]) != (0, 406)
        or max(abs(a - b) for a, b in zip(matches[0]["position"], snapshot["position"]))
        > 8
    ):
...
def vacant_flags(observer, spec):
    ...
    if spec.get("occupancy_mode") == "scene_booth":
        return unoccupied_scene_flags(observer, spec)
    offset = spec.get("vacancy_offset")
    mask = spec.get("vacancy_mask")
    value = spec.get("vacancy_value")
    if (
        type(offset) is not int
        or not 0 <= offset <= 0x4000
        or offset % 4
    ...
    flags = scene_flags(observer)
    available = []
    for flag in flags:
        if (flag["type_id"], flag["model"]) != (0, 1086):
            continue
        ...
        if before & mask != value:
            continue
        current = next(
            (r for r in scene_flags(observer) if r["uid"] == flag["uid"]), None
        )
