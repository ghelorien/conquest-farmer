...
_MODAL_RENDER_SHA256 = (
    "97c9069d8c500a9ff90b474dfb34871a36ba8dda689f1b537319c83568fcba04"
)
...
def _window(windows, name, *, prefix=False):
    ...
    if len(matches) != 1:
        raise ValueError(
            f"Expected exactly one live {name} window; found {len(matches)}"
        )
    return matches[0]
...
def _table(gui, window, label):
    ...
    return {
        ...
        "outer": table["outer"],
        "clip": table["clip"],
        "window_ownership_verified": True,
...
def _modal_candidate(adapter, model, snapshot):
    ...
    if not uid or sum(item.get("uid") == uid for item in snapshot["inventory"]) != 1:
        raise ValueError(
            "Candidate selected item does not match exactly one observed inventory item"
        )
...
def collect(session, snapshot):
    ...
    if snapshot["trade"] is not None or snapshot["request"] is not None:
        return blocked(
            "merchant_modal_open",
            "Close the merchant trade/request before listing preflight",
        )
    if snapshot["map_id"] != 1036 or snapshot["hp"] <= 0:
        return blocked(
            "merchant_not_alive_in_market",
            "Listing preflight requires a living merchant in Market",
        )
    if not snapshot["own_booth_uid"]:
        return blocked(
            "owned_booth_missing",
            "Memory does not identify an owned booth; no booth target was inferred",
        )
    if not snapshot["booth_open"]:
        return blocked(
            "owned_booth_closed",
            "Open this merchant's owned booth panel; its native model is closed",
        )
    ...
    if not model_active or model_owner != snapshot["own_booth_uid"]:
        raise ValueError(
            "Live booth model does not identify this merchant's owned booth"
        )
    windows = gui.windows()
    ...
    for key, prefix, label in (
        ("inventory_grid", "Inventory/", "##ItemTable"),
        ...
        try:
            ...
            blocked(
                key + "_unavailable",
                "Native GUI table memory is unreadable; retry observation",
            )
        else:
            result[key] = evidence
    ...
    if modals:
        ...
        blocked(
            "price_modal_not_observed",
            "No live Add Item to Booth dialog; no price controls were inferred",
        )
    fresh = gui.windows()
    fresh_relevant = [
        w
    ...
    if (
        gui.viewport_size() != viewport
        or sorted(relevant, key=lambda w: w["address"])
        != sorted(fresh_relevant, key=lambda w: w["address"])
