...
class MerchantPresentation:
    ...
    def start(self):
        ...
        self.thread = threading.Thread(
            target=self.run, name="merchant-ui-data", daemon=True
        )
        self.thread.start()
    ...
    def collect(self):
        ...
        with runtime.journal.db() as db:
            events = [
                dict(r)
                for r in db.execute("SELECT * FROM events ORDER BY id DESC LIMIT 300")
            ]
        tables = {
            c: {
                key: runtime.journal.get(c, key, [])
                for key in ("comparisons", "deferred")
            }
            for c in statuses
        }
        ...
        sales = (
            previous["sales"]
            if previous and time.time() - previous["sales"]["at"] < 5
        ...
        self.latest = {
            ...
            "tables": tables,
            "sales": sales,
            "reporting": runtime.sales_worker.status(),
    ...
    def run(self):
        while not self.stop.is_set():
            try:
                ...
                pass  # Keep the last view; the UI marks it stale.
            self.stop.wait(1)
