...
def participants_unchanged(intent, farmer, merchant):
    for role, current in (("farmer", farmer), ("merchant", merchant)):
        ...
        if any(
            exact_items(current[k]) != exact_items(before[k])
            for k in ("inventory", "booth")
        ):
...
def cancelled(intent, farmer, merchant):
    participants_unchanged(intent, farmer, merchant)
    return not any(
        snapshot.get("trade") or snapshot.get("request")
...
def run(ui, state, *, output_path=JOURNAL, before_submit=None):
    ...
    if ui.runtime.enabled(character):
        raise ValueError("Pause merchant operations before cancelling the test request")
    if character in ui.calibrating:
        raise ValueError("Merchant already has a calibration active")
    ...
    try:
        with ui.coordinator.lease(character), physical_coordinates():
            ...
            w, accept = control(driver, m)
            point = (accept[0], accept[1] + 22)
            size = driver.target.snapshot()["client_size"]
            native = driver.memory.gui.viewport_size()
            point = tuple(round(v * p / g) for v, p, g in zip(point, size, native))

            def guard():
                check()
                a, b = pair(ui, character)
                unchanged(intent, a, b)
                if control(driver, b) != (w, accept):
                    raise ValueError("Request dialog moved")
                driver.memory.gui.assert_hovered(w, "Cancel")

            state.update(
                phase="cancel_submitted", cancel_point=point, submitted_at=time.time()
            )
