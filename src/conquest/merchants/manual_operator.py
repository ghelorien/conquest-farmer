...
def visitor_text(visitor):
    if not isinstance(visitor, dict):
        return "visitor identity unavailable"
    return (
        f"{visitor.get('visitor_name', '?')} · {visitor.get('visitor_server', '?')} · "
...
def status_text(target, row, *, farmer_status=None, intent=None, now=None):
    ...
    if target == "Farmer":
        ...
        if intent.get("paused"):
            saved += " (paused)"
        intent_line = (
            f"Saved farming intent: {saved}; manual approval does not change it."
        )
    else:
        intent_line = (
            f"Saved permissions: trading/repricing {'On' if intent.get('enabled') else 'Off'}, "
    ...
    retracted = (row.get("terminal") or {}).get(
        "disposition"
    ) == "manual_admission_retracted_bot_owned"
    modal = (
        "trade" if (row.get("terminal") or {}).get("trade_still_visible") else "request"
    )
    phase = (
        f"local manual admission retracted; game {modal} still visible"
        if retracted
        else str(row.get("phase") or "unknown").replace("_", " ")
    )
    scope = "released" if retracted else row.get("fence_scope") or "target"
    lines = [
        f"{phase.title()} · {visitor_text(row.get('visitor'))}",
        f"Input fence: {scope} · session {row.get('id', '?')}",
        intent_line,
    ]
    if row.get("phase") == "approval_pending" and row.get("request_state") == "pending":
        ...
        lines.append(
            (
                "Approval decision expires now"
                if seconds == 0
                else f"Approve or reject within {seconds} second(s)"
            )
            if seconds is not None
            else "Approval deadline unavailable"
        )
        binding = row.get("approval_binding") or {}
        lines.append(f"Exact request: {binding.get('request_id', 'unavailable')}")
    if row.get("rebaseline"):
        lines.append(
            "Post-override rebaseline: two fresh matching closed-window observations are required."
        )
    if row.get("reason"):
        lines.append("Blocker: " + str(row["reason"]))
    if target == "Farmer":
        observation = (farmer_status or {}).get("observation") or {}
        maps = observation.get("qualified_full_snapshot_maps")
        if maps:
            lines.append("Qualified full-session maps: " + ", ".join(map(str, maps)))
        for key, label in (
            ("reason", "Observation blocker"),
            ("decline_blocker", "Decline blocker"),
        ):
...
def action_state(row, *, now=None):
    pending = bool(
        ...
        and row.get("approval_binding")
    )
    seconds = decision_seconds(row, now=now) if pending else None
    if seconds == 0:
        pending = False
    attention = bool(
        row and (row.get("phase") == "needs_attention" or row.get("rebaseline"))
    )
