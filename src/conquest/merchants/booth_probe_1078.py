...
def _window(windows, name, *, prefix=False):
    matches = [
        win
        for win in windows
        if (win["name"].startswith(name) if prefix else win["name"] == name)
    ]
    if len(matches) != 1:
...
def _item(snapshot):
    ...
    if len(matches) != 1 or any(
        ...
        raise ValueError(
            "The exact manually observed BreastPlate fingerprint is required"
        )
    return matches[0]
...
def _validate_snapshot(snapshot, profile, expected):
    if (
        snapshot["identity"] != expected["expected_identity"]
        ...
        or snapshot["booth"]
        or snapshot["capacity"] != 40
        or not 0 <= time.time() - snapshot["timestamp"] <= 1
    ):
        raise ValueError(
            "Exact living merchant, empty owned booth, or closed-trade evidence changed"
        )
    _item(snapshot)
...
def _farmer_safe_market(ui, expected_target=None):
    ...
    terminal = (
        route.get("phase") in ("stopped", "completed", "failed")
        or route.get("phase") == "needs_attention"
        and route.get("event") == "failed"
    )
    if (
        not terminal
        or type(route_pid) is not int
        or process_alive(route_pid) is not False
    ):
    ...
    if (
        health.get("profile_id") != ui.runtime.manual_target("Farmer")
        ...
        raise CaptureUnavailable(
            "Farmer must be alive and stopped in Market before merchant focus"
        )
    return target
...
def _policy(
    ...
    *,
    ...
    if (
        ...
        or any(
            current_control.get(key) != control.get(key)
        ...
        raise CaptureUnavailable(
            "Booth probe stopped, expired, or lacks a safe farmer handoff"
        )
    ...
    if active(runtime.journal, character) or probe_busy(ui):
        raise CaptureUnavailable(
            "Another delivery or native diagnostic owns the merchant"
        )
...
def _farmer_journals_clear(runtime):
    ...
    from conquest.merchants.delivery_probe import (
        JOURNAL as probe_path,
        TERMINAL as probe_terminal,
    )
    from conquest.merchants.trade_qualification_prep import (
        JOURNAL as prep_path,
        TERMINAL as prep_terminal,
    )
    from conquest.meteor_banking import (
        JOURNAL as meteor_path,
        TERMINAL as meteor_terminal,
    )
    ...
    if runtime.farmer_bot_owned() or protected_pending() or delivery_pending():
        raise ValueError(
            "Farmer transaction holds must be reconciled before a booth probe"
        )
    for path, terminal in (
        (probe_path, probe_terminal),
        (prep_path, prep_terminal),
        (meteor_path, meteor_terminal),
        (route_path, None),
    ):
        if Path(str(path) + ".override-intent.json").exists():
            raise ValueError(
                "Farmer journal override recovery must finish before a booth probe"
            )
        if not path.exists():
            continue
        ...
        if (
            ...
            or state
            and (
                state.get("phase") not in terminal
                if terminal is not None
                else state.get("active")
            )
        ):
            raise ValueError(
                "An unfinished farmer transaction excludes the booth probe"
            )
...
def dispatch(ui, body):
    ...
    if set(body) != (
        common if action == "merchant-booth-probe-status-1078" else fields
    ):
        raise ValueError(
            "Booth probe requires an exact request, process and item binding"
        )
    request_id = body["request_id"]
    if not isinstance(request_id, str) or not re.fullmatch(
        r"booth1078-[A-Za-z0-9_-]{8,80}", request_id
    ):
    ...
    if (
        ...
        raise ValueError(
            "This first probe supports only the exact manually observed item"
        )
    ...
    if (
        ...
        or not preflight["layout_observed"]
    ...
    if (
        native["root_hwnd"] != hwnd
        or native["foreground"] != hwnd
        or native["minimized"]
    ):
        raise ValueError(
    ...
    before = {
        ...
        "observed": observed,
        "listing_submission_permitted": False,
    }
    ...
    try:
        ...
        journal.transition(
            request_id,
            "aborted",
            {"reason": "Worker did not start", "input_attempted": False},
        )
        raise
    return {
        "request_id": request_id,
...
def _run(ui, character, profile, before, token):
    ...
    try:
        ...
        with (
            ...
            def policy():
                ...
                if (
                    ...
                    or state["foreground"] != target.hwnd
                ):
                    raise CaptureUnavailable(
                        "Standalone merchant lost foreground ownership"
                    )
            ...
            with (
                ...
                coordinator.booth_probe_scope(character, policy),
            ):
                with coordinator.lease(character, purpose=KIND):
                    ...
                    def fresh(*, modal=None):
                        ...
                        if (
                            any(snapshot[key] != baseline[key] for key in STOCK_FIELDS)
                            ...
                            raise ValueError(
                                "Exact stock, owned model, or grid changed during probe"
                            )
                        present = any(
                    ...
                    def point(logical):
                        ...
                        if any(
                            not 1 < v < maximum - 2
                            for v, maximum in zip(result, revision.client_size)
                        ):
                            raise ValueError(
                                "Input candidate is outside the native client"
                            )
                        return result
                    ...
                    while not any(
                        ...
                        if time.monotonic() >= wait_until:
                            raise ValueError(
                                "Drag result not verified; no repeat is permitted"
                            )
                        time.sleep(0.03)
                    revision = layout.stable()
                    ...
                    def dialog_guard(label, *, require_text=None):
                        ...
                        if (
                            (win, points) != binding
                            or require_text is not None
                            and value != require_text
                        ):
                            raise ValueError(
                                "Exact price dialog binding or sentinel changed"
                            )
                    ...
                    while dialog_guard("##Amount") != "123,456":
                        if time.monotonic() >= until:
                            raise ValueError(
                                "Sentinel amount not verified; leaving the exact dialog held"
                            )
                        time.sleep(0.03)
                    journal.step(
                        request_id,
                    ...
                    while any(
                        ...
                        if time.monotonic() >= until:
                            raise ValueError(
                                "Cancellation not verified; no repeat is permitted"
                            )
                        time.sleep(0.03)
                    revision = layout.stable()
                    ...
                    if any(settled[key] != after[key] for key in STOCK_FIELDS):
                        raise ValueError(
                            "Ownership changed during cancellation settlement"
                        )
                    journal.transition(
                        request_id,
                        "verified",
        ...
        try:
            journal.transition(
                request_id, "uncertain" if attempted else "aborted", result
            )
            if attempted:
                journal.set(
                    character,
                    "attention",
                    {
                        ...
                        "note": "1078 no-submit booth probe needs manual reconciliation: "
                        + str(error),
                    },
                )
...
def _type_sentinel(target, guard, stage):
    ...
    for index, digit in enumerate("123456"):
        ...
        if any(keys(key) & 0x8000 for key in (0x10, 0x11, 0x12, 0x7B)):
            raise CaptureUnavailable(
                "A physical modifier or Stop key interrupted sentinel entry"
            )
        try:
            event = Input(
