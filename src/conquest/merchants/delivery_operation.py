...
def pending():
    ...
    try:
        with sqlite3.connect(
            JOURNAL.resolve().as_uri() + "?mode=ro", uri=True, timeout=2
        ) as db:
            return bool(
                db.execute(
        ...
        raise ValueError(
            "Farmer delivery journal is unreadable; reconcile before input"
        ) from error
...
def guard_protected_assets():
    ...
    if pending():
        raise ValueError(
            "Protected warehouse withdrawal needs inventory reconciliation before continuing"
        )
    from conquest.merchants.trade_qualification_prep import pending as prep_pending

    if prep_pending():
        raise ValueError(
            "Supervised trade preparation needs reconciliation before continuing"
        )
...
def clear_settled_attention(ui, key, receipt):
    ...
    overridden = (
        receipt.get("phase") == "operator_overridden"
        and receipt.get("outcome") == "operator_overridden"
    )
    settled = (
        receipt.get("phase") == "aborted" and receipt.get("outcome") == "no_transfer"
    )
    if (
        runtime is None
        or not (overridden or settled)
        or receipt.get("cleanup_pending")
        or receipt.get("next_action") not in ("retry_delivery", "release_route")
    ...
    with runtime.journal.db() as db:
        ...
        row = db.execute(
            "SELECT value FROM state WHERE character=? AND name='delivery_reservation'",
            (character,),
        ).fetchone()
        ...
        if not reservation or reservation.get("request_id") != key:
            return False
        if overridden:
            if reservation.get("phase") != "operator_overridden":
                return False
            if (reservation.get("operator_override") or {}).get(
                "confirmation_reference"
            ) != (receipt.get("operator_override") or {}).get("confirmation_reference"):
                return False
        elif (
            reservation.get("phase") != "no_transfer_reconciled"
            or (reservation.get("disposition") or {}).get("proof_digest")
        ...
        if attention:
            if (
                attention.get("kind") != "farmer_delivery"
                or attention.get("request_id") != key
            ):
                return False
            db.execute(
                "UPDATE state SET value='null' WHERE character=? AND name='attention' AND value=?",
                (character, row[0]),
            )
...
def status(journal, key):
    ...
    if not row:
        ...
        reason = (
            admission["reason"]
            or "Admission was interrupted before any delivery worker or input"
        )
            ...
            json.dumps(
                {
                    ...
                    "uids": json.loads(admission["uids_json"]),
                    "items": items,
                    "origin": origin,
        ...
        return {
            ...
            "town_visit_id": origin.get("town_visit_id"),
            "visit_id": origin.get("visit_id"),
            "farmer_profile_id": origin.get("farmer_profile_id"),
            "uids": json.loads(admission["uids_json"]),
            "items": items,
            "outcome": "retryable_before_input",
            "evidence_outcome": "admission_rejected",
            "reason": reason,
            ...
            "delivered": [],
            "remaining": items,
            "action_trace_initialized": False,
            "last_action_stage": None,
        }
    ...
    if row["phase"] == "verified":
        evidence_outcome = evidence_outcome or "delivered"
    elif row["phase"] == "aborted":
        evidence_outcome = evidence_outcome or "aborted"
    elif row["phase"] == "operator_overridden":
        evidence_outcome = evidence_outcome or "operator_overridden"
    outcome = {
        "delivered": "transferred",
        "no_transfer": "no_transfer",
        "not_started": "retryable_before_input",
        "partial_transfer": "deferred",
        "operator_overridden": "operator_overridden",
    }.get(evidence_outcome)
    if outcome is None:
        outcome = (
            "unresolved" if row["phase"] in ("submitted", "uncertain") else "deferred"
        )
    ...
    cleanup_verified = any(
        s["stage"] in ("cleanup", "cleanup_trade") and s["status"] == "observed"
        for s in trace
    )
    receiver_verified = any(
        s["stage"] == "receiver_receipt" and s["status"] == "observed" for s in trace
    )
    if (
        row["phase"] in ("verified", "aborted", "operator_overridden")
        and cleanup_pending
        ...
        s["stage"] == "action_trace" and s["status"] == "initialized" for s in trace
    ):
    ...
    return {
        ...
        "town_visit_id": before.get("town_visit_id"),
        "visit_id": before.get("visit_id"),
        ...
        "outcome": outcome,
        "evidence_outcome": evidence_outcome or "unknown",
        "reason": reason,
        "next_action": next_action,
        "proof_digest": result.get("proof_digest"),
        "operator_override": result.get("operator_override")
        ...
        "action_trace_initialized": any(
            s["stage"] == "action_trace" and s["status"] == "initialized" for s in trace
        ),
        "last_action_stage": next(
            (s["stage"] for s in reversed(trace) if s["status"] == "before_action"),
            None,
        ),
    }
...
def operator_override(
    ...
    *,
    ...
    if not old:
        raise ValueError("Unknown farmer delivery")
    ...
    if (
        old["phase"] in ("verified", "aborted")
        and old["outcome"] != "operator_overridden"
    ):
    ...
    try:
        ...
        fresh = {
            ...
            "reason": "Fresh participant memory unavailable",
        }
    ...
    reservations.operator_override(
        ui.runtime.journal,
        character,
        key,
        operator_confirmed=operator_confirmed,
        ...
        operator=operator,
        fresh_evidence=fresh,
        cleanup_pending=source_receipt.get("cleanup_pending")
        if source_receipt
        else None,
    ...
    return {
        ...
        "receipt": receipt,
        "rechecked": fresh,
        "reservation_phase": ui.runtime.journal.get(
            character, "delivery_reservation", {}
        ).get("phase"),
    }
...
def repair_operator_link(ui, journal, key, receipt):
    ...
    if not receipt or receipt.get("phase") != "operator_overridden":
        return receipt
    character = receipt["character"]
    reservation = ui.runtime.journal.get(character, "delivery_reservation", {}) or {}
    if (
        reservation.get("request_id") == key
        and reservation.get("phase") == "operator_overridden"
    ):
        return receipt
    ...
    reservations.operator_override(
        ...
        confirmation_reference=override.get("confirmation_reference"),
        operator=override.get("operator"),
        fresh_evidence=override.get("fresh_evidence"),
        cleanup_pending=receipt.get("cleanup_pending"),
        intent=saved_intent(journal, key),
    )
    return receipt
...
def saved_intent(journal, key):
    with journal.db() as db:
        row = db.execute(
            "SELECT before_json FROM transactions WHERE id=?", (key,)
        ).fetchone()
    if not row:
        raise ValueError("Unknown farmer delivery")
...
def prepare_new(ui, journal, key, character, uids, action, origin):
    ...
    try:
        ...
        trial = action == "delivery-start" and trial_delivery_permitted(
            ui, key, character, uids, origin
        )
        if (
            action != "delivery-test"
            and not trial
            and not rollout_enabled(ui_character(ui), policy=policy)
        ):
        ...
        driver.check()
        if not ui.runtime.enabled(character):
            raise ValueError("Merchant trading is paused")
        receiver = ui.runtime.controllers.get(character)
        if receiver is None:
            raise ValueError("Delivery recipient is not connected")
        ...
        if trial:
            ...
            if (
                not trial_delivery_permitted(ui, key, character, uids, origin)
                or merchant["identity"]
                != state()["merchants"][str(character)]["identity"]
                or merchant["character_uid"]
                != state()["merchants"][str(character)]["character_uid"]
            ):
                raise ValueError(
                    "Acceptance merchant or trial permission changed before delivery admission"
                )
        items = [item for item in farmer["inventory"] if item["uid"] in uids]
        if len(items) != len(uids):
            raise ValueError("Selected delivery items are not carried")
        journal.update_delivery_admission(key, "admitted", items=items)
        intent = prepare(farmer, merchant, items)
        intent.update(origin)
        if not journal.begin(key, character, "farmer_delivery", intent, admission=True):
            raise ValueError(
                "Delivery is already prepared; poll its status before recovery"
            )
        ...
        if status(journal, key) and not journal.pending(character):
            journal.update_delivery_admission(
                ...
                reason=(
                    ...
                    else "Delivery admission failed"
                ),
            )
        raise
...
def dispatch(ui, body):
    ...
    valid = (
        set(body) == expected
        or action in ("delivery-start", "delivery-test")
    ...
    if action == "delivery-override":
        allowed = (
            ...
            | ({"incident_digest"} if "incident_digest" in body else set())
        )
        if set(body) != allowed:
            raise ValueError("Unsupported operator override arguments")
    if action == "delivery-recheck":
        if set(body) != {"action", "request_id"}:
            raise ValueError("Unsupported delivery recheck arguments")
    if (
        action
        not in (
            "delivery-start",
        ...
        or not valid
        and action not in ("delivery-recheck", "delivery-override")
    ):
    ...
    if action == "delivery-status":
        ...
        return {
            ...
            "running": running,
            "receipt": old,
            "error": ui.delivery_errors.get(key),
        }
    if action in ("delivery-recheck", "delivery-override"):
        ...
        if running:
            raise ValueError(
                "Wait for the delivery worker before rechecking this incident"
            )
        if action == "delivery-recheck":
            from conquest.merchants.delivery_bridge import pair
            ...
            fresh = {
                "observed_at": max(
                    farmer.get("timestamp", 0), merchant.get("timestamp", 0)
                ),
                "farmer": farmer,
                "merchant": merchant,
            }
            ...
            journal.step(
                ...
                "operator_recheck",
                "observed",
                {"evidence_digest": digest, "observed_at": fresh["observed_at"]},
            )
            return {
                "request_id": key,
                "running": False,
        ...
        return operator_override(
            ...
            incident_digest=body.get("incident_digest"),
        )
    if action in ("delivery-reconcile", "delivery-cleanup"):
        if old is None:
            raise ValueError("Unknown farmer delivery")
        if running:
            return {"request_id": key, "running": True, "receipt": old}
        ...
        if action == "delivery-reconcile" and (
            old["phase"] == "aborted"
    ...
    if (
        old
        and action != "delivery-reconcile"
        and (old["character"] != character or sorted(old["uids"]) != sorted(uids))
    ):
        raise ValueError("Delivery request ID reused for another batch")
    if old and action in ("delivery-start", "delivery-test"):
        raise ValueError(
            "Delivery already exists; use delivery-reconcile for read-only recovery"
        )
    if running:
        return {"request_id": key, "running": True, "receipt": old}
    if not old:
        ...
        origin = {
            "operation_id": key,
            "town_visit_id": grant.get("town_visit_id"),
            "visit_id": grant.get("visit_id"),
            "farmer_profile_id": grant.get("farmer_profile_id")
            or (context.profile.id if context else farmer_name()),
        }
        requested = body.get("items")
        if requested is not None:
            if not isinstance(requested, list) or sorted(
                exact_items(requested)
            ) != sorted(uids):
                raise ValueError(
                    "Delivery item fingerprints do not match requested UIDs"
                )
        ...
        try:
            ...
            driver, intent = prepare_new(
                ui, journal, key, character, uids, action, origin
            )
        finally:
    ...
    if action == "delivery-cleanup" and (
        token is None or token.request_id is None or token.scope != "market_visit"
    ):
    ...
    if action == "delivery-cleanup":
        ...
        if (
            token.request_id != key
            or token.farmer_profile_id != cleanup_origin.get("farmer_profile_id")
            or (getattr(ui, "grant", None) or {}).get("visit_id")
            != cleanup_origin.get("visit_id")
            or (getattr(ui, "grant", None) or {}).get("town_visit_id")
            != cleanup_origin.get("town_visit_id")
        ):
            raise ValueError(
                "Delivery cleanup grant does not match the saved operation origin"
            )

    def work():
        binding = (
            fence.bind_worker(
                token, action_capable=action == "delivery-cleanup" or not bool(old)
            )
        ...
        try:
            with binding:
                ...
                options = {
                    "sale_receipts": lambda intent, farmer, merchant: (
                        qualified_delivery_receipts(
                            ui.runtime.journal, intent, merchant
                        )
                    )
                }
                if fence is not None:
                    options["mark_read_only"] = fence.mark_read_only
                transaction = DeliveryTransaction(journal, driver, **options)
                if action == "delivery-cleanup":
                    transaction.key = key
                    cleanup_intent = saved_intent(journal, key)
                    from conquest.merchants.delivery_bridge import pair
                    ...
                    result = reconciliation_outcome(
                        ...
                        farmer,
                        merchant,
                        trace=transaction.trace(key),
                        sale_receipts=sales,
                    )
                    if (
                        result["outcome"] != old["evidence_outcome"]
                        or not result["cleanup_pending"]
                    ):
                        raise ReconciliationBlocked(
                            "Terminal delivery cleanup evidence changed"
                        )
                    ...
                    cleanup_empty_trade(
                        ...
                        check=ui.coordinator.check,
                    )
                    if fence is not None:
                        fence.mark_read_only()
                    if old["phase"] == "verified":
                        transaction.finish_receiver(key, cleanup_intent)
                elif old:
                    transaction.recover(key)
                else:
                    transaction.execute(key, character, intent)
                receipt = status(journal, key)
                clear_settled_attention(ui, key, receipt)
                if hasattr(driver, "report"):
                    ...
                    activity = (
                        "Transfer to "
                        + character
                        + " verified; ready to continue the route"
                        if ready and receipt["outcome"] == "transferred"
                        else "Transfer to "
                        + character
                        + " reconciled as "
                        + receipt["outcome"].replace("_", " ")
                        + "; next action: "
                        + receipt["next_action"].replace("_", " ")
                    )
                ...
                if receipt["next_action"] == "release_route":
                    ...
                    if (
                        attention.get("kind") == "farmer_delivery"
                        and attention.get("request_id") == key
                    ):
            ...
            current = status(journal, key)
            if (
                isinstance(error, CaptureUnavailable)
                and current
                and current["phase"] == "prepared"
                and not current["action_trace_initialized"]
            ):
                current = reject_worker_admission(journal, key)
            note = (
                str(error)
                if isinstance(error, (ValueError, OSError))
                else "Native delivery failed; reconcile before retrying"
            )
            ...
            if current and current["outcome"] == "retryable_before_input":
                if hasattr(driver, "report"):
                    driver.report(
                        "Transfer to "
                        + character
                        + " did not start; it is safe to retry",
                        "attention",
                    )
                ui.runtime.journal.event(
                    character,
            ...
            if hasattr(driver, "report"):
                driver.report(
                    ...
                    + " paused: "
                    + note
                    + ". Checking the saved transaction before continuing.",
                    "attention",
                )
            if not (
                isinstance(error, CaptureUnavailable)
                and current
                and current["phase"] == "aborted"
            ):
                ui.runtime.journal.set(
                    character,
                    "attention",
                    {"kind": "farmer_delivery", "request_id": key, "note": note},
                )
            ui.runtime.journal.event(
                character, "delivery_needs_reconciliation", request_id=key, note=note
            )

    worker = threading.Thread(target=work, daemon=True, name="farmer-delivery")
    ui.delivery_workers[key] = worker
    worker.start()
