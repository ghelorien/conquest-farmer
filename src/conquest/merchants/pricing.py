...
def comparison_key(key):
    ...
    return (
        replace(key, quality="Composition")
        if key.plus > 0 and key.quality in COMPOSITION_QUALITIES
        else key
    )
...
def historical_quote(history, key):
    matches = [
        v for k, v in history.items() if comparison_key(k) == comparison_key(key)
    ]
    if not matches:
        return None
    latest = max(v["observed_at"] for v in matches)
    return min(
        (v for v in matches if v["observed_at"] == latest),
        key=lambda v: Fraction(v["unit_price"]),
    )
...
def parse_booth_price(raw):
    ...
    if not isinstance(raw, bytes) or not re.fullmatch(
        rb"(?:[0-9]+|[1-9][0-9]{0,2}(?:,[0-9]{3})+)", raw
    ):
...
def wait_booth_price(
    read, expected, check, *, clock=time.monotonic, sleep=time.sleep, timeout=2
):
    ...
    while True:
        ...
        if clock() >= deadline:
            raise ValueError(
                "Entered booth price was not verified before timeout; no listing submitted"
            )
        sleep(0.02)
...
class ItemKey:
    ...
    def __post_init__(self):
        if (
            not self.category
            or not self.quality
            or type(self.plus) is not int
            or not 0 <= self.plus <= 12
        ):
            raise ValueError("Incomplete comparison attributes")
        if len(self.sockets) != 2 or any(
            not isinstance(s, str) or not s for s in self.sockets
        ):
...
class Listing:
    ...
    def __post_init__(self):
        if (
            not self.seller
            or type(self.price) is not int
            or type(self.quantity) is not int
            or min(self.price, self.quantity) <= 0
        ):
...
def price_item(key, listings, *, quantity=1):
    ...
    rejected = tuple(
        sorted(
            name
            for name, price in sellers.items()
            if len(sellers) >= 4
            and price < median([p for other, p in sellers.items() if other != name]) / 2
        )
    )
    ...
    if price < 1 or price > MAX_BOOTH_PRICE:
        return Decision(
            None, "Target is outside the supported silver range", len(sellers), rejected
        )
...
def quote_item(key, listings, *, quantity=1, history=None, allow_plus_conversion=True):
    ...
    if any(
        comparison_key(r.key) == comparison_key(key) and r.server == SERVER
        for r in rows
    ):
    ...
    if saved:
        ...
        if base.reference is not None:
            ...
            observed_at = None
        elif saved and not any(
            comparison_key(r.key) == comparison_key(base_key) and r.server == SERVER
            for r in rows
        ):
    ...
    return Decision(
        price, reason, 0, reference=str(reference), source_observed_at=observed_at
    )
...
def quality(kind):
    return {0: "Fixed", 6: "Refined", 7: "Unique", 8: "Elite", 9: "Super"}.get(
        kind % 10, "Normal"
    )
