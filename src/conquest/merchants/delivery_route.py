...
def pending():
    return bool(read_json(STATE).get("active"))
...
def receipt_for(uid, type_id):
    return next(
        (
            r
            for r in read_json(STATE).get("receipts", [])
            if any(i["uid"] == uid and i["type_id"] == type_id for i in r["items"])
        ),
        None,
    )
...
def warehouse_exhausted(loop, stored, remaining, *, send=request):
    ...
    if not rollout_enabled(route_character(loop), policy=policy):
        return True
    ...
    try:
        ...
        for name in CHARACTERS:
            if not states.get(name, {}).get("ready"):
                continue
            try:
                ...
                if (
                    not merchant.get("booth_open")
                    or merchant.get("trade")
                    or merchant.get("request")
                    or farmer.get("trade")
                    or farmer.get("request")
                    or available_slots(merchant) <= 0
                ):
                    continue
                path = loop.terrain.travel_path(
                    tuple(farmer["position"]), tuple(merchant["position"])
                )
                if (
                    path
                    and tuple(path[0]) == tuple(farmer["position"])
...
def candidates(loop, send, *, excluded=(), items=None):
    ...
    for name, state in status.get("characters", {}).items():
        if not state.get("ready") or name in excluded:
            continue
        try:
            pair = send({"action": "delivery-pair", "character": name})
            ...
            if not merchant_allowed(name, state, m):
                continue
            if f["map_id"] != 1036 or m["map_id"] != 1036:
                continue
    ...
    if items is not None:
        ...
        wanted = exact_items(items)
        current = exact_items(farmer["inventory"])
        if any(current.get(uid) != value for uid, value in wanted.items()):
            raise ValueError(
                "Exact requested delivery items changed before merchant planning"
            )
    ...
    for plan in plans:
        plan["position"] = by_name[plan["merchant"]]["position"]
    return plans
...
def settle(loop, send, state, *, start=False):
    ...
    if start:
        ...
        if result.get("receipt") is None:
            raise ValueError(
                "Delivery submission has no receipt; reconcile before any storage input"
            )
    ...
    while True:
        ...
        if not result.get("running"):
            if (
                ...
                or receipt.get("character") != active["merchant"]
            ...
            if exact_items(receipt.get("items", [])) != exact_items(active["items"]):
                raise ValueError(
                    "Merchant delivery item evidence changed; reconciliation required"
                )
            if any(
                active.get(field) is not None and receipt.get(field) != active[field]
                for field in ("visit_id", "town_visit_id", "farmer_profile_id")
            ):
                raise ValueError(
                    "Merchant delivery visit provenance changed; reconciliation required"
                )
            ...
            if action == "cleanup_trade_modal" and not cleanup_attempted:
                ...
                continue
            if (
                action in ("finalize_receiver_receipt", "reconcile_bilateral_ownership")
                and not attempted_reconcile
            ):
            ...
            if (
                receipt.get("phase")
                not in ("verified", "aborted", "operator_overridden")
                or receipt.get("cleanup_pending")
                or action
                not in ("release_route", "retry_delivery", "replan_remaining_delivery")
            ):
                raise ValueError(
                    "Merchant delivery needs reconciliation; valuables remain protected"
                )
            ...
            record = {
                **active,
                "items": delivered,
                "remaining": receipt.get("remaining") or [],
                "outcome": outcome,
                "proof_digest": receipt.get("proof_digest"),
                "next_action": action,
                "verified_at": time.time(),
            }
            state.setdefault("operations", []).append(record)
            if delivered:
                state.setdefault("receipts", []).append(record)
            state["active"] = None
            ...
            loop.record(
                ...
                if delivered
                else "merchant_delivery_deferred",
                request_id=key,
                merchant=active["merchant"],
                items=delivered,
                outcome=outcome,
                activity="Valuables delivered and verified in both inventories"
                if delivered
                else "Delivery incident closed; checking the next safe destination",
            )
            return record
        if time.monotonic() >= until:
            raise ValueError(
                "Merchant delivery still running; reconcile before resuming storage"
            )
        time.sleep(0.1)
...
def refill_remainder(loop, send, key, deadline, proof, revision):
    ...
    check_stop(loop)
    health = loop.health()
    if not resumable(health, proof, revision) or not clear_observation(health):
        return False
    result = send({"action": "delivery-refill", "request_id": key})
    if not result.get("refill"):
        return False
    ...
    while time.time() < deadline:
        ...
        if not any(
            s.get("refill", {}).get("enabled") and s["refill"].get("pending")
...
def approach_merchant(loop, plan, send, *, deadline=None):
    ...
    if deadline is not None:
        correction_deadline = min(correction_deadline, deadline - 15)
    ...
    for observation in range(48):
        ...
        cleanup(loop)
        if time.time() >= correction_deadline:
            return False
        ...
        if probe.get("reason") == "recipient_scene_changed":
            ...
            time.sleep(min(0.1, max(0, correction_deadline - time.time())))
            continue
        if probe.get("reason") == "recipient_absent":
            ...
            candidates = positions(
                loop.terrain, probe, used=used, deadline=correction_deadline
            )
        if not candidates:
            return False
        ...
        for candidate in candidates:
            if time.time() >= correction_deadline:
                return False
            possible = bounded_position(
                loop.terrain, probe, candidate, used=used, deadline=correction_deadline
            )
        ...
        try:
            loop.travel(
                target,
                arrival_radius=0 if short_leg else 2,
                avoid=occupied,
...
def market_storage(loop, *, send=request, items=None, on_admitted=None):
    previous = getattr(loop, "market_service_deadline", None)
    try:
        return _market_storage(loop, send=send, items=items, on_admitted=on_admitted)
    finally:
        loop.market_service_deadline = previous
    ...
    while True:
        ...
        if (
            ...
            raise OvernightStopped(
                "Farmer control or manual input changed before merchant delivery"
            )
        target = health.get("target")
        if not isinstance(target, dict) or not target.get("pid"):
            raise OvernightStopped(
                "Farmer identity unavailable before merchant delivery"
            )
        if identity is None:
            ...
            raise OvernightStopped(
                "Farmer identity or control revision changed before merchant delivery"
            )
        ...
        if attempts == 1:
            first_age = age
        if not data.get("observations_available"):
            reason = "observation_unavailable"
        elif not life:
            reason = "life_unavailable"
        elif age is None or not 0 <= age <= 1:
            reason = "observation_stale"
        elif not isinstance(data.get("monsters"), list):
            reason = "monster_observation_unavailable"
        else:
            try:
                safe = clear_observation(health)
            except (KeyError, TypeError, ValueError):
                reason = "observation_incomplete"
            else:
                if safe and time.time() < deadline:
                    return health
                if not safe:
                    # A fresh known threat is not merely a read gap. Preserve
                    # the original stop instead of authorizing other input.
                    raise ValueError(
                        "Merchant delivery stopped for unsafe Farmer observation"
                    )
        ...
        if remaining <= 0:
            ...
            loop.record(
                ...
                first_observation_age=first_age,
                last_observation_age=age,
...
def _market_storage(loop, *, send=request, items=None, on_admitted=None):
    ...
    def plans_for(excluded=()):
        return candidates(loop, send, excluded=excluded, items=selected)

    if state.get("cleanup_pending"):
        raise ValueError(
            "Reconciled partial delivery still needs its empty trade window closed"
        )
    ...
    if state.get("active"):
        remaining(settle(loop, send, state))
    else:
        from conquest.merchants.open_booth_cancel_1078 import cleanup
    ...
    if (
        not rollout_enabled(route_character(loop), policy=policy)
        and not trial_permitted(loop)
        or not transfers_enabled(route_character(loop))
    ):
        return []
    ...
    for _ in range(40):
        if not transfers_enabled(route_character(loop)):
            return receipts
        if time.time() >= deadline:
            loop.record(
                "merchant_service_deferred",
        ...
        for window in ("Dialog", "Inventory"):
            try:
                loop.town("service-close-panel", window=window)
            except ValueError as error:
                if not any(note in str(error) for note in ("not active", "absent")):
                    raise
        if not approach_merchant(loop, plan, send, deadline=deadline):
            MarketVisit().attempt(
                plan["merchant"], plan["position"], "deferred_before_input"
            )
        ...
        current = fresh[0]
        if (
            current["merchant"] != plan["merchant"]
            or current["position"] != plan["position"]
        ...
        windows = WorkWindows()
        if not windows.reserve(key, town=True, visit=visit):
            return receipts
        ...
        try:
            ...
            state["active"] = {
                ...
                "controller": {
                    "pid": loop.state["pid"],
                    "started_at": loop.state["started_at"],
                },
                "visit_id": visit["visit_id"],
            ...
            if on_admitted is not None:
                on_admitted(dict(state["active"]))
            result = settle(loop, send, state, start=True)
            MarketVisit().attempt(
                current["merchant"], current["position"], result["outcome"]
            )
            if result["items"]:
                receipts.append(result)
            remaining(result)
            if result["outcome"] != "transferred":
                deferred_merchants.add(current["merchant"])
            ...
            if release_required:
                ...
                while not send({"action": "handoff-release", "request_id": key}).get(
                    "released"
                ):
                    if time.monotonic() >= until:
                        raise ValueError(
                            "Merchant input has not released; farmer remains stopped"
                        )
                    time.sleep(0.1)
            check_stop(loop)
        from conquest.merchants.handoff import resumable
        ...
        after = loop.health()
        if not resumable(
            after, {"target": health.get("target")}, control["revision"]
