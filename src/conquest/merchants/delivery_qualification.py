...
def promote(
    ...
    *,
    ...
    state = read_json(receipt_path)
    candidate = read_json(candidate_path)
    intent = state.get("intent", {})
    farmer = state.get("farmer_after", {})
    ...
    if build == CLIENT_SHA256_1078 and any(
        ...
        raise ValueError(
            "1078 qualification requires exact-build canonical observations throughout the exchange"
        )
    if (
        farmer["identity"] != intent["farmer"]["identity"]
        or merchant["identity"] != intent["merchant"]["identity"]
    ):
        raise ValueError(
            "Input qualification requires the same client processes throughout the exchange"
        )
    if (
        state.get("recipient", {}).get("uid") != intent["merchant"]["character_uid"]
        ...
        raise ValueError(
            "Live request, placement and confirmation evidence is incomplete"
        )
    windows = {w["name"]: w for w in farmer["windows"]}
    inventory = windows["Inventory/##ItemGrid_A800F95C"]
    hud = windows["##Control"]
    controls = {
        "start_trade": dict(
            window="##Control",
            size=list(hud["geometry"][2:]),
        ...
        "inventory_item": dict(
            ...
            size=list(inventory["geometry"][2:]),
            offset=[20, 20],
            table="##ItemTable",
            columns=10,
            stride=[40, 40],
        ...
        "confirm_trade": dict(
            window="Trade##TradeWindow",
            mode="native_trade_confirm",
            label="Accept Trade",
        ),
    }
    profile = dict(
        ...
        gui_size=[
            int(hud["geometry"][0] * 2 + hud["geometry"][2]),
            int(hud["geometry"][1] + hud["geometry"][3]),
        ],
    ...
    if build == CLIENT_SHA256_1078 and (
        ...
        if previous:
            peer["previous_build_qualification"] = previous
    if (
        peer.get("character") != merchant["character"]
    ...
    evidence = str(Path(receipt_path))
    if evidence not in peer.setdefault("trade_evidence", []):
        peer["trade_evidence"].append(evidence)
    if chain_evidence is not None:
    ...
    if check is not None:
        check()
    write_json(farmer_path, profile)
    write_json(merchant_path, peer)
    return {
