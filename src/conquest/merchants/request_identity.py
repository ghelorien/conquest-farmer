...
def participant_uid(observer, name):
    ...
    entries = (
        session.read_block(checked_address(begin), end - begin) if end > begin else b""
    )
    pointers = [
    ...
    def matching_actors():
        ...
        for address in pointers:
            ...
            if struct.unpack_from("<Q", raw)[0] == base + 0x5C5E20 and raw[
                0xA4:0xC4
            ].split(b"\0")[0] == name.encode("utf-8"):
    ...
    if (
        sample_fields(session, fields) != header
