...
def _proof(before, steps, first, second):
    ...
    if (
        before.get("client_sha256") != CLIENT_SHA256_1078
        ...
        or not before.get("profile_id")
        or not before.get("farmer_target")
    ...
    if (
        dialog.get("renderer_sha256") != _MODAL_RENDER_SHA256
...
def settle(journal, request_id, first, second):
    ...
    with journal.db() as db:
        ...
        if (
            not row
            or row["kind"] != KIND
            or row["phase"] not in ("prepared", "submitted", "uncertain")
        ):
        ...
        db.execute(
            "UPDATE transactions SET phase='verified',result_json=?,updated=? WHERE id=?",
        ...
        db.execute(
            ...
            (
                row["character"],
                KIND + "_verified",
                json.dumps({"transaction_id": request_id, "result": result}),
                now,
            ),
        )
    return capability
...
def require(journal, character, snapshot):
    ...
    with journal.db() as db:
        saved = db.execute(
            "SELECT value FROM state WHERE character=? AND name=?", (character, STATE)
        ).fetchone()
        if not saved:
            raise ValueError("listing_live_receipt_missing")
        saved = json.loads(saved[0])
        row = db.execute(
            "SELECT * FROM transactions WHERE id=?", (saved["request_id"],)
        ).fetchone()
        if (
            not row
            or row["kind"] != KIND
        ...
        validated = _proof(
            json.loads(row["before_json"]), steps, proof["first"], proof["second"]
        )
    ...
    if (
        ...
        or profile.name != proof["second"]["character"]
        or profile.server != "America"
    ...
    return {
        **saved,
        "qualified_own_booth_uid": proof["own_booth_uid"],
...
def bind_current(journal, character, snapshot, request):
    ...
    with journal.db() as db:
        if db.execute(
            "SELECT 1 FROM transactions WHERE id=?", (request["request_id"],)
        ).fetchone():
            raise ValueError(
                "Existing listing request cannot be rebound to another booth"
            )
    evidence = require(journal, character, snapshot)
    preflight = snapshot.get("listing_preflight") or {}
    owned = preflight.get("owned_booth") or {}
    if (
        snapshot.get("profile_id") != evidence["profile_id"]
        ...
        raise ValueError(
            "Current owned booth model and grids require fresh verified preflight"
        )
    _validate_snapshot(
        {**snapshot, "trade": None, "request": None}, _profile(character), request
    )
    return {
        "version": 1,
...
def status(journal, character, snapshot):
    result = {
        ...
        "automatic_farmer_handoff": False,
    }
    try:
        if snapshot is None:
