...
def _safe_health(ui, control, expected_target=None, *, grant_check=lambda: None):
    ...
    def permission():
        ...
        if (
            ui.closed
            or ui.app.closing
            or ui.runtime.stop_event.is_set()
            ...
            raise CaptureUnavailable(
                "Farmer handoff rejected: Stop, manual input or control revision changed"
            )
        grant_check()
    ...
    while True:
        ...
        if (
            ...
            raise CaptureUnavailable(
                "Farmer handoff rejected: current attached worker is unavailable"
            )
        health = bridge.health()
        if (
            ui.app.observer is not observer
            or observer.bridge is not bridge
            or not getattr(ui.app.attachment, "attached", False)
        ):
        ...
        reason = (
            ...
            if age > 1
            else "observation_shape_invalid"
        )
        if not gap or time.monotonic() >= deadline:
            raise CaptureUnavailable(
                "Farmer handoff rejected: "
                + reason
...
def _preview(ui, body):
    ...
    if _row(ui.runtime.journal, body["request_id"]):
        raise ValueError(
            "Listing request already exists; reconcile it or request exact cancellation"
        )
    if not isinstance(body["item_fingerprint"], dict) or set(
        body["item_fingerprint"]
    ) != set(ITEM_FIELDS):
...
def request_operator(ui, body):
    ...
    if set(body) not in (common, fields) or not re.fullmatch(
        ...
        raise ValueError(
            "An exact listing request or unresolved cancellation request is required"
        )
    ...
    if (
        ...
        or ui.runtime.handoff
        or ui.runtime.stop_event.is_set()
        or ui.coordinator.stopped
        or ui.runtime.manual_handoff_status() is not None
    ):
        raise ValueError(
            "Release other handoffs and preserve Stop before requesting listing input"
        )
    request = dict(body)
    ...
    record = {
        "request_id": key,
        "listing_request_id": body["request_id"],
    ...
    return {
        ...
        "listing_request_id": body["request_id"],
        "mode": mode,
        "admission_expires_at": record["expires_at"],
        "max_seconds": 45,
    }
...
def validate_grant(ui, body):
    ...
    if (
        ...
        raise ValueError(
            "Listing handoff is blocked by Stop, manual control or another owner"
        )
    from conquest.merchants.observe_1078 import observe
    ...
    if key.startswith(f"merchant-refill:{character}:"):
        ...
        from conquest.merchants.owned_booth_panel_1078 import (
            pending,
            validate_grant as panel_grant,
        )
        ...
        if (
            ...
            raise ValueError(
                "Refill requires a living merchant with the exact open Market booth"
            )
        ...
        if ui.runtime.refills[character].state().get("listing1078_request"):
            ...
            authority.update(
                mode="refill_cancel", listing_request_id=request["request_id"]
            )
    else:
        authority = ui.runtime.journal.get(character, AUTHORITY) or {}
        if (
            authority.get("request_id") != key
            or authority.get("profile_id") != profile.id
        ...
        if authority["mode"] == "list":
            ...
            before = _pending(
                ui.runtime.journal, authority["listing_request_id"], character
            )
            snapshot = observe(ui.runtime, character)
            if not snapshot["closed_modal"]:
                raise ValueError(
                    "Trade/request windows must be closed for exact cancellation"
                )
...
def scope_allows(
    ui, character, *, request_id=None, scheduled=False, cleanup=False, body=None
):
    ...
    return (
        not scheduled
        and cleanup == (authority.get("mode") == "cancel")
...
def farmer_safe(ui, expected_target=None, *, deadline=None):
    ...
    if not grant:
        if ui.coordinator.purpose in (
            ...
            if not ui.safe_to_yield():
                raise CaptureUnavailable(
                    "Farmer route has not released input for merchant refill"
                )

            def idle_current():
                if (
                    getattr(ui, "grant", None) is not None
                    or not ui.safe_to_yield()
                    or deadline is not None
                    and time.monotonic() >= deadline
                ):
                    raise CaptureUnavailable(
                        "Stopped Farmer input ownership changed during refill"
                    )

            health = _safe_health(
                ui, ui.app.control.snapshot(), expected_target, grant_check=idle_current
            )
    ...
    def grant_current():
        ...
        if (
            ...
            raise CaptureUnavailable(
                "Farmer input handoff changed during observation refresh"
            )
    ...
    if (
        token.scope == "market_visit"
        and (controls.get("life") or {}).get("map_id") != 1036
    ):
        raise CaptureUnavailable("Farmer left the granted Market service visit")
    if token.scope == SCOPE:
        authority = grant.get("listing_authority") or {}
        if (
            ui.coordinator.purpose
            not in (None, "booth_listing_1078_once", "owned_booth_panel_1078")
            or ui.coordinator.owner is not None
            and ui.coordinator.owner != grant.get("character")
            or authority.get("farmer_target") != target
            or token.expires_at - time.time() > 45
        ):
            raise CaptureUnavailable(
                "Listing grant cannot authorize another input purpose or farmer"
            )
...
def release_completed_refill(ui, character):
    ...
    with runtime.lock:
        ...
        if (
            ...
            or not re.fullmatch(
                r"merchant-refill:" + re.escape(peer) + r":\d+", request
            )
            or ui.grant is not None
            or ui.grant_fence.active is not None
            or request in ui.grant_fence.requests
            or runtime.delivery_window
            or runtime.refill_window
        ...
        try:
            ...
            if (
                state.get("pending") is not False
                ...
                or any(
                    row.get("holds_automation")
                    for row in coordinator.manual_sessions.values()
                )
                or runtime.farmer_bot_owned()
                or getattr(ui, "calibrating", None)
                or probe_busy(ui)
            ):
                return False
            workers = [
                getattr(ui, name, None)
                for name in (
                    "delivery_probe_thread",
                    "trade_qualification_prep_thread",
                    "background_thread",
                )
            ]
            ...
            return (
                ui.dispatch({"action": "handoff-release", "request_id": request}).get(
                    "released"
                )
                is True
            )
...
def release_ungranted_unavailable_peer(ui, character):
    ...
    with runtime.lock:
        ...
        if panel and panel["phase"] not in (
            "verified",
            "aborted",
            "operator_overridden",
        ):
        ...
        with runtime.journal.db() as db:
            if (
                ...
                or db.execute(
                    "SELECT 1 FROM delivery_admissions WHERE phase NOT IN "
                    "('verified','aborted','operator_overridden') LIMIT 1"
                ).fetchone()
            ):
                return False
