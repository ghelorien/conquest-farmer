...
def _merchant_key(state):
    ...
    merchant = state.get("intent", {}).get("merchant", {})
    ...
    if (
        ...
        or not profile_id
        or merchant.get("character") != name
    ...
    if profiles:
        ...
        if (
            ...
            or resolved.role != "Merchant"
            or resolved.server != "America"
            or resolved.local_enabled is not True
        ):
...
def lease_authorized(ui, character):
    ...
    try:
        ...
        if (
            coordinator.purpose != "delivery_confirm_probe"
        ...
        if (
            not state
            or state.get("phase")
            not in ("farmer_confirm_verified", "merchant_confirm_submitted")
            or _merchant_key(state) != character
        ):
            return False
        if any(
            coordinator.manual_session_blocked(owner)
            for owner in (
                state.get("farmer_profile_id", "Farmer"),
        ...
        if active(runtime.journal, character) or runtime.journal.pending(character):
            return False
        ...
        ownership(
            state,
            str(character),
            runtime.manual_target(character),
            runtime.manual_target("Farmer"),
            f,
            m,
            now=time.time(),
        )
...
def run(ui, state, *, revision=None):
    ...
    if state.get("phase") not in ("offer_verified", "farmer_confirm_verified"):
        raise ValueError(
            "Submitted confirmation is read-only; never repeat confirmation input"
        )
    ...
    def manual_fence():
        if any(
            ...
            for owner in (
                state.get("farmer_profile_id", "Farmer"),
                state.get("target_profile_id", character),
            )
        ):
            raise CaptureUnavailable(
                "Manual visitor session holds a delivery participant"
            )
    ...
    def check():
        ...
        if (
            getattr(ui, "delivery_probe_thread", None) is not threading.current_thread()
            or ui.closed
            or ui.app.closing
            or c["enabled"]
            or c.get("paused")
            or c["revision"] != revision
    ...
    def save(phase, **fields):
        ...
        receipt()
        state.update(phase=phase, updated_at=time.time(), error=None, **fields)
    ...
    def exact_pair(role):
        ...
        if (
            f["trade"]["accepted"] is not (role == "merchant")
            or f["trade"]["other_accepted"] is not False
            or m["trade"]["accepted"] is not False
            or role == "farmer"
            and m["trade"]["other_accepted"] is not False
        ):
            raise ValueError(
                "Local trade confirmation state changed; no confirmation repeated"
            )
        receipt()
        return f, m
    ...
    def target_binding(role, observer, memory):
        ...
        identity = deepcopy(intent[role]["identity"])
        ...
        native1078 = (
            role == "merchant" and adapter.expected_sha256 == CLIENT_SHA256_1078
        )
        ...
        def verify():
            check()
            if (
                observer.adapter is not adapter
                ...
                raise CaptureUnavailable(
                    "Confirmation observer or owned target changed"
                )
            if role == "merchant":
                if (
                    ui.runtime.controllers.get(character) is not controller
                    or controller.driver is not driver
                    or ui.runtime.observers.get(character) is not observer
                    or driver.observer is not observer
                    or driver.target is not target
                    or driver.memory is not memory
                    or not native1078
                    and ui.hosts.get(character) is not host
                ):
    ...
    def click(role, observer, memory):
        ...
        if native1078:
            ...
            native_foreground(
                ui.runtime.controllers[character].driver,
                intent["merchant"]["identity"],
                activate=True,
            )
            f, m = exact_pair(role)
        target, native = target_binding(role, observer, memory)
        snap = f if role == "farmer" else m
        if snap["trade"]["accepted"]:
            raise ValueError(
                "This account already confirmed; reconcile without repeating"
            )
        w, point, seed = confirm_control(memory.gui, snap)
        size = target.snapshot()["client_size"]
        if size != memory.gui.viewport_size():
            raise ValueError("Native and GUI sizes differ")
        ...
        def before():
            ...
            if current["trade"]["accepted"] or confirm_control(memory.gui, current) != (
                w,
                point,
                seed,
            ):
        ...
        native()
        save(
            role + "_confirm_submitted",
            confirming_role=role,
            confirm_point=point,
            farmer_before_confirm=f,
            merchant_before_confirm=m,
        )
        foreground_click(
            target,
            *point,
            tuple(size),
            require_foreground=native1078,
    ...
    def lease(owner):
        with ui.coordinator.lock:
            ...
            if not ui.runtime.reconcile_probe_pair(character, f, m):
                ...
                raise CaptureUnavailable(
                    "Delivery confirmation needs fresh bilateral probe reconciliation"
                    + suffix
                )
            manual_fence()
            check()
            with (
                ui.coordinator.lease(owner, purpose="delivery_confirm_probe"),
    ...
    if not resume_merchant:
        with lease("Farmer"):
            ...
            presentation = prepare(
                ui,
                state,
                purpose="delivery_confirm_probe",
                revision=revision,
                deadline=deadline,
            )
            ...
            if any(
                s["trade"]["accepted"] or s["trade"]["other_accepted"] for s in (f, m)
            ):
            ...
            while True:
                ...
                if f["trade"]["accepted"]:
                    if f["trade"]["other_accepted"] or m["trade"]["accepted"]:
                        raise ValueError(
                            "Merchant confirmation changed before its submission"
                        )
                    break
                if time.monotonic() >= until:
                    raise ValueError(
                        "Farmer confirmation not verified; no second confirmation"
                    )
                time.sleep(0.05)
            save("farmer_confirm_verified", farmer_after=f, merchant_after=m)
    if character in ui.calibrating:
        raise ValueError("Merchant has another calibration active")
    ui.calibrating.add(character)
    ui.calibration_cancel[character] = threading.Event()
    try:
        with lease(character):
            ...
            while True:
                ...
                if time.monotonic() >= until:
                    raise ValueError(
                        "Trade result uncertain; reconcile before retrying any transfer"
                    )
                time.sleep(0.1)
    finally:
