...
def _observed(probe, role, snapshot, *, closed_after=None):
    ...
    if (
        _holdings(proof) != canonical_ownership(original)
        or any(snapshot.get(k) != original.get(k) for k in ("map_id", "position"))
        or snapshot.get("request") is not None
    ):
        raise BindingMismatch(
            "Empty-bot cleanup history changed ownership or participant location"
        )
    ...
    if trade is None:
        ...
        return proof
    if (
        trade.get("participant") != peer["character"]
        or trade.get("participant_uid") != peer["character_uid"]
    ...
    if snapshot.get("reader_build") == "1078-read-only-candidate":
        if (
            ...
            or trade["own_silver_text"] != "0"
            or trade["other_silver_text"] != "0"
        ):
            raise BindingMismatch(
                "Legacy empty trade observation is not the exact known reader shape"
            )
    elif trade.get("other_accepted") is not False or any(
        type(trade.get(k)) is not int or trade[k] != 0
        for k in ("own_silver", "other_silver")
    ):
        raise BindingMismatch(
            "Empty-bot cleanup history contains currency or peer approval"
        )
    return proof
...
def binding(runtime, probe):
    ...
    with store.db() as db:
        ...
        allowed = {
            probe["farmer_profile_id"]: "farmer",
            probe["target_profile_id"]: "merchant",
        }
        if any(row["target_profile_id"] not in allowed for row in rows):
            raise BindingMismatch(
                "An unrelated manual session blocks empty-bot cleanup"
            )
        for row in rows:
            role = allowed[row["target_profile_id"]]
            peer = probe["intent"]["merchant" if role == "farmer" else "farmer"]
            ...
            if (
                ...
                or row["current_request_id"] is not None
                ...
                raise BindingMismatch(
                    "Manual hold is not a pristine post-accept empty bot interval"
                )
            for table in ("manual_requests", "manual_declines"):
                if db.execute(
                    f"SELECT 1 FROM {table} WHERE session_id=?", (row["id"],)
                ).fetchone():
                    raise BindingMismatch(
                        "Manual request or decline history blocks cleanup"
                    )
            ...
            for index, e in enumerate(evidence):
                ...
                if (
                    e["digest"] != _digest(snapshot)
                    or not previous <= e["recorded_at"] <= now
                ):
                    raise BindingMismatch(
                        "Manual evidence chronology or digest changed"
                    )
                ...
                if e["error"]:
                    ...
                    if (
                        index == 0
                        or snapshot != {"reader_error": expected}
                        or e["error"] != "Request/trade window evidence is unavailable"
                        or e["ownership_digest"] is not None
                        or e["observed_at"] is not None
                    ):
                        raise BindingMismatch(
                            "Unrecognized manual history gap blocks cleanup"
                        )
                    continue
                proof = _observed(probe, role, snapshot)
                _fresh(snapshot, e["recorded_at"])
                if e["observed_at"] != snapshot["timestamp"] or e[
                    "ownership_digest"
                ] != _digest(proof):
        ...
        holds = sorted(
            (store._view(db, r["row"]) for r in records), key=lambda r: r["id"]
        )
...
def settle(runtime, receipt, farmer, merchant):
    ...
    with store.db() as db:
        ...
        for record in receipt["sessions"]["records"]:
            ...
            expected = canonical_ownership(probe["intent"][role])
            if current != expected or snapshot["timestamp"] < receipt["submitted_at"]:
                raise BindingMismatch(
                    "Closed cleanup ownership changed during settlement"
                )
            terminal = json.loads(row["terminal_json"] or "{}")
            if (
                row["phase"] == "request_withdrawn"
                and terminal.get("empty_cancel_digest") == receipt["receipt_digest"]
            ):
                continue
            ignored = {
                "phase",
            ...
            if {k: v for k, v in dict(row).items() if k not in ignored} != {
                ...
                raise BindingMismatch(
                    "Bound manual session changed before cleanup settlement"
                )
            ...
            for table, key in (
                ...
                if _summary(prefix) != record[key]:
                    raise BindingMismatch("Bound cleanup history changed")
            for e in db.execute(
                "SELECT * FROM manual_evidence WHERE session_id=? AND id>? ORDER BY id",
                (row["id"], record["evidence"]["max_id"]),
            ):
                prior = json.loads(e["snapshot_json"])
                if e["digest"] != _digest(prior):
                    raise BindingMismatch("Cleanup observation digest changed")
                if e["error"]:
                    expected_error = {
                        "farmer": "Farmer has no attached memory observer",
                        "merchant": "Attached memory reader is unavailable",
                    ...
                    if (
                        prior != {"reader_error": expected_error}
                        or e["error"] != "Request/trade window evidence is unavailable"
                        or e["ownership_digest"] is not None
                        or e["observed_at"] is not None
                    ):
                        raise BindingMismatch(
                            "Unrecognized cleanup reader gap remains held"
                        )
                    ...
                    if (
                        ...
                        and e["recorded_at"] >= row["stable_since"]
                    ):
                        db.execute(
                            "UPDATE manual_sessions SET stable_digest=NULL,stable_since=NULL,stable_evidence_id=NULL WHERE id=?",
                            (row["id"],),
                        )
            ...
            if (
                same
                and snapshot["timestamp"] - row["stable_since"] >= SETTLEMENT_SECONDS
            ):
                result = {
                    "phase": "request_withdrawn",
                    "empty_cancel_digest": receipt["receipt_digest"],
                ...
                db.execute(
                    "UPDATE manual_sessions SET phase='request_withdrawn',terminal_json=?,reason=NULL,last_observed_at=?,updated_at=? WHERE id=?",
                ...
                if store.on_settlement is not None:
                    store.on_settlement(
                        db, store._view(db, store._row(db, row["id"])), snapshot, result
                    )
            else:
                done = False
                db.execute(
...
def observe(runtime, character, snapshot):
    ...
    receipt = read()
    if not receipt or receipt.get("phase") != "cancel_verified":
        return None
    target = runtime.manual_target(character)
    matches = [
        r
        for r in receipt.get("sessions", {}).get("records", [])
        if r["row"]["target_profile_id"] == target
    ]
    if not matches:
        return None
    ...
    if (
        ...
        or not current
        or current.get("empty_cancel_digest") != receipt["receipt_digest"]
    ):
