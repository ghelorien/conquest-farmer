...
def unsubmitted_listing_safe_to_retry(before, after):
    return (
        after.get("character") == before["snapshot"]["character"]
        and after.get("identity") == before["snapshot"]["identity"]
        and after.get("booth_open")
        and not after.get("request")
        and not after.get("trade")
        and not any(w["name"] == "Add Item to Booth" for w in after.get("windows", []))
        and not any(i["uid"] == before["uid"] for i in after["booth"])
        and identities(after["inventory"]).get(before["uid"])
        == identities([before["item"]])[before["uid"]]
    )
...
def validate_trade(snapshot):
    ...
    if not trade or not trusted_delivery(
...
def received(before, after):
    ...
    if after["character"] != before["character"] or after.get("trade"):
        return False
    old, new, offered = (
        identities(before["inventory"]),
        identities(after["inventory"]),
        identities(trade["items"]),
    )
    return (
        new == {**old, **offered}
...
def listing_received(before, after):
    return not any(i["uid"] == before["uid"] for i in after["inventory"]) and any(
        i["uid"] == before["uid"]
        and i["price"] == before["price"]
        and identities([i]) == identities([before["item"]])
        for i in after["booth"]
    )
...
class MerchantController:
    ...
    def __init__(self, character, journal, driver, coordinator, *, clock=time.time):
        self.character, self.journal, self.driver, self.coordinator, self.clock = (
            character,
            journal,
            driver,
            coordinator,
    ...
    def active(self):
        return (
            self.journal.get(self.character, "enabled", False)
    ...
    def check_listing(self):
        ...
        if active(self.journal, self.character):
            raise CaptureUnavailable(
                "Reserved farmer delivery holds merchant listings until both inventories reconcile"
            )
        ...
        for record in pending:
            ...
            if record["kind"] == "delivery" and received(before, snapshot):
                self.journal.transition(
                    record["id"], "verified", {"items": before["trade"]["items"]}
                )
            elif record["kind"] == "listing" and listing_received(before, snapshot):
                self.journal.transition(
                    record["id"],
                    "verified",
                    {"uid": before["uid"], "price": before["price"]},
                )
            elif (
                record["kind"] == "listing"
                and json.loads(record.get("result_json") or "{}").get(
                ...
                raise ValueError(
                    "Unfinished transaction requires reconciliation; input is paused"
                )
    ...
    def recover_unsubmitted_listing(self, snapshot):
        ...
        if len(candidates) != 1:
            raise ValueError(
                "Multiple unsubmitted listing dialogs need operator reconciliation"
            )
        record, before = candidates[0]
        uid = before["uid"]
        if (
            snapshot.get("identity") != before["snapshot"].get("identity")
        ...
        dialogs = [
            w for w in snapshot.get("windows", []) if w["name"] == "Add Item to Booth"
        ]
        if not dialogs:
            return False  # Ordinary read-only reconcile closes this case.
        if len(dialogs) != 1:
        ...
        if (
            unpack(self.driver.observer.adapter, model + 0x50, "<I")[0] != uid
            or identities(snapshot["inventory"]).get(uid)
            != identities([before["item"]])[uid]
        ...
        with self.coordinator.lease(
            self.character, purpose="unsubmitted_listing_cancel"
        ):
            self.check_listing()
            ...
            def same_dialog():
                ...
                if (
                    stock(fresh) != expected_stock
                    or fresh.get("trade")
                    or fresh.get("request")
                    or unpack(self.driver.observer.adapter, model + 0x50, "<I")[0]
                    != uid
                    or modal_controls(self.driver.observer.adapter, fresh)
                    != modal_controls(self.driver.observer.adapter, current)
                ):
                    raise ValueError(
                        "Unsubmitted price dialog changed before cancellation"
                    )
            ...
            after = self.driver.wait_for(
                lambda s: (
                    not any(w["name"] == "Add Item to Booth" for w in s["windows"])
                ),
                self.check_listing,
            )
            if (
                stock(after) != expected_stock
                or unpack(self.driver.observer.adapter, model + 0x50, "<I")[0] != 0
            ):
                raise ValueError(
                    "Listing cancellation did not preserve exact merchant stock"
                )
            self.journal.transition(
                record["id"],
                "aborted",
    ...
    def accept_delivery(self):
        ...
        with self.coordinator.lease(self.character, purpose="trade"):
            ...
            if (
                not accepted_request
                or accepted_request.get("identity") != before["identity"]
            ...
            if (
                ...
                or offer_fingerprint(validate_trade(fresh)) != fingerprint
                or identities(fresh["inventory"]) != identities(before["inventory"])
            ...
            try:
                ...
                self.journal.transition(
                    key,
                    "verified",
                    {"items": trade["items"], "silver": trade["other_silver"]},
                )
    ...
    def accept_request(self, snapshot):
        ...
        with self.coordinator.lease(self.character, purpose="trade"):
            ...
            if (
                ...
                or fresh.get("request") != snapshot.get("request")
                or not fresh.get("request")
                or not trusted_delivery(
                    self.character,
                    fresh["request"]["participant"],
                    fresh["request"].get("participant_uid"),
                )
            ):
            ...
            opened = self.driver.wait_for(
                lambda s: (
                    bool(s.get("trade"))
                    and trusted_delivery(
                        self.character,
                        s["trade"]["participant"],
                        s["trade"].get("participant_uid"),
                    )
                ),
                self.check,
            )
            self.journal.set(
                ...
                "accepted_request",
                {
                    ...
                    "participant_uid": opened["trade"]["participant_uid"],
    ...
    def plan(
        ...
        *,
        ...
        for state in (*owned_snapshots, snapshot):
            ...
            if (
                seller in OWNED
                and state.get("server") == "America"
                and state.get("booth_open")
        ...
        plans = []
        for item in ([] if inventory_only else snapshot["booth"]) + snapshot[
            "inventory"
        ]:
            try:
                ...
                comparisons = [
                    r
                    for r in market.comparisons(item)
                    if r.seller.casefold() not in live
                ]
                decision = quote_item(
                    key,
                    comparisons + owned,
                ...
                plans.append(
                    {
                        ...
                        "observed_at": market.data["observed_at"],
                        **asdict(decision),
                    }
                )
        ...
        return sorted(
            plans, key=lambda p: (p["price"] is None, -(p["price"] or 0), p["uid"])
        )
    ...
    def apply_price(self, plan):
        ...
        with self.coordinator.lease(self.character, purpose=self.listing_purpose):
            ...
            try:
                ...
                current = self.driver.read()
                if any(
                    w["name"] == "Add Item to Booth" for w in current.get("windows", [])
                ):
                    raise ValueError(
                        "Existing price dialog needs reconciliation before automatic verification"
                    )
            ...
            if (
                ...
                and len(before["inventory"]) >= before["capacity"]
            ):
            ...
            self.journal.begin(
                ...
                self.character,
                "listing",
                {"uid": uid, "price": target, "item": item, "snapshot": before},
            )
            try:
                self.driver.list_item(before, item, target, self.check_listing)
                self.journal.transition(key, "submitted")
                after = self.driver.wait_for(
                    lambda s: listing_received(
                        {"uid": uid, "price": target, "item": item}, s
                    ),
                    self.check_listing,
                )
                self.journal.transition(
                ...
                failure = {
                    "confirmation_attempted": not isinstance(
                        error, ListingNotSubmitted
                    ),
                    "error_type": type(error).__name__,
                    "note": str(error)
                    if isinstance(error, (ValueError, CaptureUnavailable))
                    else "Listing operation failed",
                    "frames": [
                        {
                            "file": f.filename.replace("\\", "/").rsplit("/", 1)[-1],
                            "function": f.name,
                            "line": f.lineno,
                        }
