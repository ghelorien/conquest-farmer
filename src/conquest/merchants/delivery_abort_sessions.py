...
def _summary(records):
    ...
    return dict(
        count=len(records),
        max_id=records[-1]["id"] if records else None,
        last_digest=records[-1]["digest"] if records else None,
...
def _highwater(db, table, session_id):
    row = db.execute(
        f"SELECT COUNT(*),MAX(id) FROM {table} WHERE session_id=?", (session_id,)
    ).fetchone()
...
def _no_extra_holds(db):
    if (
        db.execute(
            "SELECT 1 FROM manual_rebaseline WHERE phase!='completed'"
        ).fetchone()
        or db.execute(
...
def capture(store, db, probe, *, clock=None, closed_after=None):
    ...
    rows = list(
        db.execute(
            "SELECT * FROM manual_sessions WHERE phase NOT IN "
    ...
    if len(rows) != 2 or {row["target_profile_id"] for row in rows} != {source, target}:
        raise BindingMismatch(
            "Abort requires exactly the two named unapproved probe holds"
        )
    for row in rows:
        ...
        visitor = dict(
            ...
            visitor_name=peer["character"],
            visitor_server=peer["server"],
            visitor_uid=peer["character_uid"],
        )
        if (
            ...
            or json.loads(row["character_json"])
            != {k: original[k] for k in ("character", "character_uid", "server")}
        ):
            raise BindingMismatch(
                "Abort may not bypass a genuine, approved or changed manual interval"
            )
        requests = [
            dict(r)
            for r in db.execute(
                "SELECT * FROM manual_requests WHERE session_id=? ORDER BY id",
                (row["id"],),
            )
        ]
        declines = [
            dict(r)
            for r in db.execute(
                "SELECT * FROM manual_declines WHERE session_id=?", (row["id"],)
            )
        ]
        claims = [
            dict(r)
            for r in db.execute(
                "SELECT c.* FROM manual_decline_claims c JOIN manual_requests r ON r.id=c.request_id WHERE r.session_id=?",
                (row["id"],),
            )
        ]
        audits = [
            dict(r)
            for r in db.execute(
                "SELECT * FROM manual_audit WHERE session_id=? ORDER BY id",
                (row["id"],),
            )
        ]
        evidence = [
        ...
        if (
            ...
            raise BindingMismatch(
                "Abort cannot bypass approval, decline, input or operator history"
            )
        if db.execute(
            "SELECT 1 FROM visitor_permissions WHERE target_profile_id=? AND visitor_name=? AND visitor_server=? AND visitor_uid=? AND allowed=1",
            (
            ...
            raise BindingMismatch(
                "An allowlisted manual visitor cannot use probe-abort reclassification"
            )
        if requests:
            if (
                role != "merchant"
                ...
                raise BindingMismatch(
                    "Abort requires a pristine merchant request, never a farmer request"
                )
        ...
        proof = historical_local_trade(
            probe, probe["character"], target, source, role, snapshot
        )
        if (
            first["error"]
            or first["digest"] != _digest(snapshot)
            or first["ownership_digest"] != _digest(proof)
            or first["recorded_at"] < accepted
        ):
        ...
        if requests:
            ...
            if (
                snapshot.get("request") is None
                or request["before_json"] != _json(proof)
                ...
                or binding.get("visitor") != visitor
                or binding.get("game_process_identity") != original["identity"]
                ...
                raise BindingMismatch(
                    "Original pending request differs from the supervised probe"
                )
            ...
            raise BindingMismatch(
                "Farmer hold predates the exact durable offer boundary"
            )
        ...
        if any(
            record["digest"] != _digest(json.loads(record["snapshot_json"]))
        ...
        for record in evidence:
            ...
            if record["error"] is None:
                if record["ownership_digest"] != _digest(
                    _fresh(observed, record["recorded_at"])
                ):
                    raise BindingMismatch("Manual ownership evidence digest differs")
            elif record["ownership_digest"] is not None:
                raise BindingMismatch(
                    "Reader-error evidence cannot claim ownership authority"
                )
            if not isinstance(observed, dict):
                continue
            if any(
                key in observed and observed[key] != original[key]
                for key in ("identity", "character", "character_uid", "server")
            ):
                raise BindingMismatch(
                    "Manual history contains a changed process or character"
                )
            if (
                observed.get("request") is None
                and observed.get("trade") is None
                and record["error"] is None
            ):
                if closed_after is None or observed.get("timestamp", 0) < closed_after:
                    raise BindingMismatch(
                        "Unexpected closed-window history predates durable abort submission"
                    )
                current = canonical_ownership(observed)
                expected = deepcopy(original)
                if role == "merchant":
                    expected.update(booth=current["booth"], silver=current["silver"])
                if current != expected:
                    raise BindingMismatch(
                        "Post-abort observation changed unrelated ownership"
                    )
            for field in ("request", "trade"):
                modal = observed.get(field)
                if isinstance(modal, dict):
                    if (
                        modal.get("participant") != peer["character"]
                        or type(modal.get("participant_uid")) is not int
                        or modal["participant_uid"] != peer["character_uid"]
                        or modal.get("server", observed.get("server")) != peer["server"]
                    ):
                        raise BindingMismatch(
                            "Manual history contains an incomplete or different participant"
                        )
                    if field == "request" and (
                        role != "merchant"
                        or not requests
                        or request_fingerprint(observed) != requests[0]["fingerprint"]
                    ):
                        raise BindingMismatch(
                            "Manual history contains a different request"
                        )
                if field == "trade" and isinstance(modal, dict):
                    if (
                        any(
                            modal.get(key) is not False
                            for key in ("accepted", "other_accepted")
                        )
                        or any(
                            type(modal.get(key)) is not int or modal[key] != 0
                            for key in ("own_silver", "other_silver")
                        )
                        or modal.get("items" if role == "farmer" else "own_items")
                    ):
                        raise BindingMismatch(
                            "Manual history contains approval, currency or reverse-offer input"
                        )
                    offered = exact_items(
                        modal.get("own_items" if role == "farmer" else "items", [])
                    )
                    selected = exact_items(probe["intent"]["items"])
                    if any(
                        selected.get(uid) != details for uid, details in offered.items()
                    ):
                        raise BindingMismatch(
                            "Manual history contains a different item offer"
                        )
        records.append(
            dict(
                row=dict(row),
...
def _prefix(db, expected, records):
    ...
    if len(records) != len(expected["records"]):
        raise BindingMismatch("Named abort sessions changed")
    for before, after in zip(expected["records"], records):
        ignored = {"updated_at", "last_observed_at", "reason"}
        if (
            {k: v for k, v in before["row"].items() if k not in ignored}
            != {k: v for k, v in after["row"].items() if k not in ignored}
            or before["requests"] != after["requests"]
            or before["declines"] != after["declines"]
        ...
        for table, field in (
            ...
            prefix = [
                ...
                for row in db.execute(
                    f"SELECT * FROM {table} WHERE session_id=? AND id<=? ORDER BY id",
                    (before["row"]["id"], before[field]["max_id"]),
                )
            ]
            if _summary(prefix) != before[field]:
                raise BindingMismatch("Confirmed immutable history prefix changed")
...
def refresh_binding(runtime, probe, expected, *, clock=None, closed_after=None):
    ...
    with runtime.manual_sessions.db() as db:
        records = capture(
            runtime.manual_sessions, db, probe, clock=clock, closed_after=closed_after
        )
        _prefix(db, expected, records)
        views = sorted(
            (runtime.manual_sessions._view(db, record["row"]) for record in records),
...
def check_binding(runtime, expected):
    ...
    with runtime.manual_sessions.db() as db:
        ...
        for record in expected["records"]:
            ...
            if requests != record["requests"]:
                raise BindingMismatch("Abort request state changed")
            for table, field in (
                ("manual_evidence", "evidence"),
                ("manual_audit", "audit"),
            ):
                if _highwater(db, table, key) != {
                    k: v for k, v in record[field].items() if k != "stream_sha256"
                }:
                    raise BindingMismatch("Abort session history was appended")
            if (
                db.execute(
                    "SELECT 1 FROM manual_declines WHERE session_id=?", (key,)
                ).fetchone()
                or db.execute(
                    "SELECT 1 FROM manual_decline_claims c JOIN manual_requests r ON r.id=c.request_id WHERE r.session_id=?",
                    (key,),
                ).fetchone()
                or db.execute(
                    "SELECT 1 FROM visitor_permissions WHERE target_profile_id=? AND visitor_name=? AND visitor_server=? AND visitor_uid=? AND allowed=1",
    ...
    *,
    ...
    with store.db() as db:
        ...
        for record in expected["records"]:
            ...
            baseline = db.execute(
                "SELECT * FROM manual_rebaseline WHERE source_session_id=?",
                (row["id"],),
            ).fetchone()
            if (
                row["phase"] != "operator_overridden"
                or terminal.get("confirmation_reference") != confirmation_reference
                or terminal.get("operator") != operator
                or not terminal.get("prepared_history_digest")
                or terminal.get("session_history_digest") != expected["digest"]
                or terminal.get("abort_receipt_digest") != _digest(receipt)
                or baseline is None
        ...
        if already:
            if len(already) != len(expected["records"]):
                raise BindingMismatch("Partial terminal disposition is not retryable")
            recheck()
            return already
        ...
        records = capture(
            store, db, probe, clock=lambda: now, closed_after=receipt["submitted_at"]
        )
        _prefix(db, expected, records)
        recheck()
        results = []
        for record in records:
            ...
            role = (
                "farmer"
                if row["target_profile_id"] == probe["farmer_profile_id"]
                else "merchant"
            )
            current = canonical_ownership(receipt["after"][role])
            # Pin process identity now, not on a later first poll after restart.
            db.execute(
                "INSERT INTO manual_rebaseline(id,target_profile_id,source_session_id,phase,created_at,updated_at,process_json,character_json,reason) VALUES(?,?,?,'settlement_observed',?,?,?,?,?)",
                (
                    "rebaseline:" + row["id"],
                    row["target_profile_id"],
                    row["id"],
                    ...
                    json.dumps(
                        {
                            k: current[k]
                            for k in ("character", "character_uid", "server")
                        },
                        sort_keys=True,
                    ),
        ...
        for record in records:
            ...
            terminal = dict(
                ...
                transfer_confirm_input=False,
                abort_receipt_digest=_digest(receipt),
                session_history_digest=expected["digest"],
                reason="Operator disposition after verified probe abort",
            )
