...
PINS = (
    (0x19f3d0, 1974, 'e395400a19ba0d93d910de45c7d55d99b41dacd6cf31fda6c9a83ed5d8768e0d'),
...
def assert_selected(session, gui, base, candidate):
    ...
    if (struct.unpack('<I', uid)[0] != candidate['owned_booth_uid']
    ...
    for address, raw in ((root,pointer),(manager,vtable),(manager+0xd8,selected),
                         (actor+0x78,uid),(actor+0x10,secondary),
                         (base+gui.context_rva,context_raw),(context+0xd0,capture),
    ...
    session.assert_identity()
    return {'actor_address':hex(actor),'selected_actor':hex(actor+0x10),
            'owned_booth_uid':candidate['owned_booth_uid'],'manager':hex(manager)}
