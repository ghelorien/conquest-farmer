...
def control(driver, snapshot, flag):
    ...
    for rva, code in CODE.items():
        ...
        if s.read_block(g.base + rva, len(expected)) != expected:
            raise ValueError("Open Booth handler changed")
    callback = unpack(s, model + 0x100, "<Q")[0]
    if (
        callback != model + 0xC8
        or unpack(s, callback, "<Q")[0] != g.base + 0x5C7CA0
    ...
    if not x < point[0] < x + width or not y < point[1] < y + height:
        raise ValueError("Open Booth button outside dialog")
    return w, point
...
def submit(driver, travel, before, flag, check, before_press):
    ...
    def fresh():
        ...
        if (
            ...
            or now["map_id"] != 1036
            or stock(now) != stock(before)
            or now.get("own_booth_uid")
            or now.get("trade")
            or now.get("request")
        ):
            raise ValueError("Merchant or stock changed before opening booth")
        candidate = next(
            (f for f in vacant_flags(driver.observer, spec) if f["uid"] == flag["uid"]),
            None,
        )
    ...
    def verify():
        if control(driver, fresh(), flag) != (w, point):
            raise ValueError("Open Booth confirmation changed")
        driver.memory.gui.assert_hovered(w, "Yes")

    def press():
        wait_hover_validation(verify, check)
        before_press()
    ...
    while time.monotonic() < until:
        check()
        after = driver.memory.read()
        if any(
            after[k] != before[k]
            for k in ("identity", "character_uid", "map_id", "silver")
        ) or stock(after) != stock(before):
