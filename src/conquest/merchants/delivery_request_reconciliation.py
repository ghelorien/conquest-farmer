...
def observe(ui, state):
    ...
    if observer is None:
        raise ValueError("The saved merchant is no longer attached")
    with observer.lock:
        uid = participant_uid(observer, intent["farmer"]["character"])
    if type(uid) is not int or uid != intent["farmer"]["character_uid"]:
        raise ValueError(
            "Pinned incoming request actor differs from the saved farmer UID"
        )
    return {
        "farmer": farmer,
        "merchant": merchant,
...
def persist(state):
    ...
    fd, name = tempfile.mkstemp(
        prefix=path.name + ".request-reconcile.", suffix=".tmp", dir=path.parent
    )
    temporary = Path(name)
    try:
        with os.fdopen(fd, "wb") as out:
            if out.write(raw) != len(raw):
                raise OSError("Incomplete request reconciliation journal write")
            out.flush()
            os.fsync(out.fileno())
...
def reconcile_request(ui):
    ...
    if not state or state.get("phase") not in ("request_submitted", "request_verified"):
        raise ValueError(
            "Only a submitted or already verified request can be reconciled"
        )
    original_digest = evidence_digest(state)
    intent = state["intent"]
    recipient = state.get("recipient", {})
    if (
        state["character"] != intent["merchant"]["character"]
    ...
    try:
        ...
        if first["evidence_digest"] != second["evidence_digest"] or any(
            second[role]["timestamp"] <= first[role]["timestamp"]
            for role in ("farmer", "merchant")
        ):
            raise ValueError(
                "Request ownership or identity did not stabilize across fresh observations"
            )
        ownership(intent, second["farmer"], second["merchant"], now=time.time())
        selected = probe.selected_intent(
            {**second["farmer"], "request": None},
            {**second["merchant"], "request": None},
            state.get("selected_uids"),
        )
        if exact_items(selected["items"]) != exact_items(intent["items"]):
            raise ValueError(
                "Submitted selection differs from the authorized exact item"
            )
        probe.recovery_available(ui)
        if evidence_digest(probe.read_probe()) != original_digest:
            raise ValueError(
                "Trade probe changed during read-only request reconciliation"
            )
        ...
        if idempotent:
            ...
            if not isinstance(farmer, dict) or not isinstance(merchant, dict):
                raise ValueError(
                    "Verified request lacks its original observation evidence"
                )
            saved = ownership(
                intent,
                farmer,
                merchant,
                now=max(farmer["timestamp"], merchant["timestamp"]),
            )
            ...
            state = {
                **state,
                ...
                "farmer_after": second["farmer"],
                "merchant_after": second["merchant"],
                "request_reconciliation": {
                    "original_phase": "request_submitted",
                    "original_incident_digest": original_digest,
