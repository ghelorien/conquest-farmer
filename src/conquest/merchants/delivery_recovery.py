...
def partial_result(intent, farmer, merchant, *, now=None):
    ...
    before_f = exact_items(intent["farmer"]["inventory"])
    ...
    for role, snapshot, other in (
        ...
        if any(
            snapshot[k] != before[k] for k in ("identity", "character_uid", "silver")
        ):
        ...
        if snapshot.get("request"):
            raise ValueError("An incoming request remains unresolved")
        ...
        if trade:
            if (
                ...
                raise ValueError(
                    "Offered items or acceptance must reconcile before a partial abort"
                )
    ...
    moved = {
        ...
        if uid not in f and m.get(uid) == details
    }
    remaining = {
        uid: details
        for uid, details in wanted.items()
        if f.get(uid) == details and uid not in m
    }
    if (
        set(moved) | set(remaining) != set(wanted)
        or not moved
        or not remaining
        or f != {uid: details for uid, details in before_f.items() if uid not in moved}
        or m != {**before_m, **moved}
    ):
        raise ValueError(
            "The exact partial batch and all surrounding stock have not reconciled"
        )
    return {
        "outcome": "partial_delivery_aborted",
