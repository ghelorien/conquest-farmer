...
def pending():
    ...
    return read_recovered(JOURNAL).get("phase") in (
        "prepared",
        "outbound_pending",
        "market",
        "return_pending",
...
def matching_scroll_recovery(locks, *, path=None):
    ...
    if len(locks) != 1:
        return False
    state = read_json(path or JOURNAL)
    wanted = state.get("scroll_withdrawal") or {}
    row = locks[0]
    intent = row.get("intent") or {}
    anchor = intent.get("deposit_receipt") or {}
    ...
    try:
        return (
            state.get("phase") == "market"
            and not state.get("scroll_preparation_done")
            and wanted.get("operation_id") == row.get("operation_id")
            and wanted.get("uid") == row.get("uid")
...
def settle_scroll_admission(state, result):
    old = state["scroll_withdrawal"]
    receipt = result.get("receipt") or {}
    if (
        result.get("operation_id") != old["operation_id"]
        or result.get("uid") != old["uid"]
    ):
    ...
    if result.get("phase") == "no_transfer" and result.get("no_input_proven") is True:
        ...
        return False
    if (
        result.get("phase") != "withdrawn"
        or receipt.get("item", {}).get("uid") != old["uid"]
        ...
        raise ValueError(
            "MeteorScroll withdrawal requires read-only reconciliation before merchant input"
        )
    save(
        state,
        scroll_preparation_done=True,
        scroll_withdrawal_receipt=receipt,
        scroll_warehouse_closed=False,
    )
    return True
...
def reconcile_pending_scroll(loop):
    ...
    if not matching_scroll_recovery(locks):
        return False
...
def operator_override(
    ...
    return close(
        JOURNAL,
        pending_phases=("prepared", "outbound_pending", "market", "return_pending"),
        operator_confirmed=operator_confirmed,
        confirmation_reference=confirmation_reference,
        operator=operator,
        fresh_evidence=fresh,
        incident="merchant-journey",
    )
...
def preflight(loop, send, *, require_inventory=True):
    ...
    if not enabled(route_character(loop)):
        return False
    ...
    if not rollout_enabled(
        route_character(loop), policy=policy
    ) and not trial_permitted(loop):
        return False
    try:
        if not send({"action": "delivery-readiness"}).get("qualified"):
            return False
        ...
        for name, state in states.items():
            ...
            if not isinstance(snap, dict):
                continue
            try:
                validate_snapshot(snap, name, time.time())
            except (ValueError, KeyError, TypeError):
                continue
            if (
                snap.get("booth_open")
                and not snap.get("trade")
                and not snap.get("request")
...
def start(loop, *, send=request, stored_scroll_uid=None):
    ...
    if stored_scroll_uid is not None and (
        type(stored_scroll_uid) is not int or stored_scroll_uid <= 0
    ):
        raise ValueError("Stored scroll delivery requires an exact positive item UID")
    if pending():
        if (
            stored_scroll_uid is not None
            and read_json(JOURNAL).get("stored_scroll_uid") != stored_scroll_uid
        ):
            raise ValueError("A different merchant journey needs reconciliation")
        return resume(loop, send=send)
    if not preflight(loop, send, require_inventory=stored_scroll_uid is None):
        return False
    ...
    if (
        origin == 1036
        or not route
        or not all(route.get(leg, {}).get("verified") for leg in ("outbound", "return"))
    ):
        return False
    ...
    ) != (origin, 1036, 1036, origin) or any(
        type(p.get("fare")) is not int or p["fare"] < 0 for p in (outbound, inbound)
    ):
        return False
    bank = loop.town("warehouse-money")
    reserve = outbound["fare"] + inbound["fare"] + transport_reserve()
    needed = max(0, reserve - bank["silver"])
    if needed > bank["stored_silver"]:
        return False
    if needed:
        transfer(loop, "withdraw", needed)
    state = {
        "phase": "prepared",
        "origin": origin,
        "route": route,
    ...
    loop.record(
        ...
        activity="Shopping complete; taking eligible loot to the Market merchants",
    )
    return resume(loop, send=send)
...
def start_market(loop, stored_scroll_uid, *, send=request):
    ...
    if pending():
        ...
        if (
            not old.get("market_only")
            or old.get("stored_scroll_uid") != stored_scroll_uid
        ):
            raise ValueError("A different merchant journey needs reconciliation")
        return resume(loop, send=send)
    if loop.living()["embedded_controls"]["life"]["map_id"] != 1036:
        raise ValueError(
            "Stored-scroll Market delivery requires a living farmer already in Market"
        )
    ...
    state = {
        ...
        "market_only": True,
        "route": {},
        "stored_scroll_uid": stored_scroll_uid,
        "started_at": time.time(),
        "receipts": [],
    }
    write_json(JOURNAL, state)
...
def leg(loop, state, name):
    ...
    plan = {
        **state["route"][name],
        "activity": (
            "Heading to Conductress for merchant delivery"
            if name == "outbound"
...
def verify_arrival(loop, state, name):
    before = state.get("leg_before")
    bag = loop.town("supplies")
    if (
        not before
        or before["silver"] - bag["silver"] != state["route"][name]["fare"]
        or sorted(before["items"], key=lambda i: i["uid"])
        != sorted(bag["items"], key=lambda i: i["uid"])
    ):
        raise ValueError(
            "Merchant journey fare or inventory needs reconciliation; no repeat payment"
        )
...
def current_scroll_delivery(state, item):
    ...
    wanted = exact_items([item])
    uid = item["uid"]
    withdrawal = state.get("scroll_withdrawal") or {}
    admitted = {
        row["request_id"]: row
        for row in state.get("scroll_delivery_requests", [])
        if row.get("withdrawal_operation_id") == withdrawal.get("operation_id")
    }
    minimum = max(
        state.get("started_at", 0),
        state.get("scroll_withdrawal_receipt", {}).get("verified_at", 0),
    )
    records = state.get("scroll_delivery_receipts", []) + read_json(
    ...
    for row in reversed(records):
        ...
        if (
            request
            and row.get("merchant") == request.get("merchant")
            ...
            and row.get("outcome") == "transferred"
            and row.get("proof_digest")
...
def scroll_ownership(state, send, *, stored=None, allow_storage_recheck=False):
    ...
    before = receipt.get("after", {}).get("source")
    ...
    if (
        any(
            source.get(k) != before.get(k)
            for k in ("character", "character_uid", "identity", "server", "silver")
        )
        or source.get("trade")
        or source.get("request")
    ):
        raise ValueError(
            "Retrieved scroll farmer identity, silver or modal state changed"
        )
    wanted = exact_items([item])
    uid = item["uid"]
    delivered = current_scroll_delivery(state, item)
    if inventory.get(uid) == wanted[uid]:
        return "carried", source
    if uid in inventory:
        raise ValueError("Retrieved scroll attributes changed")
    if (
        delivered
        and delivered.get("outcome") == "transferred"
        and delivered.get("proof_digest")
    ...
    if (
        stored is not None
        and exact_items(stored.get("items", [])).get(uid) == wanted[uid]
    ):
    ...
    if allow_storage_recheck:
        return "needs_storage_recheck", source
    raise ValueError(
        "Requested MeteorScroll has no exact carried, bilateral-delivery or rebanked disposition"
    )


def stored_scroll_recheck_needed(state):
    item = state.get("scroll_withdrawal_receipt", {}).get("item")
    if not item or state.get("deposit_pending"):
        return False
    disposition = state.get("scroll_disposition") or {}
    return (
        disposition.get("outcome") == "deferred_rebanked"
        and disposition.get("item") == item
    ) or any(
        row.get("uid", row.get("stored")) == item["uid"]
        and row.get("type_id") == 720027
        and row.get("verified_in_warehouse") is True
...
def recheck_stored_scroll(loop, state, send):
    ...
    if disposition != "needs_storage_recheck":
        raise ValueError(
            "Recorded scroll fallback conflicts with current ownership; reconcile before input"
        )
    approach_market_warehouse(
        loop, "Rechecking the exact stored delivery scroll after interrupted banking"
    )
    open_warehouse(loop)
    disposition, _ = scroll_ownership(
        state, send, stored=loop.town("warehouse-items", rich=True)
    )
    if disposition != "deferred_rebanked":
        raise ValueError(
            "Interrupted scroll fallback lacks fresh exact warehouse ownership"
        )
...
def admit_scroll_delivery(state, active):
    ...
    if exact_items(active.get("items", [])) != exact_items([item]):
        raise ValueError(
            "The admitted merchant request does not contain exactly the requested scroll"
        )
    requests = state.setdefault("scroll_delivery_requests", [])
    requests.append(
        {
            **active,
            "withdrawal_operation_id": state["scroll_withdrawal"]["operation_id"],
        }
    )
    save(state)
...
def close_scroll_preparation(loop, state, send):
    if not state.get("scroll_preparation_done") or not state.get(
...
def warehouse_fallback(loop, state, *, send=request):
    ...
    if state.get("acceptance_scope"):
        from conquest.merchant_loop_acceptance import (
            journey_scope,
            verify_carried_or_delivered,
        )
        ...
        if verify_carried_or_delivered(send({"action": "delivery-source"})["farmer"]):
            raise ValueError(
                "Acceptance delivery is deferred; exact item remains carried for merchant delivery"
            )
    if not state.get("deposit_pending") and not any(
        stash_candidate(i) for i in loop.town("supplies")["items"]
    ):
        return
    approach_market_warehouse(
        loop, "Storing remaining valuables in the Market warehouse"
    )
    open_warehouse(loop)
    while True:
        bag = loop.town("supplies")["items"]
        bank = loop.town("warehouse-items")
        old = state.get("deposit_pending")
        if old:
            fields = ("uid", "type_id", "amount", "plus")
            matches = [
                i for i in bank["items"] if all(i.get(k) == old.get(k) for k in fields)
            ]
            if len(matches) != 1 or any(i["uid"] == old["uid"] for i in bag):
                raise ValueError(
                    "Merchant fallback deposit is uncertain; no repeated storage input"
                )
        ...
        if delivery_route.warehouse_exhausted(loop, bank, remaining, send=send):
            request_stop(
                ...
                bank,
                remaining,
                reason="Market warehouse is full; merchant delivery has no verified storage destination. Farming stopped and automatic reconnect disabled",
            )
        if not remaining:
            break
        item = remaining[0]
...
def defer_unavailable_stored_scroll(loop, state, send):
    ...
    def unavailable():
        ...
        guard_reload()
        if (
            not enabled(route_character(loop))
            or not rollout_enabled(route_character(loop))
            or not send({"action": "delivery-readiness"}).get("qualified")
        ):
            return False
        status = send({"action": "status"})
        if (
            status.get("input_owner")
            or status.get("handoff_granted")
            or status.get("manual_handoff")
            or any(
                row.get("pending") or row.get("manual_input_fence")
                for row in status.get("characters", {}).values()
            )
        ):
            return False
        receivers = []
        for name, row in status.get("characters", {}).items():
            ...
            if (
                ...
                or not all(
                    qualification.get(k) is True for k in ("trade", "trade_request")
                )
            ):
                continue
            ...
            if snapshot.get("trade") or snapshot.get("request") or row.get("ready"):
                return False
            # Unknown readiness failures are not capacity/booth evidence.
            if (
                available_slots(snapshot) > 0
                and snapshot.get("booth_open") is not False
            ):
                return False
    ...
    if not unavailable():
        return False
    approach_market_warehouse(
        loop, "Verifying the delivery scroll remains safely banked"
    )
    open_warehouse(loop)
    ...
    for source in (before, after):
        inventory = validate_snapshot(source, farmer_name(), time.time())
        if (
            source["identity"] != loop.health()["target"]
            or source.get("trade")
            or source.get("request")
            or uid in inventory
        ):
            raise ValueError("Deferred scroll farmer ownership changed")
    if (
        any(
            before[k] != after[k]
            for k in ("identity", "character_uid", "silver", "position")
        )
    ...
    if (
        len(matches) != 1
        or matches[0].get("type_id") != 720027
        or matches[0].get("quantity") != 1
        or matches[0].get("bound") is not False
        or any(matches[0].get(k) != 0 for k in ("plus", "gem1", "gem2"))
    ):
        raise ValueError(
            "Requested scroll is not freshly verified in storage; no deferral claimed"
        )
    if not unavailable():
        close_warehouse(loop)
        return False
    ...
    save(
        ...
        scroll_disposition={
            ...
            "warehouse": stored,
            "farmer": after,
            "reason": "receiver_capacity_or_booth_unavailable",
...
def prepare_market_scroll(loop, state, *, send=request):
    ...
    """
    if state.get("acceptance_scope"):
        return False  # Never withdraw unrelated trial stock, including after restart.
    from conquest.banking import open_warehouse, close_warehouse
    from conquest.meteor_banking import approach_market_warehouse

    if state.get("scroll_preparation_done"):
        return False
    ...
    if not state.get("scroll_withdrawal"):
        ...
        if pending_item:
            ...
            matches = [
                ...
                if all(
                    i.get(k) == pending_item.get(k)
                    for k in ("uid", "type_id", "amount", "plus")
                )
            ]
            if len(matches) != 1 or any(
                i["uid"] == pending_item["uid"] for i in bag["items"]
            ):
                raise ValueError(
                    "Leftover Meteor deposit is uncertain; no repeated warehouse input"
                )
            state["receipts"].append(
                {
                    "uid": pending_item["uid"],
                    "type_id": 1088001,
                    "verified_in_warehouse": True,
                }
            )
            save(state, loose_meteor_pending=None)
        if not state.get("loose_meteor_pending") and not preflight(
            loop, send, require_inventory=False
        ):
            defer_unavailable_stored_scroll(loop, state, send)
            return False
        approach_market_warehouse(
            loop, "Retrieving a stored MeteorScroll for merchant delivery"
        )
        open_warehouse(loop)
        while True:
            bag = loop.town("supplies")
            ...
            if len(stored["items"]) >= stored["capacity"]:
                raise ValueError(
                    "No warehouse space for loose Meteors; no delivery scroll withdrawn"
                )
        ...
        old = {
            "operation_id": "journey-scroll:" + uuid.uuid4().hex,
            "uid": selected["uid"],
        }
...
def resume(loop, *, send=request):
    ...
    if state.get("acceptance_scope"):
        ...
        if state["acceptance_scope"] != current_scope:
            if not journey_scope_completed(state["acceptance_scope"]):
                raise ValueError(
                    "Acceptance journey scope changed; reconcile without new input"
                )
            ...
            if deferred is not None:
                state["stored_scroll_uid"] = deferred
            save(state)
        if state.get("acceptance_scope"):
            native = read_json(delivery_route.STATE).get("active")
            if native and not pending_delivery_matches(native):
                raise ValueError(
                    "Acceptance delivery admission changed; reconcile without new input"
                )
        ...
        if (
            state.get("acceptance_scope")
            and stored_uid is not None
            and stored_uid != scoped_uid
    ...
    if world == 1036 and state["phase"] == "market":
        ...
        if rechecked_storage:
            recheck_stored_scroll(loop, state, send)
        if not rechecked_storage and not state.get("deposit_pending"):
            delivered_before = False
            close_scroll_preparation(loop, state, send)
            if (
                not state.get("scroll_preparation_done")
                and state.get("stored_scroll_uid") is None
                and not state.get("scroll_withdrawal")
            ...
            if state.get("scroll_withdrawal_receipt"):
                ...
                if disposition == "carried":
                    ...
                    state.setdefault("scroll_delivery_receipts", []).extend(
                        receipts or []
                    )
                    save(state)
                    scroll_ownership(state, send)
    ...
    if world == origin and state["phase"] == "return_pending":
        ...
        loop.record(
            "merchant_journey_complete",
            activity="Merchant delivery and storage complete; resuming the farming route",
        )
        return True
    raise ValueError(
        "Merchant journey arrival is uncertain; valuables preserved and no repeat fare issued"
    )
