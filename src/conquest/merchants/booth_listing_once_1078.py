...
def _selected(snapshot, request):
    matches = [
        item for item in snapshot["inventory"] if item["uid"] == request["item_uid"]
    ]
    if (
        len(matches) != 1
        or _item_fingerprint(matches[0]) != request["item_fingerprint"]
    ):
...
def _validate_snapshot(snapshot, profile, request):
    if (
        snapshot["identity"] != request["expected_identity"]
        ...
        or not snapshot["booth_open"]
        or snapshot["map_id"] != 1036
        or snapshot["hp"] <= 0
        or snapshot["trade"] is not None
        or snapshot["request"] is not None
        or len(snapshot["booth"]) >= 32
        or not 0 <= time.time() - snapshot["timestamp"] <= 1
    ):
        raise ValueError(
            "Merchant identity, owned booth, capacity, or closed trade changed"
        )
...
def _merchant_intent(runtime, character):
    return {
        "operations": runtime.journal.get(character, "enabled", False) is True,
...
def _policy(
    ...
    *,
    ...
    if not scope_allows(
        ...
        scheduled=scheduled_refill,
        cleanup=explicit_cleanup,
    ):
        raise CaptureUnavailable(
            "Listing scope belongs to another exact request or operation"
        )
    ...
    if (
        ...
        or any(
            current.get(key) != control.get(key)
            for key in ("enabled", "paused", "revision")
        )
        or control["enabled"]
        or control.get("paused")
        or not explicit_cleanup
        and not any(intent.values())
        or scheduled_refill
        and not intent["refill"]
        or merchant_intent is not None
        and (
            intent["refill"] != merchant_intent["refill"]
            if scheduled_refill
        ...
        and not (
            (scheduled_refill or explicit_cleanup or operator_scope)
            and getattr(ui, "grant", None)
        ...
        raise CaptureUnavailable(
            "One-shot listing stopped or lacks a safe farmer handoff"
        )
    ...
    if request_id is None:
        ...
        len(pending) != 1
        or pending[0]["id"] != request_id
        or pending[0]["kind"] != KIND
        or pending[0]["phase"] not in phases
    ):
...
def _grids(gui, snapshot, uid):
    ...
    if (
        len(columns) != 10
        or inventory["row_height"] != 40
        or any(
            columns[n + 1]["content_x"] - columns[n]["content_x"] != 40
            for n in range(len(columns) - 1)
        )
        or table["row_height"] != 64
    ):
    ...
    if (
        not cx + 2 < destination[0] < cx + cw - 2
        or not cy + 2 < destination[1] < cy + ch - 2
    ):
...
def _modal(gui, model, snapshot, uid):
    ...
    if (
        tuple(window["geometry"]) != (x, y, width, height)
    ...
    return (
        window,
        {
            "##Amount": (start_x + 64, button_y - 13),
            "OK": (x + 68, button_y + 9),
            "Cancel": (end_x - 60, button_y + 9),
        },
        candidate["candidate_price_buffer_text"],
    )
...
def _listed(before, after, request):
    if (
        ...
        or after["hp"] <= 0
        or not after["booth_open"]
        or after["trade"] is not None
        or after["request"] is not None
    ):
        return False
    uid = request["item_uid"]
    inventory_before, inventory_after = (
        _stock(before["inventory"]),
        _stock(after["inventory"]),
    )
    ...
    booth_prices_after = {
        item["uid"]: item["price"] for item in after["booth"] if item["uid"] != uid
    }
    expected = request["item_fingerprint"]
    matches = [
        item
        for item in after["booth"]
        if item["uid"] == uid
        and item["price"] == request["price"]
        and _item_fingerprint(item) == expected
    ]
    return (
        inventory_before.get(uid) == expected
        and uid not in booth_before
        and len(matches) == 1
        and uid not in inventory_after
        and inventory_after
        == {key: value for key, value in inventory_before.items() if key != uid}
...
def _status(journal, request_id, character, *, runtime=None):
    ...
    return {
        ...
        "confirmation_marker": any(step["stage"] == "confirm_press" for step in steps),
        ...
        "routine_refill_qualified": qualified,
...
def reconcile(ui, request_id, character):
    ...
    if any(step["stage"] == "cancel_press" for step in steps):
        if marker:
            raise ValueError(
                "Conflicting listing confirmation and cancellation markers need operator reconciliation"
            )
    ...
    with MemorySession(identity["pid"], CLIENT_SHA256_1078) as session:
        ...
        first = reader.read_manual_ownership()
        if any(
            win["name"] == "Add Item to Booth"
            for win in GuiReader.for_session(session).windows()
        ):
            return _status(journal, request_id, character)
        time.sleep(0.25)
        second = reader.read_manual_ownership()
        if any(first[key] != second[key] for key in OWNERSHIP_FIELDS) or any(
            win["name"] == "Add Item to Booth"
            for win in GuiReader.for_session(session).windows()
        ):
            return _status(journal, request_id, character)
...
def dispatch(ui, body, *, scheduled_refill=False):
    ...
    if set(body) != (fields if action == allowed[0] else common):
        raise ValueError(
            "One-shot listing requires an exact item, price, and process binding"
        )
    request_id = body["request_id"]
    if not isinstance(request_id, str) or not re.fullmatch(
        r"booth-list1078-[A-Za-z0-9_-]{8,80}", request_id
    ):
    ...
    if not scope_allows(
        ui, character, request_id=request_id, scheduled=scheduled_refill, body=body
    ):
        raise CaptureUnavailable(
            "Listing differs from the exact admitted operator request"
        )
    ...
    _policy(
        ...
        scheduled_refill=scheduled_refill,
    )
    if scheduled_refill and not ui.runtime.can_start_work(38):
        raise CaptureUnavailable(
            "Scheduled listing needs thirty-eight seconds remaining in the current safe grant"
        )
    from conquest.merchants.observe_1078 import observe

    observed = observe(ui.runtime, character, listing_preflight=True)
    if (
        observed["identity"] != body["expected_identity"]
    ...
    candidates = [
        client
        for client in ui.runtime.merchant_windows()
    ...
    if scheduled_refill and not ui.runtime.can_start_work(38):
        raise CaptureUnavailable(
            "Scheduled listing preflight consumed its safe work budget; no input started"
        )
    ...
    worker = threading.Thread(
        target=_run,
        args=(ui, character, profile, before, token),
        daemon=True,
        name="booth-1078-list-once",
    )
    try:
        with WORKER_LOCK:
...
def _type_price(target, price, guard, stage):
    ...
    if guard() != "" or any(keys(key) & 0x8000 for key in (0x10, 0x11, 0x12, 0x7B)):
        raise CaptureUnavailable(
            "Native amount field is not empty or a physical key is held"
        )
    digits = str(price)
    for index, digit in enumerate(digits):
        ...
        if any(keys(key) & 0x8000 for key in (0x10, 0x11, 0x12, 0x7B)):
            raise CaptureUnavailable(
                "Manual key or Stop interrupted the one-shot price"
            )
        stage("price_digit_" + str(index + 1), input_boundary=True)
        try:
            event = Input(
                type=1, data=InputUnion(ki=KeyboardInput(0, ord(digit), 4, 0, 0))
            )
            if send(1, c.byref(event), c.sizeof(Input)) != 1:
                raise OSError("Native booth price key input was incomplete")
        finally:
            event = Input(
                type=1, data=InputUnion(ki=KeyboardInput(0, ord(digit), 6, 0, 0))
            )
            if send(1, c.byref(event), c.sizeof(Input)) != 1:
    ...
    try:
        ...
        with (
            ...
            if session.identity != request["expected_identity"]:
                raise ValueError(
                    "Exact merchant process changed before one-shot listing"
                )
            gui = GuiReader.for_session(session)
            reader = open_read_only_1078(session, profile.name)
            ...
            def policy():
                _policy(
                    ...
                    request_id=request_id,
                    deadline=deadline,
                    farmer_target=before["farmer_target"],
                ...
                with coordinator.lease(character, purpose=KIND):
                    ...
                    if before.get("scheduled_foreground_refill"):
                        ...
                        require(
                            journal,
                            character,
                            {**baseline, "client_sha256": session.expected_sha256},
                        )
                    ...
                    if not activate_client(target.hwnd, session.identity):
                        raise CaptureUnavailable(
                            "Exact merchant foreground activation was denied"
                        )
                    focus_verified = True
                    check()
                    baseline = reader.read_manual_ownership()
                    ...
                    if not _same_quote(
                        before["price_plan"],
                        _price_plan(
                            ui, character, baseline, _selected(baseline, request)
                        ),
                    ):
                        raise ValueError(
                            "Reliable item price or listing priority changed"
                        )
                    if any(win["name"] == "Add Item to Booth" for win in gui.windows()):
                        raise ValueError(
                            "Existing price dialog prevents one-shot listing"
                        )
                    if any(
                        baseline[key] != before["snapshot"][key]
                        for key in OWNERSHIP_FIELDS
                        if key in before["snapshot"]
                    ):
                        raise ValueError(
                            "Stock changed after the operator request was recorded"
                        )
                    ...
                    def fresh(*, modal=None):
                        ...
                        if changed:
                            raise ValueError(
                                "Exact merchant ownership changed: "
                                + ", ".join(changed)
                            )
                        ...
                        if current_grids != grids:
                            ...
                            for key in grids:
                                if current_grids[key] == grids[key]:
                                    continue
                                if isinstance(grids[key], dict) and isinstance(
                                    current_grids[key], dict
                                ):
                                    changed.extend(
                                        key + "." + field
                                        for field in sorted(
                                            set(grids[key]) | set(current_grids[key])
                                        )
                                        if grids[key].get(field)
                                        != current_grids[key].get(field)
                                    )
                                else:
                                    changed.append(key)
                            raise ValueError(
                                "Exact merchant grid changed: " + ", ".join(changed)
                            )
                        if gui.model(25, booth_vtable) != model:
                            raise ValueError(
                                "Exact merchant booth model pointer changed"
                            )
                        if (
                            unpack(gui.session, model + 0x4C, "<I")[0]
                            != baseline["own_booth_uid"]
                        ):
                            raise ValueError(
                                "Exact merchant displayed booth owner changed"
                            )
                        present = any(
                            win["name"] == "Add Item to Booth" for win in gui.windows()
                        )
                    ...
                    def point(logical):
                        result = tuple(
                            round(value * physical / logical_size)
                            for value, physical, logical_size in zip(
                                logical, revision.client_size, revision.gui_size
                            )
                        )
                        if any(
                            ...
                            raise ValueError(
                                "Listing input candidate is outside native client"
                            )
                        return result

                    source, destination = (
                        point(grids["source"]),
                        point(grids["destination"]),
                    )
                    fresh(modal=False)
                    journal.step(
                        request_id,
                    ...
                    stage(
                        "drag_pointer", {"source": source, "destination": destination}
                    )
                    ...
                    foreground_drag(
                        ...
                        before_press=drag_press,
                        before_release=drag_release,
                    ...
                    while not any(
                        ...
                        if time.monotonic() >= until:
                            raise ValueError(
                                "One-shot drag outcome unverified; no repeat"
                            )
                        time.sleep(0.03)
                    revision = layout.stable()
                    current = fresh(modal=True)
                    window, controls, text = _modal(
                        gui, model, current, request["item_uid"]
                    )
                    from conquest.merchants.listing_preflight_1078 import (
                        _MODAL_RENDER_SHA256,
                    )
                    ...
                    def dialog_guard(label, *, expected_text=None):
                        ...
                        while True:
                            ...
                            win, points, value = _modal(
                                gui, model, now, request["item_uid"]
                            )
                            if (
                                (win, points) != binding
                                or expected_text is not None
                                and value != expected_text
                            ):
                                raise ValueError(
                                    "Exact item, price dialog, or amount changed"
                                )
                            try:
                    ...
                    _type_price(target, request["price"], typing_guard, stage)
                    raw_price = lambda: gui.session.read_block(model + 0x54, 12).split(
                        b"\0"
                    )[0]
                    wait_booth_price(raw_price, request["price"], check)
                    displayed = _modal(
                        gui, model, fresh(modal=True), request["item_uid"]
                    )[2]
                    if parse_booth_price(displayed.encode("ascii")) != request["price"]:
                        raise ValueError(
                            "Native price buffer differs from the verified quote"
                        )
                    journal.step(
                        request_id,
                        "price",
                    ...
                    def confirm_press():
                        ...
                        if not _same_quote(
                            ...
                            _price_plan(
                                ...
                                character,
                                quote_snapshot,
                                _selected(quote_snapshot, request),
                            ),
                        ):
                            raise ValueError(
                                "Reliable item price or listing priority changed before submission"
                            )
                        confirm_guard()
                    ...
                    while time.monotonic() < until:
                        ...
                        try:
                            if (
                                reconcile(ui, request_id, character)["phase"]
                                == "verified"
                            ):
                                return
                        ...
                        time.sleep(0.1)
                    raise CaptureUnavailable(
                        "Listing submitted; exact booth ownership needs reconciliation"
                    )
            ...
            if not unchanged:
                journal.set(
                    ...
                    "attention",
                    {
                        ...
                        "note": "One-shot 1078 listing needs read-only reconciliation: "
                        + str(error),
                    },
                )
