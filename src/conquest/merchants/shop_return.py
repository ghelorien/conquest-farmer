...
class ShopReturn:
    ...
    def remember(self, snapshot):
        if (
            snapshot.get("map_id") == 1036
            and snapshot.get("booth_open")
            and snapshot.get("position")
        ):
    ...
    def begin(self):
        if self.state() and self.state()["phase"] not in (
            "complete",
            "operator_overridden",
        ):
            return
        ...
        with self.journal.db() as db:
            row = db.execute(
                "SELECT snapshot FROM sales_baseline WHERE character=?",
                (self.character,),
            ).fetchone()
        ...
        state = {
            ...
            "stalls": 0,
            "before": before,
            "home": self.journal.get(self.character, "shop_home"),
        }
        self.save(state)
    ...
    def block(self, state, note):
        ...
        self.journal.set(
            self.character, "attention", {"kind": "shop_return", "note": note}
        )
        raise ValueError(note)

    def step(self, snapshot, controller, travel):
        state = self.state()
        if not state or state["phase"] in ("complete", "operator_overridden"):
            return True
        ...
        if snapshot.get("trade") or snapshot.get("request"):
            raise CaptureUnavailable(
                "Return to Market waits for the open trade to be resolved"
            )
        if state["moves"] >= 300 or state["stalls"] >= 6:
            self.block(
                state, "Return to Market is obstructed; route retry requires attention"
            )
        world = snapshot["map_id"]
        if world not in (1002, 1036):
            self.block(
                state,
                "Unexpected map after reconnect; no unverified route will be used",
            )
        ...
        if state["phase"] == "transfer_submitted":
            if world == 1036 and state["transfer_silver"] - snapshot["silver"] == 100:
                ...
                self.block(
                    state,
                    "Conductress transfer has an uncertain result; no repeat fare was sent",
                )
        if world == 1002:
            if max(abs(a - b) for a, b in zip(snapshot["position"], (438, 444))) > 2:
                self.move(state, snapshot, controller, travel, (438, 444))
                return False
        ...
        if snapshot.get("own_booth_uid") and not snapshot["booth_open"]:
            if state["phase"] == "panel_submitted":
                if self.clock() - state["submitted_at"] <= 10:
                    return False
                self.block(
                    state, "Owned booth panel did not open; verify it before retrying"
                )
            with controller.coordinator.lease(self.character):
                controller.check()
                travel.driver.require_qualified("booth_panel")
                self.save(
                    state, "opening_panel", own_booth_uid=snapshot["own_booth_uid"]
                )
                travel.open_owned_booth(
                    snapshot,
                    controller.check,
                    before_press=lambda: self.save(
                        state, "panel_submitted", submitted_at=self.clock()
                    ),
                )
            return False
        home = state.get("home")
        if not home and not snapshot.get("own_booth_uid"):
            self.block(
                state,
                "No memory-verified Market booth location was saved before disconnect",
            )
        if not snapshot["booth_open"]:
            if state["phase"] == "shop_submitted":
                ...
                self.block(
                    state,
                    "Shop setup has an uncertain result; verify the stall before retrying",
                )
            chosen = travel.stall(home["position"])
            if (
                max(
                    abs(a - b) for a, b in zip(snapshot["position"], chosen["position"])
                )
                > 1
            ):
                self.move(
                    state, snapshot, controller, travel, tuple(chosen["position"])
                )
                return False
            with controller.coordinator.lease(self.character):
                controller.check()
        ...
        if not before:
            self.block(
                state,
                "Pre-disconnect stock is unavailable; reconcile inventory before restoring listings",
            )
        ...
        if expected != current:
            self.block(
                state,
                "Stock changed across disconnect; reconcile missing/new items before restoring the shop",
            )
        if not any(w["name"] == "Inventory" for w in snapshot.get("windows", [])):
            if state["phase"] == "inventory_submitted":
                if self.clock() - state["submitted_at"] <= 10:
                    return False
                self.block(
                    state, "Inventory panel did not open; verify it before retrying"
                )
            with controller.coordinator.lease(self.character):
                controller.check()
                travel.driver.require_qualified("inventory_panel")
                self.save(state, "opening_inventory")
                travel.open_inventory(
                    snapshot,
                    controller.check,
                    before_press=lambda: self.save(
                        state, "inventory_submitted", submitted_at=self.clock()
                    ),
                )
            return False
        ...
        for item in wanted:
            ...
            actual = next(
                i
                for i in snapshot["booth"] + snapshot["inventory"]
                if i["uid"] == item["uid"]
            )
            if actual.get("price") == item["price"]:
                continue
            if actual.get("price") is None and len(snapshot["booth"]) >= 32:
                self.block(
                    state,
                    "Booth capacity changed during restoration; remaining valuables are queued",
                )
            ...
            plan = {
                ...
                "name": item["name"],
                "price": item["price"],
                "old_price": actual.get("price"),
                "observed_at": self.clock(),
                "attributes": list(current[item["uid"]]),
                "reason": "Restore verified pre-disconnect listing",
            }
        ...
        matching_trial = bool(
            trial.get("phase") == "recovering"
            and trial_before
            and trial_before.get("character_uid") == before.get("character_uid")
            and stock(trial_before) == stock(before)
        )
        interventions = state.get("manual_interventions", []) or (
            trial.get("manual_interventions", []) if matching_trial else []
        )
        self.save(
            state,
            "complete",
            completed_at=self.clock(),
            automatic_recovery_verified=not bool(interventions),
            completion_kind="assisted_recovery"
            if interventions
        ...
        if matching_trial:
            trial.update(
                phase="assisted" if interventions else "verified",
    ...
    def operator_override(
        ...
        *,
        ...
        with self.journal.db() as db:
            ...
            row = db.execute(
                "SELECT value FROM state WHERE character=? AND name='shop_return'",
                (character,),
            ).fetchone()
            ...
            override = {
                ...
                "fresh_evidence": {"snapshot": snapshot},
            }
            state.update(
            ...
            if row:
                db.execute(
                    "UPDATE state SET value=? WHERE character=? AND name='shop_return'",
                    (encoded, character),
                )
            else:
                db.execute(
                    "INSERT INTO state(character,name,value) VALUES(?,?,?)",
                    (character, "shop_return", encoded),
                )
            ...
            db.execute(
                "INSERT OR REPLACE INTO state(character,name,value) VALUES(?,?,?)",
                (character, "new_stock", "true"),
            )
        self.journal.event(
            self.character,
            "shop_return_operator_overridden",
            original_evidence_digest=digest,
            confirmation_reference=confirmation_reference.strip(),
        )
        return state
    ...
    def move(self, state, snapshot, controller, travel, destination):
        ...
        progressed = (
            after["map_id"] == snapshot["map_id"]
            and after["position"] != snapshot["position"]
        )
