...
def stall_approach(terrain, position, flag):
    ...
    for target in ((x - 2, y), (x, y - 2), (x, y + 2)):
        try:
            path = terrain.travel_path(tuple(position), target)
        except ValueError:
            continue
        candidates.append((len(path), target))
    if not candidates:
        raise ValueError("No checked stall approach is available")
...
class ReturnDriver:
    ...
    def read(self):
        if self.travel_only:
            from conquest.merchants.memory import (
                TransitObservationChanged,
                GuiObservationChanged,
            )

            deadline = time.monotonic() + 0.4
            while True:
                try:
                    return self.driver.memory.read_travel()
    ...
    def qualify_movement(self):
        ...
        if self.driver.memory.gui.viewport_size() != profile.get("gui_size") or list(
            self.driver.target.snapshot()["client_size"]
        ) != profile.get("client_size"):
    ...
    def click(self, point, check, *, before_press, jump=False):
        ...
        with physical_coordinates():
            ...
            gui = profile["gui_size"]
            size = profile["client_size"]
            physical = tuple(round(v * p / g) for v, p, g in zip(point, size, gui))

            def guard():
                check()
                self.observer.adapter.assert_identity()
                if self.qualify_movement() != profile:
                    raise ValueError("Route calibration changed")
                before_press()

            diagnostics = {}
            result = foreground_click(
                self.driver.target,
                *physical,
                tuple(size),
                control=jump,
    ...
    def move(self, snapshot, destination, check):
        for attempt in range(4):
            try:
                return self._move(snapshot, destination, check)
    ...
    def _move(self, snapshot, destination, check):
        ...
        if snapshot["map_id"] not in self.terrains:
            self.terrains[snapshot["map_id"]] = read_terrain(
                merchant_installation(self.driver.observer.character),
                snapshot["map_id"],
            )
        ...
        def point_now():
            ...
            if struct.unpack_from("<2I", raw) != tuple(life.position):
                raise TravelPlanChanged("Route anchor changed")
            ...
            if not clear_segment(terrain, life.position, target):
                raise ValueError("Route segment became blocked")
            if o.adapter.read_block(life.object_address + 0xD8, 24) != raw:
                raise TravelPlanChanged("Route projection changed")
            return point
        ...
        def guard():
            if point_now() != point:
                raise TravelPlanChanged("Projected return waypoint moved")
            state = self.read()
            if state.get("trade") or state.get("request"):
                raise CaptureUnavailable("Trade interrupted the return route")
        ...
        self.last_move["input"] = self.click(
            point,
            check,
            before_press=guard,
        ...
        while time.monotonic() < end:
            check()
            after = self.read()
            if after["map_id"] != snapshot["map_id"]:
                raise ValueError("Unexpected map change during return movement")
            self.last_move["after"] = after["position"]
            if tuple(after["position"]) == target:
                return after
            time.sleep(0.1)
        return after
    ...
    def prepare_transfer(self, snapshot, check):
        ...
        self.qualify_movement()
        if snapshot["silver"] < 100:
            raise ValueError("Merchant needs 100 silver for the Market transfer")
        ...
        def guard():
            ...
            if read_conductress(self.observer) != npc:
                raise ValueError("Conductress moved before input")
            fresh = self.read()
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                if not any(
                    r["kind"] == 0 and "100 silver" in r["text"] for r in records
                ):
    ...
    def transfer(self, records, check):
        ...
        def guard():
            if (
                ...
                or dialog_point(self.observer, "Market", records) != point
            ):
    ...
    def stall(self, preferred):
        ...
        if spec["claim_mode"] == "dialog" and (
            not spec.get("records") or not spec.get("option")
        ):
        ...
        terrain = read_terrain(
            merchant_installation(self.driver.observer.character), 1036
        )
        position = self.read()["position"]
        for flag in sorted(
            candidates,
            key=lambda f: max(abs(a - b) for a, b in zip(f["position"], preferred)),
        ):
            try:
                _, approach = stall_approach(terrain, position, flag)
            except ValueError:
                continue
        ...
        raise ValueError(
            "No memory-verified vacant reachable stall is available nearby"
        )
    ...
    def open_owned_booth(self, snapshot, check, *, before_press=lambda: None):
        ...
        def guard():
            check()
            fresh = self.driver.memory.read(recovery=True)
            if (
                fresh.get("booth_open")
                or fresh.get("trade")
            ...
            if owned_booth_target(self.observer, target) != tile_target:
                raise ValueError(
                    "Owned booth tile target changed before opening its panel"
                )
    ...
    def open_inventory(self, snapshot, check, *, before_press=lambda: None):
        ...
        def guard():
            ...
            fresh = self.driver.memory.read(recovery=True)
            if (
                any(w["name"] == "Inventory" for w in fresh["windows"])
                or fresh.get("trade")
                or fresh.get("request")
    ...
    def prepare_shop(self, chosen, check):
        ...
        if flag is None:
            raise CaptureUnavailable(
                "Selected stall was occupied; another will be selected"
            )
        fresh = self.read()
        check()
        if fresh.get("own_booth_uid"):
            raise ValueError("Merchant already owns a booth; reopen its panel")
        if (
            fresh["map_id"] != 1036
            or max(abs(a - b) for a, b in zip(fresh["position"], chosen["position"]))
            > 1
        ):
    ...
    def start_shop(self, prepared, check):
        ...
        target = flag_target(self.observer, flag)
        point = target["point"]
        ...
        def guard():
            ...
            fresh = self.read()
            if fresh.get("own_booth_uid"):
                raise ValueError("Merchant already owns a booth; no new flag claim")
            if any(fresh[k] != snapshot[k] for k in ("identity", "map_id", "position")):
                raise ValueError("Merchant moved before claiming the stall")
            if fresh.get("trade") or fresh.get("request"):
                raise ValueError("Trade interrupted stall setup")
            current = next(
                (
                    f
                    for f in vacant_flags(self.observer, spec)
                    if f["uid"] == flag["uid"]
                ),
                None,
            )
            if current != flag:
                raise ValueError("Stall was occupied or changed before claiming it")
            ...
            for window in fresh["windows"]:
                ...
                if w >= width - 10 and h >= height - 10:
                    continue
                if (
                    x <= target["point"][0] <= x + w
                    and y <= target["point"][1] <= y + h
                ):
        ...
        if spec["claim_mode"] == "native_confirm":
            ...
            if spec.get("confirmation") != CONFIRM_CONTROL:
                raise ValueError("Native booth confirmation needs qualification")
            deadline = time.monotonic() + 3
            while time.monotonic() < deadline:
                check()
                now = self.driver.memory.read()
                if any(w["name"] == "Open Booth###Confirm" for w in now["windows"]):
                    break
                time.sleep(0.1)
            else:
                raise ValueError("Native Open Booth confirmation was not observed")
        ...
        if spec["claim_mode"] == "dialog":
            ...
            while time.monotonic() < deadline:
                check()
                try:
                    point = dialog_point(self.observer, spec["option"], spec["records"])
                    break
                except ValueError:
                    time.sleep(0.1)
            else:
                raise ValueError("Stall setup dialog was not verified")

            def confirm_guard():
                guard()
                if (
                    dialog_point(self.observer, spec["option"], spec["records"])
                    != point
                ):
