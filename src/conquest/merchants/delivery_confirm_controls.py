...
def confirm_control(gui, snapshot):
    ...
    if gui.session.expected_sha256 == CLIENT_SHA256_1078:
        ...
        w, point, _, seeds = locate(
            SimpleNamespace(memory=SimpleNamespace(gui=gui)),
            snapshot,
            "native_trade_confirm",
        )
        return w, point, seeds[0]
    for rva, code in (
        (0x10FA38, "488d0dd9b74b00"),
        (0x10FA48, "41c6869800000001"),
        (0x10FA7A, "e8311a0700"),
    ):
    ...
    if (
        len(outer["columns"]) != 3
        or len(bar["columns"]) != 2
        or bar["row_height"] != 18
    ):
        raise ValueError("Trade confirmation layout changed")
    l, t, r, b = bar["outer"]
    point = (round((l + r) / 2), round(b + 4 + 9))
