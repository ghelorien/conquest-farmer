...
def operator_override(
    ...
    *,
    ...
    with runtime.journal.db() as db:
        ...
        row = db.execute(
            "SELECT value FROM state WHERE character=? AND name='market_safety'",
            (character,),
        ).fetchone()
        ...
        if state.get("phase") == "operator_overridden":
            ...
            return state
        if not state:
            raise ValueError("No active Market safety hold is available")
        ...
        if row:
            db.execute(
                "UPDATE state SET value=? WHERE character=? AND name='market_safety'",
                (encoded, character),
            )
        else:
            db.execute(
                "INSERT INTO state(character,name,value) VALUES(?,?,?)",
                (character, "market_safety", encoded),
            )
        hold = db.execute(
            "SELECT value FROM state WHERE character=? AND name='connect_hold'",
            (character,),
        ).fetchone()
        ...
        matching_attention = (
            attention_value is None
            or isinstance(attention_value, dict)
            and attention_value.get("kind") == "market_safety"
        )
        ...
        if hold and json.loads(hold[0]) is True and matching_attention:
            db.execute(
                "UPDATE state SET value='false' WHERE character=? AND name='connect_hold' AND value=?",
                (character, hold[0]),
            )
    ...
    runtime.journal.event(
        character,
        "market_safety_operator_overridden",
        original_evidence_digest=digest,
        confirmation_reference=confirmation_reference.strip(),
    )
    return state
...
def protect(runtime, character, identity, reason, *, close=None):
    ...
    runtime.journal.set(
        ...
        "attention",
        {
            "kind": "market_safety",
            "note": reason
            + "; automation paused, client kept connected. User assistance required.",
        },
    )
    result = {
        "time": time.time(),
...
class MarketGuard:
    ...
    def check(
        ...
        if (
            coordinator
            and hasattr(coordinator, "manual_session_blocked")
        ...
        try:
            ...
            if life.map_id == 1036 and not life.dead_candidate:
                return
            reason = (
                f"Merchant outside safe Market (map {life.map_id}) or death detected"
            )
        ...
        while not r.stop_event.is_set():
            try:
                ...
                for character in CHARACTERS:
                    ...
                    if cached and (
                        client is None or cached.adapter.identity != identity
                    ):
                        cached.close()
                        self.readers.pop(character, None)
                        self.unknown_since.pop(character, None)
                    if client is None:
                        continue
                    if character not in self.readers:
                        ...
                        try:
                            self.readers[character] = r.observer_factory(
                                client, character
                            )
                        except Exception:
                            ...
                            if time.monotonic() - started >= 2:
                                ...
                                if (
                                    ...
                                    continue
                                if active(r, character):
                                    observe(r, character, identity)
                                else:
                                    protect(
                                        r,
                                        character,
                                        identity,
                                        "Merchant safety reader unavailable for two seconds",
                                    )
                            continue
                    try:
