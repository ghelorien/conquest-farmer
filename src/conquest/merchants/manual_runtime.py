...
def _probe_history_failure(error):
    ...
    while trace is not None:
        if trace.tb_frame.f_code is ManualSessionStore._probe_pair.__code__:
            ...
            check, role, index = (
                scope.get(key) for key in ("history_check", "role", "index")
            )
            if isinstance(check, str) and check in PROBE_HISTORY_CHECKS:
                result["history_check"] = check
            if isinstance(role, str) and role in ("farmer", "merchant"):
                result["history_role"] = role
            if type(index) is int and 0 <= index <= 10000000:
                result["history_index"] = index
        trace = trace.tb_next
    return result
...
def probe_attempt_projection(value):
    ...
    enums = {
        ...
        "reason": {
            ...
            "probe_changed",
            "hold_read_failed",
            "reader_or_decline_hold",
            "manual_history_validation_failed",
            "exact_bot_history",
            "manual_history_unverified",
            "fence_sync_failed",
        },
        "outcome": {"error", "protected", "validated", "reconciled"},
        "checked_through": {"final_probe_recheck"},
        "history_check": PROBE_HISTORY_CHECKS,
        "history_role": {"farmer", "merchant"},
        "error_type": {
            "ValueError",
            "OSError",
            "TypeError",
    ...
    if type(value.get("bot_owned")) is bool:
        result["bot_owned"] = value["bot_owned"]
    ...
    for key in ("target_profile_id", "farmer_profile_id"):
        ...
        if (
            ...
            and 0 < len(item) <= 128
            and item.isascii()
            and all(c.isalnum() or c in "_-." for c in item)
        ):
            result[key] = item
    for key in (
...
class ManualRuntime:
    ...
    def init_manual_sessions(self):
        ...
        self.manual_sessions = ManualSessionStore(
            self.journal.path, on_settlement=self._manual_settlement
        )
        ...
        for row in stale:
            start_rebaseline(self.journal, row["target_profile_id"], row["id"])
        self.manual_farmer_provider = lambda: None
        self.manual_1078_registry = None
        self.manual_farmer_control = lambda: {"enabled": False}
        self.manual_farmer_observation = {
            "available": False,
            "reason": "Farmer observer is not configured",
        }
    ...
    def _manual_settlement(self, db, session, snapshot, receipt):
        ...
        profiles = registry()
        role = (
            profiles.resolve(session["target_profile_id"]).role
            if profiles
            else "Farmer"
            if session["target_profile_id"] == "Farmer"
            else "Merchant"
        )
    ...
    def _manual_get(self, character, name, default=None):
        with self.journal.db() as db:
            row = db.execute(
                ...
                (self.manual_target(character), name),
            ).fetchone()
    ...
    def _manual_set(self, character, name, value):
        with self.journal.db() as db:
            db.execute(
                ...
                (self.manual_target(character), name, json.dumps(value)),
            )
    ...
    def _manual_rows(self):
        ...
        with self.manual_sessions.db() as db:
            ids = [
                ...
                for row in db.execute(
                    "SELECT id FROM manual_sessions WHERE phase NOT IN ('completed','request_withdrawn','declined_verified','operator_overridden')"
                )
            ]
        ...
        with self.journal.db() as db:
            holds = [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT value FROM state WHERE name='manual_reader_hold' AND value!='null'"
                )
            ]
        for hold in holds:
            if hold and not any(
                row["target_profile_id"] == hold["target_profile_id"] for row in rows
            ):
                rows.append(hold)
    ...
    def manual_status(self, character=None):
            ...
            rows = [
                next(
                    (
                        row
                        for row in rows
                        if row["target_profile_id"] == self.manual_target(character)
                    ),
                    None,
                )
            ]
        ...
        def view(row):
            ...
            return {
                **row,
                "fence_scope": "global" if row["ever_approved"] else "target",
        ...
        with self.coordinator.lock:
            ...
            if self._manual_rows():
                raise ManualSessionError(
                    "Resolve the existing manual/rebaseline hold before starting a global handoff"
                )
            if self.farmer_bot_owned() or any(
                self.journal.pending(character) for character in CHARACTERS
            ):
                raise ManualSessionError(
                    "Wait for the active bot transaction to reconcile before starting handoff"
                )
            ...
            if registry is not None and registry.activate_if_present():
                ...
                if farmer is None:
                    raise ManualSessionError(
                        "Attach the Farmer memory observer before starting handoff"
                    )
                participants = {self.manual_target("Farmer"): "Farmer"}
                for character in CHARACTERS:
                    if character in self.observers:
                        participants[self.manual_target(character)] = "Merchant"
            row = self.manual_handoff.start(
                participants, operator=operator, identities=identities, now=now
            )
            self._sync_manual_fence()
            return row
    ...
    def poll_manual_1078(self, *, now=None):
        ...
        if (
            registry is None
            or active is None
            or not (registry.read_only_build or registry.bindings)
        ):
            return None
        from conquest.merchants.manual_reader_registry_1078 import (
            ManualReaderIdentityChanged1078,
        )
        ...
        if not registry.bindings:
            try:
                registry.discover(
                    profile_ids=[
                        row["target_profile_id"] for row in active["participants"]
                    ],
                    expected_identities=self.manual_handoff.participant_identities(
                        active["id"]
                    ),
                )
                ...
                for row in active["participants"]:
                    self.manual_handoff.unavailable(
                        row["target_profile_id"], str(error), now=now
                    )
                self._sync_manual_fence()
                return self.manual_handoff.active()
        ...
        for target, binding in registry.bindings.items():
            try:
                snapshot = registry.read_one(target)
            except ManualReaderIdentityChanged1078 as error:
                self.manual_handoff.identity_changed(target, str(error), now=now)
                continue
            except (ValueError, OSError) as error:
                try:
                    registry.detect_replacement(target)
                except ManualReaderIdentityChanged1078 as changed:
                    self.manual_handoff.identity_changed(
                        changed.target, str(changed), now=now
                    )
                    continue
            ...
            character = (
                "Farmer"
                if binding.role == "Farmer"
                else next(
                    (item for item in CHARACTERS if self.manual_target(item) == target),
                    None,
                )
            )
            busy = bool(
                ...
                or (
                    self.farmer_bot_owned()
                    if binding.role == "Farmer"
                    else character is None
                    or reservation(self.journal, character)
                    or self.journal.pending(character)
                )
            )
            # Observe each process immediately: the next process cannot age
            # this snapshot past the store's two-second freshness boundary.
            self.manual_handoff.observe(
                target, snapshot, bot_busy=busy, mouse_idle=not mouse_active(), now=now
            )
        self._sync_manual_fence()
        return self.manual_handoff.active()
    ...
    def end_manual_handoff(self, session_id, *, operator="local UI", now=None):
        with self.coordinator.lock:
            try:
                return self.manual_handoff.end(session_id, operator=operator, now=now)
            finally:
                self._sync_manual_fence()
    ...
    def observe_manual_handoff(self, character, snapshot, *, now=None):
        ...
        try:
            ...
            busy = bool(
                self.coordinator.owner or self.farmer_bot_owned()
                if is_farmer_owner(character)
                else self.coordinator.owner
                or reservation(self.journal, character)
                or self.journal.pending(character)
            )
            from conquest.mouse_priority import active as mouse_active

            row = self.manual_handoff.observe(
                target, snapshot, bot_busy=busy, mouse_idle=not mouse_active(), now=now
            )
    ...
    def _manual_character(self, target):
        if target == self.manual_target("Farmer"):
            return "Farmer"
        matches = [
            character
            for character in CHARACTERS
            if self.manual_target(character) == target
        ]
        if len(matches) != 1:
            raise ManualSessionError("Manual target is not an attached profile")
        return matches[0]

    def approve_manual(self, binding, *, operator="local", now=None):
        if not isinstance(binding, dict):
            raise ManualSessionError("Exact displayed approval binding is required")
        ...
        with self.coordinator.lock:
            ...
            if (
                self.farmer_bot_owned()
                if farmer
                else active(self.journal, character) or self.journal.pending(character)
            ):
                raise ManualSessionError(
                    "Bot transaction must reconcile before manual approval"
                )
            observer = (
                self.manual_farmer_provider()
                if farmer
                else self.observers.get(character)
            )
            controller = (
                self.manual_farmer_controller
                if farmer
                else self.controllers.get(character)
            )
            if observer is None or (not farmer and controller is None):
                raise ManualSessionError(
                    "Fresh attached target memory is required for approval"
                )
            if farmer:
                ...
                context = current()
                if (
                    observer.character != farmer_name()
                    or registry() is not None
                    and (context is None or context.profile.role != "Farmer")
                ):
                    raise ManualSessionError(
                        "Fresh exact selected Farmer profile is required"
                    )
            if not observer.lock.acquire(blocking=False):
                raise ManualSessionError(
                    "Memory observer is busy; retry approval with fresh evidence"
                )
            try:
            ...
            if self.process_probe_owned(character, snapshot, now=now):
                raise ManualSessionError(
                    "Supervised bot probe owns this exact request; manual approval is unavailable"
                )
            try:
                return self.manual_sessions.allow_and_activate(
                    binding, snapshot, operator=operator, now=now
                )
            finally:
                self._sync_manual_fence()
    ...
    def reject_manual(self, binding, *, operator="local", now=None):
        with self.coordinator.lock:
            try:
                return self.manual_sessions.reject(binding, operator=operator, now=now)
            finally:
                self._sync_manual_fence()
    ...
    def override_manual(
        ...
        if not all(
            ...
            for value in (confirmation_reference, operator, reason)
        ):
            raise ManualSessionError(
                "Explicit confirmation, operator and reason are required"
            )
        from conquest.merchants.manual_recovery import (
            start_rebaseline,
            reset_rebaseline,
        )
        ...
        with self.coordinator.lock:
            try:
                ...
                if not row["holds_automation"]:
                    ...
                    return self.manual_sessions.operator_override(
                        session_id,
                        confirmation_reference=confirmation_reference,
                        operator=operator,
                        reason=reason,
                ...
                start_rebaseline(
                    self.journal, row["target_profile_id"], session_id, now=now
                )
                return self.manual_sessions.operator_override(
                    session_id,
                    confirmation_reference=confirmation_reference,
    ...
    def _structural_request_probe_owned(self, character, snapshot, state, *, now):
        ...
        from conquest.merchants.delivery_request_reconciliation import (
            ownership as request_ownership,
        )
        ...
        try:
            ...
            farmer = intent["farmer"]
            if (
                not isinstance(state, dict)
                or state.get("phase") not in REQUEST_PHASES
                or snapshot.get("trade") is not None
                or not isinstance(request, dict)
                or state.get("character") != str(character)
                or state.get("target_profile_id", state.get("character"))
                != self.manual_target(character)
                or state.get("farmer_profile_id", "Farmer")
                != self.manual_target("Farmer")
            ...
            for participant in (farmer, merchant):
                ...
                if (
                    participant.get("map_id") != 1036
                    or type(timestamp) not in (int, float)
                    or not math.isfinite(timestamp)
                    or not 0 <= started - timestamp <= 5
                ):
                    return False
            if farmer.get("identity") == merchant.get("identity"):
                return False
            ...
            if any(
                snapshot.get(key) != merchant.get(key)
                for key in (
                    "identity",
                    "character",
                    "character_uid",
                ...
                return False
            if (
                request.get("participant") != farmer.get("character")
                ...
                != f"{farmer.get('character')} wishes to trade with you."
            ):
                return False
            recipient = state.get("recipient", {})
            if any(
                recipient.get(left) != merchant.get(right)
                for left, right in (
                    ("uid", "character_uid"),
                    ("name", "character"),
                    ("position", "position"),
                ...
                return False
            selected = state.get("selected_uids")
            ...
            if state["phase"] != "request_submitted":
                ...
                if not isinstance(saved, dict) or any(
                    saved.get(key) != snapshot.get(key)
                    for key in (
                        "identity",
                        "character",
                        "character_uid",
                ...
                if not isinstance(saved_farmer, dict):
                    return False
                if state["phase"] == "request_verified":
                    verified_at = state.get("updated_at")
                    if (
                        type(verified_at) not in (int, float)
                        or not math.isfinite(verified_at)
                        or not started <= verified_at <= now
                    ):
                        return False
                    for receipt in (saved_farmer, saved):
                        ...
                        if (
                            ...
                            or not started <= observed_at <= verified_at
                        ):
                            return False
                saved_now = max(
                    saved_farmer.get("timestamp", float("inf")),
                    saved.get("timestamp", float("inf")),
                )
            ...
            if (
                {**live, "request": None} != {**expected_proof, "request": None}
            ...
            return evidence_digest(delivery_probe.read_probe()) == evidence_digest(
                state
            )
    ...
    def _structural_trade_probe_owned(self, character, snapshot, state, *, now):
        ...
        try:
            ...
            proof = local_ownership(
                state,
                merchant,
                self.manual_target(merchant),
                self.manual_target("Farmer"),
                "farmer" if is_farmer_owner(character) else "merchant",
                snapshot,
                now=now,
            )
    ...
    def process_probe_owned(
        self,
        character,
        snapshot,
        *,
        farmer_snapshot=None,
        now=None,
        ...
        with self._manual_observation_scope(snapshot, now=now) as deferred:
            if deferred:
                return False if require_bilateral else OBSERVATION_DEFERRED
            state = None
            try:
                ...
                if (
                    not state
                    or not farmer_side
                    and state.get("character") != str(character)
                    or state.get("phase") not in REQUEST_PHASES | TRADE_PHASES
                ):
                    return False
                merchant_character = (
                    character_name(state["character"]) if farmer_side else character
                )
                ...
                def defer():
                    ...
                    if state[
                        "phase"
                    ] in TRADE_PHASES and self._structural_trade_probe_owned(
                ...
                if (
                    farmer_snapshot is not None
                    and snapshot.get("request")
                    and state["phase"] not in REQUEST_PHASES
                ):
                    return False
                if farmer_snapshot is not None:
                    ...
                    if source is None:
                        ...
                        if snapshot.get("request") is not None:
                            return (
                                not require_bilateral
                                and not farmer_side
                                and state.get("phase") == "request_verified"
                                and self._structural_request_probe_owned(
                                    character,
                                    snapshot,
                                    state,
                        ...
                        return (
                            not require_bilateral
                            and state["phase"] in TRADE_PHASES
                            and self._structural_trade_probe_owned(
                                character,
                                snapshot,
                                state,
                                now=time.time() if now is None else now,
                    ...
                    farmer_intent = (
                        intent.get("farmer") if isinstance(intent, dict) else None
                    )
                    if (
                        source.character != farmer_name()
                        or not isinstance(farmer_intent, dict)
                        or getattr(getattr(source, "adapter", None), "identity", None)
                        != farmer_intent.get("identity")
                    ):
                        return False
                    if snapshot.get("request") and state["phase"] not in REQUEST_PHASES:
                        return defer()
                    observer = (
                        self.observers.get(merchant_character)
                        if farmer_side
                        else source
                    )
                    if observer is None:
                        return defer() if state["phase"] in TRADE_PHASES else False
                    peer_role = "merchant" if farmer_side else "farmer"
                    if (
                        getattr(getattr(observer, "adapter", None), "identity", None)
                        != intent[peer_role]["identity"]
                    ):
                        return False
                    if not observer.lock.acquire(blocking=False):
                        return defer()
                    try:
                        try:
                            observer.adapter.assert_identity()
                        except (OSError, CaptureUnavailable, ValueError):
                            return False
                        if farmer_side:
                            farmer = snapshot
                            merchant = self.controllers[
                                merchant_character
                            ].driver.read()
                        else:
                            from conquest.merchants.delivery_bridge import source_memory
                ...
                proof = ownership(
                    state,
                    merchant_character,
                    self.manual_target(merchant_character),
                    source_target,
                    farmer,
                    merchant,
                    now=now,
                ...
                if (
                    evidence_digest(delivery_probe.read_probe())
                    != proof["probe_digest"]
                ):
                    return False
            except (
            ...
            if proof["modal"] == "trade":
                return self.reconcile_probe_pair(
                    merchant_character, farmer, merchant, now=now
                )
            row = self.manual_sessions.active(self.manual_target(character))
            ...
            if row and not farmer_side and decline.get("phase") != "submitted":
                try:
                    self.manual_sessions.retract_probe_admission(
                        row["id"],
                        snapshot,
                        state,
                        farmer,
                        farmer_profile_id=source_target,
                        now=now,
                    )
    ...
    def reconcile_probe_pair(self, character, farmer, merchant, *, now=None):
        ...
        result = self._probe_reconciliation(
            character, farmer, merchant, now=now, read_only=False
        )
    ...
    def inspect_probe_reconciliation(self, character, farmer, merchant, *, now=None):
        ...
        return self._probe_reconciliation(
            character, farmer, merchant, now=now, read_only=True
        )
    ...
    def _probe_reconciliation(self, character, farmer, merchant, *, now, read_only):
        ...
        read = (
            (lambda: delivery_probe.read_probe(read_only=True))
            if read_only
            else delivery_probe.read_probe
        )
        with self.coordinator.lock:
            try:
                state = read()
                ...
                result.update(
                    ...
                    farmer_profile_id=source,
                    stage="ownership",
                    reason="ownership_unverified",
                )
                proof = ownership(
                    state, str(character), target, source, farmer, merchant, now=now
                )
                if proof["modal"] != "trade":
                ...
                result.update(
                    ...
                    stage="probe_recheck",
                    reason="probe_changed",
                )
                if evidence_digest(read()) != proof["probe_digest"]:
                    return result
                result.update(
                    bot_owned=True, stage="hold_read", reason="hold_read_failed"
                )
                ...
                with self.manual_sessions._probe_connection(True) as db:
                    db.execute("BEGIN")
                    rows = db.execute(
                        "SELECT name,value FROM state WHERE character IN (?,?) AND name IN "
                        "('unrelated_request_decline','manual_reader_hold')",
                        (target, source),
                    ).fetchall()
                    held = any(
                        (
                            value
                            and (
                                row["name"] == "manual_reader_hold"
                                or value.get("phase") == "submitted"
                            )
                        )
                        for row in rows
                        for value in (json.loads(row["value"]),)
                    )
                    result["active_session_count"] = db.execute(
                        "SELECT COUNT(*) FROM manual_sessions WHERE target_profile_id IN (?,?) "
                        "AND phase NOT IN ('completed','request_withdrawn','declined_verified','operator_overridden')",
                        (target, source),
                    ).fetchone()[0]
                if held:
                    ...
                    result.update(
                        stage="manual_history",
                        reason="manual_history_validation_failed",
                    )
                    method = (
                        self.manual_sessions.inspect_probe_pair
                        if read_only
                        else self.manual_sessions.retract_probe_pair
                    )
                    try:
                        details = method(
                            state,
                        ...
                        result.update(
                            outcome="validated" if read_only else "reconciled",
                            reason="exact_bot_history",
                        )
                        if read_only:
                            result.update(details)
                        else:
                            result["retracted_sessions"] = len(details)
                        ...
                        result.update(
                            outcome="protected",
                            reason="manual_history_unverified",
                            **_probe_history_failure(error),
                        )
                decisive_stage = result["stage"]
                result["stage"] = "final_probe_recheck"
                if evidence_digest(read()) != proof["probe_digest"]:
                    result.update(
                        bot_owned=False, outcome="error", reason="probe_changed"
                    )
                else:
                    result.update(
                        stage=decisive_stage, checked_through="final_probe_recheck"
                    )
    ...
    def reconcile_probe_owned(self, character, farmer, merchant, *, now=None):
        ...
        """
        with self.coordinator.lock:
            return self.process_probe_owned(
                character,
                merchant,
                farmer_snapshot=farmer,
                now=now,
    ...
    def process_manual(self, character, snapshot, *, decline_enabled=False, now=None):
        ...
        with self._manual_observation_scope(snapshot, now=now) as deferred:
            if deferred:
                return True
            return self._process_manual_locked(
                character, snapshot, decline_enabled=decline_enabled, now=now
            )

    def _process_manual_locked(
        self, character, snapshot, *, decline_enabled=False, now=None
    ):
        ...
        with self.coordinator.lock:
            ...
            try:
                if row:
                    from conquest.merchants.empty_delivery_cancel_sessions import (
                        observe as observe_empty_cleanup,
                    )

                    cleanup = observe_empty_cleanup(self, character, snapshot)
                    if cleanup is not None:
                        return bool(cleanup["holds_automation"])
                    if request and row["phase"] != "needs_attention":
                        try:
                            row = store.begin_request(
                                target, snapshot, session_id=row["id"], now=now
                            )
                    ...
                    return False
                if (
                    row["phase"] == "approval_pending"
                    and row["request_state"] == "pending"
                ):
                    row = store.expire(row["id"], now=now)
                    if row["request_state"] == "pending" and store.allowed(
                        row["visitor"]
                    ):
                        row = store.activate_allowed(
                            row["approval_binding"], snapshot, now=now
                        )
            except ManualSessionError as error:
                # Incomplete identity is not permission to use the old path.
                self.manual_reader_failure(
                    character,
                    snapshot,
                    "Manual visitor memory requires attention: " + str(error),
                    now=now,
                )
                return True
            finally:
        ...
        if (
            row["phase"] == "approval_pending"
            and row["request_state"] == "decline_pending"
            and not row["ever_approved"]
            and decline_enabled
            and self.can_start_work()
        ):
            if not is_farmer_owner(character) and not self.coordinator.safe_to_yield():
                with self.lock:
                    if self.handoff is None:
                        self.handoff = f"manual-decline:{row['id']}"
            ...
            try:
                controller = (
                    self.manual_farmer_controller
                    if is_farmer_owner(character)
                    else self.controllers[character]
                )
                decline_unrelated_request(
                    controller,
                    snapshot,
                    operations_enabled=True,
                    manual_session=(store, row["id"]),
                )
            finally:
                self._sync_manual_fence()
        return True
    ...
    def manual_farmer_status(self):
        return {
            "session": self.manual_status("Farmer"),
            "observation": dict(self.manual_farmer_observation),
    ...
    def farmer_bot_owned(self):
        ...
        if getattr(self, "delivery_window", None) or any(
            active(self.journal, c) for c in CHARACTERS
        ):
            return True
        ...
        with sqlite3.connect(JOURNAL.resolve().as_uri() + "?mode=ro", uri=True) as db:
            return bool(
                db.execute(
                    "SELECT 1 FROM transactions WHERE kind='farmer_delivery' AND phase NOT IN ('verified','aborted','operator_overridden') LIMIT 1"
                ).fetchone()
            )
    ...
    def run_manual_farmer(self):
        while not self.stop_event.is_set():
            ...
            if (
                ...
                and self.manual_handoff.active() is not None
            ):
