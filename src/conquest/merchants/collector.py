...
from conquest.merchants.market import (
    SOURCE,
    MarketSnapshot,
    browser_pages,
    market_integer,
...
def browser_launch_options(settings=state_path(".runtime/merchants/browser.json")):
    ...
    if not executable.is_absolute() or not executable.is_file():
        raise ValueError(
            "Configured market browser is missing; repair the local browser setup."
        )
...
def matching_count(raw):
    ...
    match = re.fullmatch(
        r"\s*([0-9]+(?:[,\u00a0\u202f ][0-9]{3})*)\s+matching items\s*", raw
    )
    if not match:
...
def collect_pages(page, *, check=lambda: None, clock=time.time):
    ...
    def settled():
        check()
        page.get_by_role("progressbar", name="Updating market items").wait_for(
            state="hidden"
        )
        page.get_by_role("button", name="Refresh", exact=True).and_(
    ...
    def metadata():
        ...
        return matching_count(raw), page.get_by_text(
            re.compile("Last booth change")
        ).inner_text().strip()

    settled()
    if (
        clean(page.get_by_role("combobox", name="Server", exact=True).inner_text())
        != "America"
    ):
        raise ValueError("Select America before collecting")
    for role, labels in [
        ("textbox", ("Item name", "Socket", "Seller")),
        ("spinbutton", ("Plus", "Min price", "Max price")),
    ]:
        if any(
            page.get_by_role(role, name=label, exact=True).input_value()
            for label in labels
        ):
            raise ValueError("Clear all market filters except America")
    if any(
        clean(page.get_by_role("combobox", name=label, exact=True).inner_text())
    ...
    data = dict(
        ...
        server="America",
        observed_at=clock(),
        initial_total=initial[0],
        initial_change=initial[1],
        pages=[],
        last_page=False,
    )
    for number in range(1, (initial[0] + 49) // 50 + 1):
        ...
        page.get_by_role("button", name=f"page {number}", exact=True).wait_for(
            state="visible"
        )
        current = metadata()
        if current != initial:
            raise ValueError(
                f"Market changed during collection on page {number}: {initial!r} -> {current!r}; discard this snapshot"
            )
        rows = (
            page.get_by_role("table", name="Live market listings")
            .get_by_role("row")
            .evaluate_all(
    ...
    if not data["last_page"] or metadata() != initial:
        raise ValueError(
            f"Market collection was incomplete or changed: {initial!r} -> {metadata()!r}, "
...
def collect_market(
    *,
    destination=state_path("reports/merchants/market.json"),
    definitions_path=installation_path(
        r"C:\Program Files\Classic Conquer 2.0\ini\itemtype.json"
    ),
    stop=None,
    timeout=600,
):
    ...
    try:
        with sync_playwright() as playwright:
            ...
            try:
                ...
                check()
                PriceHistory(
                    Path(destination).with_name("price-history.sqlite3")
                ).remember(MarketSnapshot(data))
        ...
        raise ValueError(
            "Browser collection failed or needs website verification; prices unchanged"
        ) from None
