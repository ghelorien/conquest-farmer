...
def source_intent(key, character):
    ...
    with journal.db() as db:
        row = db.execute(
            "SELECT character,kind,before_json FROM transactions WHERE id=?", (key,)
        ).fetchone()
...
def dispatch(ui, body):
    ...
    if body.get("action") == "delivery-reserve":
        if not ui.coordinator.lock.acquire(blocking=False):
            raise ValueError(
                "Wait for the current input action before reserving a delivery"
            )
        try:
...
def _dispatch(ui, body):
    ...
    if action == "delivery-reserve":
        expected.add("uids")
    if action == "delivery-disposition":
    ...
    with ui.runtime.lock:
        if action == "delivery-reserve":
            ...
            if len(items) != len(uids):
                raise ValueError("Delivery items are no longer carried")
            ...
            if exact_items(intent["items"]) != exact_items(items):
                raise ValueError(
                    "Reservation items differ from the durable source operation"
                )
            for role, fresh in (("farmer", source), ("merchant", destination)):
                if any(
                    intent[role][k] != fresh[k]
                    for k in ("identity", "character_uid", "character", "silver")
                ):
                    raise ValueError(
                        "Reservation participant changed since source preparation"
                    )
                if exact_items(intent[role]["inventory"]) != exact_items(
                    fresh["inventory"]
                ):
                    raise ValueError(
                        "Reservation inventory changed since source preparation"
                    )
                if exact_listings(intent[role].get("booth", [])) != exact_listings(
                    fresh.get("booth", [])
                ):
                    raise ValueError(
                        "Reservation booth changed since source preparation"
                    )
                if any(
                    intent[role].get(k) != fresh.get(k) for k in ("trade", "request")
                ):
                    raise ValueError(
                        "Reservation trade state changed since source preparation"
                    )
            origin = {
                k: intent[k]
                for k in (
            ...
            state = reservations.reserve(
                journal, body["request_id"], source, destination, items, origin=origin
            )
        elif action == "delivery-ready":
            state = reservations.ready(
                journal, character, body["request_id"], source, destination
            )
            ...
            if not receipt or receipt["character"] != character:
                raise ValueError(
                    "Disposition requires the matching durable source operation"
                )
            trace = [
                {**step, "payload": json.loads(step["payload"])}
                for step in source_journal.trace(body["request_id"])
