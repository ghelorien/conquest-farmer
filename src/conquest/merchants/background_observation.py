...
class BackgroundObservationReader:
    ...
    def snapshot(self, *, attempts=3):
        ...
        for attempt in range(attempts):
            ...
            if self.session.identity != identity:
                raise ValueError('Background observation process identity changed')
            return {'schema_version': 1, 'client_sha256': CLIENT_SHA256,
    ...
    def _snapshot(self):
        ...
        if not hwnd or tracked not in (0, 1) or buttons & ~0x1f or accepting_raw[0] not in (0, 1):
        ...
        if not 0 <= count <= min(capacity, MAX_EVENTS) or not 0 <= capacity <= MAX_CAPACITY:
        ...
        events = [_event(data[offset:offset + EVENT_SIZE])
        ...
        checks = [(pointer_address, pointer), (context + 0x90, backend_pointer),
                  (queue_address, queue_raw), (backend_address, backend_raw),
                  (context + 0x37c1, accepting_raw),
                  (context + 0x3ec0, hover_window_raw), (context + 0x3ef0, hover_id_raw),
                  (context + 8, config_raw), (context + 0xd94, mouse_raw),
                  (context + 0xd9c, mouse_down_raw), (context + 0xd0, capture_raw),
                  (context + 0xdac, modifiers_raw), (context + 0xe00, key_mods_raw),
                  (context + 0x3f04, active_id_raw), (context + 0x3f20, active_window_raw),
        ...
        if any(self._read(address, len(before)) != before for address, before in checks):
            raise GuiObservationChanged('Background input state changed during observation')
        return {'context': context, 'frame': struct.unpack('<I', frame_raw)[0],
                'accepting_events': bool(accepting_raw[0]),
                'backend': {'address': backend_address, 'hwnd': hwnd,
                            'mouse_hwnd': mouse_hwnd, 'mouse_tracked': bool(tracked),
                            'buttons_down': buttons},
                'hover': {'window': hover_window, 'id': struct.unpack('<I', hover_id_raw)[0]},
                ...
                'modifiers': dict(zip(('ctrl', 'shift', 'alt', 'super'), map(bool, modifiers_raw))),
                'key_mods': key_mods,
                'config_flags': struct.unpack('<I', config_raw)[0],
                'active': {'id': struct.unpack('<I', active_id_raw)[0], 'window': active_window},
                'drag': drag,
...
def _drag(raw, context):
    ...
    data_address, data_size, source_id, parent_id, data_frame = struct.unpack_from('<QiIIi', raw, 0x14)
    if any(value not in (0, 1) for value in (active, within_source, within_target, preview, delivery)):
        raise ValueError('Invalid drag state boolean')
    if not -1 <= button <= 4 or source_frame < -1 or data_frame < -1 or not 0 <= data_size <= 4096:
    ...
    return {'active': bool(active), 'within_source': bool(within_source),
            'within_target': bool(within_target), 'source_flags': flags,
            'source_frame': source_frame, 'mouse_button': button,
            'data_address': data_address, 'data_size': data_size,
            'source_id': source_id, 'source_parent_id': parent_id,
            'data_frame': data_frame, 'payload_type': type_raw.decode('ascii'),
            'preview': bool(preview), 'delivery': bool(delivery), 'source_item_uid': None}
...
def _event(raw):
    ...
    result = {'type': {1: 'mouse_position', 2: 'mouse_wheel', 3: 'mouse_button',
                       4: 'key', 5: 'text', 6: 'focus'}[kind], 'source': source}
    if kind in (1, 2):
