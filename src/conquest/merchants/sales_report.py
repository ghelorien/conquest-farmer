...
class SalesReportWorker:
    ...
    def status(self):
        with self.journal.db() as db:
            ...
            if not row:
                last = db.execute(
                    "SELECT MAX(created) FROM sales_reports WHERE phase='delivered'"
                ).fetchone()[0]
                due = (last if last is not None else self.clock()) + INTERVAL
                db.execute(
                    "INSERT INTO sales_schedule VALUES(1,?,?,?)", (due, 0, "waiting")
                )
                row = db.execute("SELECT * FROM sales_schedule WHERE id=1").fetchone()
    ...
    def step(self):
        ...
        if status in ("delivered", "already_delivered"):
            ...
            due = (
                state["next_due"]
                + (int((now - state["next_due"]) // INTERVAL) + 1) * INTERVAL
            )
            retry_at = 0
        else:
            due = state["next_due"]
        ...
        with self.journal.db() as db:
            db.execute(
                "UPDATE sales_schedule SET next_due=?,retry_at=?,status=? WHERE id=1",
                (due, retry_at, status),
            )
        return self.status()
...
def save_webhook(value):
    ...
    SECRET.write_bytes(
        win32crypt.CryptProtectData(
            value.strip().encode(), "Conquest #shops", None, None, None, 0
        )
    )
...
def send_report(
    *, journal=None, now=None, send=deliver, load=load_webhook, preview=False
):
    ...
    try:
        ...
        return {
            ...
            "note": "Configure the #shops webhook in Conquest Overview.",
        }
    ...
    with journal.db() as db:
        ...
        if old and old["phase"] in ("sending", "uncertain"):
            return {
                ...
                "note": "Previous delivery was interrupted or unconfirmed; reconcile before resending.",
            }
        if old:
            content = old["content"]
        else:
            db.execute(
                "INSERT INTO sales_reports VALUES(?,?,?, ?,NULL)",
                (key, now, "prepared", content),
            )
    ...
    try:
        ...
        rejected = isinstance(error, DeliveryError) and str(error).startswith(
            "Discord HTTP "
        )
        with journal.db() as db:
            db.execute(
                "UPDATE sales_reports SET phase=? WHERE id=?",
                ("prepared" if rejected else "uncertain", key),
            )
        return {
            "status": "retry_later" if rejected else "needs_attention",
            "note": "Discord rejected the report."
            if rejected
            else "Delivery unconfirmed; reconcile before resending.",
            "retry_after": error.retry_after if rejected else None,
        }
    with journal.db() as db:
        db.execute(
            "UPDATE sales_reports SET phase='delivered',message_id=? WHERE id=?",
            (str(message_id), key),
        )
