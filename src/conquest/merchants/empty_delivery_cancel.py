...
CODE = {
    0x10F0CB: "33d241b861000020488d0d7ec04b00e861cbf0ff4c8d4dd70f14f666490f7ef0488d157ec04b00498bcee85642f6ff",
...
def unchanged(before, current):
    for role, other in (("farmer", "merchant"), ("merchant", "farmer")):
        ...
        if any(
            a[k] != b[k]
            for k in ("identity", "character_uid", "map_id", "position", "silver")
        ):
        ...
        if b.get("request"):
            raise ValueError("Another trade request is active")
        t = b.get("trade")
        if t and (
            t.get("participant") != before[other]["character"]
            or t.get("participant_uid") != before[other]["character_uid"]
            or t.get("own_items")
            or t.get("items")
            or t.get("own_silver") != 0
            or t.get("other_silver") != 0
            or t.get("accepted") is not False
            or t.get("other_accepted") is not False
        ):
...
def control(driver, snapshot):
    ...
    for rva, encoded in {
        **{k: CLOSE_CODE[k] for k in (0x73637, 0x7367C)},
        **CODE,
    }.items():
    ...
    if unpack(s, model + 12, "<B")[0] != 1:
        raise ValueError("Trade model is not open")
    windows = [w for w in snapshot["windows"] if w["name"] == "Trade##TradeWindow"]
    if len(windows) != 1:
        raise ValueError("Trade window is absent or ambiguous")
    w = windows[0]
    x, y, width, height = w["geometry"]
    point = (round(x + width - 23.5), round(y + 12))
    if not x < point[0] < x + width or not y < point[1] < y + height:
        raise ValueError("Invalid trade close geometry")
    return w, point


def read():
    try:
        value = json.loads(Path(state_path(PATH)).read_text(encoding="utf-8"))
    except FileNotFoundError:
        return None
    if not isinstance(value, dict):
        raise ValueError("Empty cancellation receipt is unreadable")
    return value
...
def _open(ui, probe, farmer, merchant):
    ...
    if (
        probe.get("phase") != "trade_open_verified"
        or probe.get("offered_uids")
        or any(
            k in probe
            for k in (
                "placing_uid",
        ...
        raise ValueError(
            "Cleanup requires an exact bot trade before any item placement or confirmation"
        )
    ownership(
        probe,
        probe["character"],
        ui.runtime.manual_target(probe["character"]),
        ui.runtime.manual_target("Farmer"),
        farmer,
        merchant,
        now=time.time(),
    )
...
def start(ui, character):
    ...
    with ui.coordinator.lock:
        ...
        if (
            ...
            or c["enabled"]
            or c.get("paused")
            or ui.runtime.enabled(character)
            or not ui.safe_to_yield()
            or ui.runtime.manual_handoff_status() is not None
        ):
            raise ValueError(
                "Pause merchant operations and farming before exact bot cleanup"
            )
        prepared = bool(
            old
            and old.get("phase") == "prepared"
            and old.get("probe_digest") == digest(state)
            and "submitted_at" not in old
            and "point" not in old
        )
        if old and old.get("phase") != "cancel_verified" and not prepared:
            raise ValueError(
                "Existing empty cancellation is one-shot; use read-only reconciliation"
            )
        ...
        _open(ui, state, f, m)
        bound = (
            sessions.refresh_binding(ui.runtime, state, old["sessions"])
            if prepared
            else sessions.binding(ui.runtime, state)
        )
        if old:
            probe.archive_probe(old)
        cancel = {
            "phase": "prepared",
            "character": str(character),
            "probe": state,
            "probe_digest": digest(state),
            "before": {"farmer": f, "merchant": m},
            "sessions": bound,
        ...
        if prepared:
            cancel["prior_prepared_digest"] = digest(old)
        cancel = json.loads(json.dumps(cancel))
        save(cancel)
        ...
        def work():
            try:
                ...
                if (
                    digest(read()) == digest(cancel)
                    and cancel["phase"] != "cancel_verified"
                ):
                    cancel.update(error=str(error), failed_at=time.time())
                    save(cancel)

        ui.delivery_probe_thread = threading.Thread(
            target=work, daemon=True, name="empty-delivery-cancel"
        )
        ui.delivery_probe_thread.start()
        return {
            "started": True,
            "character": str(character),
            "probe_digest": cancel["probe_digest"],
        }
...
def _authorize(ui, state, expected):
    ...
    if (
        ...
        or active(ui.runtime.journal, character)
        or ui.runtime.journal.pending(character)
        or any(ctypes.windll.user32.GetAsyncKeyState(k) & 0x8000 for k in (0x7A, 0x7B))
    ):
        raise CaptureUnavailable(
            "Exact empty-bot cleanup stopped or its authority changed"
        )
    check_binding(ui.runtime, state["sessions"])
    if (
        sorted(ui.coordinator.manual_sessions.values(), key=lambda r: r["id"])
        != state["sessions"]["holds"]
    ):
        raise CaptureUnavailable("Empty cleanup manual fences changed")
    if (
        ui.runtime.manual_target(character) != state["probe"]["target_profile_id"]
...
def lease_authorized(ui, character):
    ...
    if (
        ...
        or worker is not threading.current_thread()
    ):
        return False
    driver = ui.runtime.controllers[character].driver
    observer = ui.runtime.observers.get(character)
    if (
        observer is not driver.observer
        or driver.target is not observer.operations.target
        or observer.adapter.identity != state["probe"]["intent"]["merchant"]["identity"]
    ):
        return False
...
def run(ui, state):
    ...
    def authorize():
        return _authorize(ui, state, expected[0])
    ...
    with ui.coordinator.lock:
        ...
        f, m = pair(ui, character)
        _open(ui, state["probe"], f, m)
        from conquest.merchants.empty_delivery_cancel_sessions import refresh_binding

        state["sessions"] = refresh_binding(
            ui.runtime, state["probe"], state["sessions"]
        )
        save(state)
        expected[0] = digest(state)
        with ui.coordinator.probe_abort_scope(authorize):
            with (
                ...
                check()
                native = driver.observer.adapter.expected_sha256 == CLIENT_SHA256_1078
                if native:
                    native_foreground(driver, m["identity"], activate=True)
                f, m = pair(ui, character)
                _open(ui, state["probe"], f, m)
                w, point = control(driver, m)
                size = driver.target.snapshot()["client_size"]
                if size != driver.memory.gui.viewport_size():
                    raise ValueError("Trade viewport changed")

                def guard():
                    check()
                    a, b = pair(ui, character)
                    _open(ui, state["probe"], a, b)
                    if native:
                        native_foreground(driver, b["identity"])
                    if control(driver, b) != (w, point):
                        raise ValueError("Trade close control moved")
                    driver.memory.gui.assert_hovered(w, "#CLOSE")

                def press():
                    wait_hover_validation(guard, check)
                    if submitted[0]:
                        raise ValueError("Empty cancellation was already submitted")
                    state.update(
                        phase="cancel_submitted", point=point, submitted_at=time.time()
                    )
                    save(state)
                    expected[0] = digest(state)
                    submitted[0] = True
...
def _reconcile(ui, state):
    ...
    with ui.coordinator.lock:
        if state.get("phase") not in (
            "cancel_submitted",
            "cancel_verified",
        ) or not state.get("submitted_at"):
            raise ValueError(
                "No submitted empty cancellation exists; no input replay is authorized"
            )
        if digest(read()) != digest(state):
            raise ValueError("Empty cancellation receipt changed")
        f, m = pair(ui, state["character"])
        now = time.time()
        unchanged(state["before"], {"farmer": f, "merchant": m})
        if any(
            s.get("trade") is not None or s.get("request") is not None for s in (f, m)
        ):
            return {
                "phase": state["phase"],
                "manual_settled": False,
                "reason": "Exact empty trade remains open; no click replay",
            }
        for role, snapshot in (("farmer", f), ("merchant", m)):
            if canonical_ownership(snapshot) != canonical_ownership(
                state["probe"]["intent"][role]
            ):
                raise ValueError("Closed empty cancellation ownership changed")
        if state["phase"] != "cancel_verified":
            state.update(
                phase="cancel_verified",
                after={"farmer": f, "merchant": m},
                verified_at=now,
                historical_outcome="unknown",
            )
            state["receipt_digest"] = digest(state)
            save(state)
        elif (
            digest({k: v for k, v in state.items() if k != "receipt_digest"})
        ...
        if digest(current) == state["probe_digest"]:
            result = {
                **current,
                "phase": "cancel_verified",
                "updated_at": state["verified_at"],
                "finished_at": state["verified_at"],
                "error": None,
                "empty_cancel_receipt": state,
                "empty_cancel_digest": state["receipt_digest"],
                "historical_outcome": "unknown",
                "sales_receipt": False,
                "delivery_receipt": False,
            }
