...
def operator_override(
    ...
    *,
    ...
    character = character or getattr(
        getattr(driver, "observer", None), "character", None
    )
    if not character:
    ...
    with journal.db() as db:
        ...
        row = db.execute(
            "SELECT value FROM state WHERE character=? AND name='stall_probe'",
            (character,),
        ).fetchone()
        state = json.loads(row[0]) if row else {}
        if state.get("phase") == "operator_overridden":
            if (state.get("operator_override") or {}).get(
        ...
        if row:
            db.execute(
                "UPDATE state SET value=? WHERE character=? AND name='stall_probe'",
                (encoded, character),
            )
        else:
            db.execute(
                "INSERT INTO state(character,name,value) VALUES(?,?,?)",
                (character, "stall_probe", encoded),
            )
    journal.event(
        character,
        "stall_probe_operator_overridden",
        original_evidence_digest=digest,
        confirmation_reference=confirmation_reference.strip(),
    )
    return state
...
def reconcile_interrupted_probe(driver, journal):
    ...
    record = journal.get(character, "stall_probe", {})
    ...
    try:
        read_dialog(driver.observer)
    except ValueError as error:
        if str(error) != "NPC dialog is absent":
            raise
    else:
        raise ValueError("Stall dialog remains open; no repeat interaction")
    if (
        sorted(i["uid"] for i in snapshot["inventory"])
        != sorted(record["inventory_uids"])
    ...
    if exact and (
        snapshot["inventory"] != record["inventory_before"]
        or snapshot["booth"] != record["booth_before"]
    ):
        raise ValueError("Stall interaction item attributes differ")
    record.update(
        phase="reconciled_cancelled",
        reconciled_at=time.time(),
        position_after=snapshot["position"],
        no_open_interaction=True,
        inventory_uids_and_silver_unchanged=True,
        exact_attributes_verified=exact,
...
def reconcile_manual_setup(driver, journal, record, snapshot):
    ...
    if (
        record.get("operation") not in (None, "inspect_vacant_flag")
        or flag.get("name") != "ShopFlag"
        or (flag.get("type_id"), flag.get("model")) != (0, 1086)
    ...
    with journal.db() as db:
        row = db.execute(
            "SELECT id,timestamp,payload FROM events WHERE character=? AND "
            "event='manual_stall_setup_adopted' AND timestamp>? ORDER BY id DESC LIMIT 1",
            (character, record["submitted_at"]),
        ).fetchone()
    if (
        row is None
        or json.loads(row["payload"]).get("own_booth_uid") != snapshot["own_booth_uid"]
    ):
    ...
    if (
        ...
        or any(
            fresh[k] != snapshot[k]
            for k in ("identity", "map_id", "position", "own_booth_uid")
        )
        or owned_booth(driver.observer, fresh) != booth
    ):
        raise ValueError("Manually adopted booth changed during reconciliation")
    record.update(
        phase="reconciled_manual_setup",
        reconciled_at=time.time(),
        adopted_event_id=row["id"],
        adopted_at=row["timestamp"],
        own_booth_uid=booth["uid"],
        original_inventory_result_verified=False,
        manual_setup_superseded_probe=True,
        input_qualified=False,
    )
    journal.set(character, "stall_probe", record)
...
def inspect_flag(driver, travel, journal, check, *, flag_uid=None):
    ...
    if (
        before["map_id"] != 1036
        or before["booth_open"]
        or before.get("trade")
        or before.get("request")
    ):
        raise ValueError(
            "Stall inspection requires Market, a closed booth and no trade"
        )
    from conquest.conductress import read_dialog
    ...
    try:
        ...
        raise ValueError("Close the existing NPC dialog before inspecting a stall")
    ...
    flags = (
        [owned_booth(driver.observer, before)]
        if reopening
        else [
            f
            for f in vacant_flags(driver.observer, spec)
    ...
    if len(flags) != 1:
        raise ValueError(
            "Stand beside exactly one memory-verified unattended stall flag"
        )
    flag = flags[0]
    from conquest.merchants.flag_target import flag_target, CONTROL as FLAG_CONTROL

    target = (
        owned_booth_target(driver.observer, flag)
        if reopening
        else flag_target(driver.observer, flag)
    )
    point = target["point"]
    ...
    uncovered(before)
    inventory = driver.memory.inventory.read()
    press_pending = False

    def unchanged():
        check()
        life = read_life(s, driver.observer.health_layout, character)
        if (
            life.map_id != 1036
            or list(life.position) != before["position"]
        ...
        match = (
            owned_booth(driver.observer, driver.memory.read())
            if reopening
            else next(
                (
                    f
        ...
        if match != flag:
            raise ValueError("Stall was occupied or changed before inspection")
    ...
    record = {
        ...
        "inventory_uids": [i.uid for i in inventory.items],
        "inventory_before": before["inventory"],
        "booth_before": before["booth"],
        "press_pending": False,
    }
    journal.set(character, "stall_probe", record)
    try:
        travel.click(point, check, before_press=prepare_press)
    except Exception:
        if not press_pending:
            record.update(
                phase="cancelled_before_press",
    ...
    while time.monotonic() < deadline:
        ...
        if (
            life.map_id != 1036
            or life.dead_candidate
            or inv.items != inventory.items
            or inv.silver != inventory.silver
        ):
            raise ValueError(
                "Stall response needs inventory or position reconciliation"
            )
        model = driver.memory.gui.model(25, 0x5C27F8)
        raw = s.read_block(model, 0x58)
        own = unpack(s, life.object_address + 0x3258, "<I")[0]
        try:
            records = read_dialog(driver.observer)["records"]
        except ValueError as error:
            if str(error) != "NPC dialog is absent":
                raise
            records = []
        if not reopening and any(
            w["name"] == "Open Booth###Confirm" for w in driver.memory.gui.windows()
        ):
        ...
        if raw[12] or records or (own and own != before.get("own_booth_uid", 0)):
            if raw[12] and (not own or struct.unpack_from("<I", raw, 0x4C)[0] != own):
                raise ValueError(
                    "Stall response opened a foreign booth; no qualification"
                )
            ...
            record.update(
                ...
                own_booth_uid=own,
                displayed_booth_uid=struct.unpack_from("<I", raw, 0x4C)[0],
                booth_open=bool(raw[12]),
                dialog=records,
        ...
        time.sleep(0.1)
    raise ValueError(
        "No qualified stall response; do not repeat without reconciliation"
    )
...
def confirm_observed_flag(driver, travel, journal, record, check):
    ...
    if record.get("operation") != "inspect_vacant_flag":
        raise ValueError(
            "Previous stall interaction needs reconciliation before another click"
        )
    check()
    before = driver.memory.read()
    if (
        any(before[k] != record[k] for k in ("identity", "silver"))
        or before["inventory"] != record.get("inventory_before")
        or before["booth"] != record.get("booth_before")
    ):
    ...
    if record.get("confirmation_submitted"):
        if before["map_id"] != 1036 or not before.get("own_booth_uid"):
            raise ValueError(
                "Submitted booth confirmation needs reconciliation; no repeated confirmation"
            )
        booth = owned_booth(driver.observer, before)
        if booth["position"] != [
            record["flag"]["position"][0] + 3,
            record["flag"]["position"][1],
        ]:
            raise ValueError("Claimed booth belongs to another flag")
        after = before
    else:
        ...
        def submitted():
            record.update(
                ...
                confirmation_at=time.time(),
                confirmation_control=dict(CONFIRM_CONTROL),
            )
    ...
    record.update(
        ...
        own_booth_uid=after["own_booth_uid"],
        displayed_booth_uid=after["own_booth_uid"] if after["booth_open"] else 0,
        booth_open=after["booth_open"],
        inventory_unchanged=True,
        dialog=[],
