...
def cleanup_empty_trade(
    ui, character, expected, *, operation=None, timeout=3, check=None
):
    ...
    def permitted():
        ...
        if (
            ...
            raise CaptureUnavailable(
                "Empty trade cleanup permission changed or expired"
            )
    ...
    with (
        ui.coordinator.lease(character, purpose="delivery_cleanup"),
        physical_coordinates(),
    ):
        permitted()
        farmer, merchant = pair(ui, character)
        current = {"farmer": farmer, "merchant": merchant}
        ...
        if not activate_client(driver.target.hwnd, merchant["identity"]):
            raise CaptureUnavailable(
                "Merchant focus unavailable; no cleanup input sent"
            )
        ...
        def before():
            ...
            if operation is not None:
                operation.before_action("cleanup_trade")
        ...
        while time.monotonic() < deadline:
            ...
            if not farmer.get("trade") and not merchant.get("trade"):
                ...
                if operation is not None:
                    operation.action_observed("cleanup_trade", evidence=evidence)
                return {"cleaned": True, "evidence": evidence}
            time.sleep(0.05)
    raise ValueError(
        "Empty trade cleanup unverified; reconcile before any further input"
    )
