...
def _qualified(snapshot, profile_id):
    ...
    if snapshot.get("profile_id") != profile_id or not snapshot.get(
    ...
    if (
        not snapshot.get("closed_modal")
        or snapshot.get("trade_open")
        or snapshot.get("request_open")
    ):
...
def _pending_bot_work(db, character):
    ...
    reservations = db.execute(
        "SELECT state FROM delivery_reservations WHERE character=?", (character,)
    ).fetchall()
...
def settle(runtime, character, *, now=None):
    ...
    with runtime.journal.db() as db:
        ...
        changed = db.execute(
            ...
            (encoded_after, journal_character, KEY, encoded_before),
        ).rowcount
        if changed != 1:
        ...
        db.execute(
            ...
            (
                journal_character,
                "recovery_safety_market_arrived_1078",
                json.dumps(
                    {
                        "identity": identity,
                        "ownership_digest": ownership_digest,
