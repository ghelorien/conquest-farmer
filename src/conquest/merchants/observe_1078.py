...
def _actor_identity(session):
    ...
    modules = [
        m for m in session.modules if m["name"].casefold() == player.module.casefold()
    ]
    if len(modules) != 1:
    ...
    if (
        resolve_player(session, player) != actor
...
def observe(
    runtime, character, *, listing_preflight=False, booth_target_preflight=False
):
    ...
    for identity in candidates.values():
        if time.monotonic() - started > 4:
            raise ValueError(
                "Merchant discovery expired; retry the read-only observation"
            )
        try:
    ...
    if len(matches) != 1:
        raise ValueError(
            f"{profile.name}: expected one memory-identified 1078 client, found {len(matches)}"
        )
    ...
    with MemorySession(identity["pid"], CLIENT_SHA256_1078) as session:
        ...
        if listing_preflight:
            ...
            if any(fresh[key] != snapshot[key] for key in stable_fields):
                raise ValueError(
                    "Merchant ownership or modal state changed during listing preflight"
                )
            snapshot = fresh
        if booth_target_preflight:
            ...
            if any(fresh[key] != snapshot[key] for key in stable_fields):
                raise ValueError(
                    "Merchant ownership changed during booth target preflight"
                )
            snapshot = fresh
    ...
    if time.monotonic() - started > 4:
        raise ValueError(
            "Merchant observation expired; retry the read-only observation"
        )
    fields = (
        "character",
        "character_uid",
    ...
    return {
        **{key: snapshot[key] for key in fields},
