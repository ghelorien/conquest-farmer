...
def _model(gui):
    ...
    if struct.unpack('<Q', session.read_block(base+MODEL_VTABLE+0x10, 8))[0] != base+0x96FE0:
...
def read(gui):
    ...
    if visible == b'\x00':
        ...
        return None
    values = tuple(string(session, model+offset, 256) for offset in (0x48, 0x68, 0x88, 0xA8))
    if values != STRINGS:
        raise ValueError('1078 confirmation is not the exact Open Booth prompt')
    callback = session.read_block(model+0x100, 8)
    if (not struct.unpack('<Q', callback)[0]
            or session.read_block(model+12, 1) != visible
            or tuple(string(session, model+offset, 256) for offset in (0x48, 0x68, 0x88, 0xA8)) != values
            or session.read_block(model+0x100, 8) != callback):
        raise ValueError('1078 Open Booth confirmation changed during observation')
    session.assert_identity()
    return {'model_address': model, 'title': values[0], 'message': values[1],
            'positive_label': values[2], 'negative_label': values[3],
