...
def _blocker(reason):
    return {'reconciled': False, 'blocker': reason, 'game_input': False,
            'journal_updated': False, 'input_qualified': False}
...
def _qualified(snapshot, profile_id):
    return (snapshot.get('client_sha256') == CLIENT_SHA256_1078
            ...
            and not snapshot.get('trade_open') and not snapshot.get('request_open')
            and snapshot.get('map_id') == 1036
            and type(snapshot.get('hp')) is int and snapshot['hp'] > 0
            and snapshot.get('own_booth_uid') and snapshot.get('booth_open'))
...
def reconcile(runtime, character):
    ...
    try:
        ...
        if (refill.get('pending') is not True or refill.get('status') not in
                ('checking', 'paused_budget') or not isinstance(cursor, list)
                or len(cursor) != len(_CURSOR) or set(cursor) != _CURSOR
                or any(type(uid) is not int for uid in cursor)
                or refill.get('listed') != 0 or type(since) not in (int, float)
                or refill.get('source_delivery_operation_id') is not None
                or since <= 0 or since > time.time()
        ...
        if (set(origins) != _CURSOR or any(origins[uid] != 'prior_inventory'
                for uid in _CURSOR) or any(uid not in live_inventory
                or uid not in old_inventory or any(
                    live_inventory[uid][key] != old_inventory[uid][key]
                    for key in _STOCK_FIELDS) for uid in _CURSOR)):
        ...
        if (not _qualified(second, profile_id) or not _same_stock(first, second)
        ...
        evidence = {'cursor_uids': sorted(_CURSOR),
        ...
        updated['old_cursor_reconciliation_1078'] = {
            'at': when, 'prior_state': refill, 'evidence': evidence}
        ...
        with runtime.journal.db() as db:
            ...
            if (time.time() - second['timestamp'] > 5
                    or _manual_fenced(runtime, target) or _pending_bot_work(db, owner)
            ...
            if db.execute('UPDATE state SET value=? WHERE character=? '
                    'AND name=? AND value=?',
                    (encoded, owner, 'refill', raw_refill)).rowcount != 1:
                return _blocker('refill_cursor_changed_before_commit')
            db.execute('INSERT INTO events(character,event,payload,timestamp) '
                       'VALUES(?,?,?,?)',
                       (owner, 'refill_cursor_reconciled_1078',
                        json.dumps(evidence, sort_keys=True), when))
        return {'reconciled': True, 'status': updated['status'],
                'cursor_uids': sorted(_CURSOR), 'evidence': evidence,
                'game_input': False, 'journal_updated': True,
