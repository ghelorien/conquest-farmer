...
def _context(state, character, target_profile_id, farmer_profile_id, *, now):
    ...
    for role in ("farmer", "merchant"):
        ...
        if state["phase"] != "request_submitted":
            ...
            if (
                any(
                    saved_proof[field] != original[role][field]
                    for field in ("identity", "character", "character_uid", "server")
                )
                or not started <= saved["timestamp"] <= now
                or saved.get("position") != before.get("position")
                or saved.get("map_id") != 1036
            ):
                raise ValueError(
                    "Saved probe observation identity or chronology changed"
                )
    farmer, merchant = intent["farmer"], intent["merchant"]
    if (
        merchant["character"] != str(character)
        or farmer["server"] != merchant["server"]
        or farmer["identity"] == merchant["identity"]
    ):
        raise ValueError("Probe participant identities are not distinct exact peers")
    recipient = state["recipient"]
    if (
        type(recipient.get("uid")) is not int
        ...
        or any(
            recipient.get(field) != merchant[key]
            for field, key in (
                ("uid", "character_uid"),
                ("name", "character"),
                ("position", "position"),
    ...
    if (
        ...
        or any(
            exact_items(intent["farmer"]["inventory"]).get(uid) != item
...
def _allowed_offers(state, items):
    ...
    if state["phase"] in ("trade_open_verified", "placement_submitted"):
        ...
        if (
            ...
            or any(type(uid) is not int or uid <= 0 for uid in prior)
            or len(set(prior)) != len(prior)
            or not set(prior) <= set(items)
        ):
...
def _acceptance(phase, role, trade):
    ...
    allowed = {
        ...
        ("merchant_confirm_submitted", "merchant"): (
            (False, False),
            (False, True),
            (True, False),
            (True, True),
...
def _local_trade(state, intent, original, items, role, snapshot, *, now):
    ...
    if (
        snapshot.get("request") is not None
        ...
        or any(
            type(trade.get(field)) is not int or trade[field] != 0
            for field in ("own_silver", "other_silver")
        )
        or any(
            type(trade.get(field)) is not bool
            for field in ("accepted", "other_accepted")
        )
...
def _saved_boundary(state, intent, original, items):
    ...
    if (
        ...
        or any(
            not state["started_at"] <= receipt["timestamp"] <= updated
    ...
    if merchant.get("request") is not None:
        ...
        if state["phase"] not in (
            ...
            raise ValueError(
                "Open-trade phase lacks a durable bilateral trade boundary"
            )
        return
...
def local_ownership(
    state, character, target_profile_id, farmer_profile_id, role, snapshot, *, now
):
    ...
    intent, original, items = _context(
        state, character, target_profile_id, farmer_profile_id, now=now
    )
    ...
    return {
        "probe_digest": evidence_digest(state),
        "modal": "trade",
...
def historical_local_trade(
    state, character, target_profile_id, farmer_profile_id, role, snapshot
):
    ...
    intent, original, items = _context(
        ...
        farmer_profile_id,
        now=max(
            state.get("updated_at", at),
            state.get("accepted_at", at),
            state.get("finished_at", at),
            at,
        ),
    )
    if snapshot.get("request") is not None:
        if role != "merchant":
            raise ValueError("Farmer request is not bot-owned")
        return _request_before_trade(
            state, intent, original, snapshot, now=max(at, state["accepted_at"])
        )
    ...
    farmer_accepted, merchant_accepted = (
        (trade.get("accepted"), trade.get("other_accepted"))
        if role == "farmer"
        else (trade.get("other_accepted"), trade.get("accepted"))
    )
...
def ownership(
    state, character, target_profile_id, farmer_profile_id, farmer, merchant, *, now
):
    ...
    current = {
        role: _participant(snapshot, intent[role], original[role], now)
        for role, snapshot in (("farmer", farmer), ("merchant", merchant))
    }
    if merchant.get("request") is not None:
        ...
        if state["phase"] != "request_submitted":
            saved_farmer, saved_merchant = (
                state["farmer_after"],
                state["merchant_after"],
            )
            request_ownership(
                intent,
                saved_farmer,
            ...
            if any(
                canonical_ownership(saved, require_closed=False) != current[role]
                for role, saved in (
                    ("farmer", saved_farmer),
            ...
            if any(
                not state["started_at"] <= saved["timestamp"] <= now
                for saved in (saved_farmer, saved_merchant)
            ):
        ...
        if exact_items(farmer["trade"]["own_items"]) != exact_items(
            merchant["trade"]["items"]
        ):
        ...
        if state["phase"] not in (
            ...
            farmer["trade"]["accepted"] != merchant["trade"]["other_accepted"]
