...
def current_refill_blocker(state):
    ...
    if ((state.get('refill') or {}).get('enabled') and state.get('connected')
        ...
        return (f"Auto-refill waiting: {current['unavailable_peer']} owned booth memory "
...
def summary(state, *, now, global_stopped=False):
    ...
    if state.get('manual_only_1078'):
        ...
        return '\n'.join(x for x in (title,stock,refill_problem,error.get('note')) if x)
    ...
    elif refill_problem:title='WAITING — owned peer price proof unavailable'
