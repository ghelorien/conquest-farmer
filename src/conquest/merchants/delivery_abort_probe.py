...
def read(suffix=""):
    try:
        value = json.loads(path(suffix).read_text(encoding="utf-8"))
    except FileNotFoundError:
        return None
    if not isinstance(value, dict):
        raise ValueError("Abort journal is unreadable")
    return value
...
def _prior(state):
    ...
    if previous and previous.get("probe_digest") == digest(state):
        if previous.get("phase") != "abort_prepared":
            raise ValueError(
                "Existing submitted abort is one-shot; reconcile without another close"
            )
        return previous
    return None
...
def _profiles(ui, state):
    if state.get("target_profile_id") != ui.runtime.manual_target(
        state["character"]
...
def ownership(state, farmer, merchant, *, now):
    ...
    if state.get("phase") != "offer_verified":
        raise ValueError("Only an offer_verified probe may be aborted")
    intent, original, items = _context(
        state,
        state["character"],
    ...
    if len(items) != 1:
        raise ValueError("Abort qualification requires exactly one selected probe item")
    result = {}
    for role, snapshot in (("farmer", farmer), ("merchant", merchant)):
        ...
        if role == "merchant":
            ...
            allowed[role]["silver"] = current["silver"]
            allowed[role]["booth"] = current["booth"]
...
def closed(abort, farmer, merchant, *, now):
    ...
    for role, snapshot in (("farmer", farmer), ("merchant", merchant)):
        _fresh(snapshot, now)
        current = canonical_ownership(snapshot)
        before = canonical_ownership(abort["baseline"][role], require_closed=False)
        validate_snapshot(snapshot, before["character"], now)
        if (
            snapshot["map_id"] != abort["baseline"][role]["map_id"]
            or snapshot["position"] != abort["baseline"][role]["position"]
        ):
            raise ValueError("Abort participant moved")
        expected = deepcopy(before)
        expected.update(request=None, trade=None)
        inventory = {i["uid"]: i for i in expected["inventory"]}
        if role == "farmer":
            inventory.update(
                {
                    i["uid"]: i
                    for i in canonical_ownership(abort["probe"]["intent"]["farmer"])[
                        "inventory"
                    ]
                    if i["uid"] in wanted
                }
            )
            expected["inventory"] = sorted(
                inventory.values(), key=lambda item: item["uid"]
            )
        if current != expected:
            raise ValueError(
                "Closed abort ownership differs from the immediate baseline"
            )
        if role == "merchant" and set(wanted) & (
            set(exact_items(snapshot["inventory"]))
            | set(exact_items(snapshot["booth"]))
        ):
...
def _idle(ui):
    ...
    if (
        ...
        raise CaptureUnavailable(
            "Abort requires stopped farming and a safe input handoff"
        )
    return c
...
def recheck(ui):
    with ui.coordinator.lock:
        ...
        f, m = pair(ui, state["character"], farmer_preflight=True)
        now = time.time()
        proof = ownership(state, f, m, now=now)
        bound = sessions.binding(ui.runtime, state)
        if digest(probe.read_probe()) != digest(state):
            raise ValueError("Probe changed during abort preview")
        preview = dict(
            probe=state,
            probe_digest=digest(state),
            baseline={"farmer": f, "merchant": m},
            baseline_digest=digest(proof),
            sessions=bound,
...
def _confirmed(
    value, confirmation_reference, operator_confirmed, operator, *, allow_expired=False
):
    if (
        ...
        or value.get("confirmation_reference") != confirmation_reference
        or digest({k: v for k, v in value.items() if k != "confirmation_reference"})
        != confirmation_reference
        or time.time() < value["created_at"]
        or not allow_expired
        and time.time() > value["expires_at"]
    ):
...
def start(ui, *, confirmation_reference, operator_confirmed=False, operator):
    with ui.coordinator.lock:
        ...
        _profiles(ui, state)
        if (
            digest(state) != preview["probe_digest"]
            or control["revision"] != preview["control_revision"]
        ):
        ...
        previous = _prior(state)
        if (digest(previous) if previous else None) != preview.get(
            "prior_prepared_digest"
        ):
            raise ValueError("Previous abort attempt changed after preview")
        f, m = pair(ui, state["character"], farmer_preflight=True)
        _same_open(preview, f, m, now=time.time())
        ...
        abort = {
            **preview,
            ...
            "sessions": refreshed,
            "confirmed_session_history_digest": preview["sessions"]["digest"],
        }
        ...
        def work():
            try:
                ...
                if abort.get("phase") in (
                    "abort_prepared",
                    "cancel_submitted",
                ) and digest(read()) == digest(abort):
                    abort.update(error=str(error), failed_at=time.time())
                    save(abort)

        ui.delivery_probe_thread = threading.Thread(
            target=work, daemon=True, name="delivery-probe-abort"
        )
...
def _authorization(ui, abort, expected, *, submitted):
    ...
    if (
        ...
        or c["enabled"]
        or c.get("paused")
        or c["revision"] != abort["control_revision"]
        ...
        or digest(state) != abort["probe_digest"]
        or digest(read()) != expected()
    ...
    if (
        binding != abort["sessions"]
        or sorted(ui.coordinator.manual_sessions.values(), key=lambda row: row["id"])
        != binding["holds"]
    ):
...
def lease_authorized(ui, character):
    try:
        c = ui.coordinator
        if c.purpose != PURPOSE or not c.probe_abort_authorized(
            ui.runtime.manual_target(character)
        ):
            return False
        abort = read()
        state = abort["probe"]
        if str(character) != state["character"]:
            return False
        observer = ui.runtime.observers.get(character)
        controller = ui.runtime.controllers.get(character)
        if (
            observer is None
            or observer.adapter.identity != state["intent"]["merchant"]["identity"]
            or controller is None
            or controller.driver.observer is not observer
            or controller.driver.target is not observer.operations.target
            or type(observer.operations.target.hwnd) is not int
        ...
        return not active(
            ui.runtime.journal, character
        ) and not ui.runtime.journal.pending(character)
...
def run(ui, abort):
    ...
    character = abort["probe"]["character"]
    driver = ui.runtime.controllers[character].driver
    expected = [digest(abort)]
    submitted = [False]

    def authorize():
        return _authorization(
            ui, abort, lambda: expected[0], submitted=lambda: submitted[0]
        )
    ...
    def check():
        ...
        if any(ctypes.windll.user32.GetAsyncKeyState(k) & 0x8000 for k in (0x7A, 0x7B)):
            raise CaptureUnavailable("Probe abort stopped by F11/F12")
        if any(
            ui.coordinator.manual_session_blocked(target, purpose=PURPOSE)
            for target in authorize()
        ):
            raise CaptureUnavailable("Abort participant is fenced")

    with ui.coordinator.lock:
        f, m = pair(ui, character, farmer_preflight=True)
        _same_open(abort, f, m, now=time.time())
        with ui.coordinator.probe_abort_scope(authorize):
            check()
            with (
                ui.coordinator.lease(character, purpose=PURPOSE),
                physical_coordinates(),
            ):
                check()
                f, m = pair(ui, character, farmer_preflight=True)
                _same_open(abort, f, m, now=time.time())
                window, point = control(driver, m)
                size = driver.target.snapshot()["client_size"]
                if size != driver.memory.gui.viewport_size():
                    raise ValueError("Abort viewport changed")

                def guard():
                    check()
                    a, b = pair(ui, character, farmer_preflight=True)
                    _same_open(abort, a, b, now=time.time())
                    if control(driver, b) != (window, point):
                        raise ValueError("Native trade close control moved")
                ...
                abort.update(
                    phase="cancel_submitted", submitted_at=time.time(), point=point
                )
                save(abort)
                expected[0] = digest(abort)
                submitted[0] = True
                foreground_click(
                    driver.target,
                    *point,
                ...
                while True:
                    ...
                    f, m = pair(ui, character, farmer_preflight=True)
                    if all(
                        s.get("trade") is None and s.get("request") is None
                        for s in (f, m)
                    ):
                    ...
                    if time.monotonic() >= deadline:
                        raise ValueError(
                            "Abort close is uncertain; read-only reconciliation required"
                        )
                    time.sleep(0.05)
...
def _finish(abort, farmer, merchant):
    state = probe.read_probe()
    if digest(state) != abort["probe_digest"] and not (
        state.get("phase") == "cancel_verified"
    ...
    if abort["phase"] != "cancel_verified":
        ...
        abort.update(
            phase="cancel_verified",
            after={"farmer": farmer, "merchant": merchant},
            verified_at=time.time(),
            outcome="probe_abort_restored",
            historical_outcome="unknown",
            sales_receipt=False,
            delivery_receipt=False,
        )
        ...
        closed(
            abort,
            abort["after"]["farmer"],
            abort["after"]["merchant"],
            now=abort["verified_at"],
        )
    state = probe.read_probe()
    receipt_digest = digest(abort)
    if (
        state.get("phase") == "cancel_verified"
        and state.get("abort_receipt_digest") == receipt_digest
    ):
        probe.archive_probe(state)
        return abort
    if digest(state) != abort["probe_digest"]:
        raise ValueError("Original probe changed before abort publication")
    result = {
        **state,
        "phase": "cancel_verified",
        "updated_at": abort["verified_at"],
        "abort_receipt_digest": receipt_digest,
        "abort_receipt": deepcopy(abort),
        "sales_receipt": False,
        "delivery_receipt": False,
        "rebaseline_required": True,
    }
    result["original_failure"] = {
        key: state[key] for key in ("error", "failed_at", "finished_at") if key in state
    }
...
def reconcile(ui):
    ...
    with ui.coordinator.lock:
        abort = read()
        if not abort or abort.get("phase") not in (
            "cancel_submitted",
            "cancel_verified",
        ):
        ...
        _profiles(ui, abort["probe"])
        if (
            digest(abort["probe"]) != abort["probe_digest"]
            or digest(
                ownership(
                    abort["probe"],
                    abort["baseline"]["farmer"],
                    abort["baseline"]["merchant"],
                    now=abort["prepared_at"],
                )
            )
            != abort["baseline_digest"]
        ):
            raise ValueError("Saved abort baseline is corrupt")
        if abort["phase"] == "cancel_verified":
            return _finish(abort, None, None)
...
def _verified_receipt():
    ...
    if (
        not abort
        or abort.get("phase") != "cancel_verified"
        or not state
...
def disposition_recheck(ui):
    ...
    with ui.coordinator.lock:
        ...
        preview = dict(
            abort_receipt_digest=digest(abort),
            probe_digest=digest(state),
            sessions=sessions.binding(
                ui.runtime, abort["probe"], closed_after=abort["submitted_at"]
            ),
            current={"farmer": f, "merchant": m},
        ...
        preview["confirmation_reference"] = digest(preview)
        save(preview, "-disposition-preview")
        return preview


def disposition_override(
    ui, *, confirmation_reference, operator_confirmed=False, operator
):
    probe.recovery_available(ui)
    with ui.coordinator.lock:
        ...
        abort, _state = _verified_receipt()
        _profiles(ui, abort["probe"])

        def recheck():
            current, state = _verified_receipt()
            if (
                digest(current) != preview["abort_receipt_digest"]
                or digest(state) != preview["probe_digest"]
            ):
                raise ValueError(
                    "Abort receipt changed after session disposition preview"
                )

        recheck()
        terminal = all(
            ui.runtime.manual_sessions.get(record["row"]["id"])["phase"]
            == "operator_overridden"
        ...
        if not terminal:
            ...
            if digest(_disposition_ownership(abort, f, m)) != preview["current_digest"]:
                raise ValueError(
                    "Current closed ownership changed after disposition preview"
                )
        try:
            result = sessions.disposition(
                ui.runtime,
                abort["probe"],
                abort,
                preview["sessions"],
...
def require_rebaseline(ui, old):
    ...
    if ui.runtime._manual_rows():
        raise ValueError("Manual session/rebaseline holds block a fresh probe")
    if not old or not old.get("abort_receipt"):
        return
    ids = [
        record["row"]["id"] for record in old["abort_receipt"]["sessions"]["records"]
    ]
    with ui.runtime.journal.db() as db:
        for session_id in ids:
            row = db.execute(
                "SELECT phase FROM manual_rebaseline WHERE source_session_id=?",
                (session_id,),
            ).fetchone()
            terminal = db.execute(
                "SELECT phase FROM manual_sessions WHERE id=?", (session_id,)
            ).fetchone()
            if (
                not row
                or row[0] != "completed"
                or not terminal
                or terminal[0] != "operator_overridden"
            ):
                raise ValueError(
                    "Abort session disposition and two stable rebaseline samples are required"
                )
...
def dispatch(ui, body):
    ...
    reads = {
        "probe-delivery-abort-recheck": recheck,
        "probe-delivery-abort-reconcile": reconcile,
        "probe-delivery-abort-disposition-recheck": disposition_recheck,
    }
    if action in reads:
        if set(body) != {"action"}:
            raise ValueError("Unsupported abort observation arguments")
        return reads[action](ui)
    writes = {
        "probe-delivery-abort-start": start,
        "probe-delivery-abort-disposition-override": disposition_override,
    }
    if action not in writes or set(body) != {
        "action",
        "confirmation_reference",
        "operator_confirmed",
        "operator",
