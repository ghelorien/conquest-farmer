...
def attach(ui, pid, started, *, queued_at):
    ...
    if (
        ...
        or app.control.snapshot()["enabled"]
    ...
    candidates = [
        w
        for w in app.catalog.windows()
        if w.identity["pid"] == pid and w.identity["creation_time_100ns"] == started
    ]
    if len(candidates) != 1:
        raise ValueError("Selected client is absent, replaced or ambiguous")
    candidate = candidates[0]
    if any(
        o.adapter.identity == candidate.identity for o in ui.runtime.observers.values()
    ):
    ...
    try:
        ...
        if probe.adapter.identity != candidate.identity:
            raise ValueError("Selected client changed")
