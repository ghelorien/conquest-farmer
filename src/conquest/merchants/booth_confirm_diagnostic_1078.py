...
_METHOD_PREFIX = 96
_STABLE = ('identity', 'character', 'character_uid', 'server', 'map_id',
           'hp', 'silver', 'capacity', 'inventory', 'booth', 'own_booth_uid',
           'booth_open', 'trade', 'request')
...
def _model_methods(session, module, vtable_rva):
    ...
    for slot, pointer in zip(_SLOTS, pointers):
        ...
        if not 0x1000 <= rva <= size - _METHOD_PREFIX:
            methods.append({'slot': slot, 'method_rva': None,
                            'reason': 'outside_exact_module'})
            continue
...
def collect(runtime, character):
    ...
    profile = profiles.resolve(baseline['profile_id'], role='Merchant', server='America')
    if (not profile.local_enabled or profile.name != baseline['character']
            or profile.character_uid != baseline['character_uid']):
        raise ValueError('Merchant profile changed before booth diagnostic')
    if (baseline['map_id'] != 1036 or baseline['hp'] <= 0
            or not baseline['own_booth_uid'] or not baseline['booth_open']
            or not baseline['closed_modal']):
        raise ValueError('Diagnostic requires a living idle merchant with an open owned Market booth')
    ...
    with MemorySession(identity['pid'], CLIENT_SHA256_1078) as session:
        ...
        if (tuple(modal['geometry']) != geometry or geometry[2:] != (264., 92.)
                or not raw[0x97]
                or button_height != 18 or end_x != geometry[0] + 256
                or button_y != geometry[1] + 66
                or (start_x, start_y) != (geometry[0] + 8, geometry[1] + 26)):
            raise ValueError('1078 modal geometry differs from the observed dialog layout')
    ...
    return {'read_only': True, 'input_qualified': False,
            ...
            'candidate_control_layout': {'button_right_x': end_x,
                                         'button_y': button_y,
