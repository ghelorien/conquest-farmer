...
def available(lock):
    if not lock.acquire(blocking=False):
        raise ValueError(
            "Wait for the current merchant operation before qualification promotion"
        )
    try:
        yield
    finally:
...
def validate_candidate(observer, candidate, farmer, merchant, state):
    ...
    saved = state.get("recipient", {})
    expected = intent.get("merchant", {})
    ...
    if any(
        candidate.get("target_mode", {}).get(key) != native[key]
        for key in ("rva", "value")
    ):
...
def _same_participant(snapshot, receipt):
    return all(
        snapshot.get(field) == receipt.get(field)
        for field in ("identity", "character", "character_uid", "server", "silver")
    )
...
def _same_replay_participant(snapshot, receipt):
    ...
    return (
        _same_participant(snapshot, receipt)
        and all(
            snapshot.get(field) == receipt.get(field)
            for field in ("capacity", "map_id", "own_booth_uid")
        )
        and type(receipt.get("own_booth_uid")) is int
        and receipt["own_booth_uid"] > 0
...
def _stock(snapshot):
    ...
    try:
        return exact_items(snapshot["inventory"]), exact_listings(
            snapshot.get("booth", [])
        )
...
def _verified_listing(journal, row, before, result, expected_character):
    ...
    if (
        row.get("character") != database_character
        or row.get("kind") != "listing"
        or row.get("phase") != "verified"
        or not key.startswith(f"listing:{str(database_character)}:")
        or not _number(row.get("created"))
        or not _number(row.get("updated"))
    ...
    if (
        type(before.get("uid")) is not int
        or before.get("uid") != result.get("uid")
        or type(before.get("price")) is not int
        or before["price"] <= 0
        or result.get("price") != before["price"]
        or not isinstance(before.get("item"), dict)
    ...
    for step in steps:
        ...
        if (
            payload is _MALFORMED
            or not _number(step.get("timestamp"))
            or not row["created"] <= step["timestamp"] <= row["updated"]
        ):
            return None
        if step.get("stage") == "transaction":
            phases.append((step.get("status"), payload, step["timestamp"]))
    if [phase[0] for phase in phases] != ["prepared", "submitted", "verified"]:
        return None
    if (
        phases[0][1] != {}
        or phases[1][1] is not None
        or phases[-1][1] != result
        or phases[-1][2] != row["updated"]
    ):
        return None
    matching = []
    for event in events:
    ...
    if not (
        len(matching) == 1
        and matching[0][1].get("result") == result
        ...
        "steps": [
            {
                "stage": step["stage"],
                "status": step["status"],
                "payload": _json(step["payload"]),
                "timestamp": step["timestamp"],
            }
            for step in steps
        ],
        "event": {
            "event": matching[0][0]["event"],
...
def _listing_chain_reconciles(state, journal, farmer, merchant, *, now):
    ...
    """
    intent = state.get("intent", {})
    staged_farmer, state_merchant = (
        state.get("farmer_after"),
        state.get("merchant_after"),
    )
    verified_at = state.get("verified_at")
    if (
        not isinstance(staged_farmer, dict)
        or not isinstance(state_merchant, dict)
        or not _number(verified_at)
        or verified_at > now
    ):
        return False
    try:
        ...
        if staged_at > verified_at or not reconcile(
            intent, staged_farmer, state_merchant, now=staged_at
        ):
            return False
        ...
        return False
    if (
        not _same_participant(farmer, staged_farmer)
        or not _same_participant(merchant, state_merchant)
        or current_farmer != exact_items(staged_farmer["inventory"])
        or current_booth != exact_listings(staged_farmer.get("booth", []))
        or farmer.get("trade") is not None
        or merchant.get("trade") is not None
        or farmer.get("request") is not None
        or merchant.get("request") is not None
    ...
    if any(
        initial_inventory.get(uid) != attributes or uid in initial_booth
    ...
    expected_booth = dict(initial_booth)
    ...
    for row in rows:
        ...
        if (
            not _number(row.get("created"))
            or not _number(row.get("updated"))
            or row["created"] < verified_at
            or row["created"] < previous_updated
            or row["updated"] > merchant["timestamp"]
            or not _verified_listing(
                journal, row, before, result, intent["merchant"]["character"]
            )
        ):
            return False
        uid = before["uid"]
        ...
        if (
            not stock
            or not _same_replay_participant(snapshot, state_merchant)
        ...
        if not source_inventory or uid in expected_booth:
            return False
        source = next(
            (item for item in snapshot["inventory"] if item.get("uid") == uid), None
        )
        if (
            not isinstance(source, dict)
            or source.get("price") is not None
            or source.get("bound") is not False
            or source.get("type_id") == 1088001
            or expected_inventory[uid] != item_attributes
            or result.get("old_price") is not None
    ...
    return any(uid in current_merchant_booth for uid in delivered) and (
        current_inventory,
        current_merchant_booth,
    ) == (expected_inventory, expected_booth)
...
def _chain_manifest(state, journal, farmer, merchant, *, now):
    ...
    try:
        ...
        listings = []
        for row in rows:
            evidence = _verified_listing(
                journal,
                row,
                _json(row["before_json"]),
...
def _archive_chain_manifest(receipt, manifest):
    ...
    path = (
        Path(receipt).parent.parent
        / "delivery-promotion-chain-audit"
        / f"{digest}.json"
    )
    ...
    if path.exists():
        with path.open("r+b") as stream:
            if stream.read() != raw:
                raise ValueError(
                    "Listing chain manifest differs from its audit receipt"
                )
            stream.flush()
    ...
    try:
        with os.fdopen(fd, "wb") as stream:
            if stream.write(raw) != len(raw):
                raise OSError("Incomplete listing chain manifest")
            stream.flush()
            os.fsync(stream.fileno())
        try:
...
def promote_current(ui):
    ...
    with available(ui.coordinator.lock), ExitStack() as locks:
        ...
        if (
            not state
            or state.get("phase") != "delivery_verified"
        ...
        if (
            ...
            or context
            and (context.profile.role != "Farmer" or not context.profile.local_enabled)
        ):
        ...
        if (
            ...
            raise ValueError(
                "Both exact delivery observers must be attached before promotion"
            )
        for account in (observer, receiver):
            locks.enter_context(available(account.lock))
        paths = {
            "receipt": Path(
                state_path("reports/merchants/delivery-request-probe.json")
            ),
            "candidate": Path(
                state_path("reports/merchants/trade-layout-candidate.json")
            ),
            "farmer": qualification_path(observer, migrate=False),
            "merchant": merchant_directory(character) / "qualification.json",
        }
        if (
            delivery_probe.JOURNAL.resolve() != paths["receipt"].resolve()
            or controller.driver.qualification.resolve() != paths["merchant"].resolve()
        ):
            raise ValueError(
                "Delivery qualification paths differ from the selected profiles"
            )
        ...
        if merchant_before is not None:
            try:
                peer = json.loads(merchant_before)
            ...
            if (
                ...
                raise ValueError(
                    "Existing merchant qualification belongs to another participant or build"
                )
        ...
        def guard():
            ...
            for role, snapshot in (("farmer", farmer), ("merchant", merchant)):
                ...
                if any(
                    ...
                    for key in ("identity", "character", "character_uid", "server")
                ):
                    raise ValueError(
                        "Delivery participant identity differs from the verified receipt"
                    )
            if context and any(
                intent["farmer"].get(key) != getattr(context.profile, key)
                for key in ("character_uid", "server")
            ):
                raise ValueError(
                    "Verified farmer UID differs from its selected profile"
                )
            if profiles:
                profile = profiles.resolve(
                    state["target_profile_id"], role="Merchant", server="America"
                )
                if profile.character_uid != intent["merchant"]["character_uid"]:
                    raise ValueError(
                        "Verified merchant UID differs from its selected profile"
                    )
            observed_at = time.time()
            if reconcile(intent, farmer, merchant, now=observed_at):
                if promotion_mode not in (None, "direct"):
                    raise ValueError(
                        "Fresh delivery shape changed after listing-chain qualification"
                    )
                promotion_mode = "direct"
            else:
                if promotion_mode == "direct":
                    raise ValueError(
                        "Fresh delivery shape changed after direct qualification"
                    )
                manifest = _chain_manifest(
                    state, ui.runtime.journal, farmer, merchant, now=observed_at
                )
                if manifest is None:
                    raise ValueError(
                        "Fresh bilateral ownership does not reconcile the verified delivery or its verified listing chain"
                    )
                if (
                    chain_manifest is not None
                    and manifest["digest"] != chain_manifest["digest"]
                ):
                    raise ValueError(
                        "Verified listing chain changed during qualification validation"
                    )
            ...
            if (
                ...
                raise ValueError(
                    "Delivery authority changed during qualification validation"
                )
        ...
        chain_evidence = (
            _archive_chain_manifest(receipt, chain_manifest) if chain_manifest else None
        )
        delivery_qualification.promote(
            receipt,
            paths["candidate"],
            paths["farmer"],
            paths["merchant"],
        ...
        return {
            ...
            **(
                {"listing_chain_evidence": str(chain_evidence)}
                if chain_evidence
                else {}
            ),
            "farmer": {
                "profile_id": farmer_id,
                "character": intent["farmer"]["character"],
                "character_uid": intent["farmer"]["character_uid"],
                "identity": intent["farmer"]["identity"],
                "capabilities": {"farmer_delivery": True},
            },
            "merchant": {
                "profile_id": state["target_profile_id"],
                "character": str(character),
                "character_uid": intent["merchant"]["character_uid"],
                "identity": intent["merchant"]["identity"],
