...
class MarketSnapshot:
    def __init__(self, data, *, now=None, max_age=900):
        ...
        if data.get('source') != SOURCE or data.get('server') != 'America' or data.get('complete') is not True:
        ...
        if not isinstance(rows,list) or not 0 < len(rows) <= 50000 or data.get('total') != len(rows):
            raise ValueError('Market pagination is incomplete')
        self.data = data
        self.history_catalog = {}
        self.rows, self.listings, self.entries = [], [], []
        for row in rows:
            if row.get('server') != 'America' or not isinstance(row.get('name'),str) or not row['name']:
                raise ValueError('Market row has wrong server or missing identity')
            key = ItemKey(row['category'],row['quality'],row['plus'],tuple(row['sockets']),row.get('currency','silver'))
            self.rows.append(row)
            if row.get('quantity',1) is None:
                continue  # A price without an equivalent quantity cannot compete.
            listing = Listing(row['seller'],key,row['price'],row.get('quantity',1),row['server'])
            self.listings.append(listing);self.entries.append((row,listing))
    ...
    def key_for(self, item):
        ...
        if equipment and prefix in self.data.get('ambiguous_equipment_types',[])+self.history_catalog.get('ambiguous_equipment_types',[]):
            raise ValueError('Equipment type maps to conflicting market categories')
        mapped = (self.data.get('equipment_categories',{}).get(prefix)
                  or self.history_catalog.get('equipment_categories',{}).get(prefix)) if equipment else None
        named = [r for r in self.rows if r['name'] == item['name']]
        if not named:
            named = [r for r in self.history_catalog.get('rows',[]) if r['name'] == item['name']]
        ...
        if not equipment:
            ...
            if 700001 <= item['type_id'] <= 700073 and item['type_id']%10 in (1,2,3):
                expected_quality = {1:'Normal',2:'Refined',3:'Super'}[item['type_id']%10]
    ...
    def comparisons(self, item):
        ...
        return [Listing(r.seller,ItemKey(f'{r.key.category}:{raw["name"]}',r.key.quality,r.key.plus,r.key.sockets),
            r.price,r.quantity,r.server) for raw,r in self.entries if raw['name']==item['name']]
...
def browser_pages(data, definitions):
    ...
    """
    if (data.get('server')!='America' or data.get('source')!=SOURCE or data.get('last_page') is not True
            or data.get('initial_total') != data.get('final_total')
            or data.get('initial_change') != data.get('final_change')):
        raise ValueError('Market changed during collection or the last page was not reached; refresh and retry')
    ...
    if len(rows) != data['initial_total'] or any(len(p['rows'])!=50 for p in pages[:-1]) or not 1<=len(pages[-1]['rows'])<=50:
    ...
    for cells in rows:
        ...
        quantity = 1 if known and (all(100000<=d['id']<600000 for d in known)
            or all(d.get('amountLimit')==1 and d.get('amount')==1 for d in known)) else None
        plus = cells[2].strip()
        listings.append({'name':name,'category':category,'quality':cells[1].strip(),
            'plus':0 if plus=='—' else int(plus.removeprefix('+')),'sockets':sockets,
            'seller':cells[4].strip(),'server':'America','price':market_integer(cells[6]),
            'quantity':quantity,'currency':'silver'})
    result = {'source':SOURCE,'server':'America','complete':True,'observed_at':data['observed_at'],
    ...
    result['equipment_categories'] = {p:next(iter(categories)) for p,categories in equipment_categories.items() if len(categories)==1}
    result['ambiguous_equipment_types'] = [p for p,categories in equipment_categories.items() if len(categories)!=1]
