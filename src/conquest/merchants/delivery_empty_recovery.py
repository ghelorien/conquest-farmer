...
def _marker(trace):
    return next(
        (
            step
            for step in trace
            if step["stage"] == "cleanup_trade" and step["status"] == "before_action"
        ),
        None,
    )
...
def _incident(ui, key, character, *, allow_marker=False):
    ...
    if (
        not receipt
        or receipt["character"] != character
        or receipt["phase"] != "uncertain"
    ...
    markers = [
        s
        for s in trace
        if s["stage"] == "cleanup_trade" and s["status"] == "before_action"
    ]
    if (
        not any(
        ...
        or not any(
            s["stage"] == "trade_request" and s["status"] == "observed" for s in trace
        )
        or any(
        ...
        or (
            not allow_marker
            and any(
                s["stage"] == "cleanup_trade" and s["status"] == "observed"
                for s in trace
            )
        )
    ...
    if (
        ...
        or reservation.get("phase") not in ("reserved", "offer_ready")
        or exact_items(reservation["intent"]["items"]) != exact_items(intent["items"])
    ):
...
def _baseline(intent, farmer, merchant):
    ...
    if farmer["silver"] != intent["farmer"]["silver"] or exact_listings(
        farmer.get("booth", [])
    ) != exact_listings(intent["farmer"].get("booth", [])):
    ...
    gone = [
        item
        for item in intent["merchant"].get("booth", [])
        if item["uid"] not in current
    ]
...
def _same_since(ui, baseline, fresh, *, open_trade):
    ...
    for before, current in ((before_f, farmer), (before_m, merchant)):
        if any(
            current[k] != before[k]
            for k in ("identity", "character_uid", "map_id", "position")
        ):
    ...
    if farmer["silver"] != before_f["silver"] or exact_listings(
        farmer.get("booth", [])
    ) != exact_listings(before_f.get("booth", [])):
    ...
    expected_booth, silver_gain, receipts = _sale_adjustment(
        baseline, merchant, receipts
    )
    if (
        exact_listings(merchant.get("booth", [])) != expected_booth
...
def lease_authorized(ui, character):
    ...
    if (
        ui.coordinator.purpose != PURPOSE
        or state is None
        or state["character"] != character
        or threading.current_thread()
        is not getattr(ui, "delivery_empty_recovery_worker", None)
...
def _reconcile(ui, key, character, *, deadline=None):
    ...
    while True:
        ...
        try:
            ...
            observations = [
                s["payload"]["observed_at"]
                for s in _trace(journal, key)
            ...
            if observations and now - min(observations) >= SETTLE:
                ...
                return {
                    "request_id": key,
                    "phase": "closed_verified",
                    "source_uncertain": True,
                }
            if not observations:
                journal.step(
                    key,
                    "cleanup_recovery_observation",
                    "observed",
...
def _run(ui, state, token):
    ...
    try:
        with (
            ui.grant_fence.bind_worker(token),
            ...
            def guard():
                ...
                _same_since(
                    ui,
                    state["baseline"],
                    {"farmer": fresh_f, "merchant": fresh_m},
                    open_trade=True,
                )
...
def dispatch(ui, body):
    if set(body) != {"action", "request_id"} or body["action"] not in (
        "delivery-empty-recovery",
        "delivery-empty-recovery-status",
    ):
    ...
    receipt = status(journal, key)
    if not receipt:
        raise ValueError("Unknown exact delivery request")
    ...
    try:
        with ui.runtime.lock:
            if (
                ui.grant is not None
                or ui.runtime.handoff is not None
                ...
                raise CaptureUnavailable(
                    "Empty trade cleanup needs an idle protected app"
                )
            ...
            if control["enabled"] or control.get("paused"):
                raise CaptureUnavailable(
                    "Farmer must remain Off before empty trade cleanup"
                )
            ...
            token = ui.grant_fence.activate(
                grant_id, control["revision"], expires, scope=PURPOSE
            )
            ui.grant = {
                "request_id": grant_id,
                "revision": control["revision"],
