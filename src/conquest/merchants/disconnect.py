...
def _disconnect_locked(runtime,character,*,close):
    ...
    if runtime.journal.pending(character):
        raise ValueError('Reconcile the pending merchant transaction before disconnecting')
    ...
    runtime.journal.set(character,'attention',{'kind':'protective_disconnect',
    ...
    result={'character':character,'disconnected':closed is True,'time':time.time(),
            'identity':identity}
    runtime.journal.set(character,'protective_disconnect',result)
    runtime.journal.event(character,'protective_disconnect',disconnected=closed is True)
    return result
