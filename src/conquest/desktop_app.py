...
from conquest.farm_telemetry import (
    PickupHistory,
    pickup_values,
    activity_text,
    automation_status,
...
class DesktopApp:
    def __init__(
        ...
        *,
        ...
        self.host.api.height_scale = float(
            read_json(state_path(".runtime/farmer-view.json")).get("height_scale", 1.0)
        )
        ...
        self.control = FarmingControl(
            self.output / "controls.json"
            if output
            else state_path(".runtime/native-controls.json")
        )
        self.catalog = catalog or ClientCatalog(self.backend)
        launcher = Path(
            installation_path(
                r"C:\Program Files\Classic Conquer 2.0\ImBootstrapper.exe"
            )
        )
        self.launch_watch = launch_watch or LaunchWatch(
            self.catalog, [str(launcher)], cwd=launcher.parent
        )
        ...
        heading = (
            (
                self.character_context.profile.label
                or self.character_context.profile.name
            )
            if self.character_context
            else "Conquest Farmer"
        )
        ttk.Label(self.sidebar, text=heading, font=("Segoe UI", 20, "bold")).pack(
            anchor="w"
        )
        ...
        self.client_picker = ttk.Combobox(
            client_row, textvariable=self.client_text, state="readonly"
        )
        self.client_picker.pack(side="left", fill="x", expand=True)
        self.client_picker.bind("<<ComboboxSelected>>", self.select_client)
        ttk.Button(client_row, text="Refresh", command=self.refresh_client).pack(
            side="left", padx=(6, 0)
        )
        ...
        ttk.Label(activity_frame, textvariable=self.activity_text, wraplength=455).pack(
            anchor="w"
        )
        self.route_status = {}
        ...
        ttk.Label(self.sidebar, textvariable=self.stats_text, justify="left").pack(
            anchor="w", pady=4
        )
        ...
        self.start_button = ttk.Button(
            row, text="Start farming → Conquer", command=self.start
        )
        self.start_button.pack(side="left", fill="x", expand=True)
        ttk.Button(row, text="Stop", command=self.stop).pack(side="left", padx=(8, 0))
        self.foreground_note = ttk.Label(
            self.sidebar,
            text="Temporary foreground mode · Pheasants\nF11 pauses / resumes · F12 stops\nReturns to farming when Conquer regains focus.",
            wraplength=450,
        )
        ...
        ttk.Button(row, text="Show / focus Conquer", command=self.show_game).pack(
            side="left", padx=8
        )
        ttk.Button(row, text="Reload app", command=self.restart).pack(side="left")
        ttk.Button(row, text="Retry reconnect", command=self.retry_reconnect).pack(
            side="left"
        )
        self.client_frame.pack(fill="x", pady=(0, 4))
        self.attachment_text = tk.StringVar(value="Client not attached")
        ttk.Label(
            self.client_frame, textvariable=self.attachment_text, wraplength=440
        ).pack(fill="x")
        ttk.Button(
            self.client_frame,
            text="Copy attachment diagnostics",
            command=self.copy_attachment_diagnostics,
        ).pack(anchor="w")
        ttk.Button(
            self.client_frame, text="Use separate game window", command=self.show_game
        ).pack(anchor="w")
        ttk.Button(
            self.client_frame,
            text="Retry automation setup",
            command=self.retry_behavior_setup,
        ).pack(anchor="w")
        ...
        ttk.Button(row, text="Embed client", command=self.embed).pack(
            side="left", padx=7
        )
        ttk.Button(row, text="Release client", command=self.release).pack(side="left")
        ttk.Button(
            self.setup_frame, text="Check calibration", command=lambda: self.start(True)
        ).pack(anchor="w")
        ...
        self.route_picker = ttk.Combobox(
            route_row,
        ...
        ttk.Button(
            route_row, text="Save copy", command=self.save_route_copy, padding=3
        ).pack(side="left", padx=(5, 0))
        self.route_note = tk.StringVar(
            value="Save once, reuse on future runs.\nChoose a route to load its monster group.\nFull travel / hunt / restock cycle is being validated."
        )
        ttk.Label(
            self.route_details_frame, textvariable=self.route_note, wraplength=450
        ).pack(anchor="w")
        from conquest.session_plan import plan_note

        self.session_note = tk.StringVar(value=plan_note())
        session_row = ttk.Frame(self.route_details_frame)
        session_row.pack(fill="x", pady=(4, 0))
        ...
        self.level_preset_picker = ttk.Combobox(
            self.route_details_frame,
            textvariable=self.level_preset_text,
            state="readonly",
            values=[
                p["name"] + (" · needs survey" if p["status"] == "needs_survey" else "")
                for p in self.level_presets
            ],
        )
        ...
        ttk.Button(
            farm_row, text="Farming On · F10", command=lambda: self.update_ids(True)
        ).pack(side="left", fill="x", expand=True)
        ttk.Button(farm_row, text="Off", command=lambda: self.update_ids(False)).pack(
            side="left", padx=6
        )
        ttk.Label(farm_row, text="F11 pause · F12 stop").pack(side="right")
        from conquest.merchants.farmer_preferences import (
            rollout_enabled as transfers_enabled,
        )

        try:
            self.transfer_character = TrialConfig.model_validate(
                yaml.safe_load(self.profile.read_text())
            ).character
        except (OSError, ValueError, yaml.YAMLError):
            self.transfer_character = None
        self.merchant_transfers = tk.BooleanVar(
            value=bool(
                self.transfer_character and transfers_enabled(self.transfer_character)
            )
        )
        ttk.Checkbutton(
            self.sidebar,
            text="Automatic merchant delivery",
            variable=self.merchant_transfers,
            state="normal" if self.transfer_character else "disabled",
            command=self.save_merchant_transfers,
        ...
        self.recovery_frame = ttk.LabelFrame(
            self.sidebar, text="Manual handoff & recovery", padding=6
        )
        self.recovery_frame.pack(fill="x", pady=(0, 4))
        self.recovery_text = tk.StringVar(value="Checking for unresolved farmer holds…")
        ttk.Label(
            self.recovery_frame, textvariable=self.recovery_text, wraplength=420
        ).pack(anchor="w")
        recovery_row = ttk.Frame(self.recovery_frame)
        recovery_row.pack(fill="x", pady=(4, 0))
        ttk.Button(
            recovery_row, text="Recheck", command=self.recheck_farmer_recovery
        ).pack(side="left")
        ttk.Button(
            recovery_row,
            text="Override & resume",
            command=self.override_farmer_recovery,
        ).pack(side="left", padx=(6, 0))
        ttk.Button(
            self.recovery_frame,
            text="Clear stale handoff…",
            command=self.clear_stale_handoff,
        ).pack(anchor="w", pady=(4, 0))
        self.manual_handoff_note = tk.StringVar(
            value="Manual handoff: Start, then wait for Ready before touching any client."
        )
        ttk.Label(
            self.recovery_frame, textvariable=self.manual_handoff_note, wraplength=420
        ).pack(anchor="w", pady=(5, 0))
        ttk.Button(
            self.recovery_frame, text="Manual handoff…", command=self.manual_handoff
        ).pack(anchor="w", pady=(4, 0))

        self.mouse_note = tk.StringVar(
            value="Mouse control: automatic · move mouse to take over"
        )
        ttk.Label(self.sidebar, textvariable=self.mouse_note).pack(
            anchor="w", pady=(0, 4)
        )
        ...
        self.route_details_button = ttk.Button(
            footer, text="Show route details", command=self.toggle_route_details
        )
        self.route_details_button.pack(anchor="w", pady=(4, 0))
        self.details_button = ttk.Button(
            footer, text="Show client tools & details", command=self.toggle_details
        )
        ...
        self.ids_label = ttk.Label(
            self.setup_frame, text="Matched nearby IDs (automatic)"
        )
        self.ids_label.pack(anchor="w", pady=(6, 3))
        self.ids_entry = ttk.Entry(
            self.setup_frame, textvariable=self.ids, state="readonly"
        )
        self.ids_entry.pack(fill="x")
        self.memory_text = tk.StringVar(
            value="Embed a client to see nearby monster IDs"
        )
        ttk.Label(self.setup_frame, textvariable=self.memory_text, wraplength=450).pack(
            anchor="w"
        )
        self.detail_text = tk.StringVar(value="")
        ttk.Label(self.setup_frame, textvariable=self.detail_text, wraplength=450).pack(
            anchor="w", pady=4
        )
        ...
        pickup_frame = ttk.LabelFrame(
            self.activity_panel, text="Pickup history · newest first", padding=6
        )
        ...
        self.pickup_tree = ttk.Treeview(
            pickup_body, columns=("time", "item", "amount"), show="headings", height=9
        )
        for column, title, width in [
            ...
            self.pickup_tree.column(
                column, width=width, minwidth=width, stretch=column == "item"
            )
        pickup_scroll = ttk.Scrollbar(
            pickup_body, orient="vertical", command=self.pickup_tree.yview
        )
        ...
        self.detail_text.set(
            "Embedding checks actual memory access. Administrator access is only relevant if Windows denies that read."
        )
    ...
    def save_merchant_transfers(self):
        ...
        try:
            set_delivery_enabled(
                self.transfer_character, bool(self.merchant_transfers.get())
            )
            self.detail_text.set(
                "Automatic merchant delivery "
                + ("On" if self.merchant_transfers.get() else "Off")
                + " for "
                + self.transfer_character
            )
        except (OSError, ValueError) as error:
            self.merchant_transfers.set(rollout_enabled(self.transfer_character))
            self.detail_text.set(
                "Could not save merchant transfer setting: " + str(error)
            )

    def clear_stale_handoff(self):
        if getattr(self, "_stale_handoff_checking", False):
            return
        dispatch = getattr(self, "stale_handoff_dispatch", None)
        if dispatch is None:
            messagebox.showerror(
                "Clear stale handoff",
                "Merchant controls are not ready.",
                parent=self.root,
            )
            return
                ...
                try:
                    if self.mouse_priority.active():
                        if time.monotonic() >= deadline:
                            raise ValueError(
                                "Mouse is still active. Try again when ready."
                            )
        ...
        def preview_and_confirm():
            ...
            if not messagebox.askyesno(
                ...
                + "\nItem UID: "
                + str(item.get("uid", "unknown"))
                + "\n\n"
                "Trade history is kept. Farming stays Off.",
                parent=self.root,
            ):
                self._stale_handoff_checking = False
                return
            ...
            def clear():
                dispatch(
                    {
                        "action": "delivery-stale-pre-admission-clear",
                ...
                messagebox.showinfo(
                    "Clear stale handoff",
                    "Reservation cleared. Farming remains Off.",
                    parent=self.root,
                )
    ...
    def manual_handoff(self):
        ...
        if dispatch is None:
            messagebox.showerror(
                "Manual handoff", "Merchant controls are not ready.", parent=self.root
            )
            return
        try:
            ...
            if current:
                if messagebox.askyesno(
                    "End manual handoff",
                    "Keep the mouse idle after ending. Automation stays fenced until closed windows and stable ownership have been observed for five seconds.",
                    parent=self.root,
                ):
                    result = dispatch(
                        {
                            "action": "manual-handoff-end",
                            "session_id": current["id"],
                            "operator": "farmer pane",
                        }
                    )
                    self.manual_handoff_note.set(
                        "Manual handoff: settling closed windows; keep the mouse idle."
                    )
            else:
                result = dispatch(
                    {"action": "manual-handoff-start", "operator": "farmer pane"}
                )
                self.manual_handoff_note.set(
                    "Manual handoff: Preparing — wait for Ready before touching any client."
                )
        except (OSError, ValueError) as error:
    ...
    def _farmer_recovery_incidents(self):
        ...
        try:
            ...
            for state in pending():
                ...
                incidents.append(
                    {
                        "id": str(operation),
                        "kind": "protected-withdrawal",
                        "phase": state.get("phase", "blocked"),
                        "state": state,
                        "items": (
                            state.get("items")
                            or ([state.get("uid")] if state.get("uid") else [])
                        ),
                        "digest": digest,
                        "note": state.get(
                            "error",
                            "Protected warehouse withdrawal needs reconciliation",
                        ),
            ...
            incidents.append(
                {
                    "id": "protected-withdrawal:unreadable",
                    "kind": "protected-withdrawal",
                    "phase": "blocked",
                    "state": {},
                    "items": [],
        ...
        json_journals = (
            (
                "merchant-journey",
                ("prepared", "outbound_pending", "market", "return_pending"),
            ),
            (
                "meteor-consolidation",
                (
                    "withdrawing",
                    "travelling",
                    "exchange_ready",
        ...
        for name, phases in json_journals:
            path = Path(
                state_path(
                    (
                        "reports/banking/"
                        if name != "market-route-departure"
                        else ".runtime/"
                    )
                    + name
                    + ".json"
            ...
            incidents.append(
                {
                    ...
                    or f"{name} is awaiting recovery reconciliation",
                    "path": str(path),
        ...
        if isinstance(route, dict) and route.get("phase") == "needs_attention":
            incidents.append(
                {
                    "id": "overnight:needs_attention",
                    "kind": "overnight",
                    "phase": "needs_attention",
                    "state": route,
                    "items": [],
                    "digest": self._recovery_digest(route),
                    "note": route.get("detail")
                    or route.get("error")
                    or "Route controller needs attention",
                    "path": str(route_path),
                    "pending_phases": ("needs_attention",),
                }
            )
        return incidents
    ...
    def _choose_farmer_recovery(self):
        ...
        choices = tk.Listbox(
            dialog, width=84, height=min(10, len(incidents)), exportselection=False
        )
    ...
    def _farmer_resume_if_fresh(self, revision, fresh=None, *, deadline=None):
        ...
        expected_revision, epoch = (
            ...
            else (revision, getattr(self, "_recovery_epoch", 0))
        )
        if (
            self.control.snapshot()["revision"] != expected_revision
            or getattr(self, "_recovery_epoch", 0) != epoch
        ):
            return False
        ...
        usable = (
            current.get("observations_available")
            and life.get("character") == farmer_name()
        ...
        if self.mouse_priority.active() or not usable:
            self.recovery_text.set(
                "Hold cleared; waiting for fresh memory and mouse release"
            )
            if time.monotonic() < deadline:
                self.root.after(
                    250,
                    lambda: self._farmer_resume_if_fresh(revision, deadline=deadline),
                )
            return False
        if (
            self.control.snapshot()["revision"] != expected_revision
        ...
        try:
            ...
            self.recovery_text.set("Hold cleared; " + str(error))
            return False
    ...
    def _apply_farmer_override(self, incident, digest):
        ...
        with controller_guard() as acquired:
            if not acquired:
                raise ValueError(
                    "Route is still active; wait for it to stop before overriding"
                )
            ...
            if incident["kind"] == "meteor-consolidation":
                ...
                return meteor_banking.operator_override(
                    operator_confirmed=True,
                    confirmation_reference=digest,
                    incident_digest=digest,
                    fresh_evidence=fresh,
            ...
            return operator_override(
                Path(incident["path"]),
                pending_phases=incident["pending_phases"],
                operator_confirmed=True,
                confirmation_reference=digest,
                incident_digest=digest,
                fresh_evidence=fresh,
    ...
    def refresh_farmer_recovery(self):
        ...
        if not incidents:
            ...
            return incidents
        self.recovery_text.set(
            f"{len(incidents)} recovery hold(s) need attention · select one with Recheck"
        )
        return incidents
    ...
    def override_farmer_recovery(self):
        ...
        revision = (
            ...
            getattr(self, "_recovery_epoch", 0),
        )
        incident = self._farmer_fresh_recheck(incident)
        if not self._show_recovery_preview(incident, action="override"):
            return None
        try:
            result = self._apply_farmer_override(incident, incident["digest"])
        except (OSError, ValueError) as error:
    ...
    def record(self, **fields):
        ...
        if getattr(self, "kill_session", None):
            self.last.update(self.kill_session.snapshot())
        self.last.update(
            pid=os.getpid(),
            ...
            client_tools_revision=2,
            runback_monitor_revision=1,
            meteor_loop_revision=1,
            return_path_revision=2,
            scatter_projection_revision=3,
            level_eta_revision=1,
            empty_restock_revision=1,
            supply_refill_revision=1,
            scatter_ammo_minimum_revision=1,
            market_warehouse_click_revision=1,
        )
    ...
    def refresh_client(self):
        ...
        self.client_picker.configure(
            values=[c.label for c in self.clients], state="readonly"
        )
        selected = next(
            (
                c
                ...
                if self.client
                and (c.identity, c.hwnd) == (self.client[2], self.client[1])
            ),
            None,
        )
        ...
        if selected:
            ...
            self.client_text.set(
                "Select a client" if self.clients else "Launch Conquer to begin"
            )
    ...
    def retry_reconnect(self):
        ...
        if active():
            return
        if self.observer is not None and getattr(
            self.observer, "read_only_build", False
        ):
            self.state_text.set(
                "Reconnect is disabled for this observation-only client version"
            )
            return
        if self.observer is None or not login_screen(
            self.observer.operations.target.hwnd
        ):
            return
    ...
    def reconnect_client(self):
        ...
        if active():
            raise ValueError("Storage full: automatic reconnect is disabled")
        ...
        with self.observer.lock:
            return submit_login(
                self.observer.operations.target, session=self.observer.adapter
            )
    ...
    def refocus_if_farming(self):
        ...
        if hasattr(self, "attachment") and not self.attachment.ready:
            return
        if getattr(self, "unified", None) and (
            self.unified.coordinator.owner
            or (self.unified.grant and self.unified.safe_to_yield())
        ):
            return
        import win32gui
        ...
        def town_route_active():
            ...
            return (
                route.get("phase") in ("starting", "restocking")
        ...
        result = self.refocuser.step(
            state["enabled"] or town_route_active(), focused, activate
        )
        if result is not None:
            self.last.update(
                auto_refocus={"restored": result, "attempted_at": time.time()}
            )
        ...
        try:
            ...
            with input_scope():
                if self.host.saved:
                    ...
                    if not self.host.saved:
                        raise ValueError(
                            "Farmer pane could not retain the hosted client"
                        )
                    self.root.update_idletasks()
                    self.root.lift()
                focused = activate_focused_client(
                    hwnd,
                    identity,
                    api=self.host.api,
    ...
    def poll_pointer_focus(self):
        ...
        try:
            ...
            self.mouse_note.set(
                "Mouse control: yours · resumes after 2s idle"
                if manual
                else "Mouse control: automatic · move mouse to take over"
            )
            ...
            if (
                pressed
                and self.host.saved
                and self.host.api.pointer_in_client(self.host.saved.hwnd)
            ):
                self.focus_game()
        except Exception as error:
    ...
    def start(self, calibration=False):
        ...
        try:
            ...
            if config.observation_mode != "legacy_visual":
                raise ValueError(
                    "Memory-only farming requires an exactly selected embedded client. "
        ...
        try:
            ...
            if not ctypes.windll.shell32.IsUserAnAdmin():
                ...
                elevated_start(
                    self.root.winfo_id(), application_root(), self.profile, calibration
                )
            ...
            self.record(
                state="Checking calibration" if calibration else "Starting",
                kills=0,
                attempts=0,
            )
            ...
            if not self.show_game():
                raise ValueError(
                    "Windows could not bring Conquer forward; farming remains stopped"
                )
            ...
            if tuple(win32gui.GetClientRect(self.client[1])[2:]) != config.client_size:
                ...
                if (
                    tuple(win32gui.GetClientRect(self.client[1])[2:])
                    != config.client_size
                ):
                    raise ValueError(
                        "Maximized client does not match this screen profile"
                    )
            if not calibration:
                self.update_kill_metrics("begin")
            self.thread = threading.Thread(
                target=self.run, args=(config, calibration), daemon=False
            )
            self.thread.start()
        except Exception as error:
            self.root.deiconify()
    ...
    def stop(self, *, preserve_merchant_controls=False):
        ...
        if getattr(self, "reload_cancel", None):
        ...
        self.state_text.set(
            "Stopping…" if self.thread and self.thread.is_alive() else "Off"
        )
    ...
    def restart(self):
        if getattr(self, "reload_preparing", False):
            return False
        if getattr(
            getattr(getattr(self, "unified", None), "runtime", None), "connecting", None
        ):
            self.state_text.set(
                "Wait for the merchant connection to finish before reloading"
            )
            return False
        ...
        if not info or not self.selected_route:
            self.state_text.set(
                "Reload deferred: waiting for memory connection and route"
            )
            return False
        unified = getattr(self, "unified", None)
        if unified:
            if unified.coordinator.owner or unified.calibrating:
                self.state_text.set(
                    "Wait for merchant input before preparing the farmer for reload"
                )
                return False
        ...
        self.reload_resume = bool(
            self.control.snapshot()["enabled"]
            or (
                status.get("phase") in ("hunting", "restocking", "starting")
                and process_alive(status.get("pid"))
            )
        )
        self.reload_cancel = threading.Event()
        self.reload_preparing = True
        self.record(
            reload_preparing=True,
            current_activity="Moving to a safe spot for app reload",
        )
            ...
            try:
                ...
                checked = subprocess.run(
                    [
                        ...
                        str(layout.script("start_desktop_app.py")),
                        "--check-imports",
                    ],
                    cwd=repo,
                ...
                if checked.returncode:
                    raise ValueError("Startup check failed; keeping current app")
                from conquest.safe_reload import prepare as prepare_reload

                proof = prepare_reload(
                    info,
                    route_id,
                    self.reload_cancel,
                    lambda note: self.messages.put(
                        ("reload_activity", {"activity": note})
                    ),
                )
    ...
    def _restart_now(self):
        ...
        try:
            ...
            if checked.returncode:
                self.state_text.set(
                    "Reload canceled: startup check failed. See reports/desktop-startup-error.txt"
                )
                return False
        ...
        if selected:
            args += [
                "--embed-client",
                "--client-pid",
                str(selected.identity["pid"]),
                "--client-started",
                str(selected.identity["creation_time_100ns"]),
                "--client-hwnd",
                str(selected.hwnd),
            ]
        try:
    ...
    def launch(self):
        try:
            ...
            if self.host.saved or (self.thread and self.thread.is_alive()):
                raise ValueError(
                    "Stop farming and release the current client before launching another"
                )
            if self.character_context:
                from conquest.character_context import current
                ...
                if not context.installation:
                    raise ValueError(
                        "Configure this character’s game installation on this PC first"
                    )
                launcher = context.installation / "ImBootstrapper.exe"
                self.launch_watch = LaunchWatch(
                    self.catalog, [str(launcher)], cwd=launcher.parent
                )
    ...
    def embed(self, candidate=None):
        ...
        if self.observer:
            if candidate and candidate != self.client:
                self.state_text.set(
                    "Release the current client before selecting another"
                )
                return
        ...
        try:
            ...
            if self.character_context:
                ...
                self.root.after(
                    100,
                    lambda: self.wait_for_embed_layout(observer, time.monotonic() + 2),
                )
                return
            self.finish_embed()
        except Exception as error:
    ...
    def wait_for_embed_layout(self, observer, deadline, previous=None):
        ...
        try:
            ...
            if time.monotonic() < deadline and (not fits or size != previous):
                self.root.after(
                    100, lambda: self.wait_for_embed_layout(observer, deadline, size)
                )
                return
            require_viewport(*size)
            if not self.pane.winfo_ismapped():
                raise ViewportTooSmall(
                    "Select the farmer tab and retry Embed; the game pane is not visible."
                )
            self.embed_layout_pending = False
            self.finish_embed()
        except Exception as error:
    ...
    def finish_embed(self):
        try:
            self.host.attach(
                ...
                self.client[2],
                self.pane.winfo_id(),
                self.pane.winfo_width(),
                self.pane.winfo_height(),
            )
            self.attachment.attached = True
            automation_build = bool(
                getattr(self.observer, "automation_ready_build", False)
            )
            if getattr(self, "unified", None):
                self.unified.coordinator.surface_blocks["Farmer"] = bool(
                    getattr(self.observer, "read_only_build", False)
                    and not automation_build
                )
            ...
            if not getattr(self.observer, "read_only_build", False) or automation_build:
                ...
                self.attachment_text.set(
                    "Client attached · automation ready · farming Off"
                )
    ...
    def embed_failed(self, error):
        ...
        if self.attachment.stage not in ("memory", "behavior") or not self.host.saved:
            try:
                ...
                self.attachment.evidence["restoration_error"] = type(
                    cleanup_error
                ).__name__
        ...
        self.record(
            state="Client attached; automation blocked"
    ...
    def initialize_attached_behavior(self):
        self.runtime = ControlRuntime(
            self.control, None, None, None, farmer_name(), observer=self.observer
        )
        self.runtime.start()
        ...
        if hasattr(self.observer, "start_bridge"):
            worker_info = (
                Path(state_path(".runtime")) / f"embedded-worker-{os.getpid()}.json"
            )
            self.observer.start_bridge(
                worker_info,
                self.runtime.snapshot,
                control_update=self.update_control,
                on_reload=lambda: self.messages.put(("reload_requested", {})),
                on_native_window=lambda detached: self.messages.put(
                    ("native_window_requested", {"detached": detached})
                ),
            )
            self.observer.bridge.on_reconnect = lambda: self.messages.put(
                ("reconnect_requested", {})
            )
            ...
            if hasattr(self.observer, "adapter"):
                ...
                terrain = read_terrain(
                    ...
                    current.get("life", {}).get("map_id", 1002),
                )
                self.observer.bridge.on_route_jump = RouteJumpInput(
                    self.observer, terrain
                )
                self.runtime.recovery = RouteRecovery(
                    self.control,
                    self.observer.session.identity,
                    terrain,
                    EmbeddedRecoveryInput(self.observer, self.control),
                    state_path(".runtime/death-return.json"),
                )
                if self.selected_route:
                    self.runtime.recovery.enabled = (
                        self.character_context.settings.get(
                            "recover_after_death",
                            self.selected_route.recover_after_death,
                        )
                        if self.character_context
                        else self.selected_route.recover_after_death
                    )
        ...
        if getattr(self, "unified", None):
            self.unified.coordinator.surface_blocks["Farmer"] = False
    ...
    def initialize_read_only_attachment(self):
        ...
        if initial.get("observations_available") is not True:
            raise ValueError(
                initial.get(
                    "observation_note", "Initial memory observation is unavailable"
                )
            )
        self.runtime = ControlRuntime(
            self.control,
        ...
        self.runtime.start()
        worker_info = (
            Path(state_path(".runtime")) / f"embedded-readonly-{os.getpid()}.json"
        )
        ...
        self.attachment_text.set(
            "Client attached · observation ready · input and farming disabled for this client version"
        )
    ...
    def retry_behavior_setup(self):
        ...
        if self.control.snapshot()["enabled"] or (
            ...
            return
        if getattr(self.observer, "read_only_build", False) and not getattr(
            self.observer, "automation_ready_build", False
        ):
            self.attachment_text.set(
                "Observation-only client is already attached; farming remains disabled"
            )
            return
        try:
            if self.runtime:
            ...
            if getattr(self.observer, "bridge", None):
                self.observer.bridge.close()
                self.observer.bridge = None
            self.initialize_attached_behavior()
        except Exception as error:
    ...
    def change_native_window(self, detached, *, resume=True):
        ...
        try:
            ...
            with self.observer.lock:
                ...
                self.observer.bridge.sync_window_mode(self.host)
            self.record(
                state="Native client input check" if detached else "Embedded",
                hwnd=self.client[1],
                window_mode="detached" if detached else self.host.mode,
                embed_error=None,
            )
    ...
    def embedded_layout(self):
        ...
        self.pane.pack(
            in_=getattr(self, "content_parent", self.root),
            side="right",
            fill="both",
            expand=True,
        )
    ...
    def compact(self):
        ...
        self.nearby.refresh(
            [], self.control.snapshot()["target_type_ids"], available=False
        )
        self.setup_frame.pack(fill="x", in_=self.details_button.master)
        self.details_button.configure(text="Hide client tools & details")
    ...
    def toggle_nearby(self, type_id):
        try:
            ...
            current = self.control.update(
                {"target_type_ids": sorted(types), "target_ids": []}
            )
            if self.selected_route and sorted(types) != sorted(
                self.selected_route.monster_type_ids
            ):
                self.selected_route = None
                self.route_selection_path.unlink(missing_ok=True)
                self.route_text.set("Custom monster selection")
                self.route_note.set(
                    "Saved routes are unchanged.\nChoose a route again to restore its monster group.\nFull travel / hunt / restock cycle is being validated."
                )
            if self.runtime:
                data = self.runtime.snapshot()
                self.nearby.refresh(
                    data["monsters"],
                    current["target_type_ids"],
    ...
    def display_route(self, route):
        ...
        self.route_note.set(
            f"Levels {route.recommended_levels[0]}–{route.recommended_levels[1]} · Normal monster family selected\n"
    ...
    def select_level_preset(self, _event=None):
        ...
        if not preset["saved_route"]:
            self.route_note.set(
                "Named route plan saved; travel and hunting area still need surveying.\nYour current route remains selected."
            )
            return
        route = self.route_library.load(preset["saved_route"])
        self.route_picker.current(
            next(i for i, r in enumerate(self.saved_routes) if r.id == route.id)
        )
        self.select_route()
    ...
    def select_route(self, _event=None):
        try:
            ...
            self.control.update(
                {
                    "enabled": False,
                    "target_type_ids": list(route.monster_type_ids),
                    "target_ids": [],
                }
            )
            ...
            self.route_text.set(
                route_label(self.selected_route)
                if self.selected_route
                else "Choose a saved route"
            )
    ...
    def save_route_copy(self):
        ...
        name = simpledialog.askstring(
            "Save route copy", "Name for the reusable route:", parent=self.root
        )
        if name is None:
            return
        try:
            ...
            self.saved_routes = saved_route_choices(self.route_library.all())
            self.route_picker.configure(
                values=[route_label(r) for r in self.saved_routes]
            )
            self.route_picker.current(
                next(i for i, r in enumerate(self.saved_routes) if r.id == route_id)
            )
            self.select_route()
        except (ValueError, OSError) as error:
            self.route_note.set(str(error))
    ...
    def update_ids(self, enabled):
        ...
        if enabled and getattr(self, "character_context", None):
            if (
                self.character_context.profile.role != "Farmer"
                or not self.attachment.ready
            ):
                self.memory_text.set(
                    "Selected farmer is not automation-ready; see attachment diagnostics"
                )
                return
        ...
        try:
            ...
            with self.control.lock:
                ...
                if fresh_start:
                    ...
                    path = Path(state_path(".runtime/overnight.stop"))
            ...
            if enabled and self.runtime is None:
                self.control.publish(
                    ...
                    "Embed a client to observe selected monster IDs",
                )
    ...
    def update_control(self, body):
        ...
        if (
            type(explicit_start) is not bool
            or type(explicit_stop) is not bool
            or explicit_start
            and explicit_stop
            or explicit_start
            and body.get("enabled") is not True
            or explicit_stop
        ...
        if body.get("enabled") and active():
            raise ValueError(
                "Storage full: press Farming On manually after clearing storage"
            )
        ...
        with self.control.lock:
            ...
            fresh_start = bool(
                explicit_start and not self.control.snapshot()["enabled"]
            )
        ...
        try:
            ...
            if intent["enabled"]:
                ...
                self.control.publish(
                    intent["revision"],
                    "runner_stopped",
                    "Farm runner stopped: " + reason,
                )
                self.record(
                    state="Farming could not start",
    ...
    def _start_embedded_farm(self):
        fresh_start = (
            getattr(self, "_fresh_controller_start_revision", None)
            == self.control.snapshot()["revision"]
        )
        from conquest.merchants.delivery_operation import guard_protected_assets
        ...
        from conquest import meteor_banking
        from conquest.merchants.delivery_journey import (
            pending as merchant_journey_pending,
        )
        from conquest.manual_storage_recovery import pending as manual_storage_pending

        if (
            pending()
            or meteor_banking.pending()
            or merchant_journey_pending()
            or manual_storage_pending()
        ) and self.selected_route:
            from conquest.route_controller import ensure_running

            if (
                ensure_running(self.selected_route.id, fresh_start=fresh_start)
                and fresh_start
            ):
                self._fresh_controller_start_revision = None
            return
        ...
        route = self.selected_route or self.route_library.load(
            MONSTER_NAMES[base_types[0]].lower()
        )
        ...
        ammo_type = current_arrow(
            read_equipment(self.observer),
        ...
        if terrain.map_id != route.map_id:
            terrain = read_terrain(
                installation_path(r"C:\Program Files\Classic Conquer 2.0"), route.map_id
            )
        ...
        config = config.model_copy(
            update={
                ...
                "player_anchor": (viewport[0] // 2, viewport[1] // 2),
        ...
        config = config.model_copy(
            update={
                "ammo_type": ammo_type,
                "target_threshold": 0.84,
                "interval": speed.action_interval,
                "combat_speed": speed,
                "attack_progress_timeout": 1.2,
            }
        )
        ...
        with self.observer.lock:
            ...
            self.thread = threading.Thread(
                target=self.run_embedded_farm, args=(config,), daemon=False
            )
            self.thread.start()
    ...
    def run_embedded_farm(self, config):
        ...
        try:
            ...
            with physical_coordinates():
                ...
                supervisor = NativeFarmSupervisor(
                    self.observer,
                    self.control,
    ...
    def render_matched_ids(self, data):
        available = data.get("observations_available", False)
        self.ids_label.configure(
            text="Matched nearby IDs (automatic)"
            if available
    ...
    def stop_observer(self):
        if self.runtime:
            ...
            if self.runtime.thread and self.runtime.thread.is_alive():
                raise RuntimeError(
                    "Waiting for the last memory observation to stop; retry shortly"
                )
            self.runtime = None
        if self.observer:
            self.observer.close()
    ...
    def resize_host(self, width, height):
        if (
            getattr(self, "character_context", None)
            and self.host.saved
            and self.pane.winfo_ismapped()
        ):
            from conquest.client_attachment import require_viewport, ViewportTooSmall
            ...
            try:
                ...
                if getattr(self, "_farmer_viewport_blocked", False):
                    if getattr(self, "unified", None):
                        self.unified.coordinator.surface_blocks["Farmer"] = (
                            self._farmer_surface_block_before_viewport
                        )
                    marker = (
                        self.attachment.stage,
                        repr(self.attachment.error),
                        len(self.attachment.history),
                    )
                ...
                if not getattr(self, "_farmer_viewport_blocked", False):
                    ...
                    self._farmer_viewport_attachment_marker = (
                        self.attachment.stage,
                        repr(self.attachment.error),
                        len(self.attachment.history),
                    )
                    self._farmer_surface_block_before_viewport = (
                        bool(self.unified.coordinator.surface_blocks.get("Farmer"))
                        if getattr(self, "unified", None)
                        else False
                    )
                if getattr(self, "unified", None):
                    self.unified.coordinator.surface_blocks["Farmer"] = True
                self.attachment.ready = False
                ...
                self.host.api.assert_owner(
                    self.host.saved.hwnd, self.host.saved.identity
                )
    ...
    def _poll(self):
        if time.monotonic() >= getattr(self, "_manual_handoff_status_at", 0):
            ...
            if dispatch:
                try:
                    handoff = dispatch({"action": "manual-handoff-status"}).get(
                        "handoff"
                    )
                    if handoff:
                        phase = handoff.get("phase")
                        note = {
                            "preparing": "Preparing — wait for Ready before touching any client.",
                            "ready": "Ready — trade manually; automation remains paused.",
                            "ending": "Settling — keep the mouse idle.",
                            "needs_attention": "Needs attention — do not trade.",
                        }.get(phase, "Manual handoff active.")
                        self.manual_handoff_note.set("Manual handoff: " + note)
                    else:
                        self.manual_handoff_note.set(
                            "Manual handoff: Start, then wait for Ready before touching any client."
                        )
        ...
        if not self.closing and not storage_halted:
            resume_after_embed(self)
        if not self.closing and not storage_halted:
            self.refocus_if_farming()
        if (
            not self.closing
            and not storage_halted
        ...
        if self.runtime:
            data = self.runtime.snapshot()
            if (
                self.attachment.ready
                and self.reconnect_pending
                and not login_screen(self.observer.operations.target.hwnd)
                ...
                self.reconnect_pending = False
                if self.control.snapshot()["enabled"] and not (
                    self.thread and self.thread.is_alive()
                ):
                    self.start_embedded_farm()
            self.nearby.refresh(
                data["monsters"],
                data["control"]["target_type_ids"],
                available=data.get("observations_available", False),
            )
            ...
            if (
                ...
                self.state_text.set(
                    "Farming On · "
                    + data["control"]["execution_state"].replace("_", " ")
                    if data["control"]["enabled"]
                    else "Client embedded · farming Off"
                )
            self.memory_text.set(
                data["control"]["note"]
                if data.get("observations_available")
                else data.get("observation_note", "Waiting for the client")
            )
        while not self.messages.empty():
            ...
            activity = {
                "attack_attempt": (
                    "Casting Scatter at nearby monsters"
                    if fields.get("button") == "right"
                ...
                "healing_attempt": "Using a healing potion",
                "reload_attempt": "Reloading arrows",
                ...
                "memory_pickup_attempt": ("Picking up " + item_label(fields))
                if event == "memory_pickup_attempt"
                else "",
            }
            ...
            if event == "automation_work":
                ...
                self.route_selection_path.write_text(
                    json.dumps({"route_id": route.id}), encoding="utf-8"
                )
                self.display_route(route)
                if self.runtime.recovery:
                    self.runtime.recovery.cancel()
                self.record()
                ...
                if not self.reload_cancel.is_set() and self._restart_now():
                    return False
                self.messages.put(
                    (
                        "reload_failed",
                        {"detail": "Safe handoff deferred; current app retained"},
                    )
                )
                ...
                self.record(
                    ...
                    activity="Reload deferred",
                    reload_detail=fields["detail"],
                )
                ...
                self.record(
                    ...
                    state=(
                        ...
                        if fields["state"] == "reconnect_exhausted"
                        else "Reconnecting"
                        if fields["state"] != "connection_restored"
                        else "Checking reconnected character"
                    ),
                )
                ...
                if (
                    self.control.snapshot()["enabled"]
                    and not self.mouse_priority.active()
                ):
                    self.show_game()
                ...
                "loot_discarded",
                "loot_discard_deferred",
                "loot_discard_unverified",
            ):
                self.record(
                    inventory_cleanup={
                        "event": event,
                ...
                self.record(
                    pickups=fields["total"], last_pickup=fields, loot_reader_note=None
                )
            elif event in (
                ...
                self.record(
                    health_ratio=fields["health_ratio"],
                    ammo=fields["ammo"],
                ...
                self.record(
                    ...
                    state="Returning to hunting spot",
                    boundary_return=fields,
                )
                ...
                self.record(
                    state="Travelling to hunting area"
                    if fields.get("approaching")
                    else "Patrolling"
                )
            elif event == "navigation_wait":
                self.record(
                    navigation_blocked=True, state=pause_message(fields.get("reason"))
                )
            elif event == "movement_recovery":
                self.record(
                    navigation_blocked=True,
                    state="Taking another path",
                ...
                self.record(
                    state=pause_message(fields.get("reason"))
                    if event == "paused"
                    else "Running"
                )
                ...
                self.detail_text.set(
                    fields.get("detail") or fields.get("reason", "Stopped")
                )
                if event == "failed":
                    self.root.deiconify()
            self.state_text.set(self.last["state"])
            self.stats_text.set(
                farm_stats(self.last, self.control.snapshot()["enabled"])
            )
        if not self.closing and time.monotonic() - self.telemetry_poll_at >= 0.5:
            self.telemetry_poll_at = time.monotonic()
            self.update_kill_metrics()
            try:
                self.route_status = json.loads(
                ...
                try:
                    ...
                    if ensure_running(self.selected_route.id, fresh_start=fresh_start):
                        if fresh_start:
                            self._fresh_controller_start_revision = None
                        self.record(
                            route_controller="Restarting automatic route management"
                        )
                except (OSError, ValueError) as error:
                    self.record(route_controller_error=str(error))
            if (
                self.observer is not None
                and getattr(self.observer, "read_only_build", False)
                and not self.attachment.ready
            ):
                self.state_text.set("Client embedded · observation only")
                self.activity_text.set(
                    "Input, route and recovery automation disabled for this client version"
                )
            else:
                life = self.runtime.snapshot().get("life") if self.runtime else None
                execution, activity = automation_status(
                    self.route_status, self.last, control, life
                )
        ...
        if self.closing and self.thread is None:
            try:
                ...
                self.closing = False
                self.state_text.set(
                    f"Keep this app open: client restoration failed: {error}"
                )
        if storage_halted:
            enforce(self)
        return True
...
def main():
    ...
    if args.embed_client and any(value is None or value <= 0 for value in selected):
        parser.error(
            "--embed-client needs the selected process ID, creation time and HWND"
        )
    ...
    if getattr(app, "profile_editor_requested", None):
        ...
        if "--profile-id" in args:
            args[args.index("--profile-id") + 1] = app.profile_editor_requested
        from conquest.application_layout import RuntimeLayout
