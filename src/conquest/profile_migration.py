...
def _copy(source, target):
    ...
    if source.suffix.lower() in (".sqlite", ".sqlite3"):
        ...
        with tempfile.TemporaryDirectory(
            dir=target.parent, prefix=".sqlite-source-snapshot-"
        ) as temporary:
            ...
            with (
                closing(sqlite3.connect(snapshot)) as before,
                closing(sqlite3.connect(target)) as after,
            ):
                before.backup(after)
    else:
...
def _safe_source(source):
    ...
    if not source.is_dir() or _reparse(source):
        raise ValueError(
            "Migration source must be a real local directory, not a reparse point"
        )
...
def _pruned_state_directory(relative):
    ...
    if not parts or parts[0] not in (".runtime", "reports"):
        return False
    return any(
        part in PRUNED_STATE_DIRECTORIES or part.startswith("models--")
        for part in parts[1:]
    )
...
def _safe_tree(root, relative, *, prune=None):
    ...
    if _reparse(base) or not _inside(base, root):
        raise ValueError(
            "Migration source contains a junction, symlink or escaping path"
        )
    found = []
...
def _source_token(path):
    ...
    try:
        ...
        raise ValueError(
            "Migration source file changed while it was inspected"
        ) from None
...
def _selected_source_paths(source):
    ...
    for path in _state_source_paths(source):
        if (
            path.is_file()
            and path.suffix.lower() in COPY_SUFFIXES
...
def _same_process(recorded, actual):
    ...
    if (
        type(creation) is not int
        or creation <= 0
        or actual.get("creation_time_100ns") != creation
    ):
        return False
    ...
    if path is not None:
        if not isinstance(path, str) or not isinstance(actual.get("path"), str):
            return False
        if os.path.normcase(os.path.abspath(actual["path"])) != os.path.normcase(
...
def require_offline(source, *, backend=None):
    ...
    for path in _state_source_paths(source):
        ...
        try:
            ...
            if hinted:
                raise ValueError(
                    "A durable Conquest controller receipt is unreadable"
                ) from None
            continue
        for identity in _identity_records(value):
            ...
            try:
                ...
                if getattr(error, "winerror", None) not in (None, 6, 87, 1168):
                    raise ValueError(
                        "Cannot verify whether a recorded Conquest process is still live"
                    ) from None
                continue
            if _same_process(identity, actual):
                raise ValueError(
                    "Close the existing Conquest app and workers before importing local state"
                )
...
def _diagnostics_only(root):
    ...
    return all(
        ...
        or (
            entry.is_file()
            and entry.name.lower().startswith("diagnostic")
            and entry.suffix.lower() in (".json", ".log")
        )
...
def _remap_journal(journal, by_name):
    with closing(sqlite3.connect(journal)) as db, db:
        ...
        if (
            ...
            raise ValueError(
                "Reconcile nonterminal manual sessions in the legacy installation before migration"
            )
        affected = {}
        for table in tables:
            ...
            selected = tuple(
                column
                for column in ("character", "target_profile_id")
        ...
        for table, selected in affected.items():
            ...
            for column in selected:
                ...
                for name, profile in by_name.items():
                    db.execute(
                        f"UPDATE {quoted} SET {qcolumn}=? WHERE {qcolumn}=?",
                        (profile.id, name),
                    )
    ...
    with closing(sqlite3.connect(journal)) as db:
        ...
        for profile in merchants:
            ...
            for row in db.execute(
                ...
                if (
                    trade.get("participant") == farmer.name
                    and type(uid) is int
                    and uid > 0
                ):
                    sources.add(uid)
...
def migrate_legacy(
    source, root, *, check_offline=require_offline, copy_file=_copy, lock_timeout=0
):
    ...
    from conquest.profile_bootstrap import (
        managed_root_owner,
        legacy_app_owner_if_present,
    )
    ...
    with ExitStack() as owners:
        ...
        if os.path.lexists(marker):
            ...
            try:
                ...
                raise ValueError(
                    "Destination has an invalid migration marker"
                ) from None
            if not isinstance(completed, dict) or completed.get("state") != "complete":
                raise ValueError(
                    "Destination already contains an incomplete managed installation"
                )
            return completed
        ...
        try:
            ...
            merchants = [
                registry.add(name, role="Merchant") for name in ("Spiritual", "Dutch")
            ]
            ...
            credentials = {
                ".runtime/account.dpapi": stage
                / "accounts"
                / farmer.account_id
                / "account.dpapi",
                ".runtime/merchants/spiritual/account.dpapi": stage
                / "accounts"
                / by_name["Spiritual"].account_id
                / "account.dpapi",
                ".runtime/merchants/dutch/account.dpapi": stage
                / "accounts"
                / by_name["Dutch"].account_id
                / "account.dpapi",
                ".runtime/discord-webhook.dpapi": stage
                / "characters"
                / farmer.id
                / ".runtime/discord-webhook.dpapi",
                ".runtime/merchants/shops-webhook.dpapi": stage
                / "machine-state/.runtime/merchants/shops-webhook.dpapi",
            }
            ...
            for path in selected:
                ...
                if relative in credentials:
                    ...
                    if builtin.exists() and builtin.read_bytes() == path.read_bytes():
                        continue
                    destination = (
                        stage / "characters" / farmer.id / "routes" / path.name
                    )
                elif relative_path.parts[0] in (".runtime", "reports"):
                    shared = relative.startswith(
                        (
                            ".runtime/merchants/",
                            "reports/merchants/",
                            ".runtime/shop-",
                    ...
                    destination = (
                        stage
                        / ("machine-state" if shared else "characters/" + farmer.id)
                        / relative
                    )
                else:
                    raise ValueError(
                        "Selected migration source has no destination policy"
                    )
            ...
            result = {
                ...
                "profiles": {
                    profile.name: profile.id for profile in (farmer, *merchants)
                },
                "state": "complete",
