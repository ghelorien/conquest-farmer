...
def _same_intent(left,right):
    ...
    if any(left.get(k)!=right.get(k) for k in
           ('operation_id','town_visit_id','visit_id','farmer_profile_id')):return False
    if exact_items(left['items'])!=exact_items(right['items']):return False
    for role in ('farmer','merchant'):
        a,b=left[role],right[role]
        if (any(a.get(k)!=b.get(k) for k in ('identity','character','character_uid','server',
                    'map_id','position','silver','capacity','trade','request','own_booth_uid','booth_open'))
                or exact_items(a['inventory'])!=exact_items(b['inventory'])
                or exact_listings(a.get('booth',[]))!=exact_listings(b.get('booth',[]))):return False
    return True
...
def _bag_matches_snapshot(bag, snapshot):
    ...
    if (len(items)!=len(bag['items']) or len(inventory)!=len(snapshot['inventory'])
            or set(items)!=set(inventory) or bag['silver']!=snapshot['silver']
            or bag['capacity']!=snapshot['capacity']):return False
    for uid,item in items.items():
        peer=inventory[uid]
        if any(item.get(k)!=peer.get(k) for k in ('type_id','plus')):return False
        ...
        if quantity!=peer.get('quantity'):return False
        if any(item[k]!=peer.get(k) for k in ('gem1','gem2','bound') if k in item):return False
    return True
...
def _delivery_proof(row, meteor, market, failure, target):
    ...
    origin={'town_visit_id':row['town_visit_id'],'visit_id':market['visit_id'],
            'farmer_profile_id':row['farmer_profile_id']}
    expected=deepcopy(meteor['after']);proofs=[];seen=set()
    with closing(sqlite3.connect(delivery_operation.JOURNAL.resolve().as_uri()+'?mode=ro',uri=True)) as db:
        db.row_factory=profile_row
        transactions=list(db.execute('SELECT * FROM transactions WHERE created>=? ORDER BY created',(row['required_at'],)))
        admissions=list(db.execute('SELECT * FROM delivery_admissions WHERE created>=?',(row['required_at'],)))
        if (not transactions or len(transactions)!=len(admissions)
        ...
        with closing(sqlite3.connect(receiver_path.resolve().as_uri()+'?mode=ro',uri=True)) as peer_db:
            for tx in transactions:
                before=json.loads(tx['before_json']);result=json.loads(tx['result_json'] or '{}')
                key=tx['id'];admission=admission_by_id[key]
                admission_origin=json.loads(admission['origin_json'])
                if (tx['kind']!='farmer_delivery' or tx['phase']!='verified'
                        or not meteor['started_at']<=tx['created']<=tx['updated']<failure['time']
                        or any(before.get(k)!=v or admission_origin.get(k)!=v for k,v in origin.items())
                        or before.get('operation_id')!=key or admission_origin.get('operation_id')!=key
                        or admission['phase']!='transaction_started' or admission['character']!=tx['character']
                        or before['farmer']['identity']!=target or result.get('outcome')!='delivered'
                        or result.get('remaining') or result.get('cleanup_pending')
                        or result.get('operator_additions') or result.get('sale_receipts')
                        or not _bag_matches_snapshot(expected,before['farmer'])):
                    raise ValueError('Restock delivery lacks an exact terminal ownership chain')
                offered=exact_items(before['items'])
                if (set(offered)&seen or sorted(offered)!=sorted(json.loads(admission['uids_json']))
                ...
                trace=[{**dict(s),'payload':json.loads(s['payload'])} for s in db.execute(
                    'SELECT stage,status,payload,timestamp FROM transaction_steps WHERE transaction_id=? ORDER BY id',(key,))]
                proved=reconciliation_outcome(before,result['farmer'],result['merchant'],trace=trace,
                    now=result['reconciled_at'])
                if (proved['outcome']!='delivered' or proved['cleanup_pending']
                        or proved['proof_digest']!=result.get('proof_digest')
                        or not any(s['stage']=='receiver_receipt' and s['status']=='observed'
                                   and (s['payload'] or {}).get('phase')=='verified' for s in trace)):
                    raise ValueError('Bilateral restock delivery proof is incomplete')
                receiver=peer_db.execute('SELECT state FROM delivery_reservations WHERE request_id=?',(key,)).fetchall()
                peer=json.loads(receiver[0][0]) if len(receiver)==1 else {}
                if (peer.get('phase')!='verified' or not _same_intent(peer.get('intent') or {},before)
                        or not tx['created']<=peer.get('verified_at',0)<failure['time']):
                    raise ValueError('Receiver terminal delivery receipt is unavailable')
                records=[r for r in route.get('receipts',[]) if r.get('request_id')==key]
                if (len(records)!=1 or records[0].get('outcome')!='transferred'
                        ...
                        or not tx['updated']<=records[0].get('verified_at',0)<failure['time']):
                    raise ValueError('Route delivery receipt is not linked to the bilateral proof')
                expected['items']=[i for i in expected['items'] if i['uid'] not in offered]
                if not _bag_matches_snapshot(expected,result['farmer']):
                    raise ValueError('Post-delivery Farmer ownership is unexplained')
                seen.update(offered)
                proofs.append({'request_id':key,'proof_digest':proved['proof_digest'],
                               'uids':sorted(offered),'verified_at':records[0]['verified_at']})
    attempts=market.get('attempts') or []
    if (len(attempts)!=len(proofs) or any(a.get('outcome')!='transferred' for a in attempts)
            or any(not market['started_at']<=a.get('at',0)<failure['time'] for a in attempts)):
...
def capture(loop,row,meteor,*,no_transfer=False):
    ...
    target=row.get('restock_target');visit=loop.town_visit
    if (row.get('phase')!='town_work' or row.get('reasons')!=['restock']
            or not recovery._process_identity(target) or row.get('route_id')!=loop.route.id
            or row.get('hunt_map_id')!=loop.route.map_id or meteor.get('phase')!='storing_scroll'
            or meteor.get('origin')!=loop.route.restock_map_id or meteor.get('exchange_verified') is not True
    ...
    if (status.get('input_owner') is not None or status.get('handoff_granted') is not False
            or pending is not None and qualified_listing_request(status) is None
            or manual.get('session') or manual.get('input_fenced')
            or observation.get('available') is not True or observation.get('windows_absent') is not True
            or not 0<=time.time()-observation.get('observed_at',0)<=5 or recovery._other_holds()):
