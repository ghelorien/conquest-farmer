...
class KillLootCycle:
    ...
    def validate(self, observation, now):
        ...
        if (
            ...
            or not 0
            <= observation.timestamp - observation.inventory.timestamp
            <= self.stale_after
        ):
        ...
        if self.context is not None and self.context != observation.context:
            self.fault(
                "Client or map changed during the encounter; switch Off to reset"
            )
    ...
    def plan(self, monsters, selected_ids, observation, now):
        ...
        if (
            ...
            and observation.timestamp - self.last_sample > 2
        ):
        ...
        if self.target is None:
            target = next(
                (
                    m
                    for m in monsters
                    if m["entity_id"] in selected_ids
                    and m.get("alive") is True
                    and m.get("object_address")
                ),
                None,
            )
            if target is None:
                self.state, self.note = (
                    "waiting_for_target",
                    "Waiting for a verified living target",
                )
                return None
        ...
        if self.kill is None:
            matches = [
                k
                for k in observation.kills
                if k.monster_id == self.target["entity_id"]
                and k.object_address == self.target["object_address"]
                and self.started <= k.timestamp <= observation.timestamp
                and k.event_id
            ]
            ...
            if matches:
                ...
                target = next(
                    (
                        m
                        for m in monsters
                        if m["entity_id"] == self.target["entity_id"]
                ...
                self.state, self.note = (
                    "attacking",
                    f"Waiting for confirmed kill of ID {self.target['entity_id']}",
                )
        ...
        for drop in observation.drops:
            ...
            if previous and (previous.type_id, previous.amount, previous.silver) != (
                drop.type_id,
                drop.amount,
                drop.silver,
            ):
        ...
        if self.pending:
            ...
            if (
                drop.uid not in drops
                and observation.inventory.timestamp > issued
                ...
                self.state, self.note = (
                    "waiting_for_pickup",
                    f"Confirming pickup of drop ID {drop.uid}",
                )
                return None
            else:
                self.pending = None
                if (
                    drop.uid not in drops
                    or self.attempts[drop.uid] >= self.pickup_limit
                ):
                    self.fault(
                        f"Pickup of drop ID {drop.uid} was not confirmed; holding next target"
                    )
                    return None
        ...
        if remaining:
            ...
            if uid not in drops or drops[uid].kill_event_id != self.kill.event_id:
                self.fault(
                    f"Drop ID {uid} disappeared without a confirmed pickup; holding next target"
                )
                return None
            drop = drops[uid]
            ...
            if (
                ...
                and not any(
                    i.type_id == drop.type_id and i.limit - i.amount >= drop.amount
            ...
            self.state, self.note = (
                "looting",
                f"Picking up drop ID {uid} from monster ID {self.target['entity_id']}",
            )
            return EncounterAction("pickup", self.target, drop)

        self.state, self.note = (
            "waiting_for_drops",
            f"Checking drops from monster ID {self.target['entity_id']}",
        )
    ...
    def committed(self, action, observation, now):
        ...
        if action.kind == "attack":
            ...
            self.state, self.note = (
                "attacking",
                f"Attacking monster ID {self.target['entity_id']}",
            )
        else:
            drop = action.drop
