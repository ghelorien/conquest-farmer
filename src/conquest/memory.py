...
class MemorySession:
    def __init__(self, pid: int, expected_sha256: str, executable="ImConquer.exe"):
        ...
        self.query_api = bind(
            self.backend.kernel,
            "VirtualQueryEx",
            [w.HANDLE, c.c_void_p, c.POINTER(MemoryBasicInformation), c.c_size_t],
            c.c_size_t,
        )
    ...
    def __enter__(self):
        ...
        if self.identity["architecture"] != "x64":
            raise ValueError(
                "This calibration adapter supports the inspected x64 client only"
            )
        ...
        try:
            ...
            main = next(
                (
                    ...
                    if Path(module["path"]).name.casefold()
            ...
            if main is None or self.read(main["base"], 2) != b"MZ":
                raise ValueError(
                    "Could not verify an actual read of the main module header"
                )
            return self
    ...
    def read(self, address: int, size: int) -> bytes:
        ...
        if not self.read_api(self.handle, address, buffer, size, c.byref(received)):
            raise self.backend.error(
                f"ReadProcessMemory(address=0x{address:x}, size={size})"
            )
        if received.value != size:
            raise OSError(
                f"Short memory read: expected {size}, received {received.value}"
            )
        return buffer.raw
    ...
    def regions(self):
        ...
        while address < 0x7FFFFFFFFFFF:
            ...
            size = self.query_api(
                self.handle, address, c.byref(information), c.sizeof(information)
            )
            if not size:
            ...
            if (
                ...
                and information.Type in (0x20000, 0x1000000)
                ...
                yield Region(
                    ...
                    information.AllocationBase or 0,
                    information.Type,
                    information.Protect,
                )
            address = end
    ...
    def _modules(self):
        first = bind(
            self.backend.kernel,
            "Module32FirstW",
            [w.HANDLE, c.POINTER(ModuleEntry)],
            w.BOOL,
        )
        next_module = bind(
            self.backend.kernel,
