...
WORKERS = {
    "farmer": ("run_discord_notifications.py", "reports/discord-status.json"),
...
def python_paths(python):
    ...
    return {
        str(Path(python).resolve()).casefold(),
        str(base.with_name(Path(python).name).resolve()).casefold(),
    }


def validate_worker(
    worker, report, identity, *, root, owner_pid, python, now, owned=False
):
    if worker not in WORKERS:
        raise ValueError("Unknown notification worker")
    pid = report.get("pid")
    ...
    expected = [
        ...
        str((Path(root) / "scripts" / WORKERS[worker][0]).resolve()),
    ]
    actual = identity.get("argv", [])
    if (
        not owned
        and identity.get("parent_pid") != owner_pid
        or len(actual) != 2
        ...
        or str(Path(identity.get("path", "")).resolve()).casefold()
        not in python_paths(python)
    ):
...
def same_script(left, right, python):
    return (
        ...
        and all(
            str(Path(args[0]).resolve()).casefold() in python_paths(python)
            for args in (left, right)
        )
        and str(Path(left[1]).resolve()).casefold()
        == str(Path(right[1]).resolve()).casefold()
    )
...
def ownership_chain(processes, identity, owner, *, python):
    ...
    for _ in range(3):
        ...
        with processes.pinned(parent, terminate=False) as (wrapper, _stop):
            if str(Path(wrapper["path"]).resolve()).casefold() not in python_paths(
                python
            ) or not same_script(
                wrapper.get("argv", []), identity.get("argv", []), python
            ):
                raise ValueError(
                    "Notification worker has an unrelated process ancestor"
                )
            chain.append(wrapper)
            parent = wrapper.get("parent_pid")
    raise ValueError(
        "Notification worker is not descended from the recorded native app"
    )


def prepare_handover(
    previous_root, *, workers=("farmer",), processes=None, python=None, clock=time.time
):
    ...
    """
    root = Path(previous_root).resolve(strict=True)
    ...
    if not Path(state_path(".runtime/discord.paused")).exists():
        raise ValueError("Notifications must be paused")
    app = read_json(state_path("reports/desktop-farming/app-state.json"))
    now = clock()
    if (
        type(app.get("pid")) is not int
        or not -2 <= now - app.get("updated_at", 0) <= 15
    ):
    ...
    with processes.pinned(app["pid"], terminate=False) as (owner, _stop):
        ...
        if legacy:
            entries_allowed.add(
                str(
                    (Path(legacy) / "scripts/start_desktop_app.py").resolve()
                ).casefold()
            )
        if (
            len(argv) < 2
            or str(Path(argv[1]).resolve()).casefold() not in entries_allowed
        ...
        for worker in workers:
            ...
            if type(report.get("pid")) is not int:
                raise ValueError("Notification worker receipt is missing")
            with processes.pinned(report["pid"], terminate=False) as (identity, _stop):
                expected_argv = [
                    str(python),
                    str(root / "scripts" / WORKERS[worker][0]),
                ]
        ...
        receipt = {
            ...
            "prior_owner": owner,
            "workers": entries,
            "verification": "full_process_identity_and_owned_script",
        }
...
class NativeProcesses:
    ...
    def pinned(self, pid, *, terminate=True):
        ...
        with backend.process_handle(
            pid, 0x1000 | (0x1 | 0x100000 if may_terminate else 0)
        ) as handle:

            def identity():
                path = c.create_unicode_buffer(32768)
                size = w.DWORD(len(path))
                if not backend.image_name(handle, 0, path, c.byref(size)):
                    raise backend.error("Read notifier image")
                times = [w.FILETIME() for _ in range(4)]
                if not backend.times(handle, *[c.byref(value) for value in times]):
                    raise backend.error("Read notifier lifetime")
                return {
                    "pid": pid,
                    "path": path.value,
            ...
            try:
                process = win32com.client.GetObject("winmgmts:").Get(
                    f'Win32_Process.Handle="{int(pid)}"'
                )
                command = str(process.CommandLine or "")
                parent = int(process.ParentProcessId)
            except Exception:
                raise ValueError(
                    "Notification process metadata is unavailable"
                ) from None
            finally:
            ...
            if command:
                count = c.c_int()
                argv = parse(command, c.byref(count))
                if not argv:
                    raise ValueError("Notification worker command line is unavailable")
                try:
                    arguments = [argv[i] for i in range(count.value)]
                finally:
                    free(c.cast(argv, w.HLOCAL))
            wait = bind(
                backend.kernel, "WaitForSingleObject", [w.HANDLE, w.DWORD], w.DWORD
            )
            terminate = bind(
                backend.kernel, "TerminateProcess", [w.HANDLE, w.UINT], w.BOOL
            )

            def stop(expected):
                if not may_terminate:
                    raise ValueError("Read-only process evidence cannot stop a worker")
                if identity() != expected:
                    raise ValueError("Notification worker identity changed before stop")
                if wait(handle, 0) == 0:
                    return
                if not terminate(handle, 0):
                    raise backend.error("Stop owned notification worker")
                if wait(handle, 2000) != 0:
                    raise ValueError("Owned notification worker has not exited")
...
def restart(
    worker, *, processes=None, root=None, owner_pid=None, python=None, clock=time.time
):
    ...
    python = (
        layout.python(windowed=True)
        if layout.immutable or python is None
        else Path(python)
    )
    if not python.is_file():
        raise ValueError("Notification worker Python is unavailable")
    if not Path(state_path(".runtime/discord.paused")).exists():
        raise ValueError("Pause Discord notifications before worker handover")
    processes = processes or NativeProcesses()
    if not _lock.acquire(blocking=False):
        raise ValueError("Notification worker handover already running")
    try:
        report = read_json(state_path(WORKERS[worker][1]))
        pid = report.get("pid")
        if type(pid) is not int or pid <= 0:
            raise ValueError("Notification worker receipt is missing")
        handover = read_json(state_path(HANDOVER))
        saved = handover.get("workers", {}).get(worker)
        with processes.pinned(pid) as (identity, stop):
            if saved:
                if (
                    handover.get("version") != 1
                    or saved.get("phase") != "prepared"
                    or not 0 <= clock() - handover.get("prepared_at", 0) <= 900
                    or any(
                        identity.get(key) != saved.get("identity", {}).get(key)
                        for key in ("pid", "creation_time_100ns", "path", "parent_pid")
                    )
                    or saved.get("identity", {}).get("argv")
                    and identity["argv"] != saved["identity"]["argv"]
                ):
                    raise ValueError(
                        "Prepared notification handover expired, changed or was already consumed"
                    )
                for expected in saved.get("ancestors", []):
                    with processes.pinned(expected["pid"], terminate=False) as (
                        ancestor,
                        _stop,
                    ):
                        if any(
                            ancestor.get(key) != expected.get(key)
                            for key in (
                                "pid",
                                "creation_time_100ns",
                                "path",
                            ...
                            ancestor.get("argv", []), identity.get("argv", []), python
                        ):
                            raise ValueError(
                                "Notification wrapper identity changed before handover"
                            )
                proof = validate_worker(
                    worker,
                    report,
                    identity,
                    root=handover["prior_root"],
                    owner_pid=handover["prior_owner"]["pid"],
                    python=python,
                    now=clock(),
                    owned=True,
                )
            ...
            if saved:
                saved.update(
                    phase="consumed", consumed_by=owner_pid, consumed_at=clock()
                )
        ...
        if saved:
            ...
            write_json(state_path(".runtime/notification-worker-restart.json"), receipt)
        return receipt
    finally:
        _lock.release()
