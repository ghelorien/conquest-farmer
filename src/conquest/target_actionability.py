...
def target_actionability(point, gui_size, client_size, windows=()):
    ...
    reason = None
    panel = None
    if not (0 <= logical[0] < gui[0] and 0 <= logical[1] < gui[1]):
        reason = "outside_gui"
        ...
        for window in windows or ():
            ...
            if name not in BLOCKING_PANELS and not str(name).startswith("Trade##"):
                continue
            x, y, width, height = window.get("geometry", (0, 0, 0, 0))
            if x <= logical[0] < x + width and y <= logical[1] < y + height:
                reason = "panel_occlusion"
                panel = name
                break
    messages = {
        "outside_gui": "Receiver projection is outside the live GUI viewport",
    ...
    return TargetActionability(
        reason is None,
        reason or "actionable",
