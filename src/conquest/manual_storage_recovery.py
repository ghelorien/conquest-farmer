    ...
    if JOURNAL.exists():
        with sqlite3.connect(JOURNAL.resolve().as_uri()+'?mode=ro', uri=True) as db:
            for table in ('delivery_admissions', 'transactions'):
                if db.execute(f'SELECT 1 FROM {table} WHERE created>=? LIMIT 1', (since,)).fetchone():
                    raise ValueError('Delivery admission prevents manual storage recovery')


def _originals(visit, meteor, market, body):
    from conquest.town_trade import stash_candidate
    if (visit.get('town_visit_id') != body['town_visit_id']
            or visit.get('phase') != 'town_work' or visit.get('town_work_completed_at')
            or visit.get('reasons') != ['urgent_banking', 'restock']
            or visit.get('urgent_target') != body['target']
            or not _process_identity(body['target'])
            or not visit.get('required_at', float('inf')) <= visit.get('urgent_banking_tail_completed_at', 0)
                   < meteor.get('started_at', 0)
            or meteor.get('phase') != 'storing_scroll' or meteor.get('origin') != 1011
            ...
            or market.get('phase') != 'active' or market.get('attempts')
        ...
        raise ValueError('Manual storage report does not match the interrupted native trip')
    items = [item for item in meteor.get('after', {}).get('items', []) if stash_candidate(item)]
    if (not items or len({i.get('uid') for i in items}) != len(items)
            or not any(i.get('uid') == meteor.get('scroll_uid') and i.get('type_id') == 720027 for i in items)
...
def report_command(ui, body):
    ...
    if (set(body) != {'action', 'town_visit_id', 'meteor_digest', 'target', 'statement'}
            or body['action'] != 'report-manual-phoenix-storage'
            or not isinstance(body['statement'], str) or not 1 <= len(body['statement'].strip()) <= 1000):
    ...
    with ui.coordinator.lock:
        ...
        if (app.control.snapshot().get('enabled') is not False or ui.coordinator.owner is not None
                or ui.grant is not None or getattr(app, 'thread', None) and app.thread.is_alive()
                or route.get('pid') and process_alive(route['pid']) is not False):
            raise ValueError('Manual storage report requires Farming Off and an exited route')
        info = app.last.get('worker_info_path')
        if not info: raise ValueError('Attached Farmer identity is unavailable')
        health = request(info, 'health')
        life = (health.get('embedded_controls') or {}).get('life') or {}
        visit = TownVisit().state(); meteor = read_json(JOURNAL); market = read_json(MarketVisit().path)
        if (health.get('target') != body['target'] or health.get('profile_id') != visit.get('farmer_profile_id')
                or life.get('map_id') != 1011 or life.get('dead_candidate') is not False
                or life.get('current_hp', 0) <= 0):
            raise ValueError('Manual storage report requires the original living Farmer in Phoenix')
        existing = read_json(REPORT)
        if existing and existing.get('phase') != 'completed':
            if existing.get('operator_report', {}).get('command') == body:
                return {'reported': True, 'phase': existing['phase'], 'report_id': existing['report_id'],
        ...
        testimony = {'source': 'explicit_user_report', 'command': deepcopy(body),
                     'reported_at': time.time(), 'destination_map': 1011,
                     'transaction_receipt': False}
        record = {'phase': 'reported', 'operator_report': testimony,
                  'report_id': evidence_digest(testimony), 'target': deepcopy(body['target']),
                  'town_visit_id': visit['town_visit_id'], 'farmer_profile_id': visit['farmer_profile_id'],
                  'original_visit': visit, 'original_meteor': meteor, 'original_market': market,
                  'expected_items': expected,
                  'unknown_historical_fields': {str(i['uid']): [k for k in OPTIONAL if k not in i] for i in expected}}
        _save(record)
        return {'reported': True, 'phase': 'reported', 'report_id': record['report_id'],
                'expected_items': expected, 'verified_storage': False}
...
def _safe(loop, record):
    ...
    if (testimony.get('source') != 'explicit_user_report'
            or evidence_digest(testimony) != record.get('report_id')
            or record.get('expected_items') != _originals(record['original_visit'], record['original_meteor'],
                                                         record['original_market'], testimony['command'])):
        raise ValueError('Manual storage report binding changed')
    if (row.get('town_visit_id') != record['town_visit_id']
            or row.get('farmer_profile_id') != record['farmer_profile_id']
            or row.get('urgent_target') != record['target']
            or row.get('route_id') != loop.route.id or loop.route.restock_map_id != 1011
            or row.get('reasons') != ['urgent_banking', 'restock']):
        raise ValueError('Manual storage continuation belongs to a different town visit')
...
def _sample(loop, record):
    ...
    if source.get('map_id') != 1011 or tuple(source.get('position', ())) != (227, 246) or not source.get('npc_id'):
    ...
    if _ownership(bag) != _ownership(second_bag) or loop.town('warehouse-locate') != source:
    ...
    if (len(native) != len(basic['items']) or len(full) != len(rich['items'])
            or set(native) != set(full) or basic['bank_silver'] != rich['bank_silver']
    ...
    for expected in record['expected_items']:
        uid = expected['uid']
        if (uid in carried or uid not in native
...
def _proof(loop, record):
    from conquest.restock_town_recovery import _ownership
    first = _sample(loop, record); second = _sample(loop, record)
    if (_ownership(first['bag']) != _ownership(second['bag'])
            or any(first[key] != second[key] for key in ('source', 'warehouse', 'rich_warehouse'))):
        raise ValueError('Manual warehouse ownership is not stable across two observations')
    return {'kind': 'manual_deposit_observed', 'observed_at': time.time(),
            'target': record['target'], 'report_id': record['report_id'],
            'transaction_receipt': False, 'native_cycle_verified': False,
            'first_sample': first, **second}
...
def _settle_old_records(loop, record):
    ...
    original = record['original_meteor']; current = read_json(meteor_banking.JOURNAL)
    ...
    if current.get('phase') == 'operator_overridden':
        ...
        if saved.get('original_evidence_digest') != evidence_digest(original) or saved.get('confirmation_reference') != reference:
            raise ValueError('Meteor manual observation settlement changed')
    else:
        if evidence_digest(current) != evidence_digest(original):
            raise ValueError('Original Meteor trip changed before manual storage settlement')
        operator_override(meteor_banking.JOURNAL, pending_phases=('storing_scroll',),
                          operator_confirmed=True, confirmation_reference=reference,
                          incident_digest=evidence_digest(original), incident='manual-phoenix-storage',
                          fresh_evidence={'operator_report': record['operator_report'],
                                          'manual_storage_observation': record['observation']})
    market_path = MarketVisit().path; market = read_json(market_path)
    if any(market.get(k) != record['original_market'].get(k)
           for k in ('visit_id', 'town_visit_id', 'farmer_profile_id', 'started_at', 'deadline', 'attempts')):
        raise ValueError('Original Market budget changed before manual departure settlement')
    if market.get('phase') == 'active':
        market.update(phase='departed', departed_at=time.time(), arrival_map=1011,
                      departure_source='manual_return_observed', manual_storage_report=record['report_id'],
                      native_return_verified=False)
        meteor_banking._durable_json(market_path, market)
    elif market.get('phase') != 'departed' or market.get('manual_storage_report') != record['report_id']:
...
def resume(loop):
    ...
    record = read_json(REPORT)
    if not record or record.get('phase') == 'completed': return False
    if record.get('phase') not in ('reported', 'observed', 'cash_submitting', 'cash_verified'):
        raise ValueError('Manual storage continuation has an unknown phase')
    loop.stop_farm(); _safe(loop, record)
    ...
    if row.get('town_work_completed_at'):
        if (record['phase'] != 'cash_verified'
                or row.get('manual_storage_recovery', {}).get('report_id') != record['report_id']):
            raise ValueError('Town completion lacks its manual storage boundary')
        record.update(phase='completed', completed_at=row['town_work_completed_at']); _save(record)
        return True
    ...
    if any(stash_candidate(i) for i in bag['items']) or needs_town(supply_counts(bag, loop.route), loop.route):
        raise ValueError('Manual return still requires storage or supplies; no old purchase replay')
    if record['phase'] == 'reported':
        record.update(phase='observed', observation=observation)
        _save(record)
    _settle_old_records(loop, record)
    if record['phase'] == 'cash_verified' and _ownership(bag) != _ownership(record['tail_bag']):
    ...
    if record['phase'] == 'observed':
        ...
        receipt = None
        if amount:
            record.update(phase='cash_submitting', cash_before=money, cash_direction=direction,
                          cash_amount=amount, cash_attempted_at=time.time())
            _save(record)
            _safe(loop, record)
        ...
        record.update(phase='cash_verified', cash_receipt=receipt, cash_verified_at=time.time(), tail_bag=tail_bag)
        _save(record)
    fresh = loop.town('supplies')
    if (any(stash_candidate(i) for i in fresh['items'])
            or needs_town(supply_counts(fresh, loop.route), loop.route)):
    ...
    row = loop.town_visit.state()
    row['manual_storage_recovery'] = {'report_id': record['report_id'],
        'observation': record['observation'], 'native_cycle_verified': False}
    from conquest.meteor_banking import _durable_json
    _durable_json(loop.town_visit.path, row)
    loop.town_visit.complete_town_work('restock')
    record.update(phase='completed', completed_at=time.time()); _save(record)
    loop.record('manual_storage_reconciled', report_id=record['report_id'],
                native_cycle_verified=False, activity='Observed manual Phoenix storage; town cash tail complete')
    return True
