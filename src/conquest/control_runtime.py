...
class ControlRuntime:
    def __init__(
        ...
        *,
        ...
        self.health_profile, self.entity_profile, self.character = (
            health_profile,
            entity_profile,
            character,
        )
    ...
    def _connect(self):
        health = HealthLayout.model_validate(
            yaml.safe_load(Path(self.health_profile).read_text())
        )
        entities = EntityLayout.model_validate(
            yaml.safe_load(Path(self.entity_profile).read_text())
        )
        self.health_reader = MemoryHealthReader(
            HealthWorkerSession(self.worker_info, health.player.expected_sha256),
            health,
            self.character,
        )
        self.entity_reader = MemoryEntityReader(
            WorkerPointerSession(self.worker_info, entities.expected_sha256), entities
        )
    ...
    def _observe(self):
        ...
        return {
            "monsters": [asdict(entity) for entity in entities.monsters],
            "observations_available": True,
            "observation_note": "Live memory candidates",
            "hp_candidate": health.current_hp,
            "max_hp_candidate": health.max_hp,
            "focused": window["foreground"] == window["hwnd"],
            "minimized": window["minimized"],
            "observed_at": time.time(),
            "blockers": [
                "Current HP and monster life state need live validation",
        ...
        try:
            ...
            if self.external_execution:
                with self.control.lock:
                    self.control.resolved_ids = sorted(
                        m["entity_id"]
                        for m in data["monsters"]
                        if m["entity_id"] in intent["target_ids"]
                ...
                return
            if self.recovery:
                recovery = self.recovery.step(
                    data.get("life"), data.get("focused", False)
                )
                if recovery:
                    self.encounter.reset()
                    self.control.publish(
                        intent["revision"], recovery["state"], recovery["note"]
                    )
                    with self.lock:
            ...
            if (
                ...
                and intent["enabled"]
                and self.external_failure[0] == intent["revision"]
            ):
                self.control.publish(
                    intent["revision"],
                    "runner_stopped",
                    "Farm runner stopped: " + self.external_failure[1],
                )
                with self.lock:
                    self.latest = data
                return
            ...
            current = self.control.reconcile(
                focused=data.get("focused", False),
                minimized=data.get("minimized", False),
                observed_ids=ids,
                blockers=blockers,
                observed_types={
                    m["entity_id"]: m.get("type_id") for m in data["monsters"]
                },
                pending_target=self.encounter.target,
            )
            if current["execution_state"] == "ready":
                action = self.encounter.plan(
                    data["monsters"], current["resolved_target_ids"], loot, self.clock()
                )
                if action:
                    ...
                    def execute():
                        try:
                            ...
                            if action.kind == "attack":
                                self.dispatcher(
                                    action.monster, data, current["input_mode"]
                                )
                            else:
                                self.pickup_dispatcher(
                                    action.drop, data, current["input_mode"]
                                )
                            self.encounter.committed(action, loot, issued)
                        except Exception:
                            self.encounter.fault(
                                "Input outcome is uncertain; holding next target"
                            )
                            raise

                    self.control.dispatch(
                        current["revision"],
                        action.monster["entity_id"],
                        execute,
                        type_id=action.monster.get("type_id"),
                    )
                self.control.publish(
                    current["revision"], self.encounter.state, self.encounter.note
                )
            # Keep typed observations private; the UI/HTTP snapshot stays JSON.
            data = {
                key: value for key, value in data.items() if key != "loot_observation"
            }
            ...
            self.control.publish(
                current["revision"], "waiting_for_observation", str(error)
            )
            with self.lock:
                self.latest = {
                    "monsters": [],
    ...
    def start(self):
        self.thread = threading.Thread(
            target=self._run, name="farming-controls", daemon=True
        )
        self.thread.start()
