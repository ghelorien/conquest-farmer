...
class NativeLoopTiming:
    ...
    def stage(self, name):
        ...
        if self.current:
            ...
            if self.moving:
                self.sample("moving_" + previous, now - started)
        ...
        try:
            ...
            if self.enabled and started is not None:
                try:
                    ...
                    if self.moving:
                        self.sample("moving_" + name, elapsed)
                    self.stage(previous)
                except Exception:
                    self.disable()
    ...
    def observe_arrival(self, moving, position):
        ...
        if tuple(position) == tuple(expected) and (
            self.arrival is None or self.arrival[0] != issued
        ):
            now = self.clock()
    ...
    def finish(self):
        ...
        if now - self.started < self.interval:
            return None
        result = {
            "window_seconds": round(now - self.started, 6),
            "timings": self.values,
        }
        self.values = {}
        self.started = now
