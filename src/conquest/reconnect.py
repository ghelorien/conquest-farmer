...
def login_screen(hwnd):
    ...
    return (
        ...
        and win32gui.GetClassName(hwnd) == "ImGuiShell"
...
def load_credentials(path=Path(state_path(".runtime/account.dpapi"))):
    ...
    _, plain = win32crypt.CryptUnprotectData(
        Path(path).read_bytes(), None, None, None, 0
    )
    account = json.loads(plain.decode("utf-8"))
    if set(account) != {"username", "password"} or any(
        not isinstance(v, str) or not 1 <= len(v) <= 256 for v in account.values()
    ):
...
def type_login_field(target, value, *, field_guard=None):
    ...
    for offset in range(0, len(encoded), 2):
        ...
        events.extend(
            Input(type=1, data=InputUnion(ki=KeyboardInput(0, code, flags, 0, 0)))
            for flags in (4, 6)
        )
...
class LoginErrorReader:
    ...
    def read(self):
        ...
        if (
            not all(
                math.isfinite(v)
                for v in (left, end_x, top, height, *window.position, *window.size)
            )
            or left != x + 8
            or end_x != x + window.size[0] - 8
            or height != 18.0
            or not y + 20 <= top < top + height <= y + window.size[1] - 8
            or end_x - left < 32
        ):
            raise ValueError("Login error OK button layout changed")
        if (
            s.read_block(self.root + 0x6E8, 0x22) != raw
...
def login_form_points(session, window):
    ...
    for rva, size, digest in (
        (
            0xE67F0,
            211,
            "2eeddcb3c87af4385b8b65fa1c77f8ebfa9c6da5bd18d6688e110eef6e192006",
        ),
        (
            0xE6B8A,
            36,
            "a414f6338c9dd8a7c33d0f7f64befc0d1484414dbca291b80f6efce3a676c298",
        ),
    ):
...
@coordinated_input
def submit_login(
    target, credential_path=Path(state_path(".runtime/account.dpapi")), *, session=None
):
    from conquest.desktop_runtime import physical_coordinates
    from conquest.foreground import foreground_click
    ...
    if not activate_client(target.hwnd, session.identity):
        raise CaptureUnavailable(
            "Login client did not receive verified focus; no credentials entered"
        )
