...
SETTING_TYPES = {
    ...
    "potion_cooldown": float,
    "jump_scatter": bool,
    "single_isolated_targets": bool,
    "kite_when_surrounded": bool,
    "attack_range_tiles": int,
    "pickup_enabled": bool,
    "recover_after_death": bool,
}
...
def data_root():
    ...
    if not base:
        raise ValueError(
            "LOCALAPPDATA is unavailable; configure a machine-local data directory"
        )
...
def preferences(values):
    ...
    for name, value in values.items():
        ...
        if kind is float:
            ...
            raise ValueError("Invalid " + name)
        if name == "heal_below" and not 0 < value < 1:
            raise ValueError("Healing threshold must be between zero and one")
        if name == "potion_cooldown" and not 0.5 <= value <= 10:
            raise ValueError("Invalid potion cooldown")
        if name == "attack_range_tiles" and not 1 <= value <= 20:
...
class CharacterProfile:
    ...
    def __post_init__(self):
        ...
        if (
            ...
            or not 1 <= len(self.name.strip()) <= 63
            or any(ord(c) < 32 for c in self.name)
        ):
            raise ValueError("Enter the exact in-game character name")
        if not isinstance(self.server, str) or not 1 <= len(self.server) <= 64:
            raise ValueError("Invalid server")
        if self.role not in ROLES:
            raise ValueError("Unknown role")
        if self.account_id:
            identifier(self.account_id)
        if self.character_uid is not None and (
            type(self.character_uid) is not int or self.character_uid <= 0
        ):
            raise ValueError("Invalid character UID")
        if not isinstance(self.label, str) or len(self.label) > 100:
            raise ValueError("Invalid display label")
        if self.template != "automatic":
            identifier(self.template)
        self.overrides = preferences(self.overrides)
        if not isinstance(self.trusted_sources, list):
            raise ValueError("Invalid trusted sources")
        for source in self.trusted_sources:
            if (
                set(source) != {"name", "server", "character_uid"}
                or not isinstance(source["name"], str)
                or not source["name"]
            ):
                raise ValueError(
                    "A trusted source needs an exact name, server and verified UID"
                )
            if (
                not isinstance(source["server"], str)
        ...
        if type(self.local_enabled) is not bool:
            raise ValueError("Invalid local activation setting")
...
class ProfileRegistry:
    ...
    def read(self):
        ...
        if value.get("schema_version") != SCHEMA:
            raise ValueError("Unsupported character registry version")
        for profile in value["profiles"]:
            CharacterProfile(**profile)
        ids = [p["id"] for p in value["profiles"]]
        identities = [
            (p["server"].casefold(), p["name"].casefold()) for p in value["profiles"]
        ]
        if len(ids) != len(set(ids)) or len(identities) != len(set(identities)):
            raise ValueError("Duplicate character identity")
        for key, template in value.get("templates", {}).items():
            identifier(key)
            preferences(template["settings"])
        return value

    @contextmanager
    def edit(self):
        from conquest.managed_security import (
    ...
    def add(self, name, server="America", role="Farmer", **kwargs):
        ...
        with self.edit() as value:
            if any(
                p["server"].casefold() == server.casefold()
    ...
    def update(self, profile_id, changes, *, stopped=None, pending=None):
        ...
        with self.edit() as value:
            index = next(
                (i for i, p in enumerate(value["profiles"]) if p["id"] == profile_id),
                None,
            )
            if index is None:
            ...
            sensitive = {
                name
                for name in ("role", "local_enabled", "trusted_sources")
            ...
            if sensitive:
                ...
                if "role" in sensitive:
                    ...
                    if visitors:
                        raise ValueError(
                            "Revoke the exact allowed manual visitor permissions before changing this role"
                        )
    ...
    def list_visitors(self, profile_id, *, include_revoked=False):
        ...
        with closing(
            ...
            if not db.execute(
                "SELECT 1 FROM sqlite_master WHERE type='table' AND name='visitor_permissions'"
            ).fetchone():
                return []
            ...
            rows = [
                ...
                for row in db.execute(
                    ...
                    "ORDER BY visitor_name,visitor_server,visitor_uid",
                    (profile_id,),
                )
            ]
    ...
    def revoke_visitors(self, profile_id, visitors, *, operator):
        ...
        for visitor in visitors:
            if (
                not isinstance(visitor, dict)
                or set(visitor) != set(required)
                or visitor["target_profile_id"] != profile_id
            ):
                raise ValueError(
                    "Visitor permission must be an exact key for this profile"
                )
        ...
        allowed = {
            tuple(row[name] for name in required)
            for row in self.list_visitors(profile_id)
        }
    ...
    def save_template(self, label, settings):
        if not isinstance(label, str) or not 1 <= len(label) <= 100:
            raise ValueError("Enter a template label")
        key = str(uuid.uuid4())
        with self.edit() as value:
            value["templates"][key] = {
                "label": label,
    ...
    def bind(self, profile_id, name, server, uid):
        with self.edit() as value:
            ...
            if (
                profile["name"] != name
                or profile["server"] != server
                or type(uid) is not int
                or uid <= 0
            ):
                raise ValueError(
                    "Observed character/server does not match this profile"
                )
            if profile["character_uid"] not in (None, uid):
    ...
    def effective(self, profile):
        ...
        if profile.template != "automatic" and profile.template not in templates:
            raise ValueError("Selected template is unavailable")
        base = (
            {}
            if profile.template == "automatic"
            else templates[profile.template]["settings"]
        )
    ...
    def export(self, profile_id):
        ...
        return {
            "schema_version": SCHEMA,
            "kind": "conquest-preferences",
            "role": p.role,
            "label": p.label,
            "settings": self.effective(p),
        }

    def import_preferences(self, payload, name, server="America"):
        if (
            not isinstance(payload, dict)
            or set(payload) != {"schema_version", "kind", "role", "label", "settings"}
            or payload["schema_version"] != SCHEMA
            or payload["kind"] != "conquest-preferences"
        ):
        ...
        return self.add(
            ...
            payload["role"],
            label=payload["label"],
            overrides=preferences(payload["settings"]),
        )
...
class CharacterContext:
    ...
    @property
    def state_dir(self):
        return self.root / "characters" / self.profile.id

    @property
    def credentials(self):
        return self.root / "accounts" / self.profile.account_id / "account.dpapi"

    def verify(self, snapshot):
        if (
            snapshot.get("character") != self.profile.name
            or snapshot.get("server") != self.profile.server
        ):
            raise ValueError(
                "Connected client does not match the selected character/server"
            )
        if (
            self.profile.character_uid is not None
            and snapshot.get("character_uid") != self.profile.character_uid
        ):
