...
JSON_TERMINAL = frozenset((
    'complete', 'completed', 'verified', 'aborted', 'cancelled',
    'cancelled_before_input', 'expired', 'idle', 'skipped', 'withdrawn',
    'no_transfer', 'no_transfer_reconciled', 'partial_aborted_reconciled',
    'operator_overridden',
))
def _read_only(path):
...
def _json_blockers(root, profile_id):
    ...
    for path in sorted(base.glob('*.json')):
        ...
        if active:
            blockers.append({'journal': str(path), 'kind': 'active_route',
                             'phase': phase})
        elif isinstance(phase, str) and phase and phase not in JSON_TERMINAL:
            blockers.append({'journal': str(path), 'kind': 'unfinished_phase',
                             'phase': phase})
    return blockers
            ...
            with closing(_read_only(path)) as db:
                ...
                if 'delivery_admissions' in tables:
                    row = db.execute(
                        ...
                        "AND t.phase IN ('verified','aborted','operator_overridden'))) LIMIT 1").fetchone()
                    if row:
                        blockers.append({'journal': str(path), 'kind': 'delivery_admission',
                                         'id': row[0], 'phase': row[1]})
        ...
        with closing(_read_only(path)) as db:
            ...
            if 'transactions' in tables:
                row = db.execute(
                    ...
                    "('verified','aborted','operator_overridden') LIMIT 1", (profile_id,)).fetchone()
                if row:
                    blockers.append({'journal': str(path), 'kind': 'transaction',
                                     'id': row[0], 'phase': row[1]})
            if 'delivery_admissions' in tables:
            ...
            if 'delivery_reservations' in tables:
                for request_id, encoded in db.execute(
                    ...
                    try:
                        ...
                        phase = None
                    if phase not in ('verified', 'cancelled_before_input',
                                     'no_transfer_reconciled',
                                     'partial_aborted_reconciled', 'operator_overridden'):
                        blockers.append({'journal': str(path), 'kind': 'delivery_reservation',
                                         'id': request_id, 'phase': phase})
                        break
            if 'manual_sessions' in tables:
                row = db.execute(
                    ...
                    "'operator_overridden') LIMIT 1", (profile_id,)).fetchone()
                if row:
            ...
            if 'state' in tables:
                ...
                if row:
                    try:
                        ...
                        scan = {'pending': True}
                    if scan.get('pending'):
                        blockers.append({'journal': str(path), 'kind': 'merchant_scan',
                                         'id': scan.get('request_id'), 'phase': 'pending'})
    except sqlite3.Error:
        blockers.append({'journal': str(path), 'kind': 'unreadable_journal'})
    return blockers
...
class ProfileReadiness:
    ...
    def blockers(self, profile_id):
        return (_json_blockers(self.root, profile_id)
