...
def scan_key_event(scan_code, released=False):
    ...
    return Input(
        type=1,
        data=InputUnion(
            ki=KeyboardInput(0, scan_code, 0x0008 | (2 if released else 0), 0, 0)
        ),
    )
...
def foreground_key(target, vk, expected_size, control=False, require_foreground=False):
    ...
    if before["minimized"] or before["client_size"] != list(expected_size):
        raise CaptureUnavailable(
            "Window state differs from the foreground calibration; no input sent"
        )
    activate = bind(target.backend.user, "SetForegroundWindow", [w.HWND], w.BOOL)
    send = bind(
        target.backend.user, "SendInput", [w.UINT, c.POINTER(Input), c.c_int], w.UINT
    )
    ...
    if any(key_state(code) & 0x8000 for code in (0x10, 0x11, 0x12, vk)):
        raise CaptureUnavailable(
            "A physical modifier or target key is held; no input sent"
        )
    ...
    time.sleep(0.25)
    return {
        "mode": "foreground_key_diagnostic",
        "vk": vk,
        "control": control,
        "scan_code": scan_code,
        "encoding": "scan_code",
...
def require_click_position(snapshot, hwnd, expected_size, expected_point):
    ...
    if any(
        ...
        for actual, expected in zip(snapshot["cursor"], expected_point)
    ):
        raise CaptureUnavailable("Cursor moved away from the target; no button pressed")


@coordinated_input
def foreground_click(
    target,
    x,
    y,
    expected_size,
    ...
    if expected_origin is not None and any(
        abs(a - b) > 1 for a, b in zip((origin.x, origin.y), expected_origin)
    ):
        raise CaptureUnavailable(
            f"Game moved since observation; expected {expected_origin}, actual {[origin.x, origin.y]}; no input sent"
        )
    ...
    require_click_position(
        target.snapshot(), foreground_hwnd, expected_size, (point.x, point.y)
    )

    def require_current_client_point():
        refreshed = w.POINT(x, y)
        if not to_screen(target.hwnd, c.byref(refreshed)) or (
            refreshed.x,
            refreshed.y,
        ) != (point.x, point.y):
    ...
    def control_key(released):
        event = scan_key_event(
            0x1D, released
        )  # Left Control on the inspected Windows client.
        if send(1, c.byref(event), c.sizeof(event)) != 1:
    ...
    try:
        if control:
            ...
            modifier = {
                "control": bool(key_state(0x11) & 0x8000),
            ...
            if not modifier["control"]:
                raise CaptureUnavailable(
                    "Windows did not register Control as held; no jump click sent"
                )
        try:
            require_click_position(
                target.snapshot(), foreground_hwnd, expected_size, (point.x, point.y)
    ...
    *,
    ...
    for point in (source, destination):
        if (
            ...
            or not (
                0 <= point[0] < expected_size[0] and 0 <= point[1] < expected_size[1]
            )
        ):
    ...
    try:
        ...
        for step in range(1, 9):
            if layout_guard:
                layout_guard()
            move(
                round(a.x + (b.x - a.x) * step / 8), round(a.y + (b.y - a.y) * step / 8)
            )
            time.sleep(0.04)
        require_click_position(
        ...
        mouse(0x4)
    return {
        "mode": "foreground_drag_calibration",
        "source": source,
...
@coordinated_input
def foreground_scroll(
    target, point, ticks, expected_size=(1036, 793), *, layout_guard=None
):
    """Bounded wheel input over a memory-qualified shop grid."""
    require_idle()
    if type(ticks) is not int or not 1 <= abs(ticks) <= 3:
        raise ValueError("Invalid wheel step")
    before = target.snapshot()
    hwnd = before.get("root_hwnd", target.hwnd)
    if (
        before["foreground"] != hwnd
        or before["minimized"]
        or before["client_size"] != list(expected_size)
    ):
        raise CaptureUnavailable("Shop scrolling needs focused client; no input sent")
    if not (0 <= point[0] < expected_size[0] and 0 <= point[1] < expected_size[1]):
        raise ValueError("Scroll point outside client")
    user = target.backend.user
    screen = w.POINT(*point)
    to_screen = bind(user, "ClientToScreen", [w.HWND, c.POINTER(w.POINT)], w.BOOL)
    if not to_screen(target.hwnd, c.byref(screen)):
        raise target.backend.error("ClientToScreen")
    metrics = bind(user, "GetSystemMetrics", [c.c_int], c.c_int)
    left, top, width, height = (metrics(i) for i in (76, 77, 78, 79))
    if min(width, height) <= 1:
    ...
    move = Input(
        ...
        data=InputUnion(
            mi=MouseInput(
                round((screen.x - left) * 65535 / (width - 1)),
                round((screen.y - top) * 65535 / (height - 1)),
                0,
                0xC001,
                0,
                0,
    ...
    if send(1, c.byref(move), c.sizeof(move)) != 1:
        raise target.backend.error("SendInput(scroll move)")
    time.sleep(0.03)
    if layout_guard:
        layout_guard()
    ...
    wheel = Input(
        type=0,
        data=InputUnion(mi=MouseInput(0, 0, (ticks * 120) & 0xFFFFFFFF, 0x800, 0, 0)),
    )
    if send(1, c.byref(wheel), c.sizeof(wheel)) != 1:
        raise target.backend.error("SendInput(wheel)")
    time.sleep(0.12)
