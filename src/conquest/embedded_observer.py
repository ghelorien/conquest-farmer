...
class EmbeddedObserver:
    def __init__(
        self, pid, hwnd, health_layout, entity_layout, character, *, context=None
    ):
        ...
        self.read_only_build = (
            health_layout.player.expected_sha256 == CLIENT_SHA256_1078
        )
        ...
        self.session = MemorySession(
            pid, health_layout.player.expected_sha256
        ).__enter__()
        try:
            self.operations = Operations(
                self.session, hwnd, read_only=not self.automation_ready_build
            )
            adapter = SimpleNamespace(
                ...
                request=lambda operation, body=None: self.operations.dispatch(
                    operation, body or {}
                ),
            )
            ...
            self.adapter, self.health_layout, self.character = (
                adapter,
                health_layout,
                character,
            )
        ...
        with self.lock:
            ...
            if self.character_context:
                ...
                if not login_screen(self.operations.target.hwnd):
                    ...
                    context = context_for(
                        self.character_context.profile.id, self.character_context.root
                    )
    ...
    def _observe(self):
        ...
        if self.read_only_build:
            life = self.read_life()
            writable_bridge = bool(
                self.bridge is not None and not self.bridge.read_only
            )
            try:
            ...
            return {
                ...
                "read_only_worker": not writable_bridge,
                "blockers": (
                    []
                    if writable_bridge
                    else [
    ...
    def sample_npcs(self):
        ...
        with self.lock:
            ...
            if time.monotonic() - result.started_at > 0.5:
    ...
    def start_bridge(self, path, snapshot, **callbacks):
        ...
        self.bridge = EmbeddedBridge(
            Operations(self.session, self.operations.target.hwnd, read_only=False),
            self.lock,
            path,
            snapshot,
            lifetime=None,
            on_sample_npcs=self.sample_npcs,
            on_town=self.town_trade,
            **callbacks,
        )
    ...
    def start_read_only_bridge(self, path, snapshot):
        if not self.read_only_build:
            raise ValueError(
                "Read-only bridge is only used for the exact-build observer"
            )
        from conquest.embedded_bridge import EmbeddedBridge

        self.bridge = EmbeddedBridge(
            self.operations, self.lock, path, snapshot, lifetime=None, read_only=True
        )
