...
def patrol_step(supervisor, position, route, index, boundary, *, chase=True):
    ...
    step = supervisor.patrol_step(
        ...
        **({"chase": False} if not chase else {}),
    )
    destination = getattr(supervisor, "patrol_destination", None)
    if destination is not None:
        index = next(
            (i for i, p in enumerate(route) if tuple(p) == tuple(destination)), index
        )
    return step, index
...
class PatrolSearchConfig(BaseModel):
    ...
    def validate_regions(self):
        if self.regions and (
            not 2 <= len(self.regions) <= 8
            or len({r.name for r in self.regions}) != len(self.regions)
        ):
...
class AdaptivePatrol:
    ...
    def expand(self, now):
        if (
            now - self.last_activity < self.policy.idle_seconds
    ...
    def expand_config(self, config, terrain, now, *, regional=False):
        ...
        patrol = (
            config.route
            if config.patrol_search.regions
            else self.patrol_points(terrain) or config.route
        )
            ...
            for radius in range(8):
                ...
                point = next(
                    (
                        p
                        for p in candidates
                        if x0 <= p[0] <= x1 and y0 <= p[1] <= y1 and terrain.walkable(p)
                    ),
                    None,
                )
