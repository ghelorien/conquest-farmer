...
def webhook_url(value):
    ...
    return urllib.parse.urlunsplit((parsed.scheme,parsed.netloc,parsed.path,'wait=true',''))
...
def save_webhook(value):
    ...
    SECRET.write_bytes(win32crypt.CryptProtectData(value.strip().encode(),'Conquest Discord',None,None,None,0))
...
def deliver(url,content,opener=None):
    payload={'content':content[:1900],'username':'Conquest Farmer','allowed_mentions':{'parse':[]}}
    call=urllib.request.Request(url,data=json.dumps(payload).encode(),
        headers={'Content-Type':'application/json','User-Agent':'ConquestFarmer/1.0'},method='POST')
    ...
    try:
        ...
        raise DeliveryError('Discord connection failed; notification queued',30) from None
...
def live_activity(health,app):
    ...
    if not life:
        if app.get('reconnection',{}).get('state')=='reconnect_exhausted':
            return 'STOPPED: automatic reconnect attempts exhausted; login needs attention'
    ...
    if text.startswith(('Attacking','Patrolling','Picking up','Using a healing','Reloading','Heading back','Heading to')):
        return text
    ...
    return 'Hunting Turtledoves' if app.get('selected_route')=='turtledove' else 'Hunting selected monsters'
...
def with_live_status(app):
    ...
    if not isinstance(port,int) or not 1<=port<=65535 or not info.get('token'):
        return app
    call=urllib.request.Request(f'http://127.0.0.1:{port}/health',data=b'{}',
    ...
    try:
        ...
        return {**app,'live_activity':live_activity(health,app),
                'live_farming':confirmed_farming(health), 'live_checked_at':time.time(),
                'live_position':(health.get('embedded_controls',{}).get('life') or {}).get('position'),
                'live_map':(health.get('embedded_controls',{}).get('life') or {}).get('map_id'),
                'live_hp':(health.get('embedded_controls',{}).get('life') or {}).get('current_hp'),
                'live_max_hp':(health.get('embedded_controls',{}).get('life') or {}).get('max_hp')}
...
def confirmed_farming(health):
    ...
    control=data.get('control',{})
    window=health.get('window',{})
    return bool(life and not life.get('dead_candidate') and control.get('enabled')
                and data.get('external_execution') and control.get('execution_state')=='farming'
                and not window.get('minimized') and window.get('root_hwnd')
...
def status_text(app,route,now,alive=process_alive):
    ...
    active=phase in ('starting','hunting','restocking','changing_route','recovering_route','visiting_town','reloading')
    ...
    text=app.get('live_activity') or app.get('current_activity') or app.get('state') or 'Waiting for farmer status'
...
def major_status(app, route, now, alive=process_alive):
    ...
    if lower.startswith(('heading back', 'heading to the hunting', 'travelling to hunting')):
        return 'Returning to the hunting area', 10
    if lower == 'farming is off':
        return 'Farming is off', 10
    if lower.startswith(('attacking', 'patrolling', 'hunting', 'picking up',
                         'using a healing', 'reloading', 'running', 'farming')):
        group = 'Turtledoves' if app.get('selected_route') == 'turtledove' else 'selected monsters'
...
def recent_kills(path,now,seconds=900):
    ...
    try:
        ...
        try:
            rows=db.execute("SELECT payload FROM events WHERE event='kill_verified' AND time>? AND time<=?",
                            (now-seconds,now))
            total=0
            for (payload,) in rows:
                count=json.loads(payload).get('count')
                # run_trial qualifies Scatter counter increments up to 32.
                if type(count) is not int or not 1<=count<=32:return None
                total+=count
            return total
        finally:db.close()
...
class Notifications:
    def __init__(self,state=None):
        ...
        if self.state.get('notification_policy') != 'terminal_v3':
            ...
            for key in ('awaiting_farming_restart','stall_reported','failure_since','failure_alerted'):
    ...
    def supplies_text(supplies):
        return ', '.join(f'{supplies[key]:,} {label}' for key,label in
            (('arrows','arrows'),('potions','potions'),('free_slots','free slots'),('silver','silver'))
    ...
    def updates(self,app,route,events_path,now,alive=process_alive):
        ...
        if (route.get('phase')=='reloading' or app.get('reload_preparing')) and not self.state.get('reload_in_progress'):
        ...
        if path.exists():
            ...
            if 'restock_offset' not in self.state:
                self.state['restock_offset']=size  # Never replay historical town trips.
                if route.get('phase')=='restocking' and not self.state.get('reload_in_progress') and now-route.get('updated_at',0)<=15:
            ...
            with path.open('rb') as source:
                ...
                for _ in range(200):
                    ...
                    if event=='reload_started':
                        ...
                        self.enqueue('CRITICAL: Town and Market warehouses are full. Farming stopped; closing the game client to disconnect. Automatic reconnect is disabled until you resume manually.',now,'terminal_stop')
                        self.state['failure_alerted']=True
                    elif event=='storage_overflow_started':
                        self.enqueue('Town warehouse full — taking overflow valuables to the Market warehouse.',now,'storage_overflow_started')
                    elif event=='storage_overflow_complete':
                        self.enqueue('Overflow valuables stored in Market — returning to farming.',now,'storage_overflow_complete')
                    elif event=='savings_complete':
                        self.enqueue(f"Savings goal complete: {row['silver']:,} silver. Parked safely in Twin City.",
                                     now,'savings_complete')
                        self.state['restock_in_progress']=False
                    elif event=='restock_complete':
                        self.enqueue('Restock complete — returning to hunting. '+
                                     self.supplies_text(row.get('supplies',{})),now,'restock_complete')
                        self.state['restock_in_progress']=False
                    elif (event=='return_required' or row.get('phase')=='restocking') and event not in ('failed','stopped') and not self.state.get('reload_in_progress'):
                        if not self.state.get('restock_in_progress'):
                            reason={'inventory_full':'inventory full','ammo_unavailable':'arrows needed',
                                    'potions_exhausted':'potions needed'}.get(row.get('reason'),'supplies or inventory need attention')
                            self.enqueue('Heading to town to restock — '+reason,now,'restock_started')
                            self.state['restock_in_progress']=True
        if (self.state.get('reload_in_progress') and route.get('phase')=='hunting'
                and not app.get('reload_preparing') and app.get('live_farming')
                and 0<=now-app.get('live_checked_at',0)<=3
                and 0<=now-route.get('updated_at',0)<=15):
            self.enqueue('Farming resumed after app reload preparation',now,'reload_resumed')
            self.state['reload_in_progress']=False
        if (not app and not route) or now<self.state.get('next_update_at',0):
            return
        text=status_text(app,route,now,alive)
        if (route.get('phase')=='hunting' and app.get('live_farming')
                and not app.get('navigation_blocked') and app.get('state') in ('Patrolling','Farming')):
            text='Farming / patrolling'
        details=[text]
        if 0<=now-app.get('updated_at',0)<=15:
            level=(app.get('experience') or {}).get('level')
            if level is not None:details.append(f'level {level}')
        kills=recent_kills(path.parent.parent/'desktop-farming'/'trial.sqlite3',now)
    ...
    def delivered(self,row,message_id=None,now=None):
        if message_id:
            ...
            receipts.append({**row,'message_id':message_id,
    ...
    def observe(self,app,route,now,alive=process_alive):
        ...
        progress={'position':app.get('live_position',app.get('position')),
                  'map':app.get('live_map'), 'kills':app.get('kills') or 0,
                  'pickup':(app.get('last_pickup') or {}).get('timestamp') or 0}
        def advanced(before):
            return bool(before and (
                (progress['position'] is not None and before.get('position') is not None
                 and progress['position']!=before['position'])
                or (progress['map'] is not None and before.get('map') is not None
                    and progress['map']!=before['map'])
                or progress['kills']>before.get('kills',0)
                or progress['pickup']>before.get('pickup',0)))
        blocked=(route.get('phase')=='hunting' and (app.get('navigation_blocked') is True
                 or app.get('live_activity','').lower().startswith('navigation blocked')))
        if blocked:
            stall=self.state.setdefault('navigation_stall',{'since':now,'progress':progress})
        ...
        unresponsive=(route.get('phase') in ('starting','hunting','restocking','changing_route','recovering_route','visiting_town','reloading')
        ...
        if recovered_progress and not terminal and not unresponsive:
            ...
            self.state['queue']=[row for row in self.state['queue'] if row.get('kind')!='terminal_stop']
        if terminal or unresponsive or stalled:
            self.farming_since=None
            self.state['queue']=[row for row in self.state['queue'] if row.get('kind')!='farming_resumed']
            since=self.state.setdefault('failure_since',now)
            if (stalled or now-since>=60) and not self.state.get('failure_alerted') and not self.state.get('stop_delivered'):
                if stalled and not terminal:
                    self.enqueue('Needs attention — navigation has been blocked with no movement, kills or '
                                 'verified pickups for 60 seconds; automatic path recovery has not succeeded.',
                                 now,'terminal_stop')
                    self.state['navigation_alert_progress']=progress
                else:
                    detail=text.split(':',1)[-1].strip() if terminal else 'route controller is unresponsive'
                    self.enqueue('Needs attention — bot remains stopped; no recovery observed for 60 seconds. '
                                 +detail,now,'terminal_stop')
                self.state['failure_alerted']=True
            return
        ...
        active=(route.get('phase') in ('starting','hunting','restocking','changing_route','recovering_route','visiting_town','reloading')
                and 0<=now-route.get('updated_at',0)<=15 and alive(route.get('pid')) is True)
        running=(app.get('live_farming') is True and abs(now-app.get('live_checked_at',0))<=3)
        ...
        if running:
            ...
            self.farming_since=None
        if (self.state.get('stop_delivered') and self.farming_since is not None
            ...
            for _ in range(200):
                ...
                try:
                    ...
                    if row.get('increase',0)>0:
                        prefix={'recovered_inventory':'Recovered missing pickup','inventory_gain':'Acquired'}.get(row.get('source'),'Picked up')
                        suffix=' (verified now; original pickup time unavailable)' if row.get('source')=='recovered_inventory' else ''
                        self.enqueue(f"{prefix}: {item_label(row)} ×{row['increase']}{suffix}",
                                     row.get('timestamp',now),'pickup')
                        self.state['queue'][-1].update(inventory_uid=row.get('inventory_uid'),
...
def run():
    ...
    try:
        while True:
            now=time.time()
            status={'pid':os.getpid(),'updated_at':now,'state':'Watching status changes',
            ...
            try:
                if PAUSED.exists() or not SECRET.exists():
                    ...
                    if notifications.state['queue'] and now>=notifications.state.get('retry_at',0):
                        try:
                            message_id=deliver(load_webhook(),notifications.state['queue'][0]['content'])
                            notifications.delivered(notifications.state['queue'].pop(0),message_id)
                            notifications.state.update(last_sent_at=time.time(),last_message_id=message_id,retry_at=time.time()+2)
                            notifications.state.pop('delivery_error',None)
                        except DeliveryError as error:
                            notifications.state.update(delivery_error=str(error),retry_at=time.time()+error.retry_after)
                        write_json(STATE,notifications.state)
                    status.update(queued=len(notifications.state['queue']),last_sent_at=notifications.state.get('last_sent_at'),
                                  error=notifications.state.get('delivery_error'))
            except Exception:
                status.update(state='Notifier error; retrying',error='Could not read configuration or monitoring files')
            try:
