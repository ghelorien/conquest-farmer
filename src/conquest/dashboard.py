...
def make_dashboard_server(database, port=8765, *, monitor=None, control=None, runtime=None):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def respond(self, content, status_code=200, html=False):
            ...
            self.send_header("Content-Security-Policy", f"default-src 'self'; script-src 'nonce-{token}'; style-src 'unsafe-inline'; frame-ancestors 'none'; base-uri 'none'")
        ...
        def do_GET(self):
            ...
            if path == "/":
                ...
                if runtime is not None:
                    ...
                    control.reconcile(focused=False, minimized=False, observed_ids=[],
                                      blockers=["Live farming controls are not connected to a worker"])
                    data.update({"monsters": [], "observations_available": False,
        ...
        def do_POST(self):
            ...
            if (not self.trusted_host() or (origin is not None and origin != "http://" + self.headers.get("Host", ""))
            ...
            try:
                ...
                self.respond(json.dumps({"error": "Could not save the control settings"}).encode(), 500)
...
def serve_dashboard(database, port=8765, *, monitor=None, control=None, runtime=None):
    with make_dashboard_server(database, port, monitor=monitor, control=control, runtime=runtime) as server:
        if monitor is not None:
            monitor.start()
        if runtime is not None:
            runtime.start()
