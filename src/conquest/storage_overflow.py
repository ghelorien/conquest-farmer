...
def eligible(stored,carried):
    meteors=sum(i['amount'] for i in stored['items']+carried if i['type_id']==1088001)
    return len(stored['items'])>=stored['capacity'] and meteors<10 and bool(carried)


def handle(loop,stored):
    carried=extras(loop)
    if not eligible(stored,carried):return False
    policy=read_json(POLICY)
    if not policy.get('overflow_enabled'):return False
    origin=loop.living()['embedded_controls']['life']['map_id']
    route=policy.get('origins',{}).get(str(origin))
    if not route or not all(route[leg].get('verified') for leg in ('outbound','return')):
        raise ValueError('Warehouse overflow needs a verified Market round trip from this town')
    if pending():raise ValueError('Resume the existing storage overflow trip before starting another')
    ...
    if wallet<reserve:
        bank=loop.town('warehouse-money');amount=reserve-wallet
        if bank['stored_silver']<amount:raise ValueError('Insufficient transport funds for Market overflow')
        transfer(loop,'withdraw',amount)
    state={'phase':'departing','origin':origin,'route':route,'started_at':time.time(),
           'uids':[i['uid'] for i in carried],'receipts':[]}
    write_json(JOURNAL,state)
    loop.record('storage_overflow_started',activity='Town warehouse full; taking extra valuables to Market')
    resume(loop)
    return True
...
def resume(loop):
    ...
    if world==1036:
        ...
        while True:
            ...
            if receipt.get('verified_in_warehouse') is not True:raise ValueError('Market deposit receipt missing; no repeat issued')
            state['receipts'].append(receipt);write_json(JOURNAL,state)
            loop.record('valuable_stored',**receipt,plus=item.get('plus'),activity='Stored overflow valuable in Market warehouse')
        close_warehouse(loop)
        state['phase']='returning';write_json(JOURNAL,state)
