...
class MemoryGroundReader:
    ...
    def for_session(cls, session):
        ...
        return cls(
            MemoryEntityReader.for_session(session), layout=read_build_layout(session)
        )
    ...
    def read(self):
        ...
        holder_vtable = (
            layout.ground_holder_vtable_rva if layout is not None else 0x5CCC08
        )
        actor_vtable = (
            layout.ground_actor_vtable_rva if layout is not None else 0x5CDAF0
        )
        ...
        for at in range(0, len(entries), 16):
            ...
            if (
                address != owner + 0x10
                or sample_fields(s, [(owner, "u64")])[0] != base + holder_vtable
            ):
            ...
            seen_uids.add(uid)
            record_checks.append((address, data, owner))
            # The manager also permits type-zero entries with no item actor.
            if not type_id and not actor and not obj:
                continue
            ...
            if (
                struct.unpack_from("<Q", record)[0] != base + actor_vtable
            ...
            if any(
                fresh[a:b] != record[a:b]
                for a, b in ((0, 8), (0x40, 0x50), (0x54, 0x5A))
            ):
            ...
            result.append(
                GroundItem(
                    ...
                    type_id,
                    (x, y),
                    struct.unpack_from("<Q", record, 0x48)[0],
                    plus,
                )
            )
        if any(
            s.read_block(address, 0x20) != data
            or sample_fields(s, [(owner, "u64")])[0] != base + holder_vtable
            for address, data, owner in record_checks
        ):
            raise ValueError("Ground registry item changed during sampling")
        if (
            sample_fields(s, header_fields) != header
...
def pickup_delta(drop, before, after):
    ...
    return sum(
        max(0, item.amount - previous[uid].amount)
