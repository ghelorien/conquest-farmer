...
def record_route(reader, seconds, *, interval=.5, spacing=3, clock=time.monotonic,
    ...
    if not .1 <= interval <= 5 or not 1 <= spacing <= 10:
    ...
    while clock()-started < seconds:
        ...
        if previous is not None and math.dist(previous,point) > 30:
            ...
            break
        observations.append({"elapsed":round(clock()-started,3),"position":list(point)})
    ...
    return {"schema_version":1,"map_id":map_id,"route":route,"observations":observations,
            "duration":round(clock()-started,3),"reason":reason,"qualified":False,
            "input_sent":False,"closed_loop":False}
...
def run_recording(profile, info, output, character, seconds, spacing=3):
    ...
    result = record_route(lambda:read_position(session,layout,character),seconds,spacing=spacing,
                          stopped=lambda:bool(win32api.GetAsyncKeyState(0x7B)&0x8000))
    result.update(character=character,expected_sha256=layout.expected_sha256,
                  process_identity=session.identity)
    output = Path(output)
    output.parent.mkdir(parents=True,exist_ok=True)
    output.write_text(yaml.safe_dump(result,sort_keys=False),encoding="utf-8")
    return {"output":str(output),"waypoints":len(result["route"]),"reason":result["reason"],
            "input_sent":False,"qualified":False}
