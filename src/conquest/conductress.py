...
def take_saved_trip(loop, destination_map):
    trips = (
        json.loads(TRIPS.read_text(encoding="utf-8"))["trips"] if TRIPS.exists() else []
    )
    matches = [
        t
        for t in trips
        if t["destination_map"] == destination_map and t.get("verified") is True
    ]
    if len(matches) != 1:
        return False
    trip = matches[0]
    ...
    for _ in range(30):
        ...
        if (
            ...
            and 0
            <= time.time() - health["embedded_controls"].get("observed_at", 0)
            <= 1
            and life["map_id"] == trip["arrival_map"]
            and max(
                abs(a - b) for a, b in zip(life["position"], trip["arrival_position"])
            )
            <= 2
        ):
...
def read_conductress(observer):
    ...
    found = (
        MemoryNpcReader(observer.entities, vendors=[TWIN_CONDUCTRESS])
        .read(life.map_id)
        .npcs
    )
    if len(found) != 1:
        raise ValueError("Conductress is not in the current memory scene")
    npc = found[0]
...
def read_dialog_records(session, actor, *, layout=None):
    ...
    if count == 0 and (
        capacity == 0 or (capacity <= 128 and not capacity & (capacity - 1))
    ):
        raise ValueError("NPC dialog is absent")
    if (
        not 1 <= count <= 32
        or not count <= capacity <= 128
    ...
    for index in range(count):
        shared = checked_address(
            struct.unpack_from("<Q", pointers, ((first + index) & (capacity - 1)) * 8)[
                0
            ]
        )
        ...
        content = (
            ...
            if allocated <= 15
                ...
                checked_address(struct.unpack_from("<Q", raw, 0x10)[0]), length
            )
        )[:length]
        records.append(
            {"kind": kind, "option": option, "text": content.decode("utf-8")}
        )
        checks.extend([(shared, reference), (address, raw)])
        if allocated > 15 and length:
            checks.append((struct.unpack_from("<Q", raw, 0x10)[0], content))
    window = MemoryGui(s, layout=layout).read("Dialog")
    dc = s.read_block(window.address + 0xE0, 0x38)
    right, top = struct.unpack_from("<2f", dc, 8)
    left = struct.unpack_from("<f", dc, 16)[0]
    height = struct.unpack_from("<f", dc, 0x34)[0]
    if (
        s.read_block(actor + dialog_offset, 32) != header
        or s.read_block(table, capacity * 8) != pointers
...
def validate_destination(data, destination):
    ...
    if (
        texts
        != ["Where are you heading? I can teleport you for a price of 100 silver."]
        or [r["text"] for r in options]
        != [
            "Phoenix Castle",
            "Desert City",
            "Ape Mountain",
        ...
        or [r["option"] for r in options] != list(range(7))
...
def prepare_destination(loop, destination):
    ...
    while time.monotonic() < deadline:
        ...
        if not scroll_direction(data, destination, data.get("viewport")):
            return
        loop.town(
