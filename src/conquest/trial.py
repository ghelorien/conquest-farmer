...
class TrialConfig(BaseModel):
    ...
    minimum_health: float = Field(default=0.4, gt=0, lt=1)
    interval: float = Field(default=0.3, ge=0.15, le=10)
    target_threshold: float = Field(default=0.94, ge=0.8, le=1)
    attack_progress_timeout: float = Field(default=3, ge=0.8, le=5)
    ...
    heal_below: float = Field(default=0.4, gt=0, lt=1)
    potion_cooldown: float = Field(default=1, ge=0.5, le=10)
...
def ammunition_per_attack(config):
    ...
    return (
        3
        if (
            getattr(config, "jump_scatter", False)
            or getattr(config, "attack_button", None) == "right"
        )
        else 1
    )
...
def ammunition_reload_needed(inventory, config, *, proactive=False):
    ...
    return (
        ammo is None
        or ammo.type_id != config.ammo_type
        or ammo.amount < ammunition_per_attack(config)
    )
...
def supply_stop_reason(inventory, config, now):
    ...
    if (
        ...
        if not (
            getattr(config, "ammo_key", None) is not None
            and any(
                i.type_id == config.ammo_type
                and i.amount >= ammunition_per_attack(config)
...
def choose_target(observed, hp, position, config, age):
    if age > 0.35 or hp < config.minimum_health:
        return None
    ...
    for target in observed:
        ...
        if target.world_position is not None:
            ...
            in_range = (
                max(abs(map_x - position[0]), abs(map_y - position[1]))
                <= config.attack_range_tiles
            )
        if (
            target.name in (config.monster, *getattr(config, "monster_variants", ()))
            and in_range
            and left <= map_x <= right
            and top <= map_y <= bottom
        ):
...
def visible_movement_delta(dx, dy, *, horizontal_limit=280, vertical_limit=110):
    ...
    scale = min(
        1,
        horizontal_limit / max(abs((dx - dy) * 32), 1),
...
def run_trial(
    ...
    *,
    ...
    config = config_override or TrialConfig.model_validate(
    ...
    if config.observation_mode == "memory_only" and supervisor is None:
        raise ValueError(
            "Memory-only farming is not qualified: current HP, monster alive state, "
    ...
    if any(
        not (
            config.boundary[0] <= x <= config.boundary[2]
            and config.boundary[1] <= y <= config.boundary[3]
        )
        for x, y in config.route
    ):
        raise ValueError("Route waypoint is outside the farming boundary")
    if config.approach_route:
        if config.approach_boundary is None or any(
            not (
        ...
        if not (
            config.boundary[0] <= ax <= config.boundary[2]
    ...
    if supervisor is None:
        if config.monster not in ("Pheasant", "Turtledove"):
            raise ValueError(
                "This leveling target requires the memory-only native runner"
            )
        ...
        if config.loot_allowlist:
            loot_template = cv2.imread(
                str(Path(config.inventory_templates) / "stancher-name.png"), 0
            )
    ...
    session = session_override or WorkerPointerSession(
        info_path, layout.expected_sha256
    )
    inventory_layout = InventoryLayout.model_validate(
        yaml.safe_load(Path(config.inventory_profile).read_text())
    )
    inventory_reader = MemoryInventoryReader(session, layout, inventory_layout)
    inventory_panel = (
        InventoryReader(config.inventory_templates)
        if config.healing_enabled and supervisor is None
        else None
    )
    ...
    camera = (camera_factory or DesktopFrames)(
        hwnd, config.client_size, config.capture_output, config.capture_origin
    )
    started, last_action = time.monotonic(), 0
    search = (
        AdaptivePatrol(
            config.boundary,
            config.patrol_search,
            started,
            (supervisor.recovery.terrain.width, supervisor.recovery.terrain.height),
        )
        if supervisor
        else None
    )
    ...
    rotation = (
        RegionRotation(config.patrol_search.regions)
        if supervisor and config.patrol_search.regions
        else None
    )
    if rotation:
    ...
    approaching = bool(config.approach_route) and not (
        supervisor and config.hunting_anchor
    )
    approach_waypoint = 0
    ...
    try:
        while time.monotonic() - started < seconds:
            try:
                ...
                try:
                    if timing:
                        timing.stage("inventory_player_projection")
                ...
                if (
                    supervisor
                    and hasattr(supervisor, "observe_inventory")
                ...
                if (
                    supervisor
                    and not observe_only
                    and hasattr(supervisor, "urgent_banking")
                ...
                if (
                    ...
                    config = config.model_copy(
                        update={"player_anchor": supervisor.player_anchor((x, y))}
                    )
                if timing:
                ...
                if recovery is None and not (l <= x <= r and t <= y <= b):
                    if (
                        supervisor is None
                        or observe_only
                        or config.hunting_anchor is None
                    ):
                ...
                try:
                    frame = (
                        Frame(time.monotonic(), None, camera.geometry())
                        if supervisor
                        else camera.read()
                    )
                    ...
                    continue
                hp = (
                    supervised["health_ratio"]
                    if supervised
                    else health_ratio(frame.image, config.client_size)
                )
                level = fields["level"][0]
                if not 1 <= level <= 140 or (
                ...
                if level != previous_level:
                    ...
                    event(
                        "level_observed" if previous_level is None else "level_changed",
                ...
                def dispatch(
                    ...
                    *,
                    ...
                    if (
                        camera.geometry() != frame.origin
                        or time.monotonic() - frame.timestamp > 0.35
                        or time.monotonic() - inventory.started_at > 1
                        or win32api.GetAsyncKeyState(0x7B) & 0x8000
                    ):
                        if supervisor:
                            raise CaptureUnavailable(
                                "Action expired or foreground changed; reobserving"
                            )
                    ...
                    body = {
                        ...
                        "button": button,
                        "control": control,
                        "expected_size": list(config.client_size),
                        "require_foreground": True,
                        "expected_origin": list(frame.origin),
                    }
                    with (
                        timing.measure(
                            "movement_dispatch"
                            if target is None and drop is None and not ui
                            else "other_dispatch"
                        )
                        if timing
                        else nullcontext()
                        ...
                        return (
                            supervisor.dispatch(
                                lambda: session.request("foreground-click", body),
                                target=target,
                                drop=drop,
                                expected_position=(x, y)
                                if target is None and drop is None
                                else None,
                                retarget=(
                                    lambda fresh: body.update(point=[fresh.x, fresh.y])
                                )
                                if target
                                else None,
                                attack_range=config.attack_range_tiles
                                if button == "right"
                                else config.single_attack_range_tiles,
                            )
                            if supervisor
                            else session.request("foreground-click", body)
                        )

                def dispatch_key(body):
                    return (
                        supervisor.dispatch(
                            lambda: session.request("foreground-key", body)
                        )
                        if supervisor
                        else session.request("foreground-key", body)
                    )
                ...
                if last_kill_counter is None or counter < last_kill_counter:
                    ...
                    if increment > counter_evidence["increment_limit"]:
                        ...
                        check = session.request(
                            "sample",
                            {
                                "fields": [
                                    ...
                                    {
                                        "name": "kill_counter",
                                        "address": hex(addresses["kill_counter"]),
                                        "kind": "u32",
                                    },
                                ]
                            },
                        ...
                        checked = {
                            field["name"]: field["value"] for field in check["fields"]
                        }
                        if checked["name"] != config.character:
                            raise ValueError(
                                "Character identity changed during counter verification"
                            )
                        if checked["kill_counter"] != [counter]:
                            raise CaptureUnavailable(
                                "Kill counter changed during gap verification; reobserving"
                            )
                        event(
                            "kill_counter_gap",
                            previous=last_kill_counter,
                        ...
                        event(
                            ...
                            **counter_evidence,
                            ...
                            kills_per_hour=confirmed_kills
                            * 3600
                ...
                if (
                    ...
                    if escape is not None:
                        ...
                        escape_settle_until = time.monotonic() + 0.55
                        last_action = time.monotonic()
                        event(
                            "ranged_escape",
                            source=[x, y],
                            destination=escape,
                            interrupted_attack=attack_interrupted,
                            **getattr(supervisor, "escape_context", {}),
                        )
                        continue
                ...
                if (
                    supervisor is None
                    and hp <= 0
                    and (recovery is None or recovery.phase == RecoveryPhase.RETURNING)
                ):
                    deaths += 1
                    healing = reloading = picking_up = moving = pending_attack = None
                    ...
                    recovery = DeathRecovery(
                        config.recovery, (x, y), time.monotonic(), config.boundary
                    )
                    if not config.recovery.revive_template:
                        event(
                            "revive_calibration_required",
                            image=str(output / "death.png"),
                        )
                ...
                def recover_step(button=None):
                    ...
                    if recovery.revival_verified and not was_verified:
                        ...
                        event(
                            ...
                            position=[x, y],
                            health_ratio=hp,
                            total=verified_revivals,
                        )
                    if recovery.phase != old_phase:
                        event(
                            "recovery_state",
                            state=str(recovery.phase),
                            reason=recovery.reason,
                        )
                    if action and not observe_only:
                        if action.kind == "return_walk" and (
                            supply_reason or hp < config.heal_below
                        ):
                ...
                if recovery is not None and recovery.phase in (
                    ...
                    if hp <= 0 and config.recovery.revive_template:
                        ...
                        button = revive_button(
                            frame.image, template_image, config.recovery.revive_region
                        )
                    recover_step(button)
                ...
                if (
                    supervisor
                    and hasattr(supervisor, "discard_step")
                    and not observe_only
                    and (
                        getattr(supervisor, "discard_panel_pending", False)
                        or (
                            hp >= max(0.8, config.heal_below)
                            and not defending
                ...
                if supply_reason and not (
                    healing and supply_reason == "potions_exhausted"
                ):
                ...
                if healing:
                    outcome = healing.outcome(
                        inventory, absolute_health, time.monotonic()
                    )
                    if outcome == "waiting":
                        time.sleep(0.05)
                        continue
                ...
                if (
                    ...
                    if (
                        ...
                        if receipt["consumed"]:
                            ...
                            event("healing_unneeded", reason=receipt["reason"])
                        continue
                    if config.potion_key is None:
                ...
                if recovery is not None:
                    ...
                    continue
                if reloading:
                    outcome = reloading.outcome(
                        inventory, config.ammo_type, time.monotonic()
                    )
                    if outcome == "waiting":
                        time.sleep(0.05)
                        continue
                    ...
                    pending_attack = None
                    last_action = 0
                if ammunition_reload_needed(
                    inventory, config, proactive=bool(supervisor)
                ):
                    reserve = tuple(
                        (i.uid, i.amount)
                        for i in inventory.items
                        if i.type_id == config.ammo_type
                        and i.amount >= ammunition_per_attack(config)
                    )
                    ...
                    reloading = ReloadAttempt(
                        inventory.equipped_ammo.uid
                        if inventory.equipped_ammo
                        else None,
                        reserve,
                        issued,
                ...
                if (
                    supervisor
                    and config.attack_button == "right"
                    and not observe_only
                    and hasattr(supervisor, "scatter_selection_step")
                    and supervisor.scatter_selection_step(
                        lambda point: dispatch(point, ui=True)
                    )
                ):
                ...
                if (
                    supervisor
                    and hasattr(supervisor, "xp_step")
                    and not observe_only
                    and time.monotonic() >= escape_settle_until
                    and supervisor.xp_step(lambda point: dispatch(point, ui=True))
                ):
                    pending_attack = moving = None
                    last_action = 0
                    continue
                if search and not approaching and not pending_attack and not defending:
                    expanded_config = search.expand_config(
                        config,
                        supervisor.recovery.terrain,
                        time.monotonic(),
                    ...
                    if expanded_config is not None:
                        ...
                        if not config.patrol_search.regions:
                            waypoint = min(
                                range(len(config.route)),
                                key=lambda i: math.dist((x, y), config.route[i]),
                            )
                            moving = None
                        event(
                            "patrol_expanded",
                            boundary=config.boundary,
                            idle_seconds=config.patrol_search.idle_seconds,
                            expansion=search.expansions,
                            patrol=config.route,
                            retained_regional_patrol=bool(config.patrol_search.regions),
                        )
                if moving:
                    if timing:
                        timing.verifier(moving)
                    before_position, issued, expected_position = moving
                    arrived = supervisor and math.dist((x, y), expected_position) < 0.5
                    jumped = (
                        supervisor
                        and max(
                            abs(a - b)
                            for a, b in zip(before_position, expected_position)
                        )
                        >= 8
                    )
                    settle = (
                        (
                            speed.jump_arrival_seconds
                            if config.jump_scatter and jumped
                            else 0.5
                        )
                        if arrived
                        else 1.5
                    ...
                    if (
                        supervisor
                        and jumped
                        and math.dist((x, y), before_position) < 0.5
                    ):
                        settle = min(settle, 0.8)
                    if approaching and getattr(
                        getattr(supervisor, "runback_watch", None), "urgent", False
                    ):
                        settle = min(settle, 0.5)
                    changed = math.dist((x, y), before_position) > 0.5
                    inflight_cast = (
                        speed.scatter_during_jump
                        ...
                        and not approaching
                        and not defending
                        and speed.jump_attack_guard_seconds
                    ...
                    if not inflight_cast:
                        if time.monotonic() - issued < settle:
                            if timing:
                                timing.stage("movement_settle_wait")
                            time.sleep(0.05)
                            continue
                        changed = math.dist((x, y), before_position) > 0.5
                        if (
                            changed
                            and supervisor
                            and hasattr(supervisor, "movement_succeeded")
                        ):
                            supervisor.movement_succeeded(
                                before_position, (x, y), arrived=bool(arrived)
                            )
                        movement_failures = 0 if changed else movement_failures + 1
                        event(
                            "movement_verified" if changed else "movement_stuck",
                            position=[x, y],
                        ...
                        if not changed and supervisor:
                            if getattr(supervisor, "runback_watch", None):
                                supervisor.runback_watch.recovery()
                            supervisor.movement_failed((x, y), expected_position)
                            event(
                                "movement_recovery",
                                position=[x, y],
                                blocked_landing=list(expected_position),
                        ...
                        if movement_failures >= 3 and not supervisor:
                            ...
                            break
                if timing:
                    timing.stage("post_verification")
                ...
                if pending_attack:
                    ...
                    if pending_attack_button == "right" and (
                        time.monotonic() - issued >= 0.8
                        or (
                            supervisor
                            and config.jump_scatter
                            and scatter_receipt_ready(
                                time.monotonic() - issued,
                                previous_ammo,
                                inventory.equipped_ammo.amount,
                                speed.scatter_receipt_seconds,
                                speed.scatter_receipt_arrows,
                            )
                        )
                    ):
                        pending_attack = None
                ...
                if (
                    ...
                    def loot_dispatch(point, *args, **kwargs):
                        nonlocal frame
                        if speed.coherent_projection and hasattr(
                            supervisor, "player_projection"
                        ):
                            observed_at = time.monotonic()
                            position, anchor = supervisor.player_projection()
                            if tuple(position) != (x, y) or tuple(anchor) != tuple(
                                config.player_anchor
                            ):
                                raise CaptureUnavailable(
                            ...
                            if origin != frame.origin:
                                raise CaptureUnavailable(
                                    "Game origin changed before loot input"
                                )
                ...
                reuse_scene = (
                    supervisor
                    and speed.scene_reuse_seconds > 0
                    and escape_observation is not None
                    and escape_observed_at is not None
                    and getattr(supervisor, "targets_observation_available", False)
                    and 0
                    <= time.monotonic() - escape_observed_at
                    <= speed.scene_reuse_seconds
                )
                if supervisor:
                    origin = camera.geometry()
                    if origin != frame.origin:
                        raise CaptureUnavailable(
                            "Game origin changed before target scan"
                        )
                    frame = Frame(
                        escape_observed_at if reuse_scene else time.monotonic(),
                        None,
                        origin,
                    )
                observed = (
                    escape_observation
                    if reuse_scene
                    else supervisor.memory_targets(config.client_size)
                    if supervisor
                    else targets(
                        frame.image, template, config.target_threshold, config.monster
                    )
                )
                if supervisor and time.monotonic() - frame.timestamp > 0.35:
                    raise CaptureUnavailable(
                        "Target scan expired; reobserving before attack or patrol"
                    )
                if strategy:
                ...
                attack_range = (
                    config.single_attack_range_tiles
                    if strategy and attack_button == "left"
                    else config.attack_range_tiles
                )
                targeting_config = config.model_copy(
                    update={"attack_range_tiles": attack_range}
                )
                ...
                if strategy:
                    ...
                    for candidate in observed:
                        ...
                        valid = choose_target(
                            ...
                            hp,
                            (x, y),
                            config.model_copy(update={"attack_range_tiles": radius}),
                    ...
                    target = min(
                        choices,
                        key=lambda t: (
                            (t.x - config.player_anchor[0]) ** 2
                            + (t.y - config.player_anchor[1]) ** 2
                        ),
                        default=None,
                    )
                    if target:
                        attack_button = mode(target.name)
                        supervisor.scatter_standoff = max(
                            2,
                            (
                                config.single_attack_range_tiles
                                if attack_button == "left"
                                else config.attack_range_tiles
                            )
                            - 2,
                        )
                ...
                if (
                    supervisor
                    and target is None
                    and not getattr(supervisor, "targets_observation_available", True)
                ):
                    # A changing memory scene is not evidence that the group is dead.
                    if rotation:
                        rotation.observe(
                            time.monotonic(), (x, y), False, available=False
                        )
                    time.sleep(0.05)
                    continue
                if rotation and not approaching and not defending and not observe_only:
                    occupied = target is not None or any(
                        m.current_hp
                        and m.current_hp > 0
                        and max(abs(a - b) for a, b in zip(m.position, (x, y)))
                        <= attack_range
                        for m in getattr(supervisor, "chase_monsters", ())
                    )
                    changed = rotation.observe(
                        time.monotonic(),
                        (x, y),
                        occupied,
                        available=getattr(
                    ...
                    if changed:
                        config = config.model_copy(
                            update={"route": rotation.region.patrol}
                        )
                        waypoint = 0
                        moving = None
                        supervisor.patrol_chase = None
                ...
                if (
                    ...
                    if not hold_for_scatter:
                        ...
                        collecting = supervisor.combat_loot_step(
                            inventory, (x, y), dispatch
                        )
                    if collecting:
                        time.sleep(0.05)
                        continue
                ...
                if (
                    ...
                    drops = (
                        nearby_drops(
                            ...
                            (x, y),
                            config.boundary,
                            config.loot_distance_pixels,
                            calibrated_size=config.client_size,
                            player_anchor=config.player_anchor,
                        )
                        if supervisor
                        else nearby_drops(
                            frame.image,
                    ...
                    for drop in drops:
                        ...
                        issued = time.monotonic()
                        dispatch(drop.point)
                        picking_up = PickupAttempt(
                            drop,
                            frozenset(i.uid for i in inventory.items),
                            inventory.count(drop.type_id),
                            issued,
                        )
                    ...
                    if picking_up:
                        continue
                if defending and time.monotonic() - frame.timestamp <= 0.35:
                    close = [
                        t
                        for t in observed
                        if math.dist((t.x, t.y), config.player_anchor) <= 180
                    ]
                    if close:
                        target = min(
                            close,
                ...
                if (
                    supervisor
                    and config.jump_scatter
                    and attack_button == "right"
                    and (scatter_jump_due or target is None)
                    and not approaching
                    and not defending
                    and not observe_only
                    and not moving
                    ...
                    from conquest.scatter_movement import (
                        scatter_landing,
                        wounded_group_in_range,
                    )

                    if not (
                        target
                        and attack_button == "right"
                        and wounded_group_in_range(
                            supervisor, observed, (x, y), config.attack_range_tiles
                        )
                    ):
                        scatter_boundary = (
                            rotation.region.boundary if rotation else (l, t, r, b)
                        )
                        scatter_destination = scatter_landing(
                            supervisor,
                            observed,
                            (x, y),
                            scatter_boundary,
                            config.attack_range_tiles,
                            minimum_count=1
                            if scatter_jump_due or speed.cross_region_scatter
                            else 3,
                            anchor=config.player_anchor,
                            hunting_boundary=(l, t, r, b),
                        )
                ...
                if time.monotonic() - last_action >= config.interval:
                    ...
                    if target and not observe_only:
                        ...
                        if (
                            ...
                            or time.monotonic() - frame.timestamp > 0.35
                            or (
                                supply_stop_reason(inventory, config, time.monotonic())
                                is not None
                                and not (
                                    supervisor
                            ...
                            if supervisor:
                                raise CaptureUnavailable(
                                    "Attack observation expired or foreground changed; reobserving"
                                )
                            raise ValueError("Action expired or foreground changed")
                        # Defense may have selected another group from the same fresh scene.
                        attack_button = scatter_attack_mode(
                            config, speed, strategy, isolated, target.name
                        )
                        if (
                            attack_button == "right"
                            and time.monotonic() - last_scatter_cast
                            < speed.scatter_recast_seconds
                        ):
                            time.sleep(0.03)
                            continue
                        issued = time.monotonic()
                        ...
                        event(
                            ...
                            during_jump=bool(moving),
                            movement_elapsed=time.monotonic() - moving[1]
                            if moving
                            else None,
                            ability="Scatter"
                            if attack_button == "right"
                            ...
                            distance_tiles=(
                                max(
                                    abs(target.world_position[0] - x),
                                    abs(target.world_position[1] - y),
                                )
                        ...
                        if supervisor is None:
                            cv2.imwrite(
                                str(output / f"attempt-{attempts:02d}.png"), frame.image
                            )
                        ...
                        if scatter_destination:
                            ...
                            if supervisor and config.hunting_anchor:
                                ...
                                while approach_waypoint < len(
                                    config.approach_route
                                ) - 1 and math.dist(
                                    (x, y), config.approach_route[approach_waypoint]
                                ) <= (0 if supervisor else 2):
                                    approach_waypoint += 1
                        ...
                        if movement_failures == 1 and not supervisor:
                            dx += 1
                            dy -= 1
                        if movement_failures == 2 and not supervisor:
                            dx -= 1
                            dy += 1
                        if supervisor:
                            ...
                            avoided = {
                                point
                                for (map_id, point), until in getattr(
                                    supervisor, "movement_obstructions", {}
                                ).items()
                            ...
                            if (
                                not clear_scene(point, config.client_size)
                                or not clear_segment(
                                    terrain, source, landing, avoid=avoided
                                )
                                or max(abs(dx), abs(dy)) >= 8
                                and not clear_jump(terrain, source, landing)
                            ):
                        ...
                        dispatch(
                            point, control=long_jump if supervisor else not approaching
                        )
                        ...
                        if supervisor and long_jump:
                            escape_settle_until = max(
                                escape_settle_until,
                                issued
                                + (
                                    speed.jump_attack_guard_seconds
                        ...
                        event(
                            ...
                            scatter_plan=getattr(supervisor, "scatter_plan", None)
                            if scatter_destination
                            else None,
                            region=rotation.region.name if rotation else None,
                            movement="jump"
                            if (long_jump if supervisor else not approaching)
                            else "run",
                        )
                    last_action = time.monotonic()
                time.sleep(0.08)
                ...
                if timing:
                    timing.stage("retry_wait")
                if (
                    supervisor
                    and speed.moving_observation_retry_seconds < 0.1
                    and str(error)
                    in (
                        "Life state changed during observation",
                        "Player moved before projection",
                        "Player moved before final target scan; reobserving",
                        "Player projection changed before loot input; reobserving",
                    )
                ):
