...
class RouteJumpInput:
    ...
    def __call__(self, body):
        ...
        if set(body) != {"source", "destination", "map_id", "expires_at"} | (
            {"portal_id"} if portal else set()
        ):
        ...
        if portal:
            if (
                type(body["portal_id"]) is not int
                or tuple(destination) + (body["portal_id"],) not in self.terrain.portals
                or distance > 3
                or (dx and dy)
                or not self.terrain.walkable(tuple(source))
            ):
                raise ValueError(
                    "Portal entry must approach a known portal from an adjacent safe tile"
                )
...
class BridgeJumpStepper:
    ...
    def step_to(self, destination, *, expected_position=None):
        ...
        previous_hp = life.get("current_hp")
        damage_at = -float("inf")
        previous = None
        consecutive = 0
        while time.monotonic() < deadline:
            ...
            try:
                ...
                result.update(
                    error="Route click intercepted by a shop panel",
                    outcome="panel_intercepted",
                    panel=error.panel,
                    stalled_at=list(last_position),
            ...
            if life["timestamp"] != previous:
                ...
                consecutive = (
                    consecutive + 1
                    if tuple(life["position"]) == tuple(destination)
                    else 0
                )
                settle = 0.4 if result["input"].get("movement") == "jump" else 0.1
                ...
                stall_limit = 0.5 if time.monotonic() - damage_at < 2 else 1
                if (
