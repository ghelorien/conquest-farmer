...
def ownership_rejected(before,after):
    ...
    return any(m not in seen and m.tick>=latest and m.text==OWNERSHIP_MESSAGE for m in after)
...
class SystemMessageReader:
    ...
    def read(self,*,max_age=.5):
        s=self.session;s.assert_identity();started=time.monotonic();checks=[]
        def read(address,size):
            raw=s.read_block(checked_address(address,size),size)
            if len(raw)!=size:raise ValueError('Incomplete system-message memory')
        ...
        for _ in range(64):
            if node==head:break
            if node in visited:raise ValueError('System-channel tree cycle')
            visited.add(node)
            raw=read(node,0x22)
            if raw[0x19]:break
            key=struct.unpack_from('<H',raw,0x20)[0]
            if key==2005:found=node;break
            node=struct.unpack_from('<Q',raw,0 if key>2005 else 0x10)[0]
        else:raise ValueError('System-channel tree exceeds bounds')
        messages=[]
        if found is not None:
            table,capacity,first,count=struct.unpack('<4Q',read(found+0x30,32))
            if not 0<=count<=200 or not max(count,1)<=capacity<=2048 or capacity&(capacity-1):
            ...
            for index in range(max(0,count-16),count):
                block=struct.unpack('<Q',read(table+((first+index)&(capacity-1))*8,8))[0]
                ...
                if channel!=2005 or not 0<length<=2048 or not max(length,15)<=allocated<=4096:
                    raise ValueError('System-message record differs from qualified layout')
                text=(raw[:16] if allocated<=15 else read(struct.unpack_from('<Q',raw)[0],length))[:length].decode('utf-8')
        ...
        s.assert_identity()
        if time.monotonic()-started>max_age:raise ValueError('System-message observation expired')
...
class LootOwnership:
    def __init__(self,session,path=Path(state_path('.runtime/loot-ownership.json'))):
        ...
        try:
            ...
            if data.get('identity')==self.identity:self.denied=data['denied']
        except (OSError,ValueError,KeyError):pass

    @staticmethod
    def key(drop,map_id):
        return json.dumps([map_id,drop.uid,drop.object_address,drop.type_id,list(drop.position),drop.spawn_tick],separators=(',',':'))

    def blocked(self,drop,map_id):return self.key(drop,map_id) in self.denied
    def snapshot(self):return self.reader.read()
    def reject(self,drop,map_id):
        self.denied[self.key(drop,map_id)]=time.time()
        if len(self.denied)>4096:
            self.denied=dict(sorted(self.denied.items(),key=lambda item:item[1])[-4096:])
