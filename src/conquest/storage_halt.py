...
def active():
    return bool(read_json(HALT).get("active"))


def reason():
    return read_json(HALT).get("reason", REASON)
...
def clear_by_user():
    ...
    if state.get("phase") == "market_full":
        ...
        write_json(meteor_banking.JOURNAL, state)
...
def request_stop(loop, stored, pending, *, reason=REASON):
    ...
    write_json(
        HALT,
        {
            ...
            "market_capacity": stored["capacity"],
            "market_count": len(stored["items"]),
            "pending_uids": [i["uid"] for i in pending],
            "disconnected": False,
        },
    )
    loop.phase = "stopped"
    ...
    try:
        ...
        try:
            ...
            if not backend.image_name(handle, 0, path, c.byref(size)):
                raise backend.error("Query game path")
            creation, exit_time, kernel, user = (w.FILETIME() for _ in range(4))
            if not backend.times(
                handle, *[c.byref(v) for v in (creation, exit_time, kernel, user)]
            ):
                raise backend.error("Read game creation time")
            started = (creation.dwHighDateTime << 32) | creation.dwLowDateTime
            if (
                started != identity["creation_time_100ns"]
                or path.value.casefold() != identity["path"].casefold()
            ):
                raise ValueError(
                    "Game process identity changed; no other process closed"
                )
            terminate = bind(
            ...
            if not terminate(handle, 0):
                raise backend.error("Close game for full storage")
            if wait(handle, 3000) != 0:
                raise ValueError("Game disconnection has not completed")
            return True
        finally:
            backend.close_handle(handle)
    except KeyError as error:
        raise ValueError("Incomplete game identity for disconnect") from error
...
def enforce(app, disconnect=disconnect_exact_client):
    ...
    if getattr(app, "reload_preparing", False):
        app.reload_cancel.set()
    if not halt.get("disconnected") and time.time() >= getattr(
        app, "next_storage_disconnect", 0
    ):
        ...
        try:
            ...
            halt["disconnect_error"] = str(error)
        write_json(HALT, halt)
        app.record(storage_halt=halt, state="Storage full — stopped", kills_per_hour=0)
    text = (
        "Storage full — disconnected; clear warehouse space and press Farming On to resume"
        if halt.get("disconnected")
        else "Storage full — farming stopped; disconnect pending"
    )
    if getattr(app, "last", {}).get("storage_halt") != halt:
        app.record(
            storage_halt=halt,
            state="Storage full — stopped",
            current_activity=text,
            kills_per_hour=0,
        )
    app.state_text.set(text)
    app.activity_text.set(text)
    app.memory_text.set(text)
    return True
