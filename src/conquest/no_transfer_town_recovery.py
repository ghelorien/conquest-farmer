...
def proof(
    ...
    *,
    ...
    if active and not allow_terminal_active or route.get("cleanup_pending"):
        raise ValueError(
            "Original delivery must settle before no-transfer town capture"
        )
    origin = {
        k: v
        for k, v in (
    ...
    class ReadOnlyJournal:
        ...
        def db(self):
            with closing(
                sqlite3.connect(peer_path.resolve().as_uri() + "?mode=ro", uri=True)
            ) as db:
    ...
    with closing(
        ...
        transactions = list(
            db.execute(
                "SELECT * FROM transactions WHERE created>=?", (row["required_at"],)
            )
        )
        admissions = list(
            db.execute(
        ...
        if len(transactions) != 1 or len(admissions) != 1:
            raise ValueError(
                "No-transfer recovery requires one exact native delivery admission"
            )
        ...
        observed = (
            (override.get("fresh_evidence") or {}) if operator_warehouse else result
        )
        ...
        if (
            ...
            or tx["phase"]
            != ("operator_overridden" if operator_warehouse else "aborted")
            ...
            or any(
                before.get(k) != v or admitted.get(k) != v for k, v in origin.items()
            )
            ...
            raise ValueError(
                "No-transfer recovery lacks unchanged original native ownership"
            )
        selected = exact_items(before["items"])
        if (
            list(selected) != [meteor["scroll_uid"]]
            ...
            or (
                ...
                else selected != exact_items(result.get("remaining", []))
            )
        ):
            raise ValueError(
                "No-transfer admission is not the exact original MeteorScroll"
            )
        trace = [
            {**dict(step), "payload": json.loads(step["payload"])}
        ...
        closed = [
            s
            for s in trace
            if s["stage"] == "cleanup_trade" and s["status"] == "observed"
        ]
        presses = [s for s in actions if s["stage"] == "cleanup_trade"]
        if (
            any(
                s["stage"]
                not in ("trade_target_mode", "trade_request", "cleanup_trade")
                for s in actions
            )
            or any(
            ...
            or len(closed) != 1
            or any(
                closed[0]["payload"].get(k) is not False
                for k in ("farmer_trade", "merchant_trade")
            )
            or not failure["time"]
            <= presses[0]["timestamp"]
            <= closed[0]["timestamp"]
            ...
            raise ValueError(
                "No-transfer recovery lacks an exact once-only empty cleanup"
            )
        for role in ("farmer", "merchant"):
            if role in closed[0]["payload"]:
                ...
                validate_snapshot(
                    snapshot, before[role]["character"], closed[0]["timestamp"]
                )
                if (
                    snapshot.get("identity") != before[role]["identity"]
                    or snapshot.get("trade") is not None
                    or snapshot.get("request") is not None
                    or exact_items(snapshot["inventory"])
                    != exact_items(before[role]["inventory"])
                ):
                    raise ValueError(
                        "Rich cleanup observation changed exact unoffered ownership"
                    )
        for role in ("farmer", "merchant"):
            if (
                observed[role].get("trade") is not None
                or observed[role].get("request") is not None
            ):
                raise ValueError(
                    "No-transfer participants still have open trade windows"
                )
        if operator_warehouse:
            import hashlib
            ...
            if (
                any(role not in closed[0]["payload"] for role in ("farmer", "merchant"))
                or override.get("operator_confirmed") is not True
                or not override.get("confirmation_reference")
                ...
                or not closed[0]["timestamp"]
                <= observed.get("observed_at", 0)
                <= override.get("confirmed_at", 0)
                or result.get("sale_receipts")
            ):
                raise ValueError(
                    "Warehouse-only recovery lacks an exact explicit unknown-outcome override"
                )
            for role in ("farmer", "merchant"):
                canonical_ownership(observed[role])
                validate_snapshot(
                    observed[role], before[role]["character"], observed["observed_at"]
                )
                if observed[role]["identity"] != before[role][
                    "identity"
                ] or exact_items(observed[role]["inventory"]) != exact_items(
                    before[role]["inventory"]
                ):
                    raise ValueError(
                        "Override participants or untouched inventory changed"
                    )
            proved = {"proof_digest": digest}
            sales = []
        else:
            sales = qualified_delivery_receipts(
                ReadOnlyJournal(), before, result["merchant"]
            )
        ...
        if (
            ...
            != (
                "operator_overridden"
                if operator_warehouse
                else "no_transfer_reconciled"
            )
            or not _same_intent(peer.get("intent") or {}, before)
            or disposition.get("outcome") != outcome
            or disposition.get("cleanup_pending")
            or (
                not operator_warehouse
                and disposition.get("proof_digest") != proved["proof_digest"]
            )
        ):
        ...
        if operator_warehouse:
            ...
            if (
                peer_override.get("operator_confirmed") is not True
                or peer_override.get("confirmation_reference")
                != override["confirmation_reference"]
                or peer_override.get("fresh_evidence") != observed
            ):
                raise ValueError(
                    "Source and receiver operator dispositions are not linked"
                )
        operations = [
            r
        ...
        if active:
            if (
                ...
                raise ValueError(
                    "Active route differs from terminal no-transfer operation"
                )
        ...
        record = operations[0]
        if (
            record.get("request_id") != key
            or record.get("outcome") != outcome
            or record.get("items")
            or record.get("proof_digest") != proved["proof_digest"]
            or exact_items(record.get("remaining", []))
            != ({} if operator_warehouse else selected)
            ...
            raise ValueError(
                "Original route has not consumed its linked no-transfer disposition"
            )
        attempts = market.get("attempts") or []
        if attempts and (
            len(attempts) != 1
            or attempts[0].get("outcome") != outcome
        ...
        return deepcopy(meteor["after"]), [
            {
                ...
                "cleanup_verified_at": closed[0]["timestamp"],
...
def settle_before_capture(
    ...
    if (
        ...
        raise ValueError(
            "Terminal route consumption lacks unchanged stopped Farmer ownership"
        )
    ...
    def read_only(body):
        ...
        if body.get("action") not in ("delivery-status", "delivery-reconcile"):
            raise ValueError(
                "No-transfer route consumption cannot issue gameplay commands"
            )
        return request(body)
...
def warehouse_fallback_only(loop, meteor):
    ...
    if (
        ...
        or any(
            meteor.get(k) != claim["meteor"].get(k)
    ...
    if (
        ...
        or _ownership(expected) != _ownership(claim["bag"])
        or _ownership(loop.town("supplies")) != _ownership(expected)
    ):
...
def operator_warehouse_requested(row):
    ...
    with closing(
        ...
        rows = list(
            db.execute(
                "SELECT phase FROM transactions WHERE created>=?", (row["required_at"],)
            )
        )
