...
def read_monster_health(session, layout, monster):
    ...
    module = next(
        m for m in session.modules if m["name"].casefold() == layout.module.casefold()
    )
    ...
    if (
        struct.unpack_from("<Q", record)[0]
        != module["base"] + layout.monster_vtable_rva
        ...
        or struct.unpack_from("<II", record, layout.position_offset)
    ...
    if (
        session.read_block(table_pointer, count * 4) != table
        ...
        or session.read_block(obj + layout.id_offset, 16)
        != record[layout.id_offset : layout.id_offset + 16]
        ...
        or session.read_block(obj + layout.position_offset, 8)
        != record[layout.position_offset : layout.position_offset + 8]
        or session.read_block(obj + layout.max_hp_offset, 4)
        != record[layout.max_hp_offset : layout.max_hp_offset + 4]
    ):
