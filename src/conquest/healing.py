...
def consume_inventory_potion(trade,uid):
    ...
    matches=[i for i in before.items if i.uid==uid and i.type_id==1000020 and i.amount>0]
    if len(matches)!=1:raise ValueError('Selected healing item is not a carried Painkiller')
    item=matches[0]
    if item.slot is None or not 0<=item.slot<40:raise ValueError('Healing item slot is invalid')
    if life.current_hp>=life.max_hp:return {'consumed':False,'reason':'already_full_health'}
    try:trade.shop.gui.read('Shop')
    except ValueError as error:
        if 'not active' not in str(error) and 'absent' not in str(error):raise
    else:raise ValueError('Close shop before using a healing item')
    ...
    after,healed=trade.verified_read(lambda:(trade.inventory.read(),trade.life(any_map=True)),
        lambda pair:pair[0].count(1000020)==before.count(1000020)-1
            and pair[1].object_address==life.object_address and pair[1].current_hp>fresh.current_hp,
        'Healing consumption unverified; no repeat input issued')
    return {'consumed':True,'uid':uid,'type_id':1000020,'hp_before':fresh.current_hp,
            'hp_after':healed.current_hp,'remaining':after.count(1000020)}
