...
TRACKED_PANELS = frozenset({
    'Inventory', 'Shop', 'Warehouse', 'Booth', 'Dialog', 'Add Item to Booth',
...
class LayoutSnapshot:
    ...
    def structural_key(self):
        ...
        return (self.hwnd, self.root_hwnd, self.client_size, self.client_origin,
                self.dpi, self.monitor, self.minimized, self.desktop,
                self.panels, self.gui_size)
...
def _panel_signature(windows):
    ...
    for window in windows or ():
        ...
        result.append((name,int(window.get('address',0)),tuple(geometry),
...
def native_layout(target):
    ...
    state=target.snapshot();user=target.backend.user
    ...
    try:
        monitor=int(bind(user,'MonitorFromWindow',[w.HWND,w.DWORD],w.HANDLE)(target.hwnd,2) or 0)
    except (AttributeError,TypeError):
...
class SharedLayoutRevision:
    ...
    def __init__(self,target,*,windows=lambda:(),gui_size=lambda:None,
                 manual_active=manual_mouse_active,native_reader=native_layout,
    ...
    def read(self):
        ...
        gui=self.gui_size()
        snapshot=LayoutSnapshot(int(state.get('hwnd',self.target.hwnd)),
            int(state.get('root_hwnd',self.target.hwnd)),tuple(state['client_size']),
            tuple(origin),int(dpi),int(monitor),bool(state['minimized']),
            int(state['foreground']),tuple(desktop),_panel_signature(self.windows()),
            tuple(gui) if gui is not None else None,bool(self.manual_active()))
    ...
    def require_actionable(snapshot):
        ...
        if snapshot.minimized or snapshot.foreground!=snapshot.root_hwnd:
            raise LayoutChanged('Game layout is not foreground/actionable; no input sent')
        left,top,width,height=snapshot.desktop
        x,y=snapshot.client_origin;cw,ch=snapshot.client_size
        if min(width,height,cw,ch)<=1 or x+cw<=left or y+ch<=top or x>=left+width or y>=top+height:
    ...
    def stable(self,minimum=.25,timeout=.75):
        if minimum<0 or timeout<minimum:raise ValueError('Invalid layout stability interval')
        deadline=self.clock()+timeout;current=self.read();since=self.clock()
        while self.clock()-since<minimum:
            if self.clock()>=deadline:raise LayoutChanged('Game layout did not stabilize; no input sent')
            self.sleep(min(.02,max(0,deadline-self.clock())))
            fresh=self.read()
