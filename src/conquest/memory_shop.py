    ...
    def read(self, name):
        ...
        for address in addresses:
            ...
            if key not in self.names:
                self.names[key] = (
                    s.read_block(checked_address(pointer), 96)
                    .split(b"\0")[0]
                    .decode("utf-8")
                )
            if self.names[key] == name or (
                name.endswith(("/", "_")) and self.names[key].startswith(name)
            ):
        ...
        if (
            not all(math.isfinite(v) for v in (x, y, width, height, *scroll))
            or not 0 <= x < viewport[0]
            or not 0 <= y < viewport[1]
            or not 1 <= width <= viewport[0]
            or not 1 <= height <= viewport[1]
        ):
            raise ValueError("GUI window geometry is invalid")
        latest_count, latest_capacity, latest_array = struct.unpack(
            "<IIQ", s.read_block(context + 0x3E58, 16)
        )
        if not 0 < latest_count <= latest_capacity <= 256:
            raise ValueError("GUI window list changed to invalid bounds")
        latest_entries = struct.unpack(
            "<" + "Q" * latest_count,
            s.read_block(
                checked_address(latest_array, latest_count * 8), latest_count * 8
            ),
        )
        fresh = s.read_block(address, 0x80)
        current_name = s.read_block(pointer, 96).split(b"\0")[0].decode("utf-8")
        if (
            address not in latest_entries
            or current_name != title
            or fresh[:8] != record[:8]
            or fresh[0x18:0x28] != record[0x18:0x28]
            or fresh[0x64:0x6C] != record[0x64:0x6C]
            or s.read_block(self.base + self.context_rva, 8)
            != struct.pack("<Q", context)
        ):
            raise ValueError("GUI window changed during observation")
        if size_for(s) != viewport:
            raise ValueError("Client resized during GUI observation")
...
class ShopSnapshot:
    ...
    def point(self, product):
        ...
        if (
            ...
            or self.grid.scroll[0] != 0
            or not self.grid.position[1] + 8
            < y
            < self.grid.position[1] + self.grid.size[1] - 12
        ):
        ...
        if (
            self.window.size[0] != 288.0
            or self.grid.size[0] != 248.0
            or self.window.size[1] - self.grid.size[1] != 68.0
            or self.grid.position[0] - self.window.position[0] != 20.0
            or self.grid.position[1] - self.window.position[1] != 38.0
        ):
    ...
    def read(self, vendor_id):
        ...
        for index in range(count):
            slot = struct.unpack_from(
                "<Q", data, ((offset + index) & (capacity - 1)) * 8
            )[0]
            ...
            price, healing = (
                struct.unpack_from("<I", raw, 0x48)[0],
                struct.unpack_from("<I", raw, 0x5C)[0],
            )
            if (
            ...
            products.append(
                ShopProduct(
                    ...
                    raw[0x38],
                    raw[0x3B],
                    struct.unpack_from("<H", raw, 0x3C)[0],
                    struct.unpack_from("<H", raw, 0x3E)[0],
                    raw[0x39],
                    struct.unpack_from("<H", raw, 0x52)[0],
                    struct.unpack_from("<H", raw, 0x50)[0],
                    struct.unpack_from("<H", raw, 0x54)[0],
                    struct.unpack_from("<H", raw, 0x58)[0],
                )
            )
        if len({p.type_id for p in products}) != len(products):
            raise ValueError("Duplicate shop product types")
        if (
            s.read_block(root, 0x60) != record
            or s.read_block(array, capacity * 8) != data
        ):
