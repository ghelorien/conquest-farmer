...
def _addition(expected, current, forbidden):
    ...
    if len(extra) != 1 or _owned(without) != _owned(expected):
        raise ValueError(
            "Supply observation cannot explain changed or missing original ownership"
        )
    item = extra[0]
    ...
    if (
        item["uid"] in forbidden
        or kind not in NORMAL_ARROWS
        or item.get("plus") != 0
...
def _safe(loop, target):
    ...
    if (
        ...
        or farmer.get("session")
        or farmer.get("input_fenced")
        or manual.get("sessions")
        or manual.get("handoff") is not None
    ):
        raise ValueError(
            "Unexpected supply observation lacks exclusive stopped ownership"
        )
...
def observe_addition(loop, row, claim, meteor, expected):
    ...
    if (
        ...
        or type(submitted) not in (int, float)
        or type(completed) not in (int, float)
        or not claim["captured_at"]
        <= meteor["market_verified_at"]
        <= submitted
        <= completed
    ):
        raise ValueError(
            "Unexpected supply lacks its durable completed return boundary"
        )
    ...
    forbidden.update(
        uid for proof in claim.get("delivery_proofs", []) for uid in proof["uids"]
    )
    ...
    if prior and (
        prior.get("kind") != "unexpected_supply_addition"
    ...
    for _ in range(2):
        ...
        if (
            _safe(loop, claim["target"]) != revision
            or began < completed
            or not began <= rich["timestamp"] <= time.time()
            or not 0 <= time.time() - rich["timestamp"] <= 3
            or rich["identity"] != claim["target"]
            or rich["character"] != farmer_name()
            or rich["map_id"] != loop.route.restock_map_id
            or rich.get("hp", 0) <= 0
            or rich.get("canonical_manual_ownership") is not True
            or rich["booth"]
            or rich["booth_open"]
            or rich["own_booth_uid"]
            or _owned(bag) != _owned(augmented)
            or rich["silver"] != bag["silver"]
            or rich["capacity"] != bag["capacity"]
        ):
            raise ValueError(
                "Unexpected supply ownership is not freshly stable and closed"
            )
        basic = {i["uid"]: i for i in bag["items"]}
        detailed = {i["uid"]: i for i in rich["inventory"]}
        if set(basic) != set(detailed):
            raise ValueError(
                "Independent inventory readers disagree on supply ownership"
            )
        for uid, item in basic.items():
            peer = detailed[uid]
            if (
                any(item[k] != peer[k] for k in ("uid", "type_id", "plus"))
                or peer["quantity"] != item["amount"]
                or any(
                    k in item and item[k] != peer[k] for k in ("gem1", "gem2", "bound")
                )
            ):
    ...
    if (
        ...
        or prior
        and prior.get("canonical") != second["canonical"]
    ):
        raise ValueError("Unexpected supply changed between independent observations")
    if not prior:
        claim["unexpected_supply_addition"] = {
            "kind": "unexpected_supply_addition",
            "origin": "unknown",
            "is_transaction_receipt": False,
            "town_visit_id": row["town_visit_id"],
            "target": claim["target"],
            "return_submitted_at": submitted,
            "return_before": deepcopy(before),
            "item": deepcopy(addition),
            "expected": _owned(augmented),
