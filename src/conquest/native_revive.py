...
def require_semantics(session):
    ...
    if (hashlib.sha256(read(base + RENDER_RVA, RENDER_SIZE)).hexdigest() != RENDER_SHA256
            or read(base + 0x5D6720, 8) != struct.pack('<Q', 0x400)
            or read(base + 0x5F0AC8, 4) != struct.pack('<f', 50.)
...
def point(session, size, *, require_hover=False):
    ...
    center = (popup.position[0] + popup.size[0] / 2,
              popup.position[1] + popup.size[1] / 2)
    if (control.size != (930., 102.) or control.scroll != (0., 0.)
            or popup.size != (56., 56.) or popup.scroll != (0., 0.)
            or not all(math.isfinite(v) for v in center)
            or abs(center[0] - size[0] / 2) > 1
            or abs(control.position[0] + 465. - center[0]) > 1
            or abs(control.position[1] + 102. - size[1]) > 1
            or abs(center[1] - (control.position[1] - 50.)) > 1):
