...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="Classic Conquer feasibility diagnostics"
    )
    commands = parser.add_subparsers(dest="command", required=True)
    health = commands.add_parser(
        "sample-health",
        help="Read candidate current and maximum HP from memory; never authorizes input",
    )
    ...
    entities = commands.add_parser(
        "sample-entities",
        help="Read candidate monster IDs and positions from memory; sends no input",
    )
    ...
    route = commands.add_parser(
        "record-route",
        help="Record map-coordinate travel from memory; sends no input; F12 ends recording",
    )
    ...
    route.add_argument(
        ...
        required=True,
        type=Path,
        help="Fingerprint-specific player memory profile",
    )
    ...
    dashboard = commands.add_parser(
        "dashboard", help="Show a small read-only localhost farming dashboard"
    )
    ...
    dashboard.add_argument(
        "--profile",
        type=Path,
        help="Farming profile for independent live health tracking",
    )
    dashboard.add_argument("--worker-info", type=Path)
    dashboard.add_argument(
        "--health-profile",
        type=Path,
        help="Enable live control observations with this candidate health profile",
    )
    dashboard.add_argument(
        "--entity-profile",
        type=Path,
    ...
    dashboard.add_argument(
        "--control-settings",
        type=Path,
        default=Path(state_path(".runtime/dashboard-controls.json")),
    )
    inventory = commands.add_parser(
        "sample-inventory",
        help="Read candidate inventory and equipped ammo from memory; sends no input",
    )
    ...
    trial = commands.add_parser(
        "farm-trial",
        help="Bounded supervised foreground farming; F11 pauses and F12 stops",
    )
    ...
    diagnostic = commands.add_parser(
        "diagnose",
        help="Inspect process identity and ordinary read-only access; sends no input",
    )
    diagnostic.add_argument(
        "--exe", default="ImConquer.exe", help="Exact executable basename"
    )
    diagnostic.add_argument(
        "--pid", type=process_id, help="Select one matching process"
    )
    diagnostic.add_argument(
        "--expected-sha256",
        type=sha256,
        help="Reject a changed or unsupported executable",
    )
    diagnostic.add_argument(
        "--output", type=Path, help="Also save the JSON report to this file"
    )
    calibration = commands.add_parser(
        "calibrate",
        help="Discover or refine read-only value candidates; sends no input",
    )
    ...
    calibration.add_argument(
        ...
        required=True,
        type=Path,
        help="YAML containing independently observed values and executable SHA-256",
    )
    calibration.add_argument(
        "--previous",
        type=Path,
        help="Refine candidates from this JSON file in the same process session",
    )
    ...
    calibration.add_argument(
        "--near",
        help="Search near this candidate field in --previous, rather than refining that file",
    )
    calibration.add_argument(
        "--radius",
        type=int,
        default=2048,
        help="Bytes around each still-matching --near anchor",
    )
    probe = commands.add_parser(
        "probe-input",
        help="Send one calibrated left click to the unfocused game window; verify its effect separately",
    )
    ...
    probe.add_argument(
        "--expected-size", required=True, nargs=2, type=int, metavar=("WIDTH", "HEIGHT")
    )
    probe.add_argument(
        "--watch",
        required=True,
        type=Path,
        help="Session-pinned candidate report containing hp_u32 and position_u32 for before/after checks",
    )
    probe.add_argument("--output", required=True, type=Path)
    worker = commands.add_parser(
        "worker", help="Run a short-lived localhost calibration worker for one client"
    )
    ...
    worker.add_argument(
        "--read-only",
        action="store_true",
        help="Disable every game input operation during memory calibration",
    )
    observe = commands.add_parser(
        "observe", help="Record candidate values to SQLite without sending game input"
    )
    observe.add_argument("--pid", required=True, type=process_id)
    observe.add_argument(
        "--watch", required=True, type=Path, help="Session-pinned candidate report"
    )
    observe.add_argument(
        "--field",
        action="append",
        dest="fields",
        help="Numeric candidate field to record; repeat as needed",
    )
    ...
    player = commands.add_parser(
        "sample-player",
        help="Resolve candidate player fields from a module pointer through the diagnostic worker",
    )
    ...
    try:
        if args.command == "sample-health":
            import yaml
            from conquest.memory_health import (
                HealthLayout,
                HealthWorkerSession,
                MemoryHealthReader,
            )
            ...
            try:
                ...
                session = HealthWorkerSession(
                    args.worker_info, layout.player.expected_sha256
                )
                ...
                report = {
                    "schema_version": 1,
                    "stage": "memory_health_candidate_failed",
                    "qualified": False,
        ...
        if args.command == "sample-entities":
            ...
            try:
                ...
                reader = MemoryEntityReader(
                    WorkerPointerSession(args.worker_info, layout.expected_sha256),
                    layout,
                )
                ...
                logger.error(
                    "entity_observation_failed",
                    extra={"fields": {"detail": str(error)}},
                )
                return 2
        if args.command == "record-route":
            ...
            try:
                ...
                logger.error(
                    "route_recording_failed", extra={"fields": {"detail": str(error)}}
                )
                return 2
        ...
        if args.command == "sample-inventory":
            ...
            player_layout = PlayerLayout.model_validate(
                yaml.safe_load(args.player_profile.read_text())
            )
            inventory_layout = InventoryLayout.model_validate(
                yaml.safe_load(args.inventory_profile.read_text())
            )
            session = WorkerPointerSession(
                args.worker_info, player_layout.expected_sha256
            )
            report = MemoryInventoryReader(
                session, player_layout, inventory_layout
            ).report()
            encoded = json.dumps(report, indent=2)
            if args.output:
                args.output.parent.mkdir(parents=True, exist_ok=True)
        ...
        logger.info(
            "diagnostic_started", extra={"fields": {"exe": args.exe, "pid": args.pid}}
        )
        try:
            report = diagnose(
                WindowsBackend(), args.exe, args.pid, args.expected_sha256
            )
        ...
        if args.output:
            try:
                ...
                logger.error(
                    "report_write_failed",
                    extra={"fields": {"path": str(args.output), "detail": str(error)}},
                )
                return 1
        logger.info(
            "diagnostic_finished",
            extra={
                "fields": {"gate": report.gate, "autonomous_actions_enabled": False}
            },
        )
...
def player_command(args, logger):
    ...
    try:
        ...
        return 1
    logger.info(
        "player_sample_finished",
        extra={"fields": {"stage": report["stage"], "qualified": False}},
    )
    return status
...
def observe_command(args, logger):
    ...
    logger.info(
        "observation_started",
        extra={"fields": {"pid": args.pid, "input_enabled": False}},
    )
    try:
        ...
        with MemorySession(args.pid, expected) as session:
            reader = CandidateReader(
                session, candidates, args.fields or ("hp_u32", "position_u32")
            )
            key_state = bind(
                session.backend.user, "GetAsyncKeyState", [c.c_int], c.c_short
            )
...
def probe_command(args, logger):
    ...
    logger.info(
        "input_probe_started", extra={"fields": {"pid": args.pid, "hwnd": args.hwnd}}
    )
    try:
        ...
        with MemorySession(args.pid, args.expected_sha256) as session:
            ...
            report = click_probe(
                MessageTarget(args.pid, args.hwnd), *args.point, args.expected_size
            )
            ...
            if report["changed_candidate_fields"]:
                report["outcome"] = (
                    "candidate_game_state_changed_inspect_before_any_further_input"
                )
        ...
        report = {
            "schema_version": 1,
            "stage": "input_probe_failed",
            "qualified": False,
            "error": str(error),
            "note": "If any messages were queued, inspect the game before retrying.",
        }
        status = 2
