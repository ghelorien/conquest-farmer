...
def nearby_drops(
    ...
    max_distance=180,
    *,
    calibrated_size=(1584, 861),
    player_anchor=(792, 432),
):
    width, height = calibrated_size
    if (
        frame.shape != (height, width, 3)
        or template.shape != (13, 63)
    ...
    ys, xs = np.where(scores >= 0.97)
    found = []
    for x, y in zip(xs, ys):
        point = (int(x + 31), int(y + 30))
        dx, dy = point[0] - player_anchor[0], point[1] - player_anchor[1]
        mapped = (
            round(position[0] + (dx / 32 + dy / 16) / 2),
            round(position[1] + (dy / 16 - dx / 32) / 2),
        )
        if (
            math.hypot(dx, dy) > max_distance
            or not (
                100 < point[0] < min(1380, width - 80)
                and 180 < point[1] < min(735, height - 126)
            )
            or (point[0] < 615 and point[1] > 550)
            or not (
                boundary[0] <= mapped[0] <= boundary[2]
                and boundary[1] <= mapped[1] <= boundary[3]
            )
        ):
            continue
...
class PickupAttempt:
    ...
    def outcome(self, inventory, now, timeout=2):
        if inventory.count(self.drop.type_id) > self.count_before and any(
            i.type_id == self.drop.type_id and i.uid not in self.inventory_uids
