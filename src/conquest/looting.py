...
def nearby_drops(frame, template, position, boundary, max_distance=180):
    if frame.shape != (861,1584,3) or template.shape != (13,63):
    ...
    for x,y in zip(xs,ys):
        ...
        dx,dy = point[0]-792,point[1]-432
        mapped = (round(position[0]+(dx/32+dy/16)/2),
                  round(position[1]+(dy/16-dx/32)/2))
        if (math.hypot(dx,dy) > max_distance or not (100<point[0]<1380 and 180<point[1]<735)
    ...
    return sorted(found,key=lambda d: math.dist(d.point,(792,432)))
