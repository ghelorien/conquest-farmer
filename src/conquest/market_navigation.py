...
def recovery_landing(
    terrain, source, goal, anchor, *, failed=(), used=(), viewport=DEFAULT_SIZE
):
    ...
    for distance in (12, 10, 8):
        for dx, dy in (
            ...
            if (
                point in excluded
                or direction(point) in used_directions
                or not clear_segment(terrain, source, point)
            ):
                continue
            ...
            if not clear_scene((x, y), viewport):
                continue
            candidates.append(
                (max(abs(a - b) for a, b in zip(point, goal)), -distance, point)
            )
