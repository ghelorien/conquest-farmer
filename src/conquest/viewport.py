...
def validate_size(size):
    if (len(size)!=2 or any(type(v) is not int for v in size)
            or not 640<=size[0]<=7680 or not 480<=size[1]<=4320):
...
def logical_client_size(hwnd):
    ...
    with logical_coordinates():return validate_size(win32gui.GetClientRect(hwnd)[2:])
...
def clear_scene(point,size=DEFAULT_SIZE):
    x,y=point;left,top,right,bottom=scene_bounds(size)
    ...
    return (left<x<right and top<y<bottom
            ...
            and not over_xp_popup and not over_skill_menu)
...
def revive_point(session,size):
    ...
    if control.size!=(930.,102.) or control.scroll!=(0.,0.):
        raise ValueError('Revive control-bar layout changed')
    point=(round(control.position[0]+control.size[0]/2),round(control.position[1]-51))
