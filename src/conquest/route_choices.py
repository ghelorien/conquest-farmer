...
ARCHIVED_BANDIT_ROUTES = frozenset(
    {
        ...
        "bandit-far-east",
        "bandit-east-ridge",
        "bandit-eastern-corridor",
        "bandit-southern-fields",
        "bandit-northern-fields",
        "bandit-central-eastern-fields",
        "bandit-combined-fields",
        "bandit-wide-circuit",
        "bandit-region-rotation",
    }
)
...
def saved_route_choices(routes):
    return sorted(
        ...
        key=lambda route: (
            *route.recommended_levels,
            route_label(route).casefold(),
            route.id,
        ),
    )
