...
def _snapshot(app, observer, *, clock=time.monotonic):
    ...
    result = {'schema_version': 1, 'available': True, 'character': 'Parasite',
              ...
              'control': {key: control.get(key) for key in ('enabled', 'revision', 'input_mode')},
              'selected_skill': {'available': False,
                  'reason': 'No qualified current selected-skill reader exists; learned skills do not prove selection'}}

    def optional(name, read):
        if clock() >= deadline:
            result[name] = {'available': False, 'reason': 'Observation budget exhausted'}
            return
        try:
            result[name] = {'available': True, 'snapshot': read()}
        except (ValueError, OSError, RuntimeError, AttributeError, struct.error) as error:
            result[name] = {'available': False, 'reason': 'Memory reader unavailable or unstable',
    ...
    def entities():
        ...
        ordered = sorted(scene.monsters, key=lambda monster:
            max(abs(a-b) for a, b in zip(monster.position, life.position)))
        return {'count': len(scene.monsters), 'object_count': scene.object_count,
                'sampled_at': scene.timestamp, 'nearby': [asdict(monster) for monster in ordered[:12]],
    ...
    def ground():
        ...
        ordered = sorted(items, key=lambda item: max(abs(a-b) for a, b in zip(item.position, life.position)))
        return {'count': len(items), 'nearby': [{**asdict(item), 'eligible': wanted_drop(item)}
                for item in ordered[:12]], 'truncated': len(ordered) > 12,
    ...
    if any(latest_geometry.get(key) != geometry.get(key) for key in ('pid', 'hwnd', 'client_size')):
    ...
    result['control_changed_during_read'] = app.control.snapshot().get('revision') != control.get('revision')
