...
class BackgroundSurface:
    ...
    def park(self, hwnd, identity, *, expected_size=None):
        ...
        try:
            ...
            self.size = (r - l, b - t)
            if min(self.size) <= 0 or (
                expected_size is not None and tuple(expected_size) != self.size
            ):
    ...
    def _parked(self):
        ...
        l, t, r, b = gui.GetClientRect(hwnd)
        return (
            gui.GetWindow(hwnd, 4) == 0
            and gui.IsWindowVisible(hwnd)
            and not gui.IsIconic(hwnd)
            and gui.GetWindowPlacement(hwnd)[1] != 3
    ...
    def check(self):
        ...
        return {
            "hwnd": self.saved.hwnd,
            "identity": dict(self.saved.identity),
            "client_size": list(self.size),
            "rect": list(self.rect),
    ...
    def _restore(self, state, gui):
        self._write(
            gui.SetWindowLong, state.hwnd, -16, ctypes.c_int32(state.style).value
        )
        self._write(
            gui.SetWindowLong, state.hwnd, -20, ctypes.c_int32(state.exstyle).value
        )
        ...
        def restored():
            ...
            hwnd = state.hwnd
            return (
                gui.GetWindow(hwnd, 4) == state.owner
