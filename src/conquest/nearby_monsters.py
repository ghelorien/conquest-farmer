...
class NearbyMonsters(ttk.LabelFrame):
    def __init__(self, parent, on_toggle):
        ...
        for column, title, width in [
            ("selected", "✓", 28),
            ("name", "Monster group", 160),
            ("count", "Nearby IDs", 100),
        ]:
            self.tree.heading(column, text=title)
            self.tree.column(
                column, width=width, minwidth=width, stretch=column == "name"
            )
        ...
        note_frame = ttk.Frame(
            self, height=nametofont("TkDefaultFont").metrics("linespace") * 2 + 4
        )
    ...
    def refresh(self, monsters, selected_types, *, available=True):
        ...
        records = sorted(
            groups,
            key=lambda type_id: (self.names.get(type_id, "").casefold(), type_id),
        )
        ...
        for index, type_id in enumerate(records):
            row = str(type_id)
            values = (
                "✓" if type_id in selected else "",
                self.names.get(type_id, f"Monster type {type_id}"),
