...
def city_for(map_id):
    ...
    if len(matches) != 1:
        raise ValueError(f"No saved destination town for map {map_id}")
    city = matches[0]
    terrain = read_terrain(CLIENT_ROOT, map_id)
    x, y = city["town_anchor"]
    left, top, right, bottom = city["town_boundary"]
    if (
        terrain.source_sha256 != city["terrain_sha256"]
        or not terrain.walkable((x, y))
...
def ensure_city_visit(loop, *, new_arrival=False):
    ...
    city = city_for(life["map_id"])
    identity = health["target"]
    actor = life["object_address"]
    previous = json.loads(VISIT.read_text(encoding="utf-8")) if VISIT.exists() else {}
    key = dict(
        identity=identity, map_id=life["map_id"], terrain_sha256=city["terrain_sha256"]
    )
    ...
    if (
        not new_arrival
        and plan.get("route_id") == loop.route.id
        and start.get("identity") == identity
        and start.get("map_id") == life["map_id"]
    ):
        return False
    if (
        not new_arrival
        and previous.get("completed")
        and all(previous.get(k) == v for k, v in key.items())
    ):
        return False
    write_json(VISIT, {**key, "completed": False, "started_at": time.time()})
    loop.stop_farm()
    prior = loop.phase
    loop.phase = "visiting_town"
    ...
    if (
        ...
        or after["object_address"] != actor
        or after["map_id"] != city["map_id"]
        or after["dead_candidate"]
...
def service_role(map_id, point):
    ...
    stops = [
        (3, services.get("pharmacist")),
        (5, services.get("blacksmith")),
        *services.get("equipment", []),
    ]
    return next(
        (role for role, anchor in stops if anchor and tuple(point) == tuple(anchor)),
        None,
    )
