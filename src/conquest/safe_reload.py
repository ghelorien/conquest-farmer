...
def clear_observation(health):
    ...
    life = data.get("life")
    if (
        not data.get("observations_available")
        or not isinstance(data.get("monsters"), list)
        or not life
        or life["dead_candidate"]
        or data.get("manual_mouse")
        or not 0 <= time.time() - data.get("observed_at", 0) <= 1
        or life["current_hp"] < life["max_hp"] * 0.6
    ):
        return False
    x, y = life["position"]
    # Unknown life is treated as a threat, including unselected monster types.
    return all(
        m.get("alive") is False
        or max(abs(m["position"][0] - x), abs(m["position"][1] - y)) >= CLEARANCE
        for m in data.get("monsters", [])
    )


def nearby_escape(
    terrain, position, monsters, *, anchor=None, avoid=(), viewport=(1036, 793)
):
    ...
    def clearance(p):
        return min(
            (max(abs(p[0] - x), abs(p[1] - y)) for x, y in threats), default=CLEARANCE
        )
    ...
    for dx, dy in sorted(offsets):
        ...
        if not (dx or dy) or not terrain.walkable(target):
            continue
        try:
            ...
            continue
        score = min(CLEARANCE, clearance(target)) - (len(path) - 1) * 0.03
        if anchor:
            delta = visible_route_delta(
                (step[0] - position[0], step[1] - position[1]),
                anchor,
                scene_bounds(viewport),
            )
            if delta is None:
                continue
        ...
        if not clear_segment(terrain, position, step, avoid=avoid):
            continue
        choices.append((score, -len(path), target, step))
    if not choices:
        return None
    best = max(choices)
    if clearance(best[2]) <= clearance(position):
        return None
    return best[3]
...
def _city_parking_route(loop):
    ...
    if (
        route is None
        or not hasattr(route, "restock_map_id")
        or not hasattr(route, "restock_anchor")
    ):
    ...
    terrain = read_terrain(
        installation_path(r"C:\Program Files\Classic Conquer 2.0"), city["map_id"]
    )
    if (
        len(anchor) != 2
    ...
    for dx, dy in (
        ...
        if (
            ...
            and terrain.walkable(candidate)
            and service_role(city["map_id"], candidate) is None
        ):
...
def park(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        if cancelled.is_set():
            raise ValueError("Reload canceled by user")
        health = loop.living()
        try:
            loop.care.check(health)
        ...
        valid_monsters = isinstance(monsters, list) and all(
            isinstance(m, dict)
            and isinstance(m.get("position"), (list, tuple))
            and len(m["position"]) == 2
            and all(type(v) is int for v in m["position"])
            for m in monsters
        )
        nearby = (
            sum(
                m.get("alive") is not False
                and max(abs(a - b) for a, b in zip(m["position"], life["position"]))
                < CLEARANCE
                for m in monsters
            )
            if valid_monsters
        ...
        diagnostic.update(
            ...
            map_id=life["map_id"],
            hp=life["current_hp"],
            last_observation_quiet=quiet,
            nearby_threats=nearby,
            observations_available=bool(data.get("observations_available")),
            observation_age_seconds=(
                round(time.time() - observed_at, 3)
                if type(observed_at) in (int, float)
                else None
            ),
        )
        unchanged = (
            previous and previous[0] == key and life["current_hp"] >= previous[1]
        )
        if quiet:
            if stable_since is None or not unchanged:
            ...
            try:
                anchor = memory_player_anchor(
                    SimpleNamespace(adapter=loop.care.session), SimpleNamespace(**life)
                )
                viewport = tuple(
                    health.get("window", {}).get("client_size", (1036, 793))
                )
                if (
                    allow_town_retreat
                    and retreat is None
                    ...
                    if route:
                        candidate = (
                            route.restock_anchor
                            if route.restock_map_id == life["map_id"]
                            else route.town_anchor
                            if route.map_id == life["map_id"]
                            else None
                        )
                        if candidate and loop.terrain.walkable(candidate):
                            retreat = tuple(candidate)
                            retreat_map = life["map_id"]
                            notify(
                                "Local area crowded; heading toward town for a safe app reload"
                            )
                if retreat is not None and tuple(life["position"]) != retreat:
                    ...
                    target = travel_waypoint(
                        loop.terrain, path, avoid=avoided, viewport=viewport
                    )
                    delta = visible_route_delta(
                    ...
                    target = (
                        (source[0] + delta[0], source[1] + delta[1]) if delta else None
                    )
                    if target and not clear_segment(
                        loop.terrain, source, target, avoid=avoided
                    ):
                        target = None
                else:
                    target = nearby_escape(
                        loop.terrain,
                        life["position"],
                        data.get("monsters", []),
                        anchor=anchor,
                ...
                if target:
                    ...
                    result = loop.stepper.step_to(
                        target, expected_position=tuple(life["position"])
                    )
                    diagnostic[
                        "reached_steps" if result["reached"] else "unreached_steps"
                    ] += 1
                    if result.get("error"):
                        diagnostic["last_step_error"] = str(result["error"])[:180]
...
def prepare(info, route_id, cancelled, notify):
    ...
    while time.monotonic() < deadline:
        ...
        released = (
            process_alive(status.get("pid")) is False
            or not status.get("pid")
            or status.get("phase") == "stopped"
        )
        if not health["embedded_controls"].get("external_execution") and released:
            break
        time.sleep(0.1)
    else:
        raise ValueError("Route controller did not release input; reload deferred")
    stop.unlink(missing_ok=True)
    with controller_guard() as acquired:
        if not acquired:
            raise ValueError("Another route owns input; reload deferred")
        loop = OvernightLoop(route_id)
        loop.refresh()
        loop.phase = "reloading"
        ...
        def check_stop():
            ...
            if time.monotonic() >= preparation_deadline:
                raise ValueError(
                    "Safe reload could not verify living focused control; reload deferred"
                )
            original()

        loop.check_stop = check_stop
        try:
            return park(loop, cancelled, notify, require_city=True)
        finally:
            loop.phase = "stopped"
...
def validate_handoff(info, proof):
    ...
    life = data.get("life")
    if (
        not clear_observation(health)
        or data.get("external_execution")
        or data["control"]["enabled"]
        or health["target"] != proof["target"]
        or life["map_id"] != proof["map_id"]
        or life["position"] != proof["position"]
        or life["current_hp"] < proof["hp"]
    ...
    if proof.get("city_required"):
        ...
        try:
            ...
            left, top, right, bottom = city["town_boundary"]
            if (
                city["terrain_sha256"] != proof["city_terrain_sha256"]
                or len(anchor) != 2
                or any(type(v) is not int for v in anchor)
                or not (left <= anchor[0] <= right and top <= anchor[1] <= bottom)
            ...
            raise ValueError(
                "Safe city parking proof changed; reload deferred"
            ) from error
...
def resume_after_embed(app):
    ...
    if not pending or pending.get("source_pid") == os.getpid():
        return
    if pending.get("expires_at", 0) < time.time():
        RESUME.unlink(missing_ok=True)
        return
    info = getattr(app, "last", {}).get("worker_info_path")
    if not info or not app.runtime:
        return
    from conquest.worker import request

    health = request(info, "health")
    data = health["embedded_controls"]
    if health["target"] != pending["target"]:
        RESUME.unlink(missing_ok=True)
        return
    if (
        not data.get("observations_available")
        or not data.get("life")
        or not 0 <= time.time() - data.get("observed_at", 0) <= 1
    ):
        return
    RESUME.unlink(missing_ok=True)
