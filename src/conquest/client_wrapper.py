...
def pinned_client(catalog, key):
    ...
    if len(matches)!=1:
        raise ValueError('The selected client closed or changed while the wrapper restarted')
    return matches[0]
...
class ClientCatalog:
    def __init__(self, backend, executable='ImConquer.exe', *, image_path=None, title_prefix=None):
        self.backend, self.executable = backend, executable
        self.image_path = str(Path(image_path).resolve()).casefold() if image_path else None
    ...
    def identities(self):
        ...
        for process in self.backend.processes(self.executable):
            ...
            if self.image_path and str(Path(identity['path']).resolve()).casefold()!=self.image_path:
                continue
...
class LaunchWatch:
    def __init__(self, catalog, command, *, cwd, spawn=subprocess.Popen, clock=time.monotonic):
    ...
    def poll(self):
        if not self.pending:
            return None
        candidates = [w for w in self.catalog.windows() if process_key(w.identity) not in self.before]
        if len(candidates)==1:
            self.state, self.note = 'ready','Client window found'
            return candidates[0]
        if len(candidates)>1:
            self.state, self.note = 'choose','Several new windows opened; select a client to embed'
        elif self.process.poll() not in (None,0):
            self.state, self.note = 'failed',f'Launcher exited with code {self.process.returncode}'
        elif self.clock()>=self.deadline:
            self.state, self.note = 'timed_out','No client window appeared; select one when it is ready'
        return None
