...
class CapabilityView:
    def __init__(self):
        self.value={};self.key=None;self.thread=None;self.next_read=0

    def refresh(self,observer,context):
        key=(context.profile.id,repr(observer.adapter.identity)) if observer and context else None
        if self.key!=key:self.key=key;self.value={};self.next_read=0
        if not key or (self.thread and self.thread.is_alive()) or time.monotonic()<self.next_read:return
        self.next_read=time.monotonic()+5
        def read():
            result={'profile_id':context.profile.id,'observed_at':time.monotonic()}
            try:
                if not observer.lock.acquire(timeout=.1):return
                try:
            ...
            if self.key==key:self.value=result
        self.thread=threading.Thread(target=read,daemon=True,name='profile-capabilities');self.thread.start()
    ...
    def text(self):
        ...
        equipment=value.get('equipment',{});combat=value.get('combat',{})
        return (f"Observed level: {equipment.get('level','unavailable')} · Class ID: {equipment.get('profession','unavailable')}\n"
