...
def configure(argv, *, environ=None):
    ...
    for argument in args:
        if argument == "--legacy-data-root":
            ...
            if not value or value.startswith("--"):
                raise ValueError(
                    "--legacy-data-root requires an existing absolute local directory"
                )
    ...
    if any(environ.get(name) for name in MANAGED_ENVIRONMENT):
        raise ValueError(
            "Legacy data mode conflicts with the managed profile environment"
        )
    if any(argument.split("=", 1)[0] in MANAGED_OPTIONS for argument in remaining):
        raise ValueError(
            "Legacy data mode conflicts with managed profile startup options"
        )
    ...
    def resolve(value):
        ...
        if not all(
            (resolved / name).is_dir() for name in ("profiles", "reports", ".runtime")
        ):
            raise ValueError(
                "Legacy data root must contain the existing profiles, reports and .runtime directories"
            )
        return resolved
    ...
    if inherited and resolve(inherited) != destination:
        raise ValueError(
            "Explicit legacy data root differs from the inherited legacy data root"
        )
