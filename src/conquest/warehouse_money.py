...
def money_points(reader, snapshot):
    ...
    if (
        not 128 <= count <= capacity <= 0x110000
        or not 8 <= size <= 24
        or not 8 <= native <= 24
        or padding != (4.0, 3.0)
        or spacing != (8.0, 4.0)
        or snapshot.window.size != (312.0, 460.0)
        or snapshot.grid.size != (272.0, 326.0)
        or snapshot.grid.scroll != (0.0, 0.0)
        or snapshot.grid.position
        != (snapshot.window.position[0] + 20, snapshot.window.position[1] + 96)
    ):
        raise ValueError(
    ...
    def width(label):
        ...
        if not all(math.isfinite(v) and 1 <= v <= 24 for v in values):
            raise ValueError("Warehouse font metrics changed")
        return sum(values) + 2 * padding[0]

    # Renderer 116c67: InputText width = FontSize * 8. Buttons use SameLine.
    x = snapshot.grid.position[0]
    height = size + 2 * padding[1]
    y = snapshot.grid.position[1] - spacing[1] - height / 2
    field_width = size * 8
    deposit_width = width("Deposit")
    withdraw_width = width("Withdraw")
    result = {
        "amount": (round(x + field_width / 2), round(y)),
        "deposit": (round(x + field_width + spacing[0] + deposit_width / 2), round(y)),
        "withdraw": (
            round(
                x + field_width + 2 * spacing[0] + deposit_width + withdraw_width / 2
            ),
            round(y),
        ),
    }
    if reader.gui.read("Warehouse") != snapshot.window:
        raise ValueError("Warehouse moved before money input")
    return result
...
def type_amount(target, amount, *, expected_size=None, maximum=999999999):
    ...
    expected_size = validate_size(
        target.snapshot()["client_size"] if expected_size is None else expected_size
    )
    ...
    def focus():
        ...
        if (
            view["minimized"]
            or tuple(view["client_size"]) != expected_size
    ...
    if any(keys(vk) & 0x8000 for vk in (0x10, 0x11, 0x12)):
        raise CaptureUnavailable("Physical modifiers held; no amount entered")
    send = guarded_send(
        bind(
            target.backend.user,
            "SendInput",
    ...
    def key(event):
        if not event.data.ki.dwFlags & 2:
            focus()
        if send(1, c.byref(event), c.sizeof(Input)) != 1:
            raise OSError("Warehouse amount input incomplete")
    ...
    for ch in str(amount):
        for flags in (4, 6):
            key(
                Input(
                    type=1, data=InputUnion(ki=KeyboardInput(0, ord(ch), flags, 0, 0))
                )
            )
...
def transfer(trade, direction, amount):
    if (
        direction not in ("deposit", "withdraw")
        or type(amount) is not int
        or not 1 <= amount <= 999999999
    ):
    ...
    try:
        ...
        raise CaptureUnavailable(
            f"{error}; warehouse money input interrupted; no repeat input issued"
        ) from error


def _transfer_owned(trade, direction, amount):
    npc = trade.vendor(0)
    reader = WarehouseMoneyReader.for_session(trade.observer.adapter)
    before = trade.inventory.read()
    bank = reader.read()
    points = money_points(reader, bank)
    available = before.silver if direction == "deposit" else bank.silver
    if amount > available:
        raise ValueError("Insufficient money for warehouse transfer")
    if trade.vendor(0) != npc:
        raise ValueError("Warehouseman changed before transfer")
    trade.input_attempted = True
    trade.click(points["amount"])
    type_amount(trade.observer.operations.target, amount)
    typed = trade.verified_read(
        reader.read,
        lambda b: b.amount == str(amount),
    ...
    if (
        ...
        or typed.silver != bank.silver
        or money_points(reader, typed) != points
        or trade.vendor(0) != npc
    ):
        raise ValueError("Warehouse state changed before money submission")
    trade.click(points[direction])
    after, stored = trade.verified_read(
        lambda: (trade.inventory.read(), reader.read()),
