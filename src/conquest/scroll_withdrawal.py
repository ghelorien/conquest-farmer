...
from conquest.protected_withdrawal import (
    ProtectedWithdrawal, _core_map, _digest, _identifier, _plain_observation,
    _source,
)
...
class ScrollWithdrawal(ProtectedWithdrawal):
    ...
    def _profile():
        ...
        if context is None or context.profile.role != 'Farmer':
            raise ValueError('Scroll withdrawal requires the authenticated farmer profile')
        return context

    def _plan(self, plan_id, operation_id, uid):
        if type(uid) is not int or uid <= 0 or plan_id != self.plan_id(operation_id):
            raise ValueError('Scroll withdrawal requires an exact positive UID and operation ID')
        ...
        if old:
            ...
            if (anchor.get('kind') != 'fresh_stored_meteor_scroll'
            ...
            anchor = {'kind': 'fresh_stored_meteor_scroll',
        ...
        if (item is None or item['type_id'] != SCROLL or item['quantity'] != 1
                or item['bound'] is not False or item['plus'] != 0
                or item['gem1'] != 0 or item['gem2'] != 0
                or uid in source['inventory'] or uid in source['booth']):
            raise ValueError('One exact unbound stored MeteorScroll is required; loose Meteors are forbidden')
        ...
        if (not isinstance(identity, dict)
                ...
                or type(source['character_uid']) is not int or source['character_uid'] <= 0
                or context.profile.character_uid != source['character_uid']):
            raise ValueError('Scroll withdrawal requires the exact bound character and process creation identity')
        selected = {'item': item, 'withdrawal_operation_id': operation_id,
        ...
        plan = {'plan_id': plan_id,
                'farmer': {'character': source['character'],
                           'character_uid': source['character_uid'],
                           'server': source['server'], 'process_identity': identity,
                           'client_sha256': anchor['client_sha256']},
                'market_reentry': {'snapshot': observation['source'],
        ...
        plan['plan_sha256'] = _digest({'anchor': anchor, 'item': item, 'farmer': plan['farmer']})
    ...
    def _expected_preflight(self, plan, selected, current):
        ...
        if (type(stamp) not in (int, float) or not 0 <= self.clock() - stamp <= 5
                or type(source.get('capacity')) is not int
                or not 0 < source['capacity'] <= 40
                or len(source['inventory']) >= source['capacity']):
            raise ValueError('Scroll withdrawal needs fresh inventory evidence and a free slot')
        if any(row['type_id'] == 1088001 for row in source['inventory']):
            raise ValueError('Bank leftover loose Meteors before retrieving a delivery scroll')
        if any(any(word in row.get('name', '') for word in
                   ('Confirm', 'Trade', 'Dialog', 'Shop', 'Add Item to Booth'))
               for row in source.get('windows', [])):
            raise ValueError('Close conflicting town/trade modals before retrieving a delivery scroll')
...
def reconcile(town, operation_id, uid, **options):
    ...
    if state is None:
        ...
        now=work.clock()
        state=work.journal.begin({'version':1,'operation_id':operation_id,
            'plan_id':plan['plan_id'],'uid':uid,'phase':'prepared','created_at':now,
            'input_deadline':now,'intent':{'item':selected['item'],
                'plan_sha256':plan['plan_sha256'],'deposit_receipt':selected['deposit_receipt'],
                'before':_plain_observation(observation)}})
        if state['phase']=='prepared':
            state=work.journal.transition(operation_id,'prepared','no_transfer',
                receipt={'operation_id':operation_id,'plan_id':plan['plan_id'],
                         'item':selected['item'],'input_attempted':False,
                         'admission_closed':True,'verified_at':now})
    plan, selected = work._plan(work.plan_id(operation_id), operation_id, uid)
    result=work.reconcile(state, plan, selected)
    result['no_input_proven']=(result['phase']=='no_transfer'
        and (result.get('receipt') or {}).get('input_attempted') is False
        and all(row['phase'] in ('prepared','no_transfer') for row in work.journal.history(operation_id)))
    return result
