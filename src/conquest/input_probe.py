...
class MessageTarget:
    def __init__(self, pid, hwnd):
        ...
        self.ancestor_api = bind(self.backend.user, "GetAncestor", [w.HWND,w.UINT], w.HWND)
        self.to_screen_api = bind(self.backend.user,'ClientToScreen',[w.HWND,c.POINTER(w.POINT)],w.BOOL)
        self.set_cursor_api = bind(self.backend.user,'SetCursorPos',[c.c_int,c.c_int],w.BOOL)
        self.key_state_api = bind(self.backend.user,'GetAsyncKeyState',[c.c_int],c.c_short)
...
def click_probe(target, x, y, expected_size, *, require_unfocused=True, move_settle_seconds=0,
                control=False, control_scan=0x1D,
                clock=time.monotonic, sleep=time.sleep):
    if not isinstance(move_settle_seconds,(int,float)) or not 0<=move_settle_seconds<=.25:
    ...
    if require_unfocused and before["foreground"] == before.get('root_hwnd',target.hwnd):
    ...
    try:
        if control:
            ...
            if target.snapshot()!=before:
                raise ValueError('Desktop changed during modifier preparation; no mouse message sent')
        modifier=0x8 if control else 0  # MK_CONTROL also belongs in the mouse message.
        target.post(0x200, modifier, packed)
        if move_settle_seconds:
            sleep(move_settle_seconds)
            if target.snapshot()!=before:
                raise ValueError('Window or desktop changed before button down; only mouse movement was queued')
        try:
...
def key_probe(target, vk, scan_code, expected_size, *, control_scan=None, clock=time.monotonic, sleep=time.sleep):
    ...
    started = clock()
    before = target.snapshot()
    if before["foreground"] == before.get('root_hwnd',target.hwnd) or before["minimized"]:
...
def positioned_click_probe(target,x,y,expected_size,*,sleep=time.sleep):
    ...
    if before['client_size']!=list(expected_size) or not 0<=x<expected_size[0] or not 0<=y<expected_size[1]:
    ...
    if target.snapshot()!=before or target.mouse_busy():
        raise ValueError('Desktop input is active; no cursor movement or click was sent')
    ...
    try:
        ...
        if positioned!=expected or target.mouse_busy():
            raise ValueError('Desktop changed during cursor positioning; no button message sent')
        def guarded_wait(delay):
            sleep(delay)
            if target.snapshot()!=expected or target.mouse_busy():
                raise ValueError('User input or window state changed during the positioned probe')
        result = click_probe(target,x,y,expected_size,move_settle_seconds=.2,sleep=guarded_wait)
    finally:
    ...
    after = target.snapshot()
    return {'schema_version':1,'stage':'positioned_background_probe','qualified':False,
            'before':before,'after':after,'point':[x,y],'cursor_borrowed':True,
            'focus_unchanged':before['foreground']==after['foreground'],
            'cursor_restored':before['cursor']==after['cursor'],'click':result}
