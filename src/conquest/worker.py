...
from conquest.input_probe import (
    MessageTarget,
    key_probe,
    click_probe,
    positioned_click_probe,
)
...
class Operations:
    ...
    def dispatch(self, operation, body):
        ...
        if operation == "sample":
            ...
            for field in fields:
                ...
                if kind in KINDS:
                    ...
                    value = list(
                        struct.unpack(
                            fmt, self.session.read(address, struct.calcsize(fmt))
                        )
                    )
                elif kind == "utf8":
                    value = (
                        self.session.read(address, 64)
                        .split(b"\0", 1)[0]
                ...
                result.append(
                    {"name": field.get("name"), "address": hex(address), "value": value}
                )
            ...
            for offset in range(0, len(data) - 7, 8):
                ...
                for module in self.session.modules:
                    if module["base"] <= value < module["base"] + module["size"]:
                        module_pointers.append(
                            {
                                "address": hex(start + offset),
                                "value": hex(value),
                                "module": module["name"],
                                "rva": hex(value - module["base"]),
                            }
                        )
                        break
            return {
                "module_pointers_before_anchor": module_pointers,
                "qualified": False,
            }
        if operation == "scan":
        ...
        if operation == "background-click":
            ...
            if reading.current_hp <= 0 or reading.current_hp / reading.max_hp < 0.4:
            ...
            if body.get("position_cursor", False):
                result = positioned_click_probe(
                    self.target, *body["point"], body["expected_size"]
                )
            else:
                options = (
                    {"move_settle_seconds": body["move_settle_seconds"]}
                    if "move_settle_seconds" in body
                    else {}
                )
                if "control" in body:
                    options["control"] = body["control"]
                result = click_probe(
                    self.target, *body["point"], body["expected_size"], **options
                )
        ...
        if operation == "revive-click":
            ...
            if mode == "background":
                ...
                with physical_coordinates():
                    ...
                    point = [
                        round(logical_point[0] * size[0] / viewport[0]),
                        round(logical_point[1] * size[1] / viewport[1]),
                    ]
                    ...
                    def before_press():
                        try:
                            ...
                            raise CaptureUnavailable(
                                "Revive pre-input rejected: " + str(error)
                            ) from error
                        if (
                            fresh.object_address != life.object_address
                            or fresh.map_id != life.map_id
                            or fresh.position != life.position
                            or not fresh.revive_ready_candidate
                        ):
                            raise CaptureUnavailable(
                                "Revive pre-input rejected: native life state changed"
                            )
                        from conquest.native_revive import point as native_point

                        try:
                            current = native_point(
                                adapter, viewport, require_hover=True
                            )
                        except ValueError as error:
                            raise CaptureUnavailable(
                                "Revive pre-input rejected: " + str(error)
                        ...
                        if current != logical_point:
                            raise CaptureUnavailable(
                                "Revive pre-input rejected: native popup moved"
                            )
        ...
        if operation in (
            ...
            name = (
                self.session.read(int(guard["name_address"], 0), 64)
                .split(b"\0", 1)[0]
                .decode("utf-8")
            )
            hp = struct.unpack("<I", self.session.read(int(guard["hp_address"], 0), 4))[
                0
            ]
            ...
            if operation == "foreground-key":
                return foreground_key(
                    ...
                    body["vk"],
                    body["expected_size"],
                    body.get("control", False),
                    body.get("require_foreground", False),
                )
            if operation == "foreground-drag":
                return foreground_drag(
                    self.target,
                    body["source"],
                    body["destination"],
                    body["expected_size"],
...
def serve(pid, hwnd, expected_sha256, info_path, lifetime=1800, *, read_only=False):
    ...
    if info_path.exists():
        raise ValueError(
            "Worker connection file already exists; check that worker before starting another"
        )
    with MemorySession(pid, expected_sha256) as session:
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                if not hmac.compare_digest(
                    self.headers.get("X-Conquest-Token", ""), token
                ):
                    self.send_error(403)
                    return
                try:
        ...
        server.timeout = 0.2
        key_state = bind(
            operations.target.backend.user, "GetAsyncKeyState", [c.c_int], c.c_short
        )
        try:
...
def request(info_path, operation, body=None):
    ...
    call = urllib.request.Request(
        f"http://127.0.0.1:{port}/{operation}",
