...
class FarmingControl:
    ...
    def snapshot(self):
        with self.lock:
            return {"enabled": self.enabled, "target_ids": list(self.ids),
                    'target_type_ids':list(self.type_ids),'resolved_target_ids':list(self.resolved_ids),
                    "input_mode": self.mode, "revision": self.revision,
                    "execution_state": self.execution_state, "note": self.note}

    def update(self, body):
        if not isinstance(body, dict) or not body or set(body) - {"enabled", "target_ids", "target_type_ids", "input_mode"}:
        ...
        with self.lock:
            ...
            types = target_ids(body['target_type_ids']) if 'target_type_ids' in body else self.type_ids
            ...
            if self.path is not None and (ids != self.ids or types != self.type_ids or mode != self.mode):
                self.path.parent.mkdir(parents=True, exist_ok=True)
                temporary = self.path.with_name(self.path.name + ".tmp")
                temporary.write_text(json.dumps({"target_ids": ids, 'target_type_ids':types, "input_mode": mode}), encoding="utf-8")
                os.replace(temporary, self.path)
            if (enabled, ids, types, mode) != (self.enabled, self.ids, self.type_ids, self.mode):
                self.revision += 1
                self.resolved_ids = []
            ...
            self.note = "Checking selected targets and input readiness" if enabled else "Farming is off"
            return self.snapshot()

    def finish_session(self, revision, reason):
        """An old runner may not erase a newer On/target selection."""
        with self.lock:
            if reason=='emergency_stop' or (reason=='requested_stop' and self.revision==revision):
    ...
    def dispatch(self, revision, entity_id, callback, *, type_id=None):
        ...
        with self.lock:
            if not self.enabled or revision != self.revision or not (entity_id in self.ids or type_id in self.type_ids):
                return False
            callback()
            return True
    ...
    def reconcile(self, *, focused, minimized, observed_ids, blockers=(), observed_types=None, pending_target=None):
        with self.lock:
            return self._reconcile(focused=focused,minimized=minimized,observed_ids=observed_ids,
                blockers=blockers,observed_types=observed_types or {}, pending_target=pending_target)

    def _reconcile(self, *, focused, minimized, observed_ids, blockers, observed_types, pending_target):
        self.resolved_ids = sorted({uid for uid in observed_ids if uid in self.ids or observed_types.get(uid) in self.type_ids})
        current = self.snapshot()
        if not current["enabled"]:
            return current
        if not current["target_ids"] and not current['target_type_ids']:
            state, note = "waiting_for_targets", "Select a monster group or individual ID"
        elif blockers:
            state, note = "blocked", "; ".join(blockers)
        elif minimized:
            state, note = "waiting_for_window", "Restore the game window; your switch stays on"
        elif current["input_mode"] == "foreground" and not focused:
            state, note = "waiting_for_focus", "Waiting for game focus; your switch stays on"
        elif not self.resolved_ids and not (pending_target and
                (pending_target['entity_id'] in self.ids or pending_target.get('type_id') in self.type_ids)):
            state, note = "waiting_for_targets", "Waiting for a selected monster ID to appear"
        else:
