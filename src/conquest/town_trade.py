...
def stash_candidate(item):
    ...
    return get("slot") is not None and (
        urgent_storage(item)
...
def sale_candidate(item):
    ...
    return (
        ...
        and 100000 <= kind < 600000
        and kind % 10 <= 6
        and type(plus) is int
        and plus == 0
        and get("slot") is not None
    )
...
def expendable_arrow(item, inventory):
    ...
    if ammo and ammo.type_id == item.type_id:
        total += ammo.amount
    return (
        item in inventory.items
        and item.type_id in NORMAL_ARROWS
...
def transient_observation(error):
    ...
    if isinstance(error, GuiObservationChanged):
        return True
    text = str(error)
    return any(
        part in text
        for part in (
            ...
            "observation expired",
            "Vendor moved before interaction",
            "Shop or vendor changed before buying",
            "Inventory changed before selling",
            "Shop product reference changed",
...
class TownTrade:
    ...
    def life(self, minimum_health=0, *, any_map=False):
        ...
        if (
            ...
            or life.current_hp < life.max_hp * minimum_health
        ):
    ...
    def warehouse_native_point(point, revision):
        ...
        logical = revision.gui_size
        physical = revision.client_size
        if (
            logical is None
            or len(logical) != 2
            or len(physical) != 2
            or min(*logical, *physical) <= 1
            or not 0 <= point[0] < logical[0]
            or not 0 <= point[1] < logical[1]
        ):
            raise ValueError(
                "Warehouse point is outside the current GUI/client geometry"
            )
        result = tuple(
            round(value * native / gui)
    ...
    def reread_warehouse_vendor(self, identity, *, grid_input=False):
        ...
        if fresh_identity != identity:
            raise ValueError(
                "Warehouseman model, identity or tile changed before interaction"
            )
        return fresh
    ...
    def warehouse_deposit(self, uid):
        ...
        with physical_coordinates():
            ...
            if market_warehouse:
                ...
                if max(abs(a - b) for a, b in zip(life.position, npc.position)) > 18:
                    raise ValueError(
                        "Travel closer to the vendor before opening its shop"
                    )
                ...
                def vendor_unchanged():
                    ...
                    if (
                        max(
                            abs(a - b)
                            for a, b in zip(fresh_life.position, fresh_npc.position)
                        )
                        > 18
                    ):
                        return False
                    ...
                    return fresh_identity == warehouse_identity and (
                        fresh_npc.object_address,
                        fresh_npc.entity_id,
                        fresh_npc.type_id,
                        fresh_npc.name,
                        fresh_npc.map_id,
            ...
            stored = reader.read()
            candidates = [
                i for i in before.items if i.uid == uid and stash_candidate(i)
            ]
            if len(candidates) != 1:
                raise ValueError(
                    "Selected warehouse item is absent or not a supported valuable"
                )
            item = candidates[0]
            if len(stored.items) >= stored.capacity or item.uid in {
                i.uid for i in stored.items
            }:
            ...
            if target.size != (272.0, 326.0) or target.scroll != (0.0, 0.0):
            ...
            logical_destination = (
                round(target.position[0] + target.size[0] / 2),
                round(target.position[1] + target.size[1] / 2),
            )
            source = self.warehouse_native_point(logical_source, layout_revision)
            destination = self.warehouse_native_point(
                logical_destination, layout_revision
            )
            ...
            def before_press():
                ...
                def ready():
                    ...
                    if (
                        ...
                        or current.silver != before.silver
            ...
            def before_release():
                ...
                def ready():
                    ...
                    if (
                        reader.gui.read("Warehouse/ScrollingRegion_") != target
                        or not vendor_unchanged()
                    ):
                        raise ValueError(
                            "Warehouse moved during deposit drag; item preserved if still carried"
                        )
            ...
            try:
                ...
                pass
            self.verified_read(
                lambda: (self.inventory.read(), reader.read()),
                lambda pair: deposit_received(item, before, stored, *pair),
                "Warehouse deposit was not verified; no repeat input issued",
                timeout=5,
            )
            return {
                "stored": item.uid,
    ...
    def click_npc(self, npc, reread, *, point=None):
        ...
        def unchanged():
            try:
                ...
                if transient_observation(error):
                    raise TownObservationUnavailable(
                        str(error) + "; no button pressed"
                    ) from error
                raise
            if fresh != npc:
                raise TownObservationUnavailable(
                    "Vendor moved before interaction; no button pressed"
                )
    ...
    def vendor(self, type_id, *, stable_identity_only=False):
        ...
        if stable_identity_only:
            if type_id != 0 or life.map_id != 1036:
                raise ValueError(
                    "Stable warehouse identity is restricted to Market warehouse grid operations"
                )
            from conquest.market_services import discover
            ...
            expected = vendor_identity(life.map_id, type_id)
            reader = self.npcs
        if not stable_identity_only:
            choices = [
                n
                for n in reader.read(life.map_id).npcs
                if n.type_id == expected.type_id
            ]
            if len(choices) != 1:
    ...
    def execute(self, body):
        ...
        if body == {"action": "ground-items"}:
            ...
            return {
                ...
                "drops": [
                    {**asdict(drop), "wanted": wanted_drop(drop)} for drop in drops
                ],
            }
        if body == {"action": "conductress-open"}:
            ...
            if read_conductress(self.observer) != npc:
                raise ValueError("Conductress changed during observation")
            self.input_attempted = True
            self.click_npc(npc, lambda: read_conductress(self.observer))
            self.conductress_npc = npc
            self.service_npc = (
                npc  # The shared geometry reader can scroll this dialogue.
            )
            return {"interacted": True, "npc_id": npc.entity_id}
        if (
            set(body) == {"action", "destination"}
            and body["action"] == "conductress-travel"
        ):
            from conquest.conductress import read_conductress, destination_point

            npc = read_conductress(self.observer)
            if npc != getattr(self, "conductress_npc", None):
                raise ValueError("Open this Conductress before selecting a destination")
            point = destination_point(self.observer, body["destination"])
            if self.inventory.read().silver < 100:
                raise ValueError("Conductress requires 100 silver")
            if (
                read_conductress(self.observer) != npc
                or destination_point(self.observer, body["destination"]) != point
            ):
            ...
            return {
                "destination_selected": body["destination"],
                "point": point,
                "npc_id": npc.entity_id,
            }
        action = body.get("action")
        if action == "warehouse-items" and (
            set(body) == {"action"}
            or set(body) == {"action", "rich"}
            and body["rich"] is True
        ):
        ...
        if action == "open-bank" and set(body) == {"action"}:
            ...
            self.verified_read(
                reader.read,
                lambda b: True,
                "Warehouse opening unverified; no repeat input issued",
            )
            return {"opened": True, "npc_id": npc.entity_id}
        ...
        if action == "warehouse-money" and set(body) == {"action"}:
            ...
            return {
                "silver": self.inventory.read().silver,
                "stored_silver": bank.silver,
                "amount": bank.amount,
            }
        if action in ("warehouse-money-deposit", "warehouse-money-withdraw") and set(
            body
        ) == {"action", "amount"}:
        ...
        if action == "vendor-status" and set(body) == {"action", "vendor_type"}:
            try:
                npc = (
                    self.warehouse_vendor_snapshot()[1]
                    if body["vendor_type"] == 0
                ...
                return {
                    "reachable": clear_scene(point, size_for(self.observer)),
                    "npc_id": npc.entity_id,
                    "position": npc.position,
                    "point": point,
                }
        ...
        if action == "buy-equipment" and set(body) == {
            ...
            for _ in range(20):
                ...
                if len(choices) != 1:
                    raise ValueError("Equipment absent from live shop")
                product = choices[0]
                state = read_equipment(self.observer)
                if category(product.type_id) not in VENDORS[
                    body["vendor_type"]
                ] or not upgrade_candidate(product, state):
                ...
                if any(i.type_id == product.type_id for i in before.items):
                    raise ValueError(
                        "Matching upgrade already carried; no duplicate purchase"
                    )
                try:
                    ...
                    lambda b: (
                        len(
                            [
                                i
                                for i in b.items
                                if i.type_id == product.type_id
                                and i.uid not in {j.uid for j in before.items}
                            ]
                        )
                        == 1
        ...
        if action in ("equip", "equip-arrows") and set(body) == {"action", "uid"}:
            ...
            try:
                self.shop.gui.read("Shop")
            except ValueError as error:
                if "not active" not in str(error) and "absent" not in str(error):
                    raise
            else:
                raise ValueError("Close shop before equipping")
            before = self.inventory.read()
            state = read_equipment(self.observer)
            matches = [i for i in before.items if i.uid == body["uid"]]
            if len(matches) != 1:
                raise ValueError("Equipment UID not carried")
            item = matches[0]
            s = self.observer.adapter
            ...
            eligible = (
                eligible_arrow(details, state)
                if action == "equip-arrows"
                else upgrade_candidate(details, state)
            )
            if (
                details["uid"] != item.uid
                or details["type_id"] != item.type_id
                or not eligible
            ):
                raise ValueError("Carried item is not an eligible equipment upgrade")
            try:
                self.shop.gui.read("Inventory")
            except ValueError as error:
                if "not active" not in str(error) and "absent" not in str(error):
                    raise
                self.click(inventory_button(self.shop.gui))
                self.verified_read(
                    lambda: self.shop.gui.read("Inventory"),
                    bool,
                    "Inventory opening unverified",
                )
            grid = self.shop.gui.read("Inventory/##ItemGrid_")
            if grid.size != (407.0, 175.0) or grid.scroll != (0.0, 0.0):
                raise ValueError("Inventory equip grid differs")
            if (
                self.inventory.carried_pointer(before, item.uid) != pointer
            ...
            after = self.verified_read(
                lambda: (self.inventory.read(), read_equipment(self.observer)),
                lambda pair: equip_receipt(item.uid, slot, before, state, *pair),
                "Equip unverified; item preserved and no repeat input issued",
                timeout=3,
            )
            return {
                "equipped": item.uid,
        ...
        if action == "warehouse-withdraw-meteor" and set(body) == {"action", "uid"}:
            ...
            if len(candidates) != 1 or len(before.items) >= before.capacity:
                raise ValueError(
                    "One stored Meteor and a free inventory slot are required"
                )
            item = candidates[0]
            ...
            if (
                ...
                or not 0 <= item.slot < 48
            ):
                raise ValueError(
                    "Meteor warehouse slot is not in the qualified visible grid"
                )
            point = (
                round(grid.position[0] + 20 + 40 * (item.slot % 6)),
                round(grid.position[1] + 20 + 40 * (item.slot // 6)),
            )
            fresh = self.inventory.read()
            if (
                fresh.items != before.items
                ...
                or reader.gui.read("Warehouse/ScrollingRegion_") != grid
            ):
            ...
            self.verified_read(
                ...
                "Meteor withdrawal unverified; no repeat input issued",
                timeout=3,
            )
            return {
                "withdrawn": item.uid,
        ...
        if action == "warehouse-withdraw-scroll" and set(body) == {
            "action",
            "operation_id",
            "uid",
        }:
        ...
        if action == "warehouse-reconcile-scroll" and set(body) == {
            "action",
            "operation_id",
            "uid",
        }:
        ...
        if action == "open" and set(body) == {"action", "vendor_type"}:
            ...
            time.sleep(0.25)
            result = self.verified_read(
                lambda: self.shop.read(npc.entity_id),
                lambda value: True,
        ...
        if action == "close" and set(body) == {"action", "window"}:
            ...
            def panel_signature():
                return tuple(
                    sorted(
                        ...
                        for item in GuiReader.for_session(
                            self.observer.adapter
                        ).windows()
                        if (
                            item["name"] in PANELS
                            or item["name"] in TRANSACTIONS
            ...
            try:
                self.verified_read(
                    closed, bool, "Town panel close was not verified", timeout=2
                )
            except ValueError as error:
                ...
                try:
                    current = self.shop.gui.read(body["window"])
                except ValueError as current_error:
                    if "not active" in str(current_error) or "absent" in str(
                        current_error
                    ):
                        return {"closed": True}
                    raise
                if current != window:
                    raise ValueError(
                        "Town panel changed after its close attempt"
                    ) from error
                ...
                def retry_guard():
                    ...
                    if current_panels != panels:
                        raise ValueError(
                            "Town panel set changed before retrying its close"
                        )
                    if any(name in TRANSACTIONS for name, *_ in current_panels):
                        raise ValueError(
                            "Transaction dialog appeared before retrying panel close"
                        )
                ...
                if not retry["submitted"]:
                    raise ValueError(
                        "Town panel close retry did not reach its input boundary"
                    )
                self.verified_read(
                    closed, bool, "Town panel close was not verified", timeout=2
                )
        ...
        if action == "buy" and set(body) == {"action", "vendor_type", "type_id"}:
            ...
            after = self.verified_read(
                self.inventory.read,
                lambda after: (
                    after.count(product.type_id) > before.count(product.type_id)
            ...
            return {
                "bought": product.type_id,
                "amount": after.count(product.type_id) - before.count(product.type_id),
                "price": product.price,
                "silver": after.silver,
            }
        if action in ("sell", "sell_partial_arrow") and set(body) == {
            ...
            destination = (
                round(shop.window.position[0] + shop.window.size[0] / 2),
            ...
            return {
                ...
                "type_id": item.type_id,
                "plus": item.plus,
                "silver_gained": after.silver - before.silver,
            }
