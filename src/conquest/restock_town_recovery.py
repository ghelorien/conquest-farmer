...
def _native_tail_safe(loop, target, map_id):
    ...
    if (
        ...
        or health.get("target") != target
        or loop.identity != target
        or health.get("profile_id") != loop.town_visit.profile
        or life.get("map_id") != map_id
        ...
        or data.get("manual_input_fence")
        or data.get("manual_mouse")
        or not 0 <= time.time() - data.get("observed_at", 0) <= 1
        or _other_holds()
    ):
        raise ValueError(
            "Interrupted restock continuation lacks fresh safe original ownership"
        )
...
def capture_pre_admission_tail(loop):
    ...
    if prior:
        resumed = read_json(JOURNAL)
        market = read_json(MarketVisit().path)
        if (
            prior.get("phase") != "captured"
            or row.get("phase") != "town_work"
            or row.get("reasons")
            != (
                ["urgent_banking", "restock"]
                if prior.get("capture_kind") == "booth_confirmation"
                else ["restock"]
            )
            or any(
                resumed.get(k) != prior["meteor"].get(k)
                for k in ("started_at", "scroll_uid", "meteor_uids", "origin", "after")
            )
            or any(
                market.get(k) != prior["market"].get(k)
                for k in (
                    "visit_id",
                    "town_visit_id",
                    "farmer_profile_id",
        ...
        if prior.get("capture_kind") in (
            ...
            expected, proofs = (
                ...
                if prior["capture_kind"] in ("no_transfer", "operator_warehouse")
                else _delivery_proof(*args)
            )
            if proofs != prior["delivery_proofs"] or _ownership(
                loop.town("supplies")
            ) != _ownership(expected):
    ...
    if (
        row.get("reasons") == ["urgent_banking", "restock"]
        and meteor.get("phase") == "storing_scroll"
    ):
    ...
    if any(
        ...
        and e.get("event") == "failed"
        and e.get("detail") == NO_TRANSFER_FAILURE
    ...
    if (
        ...
        raise ValueError(
            "Interrupted restock lacks its original process and exchange chain"
        )
    ...
    frames = {
        (Path(f.get("file", "")).name, f.get("function"), f.get("line"))
        for f in failure.get("failure_trace", [])
    }
    if (
        failure.get("event") != "failed"
        or failure.get("detail") != "Unqualified merchant client fingerprint"
        ...
        <= {(a, b) for a, b, c in frames}
        or failure.get("time", 0) < meteor["started_at"]
    ):
        raise ValueError(
            "Restock continuation is not the exact pre-admission grant rejection"
        )
    ...
    for event in events:
        if event.get("time", 0) <= failure["time"]:
            continue
        if event.get("event") in ("started", "stopped"):
            continue
        trace = event.get("failure_trace") or []
        last = trace[-1] if trace else {}
        if (
            event.get("event") == "failed"
            and event.get("error_type") == "builtins.ValueError"
            and event.get("detail")
            == "Merchant input or manual ownership still holds the interrupted trip"
            and Path(last.get("file", "")).name == "restock_town_recovery.py"
            and last.get("function") == "capture_pre_admission_tail"
        ):
            continue
        raise ValueError(
            "Gameplay or an unrecognized failure followed the interrupted restock"
        )
    market = read_json(MarketVisit().path)
    if (
        ...
        or not market.get("started_at", 0)
        <= failure["time"]
    ...
    if deliveries.exists():
        with sqlite3.connect(
            deliveries.resolve().as_uri() + "?mode=ro", uri=True
        ) as db:
            if (
                db.execute(
                ...
                or db.execute(
                    "SELECT 1 FROM transactions WHERE created>=? LIMIT 1",
                    (row["required_at"],),
                ).fetchone()
            ):
                raise ValueError(
                    "A delivery was admitted during this interrupted restock"
                )
...
def _capture_booth_confirmation_tail(loop, row, meteor):
    ...
    visit = loop.town_visit
    target = row.get("urgent_target")
    urgent = row.get("urgent_intent") or []
    tail = row.get("urgent_banking_tail_completed_at")
    session = (row.get("baseline") or {}).get("session_id")
    kills = read_json(state_path("reports/desktop-farming/kill-session.json"))
    if (
        row.get("phase") != "town_work"
        ...
        raise ValueError(
            "Mixed restock continuation lacks its original native transaction chain"
        )
    _native_tail_safe(loop, target, 1036)
    events = [
        e for e in _rows(EVENTS) if e.get("town_visit_id") == row["town_visit_id"]
    ...
    stored = {
        (e.get("stored"), e.get("type_id"))
        for e in events
        if e.get("event") == "valuable_stored"
        and e.get("verified_in_warehouse") is True
        and row["required_at"] <= e.get("time", 0) <= tail
    }
    if len(urgent) != len({i.get("uid") for i in urgent}) or any(
        (i.get("uid"), i.get("type_id")) not in stored for i in urgent
    ):
    ...
    if (
        len(purchases) != loop.route.supplies.healing_restock_to
        or any(
            p.get("receipt", {}).get("bought") != loop.route.supplies.healing_type
            or p["receipt"].get("amount") != 1
            or type(p["receipt"].get("price")) is not int
            or p["receipt"]["price"] <= 0
            or type(p["receipt"].get("silver")) is not int
            or p["receipt"]["silver"] < 0
            for p in purchases
        )
        or any(
            b["receipt"]["silver"] != a["receipt"]["silver"] - b["receipt"]["price"]
    ...
    if (
        ...
        raise ValueError(
            "Mixed restock failure is not the exact pre-admission booth interception"
        )
    for event in events:
        ...
        if (
            event.get("event") == "failed"
            and ("restock_town_recovery.py", "_capture_booth_confirmation_tail")
            in frames
        ):
    ...
    if (
        _ownership(bag) != _ownership(meteor["after"])
    ...
    def owned(snapshot):
        items = sorted(
            (i["uid"], i["type_id"], i["plus"], i["quantity"])
    ...
    bag_items = sorted(
        (i["uid"], i["type_id"], i["plus"], i["amount"]) for i in bag["items"]
    )
    ...
    exact_closed = (
        confirmation is None and first.get("canonical_manual_ownership") is True
    )
    if (
        owned(first) != owned(second)
        or first.get("identity") != target
        or first.get("map_id") != 1036
        or first.get("hp", 0) <= 0
        or second.get("hp", 0) <= 0
        or not 0 <= time.time() - second.get("timestamp", 0) <= 3
        or first.get("trade") is not None
        or first.get("request") is not None
        or confirmation != second.get("confirmation")
        or first.get("canonical_manual_ownership")
        != second.get("canonical_manual_ownership")
    ...
    _native_tail_safe(loop, target, 1036)
    row["pre_admission_restock_tail"] = {
        "capture_kind": "booth_confirmation",
        "target": target,
        "failure": failure,
        "market": market,
        "meteor": meteor,
        ...
        "confirmation_observation": {
            "state": "open" if exact_open else "closed",
...
def resume_pre_admission_tail(loop):
    ...
    visit = loop.town_visit
    row = visit.state()
    claim = row.get("pre_admission_restock_tail")
    if not claim or row.get("town_work_completed_at"):
        return False
    if claim.get("phase") != "captured":
        raise ValueError(
            "Interrupted restock cash tail was attempted; reconcile before any replay"
        )
    if row.get("phase") != "town_work" or row.get("reasons") != (
        ["urgent_banking", "restock"]
        if claim.get("capture_kind") == "booth_confirmation"
        else ["restock"]
    ):
        raise ValueError("Interrupted restock visit changed")
    loop.check_stop()
    initial = loop.health()
    if initial.get("target") != claim["target"] or not _process_identity(
        claim["target"]
    ):
    ...
    original = claim["meteor"]
    market = read_json(MarketVisit().path)
    if (
        meteor.get("phase") != "completed"
        or meteor.get("exchange_verified") is not True
        or any(
            meteor.get(k) != original.get(k)
            for k in ("started_at", "scroll_uid", "meteor_uids", "origin", "after")
        )
        or not original["started_at"]
        <= meteor.get("market_verified_at", 0)
        <= meteor.get("completed_at", 0)
        or any(
            market.get(k) != claim["market"].get(k)
            for k in (
                "visit_id",
                "town_visit_id",
                "farmer_profile_id",
    ...
    if (
        ...
        or any(
            ...
            or r.get("type_id") != original_items[r["stored"]]["type_id"]
            for r in receipts
        )
    ):
        raise ValueError("Meteor storage receipts are incomplete")
    expected = dict(claim["bag"])
    expected["items"] = [i for i in expected["items"] if i["uid"] not in stored]
    fare = read_json(meteor_banking.POLICY)["origins"][str(meteor["origin"])]["return"][
        "fare"
    ]
    expected["silver"] -= fare
    loop.adopt_ammunition()
    bag = loop.town("supplies")
    if _ownership(bag) != _ownership(expected) or claim.get(
        "unexpected_supply_addition"
    ):
        ...
        try:
            ...
            loop.record(
                "restock_ownership_mismatch",
                expected=expected,
                observed=bag,
                return_before=meteor.get("return_before"),
                detail=str(error),
            )
            raise
    if (
        _ownership(bag) != _ownership(expected)
        or any(stash_candidate(i) for i in bag["items"])
    ...
    _native_tail_safe(loop, claim["target"], loop.route.restock_map_id)
    if (
        _ownership(loop.town("supplies")) != _ownership(expected)
        or bank["silver"] != expected["silver"]
    ):
        raise ValueError("Cash tail ownership changed before input")
    claim.update(
        phase="cash_attempted",
        cash_before=bank,
        cash_inventory_before=bag,
...
def _ownership(bag):
    if (
        not isinstance(bag, dict)
        or not isinstance(bag.get("items"), list)
        or type(bag.get("silver")) is not int
        or type(bag.get("capacity")) is not int
    ):
        raise ValueError("Fresh ownership observation is incomplete")
    items = sorted(
        (tuple(item.get(field) for field in FIELDS) for item in bag["items"]),
        key=lambda item: item[0],
    )
    if any(type(item[0]) is not int or item[0] <= 0 for item in items) or len(
        {item[0] for item in items}
    ) != len(items):
        raise ValueError("Fresh item identities are invalid")
    return {
        "items": items,
        "equipped_ammo": bag.get("equipped_ammo"),
        "silver": bag["silver"],
...
def _remaining_after_market_bank(meteor):
    ...
    if (
        ...
        raise ValueError(
            "Meteor return is not a proved pre-submission movement recovery"
        )
    stored = {
        receipt.get("stored"): receipt
        for receipt in receipts
        if receipt.get("verified_in_warehouse") is True
    }
    scroll = meteor["scroll_uid"]
    if (
        stored.get(scroll, {}).get("type_id") != SCROLL
    ...
    expected = dict(after)
    expected["items"] = [
        item for item in after.get("items", []) if item.get("uid") not in stored
    ]
...
def _other_holds():
    ...
    return any(
        (journey(), trade(), delivery(), bool(withdrawal()), overflow(), storage_halt())
    )
...
def claim_interrupted_return(visit, *, info_path, visit_id, original_target, reference):
    ...
    if (
        ...
        or not _process_identity(original_target)
    ...
    meteor = read_json(JOURNAL)
    if (
        type(meteor.get("started_at")) not in (int, float)
    ...
    if (
        len(failures) != 1
        or failures[0]["time"] < meteor["market_verified_at"]
        or post_failure_input
    ):
        raise ValueError("Return movement failure or later transaction is ambiguous")
    if any(
        type(row.get("time")) not in (int, float) or row["time"] >= meteor["started_at"]
        for row in _rows(MONEY, allow_missing=True)
    ):
        raise ValueError("Money transfer after Meteor departure needs reconciliation")
    return visit.claim_restock_tail_recovery(
        visit_id=visit_id,
        target=target,
        evidence={
            "reference": reference,
            "interrupted_at": failures[0]["time"],
...
def require_claimed_identity_before_meteor(loop):
    ...
    if (
        ...
        raise ValueError(
            "Claimed Meteor return process or location changed; no travel input issued"
        )
...
def resume_claimed(loop):
    ...
    if row.get("restock_recovery_attempted_at"):
        raise ValueError(
            "Restock recovery tail was attempted; reconcile before any retry"
        )
    health = loop.health()
    controls = health.get("embedded_controls") or {}
    life = controls.get("life") or {}
    ...
    meteor = read_json(JOURNAL)
    if (
        target != row.get("restock_recovery_target")
        or target != loop.identity
        or not _process_identity(target)
        ...
        or not meteor.get("completed_at")
        or _other_holds()
        or delivery_enabled()
    ):
        raise ValueError(
            "Restock recovery process, Meteor receipt, or input policy changed"
        )
    loop.adopt_ammunition()
    bag = loop.town("supplies")
    if (
        _digest(_ownership(bag)) != claim.get("post_storage_ownership_digest")
    ...
    if (
        _digest(_ownership(fresh)) != claim["post_storage_ownership_digest"]
        or sum(
            item["type_id"] == METEOR and item.get("amount") == item.get("limit") == 1
            for item in fresh["items"] + stored["items"]
        )
        >= 10
    ):
    ...
    bag = loop.town("supplies")
    if (
        any(stash_candidate(item) for item in bag["items"])
        or needs_town(supply_counts(bag, loop.route), loop.route)
        or _other_holds()
    ):
    ...
    service_window(loop, town=True)
    if _other_holds() or (loop.health().get("embedded_controls") or {}).get(
        "manual_input_fence"
    ):
