...
class EmbeddedBridge:
    ...
    def __init__(
        ...
        *,
        ...
        self.deadline = (
            float("inf") if lifetime is None else time.monotonic() + lifetime
        )
            ...
            def do_POST(self):
                ...
                try:
                    ...
                    if "profile_id" in body:
                        if (
                            bridge.character_context is None
                            or body.pop("profile_id")
                            != bridge.character_context.profile.id
                        ):
                            raise ValueError(
                                "This bridge belongs to a different character profile"
                            )
                    operation = self.path.strip("/")
                    if operation not in bridge.allowed:
                        raise ValueError(
                            "Operation is not available in the embedded bridge"
                        )
                    ...
                    with bridge.lock:
                        ...
                        if (
                            operation
                            in ("foreground-click", "foreground-key", "foreground-drag")
                            and not bridge.native_probe_mode
                        ):
                            raise ValueError(
                                "Operation is not available outside a native-window diagnostic"
                            )
                        if operation in (
                            "background-click",
                            "revive-click",
                            ...
                            if (
                                ...
                                or not 0 < expiry - time.time() <= 5
                            ):
                                raise ValueError(
                                    "A background diagnostic must expire within five seconds"
                                )
                            ...
                            if operation in (
                                ...
                                if window["root_hwnd"] != window["hwnd"]:
                                    raise ValueError(
                                        "Wait for the native client window before probing"
                                    )
                        ...
                        if operation == "town":
                            ...
                            if body.get("action") not in (
                                ...
                                if (
                                    ...
                                    raise ValueError(
                                        "Town input must expire within five seconds"
                                    )
                                if bridge.snapshot()["control"]["enabled"]:
                                    raise ValueError("Stop farming before town input")
                            read_only = body.get("action") in (
                                "supplies",
                                "shop",
                                "gear",
                                "vendor-status",
                            ...
                            revision = (
                                bridge.snapshot()["control"].get("revision")
                                if not read_only
                                else None
                            )
                            ...
                            def check_town_input():
                                ...
                                if (
                                    ...
                                    raise CaptureUnavailable(
                                        "Town input permission changed; reobserve before continuing"
                                    )
                                if previous:
                                    previous()
                            ...
                            try:
                                if attach_guard:
                                    town.check_input = check_town_input
                                result = town(
                                    {k: v for k, v in body.items() if k != "expires_at"}
                                )
                            finally:
                                if attach_guard:
                                    town.check_input = previous
                        elif operation == "sample-npcs":
                            if body or bridge.on_sample_npcs is None:
                                raise ValueError(
                                    "Vendor reader is unavailable or has unsupported arguments"
                                )
                            ...
                            if (
                                set(body) != {"detached"}
                                or type(body["detached"]) is not bool
                                or bridge.on_native_window is None
                            ):
                                raise ValueError(
                                    "Native window mode is unavailable or invalid"
                                )
                            if (
                                body["detached"]
                                and bridge.snapshot()["control"]["enabled"]
                            ):
                                raise ValueError(
                                    "Stop farming before detaching the client"
                                )
                            ...
                            if body or bridge.on_reconnect is None:
                                raise ValueError(
                                    "Reconnect retry is unavailable or has unsupported arguments"
                                )
                            bridge.on_reconnect()
                            ...
                            if body or bridge.on_reload is None:
                                raise ValueError(
                                    "App reload is unavailable or has unsupported arguments"
                                )
                    ...
                    if self.path.strip("/") == "route-jump" and isinstance(
        ...
        if self.info_path.exists():
            raise ValueError(
                "An embedded connection file already exists; check its owner first"
            )
        self.server = HTTPServer(("127.0.0.1", 0), Handler)
        self.server.timeout = 0.2
        self.thread = threading.Thread(
            target=self.run, name="embedded-diagnostics", daemon=True
        )
        try:
                ...
                json.dumps(
                    {
                        ...
                        "token": self.token,
                        "target_pid": operations.session.pid,
                        "expires_at": None
                        if lifetime is None
                        else time.time() + lifetime,
                    }
                ),
                encoding="utf-8",
            )
