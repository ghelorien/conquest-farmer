...
class RegionRotation:
    def __init__(self, regions, *, window=20, empty_fraction=0.6, idle_seconds=8):
        self.regions = regions
        self.index = 0
        self.samples = deque()
        self.window = window
        self.threshold = empty_fraction
    ...
    def observe(self, now, position, occupied, *, available=True):
        ...
        if self.entered is None:
            self.entered = self.last_occupied = now
        if occupied:
            self.last_occupied = now
        if not self.samples or now - self.samples[-1][0] >= 0.25:
            self.samples.append((now, not occupied))
        while self.samples and self.samples[0][0] < now - self.window:
            self.samples.popleft()
        if occupied or now - self.entered < self.idle_seconds or len(self.samples) < 8:
            return None
        fraction = sum(empty for _, empty in self.samples) / len(self.samples)
        idle = now - self.last_occupied
        if fraction < self.threshold and idle < self.idle_seconds:
            return None
