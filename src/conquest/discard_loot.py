...
def discard_candidate(item):
    ...
    return (
        ...
        and type(get("plus")) is int
        and get("plus") == 0
        and sale_candidate(item)
    )
...
def removal_received(item, before, after):
    ...
    return after.silver == before.silver and {identity(i) for i in after.items} == {
        identity(i) for i in before.items if i.uid != item.uid
    }
...
def inventory_button(gui):
    ...
    for i in range(count):
        ...
        if struct.unpack_from("<II", record) == (0x02A99238, 0x482010):
            matches.append((address, record))
    if len(matches) != 1:
        raise ValueError("Inventory button table is absent or ambiguous")
    address, _ = matches[0]
    record = s.read_block(address, 536)
    frame = struct.unpack("<I", s.read_block(context + 0x3E38, 4))[0]
    if (
        not 0 <= frame - struct.unpack_from("<I", record, 0x70)[0] <= 3
        or struct.unpack_from("<I", record, 0x74)[0] != 6
    ):
...
class DiscardLoot:
    ...
    def discard(self, uid):
        ...
        try:
            ...
            if row is None:
                ...
                self.deferred_attempts = min(
                    getattr(self, "deferred_attempts", 0) + 1, 6
                )
    ...
    def close_inventory(self):
        ...
        t.verified_read(
            closed, bool, "Inventory cleanup is not yet confirmed", timeout=1
        )
        self.cleanup_pending = False
    ...
    def _discard(self, uid):
        ...
        if len(choices) != 1 or uid in self.attempted:
            raise ValueError(
                "Item is protected, absent, or already attempted for discard"
            )
        item = choices[0]
        life = t.life(0.8)
        ...
        try:
            ...
            t.verified_read(
                lambda: t.shop.gui.read("Inventory"),
                lambda value: True,
        ...
        window = t.shop.gui.read("Inventory")
        if (
            window.position[0] <= destination[0] <= window.position[0] + window.size[0]
            and window.position[1]
            <= destination[1]
            <= window.position[1] + window.size[1]
        ):
        ...
        if (
            ...
            raise CaptureUnavailable(
                "Inventory or player moved before discard; no drag sent"
            )
        ...
        bag, found = t.verified_read(
            receipt,
            lambda pair: removal_received(item, before, pair[0]) and len(pair[1]) == 1,
            "Discard not verified in bag and ground memory; no repeat drag issued",
            timeout=3,
        )
