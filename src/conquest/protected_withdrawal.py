...
def pending(path=None):
    ...
    try:
        ...
        try:
            ...
            rows = db.execute(
                "SELECT operation_id,plan_id,uid,phase,state FROM operations "
        ...
        for row in rows:
            ...
            if (
                ...
                or state.get("operation_id") != row["operation_id"]
                or state.get("plan_id") != row["plan_id"]
                or state.get("uid") != row["uid"]
                or state.get("phase") != row["phase"]
            ):
                raise ValueError(
                    "Protected withdrawal journal row disagrees with its state"
                )
            result.append(state)
        return result
    except Exception:
...
def core(item):
    if not isinstance(item, dict) or set(item) != set(CORE):
        raise ValueError(
            "Protected item must contain exactly the seven ownership fields"
        )
    result = {key: item[key] for key in CORE}
    if (
        type(result["uid"]) is not int
        or result["uid"] <= 0
        or type(result["type_id"]) is not int
        or result["type_id"] <= 0
        or type(result["plus"]) is not int
        or not 0 <= result["plus"] <= 12
        or type(result["gem1"]) is not int
        or result["gem1"] < 0
        or type(result["gem2"]) is not int
        or result["gem2"] < 0
        or type(result["quantity"]) is not int
...
def _selected_core(item):
    result = core(item)
    if (
        not 100000 <= result["type_id"] < 600000
        or result["quantity"] != 1
        or result["bound"] is not False
    ):
...
def _ammo(value):
    ...
    if (
        ...
        or type(result["limit"]) is not int
        or not 0 <= result["amount"] <= result["limit"]
        or result["plus"] is not None
        and type(result["plus"]) is not int
    ):
...
def _source(value):
    ...
    return {
        ...
        "character_uid": value["character_uid"],
        ...
        "hp": value["hp"],
        "silver": value["silver"],
        "inventory": _core_map(value["inventory"]),
        "booth": _core_map(value["booth"]),
        "trade": value["trade"],
...
def _read_artifact(reference, root, digest_name):
    ...
    if candidate.is_absolute():
        raise ValueError(
            "Immutable receipt references must be relative to the data root"
        )
    root = root.resolve()
    ...
    try:
        ...
        raise ValueError(
            "Immutable receipt path escapes the protected receipt directory"
        ) from error
...
def _warehouse_added(before, after, item):
    old = _core_map(before.get("items", ()))
    new = _core_map(after.get("items", ()))
    if item["uid"] in old or set(new) != set(old) | {item["uid"]}:
        return False
    return (
        {uid: value for uid, value in new.items() if uid != item["uid"]} == old
        and new[item["uid"]] == item
        and before.get("capacity") == after.get("capacity")
    )
...
def _validate_deposit_receipts(plan):
    ...
    for sequence, entry in enumerate(plan["items"], 1):
        receipt = _read_artifact(
            entry["deposit_receipt"], RECEIPT_ROOT, "receipt_sha256"
        )
        required = {
            "version",
            "kind",
        ...
        if (
            ...
            or attempt["previous_receipt_sha256"] != previous
            or attempt["release"] != plan["release"]
            or attempt["farmer"] != plan["farmer"]
            or attempt["market_visit"] != receipt["market_visit"]
            or attempt["before"] != receipt["before"]
            or core(attempt["item"]) != entry["item"]
            or receipt["verified_at"] < attempt["created_at"]
        ):
            raise ValueError(
                "Controlled deposit attempt does not bind the final receipt"
            )
        before, after = receipt["before"], receipt["after"]
        item = entry["item"]
        bsource, asource = _source(before["farmer"]), _source(after["farmer"])
        expected = dict(bsource["inventory"])
        expected.pop(item["uid"], None)
        native = receipt["native_receipt"]
        conservation = receipt["conservation"]
        flags = (
            "inventory_unchanged_except_uid",
            "silver_unchanged",
            "equipped_ammo_unchanged",
            "warehouse_added_exact_basic",
            "essential_uids_preserved",
        )
        stable = (
            "character",
            "character_uid",
        ...
        if (
            bsource["inventory"].get(item["uid"]) != item
            or asource["inventory"] != expected
            or any(bsource[name] != asource[name] for name in stable)
            or bsource["map_id"] != 1036
            or bsource["trade"] is not None
            or bsource["request"] is not None
            ...
            raise ValueError(
                "Controlled deposit receipt lacks exact conservation proof"
            )
...
def load_plan(plan_id, operation_id, uid, *, path=None, current_visit=None):
    ...
    if (
        set(plan) != required
        or plan["version"] != 1
        or plan["kind"] != "controlled_protected_withdrawal_plan"
    ):
    ...
    _identifier(plan["deposit_operation_id"], "deposit operation ID")
    _number(plan["created_at"], "plan creation")
    farmer = plan["farmer"]
    if (
        not isinstance(farmer, dict)
        or set(farmer)
        != {"character", "character_uid", "server", "process_identity", "client_sha256"}
        or not isinstance(farmer["character"], str)
        or not farmer["character"]
        or type(farmer["character_uid"]) is not int
        or farmer["character_uid"] <= 0
        or farmer["server"] != "America"
        or not isinstance(farmer["process_identity"], dict)
        or not isinstance(farmer["client_sha256"], str)
        or len(farmer["client_sha256"]) != 64
    ):
        raise ValueError("Protected withdrawal farmer identity is invalid")
    if not isinstance(plan["items"], list) or len(plan["items"]) != 2:
        raise ValueError(
            "Protected withdrawal plan must contain exactly two reviewed items"
        )
    seen_uids = set()
    seen_ops = set()
    for sequence, entry in enumerate(plan["items"], 1):
        ...
        entry["item"] = _selected_core(entry["item"])
        op = _identifier(entry["withdrawal_operation_id"], "withdrawal operation ID")
        if entry["item"]["uid"] in seen_uids or op in seen_ops:
            raise ValueError("Protected withdrawal plan repeats an item or operation")
        seen_uids.add(entry["item"]["uid"])
        seen_ops.add(op)
    selected = next(
        (entry for entry in plan["items"] if entry["item"]["uid"] == uid), None
    )
    if selected is None or selected["withdrawal_operation_id"] != _identifier(
        operation_id, "withdrawal operation ID"
    ):
        raise ValueError("Withdrawal request is not fixed by the trusted plan")
    departure = plan["market_departure"]
    reentry = plan["market_reentry"]
    if (
        not isinstance(departure, dict)
        or set(departure) != {"market_visit", "departed_observation"}
        or not isinstance(reentry, dict)
        or set(reentry) != {"observed_at", "snapshot", "equipped_ammo", "warehouse"}
    ):
        raise ValueError("Protected withdrawal transit evidence is incomplete")
    visit = departure["market_visit"]
    departed = departure["departed_observation"]
    if (
        not isinstance(visit, dict)
        or visit.get("phase") != "departed"
        or type(visit.get("departed_at")) not in (int, float)
        or visit.get("arrival_map") == 1036
        or not isinstance(departed, dict)
        or departed.get("map_id") == 1036
        or departed.get("identity") != farmer["process_identity"]
        or departed.get("character_uid") != farmer["character_uid"]
        or _number(departed.get("observed_at"), "departure observation")
        < visit["departed_at"]
    ):
    ...
    if (
        rs["map_id"] != 1036
        or rs["identity"] != farmer["process_identity"]
        or rs["character"] != farmer["character"]
        or rs["character_uid"] != farmer["character_uid"]
        or rs["server"] != "America"
        or rs["hp"] <= 0
        or rs["trade"] is not None
        or rs["request"] is not None
    ...
    _ammo(reentry["equipped_ammo"])
    reentry_stash = _core_map(reentry["warehouse"].get("items", ()))
    if type(reentry["warehouse"].get("capacity")) is not int:
        raise ValueError("Protected withdrawal reentry warehouse is invalid")
    if any(
        reentry_stash.get(entry["item"]["uid"]) != entry["item"]
        for entry in plan["items"]
    ):
        raise ValueError(
            "Protected withdrawal items are not exact in the reentry warehouse"
        )
...
class ProtectedWithdrawalJournal:
    def __init__(self, path=None, *, clock=time.time):
        self.path = Path(path or JOURNAL)
        self.clock = clock
    ...
    def for_plan(self, plan_id):
        with self.db() as db:
            rows = db.execute(
                "SELECT state FROM operations WHERE plan_id=? ORDER BY updated_at,operation_id",
                (plan_id,),
            ).fetchall()
    ...
    def begin(self, state):
        ...
        with self.db() as db:
            ...
            if row:
                ...
                if (previous["plan_id"], previous["uid"]) != (
                    state["plan_id"],
                    state["uid"],
                ):
                ...
                return previous
            active = db.execute(
                "SELECT operation_id FROM operations WHERE phase IN ('prepared','input_maybe_sent','reconciling','blocked')"
            ).fetchone()
            if active:
                raise ValueError("Another protected withdrawal needs reconciliation")
            db.execute(
            ...
            db.execute(
                "INSERT INTO history(operation_id,phase,payload,timestamp) VALUES(?,?,?,?)",
                (
                    state["operation_id"],
                    state["phase"],
                    json.dumps(
                        {
                            "intent": state["intent"],
                            "intent_digest": _digest(state["intent"]),
                        },
                        sort_keys=True,
                    ),
                    now,
    ...
    def transition(self, operation_id, expected, phase, **changes):
        legal = {
            "prepared": {"input_maybe_sent", "no_transfer", "blocked"},
            "input_maybe_sent": {"reconciling", "withdrawn", "blocked"},
            "reconciling": {"withdrawn", "blocked"},
            "blocked": {"withdrawn", "blocked"},
        }
        ...
        with self.db() as db:
            ...
            if not row:
                raise ValueError("Protected withdrawal operation is missing")
            state = json.loads(row["state"])
            current = state["phase"]
            if current == phase and current in TERMINAL:
                return state
            if current not in expected or phase not in legal.get(current, set()):
                raise ValueError(
                    f"Illegal protected withdrawal transition {current} to {phase}"
                )
            state.update(changes, phase=phase, updated_at=now)
            db.execute(
                "UPDATE operations SET phase=?,state=?,updated_at=? WHERE operation_id=? AND phase=?",
        ...
        *,
        ...
        with self.db() as db:
            ...
            if state["phase"] in TERMINAL:
                raise ValueError(
                    "A completed protected withdrawal cannot be overridden"
                )
            ...
            db.execute(
                ...
                (
                    ...
                    state["updated_at"],
                    operation_id,
                    override["original_phase"],
                ),
            )
            if db.execute("SELECT changes()").fetchone()[0] != 1:
                raise ValueError(
                    "Protected withdrawal changed during operator override"
                )
            db.execute(
                "INSERT INTO history(operation_id,phase,payload,timestamp) VALUES(?,?,?,?)",
                (
                    operation_id,
                    "operator_overridden",
                    json.dumps({"operator_override": override}, sort_keys=True),
                    state["updated_at"],
                ),
            )
            return state
        ...
        with self.db() as db:
            return [
                ...
                for row in db.execute(
                    "SELECT id,operation_id,phase,payload,timestamp FROM history WHERE operation_id=? ORDER BY id",
                    (operation_id,),
                )
            ]
...
def _plain_observation(value):
    return {
        "source": value["source"],
        "equipped_ammo": value["equipped_ammo"],
...
def _same_ownership(one, two):
    ...
    return (
        a == b
        and _ammo(one["equipped_ammo"]) == _ammo(two["equipped_ammo"])
...
def _withdrawn(before, after, item):
    ...
    expected_bag = dict(a["inventory"])
    expected_bag[item["uid"]] = item
    old_stash = _core_map(before["warehouse"]["items"])
    new_stash = _core_map(after["warehouse"]["items"])
    expected_stash = dict(old_stash)
    expected_stash.pop(item["uid"], None)
    stable = (
        "character",
        "character_uid",
        "identity",
    ...
    return (
        all(a[name] == b[name] for name in stable)
        and b["inventory"] == expected_bag
        and new_stash == expected_stash
        and _ammo(before["equipped_ammo"]) == _ammo(after["equipped_ammo"])
        and before["warehouse"]["capacity"] == after["warehouse"]["capacity"]
        and b["trade"] is None
        and b["request"] is None
    )


class ProtectedWithdrawal:
    def __init__(
        self, town, *, plan_path=None, journal=None, clock=time.time, sleep=time.sleep
    ):
        self.town = town
        self.plan_path = plan_path
        self.journal = journal or ProtectedWithdrawalJournal(clock=clock)
    ...
    def _observe(self):
        ...
        def sample():
            ...
            basic = {
                item.uid: (
                    item.type_id,
                    item.plus,
                    1 if 100000 <= item.type_id < 600000 else item.amount,
                )
                for item in bag.items
            }
            rich = {
                item["uid"]: (item["type_id"], item["plus"], item["quantity"])
                for item in source["inventory"]
            }
            if basic != rich:
                raise ValueError(
                    "Rich farmer inventory disagrees with the verified inventory deque"
                )
            warehouse = reader.read(rich_item=memory.item)
            npc = self.town.vendor(0, stable_identity_only=True)
            grid = reader.gui.read("Warehouse/ScrollingRegion_")
            if (
                grid.size != (272.0, 326.0)
                or grid.scroll != (0.0, 0.0)
                or len(warehouse.items) > warehouse.capacity
            ):
                raise ValueError(
                    "Protected warehouse grid differs from its qualified layout"
                )
            return {
                "source": source,
                "equipped_ammo": None
                if bag.equipped_ammo is None
                else asdict(bag.equipped_ammo),
                "warehouse": {
                    "items": [asdict(item) for item in warehouse.items],
        ...
        after = sample()
        if (
            not _same_ownership(before, after)
            or _npc_identity(before["npc"]) != _npc_identity(after["npc"])
            or before["grid"] != after["grid"]
        ):
            raise ValueError(
                "Farmer or warehouse changed across the protected observation"
            )
        return after
    ...
    def _plan(self, plan_id, operation_id, uid):
        ...
        return load_plan(
            plan_id, operation_id, uid, path=self.plan_path, current_visit=current
        )

    def _expected_preflight(self, plan, selected, current):
        source = _source(current["source"])
        planned = _source(plan["market_reentry"]["snapshot"])
        if (
            self.town.observer.character != plan["farmer"]["character"]
            or self.town.observer.adapter.identity != plan["farmer"]["process_identity"]
            or self.town.observer.adapter.expected_sha256
            != plan["farmer"]["client_sha256"]
            or source["identity"] != plan["farmer"]["process_identity"]
            or source["character_uid"] != plan["farmer"]["character_uid"]
            or source["map_id"] != 1036
            or source["server"] != "America"
            or source["hp"] <= 0
            or source["trade"] is not None
            or source["request"] is not None
            or source["booth"]
        ):
            raise ValueError(
                "Protected withdrawal requires the exact idle farmer in Market"
            )
        earlier = []
        for entry in plan["items"]:
        ...
        for row in earlier:
            prior = states.get(row["item"]["uid"], {})
            intent = prior.get("intent", {})
            if (
                prior.get("phase") != "withdrawn"
        ...
        if (
            any(source[name] != planned[name] for name in planned_stable)
            or source["inventory"] != expected_bag
            or source["silver"] != planned["silver"]
            or _ammo(current["equipped_ammo"])
            != _ammo(plan["market_reentry"]["equipped_ammo"])
            or current_warehouse != expected_warehouse
            or current["warehouse"]["capacity"]
            != plan["market_reentry"]["warehouse"]["capacity"]
        ):
            raise ValueError(
                "Farmer or warehouse ownership changed after verified Market reentry"
            )
        rich = _core_map(current["warehouse"]["items"])
        if rich.get(selected["item"]["uid"]) != selected["item"]:
            raise ValueError(
                "Stored protected item does not match its exact deposit fingerprint"
            )
    ...
    def _point(observation, item):
        stored = next(
            (
                row
                for row in observation["warehouse"]["items"]
                if row["uid"] == item["uid"]
            ),
            None,
        )
        ...
        if stored is None or not 0 <= stored["slot"] < 48:
            raise ValueError(
                "Protected item is outside the qualified visible warehouse grid"
            )
        point = (
            round(grid["position"][0] + 20 + 40 * (stored["slot"] % 6)),
            round(grid["position"][1] + 20 + 40 * (stored["slot"] // 6)),
        )
        if not (
            grid["position"][0] < point[0] < grid["position"][0] + grid["size"][0]
    ...
    def reconcile(self, state, plan, selected, *, observation=None, failure=None):
        ...
        try:
            current = observation or self._observe()
            ...
            if state["phase"] != "blocked":
                ...
                state = self.journal.transition(
                    state["operation_id"], state["phase"], "blocked", **changes
                )
            return self._result(state)
        before = state["intent"]["before"]
        item = selected["item"]
        if _withdrawn(before, current, item):
            state = self.journal.transition(
                ...
                receipt={
                    ...
                    "after": _plain_observation(current),
                    "after_digest": _digest(_plain_observation(current)),
            ...
            receipt = {
                ...
                "item": item,
                "before_digest": _digest(before),
                "observed_digest": _digest(_plain_observation(current)),
                "verified_at": self.clock(),
            ...
            state = self.journal.transition(
                state["operation_id"], "prepared", "no_transfer", **changes
            )
    ...
    def _result(state):
        ...
        return {
            ...
            "uid": state["uid"],
            "phase": phase,
            "outcome": phase if phase in TERMINAL else "unresolved",
            "input_attempted": phase not in ("prepared", "no_transfer"),
            "receipt": state.get("receipt"),
            "reason": state.get("error"),
            "next_action": "continue_controlled_qualification"
            if phase == "withdrawn"
            else (
                "retry_with_new_reviewed_plan"
                if phase == "no_transfer"
                else "reconcile_read_only"
            ),
        }
    ...
    def _run(self, plan_id, operation_id, uid):
        ...
        if old:
            ...
            if (
                ...
                raise ValueError(
                    "Protected withdrawal plan changed after operation admission"
                )
            return self.reconcile(old, plan, selected)
        check = getattr(self.town, "check_input", None)
        if not callable(check):
            raise ValueError(
                "Protected withdrawal requires the authenticated town input guard"
            )
        check()
        observation = self._observe()
        self._expected_preflight(plan, selected, observation)
        ...
        now = self.clock()
        deadline = now + MAX_INPUT_SECONDS
        state = {
            "version": 1,
        ...
        def input_guard(*, hover=False):
            if self.clock() >= deadline:
                raise ValueError("Protected withdrawal input deadline expired")
            check()
            layout.assert_current(layout_revision)
            fresh = self._observe()
            if (
                not _same_ownership(observation, fresh)
                or _npc_identity(fresh["npc"]) != _npc_identity(observation["npc"])
                or fresh["grid"] != observation["grid"]
                or self.town.warehouse_native_point(
                    self._point(fresh, selected["item"]), layout_revision
                )
                != point
            ):
                raise ValueError(
                    "Protected warehouse layout, slot or ownership changed before input"
                )
            if hover or state["phase"] == "input_maybe_sent":
                ...
                if (
                    unpack(gui.session, context + 0x3EC0, "<Q")[0]
                    != fresh["grid"]["address"]
                ):
                    raise HoverNotReady(
                        "Protected warehouse item is covered by another window"
                    )
        ...
        def before_press():
            ...
            wait_hover_validation(
                lambda: input_guard(hover=True), check or (lambda: None)
            )
            nonlocal state
            state = self.journal.transition(
                operation_id,
        ...
        try:
            ...
            foreground_click(
                self.town.observer.operations.target,
                *point,
                tuple(layout_revision.client_size),
                require_foreground=True,
                before_press=before_press,
                layout_guard=input_guard,
            )
            state = self.journal.transition(
                operation_id,
                "input_maybe_sent",
