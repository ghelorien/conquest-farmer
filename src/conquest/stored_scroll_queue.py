...
def database(journal):
    ...
    db = sqlite3.connect(destination, timeout=5)
    db.row_factory = sqlite3.Row
    try:
        db.execute("PRAGMA synchronous=FULL")
        db.execute(
            "CREATE TABLE IF NOT EXISTS scrolls(uid INTEGER PRIMARY KEY, stored_at REAL NOT NULL, "
...
def explicit_disposition(state, uid):
    for key in ("user_confirmed_scroll_consumption", "user_confirmed_scroll_transfer"):
        ...
        if (
            ...
            or key.endswith("transfer")
            and (
                record.get("type_id") != 720027
...
def capture(journal, state):
    uid = state.get("scroll_uid")
    stamp = state.get("market_verified_at")
    if (
        state.get("phase") != "completed"
        or state.get("exchange_verified") is not True
    ...
    receipts = [
        ...
        if r.get("type_id") == 720027
        and r.get("verified_in_warehouse") is True
        and r.get("stored", r.get("uid")) == uid
    ]
    if len(receipts) != 1:
        return False
    ...
    checksum = digest(evidence)
    deferred = state.get("delivery_deferred") or {}
    deferred_at = deferred.get("at") if deferred.get("uid") == uid else None
    if deferred_at is not None and (
        type(deferred_at) not in (int, float) or not math.isfinite(deferred_at)
    ):
    ...
    with database(journal) as db:
        ...
        if old and old["digest"] != checksum:
            raise ValueError("Stored scroll provenance changed")
        if not old:
            db.execute(
                "INSERT INTO scrolls VALUES(?,?,?,?,?,?,?)",
                (uid, stamp, encoded, checksum, "pending", deferred_at, None),
            )
        if disposition:
            db.execute(
                "UPDATE scrolls SET phase='disposed',disposition=? WHERE uid=? AND phase='pending'",
...
def pending(journal):
    ...
    with database(journal) as db:
        rows = [
            dict(r)
            for r in db.execute(
                "SELECT * FROM scrolls WHERE phase='pending' ORDER BY stored_at,uid"
            )
        ]
    for row in rows:
        row["evidence"] = json.loads(row["evidence"])
        if (
            digest(row["evidence"]) != row["digest"]
            or row["uid"] != row["evidence"].get("scroll_uid")
...
def complete(journal, uid, receipt):
    with database(journal) as db:
        db.execute(
            "UPDATE scrolls SET phase='delivered',disposition=? WHERE uid=? AND phase='pending'",
...
def defer(journal, uid, now):
    with database(journal) as db:
        ...
        if not row or row["phase"] != "pending":
            raise ValueError("Deferred scroll has no outstanding storage receipt")
