...
def close_one(trade, check=lambda: None):
    try:
        return _close_one(trade, check)
    except GuiObservationChanged as error:
        raise CaptureUnavailable(
            "GUI observation changed; reobserve panels before movement"
        ) from error
...
def _close_one(trade, check):
    ...
    if cancel(trade, check):
        return "Open Booth###Confirm"
    gui = GuiReader.for_session(trade.observer.adapter)
    windows = gui.windows()
    if any(transaction(w) for w in windows):
        raise CaptureUnavailable(
            "A transaction dialog needs reconciliation before movement"
        )
    chosen = next(
        (name for name in PANELS if any(w["name"] == name for w in windows)), None
    )
    if chosen is None:
        return None
    if chosen != "Booth":
    ...
    while time.monotonic() < until:
        check()
        if not any(w["name"] == "Booth" for w in gui.windows()):
            return "Booth"
        time.sleep(0.05)
