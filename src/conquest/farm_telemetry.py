...
try:
    definitions = json.loads(
        Path(
            installation_path(r"C:\Program Files\Classic Conquer 2.0\ini\itemtype.json")
...
def item_label(fields):
    ...
    if 100000 <= kind < 600000:
        ...
        if type(plus) is int and plus > 0:
            name += f" +{plus}"
    return name
...
class PickupHistory:
        ...
        if self.path.exists():
            with self.path.open(encoding="utf-8") as source:
                for line in source:
                    try:
                        row = json.loads(line)
                        if row["increase"] > 0 and isinstance(
                            row["timestamp"], (int, float)
                        ):
                            self.rows.append(row)
                            if row.get("inventory_uid"):
                                self.inventory_ids.add(
                                    (row["inventory_uid"], row["type_id"])
                                )
    ...
    def add(self, fields):
        ...
        if key[0] and key in self.inventory_ids:
            return None
...
def pickup_values(row):
    return (
        ...
        f"+{row['increase']}" if row.get("silver") else f"×{row['increase']}",
    )
...
def previous_route_failure(route, app):
    ...
    return (
        route.get("phase") == "needs_attention"
...
def activity_text(route, app, control, life, *, now=None):
    ...
    if life and life.get("dead_candidate"):
        ...
        if not control.get("enabled"):
            ...
            if route.get("phase") == "needs_attention" and not previous_route_failure(
                route, app
            ):
    ...
    if app.get("state") == "Reconnecting":
        ...
        if status == "waiting_for_login_input":
            return "Reconnect waiting for client focus or mouse release"
    ...
    if (
        ...
        and not previous_route_failure(route, app)
    ):
    ...
    if phase in ("restocking", "changing_route", "recovering_route", "visiting_town"):
        ...
        if (
            watch.get("result") == "travelling"
            and 0 <= now - watch.get("updated_at", 0) < 4
        ):
    ...
    state = app.get("state")
    return {
        "Running": "Hunting selected monsters",
        "Farming": "Attacking selected monsters",
        "Patrolling": "Patrolling for monsters",
        "Travelling to hunting area": "Heading back to the hunting area",
        "Paused â€” waiting for Conquer": "Waiting for Conquer to regain focus",
    }.get(state, state or "Hunting")
...
def automation_status(route, app, control, life, *, now=None):
    ...
    if (
        type(manual_revision) is int
        and manual_revision == control.get("revision")
        and not control.get("enabled")
    ):
    ...
    current_work = (
        work.get("revision") == control.get("revision")
    ...
    if current_work and work.get("state") in ("running", "attention"):
        return (
            "Running" if work["state"] == "running" else "Stopped · needs attention",
            work["activity"],
        )
    if app.get("state") == "Reconnect needs attention":
        return "Stopped · needs attention", activity_text(
            route, app, control, life, now=now
        )
    if app.get("state") == "Reconnecting":
    ...
    if route_work and not fresh:
        return (
            ...
            "No recent route update; current movement is not confirmed",
        )
    ...
    if control.get("execution_state") not in (None, "farming", "off") or app.get(
        "navigation_blocked"
    ):
...
def farm_stats(app, enabled, *, now=None):
    ...
    if disconnected:
        ...
        current = xp.get("experience_candidate")
        required = xp.get("experience_required")
        speed = xp.get("xp_per_hour_candidate")
        if (
            all(
                isinstance(v, (int, float)) and math.isfinite(v)
                for v in (current, required, speed)
            )
            and 0 <= current < required
            and speed > 0
        ):
