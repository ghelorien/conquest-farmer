...
@contextmanager
def _lock(path):
    lock = Path(str(path) + ".override.lock")
...
def _complete(path, intent):
    ...
    digest = record["original_evidence_digest"]
    if state.get("operator_override") == record:
        return state
    if evidence_digest(state) != digest:
        raise ValueError(
            "Incident changed during override recovery; recheck before overriding"
        )
    ...
    if audit.exists():
        for line in audit.read_text(encoding="utf-8").splitlines():
            try:
                ...
                continue
    if not any(
        row.get("original_evidence_digest") == digest
        and row.get("confirmation_reference") == record["confirmation_reference"]
        for row in existing
    ):
    ...
    state.update(
        phase=intent["terminal_phase"], operator_override=record, replan_required=True
    )
...
def read_recovered(path):
    ...
    with _lock(path):
        ...
        record = intent["record"]
        if (
            state.get("operator_override") != record
            and evidence_digest(state) != record["original_evidence_digest"]
        ):
            # A different incident now occupies the journal. Preserve the old
            # decision for audit, but allow a new explicit confirmation.
            superseded = Path(
                str(intent_path) + "." + evidence_digest(intent) + ".superseded"
            )
...
def operator_override(
    ...
    *,
    ...
    audit_path=None,
    incident=None,
    terminal_phase="operator_overridden",
    ...
    with _lock(path):
        ...
        record = {
            "incident": incident or path.stem,
