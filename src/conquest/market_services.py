...
def discover(entities, map_id, name, *, stable_identity_only=False):
    ...
    if name not in MODELS:
        raise ValueError("Unsupported Market service")
    ...
    bounds = sample_fields(s, headers)
    begin, end, capacity = bounds
    if not (
        begin <= end <= capacity
        and (end - begin) % p.entry_stride == 0
    ...
    entries = [
        (a + p.entry_object_offset, "u64") for a in range(begin, end, p.entry_stride)
    ]
    ...
    for a, model in zip(actors, models):
        ...
        fields = [
            (a + 0x7C, "u32"),
            (a + p.name_offset, "utf8"),
            (a + p.position_offset, "xy_u32"),
            (a + p.id_offset, "u32"),
            (a + p.kind_offset, "u32"),
            (a + p.draw_position_offset, "i32"),
            (a + p.draw_position_offset + 4, "i32"),
        ]
        ...
        if (fresh_values[:5] if stable_identity_only else fresh_values) != (
            values[:5] if stable_identity_only else values
        ):
        ...
        if found == name:
            ...
            matches.append(
                (
                    VendorIdentity(map_id, kind, found, model, tuple(position)),
                    NpcObservation(
                        a, uid, kind, found, map_id, tuple(position), (dx, dy)
                    ),
                )
            )
    if len(matches) != 1:
        raise ValueError("One memory-identified " + name + " is required in the scene")
    ...
    fresh = sample_fields(s, headers)
    fb, fe, fc = fresh
    if not (
        fb <= fe <= fc
        and (fe - fb) % p.entry_stride == 0
        and (fc - fb) % p.entry_stride == 0
        and (fc - fb) // p.entry_stride <= p.max_objects
    ):
        raise ValueError("Service scene bounds changed")
    members = sample_fields(
        s, [(v + p.entry_object_offset, "u64") for v in range(fb, fe, p.entry_stride)]
    )
    identity, npc = matches[0]
    a = npc.object_address
    fields = [
        (a, "u64"),
        (a + 0x84, "u32"),
        (a + 0x7C, "u32"),
    ...
    if time.monotonic() - started > 1:
        raise ValueError("Service observation expired")
    return matches[0]
...
def dialog_point(observer, option, expected_records):
    d = read_dialog(observer)
    if d["records"] != expected_records:
        raise ValueError("Service dialog changed before selection")
    from conquest.viewport import size_for
...
def execute(trade, body):
    ...
    if action == "service-close-panel" and set(body) == {"action", "window"}:
        if body["window"] not in ("Inventory", "Dialog"):
            raise ValueError("Unsupported service panel")
        ...
        try:
            ...
            raise
        if gui.read(body["window"]) != window:
            raise ValueError("Service panel moved")
        trade.input_attempted = True
        trade.click(
            (
                round(window.position[0] + window.size[0] - 18),
                round(window.position[1] + 18),
            )
        )
        deadline = time.monotonic() + 2
        while time.monotonic() < deadline:
            try:
                gui.read(body["window"])
            except ValueError as error:
    ...
    expected = {"action", "name"} | (
        {"option", "records"}
        if action in ("service-select", "service-scroll-dialog")
        else set()
    )
    if set(body) != expected:
        raise ValueError("Unsupported service arguments")
    life = trade.life(any_map=True)
    identity, npc = discover(trade.observer.entities, life.map_id, body["name"])
    if action == "service-locate":
        return {"identity": asdict(identity), "npc": asdict(npc)}
    if max(abs(a - b) for a, b in zip(life.position, npc.position)) > 18:
        raise ValueError("Travel closer to service NPC")
    if action == "service-open":
        from conquest.memory_shop import MemoryGui

        try:
            MemoryGui.for_session(trade.observer.adapter).read("Dialog")
        except ValueError as error:
            if not any(t in str(error) for t in ("not active", "absent")):
                raise
        else:
            execute(trade, {"action": "service-close-panel", "window": "Dialog"})
        ...
        if npc != getattr(trade, "service_npc", None):
            raise ValueError("Open this service before scrolling its dialog")
        data = read_dialog(trade.observer)
        w = data["window"]
        x, y = w.position
        ...
        point = (
            round(x + w.size[0] / 2),
            round((y + 24 + min(y + w.size[1] - 4, viewport[1] - 4)) / 2),
        )
        if not (
            0 < point[0] < viewport[0]
            and y + 20 < point[1] < min(y + w.size[1], viewport[1])
        ):
            raise ValueError("Dialog scroll target is outside the client")
        if read_dialog(trade.observer) != data:
            raise ValueError("Dialog changed before scrolling")
    else:
        if npc != getattr(trade, "service_npc", None):
            raise ValueError("Open this service before selecting its dialog")
    ...
    if (
        ...
        and dialog_point(trade.observer, body["option"], body["records"]) != point
    ):
