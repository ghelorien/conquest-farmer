...
def warehouse_identity(entities, map_id):
    ...
    known = next(
        (v for v in TOWN_VENDORS if v.map_id == map_id and v.name == "Warehouseman"),
        None,
    )
    if known:
        return known
    ...
    if len(candidates) > 8:
        raise ValueError("Ambiguous warehouse model population")
    matches = []
    for a in candidates:
        fields = [
            (a + 0x7C, "u32"),
        ...
        kind, name, point = values
        if sample_fields(s, fields) != values:
            raise ValueError("Warehouse NPC changed during observation")
        if (
            name == "Warehouseman"
    ...
    if len(matches) != 1:
        raise ValueError(
            "One memory-identified Warehouseman is required in the town scene"
        )
...
def interaction_point(npc):
    ...
    market_bank = (
        npc.map_id == 1036
        and npc.type_id == 0
        and npc.name == "Warehouseman"
        and tuple(npc.position) == (182, 180)
    )
...
class MemoryNpcReader:
    ...
    def read(self, map_id):
        ...
        started = self.clock()
        known = {
            kind: v for (world, kind), v in self.vendors.items() if world == map_id
        }
        if not known:
        ...
        entries = [
            (a + p.entry_object_offset, "u64")
            for a in range(begin, end, p.entry_stride)
        ]
        objects = sample_fields(s, entries)
        ...
        actors = [
            ...
            if vt == base + p.monster_vtable_rva
        ]
        ...
        zero_actors = [
            obj for obj, kind in zip(actors, types) if kind == 0 and 0 in known
        ]
        zero_models = dict(
            zip(
                zero_actors,
                sample_fields(s, [(obj + 0x84, "u32") for obj in zero_actors]),
            )
        )
        candidates = [
            obj
            for obj, kind in zip(actors, types)
            if kind in known and (kind != 0 or zero_models[obj] == known[0].model)
        ]
        ...
        specs = (
            ...
            (0x84, "u32"),
            (p.name_offset, "utf8"),
            (p.position_offset, "xy_u32"),
            (p.draw_position_offset, "i32"),
        ...
        if (
            sample_fields(s, entries) != objects
            or sample_fields(s, vtable_fields) != vtables
            or sample_fields(s, id_fields) != ids
            or sample_fields(s, type_fields) != types
            or sample_fields(s, headers) != header
            or sample_fields(s, [(a, "u64") for a, _ in trace]) != [v for _, v in trace]
            or sample_fields(s, [(collection, "u64")])
            != [base + p.collection_vtable_rva]
        ):
            raise ValueError("NPC scene changed during observation")
        result = []
        for index, obj in enumerate(candidates):
            uid, kind, species, model, name, position, draw_x, draw_y = values[
                index * 8 : index * 8 + 8
            ]
            ...
            if (
                ...
                raise ValueError(
                    "Vendor identity or position differs from its memory profile"
                )
            result.append(
                NpcObservation(obj, uid, kind, name, map_id, position, drawing)
            )
        if len({v.entity_id for v in result}) != len(result):
            raise ValueError("Duplicate vendor IDs")
        s.assert_identity()
        finished = self.clock()
        if not 0 <= finished - started <= 0.5:
    ...
    def require(self, map_id, entity_id):
        ...
        if len(matches) != 1:
            raise ValueError(
                "Selected vendor is not present in the current memory scene"
            )
        return matches[0]


def vendor_identity(map_id, role):
    kind = {1011: {3: 10014, 5: 10013, 4: 11}, 1020: {3: 10030, 5: 10029}}.get(
        map_id, {}
    ).get(role, role)
    matches = [v for v in TOWN_VENDORS if v.map_id == map_id and v.type_id == kind]
    if len(matches) != 1:
        raise ValueError("Vendor role has not been verified in this city")
    return matches[0]
