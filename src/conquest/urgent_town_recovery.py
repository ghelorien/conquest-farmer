...
def _rows(path):
    if not path.is_file():
        raise ValueError("Required recovery audit is unavailable")
...
def _terminal_meteor(visit, events):
    ...
    failures = [
        ...
        and started <= row["time"] < completed
        and row.get("town_visit_id") == visit["town_visit_id"]
        and any(
            Path(frame.get("file", "")).name == "meteor_banking.py"
    ...
    receipts = {
        row.get("stored"): row
        for row in meteor.get("receipts", [])
        if row.get("verified_in_warehouse") is True
    }
    scroll = meteor.get("scroll_uid")
    if (
        type(scroll) is not int
        or scroll <= 0
        or receipts.get(scroll, {}).get("type_id") != SCROLL
...
def claim_legacy(visit, *, info_path, visit_id, original_target, reference):
    ...
    starts = [
        event
        for event in events
        if event.get("event") == "urgent_banking_started"
    ...
    if any(
        event.get("event") == "urgent_banking_complete"
        and event.get("town_visit_id") == visit_id
        for event in events
    ):
    ...
    if any(
        type(receipt.get("time")) not in (int, float)
        or receipt["time"] >= row["required_at"]
        for receipt in _rows(MONEY)
    ):
        raise ValueError("Post-visit or unreadable money transfer needs reconciliation")
    intent = [
        {"uid": uid, "type_id": receipts[uid]["type_id"], "storage_map_id": 1036}
        for uid in uids
    ]
    return visit.claim_urgent_recovery(
        visit_id=visit_id,
        target=original_target,
        intent=intent,
        evidence={
            "reference": reference,
            "urgent_start_at": starts[0]["time"],
            "meteor_started_at": meteor["started_at"],
            "meteor_completed_at": meteor["completed_at"],
            "interrupted_at": failure["time"],
...
def resume_claimed(loop):
    ...
    row = visit.state()
    claim = row.get("urgent_recovery_claim")
    if not claim:
        return False
    ...
    if completed is not None:
        if (
            ...
            or completed <= 0
            or row.get("town_work_completed_kind") != "urgent_banking"
    ...
    if row.get("urgent_recovery_attempted_at"):
        raise ValueError(
            "Urgent recovery tail was already attempted; reconcile before any retry"
        )
    health = loop.health()
    ...
    if (
        ...
        raise ValueError(
            "Urgent recovery process, town, safety, or transaction hold changed"
        )
    ...
    if (
        ...
        or any(
            item["uid"] in {wanted["uid"] for wanted in row["urgent_intent"]}
    ...
    if (
        meteor.get("phase") != "completed"
        or meteor.get("completed_at") != claim["meteor_completed_at"]
    ...
    if (
        any(
            (
                phoenix.get(wanted["uid"]) != wanted["type_id"]
                if wanted.get("storage_map_id", 1011) == 1011
                else wanted["uid"] in phoenix
            )
            for wanted in row["urgent_intent"]
        )
        or claim["scroll_uid"] in phoenix
        or sum(
            item["type_id"] == METEOR and item["amount"] == item["limit"] == 1
            for item in fresh_pre["items"] + stored["items"]
        )
        >= 10
    ):
        raise ValueError(
            "Fresh warehouse ownership or new Meteor batch differs; no tail submitted"
        )
    ...
    open_warehouse(loop)
    fresh_bag = loop.town("supplies")
    fresh_bank = loop.town("warehouse-items")
    if (
        urgent_valuables(fresh_bag["items"])
    ...
    if (
        any(
            (
                banked.get(item["uid"]) != item["type_id"]
                if item.get("storage_map_id", 1011) == 1011
                else item["uid"] in banked
            )
            for item in row["urgent_intent"]
        )
        or claim["scroll_uid"] in banked
    ):
    ...
    open_warehouse(loop)
    final = loop.town("supplies")
    final_bank = loop.town("warehouse-items")
    if (
        urgent_valuables(final["items"])
        or transaction_holds()
        or receipt_for(claim["scroll_uid"], 720027)
        or not visit.reconcile_urgent_town_work(
            target=target,
            bag=final,
            warehouse=final_bank,
            meteor=read_json(JOURNAL),
