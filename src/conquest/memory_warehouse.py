...
class MemoryWarehouseReader:
    ...
    def _warehouse_model(self, block):
        ...
        if not 1 <= count <= 128:
            raise ValueError("Warehouse GUI registry is invalid")
        node = struct.unpack("<Q", block(head + 8, 8))[0]
        seen = set()
        while node != head:
            if node in seen or len(seen) >= count:
                raise ValueError("Warehouse GUI registry changed")
            seen.add(node)
            raw = block(node, 56)
            key = struct.unpack_from("<I", raw, 32)[0]
            if key == layout.warehouse_model_key:
                model = struct.unpack_from("<Q", raw, 40)[0]
                current = block(model, 13)
                if (
                    struct.unpack_from("<Q", current)[0]
                    != base + layout.warehouse_model_vtable_rva
                    or not current[12]
                ):
                    raise ValueError("Warehouse model is not active")
                return model
            node = struct.unpack_from(
                "<Q", raw, 0 if layout.warehouse_model_key < key else 16
            )[0]
    ...
    def read(self, *, rich_item=None):
        ...
        table, size, start, count = struct.unpack(
            "<4Q", block(owner + self.layout.warehouse_deque_offset, 32)
        )
        capacity = struct.unpack(
            "<I", block(owner + self.layout.warehouse_capacity_offset, 4)
        )[0]
        bank_silver = struct.unpack(
            "<I", block(owner + self.layout.warehouse_silver_offset, 4)
        )[0]
        if (
            not 1 <= capacity <= 1000
            or not 0 <= bank_silver <= 0x7FFFFFFF
        ...
        for slot in range(count):
            entry = struct.unpack(
                "<Q", block(table + ((start + slot) & (size - 1)) * 8, 8)
            )[0]
            ...
            if (
                vtable != base + self.layout.item_vtable_rva
                or not uid
                or not kind
            ...
            if rich_item is None:
                ...
                if (
                    rich.uid != uid
                    or rich.type_id != kind
                    or rich.plus != plus
                    or rich.slot != slot
                    or rich.quantity != (1 if equipment else amount)
                    or rich.gem1 != rich_raw[0x67]
                    or rich.gem2 != rich_raw[0x68]
                    ...
                    raise ValueError(
                        "Rich warehouse item disagrees with the verified basic record"
                    )
                items.append(rich)
...
def deposit_received(item, before_bag, before_stash, bag, stash):
    ...
    return (
        identity(item) in {identity(i) for i in before_bag.items}
...
def withdrawal_received(item, before_bag, before_stash, bag, stash):
    return deposit_received(item, bag, stash, before_bag, before_stash) and getattr(
        bag, "equipped_ammo", None
    ) == getattr(before_bag, "equipped_ammo", None)
...
class WarehouseMoneyReader:
    ...
    def read(self):
        ...
        if not 1 <= count <= 256:
            raise ValueError("GUI controller registry bounds changed")
        node = struct.unpack("<Q", read(head + 8, 8))[0]
        seen = set()
        for _ in range(32):
            if node in seen:
                raise ValueError("GUI controller registry cycle")
            seen.add(node)
            raw = read(node, 0x38)
            if raw[0x19]:
                raise ValueError("Warehouse controller is absent")
        ...
        if b"\0" not in value:
            raise ValueError("Warehouse amount is unterminated")
        amount = normalized_money_amount(value.split(b"\0")[0].decode("ascii"))
        if (
            len(amount) > 10
            or (amount and not amount.isdecimal())
            or silver > 0x7FFFFFFF
        ):
        ...
        if (
            self.gui.read("Warehouse") != window
            or self.gui.read("Warehouse/ScrollingRegion_") != grid
        ):
...
def money_received(direction, amount, before_bag, before_bank, bag, bank):
    ...
    return (
        bag.items == before_bag.items
        and bag.equipped_ammo == before_bag.equipped_ammo
