...
def fixed_route_windows(kills, *, started_at, now, window_seconds=900, retain=4):
    ...
    for index in range(max(0, completed - retain), completed):
        ...
        windows.append({'start': start, 'end': end, 'seconds': window_seconds,
                        'kills': count, 'kills_per_minute': count * 60 / window_seconds})
    return windows
...
def queue_area(route, *, path=STATE, policy_path=POLICY, now=None):
    ...
    if key in areas:
        return areas[key]
    entry = {
        'area_key': key, 'map_id': route.map_id,
        'monster_type_ids': list(route.monster_type_ids),
        'baseline_route_id': route.id, 'baseline_route': route.model_dump(mode='json'),
        'phase': 'needs_survey', 'queued_at': now,
...
def comparison_plan(candidate_ids, *, policy_path=POLICY):
    ...
    stages = [{'kind': 'sample', 'route_id': rid, 'minutes': sample}
    ...
    if minutes + policy['validation_minutes'] <= policy['maximum_minutes']:
        stages.append({'kind': 'validate_winner', 'minutes': policy['validation_minutes']})
...
def rank_candidates(samples, candidate_ids, *, policy_path=POLICY):
    ...
    for rid in candidates:
        ...
        if any(s.get('deaths', 0) > 0 or s.get('unsafe', False) for s in rows):
            ...
            continue
        valid = [s for s in rows if s.get('memory_verified') is True
                 and s.get('comparable') is True
                 and s['elapsed_seconds'] >= policy['sample_minutes'] * 60
                 and type(s['verified_kills']) is int and s['verified_kills'] >= 0]
        if len(valid) < policy['rounds']:
            pending.append(rid)
            continue
        seconds = sum(s['elapsed_seconds'] for s in valid)
        kills = sum(s['verified_kills'] for s in valid)
        results.append({'route_id': rid, 'eligible': True, 'verified_kills': kills,
                        'elapsed_seconds': seconds, 'kills_per_hour': kills * 3600 / seconds})
    eligible = sorted((r for r in results if r['eligible']), key=lambda r: -r['kills_per_hour'])
    return {'winner': eligible[0]['route_id'] if eligible and not pending else None,
            'pending_candidates': pending, 'ranking': eligible,
