...
@pytest.mark.parametrize("mode,slot,encoded", [(0, 1, 426), (1, 14, 0x03540000),
                                             (2, 0, 213), (3, 8, 0xD5000000)])
...
@pytest.mark.parametrize("mode,count,index", [(4,16,1), (0,0,1), (0,1025,1),
                                             (0,16,16), (0,16,-1)])
...
class Session:
    ...
    def __init__(self):
        ...
        for address, value in [(0x100100, 0x200000), (0x200000, 0x300000),
                               (0x300000, 0x100200), (0x300968, 0x400000)]:
...
@pytest.fixture
def setup_reader():
    player = PlayerLayout(expected_sha256="a" * 64, module="client.exe", root_rva=0x100,
        pointer_offsets=(0,0), vtable_rva=0x200, name_offset=0x94,
        max_hp_offset=0x3d0, position_offset=0xd8)
    layout = HealthLayout(player=player, attribute_pointer_offset=0x968, hp_attribute_index=1)
    session = Session()
...
def test_slow_sample_is_rejected(setup_reader):
    ...
    with pytest.raises(ValueError, match="expired"):
        MemoryHealthReader(session, layout, "Parasite", clock=lambda: next(ticks)).read()
