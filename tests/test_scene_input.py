...
def test_change_before_click_defers_without_input(change):
    ...
    def sleep(_):
        ...
        if change == "position":
            scene.position = (451, 449)
        if change == "viewport":
            scene.window["client_size"] = [800, 600]
...
def test_waypoint_must_stay_inside_explicit_bounds():
    ...
    with pytest.raises(ValueError, match="bounds"):
        scene.stepper(allowed_bounds=(450, 447, 452, 451)).step(
            scene.position, scene.destination
        )
    assert not scene.calls
...
def test_memory_anchor_tracks_map_edge_and_rejects_stale_projection():
    ...
    with pytest.raises(ValueError, match="changed"):
        memory_player_anchor(observer, life)
