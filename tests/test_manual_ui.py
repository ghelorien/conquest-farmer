...
def test_countdown_reaches_zero_without_extending_the_runtime_deadline():
    ...
    assert action_state(row, now=129) == {
        "approve": True,
        "reject": True,
        "override": False,
    }
    assert action_state(row, now=130) == {
        "approve": False,
        "reject": False,
        "override": False,
    }
...
def test_farmer_unsupported_map_blocker_and_fence_are_explicit():
    row = {
        ...
        "fence_scope": "target",
        "ever_approved": False,
        "reason": "Farmer manual memory unavailable: unsupported map",
    }
    farmer = {
        "session": row,
        "input_fenced": True,
...
def test_refresh_renders_detached_farmer_and_merchant_rows_and_buttons():
    ...
    farmer["id"] = "farmer-session"
    farmer["approval_binding"]["session_id"] = "farmer-session"
    ...
    ui.runtime = runtime
    ui.manual_displayed = {}
    ui.manual_texts = {target: Var() for target in ("Farmer", "Spiritual", "Dutch")}
    ui.manual_buttons = {
        target: {action: Button() for action in ("approve", "reject", "override")}
        for target in ("Farmer", "Spiritual", "Dutch")
    }
    statuses = {
        "Spiritual": {"enabled": False, "refill": {"enabled": True}},
    ...
    assert (
        ui.manual_displayed["Farmer"]["approval_binding"]["request_id"]
        == "farmer-request"
    )
    assert (
        ui.manual_displayed["Dutch"]["approval_binding"]["request_id"]
        == "merchant-request"
    )


def test_native_approval_submits_the_exact_displayed_binding_during_request_race(
    monkeypatch,
):
    shown = pending("shown-request")
    current = pending("replacement-request")
    runtime = NS(
        manual_status=lambda target: current,
    ...
    monkeypatch.setattr(
        "conquest.merchants.ui.messagebox.askyesno", lambda *a, **k: True
    )
    monkeypatch.setattr(
        "conquest.merchants.ui.messagebox.showerror",
        lambda *a, **k: pytest.fail("unexpected error"),
    )
...
def test_approval_never_changes_saved_pause_or_enablement(monkeypatch):
    shown = pending()
    state = {"enabled": False, "refill_enabled": True, "farmer_enabled": False}
    ...
    runtime = NS(
        approve_manual=Mock(side_effect=approve),
        enable=Mock(),
        set_refill_enabled=Mock(),
    )
    ui = UnifiedUI.__new__(UnifiedUI)
    ui.root = object()
    ui.runtime = runtime
    ...
    runtime.set_refill_enabled.assert_not_called()
...
def test_bridge_manual_actions_require_and_forward_exact_binding():
    ...
    assert (
        ui.dispatch({"action": "manual-status", "character": "Farmer"})["farmer"][
            "input_fenced"
        ]
        is False
    )
    ...
    runtime.approve_manual.assert_called_once_with(
        shown["approval_binding"], operator="Floor"
    )
...
def test_exact_profile_visitor_revocation_is_separate_from_delivery_trust(tmp_path):
    ...
    with store.db() as db:
        db.execute(
            ...
            (*exact.values(), time.time()),
        )
        db.execute(
            "INSERT INTO visitor_permissions VALUES(?,?,?,?,1,?)",
            (*other.values(), time.time()),
        )
    displayed = [
        {name: row[name] for name in exact}
        for row in registry.list_visitors(profile.id)
    ]

    remaining = registry.revoke_visitors(
        profile.id, [displayed[0]], operator="profile UI"
    )
    ...
    ui.dispatch(
        {
            ...
            "confirmation_reference": "manual-check",
            "operator": "Floor",
            "reason": "Reviewed",
        }
    )

    runtime.override_manual.assert_called_once_with(
        "manual-session",
        confirmation_reference="manual-check",
        operator="Floor",
        reason="Reviewed",
