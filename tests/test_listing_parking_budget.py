...
def test_earned_listing_gets_thirty_seconds_without_retrying_interval(town_service, monkeypatch, safe):
    from conquest.merchants import bridge, handoff
    from conquest import safe_reload
    r=town_service;r.health['embedded_controls']['life']['map_id']=1011
    original=bridge.request;events=[]
    def merchant(body):
        result=original(body)
        if body['action']=='status':
            result['handoff_requested']='merchant-refill:Dutch:123'
            result['characters']['Dutch'].update(refill={'enabled':True},
    ...
    def park(loop,cancelled,notify,**kwargs):
        ...
        cancelled.is_set();r.now[0]+=25 if safe else 30
        kwargs['diagnostic'].update(reached_steps=3,unreached_steps=1)
        if not safe:raise ValueError('No quiet nearby spot verified; reload deferred')
    ...
    row=next(fields['parking'] for event,fields in events if event=='merchant_parking_finished')
...
def test_earned_market_listing_parking_still_uses_original_deadline(town_service, monkeypatch):
    from conquest.merchants import bridge
    from conquest import safe_reload
    r=town_service;r.visit.begin(parent='required-town:1');r.now[0]=1058
...
def test_park_diagnostic_preserves_evidence_without_claiming_safety(monkeypatch, movement_error, available):
    import threading
    ...
    h=health();h['embedded_controls']['monsters']=[{'position':[101,350],'alive':None}]
    def living():
        h['embedded_controls'].update(observed_at=now[0] if available else None,
                                      observations_available=available)
        return h
    monkeypatch.setattr('conquest.scene_input.memory_player_anchor',lambda *a:(518,396))
    monkeypatch.setattr(s,'nearby_escape',lambda *a,**kw:(102,350) if movement_error else None)
    loop=NS(living=living,care=NS(check=lambda h:None,session=None),terrain=NS(map_id=1002),
        stepper=NS(step_to=lambda *a,**kw:{'reached':False,'error':'Route movement stopped progressing'}))
    diagnostic={}
    with pytest.raises(ValueError,match='No quiet nearby spot'):
        s.park(loop,threading.Event(),lambda note:None,seconds=1,allow_town_retreat=False,diagnostic=diagnostic)
    assert diagnostic['nearby_threats']==1 and diagnostic['last_observation_quiet'] is False
