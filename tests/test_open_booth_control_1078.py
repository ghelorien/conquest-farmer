...
def gui(monkeypatch):
    ...
    def read(address, size):
        if address == 1012: return state['visible']
        if address == 1256: return struct.pack('<Q', state['callback'])
        if 2000 <= address < 2000+len(raw): return bytes(raw[address-2000:address-2000+size])
        raise AssertionError((address, size))
    session = SimpleNamespace(read_block=read, assert_identity=lambda: None)
    window = {'name': 'Trade###Confirm', 'address': 2000,
              'geometry': (608., 387., 200., 100.), 'scroll': (0., 0.)}
    result = SimpleNamespace(session=session, windows=lambda: [window],
                             viewport_size=lambda: (1416, 876), state=state, raw=raw)
    monkeypatch.setattr(control, '_model', lambda _: 1000)
    monkeypatch.setattr(control, 'string', lambda s, at, maximum: state['strings'][(at-1000-0x48)//32])
    return result
...
@pytest.mark.parametrize('index,value', [(0, 'Trade###Confirm'), (1, 'Other question'),
                                        (2, 'Accept'), (3, 'Cancel')])
