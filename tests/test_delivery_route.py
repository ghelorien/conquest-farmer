...
def item(uid, kind=130009):
    return dict(
        uid=uid, type_id=kind, plus=0, gem1=0, gem2=0, quantity=1, bound=False, slot=0
    )
...
def admission_clock(monkeypatch):
    ...
    def health():
        ...
        return {
            "target": {"pid": 7, "creation_time_100ns": 80},
            "embedded_controls": data,
        }
...
def test_preadmission_reobserves_transient_read_before_authorizing(
    first, admission_clock
):
    x = admission_clock
    ...
    def change(data, read):
        if read == 1:
            if first == "stale":
                data["observed_at"] = x.now - 2
            else:
                data["observations_available"] = False

    x.change = change
    result = route._preadmission_health(x.loop, 1060.0, "visit")
    assert result["embedded_controls"]["observed_at"] == x.now
    assert x.reads == 2 and not x.events and x.now < 1002.0


def test_preadmission_unavailable_defers_at_original_deadline_without_input(
    admission_clock,
):
    x = admission_clock
...
def test_preadmission_control_revision_change_is_stop_not_fallback(admission_clock):
    ...
    def change(data, read):
        data["observations_available"] = False
        if read > 1:
            data["control"]["revision"] = 2

    x.change = change
    with pytest.raises(OvernightStopped, match="revision"):
        route._preadmission_health(x.loop, 1060.0, "visit")
...
def rig(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        service_visit, "MarketVisit", lambda: original_visit(tmp_path / "visit.json")
    )
    monkeypatch.setattr(safe_reload, "clear_observation", lambda h: True)
    monkeypatch.setattr(
        route, "WorkWindows", lambda: handoff.WorkWindows(tmp_path / "windows.json")
    )
    write_json(route.POLICY, {"enabled": True, "parity_verified": True})
    f = snapshot(
        "Parasite", 1, [item(100), item(101), item(102, 2000031), item(103, 1050002)]
    )
    ...
    loop = NS(
        health=observe,
        living=observe,
        state={"pid": 7, "started_at": 1.0},
        terrain=NS(
            travel_path=lambda a, b, **kw: line_tiles(a, b),
            walkable=lambda p: p[0] >= 0 and p[1] >= 0,
        ),
        check_stop=lambda: None,
    ...
    def send(body):
        ...
        if action == "delivery-target":
            ...
            dx, dy = (
                m["position"][0] - f["position"][0],
                m["position"][1] - f["position"][1],
            )
            point = [700 + (dx - dy) * 32, 350 + (dx + dy) * 16]
            viewport = [1416, 850]
            return {
                "ready": max(abs(dx), abs(dy)) <= 12 and clear_scene(point, viewport),
                "farmer_position": f["position"],
                "merchant_position": m["position"],
                "point": point,
                "viewport": viewport,
        ...
        if action == "status":
            return {"characters": {m["character"]: {"ready": True} for m in (d, s)}}
        if action == "delivery-pair":
            return {
                "farmer": copy.deepcopy(f),
                "merchant": copy.deepcopy(d if body["character"] == "Dutch" else s),
            }
        if action == "delivery-start":
            ...
            if key not in receipts:
                ...
                receipts[key] = {
                    ...
                    "proof_digest": "test-proof",
                }
                saved = read_json(route.STATE)["active"]
                receipts[key].update(
                    {
                        k: saved.get(k)
                        for k in ("visit_id", "town_visit_id", "farmer_profile_id")
                    }
                )
            return {"running": False, "receipt": receipts[key]}
        if action == "delivery-status":
            return {"running": False, "receipt": receipts.get(body["request_id"])}
...
def test_exact_requested_scroll_wins_only_slot_over_competing_plus_two(rig):
    ...
    assert [
        event["uids"] for action, event in rig.events if action == "delivery-start"
    ] == [[99]]


def test_exact_delivery_admission_is_reported_before_native_trade_submission(rig):
    wanted = item(99, 720027)
    rig.f["inventory"] = [wanted]
    admitted = []
    ...
    receipts = route.market_storage(
        rig.loop, send=rig.send, items=[wanted], on_admitted=on_admitted
    )
    assert [receipt["request_id"] for receipt in receipts] == admitted


def test_unreachable_first_merchant_defers_to_second_without_repeated_approach(
    rig, monkeypatch
):
    visits = []
    ...
    def approach(loop, plan, send, **kw):
        ...
        return (
            False if plan["merchant"] == "Dutch" else original(loop, plan, send, **kw)
        )
...
def test_full_warehouse_can_continue_with_no_loot_and_ready_merchant_capacity(rig):
    rig.f["inventory"] = []
    assert not route.warehouse_exhausted(
        rig.loop, {"items": [{}], "capacity": 1}, [], send=rig.send
    )
    assert not any(
        e in ("travel", "delivery-start", "handoff-grant") for e, _ in rig.events
    )
...
def test_remaining_valuable_cannot_be_excused_by_other_merchant_capacity(rig, kind):
    assert route.warehouse_exhausted(
        rig.loop, {"items": [{}], "capacity": 1}, [item(900, kind)], send=rig.send
    )
...
def test_full_bank_continuation_requires_current_qualified_reachable_capacity(
    rig, reason
):
    rig.f["inventory"] = []
    if reason == "disabled":
    ...
    if reason == "blocked_path":
        rig.loop.terrain.travel_path = lambda *a, **kw: []

    def send(body):
        result = rig.send(body)
        if reason == "unqualified" and body["action"] == "delivery-readiness":
            result["qualified"] = False
        if reason == "disconnected" and body["action"] == "status":
            result["characters"] = {}
        if body["action"] == "delivery-pair":
            if reason == "stale":
                result["merchant"]["timestamp"] -= 6
            if reason == "active_trade":
                result["farmer"]["trade"] = {"participant": "Dutch"}
            if reason == "wrong_character":
                result["merchant"]["character"] = "Stranger"
        return result

    assert route.warehouse_exhausted(
        rig.loop, {"items": [{}], "capacity": 1}, [], send=send
    )
    assert not any(
        e in ("travel", "delivery-start", "handoff-grant") for e, _ in rig.events
    )
...
def test_nonfull_warehouse_does_not_trigger_additional_merchant_inspection(rig):
    assert not route.warehouse_exhausted(
        rig.loop, {"items": [], "capacity": 1}, [item(900)], send=rig.send
    )
    assert not rig.events
...
def test_unknown_submission_never_falls_through_to_warehouse(rig):
    write_json(
        route.STATE,
        {"active": {"request_id": "lost", "merchant": "Dutch", "items": [item(100)]}},
    )
    write_json(route.POLICY, {"enabled": False})
...
def test_wrong_or_uncertain_receipt_prevents_fallback(rig):
    ...
    with pytest.raises(ValueError, match="reconciliation"):
        route.market_storage(rig.loop, send=send)
...
def test_manual_revision_change_never_refocuses_or_continues(rig):
    ...
    def send(body):
        result = rig.send(body)
        if body["action"] == "handoff-release":
            rig.health["embedded_controls"]["control"]["revision"] += 1
        return result
...
def test_delivery_window_respects_global_stop_and_independent_refill_permission():
    ...
    calls = []
    runtime = NS(
        refill_enabled=lambda name: name == "Dutch",
        refills={"Dutch": NS(start=lambda: calls.append("Dutch"))},
        handoff=None,
    )
    ui = NS(
        runtime=runtime,
        coordinator=NS(lock=threading.RLock(), owner=None, stopped=True),
    )
    body = {"action": "delivery-window", "request_id": "test-window"}
    with pytest.raises(ValueError, match="unavailable"):
        UnifiedUI.dispatch(ui, body)
    ...
    assert (
        calls == []
    )  # Delivery has priority; refill begins only after reconciliation.
...
def test_delivery_window_retry_does_not_reset_refill_or_replace_another_window():
    ...
    calls = []
    runtime = NS(
        refill_enabled=lambda name: True,
        refills={
            name: NS(start=lambda: calls.append("start"))
            for name in ("Dutch", "Spiritual")
        },
        handoff="existing",
        delivery_window=None,
        refill_window="existing",
    )
    ui = NS(
        runtime=runtime,
        grant=None,
    ...
    assert UnifiedUI.dispatch(
        ui, {"action": "delivery-window", "request_id": "existing"}
    ) == {"requested": "existing"}
    assert (
        runtime.refill_window == "existing"
        and runtime.delivery_window is None
        and not calls
    )
...
def test_delivery_window_has_no_refill_write_and_remains_releasable():
    ...
    runtime = NS(
        refill_enabled=lambda name: True,
        refills={name: NS(start=fail) for name in ("Dutch", "Spiritual")},
        handoff=None,
    ...
    assert (
        ui.grant is None
        and runtime.handoff == runtime.delivery_window == "failed-window"
    )
    assert UnifiedUI.dispatch(
        ui, {"action": "handoff-release", "request_id": "failed-window"}
    ) == {"released": True}
...
def test_remainder_refill_uses_original_deadline_only(rig, monkeypatch):
    ...
    monkeypatch.setattr(
        route.time, "sleep", lambda seconds: now.__setitem__(0, now[0] + seconds)
    )

    def send(body):
        calls.append(body["action"])
        if body["action"] == "delivery-refill":
            return {"refill": True, "expires_at": 1003}
...
def test_f11_prevents_refill_and_does_not_create_new_window(rig, monkeypatch):
    ...
    monkeypatch.setattr(
        route.ctypes.windll.user32, "GetAsyncKeyState", lambda key: 0x8000
    )
    with pytest.raises(OvernightStopped, match="F11"):
        route.refill_remainder(
            rig.loop,
            lambda b: pytest.fail("No command after F11"),
            "batch",
            time.time() + 10,
            {"target": rig.health["target"]},
            1,
        )
...
def test_refill_phase_requires_both_delivery_receipts(tmp_path, monkeypatch, verified):
    ...
    monkeypatch.setattr(
        op, "status", lambda journal, key: {"phase": "verified", "character": "Dutch"}
    )
    runtime = NS(
        delivery_window="batch",
        handoff="batch",
        journal=NS(
            get=lambda *a: {
                "request_id": "batch",
                "phase": "verified" if verified else "offer_ready",
            }
        ),
    ...
    ui = NS(
        runtime=runtime,
        grant={"request_id": "batch", "expires_at": 1234},
        safe_to_yield=lambda: True,
        coordinator=InputCoordinator(lambda: True, path=tmp_path / "input.lock"),
    )
    if verified:
        assert UnifiedUI.dispatch(
            ui, {"action": "delivery-refill", "request_id": "batch"}
        ) == {"refill": True, "expires_at": 1234}
...
def test_merchant_already_in_trade_range_does_not_move(rig):
    rig.f["position"] = [229, 189]
    rig.d["position"] = [232, 193]
    route.approach_merchant(
        rig.loop, {"merchant": "Dutch", "position": [232, 193]}, rig.send
    )
    assert not any(e == "travel" for e, _ in rig.events)


def test_actionable_target_at_thirteen_tiles_continues_to_checked_range(rig):
    rig.f["position"] = [226, 209]
    rig.d["position"] = [239, 213]
...
def test_merchant_approach_requires_visible_reachable_tile(rig):
    rig.f["position"] = [10, 10]
    rig.d["position"] = [40, 10]
    assert route.approach_merchant(
        rig.loop, {"merchant": "Dutch", "position": [40, 10]}, rig.send
    )
    assert rig.send({"action": "delivery-target", "character": "Dutch"})["ready"]
    assert rig.f["position"] != rig.d["position"]


def test_absent_remote_recipient_uses_only_checked_ingress_before_fresh_target_probe(
    rig,
):
    from conquest.navigation import line_tiles

    rig.f["position"] = [10, 10]
    rig.d["position"] = [50, 10]
    rig.loop.terrain.travel_path = lambda a, b, **kw: line_tiles(a, b)

    def send(body):
        if body["action"] == "delivery-target":
            distance = max(
                abs(a - b) for a, b in zip(rig.f["position"], rig.d["position"])
            )
            if distance > 12:
                ...
                return {
                    ...
                    "viewport": [1416, 850],
                    "occupied_tiles": [list(rig.f["position"])],
                }
        return rig.send(body)

    assert route.approach_merchant(
        rig.loop, {"merchant": "Dutch", "position": [50, 10]}, send
    )
...
def test_approach_applies_short_deadline_inside_travel_and_restores_outer_visit(
    rig, monkeypatch, stalled
):
    from conquest.travel_progress import TravelStalled

    now = [1000.0]
    rig.loop.market_service_deadline = 1060.0
    rig.d["position"] = [40, 10]
    monkeypatch.setattr(route.time, "time", lambda: now[0])
    original = rig.loop.travel
    ...
    def travel(point, **fields):
        ...
        if stalled:
            raise TravelStalled("test stall")
        original(point, **fields)

    rig.loop.travel = travel
    result = route.approach_merchant(
        rig.loop,
        {"merchant": "Dutch", "position": rig.d["position"]},
        rig.send,
        deadline=1060.0,
    )
...
def test_ambiguous_remote_recipient_is_hard_failure_without_movement(rig):
    def send(body):
        if body["action"] == "delivery-target":
            raise ValueError("Receiver UID is ambiguous in the farmer scene")
        return rig.send(body)

    with pytest.raises(ValueError, match="ambiguous"):
        route.approach_merchant(
            rig.loop, {"merchant": "Dutch", "position": rig.d["position"]}, send
        )
    assert not any(event == "travel" for event, _ in rig.events)


def test_changed_receiver_scene_retries_target_observation_without_movement(
    rig, monkeypatch
):
    attempts = []
    ...
    assert not any(event == "travel" for event, _ in rig.events)
    assert any(
        event == "merchant_target_deferred"
        and fields["reason"] == "recipient_scene_changed"
        for event, fields in rig.events
    )


def test_slow_target_observation_cannot_authorize_after_approach_deadline(
    rig, monkeypatch
):
    now = [1000.0]
    monkeypatch.setattr(route.time, "time", lambda: now[0])
...
def test_service_window_covers_remote_ingress_before_recipient_is_actionable(rig):
    ...
    assert (
        result
        and visit["phase"] == "active"
        and rig.loop.market_service_deadline is None
    )


def test_current_dutch_diagonal_in_range_is_not_clickable(rig):
    rig.f["position"] = [252, 221]
    rig.d["position"] = [264, 210]
    assert not rig.send({"action": "delivery-target", "character": "Dutch"})["ready"]
    assert route.approach_merchant(
        rig.loop, {"merchant": "Dutch", "position": [264, 210]}, rig.send
    )
    assert rig.send({"action": "delivery-target", "character": "Dutch"})["ready"]
            ...
            [
                {
                    ...
                    "ready": True,
                    "snapshot": m,
                    "verified_travel_distance": 1,
                }
                for m in (rig.s, rig.d)
            ],
        )

    proposed = plan()
    assert [
        (p["merchant"], [i["uid"] for i in p["items"]]) for p in proposed["deliveries"]
    ] == [("Dutch", [101])]
