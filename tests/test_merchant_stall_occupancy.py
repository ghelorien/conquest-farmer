...
@pytest.mark.parametrize(
    "case", ["known_pair", "new_occupant", "moved", "out_of_range", "unknown_layout"]
)
def test_scene_booth_occupancy_uses_owner_entity_not_flag_name(monkeypatch, case):
    ...
    if case == "new_occupant":
        snapshots[1] = [
            *snapshots[1],
            {**booth, "uid": 103061, "name": "Spiritual", "position": [265, 210]},
        ]
    ...
    if case == "out_of_range":
        positions = [(240, 210), (240, 210)]
    monkeypatch.setattr(
        "conquest.memory_life.read_life",
        lambda *a: NS(map_id=1036, dead_candidate=False, position=positions.pop(0)),
    )
    observer = NS(adapter=None, health_layout=None, character="Spiritual")
    spec = {
        "occupancy_mode": "scene_booth",
        "booth_model": 406,
        "booth_offset": [3, 0],
        "vacancy_radius": 8,
    }
    if case == "unknown_layout":
        spec["booth_offset"] = [2, 0]
    if case in ("new_occupant", "moved", "unknown_layout"):
        with pytest.raises(ValueError):
            stalls.vacant_flags(observer, spec)
    else:
        assert stalls.vacant_flags(observer, spec) == (
            [] if case == "out_of_range" else [empty]
        )


@pytest.mark.parametrize(
    "change",
    [{}, {"name": "Stranger"}, {"uid": 55}, {"model": 1086}, {"position": [100, 100]}],
)
def test_owned_booth_requires_exact_owner_identity_and_nearby_scene(
    monkeypatch, change
):
    booth = {
        "uid": 900,
    ...
    if change:
        with pytest.raises(ValueError):
            stalls.owned_booth(NS(character="Dutch"), snapshot)
    else:
        assert stalls.owned_booth(NS(character="Dutch"), snapshot) == booth


@pytest.mark.parametrize(
    "message", ["Shop flag changed while reading", "Shop flag identity is invalid"]
)
