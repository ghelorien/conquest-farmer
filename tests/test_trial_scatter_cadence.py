...
def test_fresh_target_timing_does_not_inherit_earlier_work_or_accept_slow_scans(tmp_path,monkeypatch,enabled,scan_seconds):
    ...
    monkeypatch.setattr(trial,'load_combat_speed',lambda _:CombatSpeed(selected_target_refresh=enabled))
    class Session:
    ...
    supervisor=SimpleNamespace(last_target=None,recovery=SimpleNamespace(terrain=SimpleNamespace(width=1000,height=1000)),
        ...
        attack_strategy=earlier_work,dispatch=lambda callback,**kwargs:callback(),loot_step=lambda *a:False,
    ...
    # The newer portable runtime already timestamps fresh scans for all profiles.
    assert bool(calls)==(scan_seconds<.35)
