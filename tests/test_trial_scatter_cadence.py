...
@pytest.mark.parametrize('scan_seconds',[.01,.4])
@pytest.mark.parametrize('loot_seconds',[0,.4])
...
def test_fresh_target_timing_does_not_inherit_earlier_work_or_accept_slow_scans(
        tmp_path,monkeypatch,enabled,scan_seconds,loot_seconds,projection_changed,pickup):
    ...
    now=[10.];calls=[];stages=[];projections=[0]
    config=yaml.safe_load(Path('profiles/pheasant-foreground-trial.yaml').read_text())
    config.update(character='TimingTest',observation_mode='memory_only',kite_when_surrounded=False,
        adaptive_scatter=True,jump_scatter=False,attack_button='right',route=[],healing_enabled=False,
    ...
    class Session:
        def request(self,operation,body=None):
            ...
            if operation=='sample':
                values=dict(name='TimingTest',position=[423,455],max_hp=[100],kill_counter=[0],level=[18],map=[1002])
                return {'fields':[{'name':k,'value':v} for k,v in values.items()]}
            assert operation=='foreground-click'
            calls.append(now[0]);return {}
    class Inventory:
        def __init__(self,*args):pass
        def read(self):
            now[0]+=.01
            return InventorySnapshot(now[0],now[0],(Item(1,1000000,1,1,0),),Item(2,1050000,200-3*len(calls),200,None),0,40)
    def scan(*args):
        stages.append('targets')
        now[0]+=scan_seconds
        return [Target('Pheasant',600,400,1,1,1000,(427,455),100)]
    drop=object();dispatches=[]
    def guarded_dispatch(callback,**kwargs):
        dispatches.append(kwargs)
        if pickup:assert kwargs['drop'] is drop and kwargs['target'] is None
        return callback()
    ...
    def projection():
        ...
        return ((424 if projection_changed and projections[0]%2==0 else 423,455),(518,396))
    strategy=SimpleNamespace(observe=lambda *a:None,button=lambda *a:'right',issued=lambda *a:None)
    def earlier_work():
        now[0]+=.4
        return strategy
    supervisor=SimpleNamespace(last_target=None,recovery=SimpleNamespace(terrain=SimpleNamespace(width=1000,height=1000)),
        observe=lambda:{'health_ratio':1.,'waiting':False,'defending':False},memory_targets=scan,
        attack_strategy=earlier_work,dispatch=guarded_dispatch,loot_step=loot,
    ...
    assert bool(calls)==((pickup or scan_seconds<.35) and not projection_changed)
    ...
    if pickup:
        ...
        assert all(stages[i-1]=='loot' for i,s in enumerate(stages) if s=='targets')
