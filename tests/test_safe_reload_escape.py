...
def test_escape_does_not_cross_previous_failed_landing():
    ground=terrain();source=(50,50);avoid={(54,50)}
    step=nearby_escape(ground,source,[{'position':[49,49],'alive':True}],avoid=avoid)
...
def test_escape_uses_visible_checked_diagonal_on_open_terrain():
    ground=terrain();source=(50,50);anchor=(518,396)
    step=nearby_escape(ground,source,[{'position':[49,49],'alive':None}],anchor=anchor)
...
def test_escape_preserves_solid_corner_and_actual_camera_guards(anchor):
    ground=terrain();source=(50,50)
    ground.blocked[50,51]=True;ground.blocked[51,50]=True
    step=nearby_escape(ground,source,[{'position':[49,49],'alive':True}],anchor=anchor)
...
def test_enclosing_failed_landings_do_not_allow_jumping_across_them():
    source=(50,50)
    avoid={(source[0]+dx,source[1]+dy) for dx in (-1,0,1) for dy in (-1,0,1) if dx or dy}
    assert nearby_escape(terrain(),source,[{'position':[49,49],'alive':True}],avoid=avoid) is None


def test_one_unprojectable_candidate_does_not_discard_other_checked_escapes(monkeypatch):
    from conquest import navigation
    original=navigation.travel_waypoint;calls=[]
    def waypoint(*args,**kwargs):
        calls.append(1)
        if len(calls)==1:raise ValueError('No visible route landing point')
        return original(*args,**kwargs)
    monkeypatch.setattr(navigation,'travel_waypoint',waypoint)
    ground=terrain();step=nearby_escape(ground,(50,50),[{'position':[49,49],'alive':True}])
