...
def rig(tmp_path, monkeypatch):
    ...
    x.runtime = MerchantRuntime(
        NS(identities=lambda: []),
        x.guard,
        journal=x.journal,
        market_path=tmp_path / "market.json",
    )
    for refill in x.runtime.refills.values():
        refill.clock = lambda: x.now
    x.read = lambda: {**deepcopy(x.state), "timestamp": x.now}
    x.driver = NS(
        read=x.read,
        require_qualified=lambda name: None,
        memory=NS(read=lambda **kw: x.read()),
    )
    ...
    x.runtime.observers["Dutch"] = NS(
        lock=threading.RLock(),
        adapter=NS(
            identity=x.state["identity"],
    ...
    x.guard.owner_allowed = lambda character: (
        character == "Farmer" or x.runtime.input_allowed(character)
    )
    install(x.guard)
    yield x
    install(None)
...
def test_unknown_request_is_durable_target_only_pause(rig):
    ...
    with input_scope():
        pass
    with x.guard.lease("Spiritual", purpose="trade"):
        pass
...
def test_approved_session_never_calls_trade_or_listing_and_fences_farmer(rig, window):
    ...
    for character in ("Dutch", "Spiritual"):
        with pytest.raises(CaptureUnavailable, match="Manual visitor"):
            with x.guard.lease(character, purpose="trade"):
                pytest.fail("Manual input is exclusive")
    with pytest.raises(CaptureUnavailable, match="Manual visitor"):
        with input_scope():
            pytest.fail("Farmer cannot take manual input")
...
def test_exact_permission_auto_activates_but_does_not_grant_delivery_trust(rig):
    ...
    settle(x)
    x.now += 1
    x.state["request"] = snapshot()["request"]
    x.runtime.step("Dutch")
...
def test_revocation_never_releases_active_fence_or_cancels_later_request(
    rig, monkeypatch
):
    x = rig
    row = approve(x)
...
def test_changed_request_at_decline_boundary_records_no_claim_or_input(
    ...
    before = x.read()
    controller, _ = native_rig(tmp_path, before, {**before, "request": None})
    controller.journal = x.journal
...
def test_request_change_cannot_use_displayed_binding(rig, field):
    x = rig
    x.runtime.step("Dutch")
    binding = x.runtime.manual_status("Dutch")["approval_binding"]
    x.now += 1
    x.state["request"][field] = (
        "Different visitor"
        if field == "participant"
        else 45
        if field == "participant_uid"
        else "Changed prompt"
    )
    with pytest.raises(ManualSessionError):
        x.runtime.approve_manual(binding)
    assert not x.runtime.manual_sessions.permissions()
    x.runtime.step("Dutch")
    assert (
        x.runtime.manual_status("Dutch")["phase"] == "needs_attention" and not x.calls
    )


@pytest.mark.parametrize(
    "field,value", [("pid", 8), ("path", "C:/Other/ImConquer.exe")]
)
...
def test_unresolved_bot_transaction_blocks_manual_admission_even_if_reconcile_returns(
    rig,
):
    x = rig
    x.journal.begin("unresolved", "Dutch", "listing", {"uid": 99})
...
def test_existing_bot_open_trade_is_not_quarantined(rig):
    x = rig
    x.state.update(
        request=None, trade={"participant": "Parasite", "participant_uid": 55}
    )
    x.journal.set(
        "Dutch",
        "accepted_request",
        dict(identity=x.state["identity"], participant_uid=55, opened_at=x.now),
    )
    x.runtime.step("Dutch")
...
def test_unapproved_open_trade_is_quarantined(rig):
    x = rig
    x.state.update(
        request=None, trade={"participant": "Visitor", "participant_uid": 44}
    )
    x.runtime.step("Dutch")
    assert (
        x.runtime.manual_status("Dutch")["phase"] == "needs_attention" and not x.calls
...
def test_incomplete_first_memory_is_durable_target_hold(rig):
    x = rig
    x.state["request"]["participant_uid"] = None
    x.runtime.step("Dutch")
    row = x.runtime.manual_status("Dutch")
    assert (
...
def test_approval_timeout_race_is_serialized_and_never_claims_input(rig):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = [
            pool.submit(x.runtime.approve_manual, row["approval_binding"]),
...
def test_decline_claim_only_at_native_boundary_and_never_replayed(
    rig, tmp_path, monkeypatch
):
    from test_unrelated_request import rig as native_rig, patches
    from conquest.merchants.unrelated_request import decline_unrelated_request
    ...
    x.runtime.reject_manual(row["approval_binding"])
    before = x.read()
    after = {**before, "request": None}
    controller, _ = native_rig(tmp_path, before, after)
    controller.journal = x.journal
    attempts = []
    ...
    def lost(target, a, b, size, **kw):
        with x.journal.db() as db:
            assert (
                db.execute("SELECT COUNT(*) FROM manual_decline_claims").fetchone()[0]
                == 0
            )
        kw["before_press"]()
        attempts.append(1)
    ...
    assert (
        x.runtime.manual_sessions.get(row["id"])["request_state"] == "decline_claimed"
    )
    x.now += 1
    x.runtime.step("Dutch")
...
@pytest.mark.parametrize(
    "change", ["unchanged", "add", "modify", "remove", "booth_to_bag"]
)
def test_atomic_settlement_excludes_false_sales_and_only_marks_real_new_stock(
    rig, change
):
    x = rig
    ...
    if change == "add":
        x.state["inventory"].append(item(12))
    if change == "modify":
        x.state["inventory"][0]["quantity"] = 2
    if change == "remove":
        x.state["inventory"] = []
    if change == "booth_to_bag":
        x.state["inventory"].append(item(11))
    x.now += 1
    observe(x.journal, x.read())
    x.runtime.step("Dutch")
    x.now += 5
    ...
    with x.journal.db() as db:
        assert (
            json.loads(db.execute("SELECT snapshot FROM sales_baseline").fetchone()[0])[
                "timestamp"
            ]
            == x.now
        )
...
def test_settlement_hook_failure_rolls_back_baseline_gap_and_terminal(rig):
    ...
    x.runtime.manual_sessions.on_settlement = fail
    x.now += 5
    ...
    x.runtime.manual_sessions.on_settlement = callback
    x.now += 1
    x.runtime.step("Dutch")
...
def test_merchant_replan_uses_fresh_memory_and_preserves_refill_pause(rig):
    ...
    assert not consume_farmer(
        x.journal,
        dict(timestamp=x.now, map_id=1002, inventory=[], urgent_banking=False),
    )
    x.runtime.step("Dutch")
    assert not x.calls
...
def test_farmer_signal_requires_fresh_current_map_inventory_and_valuables(rig):
    ...
    store = ManualSessionStore(
        x.journal.path, on_settlement=partial(settlement, target_role="Farmer")
    )
    ...
    with pytest.raises(ValueError):
        consume_farmer(x.journal, {"timestamp": 108}, now=108)
    evidence = dict(
        timestamp=108, map_id=1002, inventory=[{"uid": 10}], urgent_banking=True
    )
    ...
    with x.journal.db() as db:
        ...
        assert (
            saved["merchant_pending"] == 0
            and json.loads(saved["farmer_evidence"]) == evidence
        )
...
def test_override_rebaseline_survives_restart_and_requires_two_fresh_equal_samples(rig):
    ...
    observe(x.journal, x.read())
    row = approve(x)
    x.runtime.override_manual(
        row["id"],
        confirmation_reference="checked-1",
        operator="Floor",
        reason="Manual inspection",
    )
    hold = x.runtime.manual_status("Dutch")
    assert hold["rebaseline"] and hold["fence_scope"] == "target"
    assert not x.guard.manual_session_blocked() and x.guard.manual_session_blocked(
        "Dutch"
    )
    x.state.update(request=None, booth=[], silver=1097)
    x.now += 1
...
@pytest.mark.parametrize(
    "fault", ["rollover", "missing", "open_window", "ownership_change"]
)
def test_override_baseline_cannot_settle_bad_or_changing_evidence(rig, fault):
    ...
    x.runtime.override_manual(
        row["id"],
        confirmation_reference="checked",
        operator="Floor",
        reason="Explicit disposition",
    )
    x.state["request"] = None
    x.now += 1
    x.runtime.step("Dutch")
    ...
    if fault == "missing":
        x.state.pop("silver")
    if fault == "open_window":
        x.state["request"] = snapshot()["request"]
    if fault == "ownership_change":
        x.state["silver"] += 1
    x.runtime.step("Dutch")
    ...
    if fault in ("rollover", "missing"):
        ...
        x.state = snapshot(request=None)
        x.now += 10
        x.runtime.step("Dutch")
        assert x.runtime.manual_status("Dutch")["phase"] == "needs_attention"
    elif fault == "ownership_change":
        assert held["stable_since"] > first
    else:
        assert held["stable_since"] is None
    with x.journal.db() as db:
        assert db.execute("SELECT COUNT(*) FROM sales_baseline").fetchone()[0] == 0
...
def test_baseline_override_failure_and_retry_remain_target_scoped(rig):
    ...
    x.runtime.override_manual(
        hold["id"],
        confirmation_reference="fixed-reader",
        operator="Floor",
        reason="Reader repaired",
    )
    x.state = snapshot(request=None)
    x.now += 1
    x.runtime.step("Dutch")
    ...
    x.runtime.override_manual(
        held["id"],
        confirmation_reference="retry-reader",
        operator="Floor",
        reason="Rechecked reader",
    )
    x.state = snapshot(request=None)
    x.now += 1
    x.runtime.step("Dutch")
...
def test_manual_market_observation_does_not_pause_or_disconnect_intent(rig):
    ...
    observer.health_layout = object()
    guard.check(
        "Dutch", observer, read=lambda *a: NS(map_id=1002, dead_candidate=False)
    )
    assert (
        x.journal.get("Dutch", "enabled")
        and guard.observations["Dutch"]["map_id"] == 1002
    )
    ...
    recovery_observe(
        ...
        "Dutch",
        x.state["identity"],
        close=lambda *a: pytest.fail("Manual session cannot disconnect"),
    )
    assert (
        x.journal.get("Dutch", "enabled")
        and x.journal.get("Dutch", "recovery_safety")["last_progress"] == 1
    )


def test_manual_farmer_observer_keeps_life_reads_without_recovery_input(
    rig, monkeypatch
):
    ...
    supervisor.recovery.step = lambda *a: pytest.fail(
        "Manual fence must precede recovery gameplay"
    )
...
def test_manual_alert_wait_is_quiet_and_never_confirms_old_incident(rig):
    ...
    approve(x)
    state = {
        "manual_session": x.runtime.manual_status("Dutch"),
        "manual_input_fence": True,
        "connected": True,
    }
    alerts = Alerts()
    alerts.observe("Dutch", ("Old failure", 0), 100)
    ...
    assert condition(state) == (
        "Manual visitor session needs attention: Memory missing",
        0,
    )
