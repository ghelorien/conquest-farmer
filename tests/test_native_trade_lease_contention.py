...
def test_farmer_waiting_for_qualified_receiver_gets_retryable_contention(coordinator):
        ...
        try:
            with coordinator.lease("Dutch", purpose="trade"):
                ...
                if not release.wait(3):
                    raise AssertionError("Test receiver was not released")
    ...
    try:
        ...
        before = list(checks)
        with pytest.raises(
            CaptureUnavailable, match="^Another character owns game input$"
        ):
            coordinator.check()
...
def test_same_thread_real_surface_qualification_still_required(coordinator, qualified):
    coordinator.owner = "Dutch"
    coordinator.thread = threading.get_ident()
    ...
    if qualified:
        ...
        with pytest.raises(
            CaptureUnavailable,
            match="Client surface needs reattachment and input qualification",
        ):
            coordinator.check()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("manual_mouse", "Automation stopped"),
        ("manual_session", "Manual visitor session"),
    ],
)
@pytest.mark.parametrize("foreign", [False, True])
def test_stop_fence_and_manual_holds_precede_surface_and_contention(
    coordinator, hold, reason, foreign
):
    coordinator.owner = "Dutch"
    coordinator.thread = -1 if foreign else threading.get_ident()
    coordinator.purpose = "trade"
    ...
    if hold == "fence":
        ...
        def revoked():
            raise CaptureUnavailable("fence revoked")

        coordinator.fence = NS(check=revoked)
        coordinator.stopped = True  # Fence remains the first failure.
    elif hold == "stop":
        coordinator.stopped = True
    elif hold == "manual_mouse":
        coordinator.manual_active = lambda: True
    else:
        coordinator.manual_sessions = {"Dutch": {"holds_automation": True}}
    with pytest.raises(CaptureUnavailable, match=reason):
        coordinator.check()
