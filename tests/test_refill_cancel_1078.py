...
def pending(receipt, monkeypatch):
    ...
    x.state = {**x.ui.runtime.refills['Dutch'].state(), 'pending': True,
               'listing1078_engine': 1, 'listing1078_request': x.request,
               'next_check': 12345, 'last_checked': 11445, 'listed': 2, 'cursor': [91]}
    ...
    x.snapshot = {**deepcopy(x.before), 'closed_modal': True, 'listing_preflight': {
        'price_modal': {'observed': True, 'candidate_selected_item_uid': 91,
            'candidate_price_buffer_text': '', 'selected_item_uid_verified': True,
            'price_buffer_binding_verified': True, 'native_ok_cancel_handlers_verified': True}}}
    return x
...
def test_exact_preconfirmation_modal_admission_is_read_only(pending):
    ...
    assert refill.pending_cancel(x.ui.runtime, 'Dutch', x.snapshot, preflight=True) == x.request
    assert x.j.trace(x.key) == before_trace
    assert x.ui.runtime.refills['Dutch'].state() == x.state


@pytest.mark.parametrize('mutation', [None, 'uid', 'type_id', 'name', 'plus', 'gem1',
    'gem2', 'bound', 'quantity', 'slot', 'price', 'unknown_field', 'missing_field'])
@pytest.mark.parametrize('collection', ['inventory', 'booth'])
def test_derived_category_does_not_hide_any_native_stock_change(pending, collection, mutation):
    x = pending
    ...
    if mutation == 'unknown_field':
        ...
        selected[mutation] = ('Changed' if mutation == 'name' else
                              True if mutation == 'bound' else (selected[mutation] or 0)+1)
...
def test_original_preflight_shape_requires_recorded_closed_windows(pending, closed_proof):
    x = pending
...
@pytest.mark.parametrize('mutation', ['confirm', 'cancel', 'worker', 'identity', 'stock',
    'silver', 'price', 'modal_closed', 'different_uid', 'missing_pin', 'manual_receipt',
    'pause', 'missing_ownership', 'wrong_profile'])
def test_changed_or_uncertain_admission_fails_closed(pending, mutation, monkeypatch):
    ...
    if mutation in ('confirm', 'cancel'):
        ...
        monkeypatch.setitem(listing.WORKERS, x.key, SimpleNamespace(is_alive=lambda: True))
        ...
        x.snapshot['listing_preflight']['price_modal']['candidate_price_buffer_text'] = '99'
    elif mutation == 'modal_closed':
        x.snapshot['listing_preflight']['price_modal']['observed'] = False
    elif mutation == 'different_uid':
        x.snapshot['listing_preflight']['price_modal']['candidate_selected_item_uid'] = 92
    elif mutation == 'missing_pin':
        x.snapshot['listing_preflight']['price_modal']['native_ok_cancel_handlers_verified'] = False
    elif mutation == 'pause':
...
def test_refill_cancel_scope_authorizes_only_exact_cancel_and_respects_pause(pending):
    x = pending
    x.ui.grant = {'scope': handoff.SCOPE, 'character': 'Dutch', 'listing_authority': {
        'mode': 'refill_cancel', 'profile_id': 'test-dutch', 'listing_request_id': x.key}}
...
def test_observer_reuses_cancel_dispatch_only_and_preserves_schedule(pending, monkeypatch):
    x = pending
    x.ui.grant = {'scope': handoff.SCOPE, 'character': 'Dutch', 'listing_authority': {
        'mode': 'refill_cancel', 'profile_id': 'test-dutch', 'listing_request_id': x.key}}
    calls = []
    ...
    monkeypatch.setattr(listing, 'dispatch', lambda *a, **kw: pytest.fail('No listing replay'))
...
def test_cancel_receipt_consumes_old_cursor_without_claiming_listing(pending):
    ...
    x.j.transition(x.key, 'aborted', {'uid': 91, 'cancel_verified': True,
        'stock_unchanged': True, 'listing_submitted': False, 'confirmation_attempted': False,
        'cancellation_attempted': True, 'replay_allowed': False,
        'first': x.before, 'second': deepcopy(x.before)})
