...
def rig(monkeypatch, *, stop_at_retreat=False):
    ...
    def living():
        ...
        return {'window': {'client_size': [1416, 876]}, 'embedded_controls': {
    ...
    def step(target, *, expected_position):
        ...
        position[:] = target
        return {'reached': True}
    loop = SimpleNamespace(terrain=SimpleNamespace(map_id=1002, travel_path=planner,
        straight_path=planner), route=SimpleNamespace(supplies=SimpleNamespace(arrow_type=1050000)),
        record=lambda event, **kw: events.append(event), living=living,
...
def test_retreat_keeps_failed_edge_until_genuine_route_progress(monkeypatch):
    ...
    assert steps[:4] == [((194,255),(191,252)), ((194,255),(198,257)),
                         ((198,257),(204,259)), ((204,259),(204,247))]
    ...
    assert not any(start == (204,259) and failed not in avoid for start,avoid in plans)
