...
def test_two_merchants_share_download_and_restart_keeps_result(tmp_path):
    ...
    def collect(**kwargs):
        calls.append(kwargs)
        data = snapshot(100)
        Path(kwargs["destination"]).write_text(json.dumps(data))
        return data
    ...
    worker.request("Spiritual", "a")
    worker.request("Dutch", "b")
    assert worker.request("Dutch", "duplicate")["request_id"] == "b"
    worker.step()
    assert len(calls) == 1
    assert all(
        worker.state(c)["phase"] == "ready" and not worker.state(c)["pending"]
        for c in ("Spiritual", "Dutch")
    )
    restarted = MarketRefreshWorker(
        journal, threading.Event(), worker.path, collect=collect, clock=lambda: 101
    )
    restarted.step()
    assert len(calls) == 1
...
def test_failed_download_keeps_work_pending_and_retries_without_secret_text(tmp_path):
    ...
    def collect(**kwargs):
        calls.append(1)
        if len(calls) == 1:
            raise RuntimeError("secret browser session")
    ...
    worker.request("Dutch", "once")
    worker.step()
    assert (
        worker.state("Dutch")["phase"] == "failed" and worker.state("Dutch")["pending"]
    )
    ...
    now[0] = 161
    worker.step()
    assert worker.state("Dutch")["phase"] == "ready" and len(calls) == 2


def test_old_scan_with_expired_data_fetches_without_resuming_paused_operations(
    tmp_path,
):
    journal = Journal(tmp_path / "j.db")
    calls = []
    journal.request_scan("Dutch", "legacy", now=1)
...
def test_resume_legacy_batch_is_listing_only_and_completion_counts_survive_restart(
    tmp_path,
):
    journal = Journal(tmp_path / "j.db")
    journal.request_scan("Dutch", "legacy", now=1)
    ...
    assert restarted.get("Dutch", "scan")["deferred"] == 2 and not restarted.get(
        "Dutch", "enabled"
    )
...
def test_collector_uses_configured_browser_instead_of_wrong_account_cache(tmp_path):
    ...
    assert browser_launch_options(settings) == {
        "headless": True,
        "executable_path": str(executable),
    }
    executable.unlink()
    with pytest.raises(ValueError, match="browser is missing"):
        browser_launch_options(settings)
