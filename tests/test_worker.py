...
@pytest.mark.parametrize(
    "code,expected",
    [
        ("foreground_unavailable", CaptureUnavailable),
        ("input_acquisition_busy", InputAcquisitionBusy),
        ("bad_geometry", ValueError),
    ],
)
def test_worker_preserves_recoverable_focus_errors(
...
def test_sample_reads_typed_values(operations):
    ...
    result = operations.dispatch(
        "sample", {"fields": [{"name": "hp", "address": "0x20000", "kind": "u32"}]}
    )
...
def test_worker_scan_budget_is_bounded(operations, monkeypatch, field, value):
    monkeypatch.setattr(
        "conquest.worker.scan",
        lambda *_args, **_kwargs: pytest.fail("Invalid budget reached scanner"),
    )
    with pytest.raises(ValueError):
...
def test_worker_passes_requested_scan_budget(operations, monkeypatch):
    ...
    assert operations.dispatch(
        "scan", {**scan_body(), "max_mib": 1024, "max_candidates": 10000}
    ) == {"qualified": False}
...
def test_missing_jump_modifier_retries_without_click_and_releases_control(monkeypatch):
    ...
    events = []
    state = {"accept_control": False, "held": False}
    ...
    functions = {
        "SetForegroundWindow": lambda *args: True,
        "ClientToScreen": lambda *args: True,
        "GetSystemMetrics": lambda key: {76: 0, 77: 0, 78: 1920, 79: 1080}[key],
        "SendInput": send,
        "GetAsyncKeyState": lambda key: (
            0x8000 if state["held"] and key in (0x11, 0xA2) else 0
        ),
    }
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            thread_info=lambda hwnd: (1, SimpleNamespace(hwndFocus=1, hwndActive=1))
        ),
    )
    target = SimpleNamespace(
        hwnd=1,
        backend=SimpleNamespace(user=None, foreground=lambda: 1),
        snapshot=lambda: {
            "foreground": 1,
    ...
    with pytest.raises(CaptureUnavailable, match="no jump click sent"):
        foreground.foreground_click(
            target, 500, 400, (1036, 793), control=True, require_foreground=True
        )
