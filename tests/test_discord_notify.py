...
from conquest.discord_notify import (
    Notifications,
    DeliveryError,
    deliver,
    notable_drop,
...
def test_notification_targets_follow_changed_route_policy(tmp_path, monkeypatch):
    ...
    policy.write_text(
        json.dumps({"target_kills_per_minute": 60, "stretch_kills_per_minute": 80})
    )
    assert module.kill_rate_targets() == (60, 80)
    policy.write_text(
        json.dumps({"target_kills_per_minute": 60, "stretch_kills_per_minute": 50})
    )
    assert module.kill_rate_targets() == (60, 60)
    policy.write_text(
        json.dumps({"target_kills_per_minute": False, "stretch_kills_per_minute": -1})
    )
    assert module.kill_rate_targets() == (40, 50)
...
def test_full_storage_alert_is_immediate_prioritized_and_not_repeated(tmp_path):
    ...
    events.write_text(
        json.dumps({"event": "storage_full_stop", "phase": "stopped"}) + "\n"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "https://evil.test/api/webhooks/123/abc",
...
def test_webhook_requires_server_confirmation():
    assert webhook_url("https://discord.com/api/webhooks/123/abc_DEF-123").endswith(
        "?wait=true"
    )
...
def test_delivery_requires_discord_message_id():
    class Opener:
        def open(self, call, timeout):
            return io.BytesIO(b'{"id":"12345"}')

    assert (
        deliver("https://discord.com/api/webhooks/123/test", "Status changed", Opener())
        == "12345"
    )


@pytest.mark.parametrize(
    "focused,dead,expected",
    [
        (True, False, "Hunting Turtledoves"),
        (False, False, "Paused"),
        (True, True, "Dead"),
    ],
)
def test_current_memory_overrides_transient_desktop_pause_messages(
    focused, dead, expected
):
    from conquest.discord_notify import live_activity

    health = {
        "window": {"foreground": 1 if focused else 2, "root_hwnd": 1},
        "embedded_controls": {
            "life": {"dead_candidate": dead},
            "control": {"enabled": True, "execution_state": "farming"},
        },
    }
    assert live_activity(
        health, {"current_activity": "Running", "selected_route": "turtledove"}
    ).startswith(expected)
...
def test_live_hunting_flag_cannot_mask_navigation_blockage():
    ...
    health = {
        "window": {"foreground": 1, "root_hwnd": 1},
        "embedded_controls": {
    ...
    assert live_activity(
        health, {"navigation_blocked": True, "current_activity": "Running"}
    ).startswith("Navigation blocked")
...
def test_confirmed_farming_requires_active_focused_live_worker():
    ...
    for key, value in [("external_execution", False), ("life", None)]:
        ...
        changed["embedded_controls"][key] = value
...
def farming(n, now):
    n.observe(
        {"live_farming": True, "live_checked_at": now, "current_activity": "Attacking"},
        {"phase": "hunting", "updated_at": now},
        now,
        lambda pid: True,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "Paused — waiting for Conquer focus",
        "Patrolling",
        "Farming is off",
    ],
)
...
def test_policy_migration_discards_old_alert_and_drop_backlog():
    n = Notifications(
        {
            "notification_policy": "major_v2",
            "queue": [
                {"content": "STOPPED: old"},
                {"content": "Notable drop: Meteor"},
                {"content": "Farming resumed"},
            ],
        }
    )
    assert not n.state["queue"]
...
def navigation(n, now, *, blocked=True, position=(603, 576), kills=15, pickup=0):
    n.observe(
        {
            "live_activity": "Navigation blocked" if blocked else "Hunting",
            "navigation_blocked": blocked,
            "live_farming": True,
            "live_checked_at": now,
            "live_position": list(position),
            "live_map": 1002,
            "kills": kills,
            "last_pickup": {"timestamp": pickup},
        },
        {"phase": "hunting", "pid": 5, "updated_at": now},
        now,
        lambda pid: True,
    )
...
@pytest.mark.parametrize(
    "change", [{"position": (604, 576)}, {"kills": 16}, {"pickup": 120}]
)
def test_actual_progress_resets_blockage_timer(change):
    n = Notifications()
    navigation(n, 100)
    navigation(n, 150, **change)
    navigation(n, 209, **change)
    assert not n.state["queue"]
...
def test_stall_recovery_requires_progress_not_merely_cleared_flag():
    ...
    n.delivered(n.state["queue"].pop(0))
    n = Notifications(json.loads(json.dumps(n.state)))
    navigation(n, 170, blocked=False)
    navigation(n, 172, blocked=False)
    assert not n.state["queue"]
    navigation(n, 180, blocked=False, position=(604, 576))
    navigation(n, 182, blocked=False, position=(604, 576))
    assert (
        len(n.state["queue"]) == 1 and n.state["queue"][0]["kind"] == "farming_resumed"
    )
...
def test_quarter_hour_summary_is_immediate_then_persistent_and_coalesced(
    ...
    route = {
        "phase": "hunting",
        "updated_at": 100,
        "supplies": {"arrows": 1679, "potions": 15, "free_slots": 13},
    }
    n.updates(app, route, path, 100, lambda pid: True)
    assert len(n.state["queue"]) == 1
    content = n.state["queue"][0]["content"]
    assert (
        "level 17" in content and "HP 317/329" in content and "1,679 arrows" in content
    )
    assert "42 verified kills in the last 15 minutes" in content
    assert (
        "1,400 verified kills in the last hour (minimum: 2,400, stretch: 3,000)"
        in content
    )
...
def test_restock_log_events_survive_short_trip_restart_and_partial_write(tmp_path):
    path = tmp_path / "events.jsonl"
    path.write_text(
        json.dumps({"event": "restock_complete", "phase": "restocking"}) + "\n"
    )
    ...
    n = Notifications(json.loads(json.dumps(n.state)))
    with path.open("a") as out:
        out.write('"phase":"restocking","supplies":{"arrows":1700,"potions":15}}\n')
    n.updates({}, route, path, 102, lambda pid: True)
    n.updates({}, route, path, 103, lambda pid: True)
    assert [r["kind"] for r in n.state["queue"]] == [
        "restock_started",
        "restock_complete",
    ]
...
def test_reload_notices_do_not_claim_town_trip_and_resume_only_after_live_farming(
    tmp_path,
):
    path = tmp_path / "events.jsonl"
    path.write_text("")
...
def test_critical_failure_precedes_queued_routine_updates():
    n = Notifications()
    n.enqueue("status", 100, "periodic_status")
...
def test_rolling_kills_uses_verified_increments_across_session_resets(tmp_path):
    ...
    rows = [
        (99, "kill_verified", {"count": 9, "total": 99}),
        (100, "kill_verified", {"count": 8, "total": 107}),
        (101, "kill_verified", {"count": 2, "total": 109}),
        (500, "trial_started", {}),
        (501, "kill_verified", {"count": 1, "total": 1}),
        (999, "attack_attempt", {"number": 55}),
        (1000, "kill_verified", {"count": 1, "total": 2}),
        (1001, "kill_verified", {"count": 5, "total": 7}),
    ]
    db.executemany(
        "insert into events values(?,?,?)", [(t, e, json.dumps(p)) for t, e, p in rows]
    )
    db.commit()
    db.close()
...
def test_savings_completion_is_announced_once(tmp_path):
    ...
    path.write_text(
        json.dumps({"event": "savings_complete", "phase": "completed", "silver": 50021})
        + "\n"
    )
...
def test_status_write_retries_sharing_violation_with_unique_temporary(
    tmp_path, monkeypatch
):
...
def test_every_verified_pickup_is_notified_once_with_plus_and_timestamp(tmp_path):
    ...
    n.drops(path, 100)
    rows = [
        {"type_id": 421015, "plus": 1, "increase": 1, "timestamp": 101},
...
def test_pickup_delivery_receipt_links_exact_item_to_discord_message(tmp_path):
    ...
    assert (
        receipt["message_id"] == "discord-message-id" and receipt["delivered_at"] == 103
    )
    assert receipt["created_at"] == 101 and not restored.state["queue"]


def test_hourly_count_accepts_qualified_scatter_batches_but_rejects_discontinuities(
    tmp_path,
):
    import sqlite3
    from conquest.discord_notify import recent_kills
    ...
    with sqlite3.connect(path) as db:
        ...
        db.execute(
            ...
            (500, "kill_verified", json.dumps({"count": 20})),
        )
        db.execute(
            "insert into events values(?,?,?)",
            (600, "kill_verified", json.dumps({"count": 32})),
        )
    assert recent_kills(path, 1000, 3600) == 52
    with sqlite3.connect(path) as db:
        db.execute(
            "insert into events values(?,?,?)",
            (700, "kill_verified", json.dumps({"count": 33})),
        )
