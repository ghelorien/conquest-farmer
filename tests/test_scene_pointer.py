...
def test_waits_for_game_pointer_without_repeating_input(scenario):
    elapsed=[0.0];checks=[];reads=[];base=0x140000000
    def read(address,size):
        if address==base+0x19fcb1:
            return bytes(6) if scenario=='bad_code' else bytes.fromhex('8b2df5884f00')
        if address==base+0x19fcbc:return bytes.fromhex('8b35e6884f00')
        assert address==base+0x6985a8 and size==8
        reads.append(elapsed[0])
        return struct.pack('<2i',700,400 if scenario=='settles' and elapsed[0]>=.03 else 350)
    def check():
        checks.append(elapsed[0])
        if scenario=='manual_stop' and elapsed[0]>=.02:
            raise CaptureUnavailable('Manual Stop')
    def sleep(seconds):elapsed[0]+=seconds
    session=NS(expected_sha256=CLIENT_SHA256,modules=[{'name':'ImConquer.exe','base':base,'size':0x700000}],
               read_block=read,assert_identity=lambda:None)
    kwargs=dict(clock=lambda:elapsed[0],sleep=sleep,timeout=.08)
    if scenario=='settles':
        wait_scene_pointer(session,(700,400),check,**kwargs)
        assert .03<=elapsed[0]<.08 and len(reads)>=4
    else:
        error=ValueError if scenario=='bad_code' else CaptureUnavailable
        with pytest.raises(error):wait_scene_pointer(session,(700,400),check,**kwargs)
        assert elapsed[0]<=.09
        if scenario=='bad_code':assert not reads
    assert len(checks)==len(reads) or scenario=='manual_stop'


@pytest.mark.parametrize('scenario',['stable','npc_moves_after_cursor','pointer_not_observed','torn_npc_read'])
def test_shop_click_rechecks_npc_after_cursor_move_before_mouse_down(monkeypatch,scenario):
    from conquest import town_trade,scene_pointer
    trade=town_trade.TownTrade.__new__(town_trade.TownTrade)
    trade.observer=NS(adapter=NS(viewport_size=(1416,907)),operations=NS(target=object()))
    trade.life=lambda *args,**kwargs:None
    state={'cursor_moved':False,'clicked':False};presses=[]
    def vendor(kind):
        ...
        return NS(entity_id=101356,map_id=1011,type_id=10013,name='Blacksmith',
                  position=(197,226),draw_position=(701 if shifted else 700,400))
    trade.vendor=vendor
    def shop(uid):
        if not state['clicked']:raise ValueError('Requested GUI window is not active')
        return NS(products=[])
    trade.shop=NS(read=shop)
    def click(target,x,y,size,**kwargs):
        state['cursor_moved']=True
        kwargs['before_press']()
        presses.append((x,y));state['clicked']=True
    ...
    if scenario=='stable':
        ...
        with pytest.raises(town_trade.TownObservationUnavailable):trade({'action':'open','vendor_type':5})
        assert not presses
