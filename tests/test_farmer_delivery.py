...
def offered(intent):
    ...
    for own, other, is_farmer in ((f, m, True), (m, f, False)):
        own["trade"] = dict(
            ...
            own_items=intent["items"] if is_farmer else [],
            items=[] if is_farmer else intent["items"],
            own_silver=0,
            other_silver=0,
        )
    return f, m
...
def test_prefers_capacity_then_verified_distance_and_splits():
    ...
    states = [
        dict(
            ...
            snapshot=n,
            ready=True,
            verified_travel_distance=dist,
        )
        for n, dist in ((d, 1), (s, 100))
    ]
    plans = plan_deliveries(f, states, now=100)
    assert [(p["merchant"], len(p["items"])) for p in plans["deliveries"]] == [
        ("Spiritual", 5)
    ] * 4 + [("Dutch", 5)]
...
def test_preserves_supplies_equipped_and_storage_only_loot():
    ...
    rows[5]["slot"] = None
    f = snapshot("Parasite", 1, rows)
    m = snapshot("Dutch", 2)
    plan = plan_deliveries(
        f,
        [dict(character="Dutch", snapshot=m, ready=True, verified_travel_distance=1)],
        reserved=[17],
        now=100,
    )
...
def test_rejects_changed_offer_before_confirmation(change):
    i = item(10)
    f = snapshot("Parasite", 1, [i, item(11, 1050002)])
    m = snapshot("Dutch", 2)
    intent = prepare(f, m, [i], now=100)
    f, m = offered(intent)
    if change == "recipient":
        m["trade"]["participant_uid"] = 99
    if change == "currency":
        f["trade"]["other_silver"] = 1
    if change == "extra_item":
        m["trade"]["own_items"] = [item(99)]
    ...
    with pytest.raises(ValueError):
        validate_offers(intent, f, m, now=100)
...
def test_offered_item_can_leave_source_deque_but_success_needs_both_accounts():
    ...
    m = snapshot("Dutch", 2)
    intent = prepare(f, m, [i], now=100)
    of, om = offered(intent)
    of["inventory"] = of["inventory"][1:]
...
def test_unknown_result_is_journaled_and_cannot_retry(tmp_path, monkeypatch):
    ...
    class Driver:
        ...
        def require_qualified(self, c):
            pass

        def read_pair(self, m):
            return f, globals()["snapshot"]("Dutch", 2)
...
def test_receiver_waits_for_complete_batch_and_two_sided_receipt(tmp_path):
    ...
    f = snapshot("Parasite", 1, items)
    m = snapshot("Dutch", 2)
    state = r.reserve(j, "batch:1", f, m, items, now=100)
    of, om = offered(state["intent"])
    partial = copy.deepcopy(om)
    partial["trade"]["items"] = partial["trade"]["items"][:1]
...
def test_reserved_delivery_blocks_refill_even_after_planning(tmp_path):
    ...
    j = Journal(tmp_path / "journal.sqlite3")
    guard = InputCoordinator(lambda: True, path=tmp_path / "lock")
    f = snapshot("Parasite", 1, [item(10)])
    m = snapshot("Dutch", 2)
...
def test_bridge_never_takes_caller_snapshots_and_waits_for_input_owner():
    ...
    with pytest.raises(ValueError, match="arguments"):
        dispatch(
            ui,
            {"action": "delivery-pair", "character": "Dutch", "farmer": {"fake": True}},
        )
...
def test_source_journal_precedes_receiver_confirmation_and_lost_receipt_never_retrades(
    ...
    j = Journal(tmp_path / "source.sqlite3")
    calls = []
    keys = []
    ...
    class Driver:
        ...
        def read_pair(self, name):
            return offered(intent) if self.offered else (f, m)
        ...
        def wait_pair(self, name):
            return delivered(intent)
    ...
    class Peer:
        ...
        def ready(self, key, intent):
            ...
            assert (
                calls[-1] == "confirm"
            )  # Farmer releases input before receiver acceptance.
            calls.append("ready")
    ...
    with pytest.raises(OSError, match="Lost release"):
        t.run("Dutch", f["inventory"])
...
def test_source_recovery_needs_both_inventories_and_never_sends_input(
    tmp_path, monkeypatch
):
    from types import SimpleNamespace
    ...
    m = snapshot("Dutch", 2)
    intent = prepare(f, m, f["inventory"], now=100)
    af, am = delivered(intent)
    j = Journal(tmp_path / "source.sqlite3")
    j.begin("batch:1", "Dutch", "farmer_delivery", intent)
    j.transition("batch:1", "uncertain")
    calls = []
...
def test_exact_partial_disposition_is_terminal_and_idempotent_on_both_ledgers(
    tmp_path, monkeypatch
):
    ...
    f = snapshot("Parasite", 1, items)
    m = snapshot("Dutch", 2)
    intent = prepare(f, m, items, now=100)
    af = copy.deepcopy(f)
    am = copy.deepcopy(m)
    af["inventory"] = af["inventory"][1:]
    am["inventory"] = [copy.deepcopy(items[0])]
    trace = [
        {"stage": "action_trace", "status": "initialized", "payload": {}},
        {"stage": "offer_item:10", "status": "before_action", "payload": {}},
        {
            "stage": "reconciliation_observation",
            "status": "observed",
            "payload": {
                "ownership_digest": ownership_digest(intent, af, am),
                "observed_at": 94,
            },
        },
        {
    ...
    reservations.reserve(receiver, "batch:1", f, m, items, now=100)
    first = reservations.disposition(
        receiver, "Dutch", "batch:1", af, am, trace=trace, now=100
    )
    second = reservations.disposition(
        receiver, "Dutch", "batch:1", af, am, trace=trace, now=100
    )
    assert first["proof_digest"] == second["proof_digest"] == result["proof_digest"]
    assert (
        reservations.active(receiver, "Dutch") is None
        and receiver.get("Dutch", "new_stock") is True
    )


def test_reservation_preserves_origin_and_missing_reservation_can_close_no_input(
    tmp_path, monkeypatch
):
    from conquest.merchants import delivery_reservation as reservations
    ...
    state = reservations.reserve(
        journal, "batch:1", f, m, f["inventory"], origin=origin, now=100
    )
    assert all(state["intent"][name] == value for name, value in origin.items())
    with pytest.raises(ValueError, match="reused"):
        reservations.reserve(
            journal, "batch:1", f, m, f["inventory"], origin=None, now=100
        )

    missing = Journal(tmp_path / "missing.sqlite3")
    intent = prepare(f, m, f["inventory"], now=100)
    intent.update(origin)
    trace = [
        {"stage": "action_trace", "status": "initialized", "payload": {"version": 1}}
    ]
    result = reservations.disposition(
        missing, "Dutch", "batch:1", f, m, trace=trace, intent=intent, now=100
    )
    again = reservations.disposition(
        missing, "Dutch", "batch:1", f, m, trace=trace, now=100
    )
    assert (
        result["outcome"] == "no_transfer"
        and again["proof_digest"] == result["proof_digest"]
    )
    assert reservations.active(missing, "Dutch") is None


def test_source_no_transfer_recovery_commits_only_after_receiver_ack(
    tmp_path, monkeypatch
):
    monkeypatch.setattr("conquest.merchants.delivery.time.time", lambda: 100)
    f = snapshot("Parasite", 1, [item(10)])
    ...
    t = DeliveryTransaction(
        j, type("D", (), {"read_pair": lambda self, name: (f, m)})(), Peer()
    )
    assert t.recover("batch:1") == "batch:1"
    row = j.trace("batch:1")
    assert calls == [("batch:1", "no_transfer")]
    assert not j.pending("Dutch") and any(s["status"] == "aborted" for s in row)


def test_foreign_request_does_not_block_exact_no_transfer_but_farmer_request_does(
    monkeypatch,
):
    monkeypatch.setattr("conquest.merchants.delivery.time.time", lambda: 100)
    f = snapshot("Parasite", 1, [item(10)])
    ...
    foreign["request"] = {"participant": "SomeoneElse"}
    assert (
        reconciliation_outcome(intent, f, foreign, trace=trace, now=100)["outcome"]
        == "no_transfer"
    )
    own = copy.deepcopy(m)
    own["request"] = {"participant": "Parasite", "participant_uid": 1}
...
def test_attempted_request_needs_repeated_settlement_and_confirm_stays_uncertain(
    monkeypatch,
):
    monkeypatch.setattr("conquest.merchants.delivery.time.time", lambda: 100)
    f = snapshot("Parasite", 1, [item(10)])
    ...
    one = base + [
        {
            "stage": "reconciliation_observation",
            "status": "observed",
            "payload": {"ownership_digest": digest, "observed_at": 100},
        }
    ]
    ...
    stable = one + [
        {
            "stage": "reconciliation_observation",
            "status": "observed",
            "payload": {"ownership_digest": digest, "observed_at": 106},
        }
    ]
    assert (
        reconciliation_outcome(intent, f, m, trace=stable, now=106)["outcome"]
        == "no_transfer"
    )
    confirmed = stable + [
        {"stage": "farmer_confirm", "status": "before_action", "payload": {}}
    ]
    with pytest.raises(ReconciliationBlocked, match="stable terminal settlement"):
...
def test_recovery_collects_bounded_read_only_settlement_observations(tmp_path):
    ...
    class Peer:
        def disposition(self, key, saved, outcome):
            ...
            result = reconciliation_outcome(
                intent, farmer, merchant, trace=t.trace(key), now=now[0]
            )
            return {
                "request_id": key,
                "outcome": outcome,
    ...
    observations = [
        s for s in t.trace("batch:1") if s["stage"] == "reconciliation_observation"
    ]
...
def test_exact_verified_sale_receipt_is_normalized_into_delivery_proof(tmp_path):
    ...
    farmer = snapshot("Parasite", 1, [item(10)])
    merchant = snapshot("Dutch", 2)
    listed = item(99)
    listed.update(price=100)
    merchant["booth"] = [listed]
    merchant["timestamp"] = farmer["timestamp"] = 100
    intent = prepare(farmer, merchant, farmer["inventory"], now=100)
    journal = Journal(tmp_path / "receiver.sqlite3")
    observe(journal, merchant)
    current = copy.deepcopy(merchant)
    current.update(timestamp=101, booth=[], silver=197)
    observe(journal, current)
    current_farmer = copy.deepcopy(farmer)
    current_farmer["timestamp"] = 101
