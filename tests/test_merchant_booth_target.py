...
def booth():
    ...
    for a, fmt, values in [
        ...
        put(a, fmt, *values)
    session = NS(expected_sha256=CLIENT_SHA256,
    ...
    entity = dict(address=address,uid=103064,name='Spiritual',model=406,type_id=0,
                  position=[272,174],draw_position=[976,348])
    return NS(observer=NS(adapter=session,character='Spiritual'), entity=entity,
              base=base,address=address,graphics=graphics,put=put)
...
@pytest.mark.parametrize('case', ['foreign','uid','hit_kind','graphics','orientation',
                                 'code','camera','zoom','projection','torn','expired'])
def test_changed_or_unqualified_booth_has_no_target(booth,case,monkeypatch):
    b=booth.base;a=booth.address
    if case=='foreign':booth.entity['name']='Dutch'
    if case=='uid':booth.put(a+0x78,'<I',999)
    if case=='hit_kind':booth.put(a+0x2c0,'<I',0)
    if case=='graphics':booth.put(booth.graphics,'<Q',b+0x5d4850)
    if case=='orientation':booth.put(booth.graphics+0xc,'<i',-1)
    if case=='code':booth.put(b+0x262165,'<B',0)
    if case=='camera':booth.put(b+0x6545a0,'<I',2)
    if case=='zoom':booth.put(b+0x699360+0x26c,'<i',0)
    if case=='projection':booth.put(b+0x699360+0x44,'<i',12290)
    if case=='torn':
        read=booth.observer.adapter.read_block;seen=0
        def torn(addr,n):
            nonlocal seen
            if addr==a+0x78:
                seen+=1
                if seen>1:return struct.pack('<I',999)
            return read(addr,n)
        booth.observer.adapter.read_block=torn
    if case=='expired':
        ticks=iter([0,3])
        monkeypatch.setattr('conquest.merchants.booth_target.time.monotonic',lambda:next(ticks))
    with pytest.raises(ValueError):owned_booth_target(booth.observer,booth.entity)


@pytest.mark.parametrize('width,height,percent', [(1416,907,100),(1888,665,75),(2560,1440,125)])
def test_camera_size_and_zoom_are_applied_and_round_trip_to_booth(booth,width,height,percent):
    ...
    assert target['point']==tuple(round((v-d//2)*scale+d//2)
