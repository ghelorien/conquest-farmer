...
def test_diagnostics_find_hidden_merchants_without_changing_visibility(case):
    windows = [
        dict(
            hwnd=10,
            title="[Spiritual - ClassicConquer]",
            client_size=[1416, 907],
            visible=True,
        ),
        dict(
            hwnd=11,
            title="[Dutch - ClassicConquer]",
            client_size=[1888, 665],
            visible=False,
        ),
    ]
    if case == "missing":
        windows.pop()
    if case == "duplicate":
        windows.append(dict(windows[1], hwnd=12))
    if case == "launcher_only":
        windows[1]["client_size"] = [100, 100]
    catalog = NS(
        identities=lambda: [{"pid": 1}], backend=NS(windows=lambda pid: windows)
    )
    if case == "hidden":
...
def test_readonly_diagnostics_launches_the_script_from_its_source_release(
    ...
    monkeypatch.setattr(
        merchant_ui, "state_path", lambda value: str(tmp_path / Path(value).name)
    )
    monkeypatch.setattr(
        subprocess,
        "Popen",
    ...
    controller = type(
        "Controller", (), {"driver": type("Driver", (), {"memory": memory})()}
    )()
    ui = type("UI", (), {})()
    ui.runtime = type(
        "Runtime",
    ...
    result = merchant_ui.UnifiedUI.dispatch(
        ui, {"action": "start-readonly-diagnostics"}
    )
