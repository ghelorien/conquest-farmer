...
def test_reconnect_submits_once_after_stable_login_then_backs_off():
    ...
    assert len(calls) == 2
    r.step(False)
    assert events[-1]["state"] == "connection_restored"


def test_reconnect_does_not_fire_in_game_including_death():
    r = Reconnector(
        lambda: (_ for _ in ()).throw(AssertionError("Must not reconnect")),
        lambda event: None,
    )
    for _ in range(10):
        r.step(False)
...
def test_disconnected_vendor_reader_never_reads_stats(monkeypatch):
    ...
    monkeypatch.setattr(reconnect, "login_screen", lambda hwnd: True)
    monkeypatch.setattr(
        memory_life, "read_life", lambda *args: pytest.fail("Disconnected stats")
    )
...
def error_reader_fixture(monkeypatch, message=None, flag=1):
    ...
    window = SimpleNamespace(
        address=0x2000, position=(313.0, 354.0), size=(409.0, 84.0), scroll=(0.0, 0.0)
    )
    monkeypatch.setattr(
        memory_shop,
        "MemoryGui",
        lambda session: SimpleNamespace(base=0, read=lambda name: window),
    )
    message = (
        message or "Error: Connection with the server is interrupted. Please re-login. "
    ).encode()
    raw = struct.pack(
        "<QQQQBB", 0x30000, 0, len(message), max(79, len(message)), flag, 0
    )
    dc = bytearray(0x38)
    struct.pack_into("<2f", dc, 8, 714.0, 412.0)
    struct.pack_into("<f", dc, 16, 321.0)
    struct.pack_into("<f", dc, 0x34, 18.0)
    blocks = {0x698BE0 + 0x6E8: raw, 0x2000 + 0xE0: bytes(dc), 0x30000: message}
    session = SimpleNamespace(
        assert_identity=lambda: None,
        read_block=lambda address, size: blocks[address][:size],
    )
...
def test_reconnect_error_follows_live_width_and_wrapped_text_height(monkeypatch):
    ...
    window.position = (800.0, 500.0)
    window.size = (500.0, 120.0)
    dc = bytearray(blocks[0x20E0])
    struct.pack_into("<2f", dc, 8, 1292.0, 594.0)
    struct.pack_into("<f", dc, 16, 808.0)
    blocks[0x20E0] = bytes(dc)
    assert reader.read() == (1050, 603)
    window.size = (500.0, 110.0)
    with pytest.raises(ValueError, match="layout changed"):
        reader.read()
...
def test_exhausted_reconnect_reports_attention_once():
    ...
    def blocked():
        raise ValueError("Unrecognized login error")
...
def test_login_fields_and_button_follow_memory_form_origin(monkeypatch):
    ...
    monkeypatch.setattr(
        memory_shop, "MemoryGui", lambda session: SimpleNamespace(base=0)
    )
    digests = {
        b"fields": "2eeddcb3c87af4385b8b65fa1c77f8ebfa9c6da5bd18d6688e110eef6e192006",
    ...
    monkeypatch.setattr(
        ...
        "sha256",
        lambda data: (
            SimpleNamespace(hexdigest=lambda: digests[data])
            if data in digests
            else original(data)
        ),
    )
    ...
    for x, y in [(606.0, 562.0), (414.0, 397.0)]:
        window = SimpleNamespace(
            address=0x2000, position=(x, y), size=(208.0, 186.0), scroll=(0.0, 0.0)
        )
        dc = [
            x + 8,
            y + 182,
        ...
        assert login_form_points(session, window) == (
            (int(x + 104), int(y + 33)),
            (int(x + 104), int(y + 71)),
            (int(x + 104), int(y + 153)),
        )
    ...
    blocks[0x20E0] = struct.pack("<14f", *dc)
    with pytest.raises(ValueError, match="layout changed"):
        login_form_points(session, window)
