...
def cancelled(bilateral_fixture, monkeypatch):
    ...
    x.farmer = deepcopy(x.intent["farmer"])
    x.farmer["timestamp"] = 867.0
    x.merchant = deepcopy(x.intent["merchant"])
    x.merchant["timestamp"] = 867.0
    x.trace = [
        dict(
            stage="action_trace",
            status="initialized",
        ...
        dict(
            stage="trade_target_mode",
            status="before_action",
            payload={},
            timestamp=846.0,
        ),
        dict(
            stage="trade_request", status="before_action", payload={}, timestamp=847.0
        ),
        dict(
            stage="trade_request",
            status="observed",
            payload={"trade_open": True},
            timestamp=850.0,
        ),
        dict(
            stage="cleanup_trade", status="before_action", payload={}, timestamp=860.0
        ),
        dict(
            stage="cleanup_trade",
            status="observed",
            payload={"farmer_trade": False, "merchant_trade": False},
            timestamp=861.0,
        ),
    ]
    x.market["attempts"] = []
    ...
    with sqlite3.connect(x.receiver) as db:
        db.executescript(
            "CREATE TABLE sales(id INTEGER,character TEXT,observed_at REAL,phase TEXT,items TEXT,silver INTEGER);"
    ...
    def persist():
        ...
        x.trace.extend(
            dict(
                ...
                status="observed",
                payload={"ownership_digest": digest, "observed_at": at},
                timestamp=at,
            )
            for at in (862.0, 867.0)
        )
        x.result = delivery.reconciliation_outcome(
            ...
            x.merchant,
            trace=x.trace,
            sale_receipts=x.sales,
            now=867.0,
        )
        with sqlite3.connect(x.source) as db:
            db.execute(
                "UPDATE transactions SET phase='aborted',updated=868,before_json=?,result_json=?",
                (json.dumps(x.intent), json.dumps(x.result)),
            )
            db.execute(
                "UPDATE delivery_admissions SET items_json=?",
                (json.dumps(x.intent["items"]),),
            )
...
def test_exact_cancelled_no_transfer_preserves_every_original_asset(cancelled):
    ...
    assert proof[0]["cleanup_verified_at"] == 861.0
...
def test_incomplete_changed_or_uncertain_delivery_cannot_release_town(
    cancelled, change
):
    x = cancelled
    with sqlite3.connect(x.source) as db:
        ...
        if change == "no_cleanup":
            db.execute("DELETE FROM transaction_steps WHERE stage='cleanup_trade'")
        if change == "duplicate_cleanup":
            db.execute(
                "INSERT INTO transaction_steps SELECT id+100,transaction_id,stage,status,payload,timestamp FROM transaction_steps WHERE stage='cleanup_trade'"
            )
        if change in ("offered", "confirmed"):
            db.execute(
                ...
                (
                    99,
                    "delivery",
                    "offer_item:3" if change == "offered" else "farmer_confirm",
                    "before_action",
                    "{}",
                    851.0,
                ),
        ...
        if change == "short_settlement":
            db.execute(
                "DELETE FROM transaction_steps WHERE stage='reconciliation_observation' AND timestamp=862"
            )
        if change in ("lost_asset", "unexplained_silver"):
            result = deepcopy(x.result)
            if change == "lost_asset":
                result["farmer"]["inventory"].pop(0)
            else:
                result["merchant"]["silver"] += 1
            db.execute("UPDATE transactions SET result_json=?", (json.dumps(result),))
    if change == "receiver":
        with sqlite3.connect(x.receiver) as db:
            db.execute("UPDATE delivery_reservations SET state=?", ("{}",))
    if change == "active":
        x.route["active"] = {"request_id": "delivery"}
    if change == "route_digest":
        x.route["operations"][0]["proof_digest"] = "forged"
    write_json(delivery_route.STATE, x.route)
    with pytest.raises(ValueError):
        x.run()


def test_real_durable_sale_receipt_can_explain_only_receiver_booth_and_silver(
    cancelled,
):
    ...
    x.sales = [
        dict(id=1, phase="verified", observed_at=858.0, items=[listed], silver=silver)
    ]
    x.persist()
    expected, proof = x.run()
...
def test_active_route_can_only_be_prevalidated_for_read_only_settlement(cancelled):
    ...
    x.route["active"] = dict(
        request_id="delivery",
        items=x.intent["items"],
        merchant_identity=x.intent["merchant"]["identity"],
        merchant_uid=x.intent["merchant"]["character_uid"],
        **x.origin,
    )
    write_json(delivery_route.STATE, x.route)
    expected, proof = recovery.proof(
        x.row, x.meteor, x.market, x.failure, x.target, allow_terminal_active=True
    )
...
def test_rich_cleanup_payload_is_verified_without_treating_close_as_transfer(cancelled):
    ...
    payload = dict(
        farmer_trade=False,
        merchant_trade=False,
        historical_delivery_outcome="unknown",
        farmer=deepcopy(x.farmer),
        merchant=deepcopy(x.merchant),
        verified_new_sales=[],
    )
    for role in ("farmer", "merchant"):
        payload[role]["timestamp"] = 861.0
    with sqlite3.connect(x.source) as db:
        db.execute(
            "UPDATE transaction_steps SET payload=? WHERE stage='cleanup_trade' AND status='observed'",
            (json.dumps(payload),),
        )
...
def test_captured_no_transfer_selects_only_unchanged_warehouse_fallback(
    cancelled, monkeypatch, tmp_path
):
    ...
    x.market.update(
        phase="active", town_visit_id="town", farmer_profile_id="farmer", deadline=900.0
    )
    x.row.update(
        phase="town_work",
        pre_admission_restock_tail=dict(
            capture_kind="no_transfer",
            phase="captured",
            meteor=deepcopy(x.meteor),
            market=deepcopy(x.market),
            failure=x.failure,
            target=x.target,
            bag=expected,
            delivery_proofs=records,
        ),
    )
    path = tmp_path / "market.json"
    write_json(path, x.market)
    ...
    loop = NS(
        town_visit=NS(state=lambda: deepcopy(x.row)),
        town=lambda action: deepcopy(expected),
    )
    assert recovery.warehouse_fallback_only(loop, x.meteor) is True
    expected["silver"] += 1
    with pytest.raises(ValueError, match="ownership"):
        recovery.warehouse_fallback_only(loop, x.meteor)


@pytest.mark.parametrize(
    "command", ["delivery-status", "delivery-cleanup", "delivery-start"]
)
def test_pre_capture_settlement_cannot_issue_gameplay_commands(
    cancelled, monkeypatch, command
):
    ...
    loop = NS(
        ...
        health=lambda: dict(
            target=x.target, embedded_controls=dict(control=dict(enabled=False))
        ),
    ...
    if command == "delivery-status":
        recovery.settle_before_capture(
            loop, x.row, x.meteor, x.market, x.failure, x.target
        )
        assert len(calls) == 1
    else:
