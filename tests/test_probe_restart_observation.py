...
def test_verified_receipt_suppresses_restart_admission_until_farmer_attaches(
    ...
    retractions = []
    monkeypatch.setattr(
        restarted.manual_sessions,
        "retract_probe_admission",
    ...
    for _ in range(3):
        ...
        assert not restarted.process_probe_owned(
            "Dutch", x.read(), require_bilateral=True
        )
...
def test_absent_provider_does_not_use_trade_boundary_to_hide_a_queued_request(
    supervised,
):
    x = supervised
    request = x.read()
...
def test_absent_provider_retains_exact_durable_open_trade_observation_only(
    supervised, phase, offered
):
    x = supervised
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "target_profile",
        "farmer_profile",
        "saved_farmer_process",
        "saved_farmer_inventory",
        "saved_merchant_inventory",
        "saved_request_uid",
        "missing_farmer_receipt",
        "missing_merchant_receipt",
        "missing_verified_time",
        "receipt_future",
        "receipt_before_start",
        "receipt_after_verified",
        "other_farmer",
        "corrupt",
        "digest_race",
    ],
)
def test_absent_provider_never_hides_changed_request_or_receipt(
    supervised, monkeypatch, change
):
    x = supervised
    absent_source(x)
    if change == "request_uid":
        x.state["request"]["participant_uid"] += 1
    if change == "request_name":
        x.state["request"].update(
            participant="Visitor", message="Visitor wishes to trade with you."
        )
    if change == "request_extra":
        x.state["request"]["instance"] = "different"
    if change == "merchant_pid":
        x.state["identity"]["pid"] += 1
    if change == "merchant_creation_time":
        x.state["identity"]["creation_time_100ns"] += 1
    if change == "merchant_inventory":
        x.state["inventory"][0]["plus"] += 1
    if change == "merchant_silver":
        x.state["silver"] += 1
    if change == "merchant_slot":
        x.state["inventory"][0]["slot"] = 5
    if change == "target_profile":
        x.probe["target_profile_id"] = "other-target"
    if change == "farmer_profile":
        x.probe["farmer_profile_id"] = "other-farmer"
    if change == "saved_farmer_process":
        x.probe["farmer_after"]["identity"]["pid"] += 1
    if change == "saved_farmer_inventory":
        x.probe["farmer_after"]["inventory"][0]["plus"] += 1
    if change == "saved_merchant_inventory":
        x.probe["merchant_after"]["inventory"][0]["plus"] += 1
    if change == "saved_request_uid":
        x.probe["merchant_after"]["request"]["participant_uid"] += 1
    if change == "missing_farmer_receipt":
        x.probe.pop("farmer_after")
    if change == "missing_merchant_receipt":
        x.probe.pop("merchant_after")
    if change == "missing_verified_time":
        x.probe.pop("updated_at")
    if change == "receipt_future":
        x.probe["farmer_after"]["timestamp"] = 101
    if change == "receipt_before_start":
        x.probe["farmer_after"]["timestamp"] = 99
    if change == "receipt_after_verified":
        x.now = 102
        x.probe["farmer_after"]["timestamp"] = 101
    if change == "other_farmer":
        for farmer in (x.probe["intent"]["farmer"], x.probe["farmer_after"]):
            farmer["character"] = "Visitor"
    ...
    x.save()
    if change == "corrupt":
        x.path.write_text("{corrupt", encoding="utf-8")
...
@pytest.mark.parametrize(
    "phase", ["pending", "approved", "decline_claimed", "needs_attention"]
)
def test_restart_observation_leaves_all_existing_manual_sessions_unchanged(
    supervised, phase
):
    x = supervised
    store = x.runtime.manual_sessions
    ...
    if phase == "needs_attention":
        store.observe(row["id"], {"reader_error": "unavailable"}, now=x.now)
    x.runtime._sync_manual_fence()
    before = store.get(row["id"])
    audit = store.audit(row["id"])
    absent_source(x)
    x.now += 1
