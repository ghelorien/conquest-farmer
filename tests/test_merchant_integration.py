...
def test_whole_farmer_action_excludes_merchant_even_between_send_batches(tmp_path):
    ...
    try:
        with input_scope():
            ...
            thread = threading.Thread(target=merchant)
            thread.start()
            thread.join()
    ...
    try:
        with guard.lease("Dutch"):
            ...
            result = subprocess.run(
                [sys.executable, "-c", code, str(guard.path)], capture_output=True
            )
...
def test_native_tabs_bridge_authentication_handoff_and_global_stop(
    ...
    app = SimpleNamespace(
        ...
        stats_text=tk.StringVar(value=""),
        activity_text=tk.StringVar(value="Returning from Market"),
        stop=stop,
    )
    ...
    try:
        assert [unified.notebook.tab(t, "text") for t in unified.notebook.tabs()] == [
            "Overview",
            "Farmer",
            "Spiritual",
            "Dutch",
        ...
        assert (
            enlarged_size[0] > initial_size[0] + 350
            and enlarged_size[1] > initial_size[1] + 200
        )
        ...
        status_label = next(
            child
            for parent in unified.frames["Dutch"].winfo_children()
        ...
        status, recovery, help_text = (
            item[0] for item in unified.merchant_chrome["Dutch"]
        )
        ...
        assert [
            packed.index(widget)
            for widget in (status, controls, recovery, help_text, tabs)
        ] == sorted(
            packed.index(widget)
            for widget in (status, controls, recovery, help_text, tabs)
        )
        ...
        assert (
            unified.dispatch({"action": "status"})["header"]["timers"]
            == unified.timer_text.get()
        )
        ...
        call = urllib.request.Request(
            f"http://127.0.0.1:{info['port']}/merchants", data=b'{"action":"status"}'
        )
        with pytest.raises(urllib.error.HTTPError) as error:
            urllib.request.urlopen(call)
        assert error.value.code == 403
        with pytest.raises(ValueError):
            request({"action": "credentials", "password": "invalid"})
...
def test_merchant_reads_run_concurrently_without_sharing_observers(tmp_path):
    ...
    for character in ("Spiritual", "Dutch"):
        runtime.observers[character] = SimpleNamespace(
...
def test_pid_reuse_discards_stale_merchant_and_recovers_only_that_character(tmp_path):
    ...
    runtime.observers["Dutch"] = SimpleNamespace(
        adapter=SimpleNamespace(assert_identity=invalid),
        close=lambda: closed.append("Dutch"),
    )
    runtime.controllers["Dutch"] = object()
    runtime.latest["Dutch"] = {"identity": "stale"}

    def absent(character):
        raise ValueError("Not logged in")

    runtime.attach = absent
    runtime.recover = lambda character, crashed=False: recovered.append(
        (character, crashed)
    )
    runtime.step("Dutch")
...
def test_gui_reader_accepts_live_frames_that_advance_between_rpcs():
    ...
    frames = iter((100, 110))
    memory.read_block = lambda address, size: (
        struct.pack("<I", next(frames))
...
def test_gui_registry_distinguishes_draw_order_from_membership(change):
    ...
    memory = Memory()
    context, array = 0x11000, 0x15000
    windows = (0x16000, 0x17000)
    memory.put(0x10000, "<Q", context)
    memory.put(context + 0x3E58, "<IIQ", 2, 2, array)
    ...
    def read(address, size):
            ...
            if reads[0] > 1:
                return struct.pack(
                    ...
                    *(
                        windows[::-1]
                        if change == "reorder"
                        else (windows[0], 0x18000)
                        if change == "replace"
                        else (windows[0], windows[0])
                        if change == "duplicate"
                        else windows
                    ),
                )
        if address == 0x10000:
            contexts[0] += 1
            if change == "context" and contexts[0] > 1:
                return struct.pack("<Q", 0x12000)
...
def test_runtime_reconciles_submitted_request_decline_after_modal_disappears(
    tmp_path, monkeypatch
):
    from conquest.merchants.runtime import MerchantRuntime
...
def test_delivery_window_holds_unreserved_stock_but_allows_reserved_request(
    tmp_path, monkeypatch
):
    from conquest.merchants.runtime import MerchantRuntime
