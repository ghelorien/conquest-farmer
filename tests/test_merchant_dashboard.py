...
def test_on_sale_counts_listing_prices_once_and_excludes_inventory_and_stale_stock():
    ...
    states = {
        'Spiritual': {'connected': True, 'snapshot': {'timestamp': 100, 'booth_open': True,
            'booth': [{'price': 1000000, 'quantity': 10}, {'price': 250000, 'quantity': 1}],
            'inventory': [{'price': 9000000}]}},
        'Dutch': {'connected': True, 'snapshot': {'timestamp': 100, 'booth_open': True,
...
def test_header_uses_lifetime_verified_receipts_and_respects_pause():
    sales = {'characters': {
        'Spiritual': {'started_at': 0, 'last_observed_at': 100,
                      'total': {'silver': 5000000}, 'period': {'silver': 1}},
        'Dutch': {'started_at': 0, 'last_observed_at': 100,
                  'total': {'silver': 900000}, 'unconfirmed': 2}}}
    states = {'Spiritual': {'enabled': False, 'scan': {'next_scan': 110}},
              'Dutch': {'enabled': True, 'scan': {'next_scan': 200}}}
    result = header_text(sales, {'status': 'waiting', 'next_due': 3700}, states, now=100)
...
def test_missing_tracking_and_report_failure_are_visible():
    result = header_text({}, {'status': 'needs_configuration', 'next_due': 1, 'retry_at': 160}, {}, now=100)
    ...
    assert 'due now' in header_text({}, {'status': 'waiting', 'next_due': 1}, {}, now=100)['timers']
def test_status_distinguishes_refill_from_paused_operations_and_explains_blockers():
    ...
    state={'connected':True,'enabled':False,'refill':{'enabled':True,'next_check':400},
           'snapshot':{'inventory':[],'booth':[],'capacity':40},'scan':{'pending':True}}
    text=merchant_text(state,now=100)
    assert 'Shop update paused' in text and 'Resume shop update' in text and 'Auto-refill ON' in text
    state['market_refresh']={'pending':True,'phase':'fetching'}
    assert 'Downloading America' in merchant_text(state,now=100)
    state['pending']=[{'before_json':'{"item":{"name":"Ring"}}',
...
def test_active_listing_intent_is_progress_not_an_attention_error():
    from conquest.merchants.dashboard import merchant_text
    state={'connected':True,'enabled':True,'input_active':True,'scan':{'pending':True,'request_id':'a'},
        'batch_progress':{'request_id':'a','changed':2,'remaining':3,'item':'Ring'},
