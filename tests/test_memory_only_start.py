...
def test_memory_only_start_rejects_before_discovery_or_elevation(monkeypatch, calibration, clients):
    from conquest import desktop_app
    messages, records = [], []
    def forbidden(*args, **kwargs): pytest.fail('Memory-only --start must not discover, select or elevate')
    app = NS(profile=Path(__file__).resolve().parents[1]/'profiles/desktop-foreground.example.yaml',
             state_text=NS(set=messages.append), record=lambda **fields: records.append(fields),
             refresh_client=forbidden, catalog=NS(windows=forbidden), client=clients[0] if clients else None)
    monkeypatch.setattr(desktop_app, 'elevated_start', forbidden)
    desktop_app.DesktopApp.start(app, calibration)
    assert '--embed-client --client-pid PID --client-started CREATION_TIME --client-hwnd HWND' in messages[-1]
...
def test_cli_rejects_incomplete_or_misapplied_pin_before_gui(monkeypatch, arguments):
    ...
    monkeypatch.setattr(desktop_app.tk, 'Tk', lambda: pytest.fail('Invalid pin must fail before GUI'))
...
def test_existing_embedding_path_accepts_only_one_exact_identity():
    ...
    assert pinned_client(NS(windows=lambda: [other, selected]), selected.key) is selected
