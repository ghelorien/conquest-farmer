...
def rig(tmp_path, monkeypatch):
    ...
    state = NS(control={'enabled':False, 'revision':7},
        life={'object_address':0x100000, 'status':0x10, 'dead_candidate':False,
              'position':[10, 10], 'map_id':1011, 'current_hp':100, 'max_hp':100},
        target={'pid':123, 'created_at':123., 'path':'fixture'},
        manual=False, clicks=[], calls=[], events=[], post_error=None)
    ...
    def dispatch(operation, body):
        ...
        if operation == 'health':
            return {'target':deepcopy(state.target), 'window':{
                'client_size':[1036, 793], 'foreground':1, 'root_hwnd':1, 'hwnd':1,
                'minimized':False}}
        assert operation == 'foreground-click'
        return click(body)

    def snapshot():
        return {'control':deepcopy(state.control), 'life':deepcopy(state.life),
                'observed_at':time.time()}

    monkeypatch.setattr('conquest.mouse_priority.active', lambda:state.manual)
    service = EmbeddedBridge(NS(session=NS(pid=123), dispatch=dispatch,
        target=NS(snapshot=lambda:{'root_hwnd':1, 'hwnd':1})),
        threading.RLock(), tmp_path/'bridge.json', snapshot)
    ...
    care._xp_skill = xp_skill.XpSkill(observer,
        lambda event, fields:state.events.append({'event':event, **fields}))
    monkeypatch.setattr(travel_care, 'resolve_player', lambda *_:{'name':100, 'max_hp':200})
...
@pytest.mark.parametrize('mutation', ['not_ready', 'flying', 'popup', 'dead',
                                      'manual', 'enabled', 'stop', 'fence'])
...
def test_generic_and_postinput_failures_do_not_become_retryable_xp_passes(
    ...
    if failure == 'os_lease':
        import msvcrt
        def denied(*args):raise OSError('OS lease unavailable')
        monkeypatch.setattr(msvcrt, 'locking', denied)
    else:
        rig.post_error = CaptureUnavailable('Waiting for the current input action'
            if failure == 'same_text' else 'SendInput(mouse-down) failed')
    with pytest.raises(ValueError) as error:
        rig.care.check(rig.health())
...
def route_loop(rig, monkeypatch):
    ...
    loop.care = rig.care
    loop.stepper = NS(step_to=lambda *args, **kwargs:pytest.fail('Unexpected route dispatch'))
...
def test_outer_route_checks_stop_and_process_before_retrying_xp(rig, monkeypatch, mutation):
    ...
    with held_by_peer(rig.owner) as release:
        def check(health):
            try:original(health)
                ...
                if mutation == 'stop':
                    def stopped():raise OvernightStopped('Stopped by user')
...
def test_new_route_request_rejects_stale_map_position_or_blocked_terrain(monkeypatch, changed):
    ...
    sender = route_input.RouteJumpInput(NS(adapter=None, health_layout=None, character='Parasite'), terrain)
    sender.recovery_input.send = lambda *args:pytest.fail('Stale movement dispatched')
    if changed == 'map':life.map_id = 1002
    elif changed == 'position':life.position = (11, 10)
    else:terrain.blocked[10, 11] = True
    with pytest.raises(ValueError):
        sender({'source':[10, 10], 'destination':[20, 10], 'map_id':1011,
                'expires_at':time.time()+3})


@pytest.mark.parametrize('error_type', [InputAcquisitionBusy, CaptureUnavailable, ValueError])
