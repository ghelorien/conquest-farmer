...
def test_live_worker_is_reused_without_discovery_or_serve(monkeypatch):
    monkeypatch.setattr(launcher, "request", lambda *args: {
        "protocol_version": 2, "expected_sha256":
        "c2b53437ef68d687a1ef0f70c74bcf2df6027bf82b558e93330c839eb5e1c396",
        "read_only": True, "memory_read_revision": 1})
...
@pytest.mark.parametrize("change", [{"expected_sha256": "wrong"},
    {"read_only": False}, {"memory_read_revision": 0}, {"protocol_version": 1}])
def test_incompatible_worker_is_not_reused(monkeypatch, tmp_path, change):
    reply = {"protocol_version": 2, "expected_sha256": "expected",
             "read_only": True, "memory_read_revision": 1, **change}
