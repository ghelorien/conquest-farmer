...
@pytest.mark.parametrize(
    "consumed,gained", [(True, True), (False, True), (True, False), (False, False)]
)
def test_memory_inventory_potion_requires_consumption_and_hp(consumed, gained):
    ...
    before = replace(BEFORE, items=(item,))
    after = replace(before, items=() if consumed else (item,))
    clicked = []

    def life(**kwargs):
        return NS(
            object_address=123, current_hp=100 if clicked and gained else 30, max_hp=100
        )

    grid = GuiWindow(1, "Inventory/grid", (50, 100), (407.0, 175.0), (0.0, 0.0))
    ...
    trade = NS(
        life=life,
        inventory=NS(read=lambda: after if clicked else before),
        shop=NS(gui=NS(read=gui)),
        click=lambda point, button: clicked.append((point, button)),
        verified_read=verify,
    )
    if consumed and gained:
        result = consume_inventory_potion(trade, item.uid)
        assert (
            result["consumed"]
            and result["hp_after"] == 100
            and result["remaining"] == 0
        )
    else:
        with pytest.raises(ValueError, match="unverified"):
            consume_inventory_potion(trade, item.uid)
...
def test_wrong_potion_or_invalid_slot_never_sends_input(item):
    ...
    trade = NS(
        life=lambda **kw: NS(current_hp=30, max_hp=100),
        inventory=NS(read=lambda: replace(BEFORE, items=(item,))),
