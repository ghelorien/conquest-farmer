...
@pytest.mark.parametrize(
    ...
    [
        ...
        "due",
        "pending",
        "pause_during_collection",
        "denied",
    ],
)
def test_script_schedule_preserves_intent_and_requires_complete_fresh_data(
    tmp_path, mode
):
    now = time.time()
    state = {
        "characters": {
            name: {"enabled": False, "scan": {}} for name in ("Spiritual", "Dutch")
        }
    }
    merchant = state["characters"]["Dutch"]
    merchant["enabled"] = mode != "paused"
    if mode == "not_due":
        merchant["scan"] = {"next_scan": now + 1}
    if mode == "one_time":
        merchant["scan"] = {"pending": True, "one_time": True}
    if mode == "pending":
        merchant["scan"] = {"pending": True, "request_id": "old"}
    calls = []
    ...
    def collect(**kwargs):
        ...
        if mode == "denied":
            raise ValueError("Website denied access")
        if mode == "pause_during_collection":
    ...
    if requests:
        assert requests[0]["request_id"] == f"12h:{int(now // 43200)}"
    if mode in ("paused", "not_due", "one_time"):
        assert not any(c["action"] == "collect" for c in calls)
...
def test_rollout_failure_prevents_network_and_requests(tmp_path):
    ...
    with pytest.raises(ValueError, match="Missing receipts"):
        run_cycle(
            ...
            collect=collect,
            verify=verify,
            market_path=tmp_path / "missing.json",
        )
...
def test_recent_market_is_shared_between_merchants(tmp_path):
    ...
    market.write_text(json.dumps(snapshot()))
    statuses = {
        "characters": {c: {"enabled": True, "scan": {}} for c in ("Spiritual", "Dutch")}
    }
    bridge = Mock(return_value=statuses)
    collect = Mock()
    result = run_cycle(
        bridge=bridge, collect=collect, verify=lambda: None, market_path=market
    )
...
class TablePage:
    ...
    def get_by_text(self, pattern):
        return SimpleNamespace(
            inner_text=lambda: (
                "51 matching items"
                if "matching" in pattern.pattern
                else "Last booth change: "
                + ("new" if self.changed and self.number == 2 else "same")
            )
        )
    ...
    def get_by_role(self, role, name=None, exact=False):
        ...
        locator.inner_text.return_value = (
            ...
            if self.filtered and name == "Quality"
            else "\u200b"
        )
        locator.is_enabled.side_effect = lambda: (
            self.number == 1 if name == "Go to next page" else self.number > 1
        )
        locator.click.side_effect = lambda: setattr(self, "number", self.number + 1)
        locator.get_by_role.return_value = locator
        locator.evaluate_all.side_effect = lambda expression: (
            [self.row] * (50 if self.number == 1 else 1)
        )
        return locator
...
def test_progressive_count_must_settle_before_pagination():
    ...
    def read():
        return min(3, int(ticks[0] * 5)) * 400, "unchanged"
