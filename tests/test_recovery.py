...
def recovery(**kwargs):
    config = RecoveryConfig(
        enabled=True, return_route=((430, 380), (434, 384)), **kwargs
    )
...
def test_return_defers_to_healing_and_stops_on_repeated_failed_movement():
    r = recovery()
    assert tick(r, 21, health=0.39, position=(430, 380)) is None
    assert r.phase == RecoveryPhase.RETURNING
    for now in (22, 24, 26):
        assert tick(r, now, health=0.8, position=(430, 380)).kind == "return_walk"
    assert tick(r, 28, health=0.8, position=(430, 380)) is None
    assert r.reason == "return_movement_failure_limit"


def test_on_site_resurrection_completes_without_town_walk():
    r = recovery()
    assert tick(r, 21, health=0.8) is None
