...
@pytest.mark.parametrize(
    "equipped,reserve,expected,reload",
    [
        (0, 0, "ammo_unavailable", True),
        (1, 0, "ammo_unavailable", True),
        (2, 0, "ammo_unavailable", True),
        (3, 0, None, False),
        (1, 1000, None, True),
        (2, 3, None, True),
        (2, 2, "ammo_unavailable", True),
    ],
)
def test_scatter_needs_three_in_one_stack_or_reloads(
    equipped, reserve, expected, reload
):
    ...
    items = (POTION,) + (
        (replace(AMMO, uid=3, amount=reserve, slot=1),) if reserve else ()
    )
...
def test_single_attack_can_use_last_arrow():
    assert (
        supply_stop_reason(
            replace(SNAPSHOT, equipped_ammo=replace(AMMO, amount=1)), CONFIG, 10.5
        )
        is None
    )
...
def test_patrol_does_not_create_partly_used_arrow_packs(remaining):
    ...
    config = SimpleNamespace(
        ammo_type=1050000, attack_button="right", jump_scatter=True
    )
    bag = replace(
        SNAPSHOT,
        items=(replace(AMMO, uid=3, slot=0),),
        equipped_ammo=replace(AMMO, amount=remaining),
    )
