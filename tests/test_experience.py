...
def test_experience_uses_decoded_level_requirements_and_coherent_sample():
    blocks={100+0x6e8:struct.pack('<I',2),100+0x708:struct.pack('<Q',30),
            100+0x1120:struct.pack('<II',100,200)}
    session=SimpleNamespace(assert_identity=lambda:None,read=lambda address,size:blocks[address])
    ...
    session.read=lambda address,size:next(calls) if address==100+0x708 else blocks[address]
