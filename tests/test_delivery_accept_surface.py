...
def test_accept_control_binds_distinct_rendered_confirm_to_raw_trade_model(monkeypatch):
    ...
    handlers = {
        base + 0x95FD6: bytes.fromhex("e835dcfaff"),
    ...
    labels = [
        "Trade###Confirm",
        "Parasite wishes to trade with you.",
        "Accept",
        "Cancel",
    ]
    monkeypatch.setattr(
        "conquest.merchants.delivery_accept_probe.string",
    ...
    snapshot = {
        "windows": [
            {
                "name": "Open Booth###Confirm",
                "address": window,
                "geometry": [844, 282, 200, 100],
            }
        ]
    }
    assert control(driver, snapshot) == (
        {**snapshot["windows"][0], "model_address": model},
        (944, 360),
    )


@pytest.mark.parametrize("address", [0, True])
def test_accept_control_rejects_invalid_rendered_confirmation_address(
    monkeypatch, address
):
...
def test_accept_control_rejects_trade_when_open_booth_confirmation_also_exists(
    monkeypatch,
):
    ...
    snapshot = {
        "windows": [
            ...
            {
                "name": "Open Booth###Confirm",
                "address": window + 0x400,
                "geometry": [1, 1, 200, 100],
            },
        ]
    }
...
@pytest.mark.parametrize(
    "drift", [("geometry", [844, 282, 201, 100]), ("handler", None), ("label", None)]
)
def test_accept_control_rejects_layout_signature_or_raw_button_drift(
    monkeypatch, drift
):
    ...
    def read(pointer, size):
        if pointer == model + 12:
            return b"\x01"
        if pointer in (window, window + 0x400):
            return bytes(raw[:size])
        return handlers[pointer][:size]
    ...
    snapshot = {
        "windows": [
            {"name": "Open Booth###Confirm", "address": window, "geometry": geometry}
        ]
    }
...
def accept_run_fixture(monkeypatch):
    ...
    farmer = {
        "character": "Parasite",
        "character_uid": 55,
        "identity": {"pid": 8, "creation_time_100ns": 10, "path": "ImConquer.exe"},
        "server": "America",
        "position": [10, 10],
        "silver": 200,
        "inventory": [item],
    ...
    state = {
        ...
        "updated_at": 1,
        "intent": {
            "farmer": deepcopy(farmer),
            "merchant": {**deepcopy(merchant), "request": None},
            "items": [item],
        },
    }
    ...
    ui = NS(
        ...
        runtime=NS(
            controllers={"Spiritual": NS(driver=game_driver)},
    ...
    monkeypatch.setattr(
        delivery_probe_ownership, "ownership", lambda *_args, **_kwargs: {}
    )
    monkeypatch.setattr(farmer_preferences, "permits_new_delivery", lambda *_args: None)
    monkeypatch.setattr(
        "conquest.merchants.delivery_accept_probe.control",
        lambda *_args: (
            {"model_address": 2, "address": 1, "geometry": [1, 1, 200, 100]},
            (100, 100),
        ),
    )
    monkeypatch.setattr(
        "conquest.merchants.delivery_accept_probe.write_json", lambda *_args: None
    )
    monkeypatch.setattr(desktop_runtime, "physical_coordinates", nullcontext)
    monkeypatch.setattr(
        driver_module, "wait_hover_validation", lambda guard, _check: guard()
    )
...
def test_accept_preserves_both_participant_holds_before_any_lease(monkeypatch, held):
    ...
    ui.coordinator.lease = lambda *_a, **_kw: pytest.fail(
        "Held peer must prevent focus/lease"
    )
    monkeypatch.setattr(
        foreground, "foreground_click", lambda *_a, **_kw: pytest.fail("No click")
    )
...
def test_accept_keeps_coordinator_mutex_through_lease_handoff(monkeypatch):
    ...
    with pytest.raises(ReachedLease):
        run(ui, state)
...
def live_accept_control(ui, monkeypatch, *, stage, model=0x100000, window=0x200000):
    ...
    def active_model():
        return model + 0x100 if stage[0] == "model" and stage[1] else model

    def read(pointer, size):
        if pointer == active_model() + 12:
            return b"\x01"
        if pointer in (window, window + 0x400):
            return bytes(raw[:size])
        if pointer in handlers:
            code = handlers[pointer]
            return (b"\0" * len(code) if stage[0] == "handler" and stage[1] else code)[
                :size
            ]
    ...
    def strings(_session, pointer):
        ...
        if stage[0] == "raw_string" and stage[1]:
            labels[2] = "Yes"
        offset = pointer - active_model() - 0x48
        if offset not in (0, 0x20, 0x40, 0x60):
            return "changed"
        return labels[offset // 0x20]

    gui = NS(
        base=base,
        model=lambda *_args: active_model(),
        viewport_size=lambda: [1000, 800],
        assert_hovered=Mock(),
    )
    driver = NS(
        target=NS(snapshot=lambda: {"client_size": [1000, 800]}),
    ...
    monkeypatch.setattr(
        "conquest.merchants.delivery_accept_probe.farmer_name", lambda: "Parasite"
    )
    return gui, window
...
def live_confirm_snapshot(stage, window=0x200000):
    name = (
        "Trade###Confirm"
        if stage[0] == "label" and stage[1]
        else "Open Booth###Confirm"
    )
    address = window + 0x400 if stage[0] == "address" and stage[1] else window
    geometry = (
        [844, 282, 201, 100]
        if stage[0] == "geometry" and stage[1]
        else [844, 282, 200, 100]
    )
    return {
        "name": name,
        "address": address,
...
def test_accept_prepress_ignores_stale_label_and_scroll_change_for_same_raw_control(
    monkeypatch,
):
    ...
    stage = ["label", False]
    hovered, window = live_accept_control(ui, monkeypatch, stage=stage)
    merchant["windows"] = [live_confirm_snapshot(stage, window)]

    def pairs(*_args):
        current = deepcopy(merchant)
        current["windows"] = [live_confirm_snapshot(stage, window)]
...
@pytest.mark.parametrize(
    "kind", ["address", "model", "geometry", "raw_string", "handler"]
)
def test_accept_prepress_control_drift_never_reaches_press(monkeypatch, kind):
    ...
    monkeypatch.setattr(delivery_accept_probe, "control", control)
    stage = [kind, False]
    _gui, window = live_accept_control(ui, monkeypatch, stage=stage)
    merchant["windows"] = [live_confirm_snapshot(stage, window)]
...
def test_accept_rejects_one_wrong_rendered_confirm_when_accept_hover_id_is_absent(
    monkeypatch,
):
    ...
    monkeypatch.setattr(delivery_accept_probe, "control", control)
    stage = ["label", False]
    gui, window = live_accept_control(ui, monkeypatch, stage=stage)
    ...
    gui.assert_hovered.side_effect = ValueError(
...
@pytest.mark.parametrize(
    ...
    [
        ("participant_uid", 56),
        ("message", "Parasite wishes to trade?"),
        ("server", "Europe"),
    ],
)
def test_accept_first_fresh_request_mutation_never_calls_foreground_click(
...
def test_accept_before_press_request_mutation_never_reaches_press(
    monkeypatch, field, value
):
    ui, state, farmer, merchant, foreground = accept_run_fixture(monkeypatch)
    monkeypatch.setattr(
        "conquest.merchants.delivery_accept_probe.pair",
        lambda *_args: (deepcopy(farmer), deepcopy(merchant)),
    )
    presses = []
...
def accept_lease_ui(tmp_path, monkeypatch, identity):
    ...
    monkeypatch.setattr("conquest.character_context.registry", lambda: None)
    observer = NS(
        adapter=NS(identity=identity, assert_identity=Mock()),
        operations=NS(target=NS(hwnd=77)),
    )
    runtime = NS(
        observers={"Spiritual": observer},
    ...
    ui = NS(
        coordinator=NS(
            purpose="delivery_accept_probe",
            manual_session_blocked=lambda *_args, **_kwargs: False,
        ),
        runtime=runtime,
        delivery_probe_thread=NS(ident=threading.get_ident(), is_alive=lambda: True),
        calibrating={"Spiritual"},
        calibration_cancel={"Spiritual": threading.Event()},
    )
    monkeypatch.setattr(
        "conquest.merchants.delivery_reservation.active", lambda *_args: None
    )
    return ui, observer


def test_disabled_exact_accept_probe_has_its_own_narrow_lease_permission(
...
def test_accept_probe_lease_rejects_competing_or_changed_work(
    tmp_path, monkeypatch, blocker
):
    identity = {"pid": 7, "creation_time_100ns": 9, "path": "ImConquer.exe"}
    ui, observer = accept_lease_ui(tmp_path, monkeypatch, identity)
    if blocker == "window":
        ui.runtime.delivery_window = "ordinary-delivery"
    elif blocker == "refill_window":
        ui.runtime.refill_window = "refill"
    elif blocker == "refilling":
        ui.runtime.refilling = {"Spiritual": 0}
    elif blocker == "reservation":
        monkeypatch.setattr(
            "conquest.merchants.delivery_reservation.active",
            lambda *_args: {"phase": "reserved"},
        )
    elif blocker == "pending":
        ui.runtime.journal.pending = lambda _character: True
    elif blocker == "cancelled":
        ui.calibration_cancel["Spiritual"].set()
    elif blocker == "wrong_worker":
        ui.delivery_probe_thread.ident = threading.get_ident() + 1
    elif blocker == "wrong_purpose":
        ui.coordinator.purpose = "trade"
    elif blocker == "wrong_identity":
        observer.adapter.identity = {**identity, "pid": 8}
    assert not lease_authorized(ui, "Spiritual")


@pytest.mark.parametrize("hold_phase", ["manual_active", "approval_pending"])
def test_accept_probe_lease_checks_uuid_scoped_manual_hold_before_disabled_probe(
    tmp_path, monkeypatch, hold_phase
):
    ...
    ui = NS(
        coordinator=NS(
            purpose="delivery_accept_probe",
            manual_session_blocked=lambda value, **_kwargs: seen.append(value) or True,
        ),
        runtime=runtime,
        delivery_probe_thread=NS(ident=threading.get_ident(), is_alive=lambda: True),
        calibrating={key},
        calibration_cancel={key: threading.Event()},
    )
...
def bind_accept_binding(ui):
    ui.delivery_accept_binding = lambda character, expected=None: (
        UnifiedUI.delivery_accept_binding(ui, character, expected)
    )
...
def bound_show_ui(identity, *, layout=None, pane_size=(1200, 800)):
    api = NS(
        assert_owner=Mock(),
        show_async=Mock(),
        gui=NS(GetForegroundWindow=lambda: 0),
        backend=NS(),
    )
    host = NS(
        saved=NS(hwnd=77, identity=identity),
        api=api,
        mode="owned",
        resize=Mock(),
        detach=Mock(),
    ...
    ui = NS(
        ...
        frames={"Spiritual": "spiritual-frame"},
        detail_tabs={"Spiritual": NS(select=Mock())},
        root=NS(update_idletasks=Mock()),
        input_bookmarks={},
        client_panes={
            "Spiritual": NS(
                winfo_width=lambda: pane_size[0], winfo_height=lambda: pane_size[1]
            )
        },
        apply_client_compact_layout=layout,
    )
...
def managed_registry_stub():
    return NS(
        resolve=lambda profile_id, **_kwargs: NS(
            id="Spiritual", name="Spiritual", server="America", local_enabled=True
        )
    )
...
def managed_show_ui(tmp_path, monkeypatch, identity):
    ...
    for name, value in [
        ("observers", observer),
        ("hosts", host),
        ("frames", "spiritual-frame"),
        ("detail_tabs", ui.detail_tabs["Spiritual"]),
        ("client_panes", ui.client_panes["Spiritual"]),
    ]:
        mapping = ProfileMap()
        mapping[key] = value
        if name == "observers":
            ui.runtime.observers = mapping
        else:
            setattr(ui, name, mapping)
...
def test_accept_surface_embeds_only_the_exact_observer(tmp_path, monkeypatch):
    ...
    ui = NS(
        runtime=NS(observers={"Spiritual": observer}),
        hosts={
            "Spiritual": NS(saved=NS(hwnd=77, identity=identity), api=api, mode="owned")
        },
        embed_merchant=Mock(),
        show_merchant=Mock(),
        embed_delivery_accept_merchant=Mock(),
        show_delivery_accept_merchant=Mock(),
    )
    bind_accept_binding(ui)
    ...
    ui.show_delivery_accept_merchant.assert_called_once_with(
        "Spiritual", (77, identity, "Spiritual")
    )
    ui.show_merchant.assert_not_called()


def test_accept_surface_rejects_changed_saved_host_without_showing_or_reembedding(
    tmp_path, monkeypatch
):
    saved = NS(
        hwnd=78, identity={"pid": 7, "creation_time_100ns": 9, "path": "ImConquer.exe"}
    )
    api = NS(assert_owner=Mock())
    identity = {"pid": 7, "creation_time_100ns": 9, "path": "ImConquer.exe"}
    request_verified(tmp_path, monkeypatch, identity)
...
@pytest.mark.parametrize(
    "changed",
    [
        ("phase", "accept_submitted"),
        ("character", "Dutch"),
        ("identity", {"pid": 8, "creation_time_100ns": 10}),
    ],
)
def test_accept_surface_requires_active_journal_binding_before_window_action(
    ...
    field, value = changed
    if field == "identity":
        state["intent"]["merchant"]["identity"] = {**value, "path": "ImConquer.exe"}
    else:
        state[field] = value
...
def test_accept_surface_rechecks_after_specialized_embed_or_show_observer_swap(
    ...
    api = NS(assert_owner=Mock())
    host = NS(
        saved=None if phase == "embed" else NS(hwnd=77, identity=identity),
        api=api,
        mode="owned",
        attach=Mock(),
        detach=Mock(),
    ...
    replacement = NS(
        ...
        operations=NS(target=NS(hwnd=88)),
    )
    runtime = NS(observers={"Spiritual": observer})

    def swap(*_args):
        runtime.observers["Spiritual"] = replacement
    ...
    with pytest.raises(ValueError, match="exact merchant process"):
        UnifiedUI.prepare_delivery_accept_surface(ui, "Spiritual")
    if phase == "embed":
        ui.embed_delivery_accept_merchant.assert_called_once_with(
            "Spiritual", (77, identity, "Spiritual")
        )
...
def test_actual_specialized_show_rejects_undersized_attached_pane_before_resize(
    tmp_path, monkeypatch
):
    identity = {"pid": 7, "creation_time_100ns": 9, "path": "ImConquer.exe"}
    request_verified(tmp_path, monkeypatch, identity)
    ...
    with pytest.raises(ValueError, match="viewport"):
        UnifiedUI.show_delivery_accept_merchant(
            ui, "Spiritual", (77, identity, "Spiritual")
        )
...
def test_actual_specialized_show_rechecks_observer_after_layout_before_resize(
    tmp_path, monkeypatch
):
    identity = {"pid": 7, "creation_time_100ns": 9, "path": "ImConquer.exe"}
    request_verified(tmp_path, monkeypatch, identity)
    ...
    ui.apply_client_compact_layout = lambda: ui.runtime.observers.update(
        Spiritual=replacement
    )
    with pytest.raises(ValueError, match="exact merchant process"):
        UnifiedUI.show_delivery_accept_merchant(
            ui, "Spiritual", (77, identity, "Spiritual")
...
@pytest.mark.parametrize(
    "fault", ["missing", "disabled", "renamed", "different_profile", "other_server"]
)
def test_managed_accept_profile_binding_fails_before_any_tab_or_window_mutation(
    tmp_path, monkeypatch, fault
):
    identity = {"pid": 7, "creation_time_100ns": 9, "path": "ImConquer.exe"}
    registry, profile, key, ui, host, api = managed_show_ui(
    ...
    state = delivery_probe.read_probe()
    if fault == "missing":
        state["target_profile_id"] = "missing-profile"
    elif fault == "disabled":
        registry.update(profile.id, {"local_enabled": False})
    elif fault == "renamed":
    ...
    with pytest.raises(ValueError, match="profile|request-verified"):
        UnifiedUI.show_delivery_accept_merchant(
            ui, "Spiritual", (77, identity, profile.id)
        )
...
def test_managed_profile_name_binds_plain_journal_character_for_specialized_show(
    tmp_path, monkeypatch
):
    identity = {"pid": 7, "creation_time_100ns": 9, "path": "ImConquer.exe"}
    _registry, profile, key, ui, host, _api = managed_show_ui(
        tmp_path, monkeypatch, identity
    )
...
def test_managed_prepare_input_uses_profile_key_after_callback(tmp_path, monkeypatch):
    ...
    ui.grant_fence = None
    ui.prepare_delivery_accept_surface = lambda character: (
        UnifiedUI.prepare_delivery_accept_surface(ui, character)
    )
    ui.show_delivery_accept_merchant = lambda character, expected: (
        UnifiedUI.show_delivery_accept_merchant(ui, character, expected)
    )
...
def test_ui_callback_failure_preserves_callback_and_cause_details():
    ...
    assert detail == (
        "Embedded client UI action failed: RuntimeError: handoff callback failed"
