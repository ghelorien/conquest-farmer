...
def test_alive_route_is_not_duplicated_and_stopped_route_can_restart(monkeypatch,tmp_path):
    ...
    calls=[]
    monkeypatch.setattr(r.subprocess,'Popen',lambda *a,**kw:calls.append((a,kw)) or SimpleNamespace(pid=2))
    monkeypatch.setattr(routes,'RouteLibrary',lambda root:SimpleNamespace(load=lambda route:None))
...
def test_controller_process_lock_blocks_duplicates_and_releases(tmp_path):
    with r.controller_guard(tmp_path/'lock') as first:
        assert first
        with r.controller_guard(tmp_path/'lock') as duplicate:assert not duplicate
    with r.controller_guard(tmp_path/'lock') as later:assert later
