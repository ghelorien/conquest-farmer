...
def dialog(monkeypatch):
    ...
    data={'records':records,'window':SimpleNamespace(position=(378.,20.),size=(280.,198.),scroll=(0.,0.)),
          'table':(398.,130.,618.,88.)}
...
def test_choices_use_live_dialog_table_position(dialog):
    ...
    dialog['window'].position=(478.,120.)
    dialog['table']=(498.,230.,718.,88.)
...
def test_dialog_reader_copies_and_rechecks_deque_and_strings(monkeypatch):
    actor,table,shared,record,window=0x100000,0x200000,0x300000,0x400000,0x500000
    memory={actor+0x1060:struct.pack('<4Q',table,1,0,1),table:struct.pack('<Q',shared),
            shared:struct.pack('<2Q',record,0)}
    raw=bytearray(48);struct.pack_into('<3I',raw,0,1,32,0)
    raw[16:22]=b'Market';struct.pack_into('<2Q',raw,32,6,15);memory[record]=bytes(raw)
    dc=bytearray(56);struct.pack_into('<2f',dc,8,618,130);struct.pack_into('<f',dc,16,398)
    struct.pack_into('<f',dc,52,88);memory[window+0xe0]=bytes(dc)
    adapter=SimpleNamespace(read_block=lambda a,n:memory[a][:n])
    observer=SimpleNamespace(adapter=adapter,health_layout=None,character='Parasite')
    monkeypatch.setattr(c,'read_life',lambda *a:SimpleNamespace(object_address=actor,dead_candidate=False))
    monkeypatch.setattr(c,'MemoryGui',lambda s:SimpleNamespace(read=lambda name:SimpleNamespace(address=window)))
...
def test_uncertain_teleport_is_not_paid_again(monkeypatch,tmp_path,dialog):
    ...
    path.write_text(json.dumps({'trips':[dict(destination_map=1011,source_map=1002,verified=True,
        option='Phoenix Castle',arrival_map=1002,arrival_position=[958,555],price=100)]}))
    monkeypatch.setattr(c,'TRIPS',path);monkeypatch.setattr(c.time,'sleep',lambda seconds:None)
    ...
    calls.clear()
    with pytest.raises(ValueError,match='source map'):c.take_saved_trip(loop,1011)
    assert calls==[]
