...
def terrain():
    return SimpleNamespace(travel_path=lambda a, b, **kw: line_tiles(a, b),
                           walkable=lambda p: True, portals=set())


def probe():
    return {'reason': 'recipient_absent', 'point': None, 'farmer_position': [230, 228],
            'merchant_position': [230, 173], 'occupied_tiles': [], 'viewport': [1416, 876]}


def test_failed_ingress_click_uses_another_checked_landing_not_a_fake_obstacle():
    world = terrain(); sample = probe(); source = tuple(sample['farmer_position'])
...
def test_alternative_preserves_solid_tile_guard():
    world = terrain(); sample = probe()
    world.walkable = lambda p: p == tuple(sample['farmer_position'])
    assert ingress_position(world, sample) is None


def test_approach_reobserves_after_failed_leg_then_moves_to_different_ingress(monkeypatch):
    from conquest.merchants import delivery_route
    from conquest.travel_progress import TravelStalled
    sample = probe(); source = tuple(sample['farmer_position']); moves = []; events = []
    loop = SimpleNamespace(terrain=terrain(), check_stop=lambda: None,
