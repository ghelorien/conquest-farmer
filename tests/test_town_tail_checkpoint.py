...
def test_restart_settles_only_proved_final_write_without_replaying_work(tail):
    ...
    loop.town_visit = t.TownVisit(
        visit.path, clock=visit.clock, probe=lambda: {}, profile="farmer"
    )
    bag["timestamp"] = 20.0  # Fresh read time is not changed ownership.
    assert t.resume_verified_tail(loop)
    assert visit.state()["town_work_completed_kind"] == "restock"
    assert (
        visit.state()["phase"] == "town_work"
    )  # A new verified hunt is still required.
...
def test_legacy_unfinished_town_work_remains_held_even_with_full_supplies(tail):
    ...
    with pytest.raises(ValueError, match="reconciliation"):
        visit.require_town_work_complete()
    assert not calls
...
def test_changed_or_unsafe_final_boundary_never_completes_or_sends_input(
    tail, monkeypatch, changed
):
    from conquest import urgent_town_recovery, overnight
    ...
    if changed == "process":
        health["target"]["pid"] = 8
    if changed == "map":
        health["embedded_controls"]["life"]["map_id"] = 1036
    if changed == "route":
        loop.route.id = "other"
    if changed == "silver":
        bag["silver"] += 1
    if changed == "item":
        bag["items"][0]["uid"] = 3
    if changed == "ammo":
        bag["equipped_ammo"]["amount"] -= 1
    if changed == "capacity":
        bag["capacity"] = 41
    if changed == "manual":
        health["embedded_controls"]["manual_input_fence"] = True
    if changed == "mouse":
        health["embedded_controls"]["manual_mouse"] = True
    if changed == "dead":
        health["embedded_controls"]["life"]["dead_candidate"] = True
    if changed == "stale":
        health["embedded_controls"]["observed_at"] = 990
    if changed == "hold":
        monkeypatch.setattr(urgent_town_recovery, "transaction_holds", lambda: True)
    if changed == "supplies":
        monkeypatch.setattr(overnight, "needs_town", lambda *a: True)
...
def test_completion_write_failure_preserves_proof_for_exact_read_only_retry(
    tail, monkeypatch
):
    loop, visit, health, bag, calls, now = tail
    t.checkpoint_verified_tail(loop, "restock")
    ...
    monkeypatch.setattr(
        t, "write_json", lambda *a: (_ for _ in ()).throw(OSError("disk full"))
    )
