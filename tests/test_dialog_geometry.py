...
@pytest.mark.parametrize(
    "width,origin,viewport",
    [
        (280, (100, 20), (1020, 754)),
        (356, (378, 20), (1420, 1009)),
        (480, (900, 400), (1920, 1080)),
    ],
)
def test_every_saved_service_choice_handles_moved_resized_dialogs(
    step, width, origin, viewport
):
    options = [r for r in step["records"] if r["kind"] == 1]
    rows = math.ceil(len(options) / 2)
    x, y = origin
    data = {
        "records": step["records"],
        "window": {
            "position": origin,
            "size": (width, 160 + rows * 22),
            "scroll": (0, 0),
        },
    ...
    for i, choice in enumerate(options):
        ...
        assert px == round(x + 20 + (i % 2 + 0.5) * (width - 40) / 2)
        assert py == y + 110 + (i // 2) * 22 + 11


def choices(top=80, scroll=0):
    return {
        "records": [{"kind": 1, "option": i, "text": str(i)} for i in range(8)],
...
def test_client_edges_also_clip_choices():
    ...
    with pytest.raises(ValueError, match="clips"):
        option_point(data, "2", (800, 110))
    with pytest.raises(ValueError, match="horizontally"):
        option_point(data, "1", (300, 754))


@pytest.mark.parametrize(
    "failure", ["duplicate", "input", "nan", "row_height", "narrow"]
)
def test_unqualified_layout_never_produces_point(failure):
    data = choices()
    if failure == "duplicate":
        data["records"][1]["text"] = "0"
    if failure == "input":
        data["records"].append({"kind": 2, "text": "amount"})
    if failure == "nan":
        data["table"] = (120, float("nan"), 436, 88)
    if failure == "row_height":
        data["table"] = (120, 80, 436, 90)
    if failure == "narrow":
        data["window"]["size"] = (40, 120)
...
def test_saved_route_scrolls_to_selected_option_before_selection(monkeypatch):
    ...
    def town(action, **kw):
        ...
        if action == "service-scroll-dialog":
            assert kw["option"] == "6"
            data["table"] = (120, 30, 436, 88)
            data["window"]["scroll"] = (0, 50)
    ...
    assert m.select_saved_dialog(
        loop, "MillionaireLee", {"records": data["records"], "option": "6"}
    ) == {"interacted": True}
...
@pytest.mark.parametrize(
    "option,top,scroll,direction", [("6", 80, 0, -1), ("0", 30, 50, 1)]
)
def test_service_scroll_uses_live_geometry_and_exact_option(
    monkeypatch, option, top, scroll, direction
):
    from types import SimpleNamespace as NS
    from conquest import market_services as m, foreground

    data = choices(top, scroll)
    data["window"] = NS(**data["window"])
    npc = NS(position=(200, 200), draw_position=(400, 400), entity_id=42)
    observer = NS(
        adapter=NS(viewport_size=lambda: (1420, 1009)),
        entities=None,
    ...
    trade = NS(
        observer=observer,
        service_npc=npc,
        life=lambda **kw: NS(map_id=1036, position=(200, 200)),
        input_attempted=False,
        click=lambda p: pytest.fail("Use the wheel, not a guessed scrollbar button"),
    )
    monkeypatch.setattr(m, "discover", lambda *a: (None, npc))
    monkeypatch.setattr(m, "read_dialog", lambda *a: data)
    calls = []
    monkeypatch.setattr(foreground, "foreground_scroll", lambda *a: calls.append(a))
    body = {
        "action": "service-scroll-dialog",
        "name": "MillionaireLee",
...
def test_twin_city_scrolls_before_fare_and_honors_stop(monkeypatch):
    ...
    def town(action, **kw):
        ...
        if action == "service-dialog":
            return data
        data["table"] = (120, 60, 436, 88)
        data["window"]["scroll"] = (0, 20)
