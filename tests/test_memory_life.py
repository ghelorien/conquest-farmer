...
@pytest.mark.parametrize('status,appearance,expected',[
    (0x420,98,True),(0x620,98,True),(0x200,0,False),(0x20,98,False),
    (0x400,98,False),(0x420,99,False),(0x420,0,False),(0,98,False)])
def test_ghost_candidate_requires_both_observed_indications(status,appearance,expected):
...
class Session:
    ...
    identity={'pid':1}
    def __init__(self):
        self.data={0x10030:struct.pack('<Q',0x420),0x100c0:struct.pack('<I',98),
                   0x100d8:struct.pack('<II',435,453),0x20000:struct.pack('<I',1002),
...
def reader(monkeypatch):
    ...
    monkeypatch.setattr(life,'resolve_player',lambda s,p:{'object':0x10000,'position':0x100d8,'map':0x20000})
    monkeypatch.setattr(life,'MemoryHealthReader',lambda *args,**kwargs:SimpleNamespace(
...
def test_recycled_player_pointer_rejects_observation(monkeypatch):
    ...
    def resolve(*args):
        nonlocal calls
        calls+=1
        return {'object':0x10000 if calls<=2 else 0x30000,'position':0x100d8,'map':0x20000}
