...
def run(tmp_path):
    ...
    s = dict(
        ...
        silver=1000,
        inventory=[],
        booth=[item(1, 100), item(2, 900)],
        booth_open=True,
        trade=None,
        request=None,
        windows=[{"name": "Inventory"}],
    )
    r.remember(s)
    r.save(
        {
    ...
    def check():
        if not active[0]:
            raise CaptureUnavailable("paused")
        guard.check()
    ...
    t = SimpleNamespace(
        read=lambda: copy.deepcopy(s),
        qualify_movement=lambda: None,
        move=lambda before, destination, check: (
            calls.append(("move", destination)) or {**s, "position": list(destination)}
        ),
        prepare_transfer=lambda before, check: ["Market"],
        transfer=lambda records, check: calls.append(("transfer", records)),
        stall=lambda preferred: {"position": preferred},
        prepare_shop=lambda chosen, check: copy.deepcopy(s),
        start_shop=lambda before, check: calls.append(("shop", before)),
    )
...
def test_reconnect_routes_to_conductress_before_stock_reconciliation(run):
    run.s.update(
        map_id=1002, position=[400, 400], inventory=[], booth=[], booth_open=False
    )
...
def test_arrival_requires_correct_fare_then_routes_to_stall(run):
    run.r.save(
        run.r.state(), "transfer_submitted", transfer_silver=1100, submitted_at=100
    )
...
def test_wrong_fare_never_claims_recovery(run):
    run.r.save(
        run.r.state(), "transfer_submitted", transfer_silver=1500, submitted_at=80
    )
    with pytest.raises(ValueError, match="uncertain"):
        run.r.step(run.s, run.c, run.t)
...
def test_pause_and_farmer_handoff_prevent_movement(run):
    ...
    with pytest.raises(CaptureUnavailable, match="Paused"):
        run.r.step(run.s, run.c, run.t)
    run.active[0] = True
    run.c.coordinator.safe_to_yield = lambda: False
    with pytest.raises(CaptureUnavailable, match="handoff"):
        run.r.step(run.s, run.c, run.t)
...
def test_closed_shop_records_intent_and_never_blindly_repeats(run):
    ...
    assert len(run.calls) == 1
    run.now[0] += 11
    with pytest.raises(ValueError, match="Shop setup"):
        run.r.step(run.s, run.c, run.t)
...
def test_shopflag_name_alone_never_proves_vacancy(monkeypatch):
    ...
    with pytest.raises(ValueError, match="qualification"):
        vacant_flags(observer, {})
    assert (
        vacant_flags(
            observer,
            {"vacancy_offset": 64, "vacancy_mask": 0xFFFFFFFF, "vacancy_value": 0},
        )
        == []
    )
    monkeypatch.setattr(memory, "unpack", lambda *args: (0,))
    assert vacant_flags(
        observer, {"vacancy_offset": 64, "vacancy_mask": 0xFFFFFFFF, "vacancy_value": 0}
    ) == [flag]


@pytest.mark.parametrize(
    "panel,capability,method,phase",
    [
        ("booth", "booth_panel", "open_owned_booth", "panel_submitted"),
        ("inventory", "inventory_panel", "open_inventory", "inventory_submitted"),
    ],
)
def test_missing_owned_panel_opens_once_and_reconciles_after_restart(
    run, panel, capability, method, phase
):
...
def test_owned_booth_never_claims_another_flag_without_panel_qualification(run):
    ...
    def unqualified(cap):
        raise ValueError("pending qualification")
...
def test_recovery_trial_preserves_manual_intervention(run, assisted):
    ...
    trial["before"]["timestamp"] -= 0.02
    if assisted:
        trial["manual_interventions"] = ["User completed Conductress transfer"]
    run.j.set("Dutch", "recovery_trial", trial)
    assert run.r.step(run.s, run.c, run.t)
    state = run.r.state()
    result = run.j.get("Dutch", "recovery_trial")
    ...
    if assisted:
        assert state["manual_interventions"] == trial["manual_interventions"]
