...
def test_gui_resize_during_read_is_rejected():
    ...
    gui, _ = gui_fixture()
    sizes = iter([(1420, 1009), (1600, 1100)])
...
def test_revive_tracks_memory_control_bar_without_stretching_coordinates(monkeypatch):
    control = NS(position=(245.0, 907.0), size=(930.0, 102.0), scroll=(0.0, 0.0))
    monkeypatch.setattr(
        "conquest.memory_shop.MemoryGui", lambda s: NS(read=lambda name: control)
    )
    assert revive_point(NS(), (1420, 1009)) == (710, 856)
    control.position = (250.0, 907.0)
    with pytest.raises(ValueError, match="anchoring"):
        revive_point(NS(), (1420, 1009))
...
def test_player_anchor_and_jump_projection_use_new_viewport():
    ...
    assert memory_player_anchor(
        NS(adapter=adapter), NS(position=(214, 259), object_address=100000)
    ) == (1100, 820)
...
@pytest.mark.parametrize("expected_size", [None, (1420, 1009), (1036, 793)])
@pytest.mark.parametrize("resize", [False, True])
def test_warehouse_typing_accepts_current_size_but_stops_if_resized(
    monkeypatch, resize, expected_size
):
    import ctypes
    ...
    target = NS(
        hwnd=1,
        backend=NS(user=None, foreground=lambda: 2),
    ...
    monkeypatch.setattr(
        "conquest.win32.bind",
        lambda user, name, *args: send if name == "SendInput" else lambda vk: 0,
    )
    if expected_size == (1036, 793):
        with pytest.raises(CaptureUnavailable, match="focus"):
            type_amount(target, 123, expected_size=expected_size)
        assert (
            not events
        )  # Merchant calibration must not be replaced by the current dimensions.
    elif resize:
        with pytest.raises(CaptureUnavailable, match="focus"):
            type_amount(target, 123, expected_size=expected_size)
