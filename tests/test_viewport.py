...
def test_warehouse_typing_accepts_current_size_but_stops_if_resized(monkeypatch,resize,expected_size):
    import ctypes
    ...
    if expected_size == (1036,793):
        ...
        assert not events  # Merchant calibration must not be replaced by the current dimensions.
    elif resize:
        with pytest.raises(CaptureUnavailable,match='focus'):type_amount(target,123,expected_size=expected_size)
