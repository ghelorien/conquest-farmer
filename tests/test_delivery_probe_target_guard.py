...
def test_supervised_request_guard_ignores_other_players_but_rejects_target_drift(monkeypatch,change):
    ...
    def account(name,uid,stock):
        return dict(character=name,character_uid=uid,identity={'pid':uid},server='America',
            timestamp=time.time(),map_id=1036,hp=100,silver=200,capacity=40,position=[10,10],
            inventory=stock,booth=[],trade=None,request=None,windows=[{'name':'##Control'}])
    farmer,merchant=account('Parasite',1,[item]),account('Spiritual',2,[])
    intent=deepcopy({'farmer':farmer,'merchant':merchant,'items':[item]})
    recipient={'address':100,'uid':2,'name':'Spiritual','position':[10,10],
               'point':[400,300],'occupied_tiles':[[10,10],[20,20],[30,30]]}
    changed=deepcopy(recipient)
    if change=='occupied_move':changed['occupied_tiles'][1]=[21,20]
    ...
    if change=='position':changed['position']=[11,10]
    if change=='point':changed['point']=[401,300]
    calls=[];presses=[];saved=[];targeting=[False]
    ...
    def click(target,x,y,size,**kwargs):
        ...
        if len(presses)==1:
            targeting[0]=True
            if change=='post_hud_position':farmer['position']=[11,10]
        else:merchant['request']={'participant':'Parasite','participant_uid':1}
    gui=NS(viewport_size=lambda:[1000,800],assert_hovered=lambda *args,**kwargs:None)
    ...
    monkeypatch.setattr(probe,'write_probe',lambda path,state:saved.append(deepcopy(state)))
    ...
    if change.startswith('occupied_'):
        ...
        with pytest.raises((ValueError,CaptureUnavailable)):probe.run(ui,intent,1,state)
        assert presses==[(500,600)]
        assert state['phase']==('targeting_verified' if change=='post_hud_position' else 'request_submitted')
