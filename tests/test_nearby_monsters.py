...
def test_refresh_groups_ids_and_retains_species_selection(picker):
    ...
    monsters = [
        {"name": "Pheasant", "entity_id": 81, "type_id": 1},
        {"name": "Pheasant", "entity_id": 82, "type_id": 1},
    ]
    ...
    widget.refresh(
        monsters + [{"name": "Pheasant", "entity_id": 83, "type_id": 1}], [1]
    )
...
def test_unchanged_refresh_does_not_move_rows_or_rewrite_note(picker, monkeypatch):
    ...
    widget.refresh(monsters, [1])
    monkeypatch.setattr(
        widget.tree, "move", lambda *a, **k: pytest.fail("Unnecessary row move")
    )
    monkeypatch.setattr(
        widget.note, "set", lambda *a: pytest.fail("Unnecessary text redraw")
    )
    widget.refresh(monsters, [1])
...
def test_live_labels_and_ids_cannot_change_embedded_sidebar_width(
    ...
    app = DesktopApp(
        tk_root,
        "unused.yaml",
        catalog=SimpleNamespace(windows=lambda: []),
        output=tmp_path,
    ...
    try:
        ...
        for count, note in [
            (0, "Waiting"),
            (128, "Current HP and monster life state need live validation; " * 3),
            (1, "Ready"),
        ]:
            app.memory_text.set(note)
            app.render_matched_ids(
                {
                    "observations_available": True,
                    "control": {"resolved_target_ids": list(range(count))},
                }
            )
            app.nearby.refresh(
                [
                    {"name": "Pheasant", "entity_id": i, "type_id": 1}
        ...
        app.render_matched_ids(
            {"observations_available": True, "control": {"resolved_target_ids": [0]}}
        )
        app.render_matched_ids(
            {"observations_available": False, "control": {"resolved_target_ids": []}}
        )
