...
def test_level_read_rejects_stale_dead_or_changed_character(monkeypatch):
    ...
    state = {
        "embedded_controls": {
            "observed_at": 10,
            "life": {"object_address": 1000, "dead_candidate": False},
        }
    }
...
def test_every_bracket_checks_both_ends_and_saved_monster(entry):
    for level in entry["levels"]:
        ...
        if route:
            ...
            assert route.monster_type_ids == tuple(
                m["type_id"] for m in lr.monster_family(entry["monster_type_id"])
            )
        else:
