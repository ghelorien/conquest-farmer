...
def scene(monkeypatch):
    ...
    def sample(session, fields):
        if fields == headers:
            return [begin, begin + 16, begin + 16]
        if fields == [(actor, "u64")]:
            return [base + 0x5C5E20]
        if fields == []:
            return []
    ...
    observer = NS(
        adapter=s,
        character="Spiritual",
        entities=NS(layout=p, _resolve=lambda: (base, collection, [])),
    )
    peer = {
        "character_uid": 1173856,
        "character": "Dutch",
        "position": [211, 196],
        "identity": {"pid": 2},
    }
...
def test_cross_client_peer_requires_exact_identity_and_position(scene, mismatch):
    if mismatch == "uid":
        scene.peer["character_uid"] += 1
    if mismatch == "name":
        scene.peer["character"] = "Spiritual"
    if mismatch == "position":
        scene.peer["position"] = [212, 196]
    if mismatch:
        with pytest.raises(ValueError):
            identity_evidence.peer_evidence(scene.observer, scene.peer)
    else:
