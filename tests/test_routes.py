...
def definition():
    return SavedRoute(id='test',name='Test route',map_id=1002,monster_type_ids=(1,),
        recommended_levels=(1,7),town_anchor=(1,1),hunting_anchor=(5,5),
        hunting_boundary=(4,4,6,6),patrol=((4,4),(5,5)),
        outbound_waypoints=((1,1),(1,5),(5,5)),return_waypoints=((5,5),(1,5),(1,1)),
        terrain_sha256='a'*64,tasks=('travel_to_hunt','hunt_and_loot','return_to_town',
            'restock','check_equipment','resume_hunt'))
...
@pytest.mark.parametrize('changes',[
    {'id':'../outside'}, {'recommended_levels':(12,7)},
    {'patrol':((50,50),)}, {'outbound_waypoints':((2,2),(5,5))},
...
def test_travel_replans_from_live_position_and_checks_map_version():
    ...
    assert plan_travel(route,terrain,(5,5),phase='patrol',patrol_index=2)['destination']==[4,4]
...
def test_supply_return_counts_equipped_arrows_and_all_spare_stacks():
    route=definition()
    inventory=InventorySnapshot(0,1,(Item(1,1050000,100,200,0),Item(2,1050000,5,200,1),
        Item(3,1000000,3,99,2)),Item(4,1050000,95,200,None),500,40)
    ...
    assert route.return_reasons(replace(inventory,items=(),equipped_ammo=None))==[
        'arrows_low','healing_supplies_low']
...
def test_route_families_include_nearby_level_variants_but_never_bosses_or_far_higher_bandits():
    ...
    for kind in (8302,1401,55,79,8102,8202):
        with pytest.raises(ValueError):route_monster_names(route.model_copy(update={'monster_type_ids':(7,kind)}))
