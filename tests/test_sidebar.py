...
def test_lower_controls_reachable_without_resizing_game_pane():
    ...
    root.withdraw()
    side = ScrollableSidebar(root)
    side.pack(side="left", fill="y")
    side.embedded()
    pane = tk.Frame(root)
    pane.pack(side="right", fill="both", expand=True)
    for i in range(40):
        ttk.Label(side.content, text=f"Field {i}").pack(fill="x", pady=3)
    footer = ttk.Button(side.content, text="Client tools")
    footer.pack()
    try:
        root.deiconify()
        root.update()
        ...
        side.canvas.yview_moveto(1)
        root.update()
        assert (
            footer.winfo_rooty() + footer.winfo_height()
            <= side.canvas.winfo_rooty() + side.canvas.winfo_height()
        )
