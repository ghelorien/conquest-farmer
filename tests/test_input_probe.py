...
def test_health_snapshot_fences_only_access_denied_cursor_observation():
    ...
    target.backend = SimpleNamespace(
        window_pid=pid,
        client_rect=rect,
        foreground=lambda: 7,
        iconic=lambda _root: False,
...
def test_positioned_probe_restores_cursor_without_activating_game():
    ...
    assert (
        result["focus_unchanged"]
        and result["cursor_restored"]
...
def test_mouse_motion_can_settle_before_a_button_is_dispatched():
    ...
    def sleep(delay):
        waits.append((delay, len(target.messages)))

    result = click_probe(
        target, 10, 20, [800, 600], move_settle_seconds=0.2, sleep=sleep
    )
    assert waits[0] == (0.2, 1)
    assert result["move_settle_seconds"] == 0.2
...
def test_desktop_change_while_motion_settles_prevents_button_dispatch():
    ...
    def sleep(delay):
        target.state["foreground"] = 100

    with pytest.raises(ValueError, match="only mouse movement"):
        click_probe(target, 10, 20, [800, 600], move_settle_seconds=0.2, sleep=sleep)
    assert [message[0] for message in target.messages] == [0x200]


@pytest.mark.parametrize(
    "key,value",
    [("foreground", 100), ("minimized", True), ("client_size", [1000, 600])],
)
...
def test_background_control_chord_uses_only_window_messages():
    target = FakeTarget()
    result = key_probe(
        target, 0x7A, 0x57, [800, 600], control_scan=0x1D, sleep=lambda _: None
    )
...
def test_modified_mouse_probe_releases_control_when_mouse_move_fails():
    ...
    def fail(message, wparam, lparam):
        if message == 0x200:
            raise OSError("Mouse move queue failed")
