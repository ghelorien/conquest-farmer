...
def test_reload_and_inventory_cleanup_use_logical_coordinates(monkeypatch, fails):
    ...
    def trade(body):
        assert active[0]
        calls.append(body)
        if fails and body["action"] == "equip-arrows":
            raise ValueError("Uncertain receipt")
        return {"equipped": 7}

    s = n.NativeFarmSupervisor.__new__(n.NativeFarmSupervisor)
    s.observer = NS(town_trade=trade)
    s.dispatch = lambda callback: callback()
    bag = NS(items=[NS(uid=7, type_id=1050001, amount=1000)])
    if fails:
        with pytest.raises(ValueError, match="Uncertain receipt"):
            s.reload_arrows(bag, 1050001)
    else:
        assert s.reload_arrows(bag, 1050001) == {"equipped": 7}
    assert calls == [
        {"action": "equip-arrows", "uid": 7},
...
@pytest.mark.parametrize(
    "failure", [None, "Equipment changed before equip input", "Uncertain receipt"]
)
def test_cleanup_failure_preserves_reload_result_and_retries_cleanup(
    monkeypatch, failure
):
    ...
    if failure:
        expected = (
            CaptureUnavailable
            if failure.startswith("Equipment changed")
            else ValueError
        )
...
def test_reload_after_guard_rejection_selects_fresh_reserve(monkeypatch):
    ...
    assert s.reload_arrows(
        NS(items=[NS(uid=8, type_id=1050001, amount=900)]), 1050001
    ) == {"equipped": 8}
    assert attempts == [7, 8]
