...
def town(coordinator, monkeypatch):
    ...
    shop = ShopSnapshot(123, (product,),
        GuiWindow(1, 'Shop', (0., 0.), (288., 268.), (0., 0.)),
        GuiWindow(2, 'Shop/Items', (20., 38.), (248., 200.), (0., 0.)))
    state = SimpleNamespace(
        inventory=InventorySnapshot(1., 1., (), None, 1000, 40),
        shop=shop, vendor=SimpleNamespace(entity_id=123),
        invalid=None, clicks=[], reads=[], requests=[], events=[], living=0,
        after_wait=lambda:None, click_error=None)
    ...
    def inventory():
        state.reads.append(('inventory', state.inventory.silver, len(state.inventory.items)))
    ...
    def click(*args, **kwargs):
        ...
        item = Item(100+len(state.inventory.items), 1000020, 1, 1, len(state.inventory.items))
        state.inventory = replace(state.inventory, items=(*state.inventory.items, item),
...
def test_sequential_verified_purchases_wait_then_buy_only_remaining_count(town, coordinator):
    assert town.loop.buy_supply(3, 1000020)
    first = next(fields['receipt'] for event, fields in town.events if event == 'purchase')
    ...
    assert len([event for event, _ in town.events if event == 'town_observation_retry']) == 1
...
@pytest.mark.parametrize('mutation,expected', [
    ('vendor', 'Vendor is not'), ('shop', 'different vendor'),
    ('product', 'not sold'), ('inventory', 'inventory room'),
    ('silver', 'Insufficient funds'), ('life', 'living character'),
    ('stop', 'Automation stopped'), ('mouse', 'manual input active'),
])
...
@pytest.mark.parametrize('message', [
    'Waiting for the current input action', 'Focus changed during click',
    'SendInput(mouse-down) failed', 'Purchase was not verified; no repeat purchase issued',
])
...
def test_purchase_receipt_timeout_after_click_never_repeats(town):
    ...
    town.trade.verified_read = lambda read, accept, failure:original(read, accept, failure, timeout=0)
