...
def test_manual_stop_pause_and_identity_changes_prevent_resume():
    ...
    h = {
        "target": proof["target"],
        "embedded_controls": {"control": {"revision": 7, "enabled": False}},
    }
...
def test_native_window_releases_before_resume_and_honors_f11(
    tmp_path, monkeypatch, pause, connected
):
    ...
    health = {
        "target": {"pid": 7},
        "embedded_controls": {"control": {"enabled": True, "revision": 1}},
    }
    monkeypatch.setattr(handoff.time, "time", lambda: now[0])
    monkeypatch.setattr(
        handoff.time, "sleep", lambda seconds: now.__setitem__(0, now[0] + seconds)
    )
    ...
    monkeypatch.setattr(
        ...
        lambda path: (
            ...
            if str(path).endswith("merchant-deliveries.json")
    ...
    monkeypatch.setattr(
        ctypes.windll.user32, "GetAsyncKeyState", lambda key: 0x8000 if pause else 0
    )
    ...
    def merchant(body):
        ...
        if action == "status":
            return {
                "handoff_requested": "window:1",
                "characters": {
                    "Dutch": {
                        "connected": connected,
                        "enabled": True,
                        "credentials_saved": True,
                        "qualification": {"login": True},
                    }
                },
            }
...
def test_old_market_incident_cannot_bypass_refill_interval():
    ...
    s = {
        "handoff_requested": "merchant-return:Spiritual:123",
        "characters": {
            "Spiritual": {
                "snapshot": {"map_id": 1036},
                "shop_return": {"phase": "needs_attention"},
            }
        },
    }
...
def town_service(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        worker, "request", lambda *args, **kwargs: copy.deepcopy(health)
    )
    ...
    def merchant(body):
        ...
        if action == "status":
            ...
            return {
                "handoff_requested": requested[0],
                "characters": {"Dutch": {"connected": True, "enabled": False}},
            }
        if action == "refill-check":
            requested[0] = body["request_id"]
    ...
    def park(loop, cancelled, notify, **kwargs):
        cancelled.is_set()
        now[0] += min(park_cost[0], kwargs["seconds"])
        return {"target": health["target"]}
    ...
    loop = SimpleNamespace(
        ...
        phase="restocking",
        health=lambda: copy.deepcopy(health),
        stop_farm=lambda: None,
        check_stop=lambda: None,
        record=lambda *args, **kwargs: None,
        focus=lambda h: None,
        market_service_deadline=123,
    )
    return SimpleNamespace(
        ...
        run=lambda: handoff.service_window(loop, town=True),
    )
...
def test_refill_after_delivery_consumes_only_same_visit_remainder(town_service):
    r = town_service
    visit = r.visit.begin(parent="required-town:1")
    r.windows.reserve("delivery-first", town=True, visit=visit)
    r.windows.started()
    r.windows.finish("released")
    r.now[0] = 1050
    assert r.run()
    grant = next(c for c in r.calls if c["action"] == "handoff-grant")
...
def test_non_market_town_retains_fifteen_seconds_without_market_visit(town_service):
    r = town_service
    r.health["embedded_controls"]["life"]["map_id"] = 1002
    assert r.run()
    grant = next(c for c in r.calls if c["action"] == "handoff-grant")
...
def test_lost_grant_acknowledgement_still_revokes_possible_input(town_service):
    r = town_service
    r.grant_error[0] = True
    with pytest.raises(OSError, match="Lost grant acknowledgement"):
        r.run()
...
def test_rejected_admission_defers_and_only_restores_prior_on(
    town_service, monkeypatch, was_enabled
):
    from conquest.merchants import bridge
    from conquest import worker

    r = town_service
    r.health["embedded_controls"]["control"]["enabled"] = was_enabled
    ...
    def merchant(body):
        if body["action"] == "handoff-grant":
            r.calls.append(copy.deepcopy(body))
            submitted[0] = True
            raise bridge.MerchantRejected(
                "Owned booth changed during fresh qualification"
            )
    ...
    def stop():
        r.health["embedded_controls"]["control"].update(enabled=False, revision=2)

    def farm(info, action, body=None):
        if action == "controls":
            controls.append(body)
            r.health["embedded_controls"]["control"].update(body)
