...
@pytest.mark.parametrize('change',[
    'unrelated_uid','unrelated_name','unrelated_vtable','unrelated_all',
    'new_duplicate','name_takeover','target_uid','target_name','target_vtable',
    'new_matching_vtable','collection','entries','trace','process','timeout',
])
def test_complete_second_scan_preserves_unique_target_while_unrelated_actors_change(monkeypatch,change):
    ...
    fresh=dict(initial)
    if change in ('unrelated_uid','unrelated_all'):fresh[other]=record(10,'Unrelated')
    if change in ('unrelated_name','unrelated_all'):fresh[other]=record(10,'Another')
    if change in ('unrelated_vtable','unrelated_all'):fresh[other]=record(10,'Another',base+0x1234)
    if change=='new_duplicate':fresh[other]=record(1173490,'Parasite')
    if change=='name_takeover':
        fresh[actor]=record(9,'Unrelated');fresh[other]=record(1173490,'Parasite')
    if change=='target_uid':fresh[actor]=record(1173491,'Parasite')
    if change=='target_name':fresh[actor]=record(1173490,'Changed')
    if change=='target_vtable':fresh[actor]=record(1173490,'Parasite',base+0x1234)
    ...
    reads={};identity_checks=[];samples=[];trace_address=0x50000000
    def read(address,size):
        reads[address]=reads.get(address,0)+1
        if address==base+0x11045e:return bytes.fromhex('e8cd160700488d98d80f0000')
        if address==begin:
            return entries if change!='entries' or reads[address]==1 else struct.pack('<4Q',0,other,0,actor)
        assert address in initial
        return initial[address] if reads[address]==1 else fresh[address]
    def sample(session,fields):
        samples.append(fields)
        if fields==[(trace_address,'u64')]:return [100 if change!='trace' else 101]
        end=begin+32
        if change=='collection' and len(samples)>1:end+=16
        return [begin,end,begin+64]
    def assert_identity():
        identity_checks.append(True)
        if change=='process' and len(identity_checks)>1:raise ValueError('Game process changed')
    clock=iter([0,3 if change=='timeout' else .1])
    monkeypatch.setattr(identity,'sample_fields',sample)
    monkeypatch.setattr(identity.time,'monotonic',lambda:next(clock))
    observer=NS(adapter=NS(read_block=read,assert_identity=assert_identity,expected_sha256=CLIENT_SHA256),
        entities=NS(_resolve=lambda:(base,collection,[(trace_address,100)]),
            layout=NS(entry_stride=16,entry_object_offset=8,begin_offset=0x58,
                      end_offset=0x60,capacity_offset=0x68,max_objects=1000)))
    if change.startswith('unrelated_'):
        ...
        with pytest.raises(ValueError):identity.participant_uid(observer,'Parasite')
