...
def test_empty_ammo_can_reload_only_with_calibrated_binding_and_reserve():
    ...
    assert (
        supply_stop_reason(replace(BEFORE, items=(POTION,)), CONFIG, 10)
        == "ammo_unavailable"
    )
    assert (
        supply_stop_reason(
            BEFORE, SimpleNamespace(ammo_type=1050000, potion_type=1000000), 10
        )
        == "ammo_unavailable"
    )
...
def test_reload_requires_new_equipped_identity_from_observed_reserves():
    ...
    after = replace(
        BEFORE, items=(POTION,), equipped_ammo=replace(RESERVE, slot=None, amount=198)
    )
    assert attempt.outcome(after, 1050000, 10.5) == "verified"
    for uid, amount, type_id in [
        (1, 200, 1050000),
        (9, 200, 1050000),
        (2, 0, 1050000),
        (2, 200, 1000000),
    ...
    assert (
        attempt.outcome(replace(after, items=(RESERVE, POTION)), 1050000, 12)
        == "unverified"
    )
