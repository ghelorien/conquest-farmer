...
def test_live_loop_revives_before_supply_stop_and_returns_without_combat(
    tmp_path, monkeypatch, has_supplies
):
    import win32api
    ...
    cv2.imwrite(
        str(image_path),
        np.random.default_rng(1).integers(0, 256, (20, 70, 3), dtype=np.uint8),
    )
    profile["recovery"] = {
        "enabled": True,
        "revive_template": str(image_path),
        "revive_region": [650, 250, 950, 450],
        "return_route": [[430, 380], [430, 400], [430, 420], [425, 450]],
    }
    path = tmp_path / "profile.yaml"
    profile["observation_mode"] = (
        "legacy_visual"  # Exercise the historical loop with fakes.
    )
    ...
    class Session:
        ...
        def request(self, name, args=None):
            ...
            assert name == "foreground-click", (
                "Dead characters must not receive potion keys"
            )
            ...
            if state["health"] == 0:
                ...
                state.update(health=0.9, position=[430, 380])
            else:
                px, py = args["point"][0] - 792, args["point"][1] - 432
                dx, dy = (px / 32 + py / 16) / 2, (py / 16 - px / 32) / 2
                state["position"] = [
                    round(state["position"][0] + dx),
                    round(state["position"][1] + dy),
                ]
    ...
    class Inventory:
        ...
        def read(self):
            ...
            return InventorySnapshot(
                now[0], now[0], items, Item(99, 1050000, 100, 200, None), 0, 40
            )
    ...
    result = trial.run_trial(
        path, "unused", tmp_path / "output", 50, logging.getLogger("test")
    )
