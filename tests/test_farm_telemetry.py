...
def test_level_eta_uses_remaining_experience_and_net_hourly_rate():
    ...
    app = {
        "kills": 1234,
        "kills_per_hour": 1800,
        "experience_observed_at": 100,
        "experience": {
            "level": 55,
            "experience_candidate": 250,
            "experience_required": 1000,
            "xp_per_hour_candidate": 500,
        },
    }
    assert (
        farm_stats(app, True, now=101)
    ...
    text = activity_text(
        {
            "phase": "restocking",
            "updated_at": 99,
            "activity": "Heading to Pharmacist to buy Painkiller",
        },
        {"state": "Stopped"},
        {"enabled": False},
        None,
...
def test_dead_character_overrides_stale_travel_description():
    text = activity_text(
        {"phase": "restocking", "updated_at": 99, "activity": "Heading to Blacksmith"},
        {},
        {"enabled": False},
        {"dead_candidate": True},
        now=100,
...
def test_stale_route_status_does_not_claim_live_travel():
    assert (
        activity_text(
            {
                "phase": "restocking",
                "updated_at": 50,
                "activity": "Heading to Blacksmith",
            },
            {},
            {"enabled": False},
            None,
        ...
        == "Waiting for a current route update"
    )
...
def test_recent_healing_event_has_readable_activity():
    assert (
        activity_text(
            {"phase": "hunting"},
            {"activity": "Using a healing potion", "activity_at": 99},
            {"enabled": True, "execution_state": "farming"},
            None,
            now=100,
        )
...
def test_a_single_pickup_does_not_trigger_a_town_trip(free_slots):
    assert not needs_town(
        {"arrows": 1000, "potions": 10, "free_slots": free_slots},
        RouteLibrary().load("turtledove"),
    )
...
def test_navigation_failure_is_not_reported_as_lost_focus():
    ...
    assert pause_message("Waiting for a traversable patrol step").startswith(
        "Navigation blocked"
    )
    assert (
        pause_message("Game lost focus; no input sent")
        == "Waiting for Conquer to regain focus"
    )
    assert pause_message("Life state changed during observation").startswith(
        "Waiting: Life state"
    )
...
@pytest.mark.parametrize(
    "phase", ["changing_route", "recovering_route", "visiting_town"]
)
def test_city_transition_activity_does_not_look_like_farming_off(phase):
    assert (
        activity_text(
            dict(phase=phase, updated_at=99, activity="Heading to PhoenixCity town"),
            {},
            {"enabled": False},
            None,
            now=100,
        )
        == "Heading to PhoenixCity town"
    )
...
def test_user_stop_clears_display_rate_without_deleting_kill_history(
    ...
    app.last = {"kills_per_hour": 1900, "kills": 390}
    app.record = lambda **fields: app.last.update(fields)
    app.thread = None
    app.runtime = None
    app.memory_text = SimpleNamespace(set=lambda value: None)
    app.update_ids(False)
    assert app.last["kills_per_hour"] == 0 and app.last["kills"] == 390
    assert not app.control.snapshot()["enabled"]
    assert (
        tmp_path / ".runtime/overnight.stop"
    ).read_text() == "Stopped by user: Farming Off"


def test_recovered_inventory_history_is_labeled_and_deduplicated_across_reload(
    tmp_path,
):
    path = tmp_path / "pickups.jsonl"
    history = PickupHistory(path)
