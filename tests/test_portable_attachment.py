...
def test_restored_app_is_reachable_after_monitor_removed(resolution, scale):
    ...
    if w < 1036 or h < 793:
        with pytest.raises(ViewportTooSmall):
            require_viewport(w, h)
    else:
        assert require_viewport(w, h) == (w, h)


def test_mixed_monitor_negative_origin_preserved():
    assert fit_geometry(
        1000, 700, -1200, 50, [(-1920, 0, 0, 1040), (0, 0, 3840, 2120)]
    ) == (1000, 700, -1200, 50)


def test_install_discovery_does_not_depend_on_program_files(tmp_path):
    root = tmp_path / "Games/My Conquest"
    ...
    with pytest.raises(ValueError):
        find_installation(root / "Other.exe")


def test_diagnostics_never_copy_exception_secrets():
    status = AttachmentStatus()
    status.enter("behavior", pid=123)
    status.attached = True
    try:
        raise ValueError("password and webhook secret")
...
def test_behavior_failure_keeps_host_attached(monkeypatch):
    ...
    app.state_text = Var()
    app.attachment_text = Var()
    app.attachment = AttachmentStatus()
    app.character_context = None
    app.observer = None
    app.thread = None
    app.runtime = None
    app.client = (7, 8, {"creation_time_100ns": 9})
    app.refresh_client = lambda: None
    app.root = SimpleNamespace(update_idletasks=lambda: None)
    app.pane = SimpleNamespace(
        winfo_id=lambda: 10, winfo_width=lambda: 1200, winfo_height=lambda: 900
    )
    app.embedded_layout = lambda: None
    app.compact = lambda: None
    app.record = lambda **_: None
    closed = []
    app.stop_observer = lambda: closed.append(True)
    ...
    host.attach = attach
    host.detach = lambda: closed.append("detach")
    app.host = host
    app.observer_factory = lambda *args: SimpleNamespace()

    def fail():
        app.attachment.enter("behavior")
        raise ValueError("terrain missing")
...
def test_hosting_failure_restores_native_window():
    ...
    events = []
    api = SimpleNamespace(
        snapshot=lambda *a: SimpleNamespace(hwnd=1, identity={"pid": 2}),
        require_matching_dpi=lambda *a: None,
        owns_window=lambda *a: True,
    ...
    with pytest.raises(OSError):
        host.attach(1, {"pid": 2}, 3, 1200, 900)
...
def layout_app(size):
    ...
    app.observer = object()
    app.closing = False
    app.embed_layout_pending = True
    app.attachment = AttachmentStatus()
    app.attachment.enter("attachment")
    callbacks = []
    finished = []
    errors = []
...
def test_embed_waits_for_native_maximize_and_stable_pane(monkeypatch):
    ...
    size = [999, 650]
    app, callbacks, finished, errors = layout_app(size)
...
def test_released_observer_cannot_attach_from_queued_resize():
    ...
    assert (
        not callbacks and not finished and not errors and not app.embed_layout_pending
    )
