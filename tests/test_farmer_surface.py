...
def ui(enabled=False, safe=True):
    ...
    app = N(
        closing=False,
        control=N(snapshot=lambda: dict(state)),
        show_game=lambda: calls.append("focus") or True,
    )
    return (
        N(
            closed=False,
            app=app,
            coordinator=N(check=lambda: None, owner=None),
            safe_to_yield=lambda: safe,
        ),
        calls,
        state,
    )
...
def test_expired_request_cannot_focus_later():
    u, c, s = ui()
    with pytest.raises(ValueError):
        focus_idle_farmer(u, queued_at=time.monotonic() - 4)
    assert not c
...
def test_observation_and_execution_telemetry_do_not_change_idle_intent():
    ...
    def focus():
        c.append("focus")
        s.update(
            resolved_target_ids=[44],
            execution_state="waiting_for_targets",
