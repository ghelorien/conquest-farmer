...
from conquest.merchants.pricing import ItemKey,Listing,price_item,socket_name,parse_booth_price,wait_booth_price
...
@pytest.mark.parametrize('raw,expected',[(b'123,456',123456),(b'123456',123456),(b'999,999,999',999999999),(b'1',1)])
def test_native_booth_price_format(raw,expected):
    assert parse_booth_price(raw)==expected


@pytest.mark.parametrize('raw',[b'',b'0',b'12,34',b'123,456x',b'-123',b'1,000,000,000',b'123 456',b'123.456'])
def test_invalid_native_booth_price_is_never_accepted(raw):
    with pytest.raises(ValueError):parse_booth_price(raw)
...
def test_price_verification_waits_for_render_without_repeating_input(mode):
    ...
    def read():return b'123,456' if elapsed[0]>=.1 and mode=='renders' else b''
    def check():
        checks.append(elapsed[0])
        if mode=='stopped' and elapsed[0]>=.04:raise CaptureUnavailable('Stopped')
    args=dict(clock=lambda:elapsed[0],sleep=sleep,timeout=.2)
    if mode=='renders':assert wait_booth_price(read,123456,check,**args)==123456
    else:
...
@pytest.mark.parametrize('prices,target,excluded',[
...
@pytest.mark.parametrize('price',[0,-1,1_000_000_000,2_147_483_647,True,99.5,'100'])
def test_invalid_booth_price_cannot_remove_existing_listing(price):
    ...
    with pytest.raises(ValueError,match='booth silver range'):
        MerchantDriver.list_item(driver,{},dict(uid=1,price=100,slot=0),price,lambda:None)
    assert calls==[]


def test_owned_sellers_match_but_do_not_count_as_independent_sellers():
    rows = listings([100,110,120,130])+[Listing('SPIRITUAL',KEY,1),Listing('Dutch',KEY,1),Listing('seller0',KEY,200)]
    assert price_item(KEY,rows).price==1
    assert price_item(KEY,rows).sellers==4
    assert price_item(KEY,rows[:3]+rows[4:]).price==1


@pytest.mark.parametrize('spiritual,dutch,target',[
    (99,99,99), (100,100,100), (90,95,90), (95,90,90),
    (101,120,99), (100,120,100),
])
def test_owned_floor_matches_and_only_lower_outside_prices_trigger_undercut(spiritual,dutch,target):
    rows=listings([100,110,120,130])+[
        Listing('SpIrItUaL',KEY,spiritual),Listing('DUTCH',KEY,dutch)]
    ...
    for _ in range(3):
        rows=listings([100,110,120,130])+[Listing(c,KEY,target) for c in ('Spiritual','Dutch')]
        assert price_item(KEY,rows).price==target


def test_owned_floor_uses_valid_competitors_and_never_changes_outlier_median():
    result=price_item(KEY,listings([10,100,100,100])+[Listing('Dutch',KEY,90)])
    assert result.price==90 and result.excluded==('seller0',)


def test_owned_matching_respects_attributes_server_and_stack_rounding():
    rows=listings([100,110,120,130])
    assert price_item(KEY,rows+[Listing('Dutch',replace(KEY,plus=1),1),
                               Listing('Spiritual',KEY,1,server='Europe')]).price==99
...
def test_group_attributes_and_quantity_currency():
    ...
    for wrong in (replace(KEY,plus=1),replace(KEY,quality='Elite'),replace(KEY,category='Boots'),replace(KEY,sockets=('Empty','No socket'))):
...
def item(uid=1,**extra):
    return dict(uid=uid,type_id=130009,name='TaijiRobe',plus=2,gem1=0,gem2=0,quantity=1,bound=False,slot=0,price=None,**extra)


def snapshot():
    return {'character':'Spiritual','identity':{'pid':100,'creation_time_100ns':1000},'timestamp':time.time(),
        'capacity':40,'inventory':[item()],'silver':1000,'booth':[],'booth_open':True,'request':None,
        'trade':{'participant':'Parasite','participant_uid':999,'own_items':[],'items':[item(2)],
                 'own_silver':0,'other_silver':0,'accepted':False,'other_accepted':False}}


@pytest.mark.parametrize('one_time',[False,True])
def test_last_booth_slot_goes_to_highest_total_value_and_queue_stays_ranked(tmp_path,monkeypatch,one_time):
    from conquest.merchants.runtime import MerchantRuntime
    state=snapshot();state['trade']=None
    state['inventory']=[item(uid) for uid in (10,20,30,40,50)]
    state['inventory'][2]['quantity']=2
    state['booth']=[{**item(uid),'bound':True,'price':900} for uid in range(100,131)]
    prices={10:100,20:200,30:150,40:5000,50:200}
    market=SimpleNamespace(data={'observed_at':time.time()},
        key_for=lambda i:replace(KEY,category=str(i['uid'])),
        comparisons=lambda i:[Listing('seller'+str(n),replace(KEY,category=str(i['uid'])),prices[i['uid']])
                              for n in range(0 if i['uid']==40 else 4)])
    monkeypatch.setattr('conquest.merchants.runtime.MarketSnapshot',lambda data:market)
    monkeypatch.setattr('conquest.merchants.runtime.PriceHistory',lambda path:SimpleNamespace(
        remember=lambda market:None,catalog=lambda:{},quotes=lambda:{}))
    path=tmp_path/'market.json';path.write_text('{}')
    ...
    if one_time:
        ...
        journal.set('Spiritual','market_refresh',{'pending':False,'phase':'ready'})  # Supplied market fixture is collected.
    else:runtime.enable('Spiritual',True)
    submitted=[]
    def list_item(before,stock,price,check):
        check();submitted.append(stock['uid'])
        state['inventory']=[i for i in state['inventory'] if i['uid']!=stock['uid']]
        state['booth'].append({**stock,'price':price})
    driver=SimpleNamespace(read=lambda:copy.deepcopy(state),require_qualified=lambda capability:None,prepare_listing=lambda *args:None,
        list_item=list_item,wait_for=lambda predicate,check:copy.deepcopy(state) if predicate(state) else None)
    runtime.controllers['Spiritual']=MerchantController('Spiritual',journal,driver,guard)
    runtime.observers['Spiritual']=SimpleNamespace(adapter=SimpleNamespace(identity=state['identity'],assert_identity=lambda:None),lock=threading.RLock())
    ...
    if one_time:assert not runtime.enabled('Spiritual')


@pytest.mark.parametrize('same_process,target',[(True,90),(False,99)])
def test_runtime_passes_only_current_process_owned_prices(tmp_path,monkeypatch,same_process,target):
    from conquest.merchants.runtime import MerchantRuntime
    state=snapshot();state.update(trade=None,server='America')
    peer={**state,'character':'Dutch','identity':{'pid':200,'creation_time_100ns':2000},
          'inventory':[],'booth':[{**item(2),'price':90}]}
    data=market_data();data['observed_at']=time.time()
    path=tmp_path/'market.json';path.write_text(json.dumps(data))
    journal=Journal(tmp_path/'journal.sqlite3')
    guard=InputCoordinator(lambda:True,path=tmp_path/'input.lock')
    runtime=MerchantRuntime(object(),guard,journal=journal,market_path=path)
    ...
    submitted=[]
    def list_item(before,stock,price,check):
        check();submitted.append(price)
        state['inventory']=[];state['booth']=[{**stock,'price':price}]
    driver=SimpleNamespace(read=lambda:copy.deepcopy(state),require_qualified=lambda capability:None,
        prepare_listing=lambda *args:None,list_item=list_item,
        wait_for=lambda predicate,check:copy.deepcopy(state) if predicate(state) else None)
    runtime.controllers['Spiritual']=MerchantController('Spiritual',journal,driver,guard)
    runtime.observers['Spiritual']=SimpleNamespace(adapter=SimpleNamespace(identity=state['identity'],assert_identity=lambda:None),lock=threading.RLock())
    runtime.observers['Dutch']=SimpleNamespace(adapter=SimpleNamespace(identity={
        **peer['identity'],'creation_time_100ns':2000 if same_process else 3000}))
...
def test_journal_intent_isolation_idempotence_and_restart(journal):
    ...
    changed = copy.deepcopy(state);changed['silver'] += 1
...
class Driver:
    ...
    def prepare_listing(self,*args):
        pass
    def read(self):
        return copy.deepcopy(self.states.pop(0) if len(self.states)>1 else self.states[0])
...
def controller(journal,tmp_path,states,authorized=True):
    ...
    if authorized:
        journal.set('Spiritual','accepted_request',{'identity':state['identity'],'participant_uid':999,'opened_at':time.time()})
...
def test_disconnect_leaves_uncertain_intent_then_receipt_reconciles(journal,tmp_path):
    ...
    after = copy.deepcopy(before);after.update(trade=None,inventory=[item(),item(2)])
    control.reconcile(after)
    assert not journal.pending('Spiritual') and driver.inputs==['accept']


@pytest.mark.parametrize('blocker',['none','confirmation','dialog','trade','identity','attributes'])
def test_only_proven_unsubmitted_listings_can_automatically_retry(journal,tmp_path,blocker):
    from conquest.merchants.controller import MerchantController
    before=snapshot();before.update(trade=None,booth_open=True,windows=[])
    record={'uid':1,'price':100,'item':before['inventory'][0],'snapshot':before}
    journal.begin('interrupted','Spiritual','listing',record)
    journal.transition('interrupted','uncertain',{'confirmation_attempted':blocker=='confirmation'})
    after=copy.deepcopy(before)
    if blocker=='dialog':after['windows']=[{'name':'Add Item to Booth'}]
    if blocker=='trade':after['request']={'participant':'Parasite'}
    if blocker=='identity':after['identity']['creation_time_100ns']+=1
    if blocker=='attributes':after['inventory'][0]['plus']+=1
    c=MerchantController('Spiritual',journal,None,InputCoordinator())
    if blocker=='none':
        c.reconcile(after);assert not journal.pending('Spiritual')
    else:
        with pytest.raises(ValueError,match='reconciliation'):c.reconcile(after)
...
def test_exclusive_input_revocation_manual_stop_and_nested_lease(tmp_path):
    safe = [True];manual = [False]
    guard = InputCoordinator(lambda:safe[0],lambda:manual[0],tmp_path/'input.lock')
    errors = []
    with guard.lease('Spiritual'):
        ...
        safe[0] = False
        with pytest.raises(CaptureUnavailable):guard.check()
    assert guard.owner is None
    safe[0] = True;manual[0] = True
    with pytest.raises(CaptureUnavailable):
        with guard.lease('Dutch'):pass
    manual[0] = False;guard.stop()
    with pytest.raises(CaptureUnavailable):guard.check()
    guard.resume();guard.check()
...
def test_recovery_limit_survives_restart_does_not_resume_paused_merchant(journal):
    ...
    with pytest.raises(ValueError,match='exhausted'):
        Recovery('Dutch',journal,clock=lambda:now[0]).attempt(lambda:calls.append(1))
    assert len(calls)==3
    recovery.retry()
...
def market_data():
    return {'source':'https://conqueronline.net/market','server':'America','complete':True,'observed_at':1000,'total':4,
        'listings':[dict(name=name,category='Coat',quality='Super',plus=2,sockets=['No socket','No socket'],seller=f'Seller{i}',price=100+i*10,server='America')
            for i,name in enumerate(['TaijiRobe','TaoRobe','AnotherRobe','OtherRobe'])]}


@pytest.mark.parametrize('mode,target',[
    ('fresh',90),('sold',99),('stale',80),('future',80),('wrong_server',80),
    ('closed',80),('incomparable',99),('own_snapshot',85),
])
def test_plan_prefers_fresh_owned_booths_over_delayed_website(mode,target):
    ...
    state=snapshot();state.update(server='America',timestamp=1000,trade=None)
    peer={**state,'character':'Dutch','identity':{'pid':200,'creation_time_100ns':2000},
          'inventory':[],'booth':[{**item(2),'price':90}]}
    if mode=='sold':peer['booth']=[]
    if mode=='stale':peer['timestamp']=994
    if mode=='future':peer['timestamp']=1001
    if mode=='wrong_server':peer['server']='Europe'
    if mode=='closed':peer['booth_open']=False
    if mode=='incomparable':peer['booth'][0]['plus']=1
    if mode=='own_snapshot':state['booth']=[{**item(3),'price':85}]
...
def test_owned_match_is_allowed_in_sparse_market():
    ...
    data=market_data();data['listings'].pop();data['total']=3
    market=MarketSnapshot(data,now=1000)
    control=MerchantController('Spiritual',None,None,None,clock=lambda:1000)
...
def test_equipment_compares_across_names_and_levels_and_snapshot_expires():
    ...
    for mutate in (lambda d:d.update(complete=False),lambda d:d.update(server='Europe'),lambda d:d.update(total=5),lambda d:d.update(observed_at=0)):
        data = market_data();mutate(data)
        with pytest.raises(ValueError):MarketSnapshot(data,now=1000)
    unknown = item();unknown['name']='Unknown'
    with pytest.raises(ValueError):market.key_for(unknown)
...
def test_separate_encrypted_credentials(tmp_path,monkeypatch):
    ...
    with pytest.raises(ValueError):credential_path('../escape')


@pytest.mark.parametrize('old_price',[10,1000])
def test_repricing_increases_and_decreases_require_matching_receipt(journal,tmp_path,old_price):
    from conquest.merchants.controller import identities
    before = snapshot();before.update(trade=None,inventory=[],booth=[item()])
    before['booth'][0]['price'] = old_price
    after = copy.deepcopy(before);after['booth'][0]['price']=99
    control,driver = controller(journal,tmp_path,[before,after])
    driver.list_item = lambda *args:driver.inputs.append('list')
    plan = {'uid':1,'price':99,'old_price':old_price,'observed_at':time.time(),
...
def test_expired_repricing_never_sends_input(journal,tmp_path):
    before = snapshot();before.update(trade=None)
    control,driver = controller(journal,tmp_path,[before])
    with pytest.raises(ValueError,match='expired'):
        control.apply_price({'uid':1,'price':99,'old_price':None,'observed_at':0,'attributes':[]})
...
def test_preflight_focus_denial_has_no_transaction_to_reconcile(journal,tmp_path):
    ...
    before=snapshot();before.update(trade=None)
    control,driver=controller(journal,tmp_path,[before])
    def denied(*args):raise CaptureUnavailable('Waiting for foreground focus')
...
def test_missing_booth_calibration_is_verified_before_listing_intent(journal,tmp_path,monkeypatch):
    ...
    before=snapshot();before['trade']=None
    after=copy.deepcopy(before);after.update(inventory=[],booth=[{**item(),'price':99}])
    control,driver=controller(journal,tmp_path,[before,before,after])
    qualified=[False];calls=[]
    def require(capability):
        if not qualified[0]:raise ValueError('Needs calibration')
    def verify(probe,j,check):
        check();assert not j.pending('Spiritual')
        calls.append('verified');qualified[0]=True
    ...
    monkeypatch.setattr('conquest.merchants.qualification.verify_booth_controls',verify)
    monkeypatch.setattr('conquest.desktop_runtime.physical_coordinates',nullcontext)
    control.apply_price({'uid':1,'price':99,'old_price':None,'observed_at':time.time(),
...
def test_browser_cells_validate_totals_and_unknown_stack_quantities():
    ...
    row = ['TaijiRobe\n\nTaoist Robe','Super','+2','No socket\nNo socket','Trader','America','100,000']
    data = dict(source='https://conqueronline.net/market',server='America',last_page=True,
        observed_at=time.time(),initial_total=1,final_total=1,initial_change='same',final_change='same',
...
def test_equipment_type_mapping_does_not_require_exact_name_or_quality():
    ...
    market = MarketSnapshot(data,now=1000)
    unseen = item();unseen['name']='UnlistedLowerLevelCoat'
    assert price_item(market.key_for(unseen),market.comparisons(unseen)).price==99
    changed = item();changed['type_id']=130008
