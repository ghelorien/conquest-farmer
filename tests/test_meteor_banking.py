...
def route(monkeypatch):
    ...
    bag = []
    local = [item(i) for i in range(10)]
    market = []
    events = []
    ...
    exchange = {
        ...
        "fee": 0,
        "identity": {"name": "MillionaireLee"},
        "approach": [230, 240],
        "dialogs": [{"option": "intro"}, {"option": "pack"}],
    }
    write_json(
        m.POLICY,
        {
    ...
    def town(action, **fields):
        ...
        if action == "supplies":
            return {"items": list(bag), "silver": state["silver"], "capacity": 40}
        if action == "warehouse-items":
            return {"items": list(bank), "capacity": state["capacity"]}
        if action == "warehouse-money":
            return {"silver": state["silver"], "stored_silver": 5000}
        if action == "service-locate":
            return {"identity": exchange["identity"]}
        if action in ("service-open", "service-close-panel"):
            return {}
        if action in ("warehouse-withdraw-meteor", "warehouse-deposit"):
            source, target = (
                (bank, bag) if action == "warehouse-withdraw-meteor" else (bag, bank)
            )
            value = next(i for i in source if i["uid"] == fields["uid"])
            source.remove(value)
            target.append(value)
    ...
    def trip(loop, leg, *, before_submit=None):
        if before_submit:
            before_submit()
        if state["map"] == 1036:
            assert not m.carried(loop)
        state["map"] = leg["destination_map"]
        state["silver"] -= leg["fare"]
    ...
    monkeypatch.setattr(m, "trip", trip)
    monkeypatch.setattr(m, "select_saved_dialog", select)
    monkeypatch.setattr(
    ...
    loop = NS(
        ...
        travel=lambda *a, **kw: events.append(("travel", a[0])),
    )
...
def test_interrupted_market_cleans_prompt_before_banking_without_new_deadline(route):
    ...
    assert events.index(("cancel-prompt", 1036)) < events.index(
        ("warehouse-deposit", 99)
    )
    assert loop.market_service_deadline == 1
    assert not any(e[0] == "choice" for e in events)


def test_override_archives_full_record_then_replans_from_fresh_bag_and_warehouse(
    ...
    m.operator_override(
        ...
        confirmation_reference="operator-click",
    ...
    rows = [
        json.loads(line) for line in m.AUDIT.read_text(encoding="utf-8").splitlines()
    ]
    archived = next(
        row for row in rows if row.get("record_type") == "meteor_terminal_journal"
    )
    assert archived["journal"]["operator_override"]["original_state"] == old
    assert (
        archived["journal"]["operator_override"]["fresh_evidence"]["supplies"]["items"]
        == []
    )
    assert {item["uid"] for item in local} == set()


def test_meteor_override_requires_the_previewed_digest_and_is_idempotent(
    route, monkeypatch, tmp_path
):
    loop, state, bag, local, market, events = route
    monkeypatch.setattr(m, "AUDIT", tmp_path / "meteor-audit.jsonl")
    ...
    with pytest.raises(ValueError, match="previewed Meteor incident digest"):
        m.operator_override(
            loop,
            operator_confirmed=True,
            confirmation_reference="click",
            incident_digest="stale",
        )
    digest = evidence_digest(original)
    result = m.operator_override(
        loop,
        operator_confirmed=True,
        confirmation_reference="click",
...
def test_legacy_generic_override_audit_is_imported_once_before_terminal_replacement(
    route, monkeypatch, tmp_path
):
    loop, state, bag, local, market, events = route
    monkeypatch.setattr(m, "AUDIT", tmp_path / "meteor-audit.jsonl")
    legacy = {
        ...
        "original_state": {"phase": "travelling", "meteor_uids": [1]},
    ...
    assert (
        len(
            [row for row in rows if row.get("record_type") == "generic_override_import"]
        )
        == 1
    )
    assert (
...
def test_archive_new_journal_crash_boundary_retries_without_duplicate_audit(
    route, monkeypatch, tmp_path
):
    loop, state, bag, local, market, events = route
    monkeypatch.setattr(m, "AUDIT", tmp_path / "meteor-audit.jsonl")
...
def test_malformed_canonical_audit_blocks_terminal_replacement(
    route, monkeypatch, tmp_path
):
    loop, state, bag, local, market, events = route
    monkeypatch.setattr(m, "AUDIT", tmp_path / "meteor-audit.jsonl")
    ...
    terminal = {
        "phase": "operator_overridden",
        "operator_override": {"original_state": {"phase": "travelling"}},
    }
    write_json(m.JOURNAL, terminal)
    with pytest.raises(ValueError, match="malformed JSON"):
        m._replace_overridden_journal(
            {"phase": "withdrawing", "origin": 1011, "meteor_uids": list(range(10))}
        )
    assert read_json(m.JOURNAL) == terminal
...
def test_malformed_archive_intent_fails_closed_without_replacing_journal(tmp_path):
    ...
    write_json(
        m._intent_path(),
        {"terminal_digest": "not-a-journal", "terminal_journal": "corrupt"},
    )
...
def test_canonical_audit_append_is_concurrent_and_idempotent(monkeypatch, tmp_path):
    ...
    record = {
        ...
        "archive_key": "terminal-journal:one",
        "journal": {"phase": "operator_overridden"},
    }
...
def test_banked_scroll_resumes_return_without_reexchange(route):
    ...
    write_json(
        m.JOURNAL, {"phase": "stored_in_market", "origin": 1011, "scroll_uid": 99}
    )
    assert m.resume(loop) and state["map"] == 1011
    assert not any(
        e[0] in ("choice", "warehouse-withdraw-meteor", "warehouse-deposit")
        for e in events
    )
...
def test_merchant_delivered_scroll_allows_verified_return(route, monkeypatch):
    ...
    def deliver(loop):
        ...
        write_json(
            ...
            {"receipts": [{"request_id": "verified-trade", "items": [value]}]},
        )
...
def test_missing_banked_scroll_blocks_departure(route):
    ...
    with pytest.raises(ValueError, match="not in Market storage"):
        m.resume(loop)
...
def test_user_consumed_scroll_still_banks_remaining_valuables_before_return(route):
    ...
    write_json(
        m.JOURNAL,
        {
            ...
            "origin": 1011,
            "scroll_uid": 99,
            "user_confirmed_scroll_consumption": {
                "uid": 99,
                "confirmed": True,
...
@pytest.mark.parametrize(
    "uid,confirmed,source",
    [
        (98, True, "explicit user confirmation"),
        (99, False, "explicit user confirmation"),
        (99, True, "inferred from missing item"),
    ],
)
def test_scroll_consumption_requires_explicit_confirmation_for_exact_uid(
    route, uid, confirmed, source
):
    loop, state, bag, local, market, events = route
    state["map"] = 1036
    write_json(
...
def test_uncertain_departure_never_pays_again(route):
    loop, state, bag, local, market, events = route
    write_json(
        m.JOURNAL, {"phase": "travelling", "origin": 1011, "departure_attempted": True}
    )
...
def test_user_moved_scroll_banks_other_valuables_without_inventing_delivery(route):
    ...
    write_json(
        m.JOURNAL,
        {
            ...
            "origin": 1011,
            "scroll_uid": 99,
            "user_confirmed_scroll_transfer": {
                "uid": 99,
                "type_id": m.SCROLL,
                "confirmed": True,
                "source": "explicit user confirmation",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"type_id": m.METEOR},
        {"confirmed": False},
        {"source": "inferred from missing item"},
        {"destination": "unknown"},
    ],
)
...
def test_full_market_stops_and_never_returns(route, monkeypatch):
    loop, state, bag, local, market, events = route
    state.update(map=1036, capacity=1)
    bag.append(item(99, m.SCROLL))
    write_json(m.JOURNAL, {"phase": "storing_scroll", "origin": 1011, "scroll_uid": 99})

    def stop(*args):
        raise RuntimeError("storage halt")

    monkeypatch.setattr(storage_halt, "request_stop", stop)
    with pytest.raises(RuntimeError, match="storage halt"):
        m.resume(loop)
...
def test_market_approach_avoids_blocked_frontage_and_early_npc_stop():
    ...
    loop = NS(
        living=lambda: {
            "embedded_controls": {"life": {"map_id": 1036, "position": [190, 189]}}
        },
        town=lambda *a, **kw: None,
        travel=lambda *a, **kw: calls.append((a, kw)),
    )
...
def test_market_approach_uses_reachable_warehouse_without_waypoint_movement():
    loop = NS(
        living=lambda: {
            "embedded_controls": {"life": {"map_id": 1036, "position": [182, 193]}}
        },
        town=lambda action, **kw: (
            {"reachable": True} if action == "vendor-status" else None
        ),
...
def test_market_approach_accepts_adjacent_tile_instead_of_crowded_exact_target():
    loop = NS(
        living=lambda: {
            "embedded_controls": {"life": {"map_id": 1036, "position": [183, 185]}}
        },
        town=lambda *a, **kw: None,
        travel=lambda *a, **kw: pytest.fail("Already beside approach"),
    )
    m.approach_market_warehouse(loop, "Bank valuables")
...
def test_market_approach_stays_at_verified_bank_tile():
    loop = NS(
        living=lambda: {
            "embedded_controls": {"life": {"map_id": 1036, "position": [182, 184]}}
        },
        town=lambda *a, **kw: None,
        travel=lambda *a, **kw: pytest.fail("Already at bank"),
    )
    m.approach_market_warehouse(loop, "Bank valuables")
...
def test_market_approach_recovers_obstructed_southern_crossing_once():
    ...
    loop = NS(
        living=lambda: {
            "embedded_controls": {"life": {"map_id": 1036, "position": [201, 215]}}
        },
        town=lambda *a, **kw: None,
        travel=travel,
    )
...
def test_market_frontage_reposition_stops_as_soon_as_warehouse_reachable():
    ...
    def town(action, **kw):
        if action == "vendor-status":
            return {"reachable": len(calls) >= 2}
        panels.append(kw["window"])

    loop = NS(
        living=lambda: {
            "embedded_controls": {"life": {"map_id": 1036, "position": [183, 190]}}
        },
        town=town,
        travel=travel,
    )
...
def test_market_progress_stall_uses_bounded_frontage_corridor_from_eastern_edge():
    ...
    def travel(point, **kw):
        ...
        if len(calls) == 1:
            raise TravelStalled(
                "Route made no improving progress after bounded recovery"
            )
        position[:] = point

    loop = NS(
        living=lambda: {
            "embedded_controls": {"life": {"map_id": 1036, "position": position.copy()}}
        },
        town=lambda action, **kw: (
            {"reachable": position == [176, 183]} if action == "vendor-status" else None
        ),
        travel=travel,
    )
    m.approach_market_warehouse(loop, "Bank valuables")
...
def test_market_progress_stall_rechecks_vendor_before_any_recovery():
    ...
    loop = NS(
        ...
        town=lambda action, **kw: (
            {"reachable": bool(calls)} if action == "vendor-status" else None
        ),
        travel=travel,
    )
    m.approach_market_warehouse(loop, "Bank valuables")
...
@pytest.mark.parametrize(
    "code,count", [("no_progress", 2), ("service_deadline", 1), ("unknown", 1)]
)
...
def test_market_approach_does_not_retry_unrelated_or_repeated_failure():
    ...
    calls.clear()
    loop.living = lambda: {
        "embedded_controls": {"life": {"map_id": 1036, "position": [230, 240]}}
    }
    with pytest.raises(ValueError, match="remains obstructed"):
        m.approach_market_warehouse(loop, "Bank valuables")
    assert calls == [(186, 184)]
...
def test_service_open_retries_only_open_after_range_walk(monkeypatch):
    clock = [0.0]
    opens = []
    records = [{"kind": 1, "text": "Yeah. Thanks."}]
...
def test_service_open_never_selects_or_retries_changed_dialog(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *a: {"records": [{"text": "Unexpected service"}]},
    )
    loop = NS(
        info="test",
        living=lambda: None,
...
def test_market_start_resumes_before_city_and_hunt(monkeypatch):
    ...
    monkeypatch.setattr(
        banking, "close_warehouse", lambda loop: events.append("close bank")
    )
    monkeypatch.setattr(
        city_travel, "ensure_city_visit", lambda loop: events.append("town check")
    )
    monkeypatch.setattr(overnight, "request", lambda *a: None)

    def hunt():
        events.append("hunt")
        raise overnight.OvernightStopped("test finished")

    loop = NS(
        check_stop=lambda: None,
        refresh=lambda: None,
        record=lambda *a, **kw: None,
        stop_farm=lambda: events.append("pause"),
        prepare_supplies=lambda: events.append("supplies"),
        select_level_route=lambda: None,
        hunt=hunt,
        info=None,
        route=NS(map_id=1011),
        health=lambda: (
            events.append("fresh health")
            or {"embedded_controls": {"life": {"map_id": 1011}}}
        ),
        town_visit=NS(
            state=lambda: {},
            require_town_work_complete=lambda: events.append("town work guard"),
        ),
...
@pytest.mark.parametrize(
    "start,uses_aisle",
    [((240, 175), True), ((239, 199), True), ((238, 199), False), ((239, 200), False)],
)
def test_market_return_leaves_northeast_booths_via_clear_aisle(start, uses_aisle):
    ...
    plan = {
        ...
        "activity": "Returning to Phoenix",
        "approach": [300, 250],
        "npc": "Mark.Controller",
        "waypoints": [[225, 206], [260, 230]],
    }
    with pytest.raises(ReachedService):
        m.trip(loop, plan)
    aisle = [
        (point, kwargs)
        for point, kwargs in calls
        if "central Market aisle" in kwargs["activity"]
    ]
