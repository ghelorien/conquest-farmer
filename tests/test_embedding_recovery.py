...
def app_for(tmp_path, enabled=False):
    ...
    app.messages=Queue();app.thread=None
    app.client=(42,99,{'pid':42,'creation_time_100ns':123})
    app.records=[];app.record=lambda **fields:app.records.append(fields)
    app.state_text=NS(set=lambda value:None)
    app.pane=NS(winfo_id=lambda:10,winfo_width=lambda:1420,winfo_height=lambda:1009)
    app.calls=[]
    def attach(*args):
        assert app.observer.lock._is_owned()
        app.calls.append(args);app.host.saved=object()
    def detach():
        app.calls.append('detach');app.host.saved=None
...
def test_manual_stop_wins_during_reattach(tmp_path):
    ...
    def stop_then_attach(*args):
        app.control.update({'enabled':False});attach(*args)
...
def test_detach_rejected_until_intent_and_runner_are_stopped(tmp_path,enabled,running):
    app=app_for(tmp_path,enabled)
...
def test_failed_reattach_keeps_connection_for_retry(tmp_path):
    app=app_for(tmp_path,True);observer=app.observer
    attach=app.host.attach
...
def test_reload_restores_host_before_safe_handoff(tmp_path):
    app=app_for(tmp_path,True)
    app.last={};app.selected_route=None
    app._restart_now=lambda:pytest.fail('Detached reload must not bypass safe handoff')
