...
def bridge(tmp_path):
    ...
    service = EmbeddedBridge(
        operations, threading.RLock(), info, lambda: {"control": control}
    )
...
def test_health_exposes_the_current_manual_session_fence_without_changing_intent(
    bridge, monkeypatch
):
    ...
    blocked = [False]
    monkeypatch.setattr(
        coordination, "manual_session_blocked", lambda character: blocked[0]
    )
    control["enabled"] = True
...
def test_vendor_reader_is_read_only_bound_callback(bridge):
    ...
    service.on_sample_npcs = lambda: {
        "snapshot": {"npcs": []},
        "shop_items_qualified": False,
    }
    control["enabled"] = True
...
@pytest.mark.parametrize(
    "operation", ["foreground-click", "foreground-key", "shutdown", "scan"]
)
...
def test_bridge_requires_fresh_stopped_diagnostic(bridge, operation):
    ...
    control["enabled"] = False
    assert (
        request(info, operation, {"expires_at": time.time() + 4})["qualified"] is False
    )
    assert len(calls) == 1
...
def test_bridge_rejects_wrong_token_before_dispatch(bridge):
    ...
    call = urllib.request.Request(
        f"http://127.0.0.1:{connection['port']}/health",
        data=b"{}",
        headers={"X-Conquest-Token": "wrong"},
    )
...
def test_town_preinput_retry_classification_survives_bridge(bridge):
    ...
    with pytest.raises(TownObservationUnavailable):
        request(
            info,
            "town",
            {"action": "open", "vendor_type": 5, "expires_at": time.time() + 3},
        )
...
def test_app_lifetime_bridge_has_no_timed_expiry_and_closes_normally(tmp_path):
    operations = SimpleNamespace(
        session=SimpleNamespace(pid=1), dispatch=lambda *args: {}
    )
    ...
    try:
        ...
        assert (
            request(info, "health")["embedded_controls"]["control"]["enabled"] is False
        )
    finally:
        service.close()
    assert not service.thread.is_alive() and not info.exists()
...
@pytest.mark.parametrize(
    "saved,mode,expected",
    [(None, "child", True), (object(), "owned", True), (object(), "child", False)],
)
def test_window_input_mode_tracks_applied_host_state(bridge, saved, mode, expected):
    service, info, calls, control = bridge
    queued = []
    service.on_native_window = lambda detached: queued.append(detached)
...
def test_reattach_accepts_on_intent_and_reports_applied_state(bridge):
    service, info, calls, control = bridge
    queued = []
    service.on_native_window = queued.append
    service.sync_window_mode(SimpleNamespace(saved=None, mode="owned"))
    control["enabled"] = True
    assert request(info, "native-window-mode", {"detached": False}) == {
        "window_change_queued": True
    }
    assert queued == [False]
...
def test_ground_diagnostic_is_read_only_while_farming(bridge):
    ...
    control["enabled"] = True
    assert (
        request(info, "town", {"action": "ground-items"})["source"]
        == "read_only_memory"
    )
    assert calls == []
...
def test_reconnect_retry_queues_only_bound_action_without_changing_farm_intent(bridge):
    service, info, calls, control = bridge
    retries = []
    service.on_reconnect = lambda: retries.append(1)
    control["enabled"] = True
