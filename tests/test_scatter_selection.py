...
def reader_fixture(monkeypatch):
    monkeypatch.setattr(s,'MemoryGui',lambda session:NS(base=0,read=lambda name:None))
    blocks={0x6986c0:struct.pack('<QQ',0x10000,1),0x10008:struct.pack('<Q',0x20000)}
    node=bytearray(0x38);struct.pack_into('<3Q',node,0,0x10000,0x10000,0x10000)
    struct.pack_into('<I',node,0x20,1);struct.pack_into('<Q',node,0x28,0x30000)
    control=bytearray(0xfc);struct.pack_into('<Q',control,0,0x5c5a38)
    blocks[0x20000]=node;blocks[0x30000]=control
    reader=s.SelectionReader(NS(read_block=lambda a,n:bytes(blocks[a][:n]),assert_identity=lambda:None))
...
def test_selected_id_is_read_from_qualified_control(monkeypatch):
    ...
    with pytest.raises(ValueError,match='identity'):reader.selected()
...
def test_registry_cycle_is_rejected(monkeypatch):
    ...
    with pytest.raises(ValueError,match='unavailable'):reader.selected()


def test_selection_waits_for_memory_confirmation(monkeypatch):
    selected=[0];opened=[False];now=[0.];events=[];clicks=[]
    def window(name):
        if not opened[0]:raise ValueError('Not open')
        return NS()
    reader=NS(selected=lambda:selected[0],gui=NS(read=window),
              menu_point=lambda:(1148,979),scatter_point=lambda actor:(1296,857))
    monkeypatch.setattr(s,'SelectionReader',lambda adapter:reader)
    monkeypatch.setattr(s.time,'monotonic',lambda:now[0])
    monkeypatch.setattr('conquest.memory_life.read_life',lambda *args:NS(dead_candidate=False,object_address=7))
    state=s.ScatterSelection(NS(adapter=None,health_layout=None,character='Parasite'),lambda e,p:events.append(e))
...
def test_selectable_scatter_does_not_depend_on_skill_level(monkeypatch,level):
    ...
    skill=bytearray(0x38);struct.pack_into('<Q',skill,0,0x5cff78)
    struct.pack_into('<I',skill,8,1);struct.pack_into('<II',skill,0x10,8001,level)
    skill[0x18:0x20]=b'Scatter\0';blocks[0x60000]=skill
    assert reader.entries(actor)==[8001]
    struct.pack_into('<I',skill,8,0)
    with pytest.raises(ValueError,match='unavailable'):reader.entries(actor)


def test_icon_uses_current_popup_and_column_geometry(monkeypatch):
    reader,_,_=reader_fixture(monkeypatch)
    window=NS(name='Skills',position=(1268.,829.),size=(152.,60.),scroll=(0.,0.))
    ...
    reader.table=lambda w:(7,(1276.,837.,1412.,881.),columns)
    assert reader.scatter_point(1)==(1296,857)
    window.scroll=(0.,10.)
    with pytest.raises(ValueError,match='scrolled'):reader.scatter_point(1)
