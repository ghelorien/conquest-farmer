...
def setup():
    ...
    f = snap("Parasite", 1, [one, two])
    m = snap("Spiritual", 2, [three])
    intent = {
...
def test_ambiguous_partial_outcome_never_reconciles(change):
    ...
    if change in ("offer", "accepted"):
        ...
        if change == "offer":
            m["trade"]["items"] = [i["items"][1]]
        else:
            m["trade"]["accepted"] = True
    with pytest.raises(ValueError):
        partial_result(i, f, m)
