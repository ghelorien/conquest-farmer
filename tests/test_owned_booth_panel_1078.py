...
def state(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr('conquest.merchants.booth_listing_once_1078._profile',lambda c:profile)
    snap=dict(identity={'pid':2,'creation_time_100ns':3},character='Dutch',character_uid=8,
        server='America',map_id=1036,position=[230,205],hp=93,silver=9,capacity=40,
        inventory=[{'uid':5,'type_id':720027,'quantity':1}],booth=[],own_booth_uid=77,
        booth_open=False,closed_modal=True,timestamp=time.time(),client_sha256=CLIENT_SHA256_1078)
    body=dict(action='merchant-owned-panel-prepare-1078',character='Dutch',
        request_id='booth-open1078-synthetic-one',expected_identity=snap['identity'],
        expected_character_uid=8,expected_own_booth_uid=77)
    before=dict(request=body,profile_id=profile.id,snapshot=snap,hwnd=12,client_sha256=CLIENT_SHA256_1078)
    journal=Journal(tmp_path/'journal.sqlite3');journal.begin(body['request_id'],'Dutch',panel.KIND,before)
    ...
    return NS(j=journal,snap=snap,body=body,before=before,ui=ui,key=body['request_id'],profile=profile)


def opened(x):
    s=deepcopy(x.snap);s.update(booth_open=True,timestamp=time.time(),listing_preflight={
...
def test_closed_panel_full_assets_and_new_actor_binding(state):
    x=state;panel._closed(x.snap,x.body,x.profile)
    for field,value in [('own_booth_uid',78),('character_uid',9),('identity',{'pid':3}),
                        ('booth_open',True),('closed_modal',False)]:
        with pytest.raises(ValueError):panel._closed({**x.snap,field:value},x.body,x.profile)


@pytest.mark.parametrize('change',['inventory','silver','booth','owner','grid','modal'])
def test_changed_assets_or_incomplete_native_open_cannot_settle(state,change):
    x=state;s=opened(x)
    if change=='inventory':s['inventory'][0]['uid']=6
    elif change=='silver':s['silver']+=1
    elif change=='booth':s['booth']=[{'uid':9}]
    elif change=='owner':s['listing_preflight']['owned_booth']['owner_uid']=88
    elif change=='grid':s['listing_preflight']['inventory_grid']=None
    else:s['listing_preflight']['price_modal']['observed']=True
    with pytest.raises(ValueError):panel._verify_open(s,x.snap)


def test_open_without_input_marker_does_not_qualify(state,monkeypatch):
    x=state
    monkeypatch.setattr('conquest.merchants.observe_1078.observe',lambda *a,**k:pytest.fail('Unsent proof is not reconciled'))
...
def test_crash_after_marker_before_phase_update_never_starts_worker(state,monkeypatch):
    ...
    monkeypatch.setattr(panel.threading,'Thread',lambda *a,**k:pytest.fail('A marked click cannot be replayed'))
...
def test_request_uids_require_exact_positive_integers(state,field,value):
    x=state
    with pytest.raises(ValueError):panel._closed(x.snap,{**x.body,field:value},x.profile)
...
def test_panel_scope_never_admits_listing_or_cancel(state):
    x=state;x.ui.grant={'scope':'listing_1078','character':'Dutch','listing_authority':{
        'mode':'open_panel','panel_request_id':x.key,'profile_id':x.profile.id}}
...
def test_native_selected_actor_must_be_exact_and_gui_unclaimed():
    base=0x10000000;actor=0x30000000;manager=0x40000000;context=0x50000000
    memory={base+0x6c4f08:struct.pack('<Q',manager),manager:struct.pack('<Q',base+0x5eab40),
        manager+0xd8:struct.pack('<Q',actor+0x10),actor+0x78:struct.pack('<I',77),
        actor+0x10:struct.pack('<Q',base+0x5eb2d0),base+0x6b5ef0:struct.pack('<Q',context),
        context+0xd0:b'\0',context+0x3f04:b'\0'*4}
    session=NS(read=lambda a,n:memory[a],assert_identity=lambda:None)
    gui=NS(context_rva=0x6b5ef0);candidate={'actor_address':hex(actor),'owned_booth_uid':77}
    assert hover.assert_selected(session,gui,base,candidate)['selected_actor']==hex(actor+0x10)
    memory[manager+0xd8]=struct.pack('<Q',actor+0x20)
    with pytest.raises(hover.HoverNotReady):hover.assert_selected(session,gui,base,candidate)
    memory[manager+0xd8]=struct.pack('<Q',actor+0x10);memory[context+0xd0]=b'\1'
    with pytest.raises(hover.HoverNotReady):hover.assert_selected(session,gui,base,candidate)


@contextmanager
def nothing(*args,**kwargs):yield


def test_durable_marker_precedes_click_and_uncertain_never_replays(state,monkeypatch):
    x=state;grant={'scope':'listing_1078','character':'Dutch','request_id':'merchant-refill:Dutch:1',
        'expires_at':time.time()+45,'listing_authority':{'mode':'open_panel','panel_request_id':x.key,
        'profile_id':x.profile.id,'farmer_target':{'pid':10}}}
    x.ui.grant=grant;x.ui.app=NS(control=NS(snapshot=lambda:{'enabled':False,'revision':1}))
    x.ui.safe_to_yield=lambda:True
    x.ui.coordinator=NS(fence=NS(bind_worker=nothing),owned_panel_scope=nothing,lease=nothing)
    monkeypatch.setattr(panel,'_idle',lambda *a,**k:None)
    ...
    monkeypatch.setattr('conquest.merchants.reader_1078.open_read_only_1078',
        lambda *a:NS(read_manual_ownership=lambda:{**x.snap,'timestamp':time.time()}))
    candidate={'actor_address':'0x123456','owned_booth_uid':77,'projection_candidate':{'candidate_point':[500,400]}}
    monkeypatch.setattr('conquest.merchants.booth_target_1078.collect',lambda *a:deepcopy(candidate))
    ...
    monkeypatch.setattr('conquest.layout_revision.SharedLayoutRevision',lambda *a,**k:
        NS(stable=lambda:revision,assert_current=lambda r:None))
    calls=[]
    def click(*args,**kwargs):
        kwargs['before_press']();kwargs['before_mouse_down']()
        with x.j.db() as db:
            assert db.execute("SELECT COUNT(*) FROM transaction_steps WHERE stage='panel_click'").fetchone()[0]==1
    ...
    panel.step(x.ui,'Dutch',opened(x))
    assert calls==['mouse-down-boundary'] and panel.status(x.j,'Dutch',x.key)['phase']=='verified'
