...
@pytest.mark.parametrize('change',[None,'farmer_name','trade','farmer_request','foreign_request',
                                 'identity','uid','currency','position','inventory','stale'])
def test_cancel_requires_exact_unchanged_participants(change):
    intent,f,m=participants('AnotherFarmer' if change=='farmer_name' else 'Parasite')
    if change=='trade':m['trade']={'participant':'Parasite'}
    if change=='farmer_request':f['request']={'participant':'Someone'}
    if change=='foreign_request':m['request']['participant']='Someone'
    if change=='identity':m['identity']={'pid':3}
    if change=='uid':m['character_uid']=3
    if change=='currency':f['silver']=199
    if change=='position':m['position']=[11,10]
    if change=='inventory':f['inventory']=[]
    if change=='stale':m['timestamp']-=20
    if change in (None,'farmer_name'):probe.unchanged(intent,f,m)
    else:
        with pytest.raises(ValueError):probe.unchanged(intent,f,m)


@pytest.mark.parametrize('case',['success','cancelled','expired','moved','hover','after_identity','after_stock'])
def test_cancel_click_is_guarded_and_result_must_reconcile(monkeypatch,tmp_path,case):
    ...
    intent,f,m=participants();state={'character':'Spiritual','intent':intent}
    clock=[10.0];presses=[];saved=[]
    window={'name':'###Confirm','geometry':[1,2,300,200]}
    control_state={'enabled':False,'revision':5}
    ui=NS(closed=False,app=NS(closing=False,control=NS(snapshot=lambda:dict(control_state))),
    ...
    driver=NS(target=NS(snapshot=lambda:{'client_size':[1000,800]}),
              memory=NS(gui=NS(viewport_size=lambda:[1000,800],assert_hovered=hovered)))
    ui.runtime=NS(enabled=lambda name:False,controllers={'Spiritual':NS(driver=driver)})
    ...
    def click(target,x,y,size,**kwargs):
        ...
        if case=='cancelled':ui.calibration_cancel['Spiritual'].set()
        if case=='expired':clock[0]+=16
        if case=='moved':window['geometry'][0]+=1
        kwargs['before_press']()
        presses.append((x,y));m['request']=None
        if case=='after_identity':m['identity']={'pid':99}
        if case=='after_stock':f['inventory']=[]
    monkeypatch.setattr(foreground,'foreground_click',click)
    if case=='success':
        probe.run(ui,state)
        assert state['phase']=='cancel_verified' and presses==[(100,122)]
    else:
        with pytest.raises((ValueError,CaptureUnavailable)):probe.run(ui,state)
