    ...
    panels = [
        {
            "name": "Inventory",
            "address": 9,
            "geometry": [10, 20, 200, 300],
            "scroll": [0, 0],
        }
    ]
    target = SimpleNamespace(hwnd=7)
    manager = SharedLayoutRevision(
        target,
        native_reader=native(state),
        windows=lambda: panels,
        gui_size=lambda: (1000, 800),
...
def test_manual_input_and_offscreen_native_window_are_not_actionable():
    ...
    manager = SharedLayoutRevision(
        SimpleNamespace(hwnd=7), native_reader=native(state), manual_active=lambda: True
    )
    with pytest.raises(LayoutChanged, match="Manual input"):
        manager.read()
    manager = SharedLayoutRevision(
        SimpleNamespace(hwnd=7),
...
def test_structural_mismatch_invalidates_cache_even_if_layout_returns_to_old_value():
    ...
    manager.qualified()
    assert now[0] >= qualified_at + 0.25
