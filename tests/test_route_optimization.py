...
from conquest.route_optimization import comparison_plan, fixed_route_windows, queue_area, rank_candidates
from conquest.routes import RouteLibrary


def sample(rid, index, kills, seconds=900, **changes):
    return dict(sample_id=f'{rid}-{index}', route_id=rid, verified_kills=kills,
                elapsed_seconds=seconds, memory_verified=True, comparable=True,
                deaths=0, **changes)
...
def test_long_downtime_is_included_in_pooled_rate():
    rows=[sample('a',1,300),sample('a',2,300,1800),
          sample('b',1,240),sample('b',2,240)]
...
def test_short_burst_and_missing_comparison_cannot_select_winner():
    rows=[sample('a',1,500),sample('a',2,500),sample('b',1,0),sample('b',2,100,300)]
    result=rank_candidates(rows,['a','b'])
...
def test_unsafe_fast_route_is_rejected_even_when_sample_interrupted():
    rows=[sample('a',1,100),sample('a',2,100),
          sample('b',1,500,300,unsafe=True)]
    result=rank_candidates(rows,['a','b'])
    assert result['winner']=='a'
    assert result['rejected']==[{'route_id':'b','eligible':False,'reason':'unsafe'}]


def test_duplicate_or_unverified_results_cannot_qualify():
    rows=[sample('a',1,500),sample('a',2,500),sample('b',1,0),sample('b',2,0)]
...
def test_fixed_windows_keep_downtime_and_do_not_count_partial_or_boundary_twice():
    ...
    assert windows[0]['kills_per_minute'] == .8
