...
class ObservedLock:
    ...
    def acquire(self, *args, **kwargs):
        ...
        if not acquired:self.waiting.set()
        return acquired
...
def prepare(x, role, transition):
    if transition == 'trade_to_terminal':
        ...
        x.farmer['request'] = {'participant': 'Dutch', 'participant_uid': 123,
...
def advance(x, transition, delay=6):
    ...
    if transition == 'request_to_trade':open_trade(x)
    else:
        x.farmer['trade'] = None
...
def test_fresh_observation_waiting_in_input_queue_cannot_create_a_manual_hold(
    ...
    monkeypatch.setattr(manual_farmer, 'controller', lambda *_a: pytest.fail('Deferred observation cannot qualify input'))
    if entry == 'manual':call = lambda: x.runtime.process_manual(owner, snapshot)
    elif role == 'farmer':call = x.runtime.observe_manual_farmer
    else:call = lambda: x.runtime.step('Dutch')
    ...
    if transition == 'request_to_trade':
        ...
        fresh['request'] = {'participant': 'Visitor', 'participant_uid': 777,
...
@pytest.mark.parametrize('role', ['farmer', 'merchant'])
@pytest.mark.parametrize('fault', ['stale_on_entry', 'missing_evidence', 'changed_while_queued'])
def test_invalid_observation_is_not_hidden_by_coordinator_wait(supervised, role, fault):
    x = supervised
    open_trade(x, phase='offer_verified', offered=True)
    owner = 'Farmer' if role == 'farmer' else 'Dutch'
    snapshot = x.farmer_read() if role == 'farmer' else x.read()
    if fault == 'stale_on_entry':snapshot['timestamp'] -= 6
    if fault == 'missing_evidence':snapshot.pop('inventory')
    def finish():
        advance(x, 'trade_to_terminal')
        if fault == 'changed_while_queued':snapshot.pop('inventory')
...
def test_expired_queued_observation_preserves_existing_genuine_session(supervised, role, approved):
    x = supervised
    owner = 'Farmer' if role == 'farmer' else 'Dutch'
    incoming = x.farmer_read() if role == 'farmer' else x.read()
    incoming['request'] = {'participant': 'Visitor', 'participant_uid': 777,
                           'message': 'Visitor wishes to trade with you.'}
    store = x.runtime.manual_sessions
    row = store.begin_request(owner, incoming, now=x.now)
    if approved:row = store.allow_and_activate(row['approval_binding'], incoming, operator='Floor', now=x.now)
    x.runtime._sync_manual_fence()
    original_row, audit, fence = deepcopy(store.get(row['id'])), store.audit(), deepcopy(x.guard.manual_sessions)
    open_trade(x, phase='offer_verified', offered=True)
    snapshot = x.farmer_read() if role == 'farmer' else x.read()
    queued_call(x, lambda: x.runtime.process_manual(owner, snapshot), lambda: advance(x, 'trade_to_terminal'))
...
def test_probe_queue_deferral_never_becomes_a_bilateral_proof(supervised, require_bilateral, explicit_time):
    x = supervised
    open_trade(x, phase='offer_verified', offered=True)
    snapshot = x.read()
    evidence_now = x.now if explicit_time else None
    result = queued_call(x, lambda: x.runtime.process_probe_owned('Dutch', snapshot,
        require_bilateral=require_bilateral, now=evidence_now), lambda: advance(x, 'trade_to_terminal'))
