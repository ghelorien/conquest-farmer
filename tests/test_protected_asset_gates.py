...
def locked_assets(monkeypatch,tmp_path):
    ...
    journal.begin({'operation_id':'withdrawal-1','plan_id':'qualification-1','uid':101,
                   'phase':'prepared','intent':{'item':{'uid':101}}})
    return journal


@pytest.mark.parametrize('phase',['prepared','input_maybe_sent','reconciling','blocked'])
def test_every_unresolved_phase_blocks_reload_before_other_journal_exists(locked_assets,monkeypatch,tmp_path,phase):
    if phase!='prepared':
...
def test_terminal_no_transfer_releases_reload_gate(locked_assets,monkeypatch,tmp_path):
    locked_assets.transition('withdrawal-1','prepared','no_transfer',receipt={'input_attempted':False})
...
def test_malformed_protected_journal_cannot_be_ignored(monkeypatch,tmp_path):
    path=tmp_path/'broken.sqlite3';path.write_bytes(b'not a sqlite database')
    monkeypatch.setattr(protected,'JOURNAL',path)
    with pytest.raises(ValueError,match='inventory reconciliation'):
        delivery.guard_protected_assets()


def test_ui_on_and_bridge_on_reject_before_changing_intent_or_clearing_halt(locked_assets,monkeypatch):
    ...
    notes=[];records=[];app.record=lambda **fields:records.append(fields)
    monkeypatch.setattr(storage_halt,'clear_by_user',lambda:pytest.fail('On cleared halt before asset lock check'))
...
def test_manual_off_still_works_with_asset_lock(locked_assets):
    ...
    from conquest.desktop_app import DesktopApp
    app=DesktopApp.__new__(DesktopApp);app.control=FarmingControl()
    app.control.update({'enabled':True});app.messages=Queue();app.thread=None
...
def test_runner_cannot_resume_old_intent_before_runtime_exists(locked_assets):
    from conquest.control import FarmingControl
    from conquest.desktop_app import DesktopApp
    app=DesktopApp.__new__(DesktopApp);app.control=FarmingControl();app.runtime=None
    app.control.update({'enabled':True});records=[];app.record=lambda **fields:records.append(fields)
...
def test_new_deliveries_cannot_use_locked_assets_but_status_remains_read_only(locked_assets,monkeypatch,tmp_path):
    monkeypatch.setattr(delivery,'JOURNAL',tmp_path/'delivery.sqlite3')
    ui=SimpleNamespace()
    for action in ('delivery-start','delivery-test'):
        with pytest.raises(ValueError,match='inventory reconciliation'):
            delivery.dispatch(ui,{'action':action,'request_id':'other-operation','character':'Dutch','uids':[101]})
    assert not delivery.JOURNAL.exists()
    result=delivery.dispatch(ui,{'action':'delivery-status','request_id':'other-operation'})
