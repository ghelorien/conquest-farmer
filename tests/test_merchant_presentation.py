...
def test_slow_observer_does_not_block_ui_snapshot_or_replace_last_complete_view(
    tmp_path,
):
...
@pytest.mark.parametrize(
    "released,owner,attached,expected",
    [
        (False, None, False, True),
        (True, None, False, False),
        (False, "Farmer", False, False),
        (False, None, True, False),
    ],
)
def test_selected_client_shows_without_reading_shop_state_or_verifying(
    released, owner, attached, expected
):
    ui = SimpleNamespace(
        closed=False,
        ...
        calibrating=set(),
        notebook=SimpleNamespace(select=lambda: "Dutch"),
        frames={"Dutch": "Dutch", "Spiritual": "Spiritual"},
        released_clients={"Dutch"} if released else set(),
        client_panes={"Dutch": SimpleNamespace(winfo_ismapped=lambda: True)},
        auto_embed_retry={},
        hosts={"Dutch": SimpleNamespace(saved=object())} if attached else {},
        runtime=SimpleNamespace(
            observers={"Dutch": object()}, manual_handoff_status=lambda: None
        ),
        embed_client=Mock(),
    )
...
@pytest.mark.parametrize(
    "mode", ["delayed", "covered", "behind_wrapper", "wrong_size", "stop"]
)
def test_input_waits_for_exclusive_native_surface(mode):
    ...
    now = [0.0]
    gui = SimpleNamespace(
        GetClientRect=lambda p: (0, 0, 1200, 800),
        ClientToScreen=lambda p, xy: (2500, 200),
        GetAncestor=lambda h, flag: 10,
        IsWindowVisible=lambda h: h == 10 or now[0] >= 0.05,
        IsIconic=lambda h: False,
        GetWindowRect=lambda h: (
            2500,
            200,
            3700 if mode != "wrong_size" else 3600,
            1000,
        ),
    )
    host = SimpleNamespace(
        saved=SimpleNamespace(hwnd=20, identity={}),
        parent=10,
        api=SimpleNamespace(
            gui=gui,
            assert_owner=lambda *a: None,
            is_above=lambda *a: mode != "behind_wrapper",
        ),
    )
    sibling = SimpleNamespace(
        saved=SimpleNamespace(hwnd=30),
        api=SimpleNamespace(
            gui=SimpleNamespace(
                IsWindowVisible=lambda h: mode == "covered" or now[0] < 0.1
            )
        ),
    )

    def check():
        if mode == "stop" and now[0] >= 0.025:
    ...
    if mode == "delayed":
        wait_for_merchant_surface(
            host, [sibling], check, clock=lambda: now[0], sleep=sleep
        )
        assert now[0] >= 0.1
    else:
...
def test_handoff_hides_previous_native_client_before_resizing_selected():
    ...
    hosts = {
        c: SimpleNamespace(
            saved=SimpleNamespace(hwnd=h, identity={"pid": h}),
            api=SimpleNamespace(
                gui=SimpleNamespace(GetForegroundWindow=lambda: 0),
                assert_owner=lambda *a: None,
    ...
    ui = SimpleNamespace(
        ...
        runtime=SimpleNamespace(
            observers={
                c: SimpleNamespace(adapter=SimpleNamespace(identity=h.saved.identity))
                for c, h in hosts.items()
            }
        ),
        notebook=SimpleNamespace(select=lambda *a: calls.append(("select", *a))),
        frames={"Spiritual": "S", "Dutch": "D"},
        detail_tabs={c: SimpleNamespace(select=lambda *a: None) for c in hosts},
        input_bookmarks={},
        root=SimpleNamespace(update_idletasks=lambda: None),
        resize_merchant=lambda c, **kw: calls.append(("resize", c)),
    )
