...
def test_pickup_requires_new_item_of_selected_type_and_quantity_gain():
    ...
    assert (
        attempt.outcome(replace(before, items=before.items + (other,)), 12)
        == "unverified"
    )
    new = Item(3, 1000000, 1, 1, 1)
    assert (
        attempt.outcome(replace(before, items=before.items + (new,)), 10.5)
        == "verified"
    )
...
def test_exact_drop_label_excludes_wrong_color_and_far_items():
    ...
    assert (
        nearby_drops(frame, template, (435, 460), (405, 442, 440, 472), max_distance=50)
        == []
    )
