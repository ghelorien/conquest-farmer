...
def farmer():
    ...
    data = {'monsters':[{'entity_id':77,'type_id':1,'object_address':177,'alive':True},
                        {'entity_id':88,'type_id':1,'object_address':188,'alive':True}],
            'observations_available':True, 'focused':False,
            'loot_observation':LootObservation('client/map',10,(),(),inventory)}
    now = [10]
    runtime = ControlRuntime(control,None,None,None,'Parasite',observer=lambda:dict(data),
        dispatcher=lambda m,*_:calls.append(('attack',m['entity_id'])),
        pickup_dispatcher=lambda d,*_:calls.append(('pickup',d.uid)),clock=lambda:now[0])
    def step(time, **changes):
        ...
        data['loot_observation'] = replace(previous,timestamp=time,inventory=inventory,**changes)
        runtime.step()
...
def test_killed_id_loot_precedes_next_monster_and_confirms_stack_gain():
    ...
    step(10.3,kills=(kill(),),drops=(drop(),replace(drop(102),kill_event_id='other-kill')))
    assert calls==[('attack',77),('pickup',101)]
    inventory = replace(data['loot_observation'].inventory,items=(replace(old_stack,amount=6),))
...
@pytest.mark.parametrize('evidence', [(),(replace(kill(),monster_id=88),),
    (replace(kill(),object_address=999),),(replace(kill(),timestamp=9),)])
...
def test_full_inventory_blocks_pickup_and_retargeting():
    runtime, control, calls, data, step = farmer()
    step(10)
    inventory = replace(data['loot_observation'].inventory,capacity=1,items=(Item(1,55,1,1,0),))
...
def test_missing_loot_reader_blocks_attack_even_with_attack_dispatcher():
    ...
    runtime.step()
    assert not calls
    assert 'Monster-to-loot observations are not connected' in control.snapshot()['note']
