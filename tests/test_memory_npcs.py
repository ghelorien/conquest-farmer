...
@pytest.mark.parametrize('map_id,name,position,expected_y', [
    (1036, 'Warehouseman', (182, 180), 249),
    (1011, 'Warehouseman', (227, 246), 281),
    (1036, 'MillionaireLee', (242, 242), 281),
])
def test_warehouse_interaction_point_preserves_other_npcs(map_id,name,position,expected_y):
...
class Replay:
    def __init__(self):
        self.layout = EntityLayout.model_validate(yaml.safe_load(
            Path('profiles/classic-1074-entities-candidate.yaml').read_text()))
        self.expected_sha256 = self.layout.expected_sha256
        self.modules = [{'name':'ImConquer.exe', 'base':0x140000000, 'size':0x3000000}]
        rows = json.loads(Path('tests/fixtures/town-vendor-memory.json').read_text())['records']
        self.blocks = {r['address']:bytearray(base64.b64decode(r['data_base64'])) for r in rows}
        self.pointers = {
            0x140000000+self.layout.root_rva:0x200000, 0x200018:0x300000,
            0x300008:0x400000, 0x400000:0x140000000+self.layout.collection_vtable_rva,
            0x400058:0x500000, 0x400060:0x500020, 0x400068:0x500020,
    ...
    def request(self, operation, body):
        ...
        for f in body['fields']:
            ...
            if address in self.pointers:
                ...
                obj = next(obj for obj, b in self.blocks.items() if obj <= address < obj+len(b))
                data, offset = self.blocks[obj], address-obj
                if f['kind'] == 'utf8':
                    value = bytes(data[offset:offset+64]).split(b'\0')[0].decode('utf-8')
                else:
                    fmt = {'u64':'<Q', 'u32':'<I', 'i32':'<i', 'xy_u32':'<II'}[f['kind']]
...
def test_recorded_town_vendors_have_ids_types_and_positions(replay):
    ...
    assert (rows[100102].name, rows[100102].type_id, rows[100102].position) == ('Pharmacist', 3, (466,327))
    assert (rows[100104].name, rows[100104].type_id, rows[100104].position) == ('Blacksmith', 5, (452,330))
...
def test_mismatched_vendor_type_model_or_world_coordinate_rejected(replay, offset, value):
    memory, reader = replay
...
def test_expired_snapshot_is_not_usable(replay):
    _, reader = replay
    reader.clock = iter([0., .6]).__next__
...
def test_warehouse_type_zero_requires_matching_model_name_and_tile(replay):
    ...
    assert any(n.name=='Warehouseman' and n.position==(409,351) for n in reader.read(1002).npcs)
...
def test_phoenix_service_roles_resolve_city_specific_memory_types():
    ...
    assert (expected.type_id,expected.model,expected.position)==(10014,230,(189,252))
    assert vendor_identity(1002,3).type_id==3
    with pytest.raises(ValueError):vendor_identity(1011,1)
