...
from conquest.merchants.background_probe import diagnostic_lease, start_probe, reserved_surface, start_surface_restore
...
def test_diagnostic_lease_excludes_existing_input_without_focus_or_mouse_idle(tmp_path):
    coordinator = SimpleNamespace(lock=threading.RLock(), owner=None, thread=None, stopped=False,
        path=tmp_path/'input.lock', on_acquire=Mock(side_effect=AssertionError('No focus callback')),
        manual_active=Mock(side_effect=AssertionError('Unrelated mouse activity is permitted')))
    ui = SimpleNamespace(coordinator=coordinator, safe_to_yield=lambda:True, calibrating=set(),
        runtime=SimpleNamespace(refilling={},controllers={'Spiritual':object()},enabled=lambda c:False,
    ...
    with pytest.raises(RuntimeError, match='probe failed'):
        with diagnostic_lease(ui,'Spiritual'):
            ...
            with pytest.raises(ValueError,match='idle'):
                with diagnostic_lease(ui,'Dutch'):pass
    ...
    with pytest.raises(ValueError,match='idle'):
        with diagnostic_lease(ui,'Spiritual'):pass
...
@pytest.mark.parametrize('mode',['post-hover','park-barrier-click','park-ctrl','park-drag-cancel','park-price-cancel'])
...
def test_running_probe_cannot_be_replaced():
    with pytest.raises(ValueError,match='already running'):
        start_probe(SimpleNamespace(background_probe={'state':'running'}),'Dutch','observe')


def test_failed_surface_restore_is_retained_and_can_be_retried(monkeypatch,tmp_path):
    class Surface:
        def __init__(self): self.saved = None; self.fail = True
        def park(self, *args, **kwargs): self.saved = object()
        def restore(self):
            if self.fail: raise ValueError('Native restore refused')
            self.saved = None
    ...
    ui.coordinator = SimpleNamespace(lock=threading.RLock(),owner=None,path=tmp_path/'input.lock')
    ui.ui_requests = Mock()
    monkeypatch.setattr('conquest.merchants.background_probe.threading.Thread',
...
def test_restore_failure_keeps_recovery_record_and_never_calls_completion(monkeypatch,tmp_path):
    ui=SimpleNamespace(background_surfaces={'Dutch':SimpleNamespace(restore=Mock(side_effect=ValueError('manual takeover')))},
        background_probe={'state':'restoration_required'},
        coordinator=SimpleNamespace(lock=threading.RLock(),owner=None,path=tmp_path/'input.lock'),ui_requests=Mock())
    monkeypatch.setattr('conquest.merchants.background_probe.threading.Thread',
                        lambda *,target,**kwargs:SimpleNamespace(start=target))
...
def test_partial_park_error_still_restores_and_releases_reservation(monkeypatch):
    class Surface:
        ...
        def restore(self):self.saved=None
    monkeypatch.setattr('conquest.background_surface.BackgroundSurface',Surface)
    ui=SimpleNamespace(background_surfaces={})
    target=SimpleNamespace(hwnd=20,snapshot=lambda:{'client_size':[1036,793]})
    with pytest.raises(ValueError,match='acknowledgement'):
        with reserved_surface(target,{},'park-observe',ui=ui,character='Dutch'):pass
...
def test_ui_close_defers_until_window_can_be_restored(state):
    ...
    ui=SimpleNamespace(background_probe={'state':state},
        background_surfaces={'Dutch':object()} if state=='restoration_required' else {},
        defer_background_action=Mock(),closed=False,
...
def test_explicit_embed_waits_while_periodic_layout_cannot_touch_parked_window():
    from conquest.merchants.ui import UnifiedUI
    ui=SimpleNamespace(background_probe={'state':'restoration_required'},
        background_surfaces={'Dutch':object()},defer_background_action=Mock())
