...
def test_alternate_is_long_visible_and_never_repeats_failed_landing():
    ...
    for _ in range(3):
        p = recovery_landing(
            t, (200, 200), (230, 200), (518, 396), failed=failed, used=used
        )
        assert (
            p not in used | failed
            and 8 <= max(abs(a - b) for a, b in zip(p, (200, 200))) <= 12
        )
        assert clear_segment(t, (200, 200), p)
        length = max(abs(p[0] - 200), abs(p[1] - 200))
        direction = ((p[0] - 200) / length, (p[1] - 200) / length)
        assert direction not in directions
...
def test_failed_bridge_jump_keeps_running_until_crossing_cleared(monkeypatch):
    ...
    loop.terrain.map_id = 1011
    position = [200, 200]
    steps = []
    loop.living = lambda: {"embedded_controls": {"life": {"position": position.copy()}}}
    loop.care = NS(check=lambda h: None, session=None)
    ...
    def step(target, expected_position):
        ...
        if position[0] < 212:
            assert distance <= 4, "Retried jump before crossing cleared"
        position[:] = target
        return {"reached": True}
...
def test_market_travel_recovers_early_and_remains_bounded(monkeypatch, mode, map_id):
    ...
    recovery_event = (
        "market_movement_recovery" if map_id == 1036 else "town_movement_recovery"
    )

    def living():
        if mode == "manual_stop" and len(steps) >= 2:
    ...
    if mode == "recover":
        ...
        with pytest.raises(
            ValueError, match="obstructed|no position progress|no improving progress"
        ):
            loop.travel((230, 200))
