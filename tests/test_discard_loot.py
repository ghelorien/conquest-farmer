...
import pytest
from conquest.discard_loot import discard_candidate, ground_key, ignored_drop, removal_received
from conquest.memory_inventory import Item
from conquest.memory_ground import GroundItem


@pytest.mark.parametrize('type_id,plus,slot,expected',[
    (480003,0,0,True),(480003,1,0,False),(480003,None,0,False),
    (480003,0,None,False),(480007,0,0,False),(1088001,0,0,False),
    (1088000,0,0,False),(700001,0,0,False),(1000020,0,0,False),
    (1050000,0,0,False),(1000000,0,0,True),(1001000,0,0,True)])
def test_discard_preserves_unknown_plus_equipped_rare_and_supplies(type_id,plus,slot,expected):
...
def test_discard_receipt_requires_exact_removal_and_no_other_losses():
    item=Item(1,480003,1,1,0,0);keep=Item(2,1088001,1,1,1,0)
    before=NS(items=(item,keep),silver=100)
    assert removal_received(item,before,NS(items=(replace(keep,slot=0),),silver=100))
...
def test_persisted_discard_attempt_is_never_repeated(monkeypatch,tmp_path):
    ...
    path=tmp_path/'journal.json';write_json(path,[{'uid':1,'state':'attempted'}])
    monkeypatch.setattr(module,'JOURNAL',path)
    monkeypatch.setattr(module,'MemoryGroundReader',lambda entities:None)
    trade=NS(observer=NS(entities=None),inventory=NS(read=lambda:NS(items=(Item(1,480003,1,1,0,0),))))
    discarder=module.DiscardLoot(trade)
    with pytest.raises(ValueError,match='already attempted'):discarder.discard(1)
...
def test_inventory_button_uses_current_table_column_and_rejects_stale_frame():
    ...
    base=0x140000000;context=0x100000;table=0x200000;columns=0x300000
    record=bytearray(536);column=bytearray(104)
    ...
    data={base+0x6966f0:struct.pack('<Q',context),context+0x4338:struct.pack('<IIQ',1,1,table),
          context+0x3e38:struct.pack('<I',100)}
    def read(address,size):
        if address==table:return bytes(record)
        if address==columns+104:return bytes(column)
        if address==columns+104+0x34:return bytes(column[0x34:0x3c])
        return data[address]
    gui=NS(base=base,session=NS(read_block=read,assert_identity=lambda:None),
           read=lambda name:NS(position=(53,691),size=(930,102)))
    assert inventory_button(gui)==(656,753)
    data[context+0x3e38]=struct.pack('<I',104)
    with pytest.raises(ValueError,match='not current'):inventory_button(gui)
...
def test_native_looter_ignores_discarded_record_but_accepts_a_new_drop():
    ...
    supervisor.map_id=1002;supervisor.ground_items=lambda:(drop,)
...
def test_uncertain_drag_is_quarantined_panel_closed_and_farming_can_continue(monkeypatch,tmp_path):
    from conquest import discard_loot as module
    monkeypatch.setattr(module,'JOURNAL',tmp_path/'journal.json')
    discarder=module.DiscardLoot.__new__(module.DiscardLoot)
    discarder.records=[];discarder.attempted=set();discarder.cleanup_pending=False
    calls=[]
    def attempt(uid):
        calls.append(uid)
    ...
    def close():discarder.cleanup_pending=False
    discarder._discard=attempt;discarder.close_inventory=close
    result=discarder.discard(42)
    assert result['state']=='unverified' and not discarder.cleanup_pending
    with pytest.raises(ValueError,match='already attempted'):discarder.discard(42)
    assert calls==[42]
    from conquest.discord_notify import read_json
    assert read_json(module.JOURNAL)[0]['state']=='unverified'


def test_cleanup_remains_pending_if_character_dies_during_uncertain_drag(monkeypatch,tmp_path):
    from conquest import discard_loot as module
    from conquest.capture import CaptureUnavailable
    monkeypatch.setattr(module,'JOURNAL',tmp_path/'journal.json')
    discarder=module.DiscardLoot.__new__(module.DiscardLoot)
    discarder.records=[];discarder.attempted=set();discarder.cleanup_pending=True
    def attempt(uid):
        discarder.records.append({'uid':uid,'state':'attempted'})
        discarder.attempted.add(uid)
        raise ValueError('No receipt')
    def close():raise CaptureUnavailable('Character died')
    discarder._discard=attempt;discarder.close_inventory=close
    assert discarder.discard(42)['state']=='unverified'
    assert discarder.cleanup_pending


def test_pending_panel_cleanup_runs_before_new_discard_without_inventory_candidates(monkeypatch):
    from contextlib import nullcontext
    from conquest import native_farm
    monkeypatch.setattr(native_farm,'logical_coordinates',nullcontext)
    supervisor=native_farm.NativeFarmSupervisor.__new__(native_farm.NativeFarmSupervisor)
    calls=[]
    supervisor.discarder=NS(cleanup_pending=True,close_inventory=lambda:calls.append('close'))
    supervisor.dispatch=lambda callback:callback()
...
def test_pre_drag_reader_failure_defers_without_marking_item_attempted(failure):
    ...
    discarder=module.DiscardLoot.__new__(module.DiscardLoot)
    discarder.records=[];discarder.attempted=set();discarder.cleanup_pending=True
    def attempt(uid):raise failure('Ground observation unavailable before discard; no drag sent')
    def close():discarder.cleanup_pending=False
    discarder._discard=attempt;discarder.close_inventory=close
    result=discarder.discard(42)
    assert result['state']=='deferred' and result['retry_after']==10
    assert discarder.next_attempt_at>time.monotonic()+9
    assert not discarder.records and not discarder.attempted and not discarder.cleanup_pending


def test_pre_drag_capture_failure_defers():
    from conquest.capture import CaptureUnavailable
    test_pre_drag_reader_failure_defers_without_marking_item_attempted(CaptureUnavailable)
...
def test_discard_cooldown_leaves_combat_available():
    ...
    supervisor=NativeFarmSupervisor.__new__(NativeFarmSupervisor)
    supervisor.pending_loot=None;supervisor.defending=False
    supervisor.discarder=NS(cleanup_pending=False,next_attempt_at=time.monotonic()+10)
    assert not supervisor.discard_step(NS(items=(Item(1,480003,1,1,0,0),)))


@pytest.mark.parametrize('failure',[ValueError,OSError])
def test_pending_panel_reader_failure_retries_instead_of_terminating(monkeypatch,failure):
    from contextlib import nullcontext
    from conquest import native_farm
    from conquest.capture import CaptureUnavailable
    monkeypatch.setattr(native_farm,'logical_coordinates',nullcontext)
    supervisor=native_farm.NativeFarmSupervisor.__new__(native_farm.NativeFarmSupervisor)
    def close():raise failure('GUI snapshot changed')
...
def test_ground_failure_does_not_open_inventory():
    ...
    def ground():raise ValueError('Ground scene changed during sampling')
    trade=NS(inventory=NS(read=lambda:NS(items=(Item(1,480003,1,1,0,0),))),
    ...
    d.trade=trade;d.attempted=set();d.cleanup_pending=False
