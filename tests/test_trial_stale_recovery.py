...
def test_native_targets_use_their_own_age_after_slow_skill_checks(
    tmp_path, monkeypatch, scan_delay
):
    import win32api
    ...
    class Inventory:
        ...
        def read(self):
            return InventorySnapshot(
                now[0], now[0], (), Item(2, 1050000, 200, 200, None), 0, 40
            )
    ...
    def targets(*args):
        now[0] += scan_delay
        return [Target("Pheasant", 600, 400, 1)]

    supervisor = NS(
        last_target=None,
        finish_target=lambda *a: None,
        recovery=NS(terrain=NS(width=1000, height=1000)),
        observe=lambda: {"waiting": False, "health_ratio": 1},
        xp_step=skill,
        memory_targets=targets,
        loot_step=lambda *a: False,
        dispatch=lambda cb, **kw: cb(),
    )
    ...
    assert bool(clicks) == (scan_delay < 0.35)


@pytest.mark.parametrize("manual_stop", [False, True])
def test_slow_native_samples_keep_recovery_alive_then_heal(
    tmp_path, monkeypatch, manual_stop
):
    import win32api
    ...
    class Inventory:
        ...
        def read(self):
            reads.append(1)
            now[0] += 0.1
            old = now[0] - 0.95 if len(reads) <= 4 else now[0]
            if manual_stop and len(reads) == 3:
                (output / "stop.request").write_text("Manual Stop")
            potions = () if keys else (Item(1, 1000000, 1, 1, 0),)
            return InventorySnapshot(
                old, now[0], potions, Item(99, 1050000, 200, 200, None), 0, 40
            )
    ...
    def observe():
        ...
        return {"waiting": False, "health_ratio": 0.9 if keys else 0.3}
    ...
    result = trial.run_trial(
        path, "unused", output, 4, logging.getLogger("test"), supervisor=supervisor
    )
    events = [
        e
        for (e,) in sqlite3.connect(output / "trial.sqlite3").execute(
