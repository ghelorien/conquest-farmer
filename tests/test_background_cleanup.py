...
from conquest.merchants.background_probe import (
    BackgroundCleanupRequired, cleanup_probe, settle_input, restore_surface, restoration_lease,
    reconciliation_read)


def state(frame, *, queued=0, down=False):
    return {'frame':frame,'context':0x10000,'queue':{'size':queued},
            'backend':{'buttons_down':int(down)},'mouse_down':[down]*5,
            'active':{'id':0},'modifiers':{'ctrl':False},'key_mods':0}
...
def reader_states(states):
    ...
    return SimpleNamespace(snapshot=snapshot,session=SimpleNamespace(read_block=lambda *a:bytes(0x285*16)))


def settle(reader, **kwargs):
    now=[0]
    def sleep(seconds):now[0]+=seconds
    return settle_input(reader,timeout=.1,clock=lambda:now[0],sleep=sleep,**kwargs)


def test_neutral_old_snapshot_does_not_pass_before_queued_down_and_release():
    reader=reader_states([state(10),state(10),state(11,queued=3),state(12,down=True),
                          state(13),state(14)])
...
def snapshots():
    return {'identity':{'pid':7},'inventory':[],'booth':[],'silver':100,'position':[232,213]}


def test_failed_input_still_reads_final_stock_and_keeps_surface_parked(monkeypatch):
    monkeypatch.setattr('conquest.merchants.background_probe.settle_input',
                        Mock(side_effect=ValueError('queued input remains')))
    journal=Mock()
    ui=SimpleNamespace(coordinator=SimpleNamespace(stop=Mock()),runtime=SimpleNamespace(journal=journal))
    report={'button_messages_sent':True,'outcome':'stopped','error':'manual cancellation'}
    driver=SimpleNamespace(read=Mock(return_value=snapshots()))
    surface=SimpleNamespace(restore=Mock(),before_restore=lambda:cleanup_probe(ui,'Dutch',report,object(),driver,snapshots()))
...
def test_changed_stock_is_not_reported_as_success_or_emergency_input_stop(monkeypatch):
    monkeypatch.setattr('conquest.merchants.background_probe.settle_input',lambda *a,**k:{'settled':True})
    ui=SimpleNamespace(coordinator=SimpleNamespace(stop=Mock()),runtime=SimpleNamespace(journal=Mock()))
    before=snapshots();after={**before,'silver':200}
    report={'input_messages_sent':True,'outcome':'observed'}
    cleanup_probe(ui,'Dutch',report,object(),SimpleNamespace(read=lambda:after),before)
...
def test_failed_read_is_unknown_even_if_old_success_evidence_exists():
    ui=SimpleNamespace(coordinator=SimpleNamespace(stop=Mock()),runtime=SimpleNamespace(journal=Mock()))
    report={'stock_unchanged':True,'position_unchanged':True,'outcome':'observed'}
    cleanup_probe(ui,'Dutch',report,object(),SimpleNamespace(read=Mock(side_effect=ValueError('disconnected'))),snapshots())
...
def test_final_reconciliation_retries_gui_races_without_repeating_input():
    ...
    driver=SimpleNamespace(read=Mock(side_effect=[ValueError('Invalid GUI geometry'),
        GuiObservationChanged('GUI registry changed'),snapshots()]))
...
@pytest.mark.parametrize('error',[ValueError('Process identity changed'),
    ValueError('GUI frame counter reset during observation'),OSError('Memory read failed')])
...
def test_restoration_respects_separate_process_lock_and_still_works_when_stopped(tmp_path):
...
"""
    ...
    ui=SimpleNamespace(coordinator=SimpleNamespace(lock=threading.RLock(),owner=None,thread=None,path=path,stopped=True))
    try:
        assert child.stdout.readline().strip()=='ready'
        with pytest.raises(OSError):
            with restoration_lease(ui):pytest.fail('Foreign lease was ignored')
