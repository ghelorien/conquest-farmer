...
def bank(monkeypatch):
    ...
    def town(action, **fields):
        if action == "supplies":
            return {"items": deepcopy(carried)}
        if action == "warehouse-items":
            return {"items": deepcopy(stored), "capacity": state["capacity"]}
        assert action == "warehouse-deposit"
        uid = fields["uid"]
        calls.append(uid)
        value = next(value for value in carried if value["uid"] == uid)
        if state["uncertain"]:
            return {"stored": uid, "type_id": value["type_id"]}
        carried.remove(value)
        stored.append(value)
        return {
            "stored": uid,
...
def test_urgent_after_shopping_skips_optional_travel_only_when_storage_succeeds(
    monkeypatch, fits
):
    ...
    monkeypatch.setattr(
        b, "open_warehouse", lambda loop: {"silver": 200, "stored_silver": 0}
    )
    monkeypatch.setattr(b, "close_warehouse", lambda loop: calls.append("close"))
    monkeypatch.setattr(b, "transport_reserve", lambda: 200)
    monkeypatch.setattr(
        b, "stash_urgent_valuables", lambda loop: calls.append("urgent") or fits
    )
    monkeypatch.setattr(
        b, "stash_valuables", lambda loop, **kw: calls.append(("ordinary", kw))
    )
    monkeypatch.setattr(bank_stock_alerts, "record", lambda loop: None)
    assert b.after_shopping(NS(), urgent=True)
    assert calls == (
        ["urgent"] + ([] if fits else [("ordinary", {"deliver": True})]) + ["close"]
    )
...
def test_failed_priority_deposit_never_enters_consolidation_or_delivery(monkeypatch):
    ...
    def uncertain(loop):
        raise ValueError("deposit uncertain")

    monkeypatch.setattr(b, "stash_urgent_valuables", uncertain)
    monkeypatch.setattr(
        b,
        "stash_valuables",
        lambda *a, **kw: pytest.fail("No fallback after uncertain deposit"),
    )
