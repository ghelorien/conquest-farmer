...
def test_resupply_counts_include_equipped_and_spare_arrows():
    ...
    snapshot = {
        "items": [
            {"type_id": 1050000, "amount": 190},
            {"type_id": 1000020, "amount": 3},
        ],
        "equipped_ammo": {"type_id": 1050000, "amount": 20},
        "silver": 1000,
        "capacity": 40,
    }
...
@pytest.mark.parametrize(
    "field,value",
    [("potions", 0), ("free_slots", 0), ("arrows", 0), ("arrows", 1), ("arrows", 2)],
)
...
@pytest.mark.parametrize(
    "type_id", [500005, 500015, 130005, 1050000, 1000020, 710001, 420009, 421006]
)
def test_automatic_sales_keep_bows_armor_supplies_special_and_high_quality_items(
    type_id,
):
...
@pytest.mark.parametrize(
    "protected_type",
    [500005, 420005, 121007, 1088000, 1088001, 710001, 1000020, 1050000],
)
def test_town_trade_does_not_sell_a_protected_item(protected_type):
    ...
    trade.inventory = SimpleNamespace(
        read=lambda: SimpleNamespace(
            items=[SimpleNamespace(uid=1, type_id=protected_type)]
        )
    )
...
@pytest.mark.parametrize(
    "detail",
    [
        "NPC scene changed during observation",
...
@pytest.mark.parametrize(
    "detail",
    [
        "Inventory changed during observation",
...
def test_trade_receipt_read_retries_without_repeating_input(monkeypatch):
    ...
    trade = TownTrade.__new__(TownTrade)
    assert trade.verified_read(
        read, lambda value: value["silver"] == 800, "unverified"
    ) == {"silver": 800}
    assert len(reads) == 4
...
def test_restart_closes_panels_and_keeps_loot_space_when_supplies_are_adequate():
    ...
    def town(action, **fields):
        ...
        if action == "supplies":
            return snapshot
        assert action != "buy", (
            "Do not consume reserved loot space for optional top-ups"
        )
        return {"ok": True}
...
def test_full_protected_bag_reaches_storage_before_final_restock_check(
    monkeypatch, stored
):
    ...
    def review(vendor):
        ...
        if len(bag["items"]) == 3:
            bag["items"].append(
                {"uid": 13, "type_id": 500009, "amount": 1, "plus": 2, "slot": 3}
            )

    monkeypatch.setattr(
        equipment, "EquipmentReview", lambda loop: SimpleNamespace(visit=review)
    )
    calls = []
    ...
    loop.town = town
    loop.travel = lambda target: None
    loop.sell_junk = lambda vendor: None
    loop.record = lambda *args, **fields: None
    if stored:
        loop.restock()
        assert loop.cycles == 1
    else:
        with pytest.raises(ValueError, match="after restocking and storage"):
            loop.restock()
        assert loop.cycles == 0
...
def test_full_bag_is_stored_before_essential_shop_input(monkeypatch, space_freed):
    ...
    calls = []
    bag = {"items": [{"uid": 12, "type_id": 1088001}], "capacity": 1}
    loop = OvernightLoop.__new__(OvernightLoop)
    loop.town = lambda action, **fields: (
        bag if action == "supplies" else calls.append((action, fields))
    )
    loop.travel = lambda target: calls.append(("travel", target))
    loop.record = lambda *a, **kw: None
    monkeypatch.setattr(
        banking, "open_warehouse", lambda current: calls.append("warehouse")
    )
    monkeypatch.setattr(
        banking, "close_warehouse", lambda current: calls.append("closed")
    )
    ...
    if space_freed:
        ...
        with pytest.raises(ValueError, match="no purchase issued"):
            loop.shopping_space(5, (100, 200))
...
def test_unconfirmed_topup_only_defers_when_supplies_and_money_stay_unchanged(
    monkeypatch, change
):
    from conquest import overnight
...
def test_default_route_has_no_deadline_but_manual_stop_still_works(
    ...
    monkeypatch.setattr(
        overnight,
        "RouteLibrary",
        lambda: SimpleNamespace(load=lambda name: SimpleNamespace(map_id=1002)),
    )
    monkeypatch.setattr(overnight, "read_terrain", lambda *args: None)
    monkeypatch.setattr(
        overnight.ctypes.windll.user32, "GetAsyncKeyState", lambda key: 0
    )
    loop = OvernightLoop()
    ...
    with pytest.raises(OvernightStopped, match="Stopped by user"):
        loop.check_stop()
...
def test_time_limit_only_applies_when_explicit(monkeypatch, tmp_path):
    ...
    loop.check_stop()
    now[0] = 3600
    with pytest.raises(OvernightStopped, match="duration finished"):
        loop.check_stop()


@pytest.mark.parametrize(
    "reason", ["inventory_full", "ammo_unavailable", "potions_exhausted"]
)
def test_supply_runner_stop_hands_off_to_town_even_if_character_died(
    monkeypatch, reason
):
    from conquest import overnight
    ...
    loop.health = lambda: {
        "embedded_controls": {
            "control": {
                ...
                "note": "Farm runner stopped: " + reason,
            },
    ...
    loop.health = lambda: {
        "embedded_controls": {
            "control": {
                "enabled": True,
                "execution_state": "runner_stopped",
                "note": "Farm runner stopped: unknown_failure",
            }
        }
    }
    monkeypatch.setattr(overnight, "request", lambda *args, **kwargs: None)
...
@pytest.mark.parametrize(
    "kind,amount,total,expected",
    [
        (1050000, 25, 625, True),
        (1050000, 13, 900, True),
        (1050000, 25, 624, False),
        (1050000, 200, 2000, False),
        (1088000, 1, 2000, False),
        (500005, 1, 2000, False),
    ],
)
def test_partial_arrow_sales_preserve_reserve_full_stacks_and_valuables(
    kind, amount, total, expected
):
    ...
    inventory = SimpleNamespace(
        items=[item], equipped_ammo=None, count=lambda kind: total
    )
...
def test_partial_arrow_cleanup_stops_at_four_free_slots():
    ...
    def town(action, **fields):
        if action == "supplies":
            return {"items": list(rows), "capacity": 8, "silver": 100}
        assert action == "sell_partial_arrow"
        sales.append(fields["uid"])
        rows[:] = [i for i in rows if i["uid"] != fields["uid"]]
...
def test_equipment_sales_require_known_zero_plus_and_preserve_valuables(
    kind, plus, slot, expected
):
...
@pytest.mark.parametrize(
    "full,low_arrows,expected",
    [(False, False, False), (True, False, True), (False, True, True)],
)
def test_restart_restock_depends_on_supplies_not_farming_switch(
    enabled, full, low_arrows, expected
):
    loop = OvernightLoop.__new__(OvernightLoop)
    loop.route = RouteLibrary().load("turtledove")
    loop.living = lambda: {"embedded_controls": {"control": {"enabled": enabled}}}
    rows = [
        {"type_id": 1050000, "amount": 0 if low_arrows else 600},
    ...
    def town(action, **fields):
        if action == "gear":
            return {"level": 10, "equipment": {}}
        if action == "close":
            panels.append(fields["window"])
        return {"items": rows, "capacity": 2 if full else 40, "silver": 1000}

    loop.town = town
    calls = []
    loop.stop_farm = lambda: calls.append("off")
    loop.restock = lambda: calls.append("town")
    loop.record = lambda *args, **kwargs: None
...
def test_arrow_refill_reopens_shop_after_deferred_equipment_review(monkeypatch):
    ...
    def town(action, **fields):
        if action == "supplies":
            return {"items": rows, "capacity": 40, "silver": 10000}
        if action == "open":
            opened[0] = True
        if action == "close" and fields["window"] == "Shop":
            opened[0] = False
        if action == "buy":
            assert opened[0], "Refill attempted through a closed shop"
            purchases.append(fields["type_id"])
            rows.append({"type_id": 1050000, "amount": 200})
            return {"verified": True}
        return {}

    loop.town = town
    loop.travel = lambda p: None
    loop.sell_junk = lambda v: None
    loop.recycle_small_arrows = lambda: None
    loop.record = lambda *a, **kw: None
    loop.restock()
...
def test_optional_equipment_shop_failure_does_not_block_supplied_route(monkeypatch):
    ...
    monkeypatch.setattr(
        equipment,
        "EquipmentReview",
        lambda loop: SimpleNamespace(visit=lambda vendor: None),
    )
    loop = OvernightLoop.__new__(OvernightLoop)
...
def test_temporary_obstruction_of_only_town_corridor_retries_with_running_steps(
    monkeypatch,
):
...
def test_town_arrival_uses_nearby_interaction_position_without_repeated_steps():
    ...
    loop.stepper = SimpleNamespace(
        step_to=lambda *a, **k: pytest.fail("Already in shop interaction range")
    )
...
def test_empty_travel_potions_do_not_strand_character_before_town():
    ...
    care.inventory = SimpleNamespace(
        read=lambda: SimpleNamespace(count=lambda type_id: 0)
    )
    events = []
    ...
    health = {
        "embedded_controls": {
            ...
            "life": dict(dead_candidate=False, current_hp=100, max_hp=500),
        }
    }
    care.check(health)
    care.check(health)
...
def test_town_travel_tracks_actual_progress_instead_of_lifetime_failures(
...
def test_camera_edge_routes_around_hud_using_visible_walking_prefix(monkeypatch):
    ...
    def anchor(*a):
        return (518, 128) if position == [10, 10] else (518, 396)
...
@pytest.mark.parametrize(
    ...
    [(0, None, 0), (0x8000000, None, 0), (0x10, None, 1), (0x8000000, {"at": 1}, 1)],
)
def test_travel_xp_does_not_rescan_idle_state_after_first_activation(
    status, pending, checks
):
    from conquest.travel_care import TravelCare
    ...
    care = TravelCare.__new__(TravelCare)
    calls = []
    care._xp_skill = SimpleNamespace(
        pending=pending, step=lambda dispatch: calls.append(True) or False
    )
...
def test_town_handoff_cancels_old_death_return_before_new_travel(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(overnight, "RECOVERY_CHECKPOINT", checkpoint)
    ...
    loop.health = lambda: {
        "target": {"pid": 1},
        "embedded_controls": {"external_execution": False},
    }
    calls = []
    ...
    def request(info, op, body):
        calls.append(body)
        if "route_id" in body:
            write_json(checkpoint, {"phase": "cancelled"})
...
def test_route_shutdown_ignores_removed_bridge_receipt(monkeypatch):
    ...
    loop = OvernightLoop.__new__(OvernightLoop)
    loop.phase = "starting"
    loop.info = "removed-bridge.json"
    loop.check_stop = lambda: None
    loop.refresh = lambda: None
    loop.record = lambda *args, **kwargs: None
    loop.health = lambda: (_ for _ in ()).throw(
        overnight.OvernightStopped("Stopped by user")
    )
    monkeypatch.setattr(
        overnight, "request", lambda *args: (_ for _ in ()).throw(FileNotFoundError())
    )
    monkeypatch.setattr(
        overnight.ctypes,
        "windll",
        SimpleNamespace(
            kernel32=SimpleNamespace(SetThreadExecutionState=lambda *_: None)
        ),
        raising=False,
    )
    loop.run()
...
def test_non_market_typed_stall_keeps_survival_and_recovery_active(monkeypatch):
    ...
    loop.living = lambda: (
        calls.append("living")
        or {"embedded_controls": {"life": {"map_id": 1002, "position": [300, 200]}}}
    )
...
@pytest.mark.parametrize(
    "reason,map_id,pending",
    [
        ("service_deadline", 1002, False),
        ("no_progress", 1036, False),
        ("no_progress", 1002, True),
    ],
)
def test_market_deadline_or_unresolved_trade_cannot_enter_route_retry(
    monkeypatch, reason, map_id, pending
):
    from conquest.merchants import delivery_route
    from conquest.travel_progress import TravelStalled
    ...
    with pytest.raises(TravelStalled) as raised:
        loop.recover_travel_stall(error)
...
def test_service_travel_finishes_when_memory_npc_is_reachable():
    loop = OvernightLoop.__new__(OvernightLoop)
    loop.route = RouteLibrary().load("bandit")
    loop.terrain = SimpleNamespace(map_id=1036)
    loop.record = lambda *a, **k: None
...
def test_travel_healing_focus_race_retries_without_marking_unsent_potion_used(
    monkeypatch,
):
    ...
    care.pending = None
    care.last_heal = -float("inf")
    care.notify = lambda e: None
    care.inventory = NS(
        read=lambda: NS(
            count=lambda item: 3, items=[NS(uid=42, type_id=1000020, amount=1)]
        )
    )
    health = {
        "embedded_controls": {
            "control": {"enabled": False},
            "life": {"current_hp": 300, "max_hp": 834, "dead_candidate": False},
        }
    }
    ...
    care.check(health)
    assert [a[2]["action"] for a in sent] == [
        "consume-healing",
        "close",
    ] and care.pending is None
...
def test_travel_heals_at_seventy_percent_and_does_not_stop_when_damage_masks_potion(
    monkeypatch,
):
    from types import SimpleNamespace as NS
    from conquest import travel_care as t
    ...
    def inventory():
        value = count[0]
        return NS(
            count=lambda item: value, items=[NS(uid=42, type_id=1000020, amount=1)]
        )
    ...
    health = {
        "embedded_controls": {
            "control": {"enabled": False},
            "life": {"current_hp": 700, "max_hp": 1000, "dead_candidate": False},
        }
    }
    now = [100]
    ...
    route = route.model_copy(
        update={
            "supplies": route.supplies.model_copy(
                update={"arrows_return_below": 200, "healing_return_below": 6}
            )
        }
    )
    assert not needs_town(
        {"arrows": arrows, "potions": potions, "free_slots": 1}, route
    )
...
def test_pharmacist_skipped_when_potions_full_and_only_protected_loot():
    ...
    snapshot = {
        "items": [
            {
                "type_id": route.supplies.healing_type,
                "amount": route.supplies.healing_restock_to,
            },
...
@pytest.mark.parametrize(
    "failure,change,arrows,potions,allowed",
    [
...
def test_optional_arrow_panel_failure_requires_unchanged_stock(
    monkeypatch, failure, change, arrows, potions, allowed
):
    import copy
    from conquest import overnight
    ...
    bag = {
        "items": [
            {"uid": 2, "type_id": loop.route.supplies.healing_type, "amount": potions}
        ],
        "equipped_ammo": {
            "uid": 1,
            "type_id": loop.route.supplies.arrow_type,
            "amount": arrows,
        },
        "capacity": 40,
        "silver": 3000,
    ...
    if allowed:
        ...
        with pytest.raises(ValueError, match=failure):
            loop.open_arrow_refill()
        assert not events
    assert calls.count("open") == 1 and "buy" not in calls


@pytest.mark.parametrize("still_carried", [False, True])
def test_urgent_bank_deposits_before_hunting_without_supply_shopping(
    monkeypatch, still_carried
):
    from conquest import banking, return_scroll, world_travel
    from conquest.merchants import handoff

    monkeypatch.setattr(
        handoff, "service_window", lambda loop, **kw: calls.append(("refill", kw))
    )
    calls = []
    item = {"uid": 123, "type_id": 500003, "plus": 2, "slot": 0, "amount": 1}
    bag = {
        "items": [item],
        "capacity": 40,
        "silver": 200,
        "equipped_ammo": {"type_id": 1050001, "amount": 900},
    }
    ...
    if still_carried:
        ...
        assert calls[-1] == ("refill", {"town": True})


@pytest.mark.parametrize("manual_stop", [False, True])
def test_input_handoff_during_travel_reobserves_or_honors_stop(
    monkeypatch, manual_stop
):
    from conquest import scene_input, overnight
    ...
    if manual_stop:
        ...
        loop.travel((14, 10))
        assert position == [14, 10] and len(steps) == 2
    assert stops


def test_initial_blacksmith_open_failure_still_completes_storage_when_stocked(
    monkeypatch,
):
