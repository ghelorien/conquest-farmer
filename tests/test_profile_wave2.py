...
def test_profile_readiness_distinguishes_terminal_unresolved_and_unrelated(
    tmp_path, monkeypatch
):
    registry = ProfileRegistry(tmp_path)
    first = registry.add("First", role="Merchant")
...
def test_profile_readiness_covers_target_scoped_manual_recovery(tmp_path):
    ...
    with journal.db() as db:
        ...
        db.execute(
            "INSERT INTO manual_rebaseline VALUES(?,?,?)",
            ("other-baseline", other.id, "needs_attention"),
        )
        db.execute(
            "INSERT INTO manual_replans VALUES(?,?,?,?)",
        ...
        for name, value in (
            ...
            db.execute(
                "INSERT INTO state VALUES(?,?,?)", (other.id, name, json.dumps(value))
            )
    ...
    def blocked():
        with pytest.raises(ValueError, match="this profile"):
            registry.update(
                first.id,
                {"local_enabled": not registry.resolve(first.id).local_enabled},
            )

    def succeeds():
        registry.update(
            first.id, {"local_enabled": not registry.resolve(first.id).local_enabled}
        )
    ...
    with journal.db() as db:
        db.execute(
            "UPDATE manual_rebaseline SET phase='completed' WHERE id='first-baseline'"
        )
    succeeds()

    state(
        "manual_reader_hold",
        {"target_profile_id": first.id, "phase": "needs_attention"},
    )
    visitor = {
    ...
    with journal.db() as db:
        db.execute(
            "UPDATE manual_replans SET merchant_pending=0 WHERE session_id='first-replan'"
        )
    succeeds()
    ...
    blocked()
    state("accepted_request", None)
    succeeds()
    registry.update(first.id, {"role": "Farmer"})
    registry.update(first.id, {"role": "Merchant"})
    ...
    blocked()
    state("unrelated_request_decline", {"phase": "verified"})
    registry.update(
        first.id,
        {
            "trusted_sources": [
...
def test_farmer_readiness_is_profile_scoped_and_terminal_history_is_inert(tmp_path):
    ...
    other = registry.add("Other")
    own = (
        context_for(first.id, tmp_path).state_dir
        / "reports/banking/merchant-journey.json"
    )
    foreign = (
        context_for(other.id, tmp_path).state_dir
        / "reports/banking/merchant-journey.json"
    )
...
def test_role_requires_explicit_exact_visitor_revocation(tmp_path):
    registry = ProfileRegistry(tmp_path)
    merchant = registry.add("Seller", role="Merchant")
    ...
    with store.db() as db:
        db.execute(
            "INSERT INTO manual_sessions(id,target_profile_id,visitor_json,process_json,"
    ...
    with store.db() as db:
        db.execute(
            "UPDATE manual_sessions SET phase='operator_overridden',terminal_json='{}' "
            "WHERE id='open'"
        )
...
def test_migration_remaps_every_character_table_and_manual_targets(tmp_path):
    ...
    journal = legacy_journal(source)
    journal.admit_delivery(
        "admission", "Dutch", [7], {"operation_id": "admission"}, items=None
    )
    ...
    with manual.db() as db:
        ...
        db.execute(
            "INSERT INTO manual_audit(session_id,event,at,payload_json,previous_digest,digest) "
            "VALUES(?,?,?,?,?,?)",
            ("done", "kept", 1, "{}", "", "audit-digest"),
        )
        db.execute(
            "CREATE TABLE custom_character_receipts(character TEXT, receipt TEXT)"
        )
        db.execute(
            "INSERT INTO custom_character_receipts VALUES(?,?)", ("Dutch", "kept")
        )
        ...
        db.execute(
            "INSERT INTO pending_receipts VALUES(?,?)", ("Dutch", "pending-proof")
        )
    destination = tmp_path / "managed"
    result = migrate_legacy(source, destination, check_offline=lambda _: None)
    dutch = result["profiles"]["Dutch"]
    path = destination / "machine-state/reports/merchants/journal.sqlite3"
    with sqlite3.connect(path) as db:
        assert (
            db.execute("SELECT character FROM delivery_admissions").fetchone()[0]
            == dutch
        )
        assert db.execute(
            "SELECT character,receipt FROM custom_character_receipts"
        ).fetchone() == (dutch, "kept")
        assert db.execute(
            "SELECT character,receipt FROM pending_receipts"
        ).fetchone() == (dutch, "pending-proof")
        assert (
            db.execute("SELECT target_profile_id FROM visitor_permissions").fetchone()[
                0
            ]
            == dutch
        )
        assert (
            db.execute("SELECT target_profile_id FROM manual_sessions").fetchone()[0]
            == dutch
        )
        assert (
            db.execute("SELECT digest FROM manual_audit").fetchone()[0]
            == "audit-digest"
        )
        triggers = {
            row[0]
...
def test_migration_remaps_active_farmer_json_journals_but_preserves_history(tmp_path):
    source = tmp_path / "legacy"
    bank = source / "reports/banking"
    write_json(
        bank / "town-visit.json",
        {
            "version": 1,
            "town_visit_id": "current-town",
            "farmer_profile_id": "Parasite",
            "phase": "complete",
            "history": [
                {
                    "town_visit_id": "older-town",
                    "farmer_profile_id": "Parasite",
                    "phase": "complete",
    ...
    write_json(
        bank / "merchant-service-visit.json",
        {
            "version": 1,
            "visit_id": "market-visit",
            "farmer_profile_id": "Parasite",
            "phase": "active",
            "attempts": [{"farmer_profile_id": "Parasite", "outcome": "deferred"}],
        },
    )
    write_json(
        bank / "merchant-route.json",
        {
            "active": {"request_id": "live-route", "farmer_profile_id": "Parasite"},
            "operations": [
                {"request_id": "old-route", "farmer_profile_id": "Parasite"}
            ],
            "receipts": [
                {"request_id": "old-receipt", "farmer_profile_id": "Parasite"}
            ],
        },
    )
    # Similar-looking diagnostic data is not an active controller journal.
    write_json(
        bank / "migration-diagnostic.json",
        {"farmer_profile_id": "Parasite", "phase": "complete"},
    )

    destination = tmp_path / "managed"
    result = migrate_legacy(source, destination, check_offline=lambda _: None)
    farmer = result["farmer_profile_id"]
    managed = destination / "characters" / farmer / "reports/banking"
...
def test_migration_refuses_nonterminal_manual_binding_instead_of_rewriting_evidence(
    tmp_path,
):
    source = tmp_path / "legacy"
    journal = legacy_journal(source)
    ...
    with manual.db() as db:
        db.execute(
            ...
            (
                "open",
                "Dutch",
                json.dumps({"target_profile_id": "Dutch"}),
                "{}",
                "{}",
                "needs_attention",
                1,
...
def test_migration_copies_only_recognized_dpapi_secrets(tmp_path):
    source = tmp_path / "legacy"
    recognized = source / ".runtime/account.dpapi"
    recognized.parent.mkdir(parents=True)
    recognized.write_bytes(b"account")
    leak = source / ".runtime/private-backup.dpapi"
    leak.write_bytes(b"must-not-copy")
    nested = source / "reports/merchants/leaked.dpapi"
...
def test_migration_prunes_long_generated_trees_before_descent_and_keeps_durable_state(
    ...
    journal = legacy_journal(source)
    journal.set("Dutch", "silver_total", 321)
    config = source / "profiles/desktop-foreground.local.yaml"
    config.parent.mkdir(parents=True)
    config.write_text("character: CacheFarmer\n", encoding="utf-8")
    route = source / "profiles/routes/cache-regression.yaml"
    ...
    model_file.write_text('{"generated":true}', encoding="utf-8")
    report_cache = (
        source / "reports/cache" / ("generated-report-component-" * 4) / "cached.json"
    )
    report_cache.parent.mkdir(parents=True)
    report_cache.write_text("{}", encoding="utf-8")
    ...
    def guarded_walk(top, *args, **kwargs):
        for current, directories, files in real_walk(top, *args, **kwargs):
            ...
            assert not any(
                current == root or current.is_relative_to(root) for root in excluded
    ...
    result = migrate_legacy(source, destination, check_offline=lambda _: None)
    farmer = result["farmer_profile_id"]
    spiritual = result["profiles"]["Spiritual"]
    dutch = result["profiles"]["Dutch"]
    ...
    assert json.loads(
        (
            ...
            / "characters"
            / farmer
            / ".runtime/route-optimization-state.json"
        ).read_text()
    ) == {"winner": "north"}
    assert json.loads(
        (destination / "machine-state/.runtime/merchants/shop-state.json").read_text()
    ) == {"booths": 2}
    with sqlite3.connect(
        destination / "machine-state/reports/merchants/journal.sqlite3"
    ) as db:
        assert (
            json.loads(
                db.execute(
                    "SELECT value FROM state WHERE character=? AND name='silver_total'",
                    (dutch,),
                ).fetchone()[0]
            )
            == 321
    ...
    assert (
        destination / "characters" / farmer / "farmer.local.yaml"
    ).read_text() == "character: CacheFarmer\n"
    assert (
        destination / "characters" / farmer / "routes/cache-regression.yaml"
    ).exists()
    assert (
        destination / "accounts" / registry.resolve(farmer).account_id / "account.dpapi"
    ).read_bytes() == b"farmer-account"
    assert (
        destination
        / "accounts"
        / registry.resolve(spiritual).account_id
        / "account.dpapi"
    ).read_bytes() == b"spiritual-account"
    assert (
        destination / "accounts" / registry.resolve(dutch).account_id / "account.dpapi"
    ).read_bytes() == b"dutch-account"
    assert (
        destination / "characters" / farmer / ".runtime/discord-webhook.dpapi"
    ).read_bytes() == b"farmer-webhook"
    assert (
        destination / "machine-state/.runtime/merchants/shops-webhook.dpapi"
    ).read_bytes() == b"shops-webhook"
    assert not list(destination.rglob("model.json")) and not list(
        destination.rglob("cached.json")
    )
...
def test_offline_check_uses_full_identity_and_scans_all_durable_receipts(tmp_path):
    source = tmp_path / "legacy"
    write_json(
        source / ".runtime/unusual.json",
        {"pid": 41, "creation_time_100ns": 500, "path": r"C:\Python\python.exe"},
    )
    write_json(
        source / "reports/controller-two.json",
        {
            "identity": {
                "pid": 42,
                "creation_time_100ns": 600,
    ...
    class Backend:
        def __init__(self, second_creation=601):
            self.second_creation = second_creation

        def identity(self, pid):
            if pid == 41:
                return {
                    "pid": 41,
                    "creation_time_100ns": 501,
                    "path": r"C:\Python\python.exe",
                }
            return {
                "pid": 42,
                "creation_time_100ns": self.second_creation,
                "path": r"C:\Python\pythonw.exe",
            }
...
def test_migration_lock_and_legacy_app_fence_exclude_concurrency(tmp_path):
    ...
    source = tmp_path / "legacy"
    write_json(source / ".runtime/state.json", {"value": 1})
    destination = tmp_path / "managed"
    ...
    with managed_root_owner(source, timeout=0):
        with pytest.raises(ValueError, match="running"):
            migrate_legacy(source, destination, check_offline=lambda _: None)
    destination.mkdir()
    (destination / "app.lock").write_bytes(b"0")
    with _owner_file(destination / "app.lock", 0, "held"):
        with pytest.raises(ValueError, match="running"):
            migrate_legacy(source, destination, check_offline=lambda _: None)
    (destination / "app.lock").unlink()
    destination.rmdir()
...
def test_diagnostics_destination_moves_aside_and_restores_on_failure(tmp_path):
    ...
    write_json(source / ".runtime/state.json", {"value": 1})
    destination = tmp_path / "managed"
    write_json(destination / "diagnostics/failure.json", {"kept": True})
    result = migrate_legacy(source, destination, check_offline=lambda _: None)
    rollback = Path(result["diagnostics_rollback"])
    assert (rollback / "diagnostics/failure.json").exists()
    assert json.loads((rollback / "diagnostics/failure.json").read_text()) == {
        "kept": True
    }
    assert (destination / "profiles.json").exists()

    second = tmp_path / "second"
    write_json(second / "diagnostics/failure.json", {"kept": True})
    ...
    assert json.loads((second / "diagnostics/failure.json").read_text()) == {
        "kept": True
    }
...
def test_failed_migration_removes_only_its_stage_and_restores_diagnostics(
    ...
    write_json(state, {"value": 1})
    destination = tmp_path / "managed"
    write_json(destination / "diagnostics/failure.json", {"kept": True})
    existing_stage = tmp_path / "managed.migration-preexisting"
    existing_stage.mkdir()
    (existing_stage / "keep.txt").write_text("keep", encoding="utf-8")
    existing_backup = tmp_path / "managed.diagnostics-before-migration-preexisting"
    existing_backup.mkdir()
    (existing_backup / "keep.txt").write_text("keep", encoding="utf-8")
    unrelated = tmp_path / "other-managed"
    unrelated.mkdir()
    before = set(tmp_path.glob("managed.migration-*"))
    source_bytes = state.read_bytes()

    def offline(_source):
        if failure_point == "offline":
            raise OSError("offline check failed")
    ...
    assert json.loads((destination / "diagnostics/failure.json").read_text()) == {
        "kept": True
    }
...
def test_existing_managed_destination_is_rejected(tmp_path):
    ...
    source.mkdir()
    destination = tmp_path / "managed"
    write_json(destination / "profiles.json", {"managed": True})
...
def test_destination_reparse_is_rejected_before_idempotence_marker(
    tmp_path, monkeypatch
):
    source = tmp_path / "legacy"
    source.mkdir()
    destination = tmp_path / "managed"
    write_json(destination / "migration.json", {"state": "complete"})
...
def test_source_mutation_is_rejected_without_activation(tmp_path):
    source = tmp_path / "legacy"
    item = source / ".runtime/state.json"
    write_json(item, {"value": 1})
    destination = tmp_path / "managed"
    ...
    with pytest.raises(ValueError, match="changed"):
        migrate_legacy(
            source, destination, check_offline=lambda _: None, copy_file=mutate
        )
    assert not destination.exists()
...
def test_credential_source_mutation_is_rejected_before_managed_activation(tmp_path):
    ...
    source = tmp_path / "legacy"
    credential = source / ".runtime/merchants/shops-webhook.dpapi"
    credential.parent.mkdir(parents=True)
    credential.write_bytes(b"first-secret")
    destination = tmp_path / "managed"
...
def test_source_reparse_is_rejected_before_inspection(tmp_path, monkeypatch):
    ...
    actual = profile_migration._reparse
    monkeypatch.setattr(
        profile_migration, "_reparse", lambda path: Path(path) == source or actual(path)
    )
...
def test_sqlite_wal_mutation_during_backup_is_rejected(tmp_path):
    source = tmp_path / "legacy"
    database = source / "reports/merchants/journal.sqlite3"
    database.parent.mkdir(parents=True)
    live = sqlite3.connect(database)
    live.execute("PRAGMA journal_mode=WAL")
    live.execute("CREATE TABLE sample(value TEXT)")
    live.execute("INSERT INTO sample VALUES('before')")
    live.commit()
    destination = tmp_path / "managed"
    ...
    def mutate_wal(original, target):
        ...
        if Path(original) == database:
            live.execute("INSERT INTO sample VALUES('after')")
            live.commit()

    try:
        with pytest.raises(ValueError, match="changed"):
            migrate_legacy(
                source, destination, check_offline=lambda _: None, copy_file=mutate_wal
            )
    finally:
        live.close()
    assert not destination.exists()
...
def test_manifest_rejects_file_changed_after_its_individual_copy(tmp_path):
    source = tmp_path / "legacy"
    first = source / ".runtime/first.json"
    second = source / ".runtime/second.json"
    write_json(first, {"value": 1})
    write_json(second, {"value": 2})
    copied = []

    def mutate_late(original, target):
        profile_migration._copy(original, target)
        copied.append(Path(original))
        if len(copied) == 2:
    ...
    with pytest.raises(ValueError, match="before activation"):
        migrate_legacy(
            source, destination, check_offline=lambda _: None, copy_file=mutate_late
        )
    assert not destination.exists()
