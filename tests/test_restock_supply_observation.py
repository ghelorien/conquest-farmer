...
def rig(monkeypatch):
    ...
    x.expected = dict(
        ...
        capacity=40,
        equipped_ammo=dict(
            uid=2, type_id=1050002, amount=1207, limit=5000, plus=0, slot=None
        ),
    )
    x.bag = deepcopy(x.expected)
    x.bag["items"].append(deepcopy(x.arrow))
    original = deepcopy(x.expected)
    original["items"].append(
        dict(uid=3, type_id=720027, amount=1, limit=1, plus=0, slot=1)
    )
    x.claim = dict(
        phase="captured",
        captured_at=900.0,
    ...
    x.health = dict(
        target=x.target,
        profile_id="farmer",
        embedded_controls=dict(
            observed_at=1000.0,
            control=dict(enabled=False, paused=False, revision=2),
            life=dict(map_id=1011, dead_candidate=False, current_hp=100),
    ...
    monkeypatch.setattr(
        bridge,
        "request",
        lambda body: deepcopy(x.status if body["action"] == "status" else x.manual),
    )
    x.rich = dict(
        identity=x.target,
        ...
        hp=100,
        timestamp=999.0,
        canonical_manual_ownership=True,
        inventory=[
            dict(
                uid=i["uid"],
                type_id=i["type_id"],
    ...
    def read(*_):
        x.reads += 1
        x.now += 0.1
        result = deepcopy(x.rich)
        result["timestamp"] = x.now
        if getattr(x, "change_second", False) and x.reads == 2:
            result["inventory"][0]["quantity"] += 1
        return result
    ...
    x.loop = NS(
        info="isolated",
        identity=x.target,
        town_visit=NS(profile="farmer"),
        route=NS(restock_map_id=1011),
        check_stop=lambda: None,
        health=lambda: deepcopy(x.health),
        town=lambda action: deepcopy(x.bag),
    )
    x.run = lambda: observation.observe_addition(
        x.loop, x.row, x.claim, x.meteor, x.expected
    )
    return x
...
def test_positive_supply_records_unknown_origin_and_preserves_original_claim(rig):
    x = rig
    original = deepcopy(x.claim["bag"])
    result = x.run()
    proof = x.claim["unexpected_supply_addition"]
    ...
    assert (
        proof["observations"][0]["ownership"]["timestamp"]
        < proof["observations"][1]["ownership"]["timestamp"]
    )


@pytest.mark.parametrize(
    "change", ["missing", "amount", "silver", "ammo", "capacity", "limit"]
)
...
def test_only_one_known_plain_normal_arrow_is_eligible(rig, change):
    x = rig
    if change == "gem":
        x.rich["inventory"][1]["gem1"] = 1
    elif change == "bound":
        x.rich["inventory"][1]["bound"] = True
    elif change == "unknown_bound":
        del x.rich["inventory"][1]["bound"]
    else:
        item = x.meteor["return_before"]["items"][1]
        if change == "unknown_type":
            item["type_id"] = 1088000
        if change == "plus":
            item["plus"] = 1
        if change == "zero":
            item["amount"] = 0
        if change == "over_limit":
            item["amount"] = 5001
        if change == "wrong_limit":
            item["limit"] = 4999
        if change == "duplicate_addition":
            x.meteor["return_before"]["items"].append(dict(item, uid=6))
        if change == "delivered_uid":
            item["uid"] = 5
    with pytest.raises(ValueError):
        x.run()
    assert not x.saved
...
@pytest.mark.parametrize(
    "hold", ["stop", "identity", "mouse", "trade", "handoff", "owner", "unstable"]
)
def test_fresh_closed_exclusive_same_identity_proof_is_mandatory(rig, hold):
    ...
    if hold == "identity":
        x.health["target"] = dict(x.target, pid=8)
    if hold == "mouse":
        x.health["embedded_controls"]["manual_mouse"] = True
    if hold == "trade":
        x.rich["trade"] = {"participant": "visitor"}
    if hold == "handoff":
        x.manual["handoff"] = {"phase": "active"}
    if hold == "owner":
        x.status["input_owner"] = "Dutch"
    if hold == "unstable":
        x.change_second = True
    with pytest.raises(ValueError):
        x.run()
    assert not x.saved


def test_restart_revalidates_same_json_roundtripped_evidence_without_absorbing_more(
    rig,
):
    x = rig
    x.run()
    x.row = json.loads(json.dumps(x.row))
    x.claim = x.row["pre_admission_restock_tail"]
