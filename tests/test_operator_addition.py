...
def confirm(case, **kwargs):
    ...
    return record_confirmation(
        ...
        sale_receipts=kwargs.get("sale_receipts", ()),
        now=105,
    )
...
def test_manual_addition_keeps_batch_locked_until_stable_bilateral_settlement(case):
    ...
    with journal.db() as db:
        assert (
            db.execute(
                "SELECT phase FROM transactions WHERE id=?", ("test",)
            ).fetchone()[0]
            == "uncertain"
        )
...
def test_verified_sale_receipt_format_does_not_invalidate_settlement(case):
    ...
    assert (
        reconciliation_outcome(
            intent, f, m, trace=trace(journal), sale_receipts=receipts, now=105
        )["outcome"]
        == "no_transfer"
    )
...
def test_annotation_never_explains_other_changes_or_consequential_input(case, fault):
    journal, intent, f, m = case
    if fault == "offered":
        journal.step("test", "offer_item:10", "before_action")
    if fault == "confirmed":
        journal.step("test", "farmer_confirm", "before_action")
    ...
    if fault == "identity":
        m["identity"] = {"pid": 99}
    if fault == "extra_unknown":
        m["inventory"].insert(0, {**m["inventory"][0], "uid": 40})
    if fault == "active_request":
        m["request"] = {"participant": "Parasite"}
    if fault == "reserved_uid":
        m["inventory"][-1]["uid"] = 10
...
@pytest.mark.parametrize(
    "fault", ["plus", "socket", "binding", "quantity", "operation", "duplicate"]
)
...
def test_both_journals_agree_on_untouched_batch_with_operator_addition(case, tmp_path):
    ...
    reserve(
        ...
        intent["merchant"],
        intent["items"],
        origin={"operation_id": "test", "farmer_profile_id": "Parasite"},
        now=100,
    )
    confirm(case)
    for stamp in (100, 105):
    ...
    class Driver:
        def read_pair(self, character):
            return copy.deepcopy(f), copy.deepcopy(m)
    ...
    with journal.db() as db:
        result = db.execute(
            "SELECT phase,result_json FROM transactions WHERE id=?", ("test",)
        ).fetchone()
    ...
    assert (
        receiver.get("Spiritual", "delivery_reservation")["phase"]
