...
def rig(monkeypatch):
    ...
    events = []
    gui = NS(
        windows=lambda: deepcopy(state["windows"]),
...
def test_seller_panels_are_never_closed(rig, character):
    trade, state, events = rig
    trade.observer = NS(character=character)
    with pytest.raises(ValueError, match="farmer"):
        panels.close_one(trade)
    assert events == []


@pytest.mark.parametrize(
    "name", sorted(panels.TRANSACTIONS) + ["Trade###Confirm", "Unknown###Confirm"]
)
def test_transaction_blocks_cleanup(rig, name):
    trade, state, events = rig
    state["windows"] = [{"name": "Shop"}, {"name": name}]
    with pytest.raises(CaptureUnavailable, match="reconciliation"):
        panels.close_one(trade)
    assert events == []
...
def test_regular_panel_uses_existing_close_action(rig, name):
    trade, state, events = rig
    state["windows"] = [{"name": name}]
    assert panels.close_one(trade) == name
    assert events == [
        {
            "action": "service-close-panel" if name == "Dialog" else "close",
            "window": name,
        }
    ]
...
def test_foreign_booth_closes_only_verified_display_control(rig):
    trade, state, events = rig
    state["windows"] = [{"name": "Booth", "geometry": [1, 2, 30, 40]}]
    ...
    def check():
        ...
        if len(checks) > 1:
            if change == "transaction":
                state["windows"].append({"name": "###Confirm"})
            elif change == "moved":
                state["windows"][0]["geometry"][0] += 1
            else:
                raise CaptureUnavailable("Stopped")

    with pytest.raises(CaptureUnavailable):
        panels.close_one(trade, check)
    assert events == []
...
def test_travel_cleanup_is_throttled_and_reobserves_after_closing(monkeypatch, closed):
    ...
    def request(info, operation, body):
        ...
        calls.append(body)
        return {"closed_panel": closed}

    monkeypatch.setattr(module, "request", request)
    health = {
        "embedded_controls": {
            "control": {"enabled": False},
            "life": {"dead_candidate": False, "current_hp": 100, "max_hp": 100},
        }
    }
    if closed:
        with pytest.raises(module.TravelStateChanged, match="rechecking"):
            care.check(health)
    else:
        care.check(health)
    care.check(health)
    ...
    now[0] += 1
    if closed:
        with pytest.raises(module.TravelStateChanged):
            care.check(health)
    else:
        care.check(health)
    assert len(calls) == 2
...
def test_changing_gui_registry_defers_cleanup_without_stopping_farmer(rig, monkeypatch):
    ...
    def changed():
        raise panels.GuiObservationChanged("GUI registry changed")

    monkeypatch.setattr(
        panels, "GuiReader", NS(for_session=lambda adapter: NS(windows=changed))
    )
...
def test_unconfirmed_panel_close_rechecks_but_is_bounded(monkeypatch):
    ...
    with pytest.raises(TravelStateChanged, match="Rechecking"):
        care.check(health)
    care.next_panel_check = 0
