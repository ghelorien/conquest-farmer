...
def setup(monkeypatch):
    ...
    observer = SimpleNamespace(
        ...
        operations=SimpleNamespace(
            target=SimpleNamespace(
                snapshot=lambda: {"foreground": 1, "root_hwnd": 1, "minimized": False}
            )
        ),
    )
    ...
    recovery = SimpleNamespace(
        step=lambda state, focused: {
            "state": "waiting_for_revive",
            "note": "Waiting for Revive",
        }
    )
    supervisor = native_farm.NativeFarmSupervisor(
        observer, control, recovery, lambda *args: notifications.append(args)
    )
...
def test_loot_click_uses_memory_camera_anchor_in_resized_viewport(monkeypatch):
    ...
    monkeypatch.setattr(
        "conquest.scene_input.memory_player_anchor", lambda *args: (950, 600)
    )
...
@pytest.mark.parametrize(
    "kind",
    sorted(
        __import__("conquest.valuables", fromlist=["DRAGONBALL_TYPES"]).DRAGONBALL_TYPES
    ),
)
def test_every_dragonball_has_priority_over_closer_super_gear(monkeypatch, kind):
    ...
    supervisor.ground_items = lambda: (
        GroundItem(1, 1000, 130009, (10, 10)),
        GroundItem(2, 2000, kind, (11, 10)),
    )
    clicks = []
    supervisor.loot_step(
        SimpleNamespace(silver=0, items=(), capacity=40),
...
def test_focus_loss_and_death_do_not_change_farming_intent(monkeypatch):
    supervisor, control, life, _ = setup(monkeypatch)
    supervisor.observer.operations.target.snapshot = lambda: {
        "foreground": 99,
        "root_hwnd": 1,
        "minimized": False,
    }
...
def test_target_recheck_does_not_substitute_a_new_monster_at_same_point(monkeypatch):
    ...
    monster = SimpleNamespace(
        ...
        object_address=1000,
        type_id=1,
        draw_position=(500, 400),
    )
    supervisor.observer.entities = SimpleNamespace(
        layout=None, read=lambda: SimpleNamespace(monsters=[monster])
    )
    first = supervisor.match_targets([Target("Pheasant", 500, 390, 0.88)])[0]
    assert first.entity_id == 25
    monster.entity_id = 26
    with pytest.raises(CaptureUnavailable):
        supervisor.dispatch(
            lambda: pytest.fail("Replacement monster must not inherit old target"),
            target=first,
        )
...
def test_aim_refresh_tracks_same_live_monster_without_bypassing_guards(
    ...
    if change == "hud":
        monster.draw_position = (650, 730)
    if change == "dead":
        monkeypatch.setattr(monster_health, "read_monster_health", lambda *a: 0)
    points = []
    clicks = []

    def execute():
        return supervisor.dispatch(
            lambda: clicks.append(True),
            target=target,
            retarget=lambda t: points.append((t.x, t.y)),
            attack_range=10,
        )
...
def test_moving_during_memory_sample_retries_without_input_or_switching_off(
    ...
    with pytest.raises(CaptureUnavailable):
        if operation == "observe":
            ...
            supervisor.dispatch(
                lambda: pytest.fail("Incoherent life sample must not allow input")
            )
    assert control.snapshot()["enabled"]


@pytest.mark.parametrize("outcome", ["verified", "before_input", "uncertain"])
def test_healing_cleanup_does_not_mask_receipt_or_repeat_uncertain_input(
    monkeypatch, outcome
):
    ...
    def trade(body):
        ...
        if body["action"] == "close":
            raise ValueError("Life state changed during observation")
        if outcome == "before_input":
            raise TownObservationUnavailable("Life state changed during observation")
    ...
    if outcome == "verified":
        assert supervisor.heal_potion(42)["consumed"]
    elif outcome == "before_input":
        with pytest.raises(CaptureUnavailable, match="reobserving"):
...
def test_healing_inventory_open_failure_reobserves_but_uncertain_consumption_does_not(
...
def test_memory_targets_use_group_ids_and_coordinates_without_images(monkeypatch):
    ...
    monsters = [
        MonsterObservation(
            1000, 25, "Turtledove", (12, 12), (600, 400), 81, 7, type_id=2
        ),
        MonsterObservation(
            2000, 26, "Pheasant", (12, 12), (650, 400), 33, 1, type_id=1
        ),
        MonsterObservation(
            3000, 27, "Turtledove", (12, 12), (450, 580), 81, 7, type_id=2
        ),
    ]
    supervisor.observer.entities = SimpleNamespace(
    ...
    assert [(t.entity_id, t.object_address, t.x, t.y) for t in selected] == [
        (25, 1000, 600, 400)
    ]
...
def test_pickup_requires_both_inventory_increase_and_ground_disappearance(monkeypatch):
    ...
    after = SimpleNamespace(
        silver=100,
        items=(SimpleNamespace(uid=99, type_id=1088001, amount=1),),
        capacity=40,
    )
...
def test_unreachable_patrol_waypoint_tries_another_saved_point(monkeypatch):
    ...
    assert supervisor.patrol_step(
        (10, 10), (20, 20), (0, 0, 30, 30), alternatives=((20, 20), (14, 10))
    ) == (14, 10)
...
def test_alternate_patrol_path_cannot_leave_saved_boundary(monkeypatch):
    supervisor, _, _, _ = setup(monkeypatch)
    supervisor.recovery.terrain = SimpleNamespace(
        path=lambda *a, **k: [(10, 10), (31, 10), (14, 10)]
    )
    with pytest.raises(CaptureUnavailable, match="traversable"):
        supervisor.patrol_step(
            (10, 10), (20, 20), (0, 0, 30, 30), alternatives=((14, 10),)
        )
...
def test_failed_landing_replans_direction_then_returns_to_long_jumps(monkeypatch):
    ...
    supervisor.recovery.terrain = TerrainMap(
        1002, 40, 40, np.zeros((40, 40), dtype=bool), "", (), ()
    )
...
def test_chase_uses_health_checked_monsters_including_offscreen_not_corpse_records(
    monkeypatch,
):
    from conquest.memory_entities import MonsterObservation
    from conquest import monster_health
    ...
    supervisor, control, _, _ = setup(monkeypatch)
    control.update({"target_type_ids": [2]})
    dead = MonsterObservation(
        1000, 25, "Turtledove", (11, 10), (600, 400), 81, 7, type_id=2
    )
    living = MonsterObservation(
        2000, 26, "Turtledove", (14, 10), (1100, 400), 81, 7, type_id=2
    )
    unknown = MonsterObservation(
        3000, 27, "Turtledove", (12, 10), (620, 400), 81, 7, type_id=2
    )

    def hp(session, layout, monster):
        if monster is unknown:
            raise ValueError("Health observation changed")
        return 0 if monster is dead else 81

    monkeypatch.setattr(monster_health, "read_monster_health", hp)
    supervisor.observer.entities = SimpleNamespace(
        layout=None, read=lambda: SimpleNamespace(monsters=(dead, living, unknown))
    )
...
def test_long_approach_budget_and_cache_follow_fresh_position(monkeypatch):
    ...
    supervisor.recovery.terrain = SimpleNamespace(path=path)
    assert supervisor.patrol_step((10, 10), (40, 10), (0, 0, 50, 50), chase=False) == (
        22,
        10,
    )
    assert supervisor.patrol_step((22, 10), (40, 10), (0, 0, 50, 50), chase=False) == (
        34,
        10,
    )
    assert len(calls) == 1
...
def test_successful_detour_restores_jumps_without_forgetting_failed_tiles(monkeypatch):
    ...
    avoided = dict(supervisor.movement_obstructions)
    ...
    assert (
        supervisor.movement_run_until == 0
        and supervisor.movement_obstructions == avoided
    )


@pytest.mark.parametrize(
    "age,map_id,detect", [(1, 1002, True), (13, 1002, False), (1, 1011, False)]
)
def test_repeated_return_to_same_source_avoids_replaying_acknowledged_move(
    monkeypatch, age, map_id, detect
):
    import numpy as np
    from conquest.navigation import TerrainMap
    ...
    monkeypatch.setattr(native_farm.time, "monotonic", lambda: now[0])
    supervisor.recovery.terrain = TerrainMap(
        1002, 50, 50, np.zeros((50, 50), dtype=bool), "", (), ()
    )
...
def test_ranged_escape_requires_fresh_living_threats_clear_path_and_boundary(
    monkeypatch,
):
    import numpy as np
    from conquest.navigation import TerrainMap
    ...
    supervisor.scene_timestamp = 98.0
    assert supervisor.ranged_escape((20, 20), (5, 5, 40, 40)) is None


def test_chase_retains_last_seen_location_briefly_without_claiming_a_live_target(
    monkeypatch,
):
    from conquest.memory_entities import MonsterObservation
    ...
    monkeypatch.setattr(native_farm.time, "monotonic", lambda: now[0])
    supervisor.scene_timestamp = 100.0
    supervisor.chase_monsters = (
        MonsterObservation(
            1000, 25, "Turtledove", (20, 10), (1100, 400), 81, 7, type_id=2
        ),
    )
    destinations = []
...
def test_escape_avoids_landing_inside_another_living_group(monkeypatch):
    ...
    supervisor.escape_monsters = tuple(
        SimpleNamespace(position=p)
        for p in ((21, 20), (20, 21), (8, 20), (8, 21), (9, 20))
    )
    point = supervisor.ranged_escape((20, 20), (1, 1, 45, 45))
    assert point is not None
    assert (
        sum(
            max(abs(a - b) for a, b in zip(point, m.position)) <= 4
            for m in supervisor.escape_monsters
        )
        < 2
    )
    supervisor.escape_ready_at = 101.0
...
def test_escape_rechecks_source_position_before_input(monkeypatch):
    ...
    with pytest.raises(CaptureUnavailable, match="Player moved"):
        supervisor.dispatch(
            lambda: pytest.fail("Moving source must be reobserved"),
            expected_position=(9, 10),
        )
    calls = []
...
def test_looter_skips_nearby_junk_and_selects_allowed_enhanced_item(monkeypatch):
    ...
    assert supervisor.loot_step(
        SimpleNamespace(items=(), capacity=40),
        (10, 10),
...
def test_scatter_group_at_four_to_twelve_tiles_does_not_trigger_retreat(monkeypatch):
    ...
    supervisor.scene_timestamp = 100.0
    supervisor.escape_monsters = tuple(
        SimpleNamespace(position=p) for p in ((24, 20), (20, 25), (28, 20), (32, 20))
    )
    ...
    assert supervisor.ranged_escape((20, 20), (0, 0, 50, 50)) is None
    supervisor.escape_damage_consumed_at = 99.0
    now[0] = 102.0
    supervisor.scene_timestamp = 102.0
    assert supervisor.ranged_escape((20, 20), (0, 0, 50, 50)) is None
...
def test_one_adjacent_enemy_without_damage_does_not_trigger_retreat(monkeypatch):
    ...
    supervisor.escape_monsters = (
        SimpleNamespace(position=(21, 20)),
        SimpleNamespace(position=(22, 20)),
    )
    assert supervisor.ranged_escape((20, 20), (0, 0, 50, 50)) is None
...
def test_changing_target_memory_is_not_a_cleared_group(monkeypatch):
    ...
    monster = MonsterObservation(
        1000, 25, "Pheasant", (12, 12), (600, 400), 81, 7, type_id=1
    )
    supervisor.observer.entities = SimpleNamespace(
        layout=None, read=lambda: SimpleNamespace(monsters=[monster])
    )
    ...
    assert (
        ...
        and not supervisor.targets_observation_available
    )
    supervisor.observer.entities.read = lambda: SimpleNamespace(monsters=[])
    assert (
        supervisor.memory_targets() == [] and supervisor.targets_observation_available
    )
...
def test_scatter_approach_stops_before_jumping_onto_a_group(monkeypatch):
    ...
    monkeypatch.setattr(native_farm.time, "monotonic", lambda: 100.0)
    supervisor.scene_timestamp = 100.0
    supervisor.scatter_standoff = 6
    supervisor.chase_monsters = (
        MonsterObservation(
            1000, 25, "Pheasant", (22, 10), (900, 400), 81, 7, type_id=1
        ),
    )
    supervisor.recovery.terrain = SimpleNamespace(
        path=lambda source, destination, **k: [(x, 10) for x in range(10, 23)]
    )
...
def test_currency_is_ignored_during_combat_and_idle(monkeypatch):
    ...
    now = [10.0]
    monkeypatch.setattr(native_farm.time, "monotonic", lambda: now[0])
    supervisor.ground_items = lambda: tuple(
        GroundItem(i + 1, 1000 + i * 100, kind, (11 + i, 10))
...
def test_both_bandit_groups_selected_but_king_is_only_observed_for_escape(monkeypatch):
    ...
    supervisor.defending = True
    monsters = [
        MonsterObservation(1000, 1, "Bandit", (12, 12), (600, 400), 817, 32, type_id=7),
        MonsterObservation(
            2000, 2, "BanditL33", (12, 11), (650, 400), 2451, 33, type_id=66
        ),
        MonsterObservation(
            3000, 3, "BanditKing", (11, 10), (550, 400), 45000, 32, type_id=8302
        ),
    ]
    supervisor.observer.entities = SimpleNamespace(
        layout=None, read=lambda: SimpleNamespace(monsters=monsters)
...
def test_target_name_filter_accepts_only_explicit_route_variants():
    ...
    config = SimpleNamespace(
        ...
        player_anchor=(518, 396),
        max_distance_pixels=380,
        attack_range_tiles=12,
        monster="Bandit",
        monster_variants=("BanditL33",),
    )
    for name, wanted in [
        ("Bandit", True),
        ("BanditL33", True),
...
def test_system_ownership_rejection_skips_same_drop_and_continues_other_loot(
    monkeypatch,
):
    from conquest.memory_ground import GroundItem
    ...
    guard = SimpleNamespace(
        snapshot=lambda: tuple(messages),
        blocked=lambda d, m: d in denied,
        reject=lambda d, m: denied.add(d),
    )
    ...
    messages.append(SystemMessage(0x9000, 10001, OWNERSHIP_MESSAGE))
    now[0] += 0.1
    assert not supervisor.loot_step(bag, (10, 10), click)
    assert (
        drop in denied and supervisor.pending_loot is None and supervisor.pickups == 0
    )
    now[0] += 100
    assert not supervisor.loot_step(bag, (10, 10), click)
    assert clicks == [drop]
...
def test_chase_does_not_stop_at_world_range_while_aim_is_obscured(monkeypatch):
    ...
    monkeypatch.setattr(native_farm.time, "monotonic", lambda: 100.0)
    supervisor.scene_timestamp = 100.0
    supervisor.scatter_standoff = 10
    supervisor.chase_monsters = (
        MonsterObservation(
            1000, 25, "Pheasant", (16, 16), (518, 588), 81, 7, type_id=1
        ),
    )
...
def test_unselected_distant_attacker_is_available_for_escape_not_attack(monkeypatch):
    ...
    supervisor.position = (10, 10)
    supervisor.defending = True
    threat = MonsterObservation(
        1000, 25, "HeavyGhostL23", (18, 10), (774, 524), 1096, 23, type_id=64
    )
    supervisor.observer.entities = SimpleNamespace(
        layout=None, read=lambda: SimpleNamespace(monsters=[threat])
    )
...
def test_unreachable_sweep_point_advances_to_forward_alternative(monkeypatch):
    ...
    def path(source, destination, limit):
        calls.append(destination)
        if destination == (20, 20):
            raise ValueError("Unreachable within boundary")
        direction = 1 if destination[0] > source[0] else -1
        return [
            (x, 10) for x in range(source[0], destination[0] + direction, direction)
        ]
    ...
    step, index = patrol_step(
        supervisor, (14, 10), route, (index + 1) % len(route), (0, 0, 30, 30)
    )
    assert step == (18, 10) and index == 3


def test_valuable_loot_is_attempted_during_combat_without_waiting_for_clear_scene(
    monkeypatch,
):
    from conquest.memory_ground import GroundItem
...
def test_meteor_cooldown_and_disappearance_are_explained_in_audit(monkeypatch):
    ...
    bag = SimpleNamespace(silver=0, items=(), capacity=40)
    supervisor.loot_step(
        bag, (10, 10), lambda *a, **kw: pytest.fail("Cooldown must not click")
    )
    observation = next(
        fields for event, fields in notes if event == "memory_loot_observed"
    )
    assert observation["valuable_drops"][0]["reason"] == "pickup_cooldown"
    assert observation["player_position"] == (10, 10)
    supervisor.ground_items = lambda: ()
    supervisor.loot_step(
        bag, (10, 10), lambda *a, **kw: pytest.fail("Absent drop must not click")
    )
...
def test_unavailable_approach_logs_once_without_movement(monkeypatch):
    ...
    for _ in range(2):
        assert (
            supervisor.approach_loot(
                meteor, (10, 10), lambda *a, **kw: pytest.fail("No terrain")
            )
            is False
        )
    assert len(notes) == 1
...
def test_obscured_meteor_is_approached_on_checked_terrain(monkeypatch):
    ...
    meteor = GroundItem(1, 1000, 1088001, (50, 63))
    supervisor.ground_items = lambda: (meteor,)
    clicks = []
    assert supervisor.loot_step(
        SimpleNamespace(silver=0, items=(), capacity=40),
...
def test_recycled_valuable_does_not_authorize_approach(monkeypatch):
    ...
    supervisor.read_life = lambda: life
    supervisor.ground_items = lambda: ()
    assert not supervisor.approach_loot(
        GroundItem(1, 1000, 1088001, (50, 63)),
        (50, 50),
        lambda *a, **kw: pytest.fail("Stale valuable must not move the player"),
    )


def test_inventory_gain_records_plus_item_without_ground_click_or_ground_reader(
    monkeypatch,
):
...
def test_inventory_tracker_ignores_currency_and_old_items_and_does_not_repeat_returned_uid(
    monkeypatch,
):
    supervisor, _, _, events = setup(monkeypatch)
...
def test_pending_ground_pickup_and_inventory_gain_emit_one_event(monkeypatch):
    ...
    after = SimpleNamespace(
        silver=100,
        items=(SimpleNamespace(uid=99, type_id=1088001, amount=1, plus=0),),
        capacity=40,
    )
    supervisor.observe_inventory(before)
    supervisor.ground_items = lambda: (drop,)
...
def test_arrow_panel_open_failure_retries_but_uncertain_equip_does_not(
    monkeypatch, error, retryable
):
    monkeypatch.setattr(native_farm, "logical_coordinates", nullcontext)
    supervisor = native_farm.NativeFarmSupervisor.__new__(
    ...
    def trade(body):
        calls.append(body)
        if body["action"] == "equip-arrows":
            raise ValueError(error)

    supervisor.observer = SimpleNamespace(town_trade=trade)
    supervisor.dispatch = lambda callback: callback()
    inventory = SimpleNamespace(
        items=[SimpleNamespace(type_id=1050001, amount=1000, uid=123)]
    )
    with pytest.raises(CaptureUnavailable if retryable else ValueError):
        supervisor.reload_arrows(inventory, 1050001)
    assert calls == [
        {"action": "equip-arrows", "uid": 123},
        {"action": "close", "window": "Inventory"},
    ]
...
def test_fast_torn_life_retry_is_parasite_only(monkeypatch, character, expected):
    supervisor, _, life, _ = setup(monkeypatch)
    supervisor.observer.character = character
    calls = []
...
def test_coherent_projection_keeps_fresh_position_and_life_guards(monkeypatch, changed):
    ...
    if changed == "dead":
        life.dead_candidate = True
    if changed == "ghost":
        life.ghost_candidate = True
    if changed == "map":
        life.map_id = 1036
    if changed == "none":
        assert supervisor.player_projection() == ((21, 22), (518, 396))
    else:
        with pytest.raises(CaptureUnavailable):
            supervisor.player_projection()


def test_unconfirmed_meteor_retries_instead_of_patrolling_away_for_a_minute(
    monkeypatch,
):
    from conquest.memory_ground import GroundItem
    ...
    now = [100.0]
    monkeypatch.setattr(native_farm.time, "monotonic", lambda: now[0])
    drop = GroundItem(1, 1000, 1088001, (11, 10))
    s.ground_items = lambda: (drop,)
    bag = SimpleNamespace(silver=0, items=(), capacity=40)
    clicks = []
    click = lambda *a, **kw: clicks.append(kw)
    assert s.loot_step(bag, (10, 10), click)
    now[0] = 102
    assert s.loot_step(bag, (10, 10), click) and len(clicks) == 1
    now[0] = 103.1
    assert s.loot_step(bag, (10, 10), click) and len(clicks) == 1
    assert s.loot_cooldowns[(1, 1000)] == 104.1
    now[0] = 104.2
    assert s.loot_step(bag, (10, 10), click) and len(clicks) == 2
