...
def enabled(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda: {
            "enabled": True,
            "transport_reserve": 200,
            "withdraw_essentials": True,
        },
    )
    monkeypatch.setattr(b, "stash_valuables", lambda loop, **kwargs: None)


def test_pending_merchant_trade_prevents_cleanup_input_after_storage_error(
    enabled, monkeypatch
):
    ...
    def fail(*a, **kw):
        raise ValueError("uncertain trade")

    monkeypatch.setattr(b, "stash_valuables", fail)
    monkeypatch.setattr(
        b,
        "close_warehouse",
        lambda loop: pytest.fail("No panel input during uncertain trade"),
    )
    with pytest.raises(ValueError, match="uncertain trade"):
        b.after_shopping(NS())


@pytest.mark.parametrize(
    "wallet,stored,expected",
    [
        (14221, 0, ("deposit", 14021)),
        (200, 500, None),
        (50, 1000, ("withdraw", 150)),
        (50, 60, ("withdraw", 60)),
    ],
)
def test_after_shopping_banks_everything_above_fares(
    enabled, monkeypatch, wallet, stored, expected
):
    calls = []
    monkeypatch.setattr(
        b, "open_warehouse", lambda loop: {"silver": wallet, "stored_silver": stored}
    )
    monkeypatch.setattr(
        b, "transfer", lambda loop, direction, amount: calls.append((direction, amount))
    )
...
def test_transport_withdraws_only_shortfall_before_departure(enabled, monkeypatch):
    calls = []
    loop = NS(town=lambda action: {"silver": 75})
    monkeypatch.setattr(
        b, "open_warehouse", lambda loop: {"silver": 75, "stored_silver": 10000}
    )
    monkeypatch.setattr(
        b, "transfer", lambda loop, direction, amount: calls.append((direction, amount))
    )
...
def test_restock_budget_does_not_fund_arrows_at_ten_partial_packs(monkeypatch):
    ...
    bag = {
        "items": [
            {"uid": i, "type_id": 1050001, "amount": 24, "limit": 1000}
            for i in range(10)
        ]
...
def test_speed_refill_funds_both_packs_after_spent_remnant_recycling(
    monkeypatch, remnant_location
):
    from conquest.routes import RouteLibrary
    ...
    bag = {
        "items": [{"type_id": 1000020, "amount": 5, "limit": 1}],
        "equipped_ammo": None,
        "capacity": 40,
        "silver": 200,
    }
    if remnant_location == "equipped":
        bag["equipped_ammo"] = remnant
    else:
        bag["items"].append(remnant)
    original = b.read_json
    catalog = {
        "cities": {
            str(route.restock_map_id): {
                "5": {"products": [{"type_id": 1050002, "level": 73, "price": 34000}]}
            }
        }
    }
    monkeypatch.setattr(
        ...
        lambda path, *a, **kw: (
            catalog
            if str(path).endswith("archer-shop-catalog.json")
...
def test_restock_withdrawal_budget_is_essential_supplies_and_fares(
    enabled, monkeypatch
):
    from conquest.routes import RouteLibrary
    ...
    route = route.model_copy(
        update={
            "supplies": route.supplies.model_copy(
                update={"arrows_restock_to": 1600, "healing_restock_to": 10}
            )
        }
    )
    bag = {
        "silver": 200,
        "items": [{"type_id": 1000020, "amount": 2, "limit": 1}],
        "equipped_ammo": {"type_id": 1050000, "amount": 100, "limit": 200},
        "capacity": 40,
    }
    assert (
        b.shopping_budget(route, bag) == 880
    )  # One spare pack + eight potions + fares.
    calls = []
    loop = NS(route=route, town=lambda action: bag)
    monkeypatch.setattr(
        b, "open_warehouse", lambda loop: {"silver": 200, "stored_silver": 10000}
    )
    monkeypatch.setattr(
        b, "transfer", lambda loop, direction, amount: calls.append((direction, amount))
    )
...
def test_warehouse_approach_uses_live_reachability(reachable):
    ...
    loop = NS(
        living=lambda: {
            "embedded_controls": {"life": {"map_id": 1002, "position": [426, 337]}}
        },
        town=town,
    ...
    if travel:
        assert travel[0]["vendor_type"] == 0
    assert [action for action, fields in calls][-4:] == [
        "warehouse-locate",
        "close",
        "open-bank",
        "warehouse-money",
...
def test_market_bank_removes_intercepting_shop_before_alternate_target(monkeypatch):
    ...
    def town(action, **fields):
        ...
        if action == "warehouse-locate":
            return {"position": [182, 180]}
        if action == "close":
            panels.discard(fields["window"])
        if action == "open-bank":
    ...
    loop = NS(
        living=lambda: {
            "embedded_controls": {"life": {"map_id": 1036, "position": [195, 176]}}
        },
        town=town,
    ...
    assert not any(
        action in ("buy", "sell", "warehouse-deposit") for action, _ in calls
    )
...
def test_warehouse_panel_open_timeout_rechecks_without_repeating_transfer(monkeypatch):
    ...
    loop = NS(
        living=lambda: {
            "embedded_controls": {"life": {"map_id": 1002, "position": [413, 354]}}
        },
        town=town,
        record=lambda *a, **kw: None,
    )
...
def test_warehouse_uncertain_money_error_is_not_retried(monkeypatch):
    ...
    def town(action, **fields):
        ...
        if action == "vendor-status":
            return {"reachable": True}
        if action == "open-bank":
            raise ValueError("Unexpected bank balance")
        return {}
    ...
    with pytest.raises(ValueError, match="Unexpected bank balance"):
        b.open_warehouse(loop)
...
def test_stash_plus_and_meteor_by_inventory_uid_and_require_receipt():
    calls = []
    items = [
        {"uid": 1, "type_id": 530013, "plus": 2, "slot": 0},
    ...
    assert calls == [
        ("warehouse-deposit", {"uid": 1}),
        ("warehouse-deposit", {"uid": 2}),
    ]
...
def test_unverified_valuable_deposit_is_not_repeated():
    ...
    def town(action, **fields):
        if action == "supplies":
            return {"items": [{"uid": 1, "type_id": 1088001, "slot": 0}]}
        calls.append(action)
        return {}
...
def test_bandit_two_pack_refill_withdraws_full_shopping_budget(enabled, monkeypatch):
    ...
    bag = {
        "silver": 200,
        "items": [],
        "equipped_ammo": {"type_id": 1050001, "amount": 0, "limit": 1000},
        "capacity": 40,
    }
    calls = []
    loop = NS(route=route, town=lambda action: bag)
    monkeypatch.setattr(
        b, "open_warehouse", lambda loop: {"silver": 200, "stored_silver": 100000}
    )
    monkeypatch.setattr(
        b, "transfer", lambda loop, direction, amount: calls.append((direction, amount))
    )
...
def test_level_73_budget_funds_speed_upgrade_only_when_pack_room_exists(monkeypatch):
    ...
    bag = {
        "items": [
            {"uid": 1, "type_id": 1050001, "amount": 1000, "limit": 1000},
...
def test_macaque_potion_restock_does_not_require_unavailable_arrow_quote(monkeypatch):
    ...
    bag = {
        "items": [{"uid": 1, "type_id": 1050002, "amount": 5, "limit": 5000}],
...
def test_apecity_speed_arrow_quote_funds_a_real_one_pack_gap(monkeypatch):
    ...
    bag = {
        "items": [
            {"uid": 1, "type_id": 1000020, "amount": 1, "limit": 1} for _ in range(5)
        ],
        "equipped_ammo": {"uid": 2, "type_id": 1050002, "amount": 2031, "limit": 5000},
        "capacity": 40,
        "silver": 1032,
...
def test_market_open_uses_one_lower_click_then_verifies(monkeypatch):
    ...
    loop = NS(
        living=lambda: {
            "embedded_controls": {"life": {"map_id": 1036, "position": [186, 184]}}
        },
        town=town,
        record=lambda *a, **kw: None,
    )
...
def test_urgent_banking_only_carried_dragonballs_and_selected_gear(
    kind, plus, slot, wanted
):
...
@pytest.mark.parametrize(
    "changed", [None, "silver", "stored_silver", "items", "equipped_ammo", "capacity"]
)
def test_money_layout_retry_requires_unchanged_balances_and_inventory(
    ...
    def town(action, **fields):
        ...
        if len(attempts) == 1:
            raise ValueError(
                "Warehouse money control geometry differs from renderer profile"
            )
        return {
            "verified": True,
            "direction": "deposit",
    ...
    def open_bank(loop):
        ...
        value = dict(bank)
        if changed in ("silver", "stored_silver"):
            value[changed] += 1
        return value
...
def test_uncertain_money_submission_is_never_retried(monkeypatch, error):
    ...
    monkeypatch.setattr(
        b,
        "close_warehouse",
        lambda loop: pytest.fail("No recovery input after uncertain submission"),
    )
    with pytest.raises(ValueError, match=error):
        b.transfer(NS(town=town), "deposit", 98)
...
def test_two_market_open_failures_use_one_closer_checked_approach(monkeypatch):
    ...
    loop = NS(
        ...
        town=town,
        travel=travel,
        terrain=NS(walkable=lambda p: p == (186, 180)),
        record=lambda *a, **kw: None,
    )
    b.open_warehouse(loop)
