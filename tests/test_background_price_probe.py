...
def fixture(monkeypatch, failure=None):
    ...
    booth = dict(
        name="Booth",
        address=0x200000,
        geometry=[420.0, 20.0, 200.0, 200.0],
        scroll=[0.0, 0.0],
    )
    modal = dict(
        name=module.DIALOG,
        address=0x300000,
        geometry=[200.0, 200.0, 264.0, 92.0],
        scroll=[0.0, 0.0],
    )
    current = dict(
        identity={"pid": 7},
        ...
        silver=0,
        position=[10, 10],
        windows=[inventory, booth],
        trade=None,
        request=None,
    )
    before = deepcopy(current)
    controls = {
        name: dict(offset=point, size=[264.0, 92.0], window=module.DIALOG)
        for name, point in [
            ("price_field", [72.0, 53.0]),
            ("cancel_listing", [196.0, 75.0]),
            ("confirm_listing", [68.0, 75.0]),
        ]
    }
    monkeypatch.setattr(
        module, "modal_controls", lambda session, snapshot: deepcopy(controls)
    )
    ...
    neutral = dict(
        ...
        hover={"window": inventory["address"], "id": 888},
        mouse_position=[520.0, 120.0],
    )
    ...
    session = SimpleNamespace(
        identity={"pid": 7}, assert_identity=lambda: None, read_block=read
    )
    posts, stages = [], []
    ...
    gui = SimpleNamespace(
        viewport_size=lambda: [800, 600], model=lambda *args: 0x400000
    )
    driver = SimpleNamespace(
        target=target, memory=SimpleNamespace(gui=gui), read=lambda: deepcopy(current)
    )
    ...
    def sample(stage):
        ...
        if stage == "cancel_listing-release" and failure != "cancel":
            current["windows"] = [
                w for w in current["windows"] if w["name"] != module.DIALOG
            ]
            uid[0] = 0
        if stage == "price-text-verified":
            state["active"]["id"] = field
        report["samples"].append({"stage": stage, "gui": deepcopy(state)})

    return (
        (driver, before, reader, report, sample, lambda: None),
        posts,
        stages,
        current,
        price,
...
def test_exact_uid_price_text_and_cancel_without_confirm_or_enter(monkeypatch):
    ...
    assert (
        report["price_text_verified"] == 123456 and report["listing_submitted"] is False
    )
...
@pytest.mark.parametrize(
    "failure", ["uid", "nonblank", "payload", "drop-window", "text", "cancel"]
)
def test_failures_never_confirm_and_only_cancel_same_uid(monkeypatch, failure):
    ...
    with pytest.raises(ValueError):
        module.run_price_probe(*args)
    report = args[3]
    assert report["listing_submitted"] is False and not report.get(
        "price_probe_verified"
    )
...
def test_full_booth_rejected_before_input(monkeypatch):
    ...
    args[1]["booth"] = [
        dict(args[1]["inventory"][0], uid=200 + i, price=1) for i in range(32)
    ]
