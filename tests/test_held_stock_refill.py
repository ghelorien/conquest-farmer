...
def fixture(tmp_path):
    def item(uid):
        return dict(uid=uid, type_id=1088001, plus=0, gem1=0, gem2=0, quantity=1)
    ...
    incident = {
        ...
        "note": "Stock missing",
        "before": {"inventory": [item(1)], "booth": []},
    }
    j.set("Dutch", "shop_return", incident)
    r = NS(
        journal=j,
        refill_enabled=lambda c: True,
        returns={"Dutch": NS(state=lambda: j.get("Dutch", "shop_return"))},
...
def test_current_refill_preserves_original_incident_and_missing_ids(tmp_path):
    ...
    assert (
        e["incident_unresolved"] and e["missing_uids"] == [1] and e["new_uids"] == [2]
    )
...
def test_unsafe_or_unqualified_refill_is_rejected(tmp_path, change):
    r, s, _ = fixture(tmp_path)
    assert not allowed(r, "Dutch", s)
    s.update(
        timestamp=time.time(), **{k: v for k, v in change.items() if k != "timestamp"}
    )
