...
def item(uid,kind=113348):return dict(uid=uid,type_id=kind,amount=1,limit=1,slot=uid,plus=0)


@pytest.fixture
def scenario(monkeypatch):
    local=[item(i,1088001) for i in range(9)]+[item(20)]
    bag=[item(30),item(31)];market=[];events=[]
    ...
    def transfer(loop,direction,amount):state['wallet']+=amount;events.append(('withdraw_silver',amount))
    monkeypatch.setattr(banking,'transfer',transfer)
    def town(action,**fields):
        if action=='service-close-panel':return {}
        if action=='vendor-status':return {'reachable':True}
        if action=='supplies':return {'items':list(bag),'silver':state['wallet']}
        if action=='warehouse-money':return {'stored_silver':1000}
        if action=='warehouse-items':return {'items':list(market),'capacity':state['capacity']}
        if action=='warehouse-deposit':
            value=next(v for v in bag if v['uid']==fields['uid']);bag.remove(value);market.append(value)
    ...
    loop=NS(town=town,living=lambda:{'embedded_controls':{'life':{'map_id':state['map'],'position':[190,189]}}},
        record=lambda event,**kw:events.append((event,kw)),travel=lambda *a,**kw:None)
...
def test_only_carried_overflow_moves_and_transport_reserve_survives(scenario):
    ...
    assert len(local['items'])==10 and not bag and [v['uid'] for v in market]==[30,31]
...
def test_full_market_stops_even_when_last_deposit_fills_it(scenario,monkeypatch,capacity,expected):
    loop,local,bag,market,state,events=scenario;state['capacity']=capacity
    class Stopped(Exception):pass
    def stop(loop,stored,pending):
        assert len(stored['items'])==capacity
        raise Stopped()
    monkeypatch.setattr(halt,'request_stop',stop)
    with pytest.raises(Stopped):o.handle(loop,local)
...
def test_resume_after_verified_deposit_does_not_repeat_uid(scenario):
    ...
    market.append(bag.pop(0));state['map']=1036
    write_json(o.JOURNAL,{'phase':'market','origin':1011,'route':plan,'receipts':[]})
...
def test_uncertain_departure_never_pays_again(scenario):
    loop,local,bag,market,state,events=scenario
    write_json(o.JOURNAL,{'phase':'departing','origin':1011,'route':{},'departure_attempted':True})
    with pytest.raises(ValueError,match='no repeat fare'):o.resume(loop)
    assert not events
...
def test_already_full_market_does_not_attempt_any_deposit(scenario,monkeypatch):
    ...
    def stop(*args):raise RuntimeError('full storage')
    monkeypatch.setattr(halt,'request_stop',stop)
    with pytest.raises(RuntimeError,match='full storage'):o.handle(loop,local)
