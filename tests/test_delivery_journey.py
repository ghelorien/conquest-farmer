...
def trip(monkeypatch):
    ...
    def town(action,**fields):
        ...
        if action=='warehouse-items':return {'items':detailed(bank) if fields.get('rich') else copy.deepcopy(bank),'capacity':40}
...
def scroll_trip(trip,monkeypatch,*,lost=False):
    ...
    trip.bag[:]=[item(3,1050002)];trip.bank[:]=[item(99,720027)]
    original=trip.loop.town
    def town(action,**fields):
        if action=='warehouse-deposit' and read_json(journey.JOURNAL).get('loose_meteor_pending'):
            row=next(i for i in trip.bag if i['uid']==fields['uid'])
            assert row['type_id']==1088001
            trip.bag.remove(row);trip.bank.append(row);trip.events.append(('loose_deposit',row['uid']))
            return {'uid':row['uid'],'type_id':1088001,'verified_in_warehouse':True}
        if action in ('warehouse-withdraw-scroll','warehouse-reconcile-scroll'):
            ...
            if action=='warehouse-withdraw-scroll':
                ...
                trip.bank.remove(row);trip.bag.append(row)
                trip.events.append(('scroll_withdraw',row['uid']))
                if lost:raise OSError('lost scroll withdrawal acknowledgement')
            else:trip.events.append(('scroll_reconcile',fields['uid']))
            source=trip.send({'action':'delivery-source'})['farmer']
            core={k:next(i for i in source['inventory'] if i['uid']==fields['uid'])[k]
                  for k in ('uid','type_id','plus','gem1','gem2','quantity','bound')}
            return {**fields,'phase':'withdrawn','receipt':{
                'item':core,'after':{'source':source},
    ...
    def deliver(loop,*,send,items=None,on_admitted=None):
        ...
        for row in list(trip.bag):
            if row['type_id']==720027:
                if items is not None:assert [i['uid'] for i in items]==[row['uid']]
                rich=next(i for i in trip.send({'action':'delivery-source'})['farmer']['inventory'] if i['uid']==row['uid'])
                active={'request_id':'journey-test-request','items':[rich],'merchant':'Dutch','started_at':time.time()}
                if on_admitted is not None:on_admitted(active)
                trip.bag.remove(row);trip.merchant.append(row)
                trip.events.append(('scroll_delivery',row['uid']))
                receipts.append({**active,'outcome':'transferred','proof_digest':'bilateral-proof','verified_at':time.time()})
        if receipts:write_json(delivery_route.STATE,{'receipts':receipts})
        return receipts
    monkeypatch.setattr(delivery_route,'market_storage',deliver)
...
def test_already_in_market_entry_needs_neither_carried_loot_nor_money_action(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
...
def test_loose_meteor_leftovers_are_banked_before_exact_scroll_retrieval(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    trip.bag.append(item(77,1088001))
    assert journey.start_market(trip.loop,99,send=trip.send)
    assert trip.events.index(('loose_deposit',77))<trip.events.index(('scroll_withdraw',99))
    assert [row['uid'] for row in trip.merchant]==[99]
    assert [row['uid'] for row in trip.bank]==[77]


def test_market_entry_cannot_start_any_delivery_before_banking_loose_leftovers(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    trip.bag.append(item(77,1088001))
...
def test_lost_scroll_ack_reconciles_before_movement_and_never_withdraws_twice(trip,monkeypatch):
    scroll_trip(trip,monkeypatch,lost=True);trip.world['map']=1036
...
def test_missing_requested_scroll_does_not_substitute_another_uid_or_loose_meteor(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
...
def test_unsettled_loose_meteor_deposit_reconciles_before_any_movement(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    write_json(journey.JOURNAL,{'phase':'market','origin':1036,'market_only':True,'route':{},
        'stored_scroll_uid':99,'receipts':[],'loose_meteor_pending':item(77,1088001)})
    with pytest.raises(ValueError,match='deposit is uncertain'):
        journey.resume(trip.loop,send=trip.send)
    assert trip.events==[]


def test_delivery_after_scroll_retrieval_reconciles_before_warehouse_fallback(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    write_json(journey.JOURNAL,{'phase':'market','origin':1036,'market_only':True,'route':{},
        'stored_scroll_uid':99,'receipts':[],'scroll_preparation_done':True,
        'scroll_withdrawal':{'uid':99,'operation_id':'withdrawn'}})
    def uncertain(*args,**kwargs):raise ValueError('trade uncertain')
...
def test_only_proven_no_input_admission_gets_a_new_operation_id(trip,monkeypatch,prior):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    old={'uid':99,'operation_id':'old-operation'}
    write_json(journey.JOURNAL,{'phase':'market','origin':1036,'market_only':True,'route':{},
        'stored_scroll_uid':99,'receipts':[],'scroll_withdrawal':old})
    original=trip.loop.town
    def town(action,**fields):
        if action=='warehouse-reconcile-scroll' and fields==old:
            trip.events.append(('read_only_no_input',prior))
            return {**old,'phase':'no_transfer','no_input_proven':True,
                    'receipt':{'input_attempted':False,'admission_closed':prior=='missing_admission'}}
...
@pytest.mark.parametrize('phase',['input_maybe_sent','reconciling','blocked','no_transfer'])
...
def test_carried_eligible_item_is_delivered_after_loose_bank_when_no_scroll_exists(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    trip.bag[:]=[item(1),item(77,1088001),item(3,1050002)];trip.bank[:]=[]
    write_json(journey.JOURNAL,{'phase':'market','origin':1036,'market_only':True,'route':{},'receipts':[]})
    def deliver(loop,*,send):
        assert ('loose_deposit',77) in trip.events
        gear=next(row for row in trip.bag if row['uid']==1)
        trip.bag.remove(gear);trip.merchant.append(gear);trip.events.append(('gear_delivery',1))
    monkeypatch.setattr(delivery_route,'market_storage',deliver)
    assert journey.resume(trip.loop,send=trip.send)
...
def test_requested_market_scroll_stays_pending_if_readiness_disappears(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    reads=[]
    def send(body):
...
def test_current_scroll_ownership_is_revalidated_before_closing_or_trading(trip,monkeypatch,changed):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    def send(body):
        value=trip.send(body)
        if body['action']=='delivery-source' and ('scroll_withdraw',99) in trip.events:
            if changed=='identity':value['farmer']['identity']={'pid':99}
            else:value['farmer']['inventory']=[i for i in value['farmer']['inventory'] if i['uid']!=99]
        return value
...
def test_crash_after_preparation_save_closes_warehouse_before_delivery_on_resume(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    def crash(loop):raise OSError('crashed before warehouse close')
    ...
    monkeypatch.setattr(banking,'close_warehouse',lambda loop:trip.events.append(('recovered_close',1036)))
    assert journey.resume(trip.loop,send=trip.send)
    assert trip.events.index(('recovered_close',1036))<trip.events.index(('scroll_delivery',99))
    assert trip.events.count(('scroll_withdraw',99))==1


def test_trade_reconciliation_precedes_close_after_preparation_crash(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    def crash(loop):raise OSError('before close')
    monkeypatch.setattr(banking,'close_warehouse',crash)
    with pytest.raises(OSError):journey.start_market(trip.loop,99,send=trip.send)
    write_json(delivery_route.STATE,{'active':{'request_id':'uncertain'}})
    def settle(*args):trip.events.append(('read_only_trade',None));raise ValueError('trade uncertain')
    monkeypatch.setattr(delivery_route,'settle',settle)
    monkeypatch.setattr(banking,'close_warehouse',lambda loop:pytest.fail('closed before trade settlement'))
    with pytest.raises(ValueError,match='trade uncertain'):journey.resume(trip.loop,send=trip.send)
    assert trip.events[-1][0]=='read_only_trade'


def test_undelivered_exact_scroll_has_explicit_verified_rebanked_disposition(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
...
def test_interrupted_exact_scroll_rebank_reobserves_rich_warehouse_before_completion(trip,monkeypatch,boundary):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    monkeypatch.setattr(delivery_route,'market_storage',lambda *a,**kw:[])
    original_save=journey.save
    def crash(state,**fields):
        should_crash=((boundary=='deposit_cleared' and fields.get('deposit_pending','absent') is None
                       and (state.get('deposit_pending') or {}).get('uid')==99)
                      or boundary=='disposition_saved' and fields.get('scroll_disposition',{}).get('outcome')=='deferred_rebanked')
        original_save(state,**fields)
        if should_crash:raise OSError('crashed after verified fallback checkpoint')
    monkeypatch.setattr(journey,'save',crash)
    with pytest.raises(OSError,match='fallback checkpoint'):
        journey.start_market(trip.loop,99,send=trip.send)
    saved=read_json(journey.JOURNAL)
    assert saved['deposit_pending'] is None and not any(i['uid']==99 for i in trip.bag)
    assert bool(saved.get('scroll_disposition'))==(boundary=='disposition_saved')
    monkeypatch.setattr(journey,'save',original_save)
    monkeypatch.setattr(delivery_route,'market_storage',lambda *a,**kw:pytest.fail('rebank recovery started merchant work'))
    original_town=trip.loop.town;reads=[]
    def town(action,**fields):
        assert action not in ('warehouse-withdraw-scroll','warehouse-reconcile-scroll')
        if action=='warehouse-items' and fields.get('rich'):reads.append(True)
        return original_town(action,**fields)
    trip.loop.town=town
    assert journey.resume(trip.loop,send=trip.send)
    assert reads and read_json(journey.JOURNAL)['phase']=='completed'
    assert read_json(journey.JOURNAL)['scroll_disposition']['outcome']=='deferred_rebanked'
    assert trip.events.count(('scroll_withdraw',99))==1


def test_stale_historical_bilateral_receipt_cannot_complete_current_withdrawal(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    def crash(loop):raise OSError('before close')
    monkeypatch.setattr(banking,'close_warehouse',crash)
    with pytest.raises(OSError):journey.start_market(trip.loop,99,send=trip.send)
    state=read_json(journey.JOURNAL);selected=state['scroll_withdrawal_receipt']['item']
    write_json(delivery_route.STATE,{'receipts':[{'request_id':'historical-other-journey',
        'items':[selected],'outcome':'transferred','proof_digest':'old-valid-proof','merchant':'Dutch',
        'started_at':state['started_at']-100,'verified_at':time.time()}]})
    trip.bag[:]=[row for row in trip.bag if row['uid']!=99]
    monkeypatch.setattr(banking,'close_warehouse',lambda loop:pytest.fail('closed with unresolved ownership'))
    with pytest.raises(ValueError,match='no exact carried'):
        journey.resume(trip.loop,send=trip.send)
    assert journey.pending() and not read_json(journey.JOURNAL).get('scroll_disposition')


def test_current_journey_admission_recovers_bilateral_receipt_saved_before_return(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    ...
    with pytest.raises(OSError,match='bilateral receipt'):
        journey.start_market(trip.loop,99,send=trip.send)
    state=read_json(journey.JOURNAL)
    assert state['scroll_delivery_requests'] and not state.get('scroll_delivery_receipts')
    monkeypatch.setattr(delivery_route,'market_storage',lambda *a,**kw:pytest.fail('already transferred'))
    assert journey.resume(trip.loop,send=trip.send)
    state=read_json(journey.JOURNAL)
    assert state['scroll_disposition']['outcome']=='delivered'
    assert state['scroll_disposition']['receipt']['request_id']=='journey-test-request'
    assert trip.events.count(('scroll_delivery',99))==1


def test_saved_rebanked_disposition_cannot_replace_fresh_missing_warehouse_evidence(trip,monkeypatch):
    scroll_trip(trip,monkeypatch);trip.world['map']=1036
    monkeypatch.setattr(delivery_route,'market_storage',lambda *a,**kw:[])
    assert journey.start_market(trip.loop,99,send=trip.send)
    state=read_json(journey.JOURNAL)
    assert state['scroll_disposition']['outcome']=='deferred_rebanked'
    state['phase']='market';write_json(journey.JOURNAL,state)
    trip.bank.clear()
    monkeypatch.setattr(delivery_route,'market_storage',lambda *a,**kw:pytest.fail('merchant work during bank recheck'))
    with pytest.raises(ValueError,match='no exact carried'):
        journey.resume(trip.loop,send=trip.send)
