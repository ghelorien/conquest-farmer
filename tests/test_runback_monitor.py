...
def test_manual_motion_and_focus_gaps_do_not_inflate_bot_speed_or_stalls(tmp_path):
    m, s = make(tmp_path)
    s(0)
    s(0.5, x=20, paused=True)
    s(10, x=25, paused=True)
    s(10.5, x=26)
    assert m.distance == 0 and m.stalls == 0 and not m.urgent
    s(11, x=28)
    assert m.distance == 2 and m.active_seconds == 0.5
...
def test_death_is_counted_once_and_finish_preserves_the_failure(tmp_path):
    ...
    m.finish("arrived")
    rows = [
        json.loads(x) for x in (tmp_path / "history.jsonl").read_text().splitlines()
    ]
    assert (
        len(rows) == 1 and rows[0]["deaths"] == 1 and rows[0]["minimum_hp_percent"] == 0
    )
