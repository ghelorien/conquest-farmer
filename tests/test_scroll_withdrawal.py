...
def work(tmp_path, monkeypatch, observations, *, click="after"):
    town = Town()
    journal = ProtectedWithdrawalJournal(
        tmp_path / "withdraw.sqlite3", clock=lambda: 200
    )
    task = ScrollWithdrawal(
        town, journal=journal, clock=lambda: 200, sleep=lambda _: None
    )
    profile = NS(
        id="b3a6b949-d20e-49c6-9337-e82b1876159d", role="Farmer", character_uid=1173490
    )
    ...
    def foreground(*args, **kwargs):
        calls.append("click")
        if click == "before":
            raise ValueError("manual input hold")
        kwargs["before_press"]()
        if click == "after":
...
def test_banked_scroll_has_exact_receipt_and_lost_ack_never_replays(
    tmp_path, monkeypatch
):
    before = sample()
    after = sample(bag=(scroll(),), stash=())
...
def test_normal_click_reconciles_before_terminal_receipt(tmp_path, monkeypatch):
    task, journal, calls, _ = work(
        tmp_path,
        monkeypatch,
        [sample(), sample(), sample(bag=(scroll(),), stash=())],
        click="success",
    )
...
def test_snapshot_clock_advancing_does_not_break_durable_operation_binding(
    tmp_path, monkeypatch
):
    first = sample()
    first["source"]["timestamp"] = 199
    task, journal, calls, _ = work(
        tmp_path, monkeypatch, [first, sample(), sample(bag=(scroll(),), stash=())]
    )
    assert run(task)["phase"] == "withdrawn"
    assert run(task)["phase"] == "withdrawn"
    assert calls == ["click"]
...
def test_possible_click_unchanged_ownership_blocks_every_new_operation(
    tmp_path, monkeypatch
):
...
def test_other_stored_types_cannot_use_scroll_withdrawal(tmp_path, monkeypatch, kind):
    task, journal, calls, _ = work(
        tmp_path, monkeypatch, [sample(stash=(item(99, kind, plus=0),))]
    )
...
def test_unqualified_scroll_attributes_are_rejected_before_admission(
...
def test_restart_profile_mismatch_cannot_reconcile_another_profile(
    tmp_path, monkeypatch
):
    task, journal, calls, profile = work(tmp_path, monkeypatch, [sample()])
    assert run(task)["phase"] == "blocked"
    profile.id = "another-profile"
    with pytest.raises(ValueError, match="another farmer profile"):
        run(task)
    assert calls == ["click"]
...
def test_dispatch_is_narrow_and_reconcile_is_a_separate_read_only_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(
        scroll_withdrawal,
        "withdraw",
        lambda *args: calls.append(("withdraw", args)) or {},
    )
    monkeypatch.setattr(
        scroll_withdrawal,
        "reconcile",
        lambda *args: calls.append(("reconcile", args)) or {},
    )
    for verb in ("withdraw", "reconcile"):
        assert (
            town(
                {
                    "action": "warehouse-" + verb + "-scroll",
                    "operation_id": "op",
                    "uid": 99,
                }
            )
    ...
    with pytest.raises((ValueError, AttributeError)):
        town(
            {
                "action": "warehouse-withdraw-scroll",
                "operation_id": "op",
                "uid": 99,
                "type_id": 1088001,
            }
...
def test_read_only_reconcile_closes_missing_admission_and_blocks_late_submission(
    tmp_path, monkeypatch
):
    from conquest import scroll_withdrawal
...
def test_missing_admission_cannot_close_when_exact_item_is_not_in_warehouse(
    tmp_path, monkeypatch
):
    from conquest import scroll_withdrawal

    task, journal, calls, _ = work(
        tmp_path, monkeypatch, [sample(bag=(scroll(),), stash=())]
    )
    monkeypatch.setattr(scroll_withdrawal, "ScrollWithdrawal", lambda *a, **kw: task)
