...
def setup(tmp_path,monkeypatch):
    ...
    runtime=MerchantRuntime(object(),guard,journal=Journal(tmp_path/'state.sqlite3'))
    guard.owner_allowed=runtime.input_allowed
    path=tmp_path/'account.dpapi';path.write_bytes(b'encrypted-placeholder')
    monkeypatch.setattr(connect,'credential_path',lambda c:path)
    ui=NS(runtime=runtime,coordinator=guard,closed=False,connect_threads={},safe_to_yield=lambda:True,
          app=NS(control=NS(snapshot=lambda:{'revision':0})))
    return NS(runtime=runtime,guard=guard,ui=ui,path=path)


@pytest.mark.parametrize('reason',['missing_login','busy','stopped','unsafe','uncertain_fare'])
def test_start_refuses_missing_prerequisites_without_launch(setup,reason):
    x=setup
    if reason=='missing_login':x.path.unlink()
    if reason=='busy':x.runtime.connecting['Dutch']=7
    if reason=='stopped':x.guard.stop()
    if reason=='unsafe':x.ui.safe_to_yield=lambda:False
    if reason=='uncertain_fare':x.runtime.journal.set('Spiritual','connect_market',{'phase':'transfer_pending'})
    with pytest.raises(ValueError):connect.start(x.ui,'Spiritual')
    assert not x.ui.connect_threads
    assert not x.runtime.enabled('Spiritual')


@pytest.mark.parametrize('purpose',['connect','connect_launch','listing','trade','refill',None])
def test_connection_has_scoped_permission_without_enabling_trades(setup,purpose):
    x=setup;x.runtime.connecting['Dutch']=threading.get_ident();x.runtime.connect_checks['Dutch']=lambda:True
    if purpose in ('connect','connect_launch'):
        with x.guard.lease('Dutch',purpose=purpose):x.guard.check()
    else:
        with pytest.raises(CaptureUnavailable):
            with x.guard.lease('Dutch',purpose=purpose):pytest.fail('Unrelated input')
    assert not x.runtime.enabled('Dutch')


def test_manual_stop_cancels_connection(setup):
    x=setup;cancel=threading.Event();x.runtime.connect_cancel['Dutch']=cancel
...
def test_existing_client_selection_never_steals_assigned_account(setup,owner):
    ...
    windows=[] if owner=='missing' else [candidate,candidate] if owner=='ambiguous' else [candidate]
    x.runtime.catalog=NS(windows=lambda:windows)
    observer=NS(adapter=NS(identity=candidate.identity))
    if owner=='farmer':x.ui.app.observer=observer
    if owner=='other_merchant':x.runtime.observers['Dutch']=observer
    if owner=='unassigned':assert connect.select_client(x.ui,'Spiritual',20) is candidate
    else:
        with pytest.raises(ValueError):connect.select_client(x.ui,'Spiritual',20)


@pytest.mark.parametrize('progress',[False,True])
def test_movement_capability_is_written_only_after_verified_trial(tmp_path,monkeypatch,progress):
    dimensions={'client_size':[1000,800],'gui_size':[1000,800]}
    monkeypatch.setattr(connect,'geometry',lambda d:dimensions)
    driver=NS(qualification=tmp_path/'qualification.json',observer=NS(character='Dutch',adapter=NS(expected_sha256='pinned')))
    ...
    if progress:
        assert connect.qualify_move(driver,travel,before,(438,444),lambda:None)==after
        assert read_json(driver.qualification)['capabilities']=={'market_return':True}
    else:
        with pytest.raises(ValueError,match='progress'):connect.qualify_move(driver,travel,before,(438,444),lambda:None)
...
def test_arriving_in_market_keeps_ordinary_work_held(setup,monkeypatch):
    ...
    x.runtime.return_drivers['Dutch']=NS(read=lambda:{'map_id':1036,'position':[100,100]})
    monkeypatch.setattr('conquest.reconnect.login_screen',lambda hwnd:False)
    monkeypatch.setattr('conquest.memory_life.read_life',lambda *a:NS(character='Dutch',map_id=1036))
...
def test_login_focus_failure_precedes_loading_credentials(monkeypatch):
    ...
    monkeypatch.setattr('conquest.focus_recovery.activate_client',lambda hwnd,identity:calls.append((hwnd,identity)) or False)
    monkeypatch.setattr(reconnect,'load_credentials',lambda *a:pytest.fail('No credential load before focus'))
