...
def test_selected_profile_keeps_delivery_preferences_in_its_state_directory(tmp_path,monkeypatch):
    ...
    first=registry.add('Kilhiam');second=registry.add('FreshArcher')
    ...
    assert (context_for(first.id,tmp_path).state_dir/'.runtime/farmer-transfer-preferences.json').exists()
