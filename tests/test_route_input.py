...
@pytest.mark.parametrize(
    "mode", ["blocked", "partial", "moving", "arrived", "attacked"]
)
def test_route_progress_watchdog_keeps_care_active_and_does_not_interrupt_movement(
    monkeypatch, mode
):
    now = [10.0]
    care = []
    ...
    def request(info, operation, body=None):
        if operation == "route-jump":
            inputs.append(body)
            return {"issued": True, "movement": "run"}
        elapsed = now[0] - 10
        x = 10
        if inputs:
            if mode == "partial" and elapsed >= 0.3:
                x = 11
            if mode == "moving":
                x = min(14, 10 + int(elapsed / 0.7))
            if mode == "arrived":
                x = 14
        return {
    ...
    stepper = route_input.BridgeJumpStepper(
        "unused", on_life=lambda h: care.append(now[0])
    )
    result = stepper.step_to((14, 10), expected_position=(10, 10))
    assert len(inputs) == 1
    assert max(b - a for a, b in zip(care, care[1:])) < 0.06
    if mode == "attacked":
        assert not result["reached"] and 0.5 <= now[0] - 10 < 0.7
    elif mode in ("blocked", "partial"):
        assert (
            not result["reached"]
            and result["error"] == "Route movement stopped progressing"
        )
...
@pytest.mark.parametrize(
    "note",
    [
        "Closed a shop panel; rechecking the route",
        "Mouse control is yours; waiting for idle",
    ],
)
def test_panel_after_movement_triggers_replanning_but_manual_pause_still_propagates(
    monkeypatch, note
):
    ...
    if note.startswith("Closed"):
        ...
        with pytest.raises(TravelStateChanged):
            stepper.step_to((20, 10))
