...
def receipt(tmp_path, monkeypatch):
    monkeypatch.setattr(
        listing,
        "_profile",
        lambda character: SimpleNamespace(
    ...
    request = dict(
        ...
        character="Dutch",
        request_id="booth-list1078-synthetic01",
        item_uid=91,
    ...
    stages = [
        ...
        (
            "native_dialog",
            "verified",
            {"uid": 91, "renderer_sha256": _MODAL_RENDER_SHA256},
        ),
        ...
        (
            "confirm_press",
            "before_mouse_down",
            {"uid": 91, "price": 73000, "owned_booth_uid": 18},
        ),
    ]
...
def test_verified_exact_receipt_promotes_only_same_process_open_booth_path(receipt):
    ...
    changed = {
        **x.second,
        "identity": {**x.second["identity"], "creation_time_100ns": 3},
    }
...
def test_receipt_tampering_is_rejected(receipt):
    ...
    with x.j.db() as db:
        result = json.loads(
            db.execute("SELECT result_json FROM transactions").fetchone()[0]
        )
...
def make_ui(journal, state):
    ...
    runtime = SimpleNamespace(
        journal=journal,
        listing1078_lock=threading.Lock(),
        lock=threading.RLock(),
        handoff=None,
        refills={"Dutch": schedule},
        observers={
            "Dutch": SimpleNamespace(
                hwnd=55, adapter=SimpleNamespace(identity=state["identity"])
            )
        },
    )
    runtime.refill_enabled = lambda character: journal.get(
        character, "refill_enabled", True
    )
    ...
    ui = SimpleNamespace(
        ...
        app=SimpleNamespace(
            control=SimpleNamespace(
                snapshot=lambda: {"enabled": False, "paused": False}
            )
        ),
    )
    return ui
...
def test_refill_pause_independent_of_operations(receipt, monkeypatch):
    ...
    assert (
        refill_1078.step(ui, "Dutch", x.second)["blocker"]
        == "refill_paused_or_global_stop"
    )
...
def test_prepared_receipt_after_restart_never_replays(receipt, monkeypatch):
    ...
    monkeypatch.setattr(
        listing, "dispatch", lambda *a, **k: pytest.fail("Cannot replay")
    )
    for _ in range(2):
        assert (
            refill_1078.step(ui, "Dutch", x.before)["blocker"]
...
def test_legacy_cursor_is_not_silently_discarded(receipt):
    ...
    assert (
        refill_1078.step(ui, "Dutch", x.before)["blocker"]
        == "legacy_refill_cursor_needs_reconciliation"
    )
    assert ui.runtime.refills["Dutch"].state()["cursor"] == [91]


def test_due_scheduler_uses_shared_highest_value_plan_with_operations_paused(
    receipt, monkeypatch
):
    x = receipt
    capability.settle(x.j, x.key, x.first, x.second)
    ...
    monkeypatch.setattr(
        listing_plan_1078,
        "plan",
        lambda *a: [{"uid": 93, "price": 90000}, {"uid": 92, "price": None}],
    )
    requests = []
    monkeypatch.setattr(
        listing,
        "dispatch",
        lambda ui, request, **kw: (
...
def test_unreadable_owned_peer_holds_refill_without_pricing_or_advancing_timer(
    receipt, monkeypatch
):
    ...
    monkeypatch.setattr(
        ...
        lambda *_: (_ for _ in ()).throw(
            listing_plan_1078.OwnedPeerUnavailable(
    ...
    monkeypatch.setattr(
        listing, "dispatch", lambda *a, **k: pytest.fail("No listing input allowed")
    )
    result = refill_1078.step(ui, "Dutch", current)
...
def test_peer_reader_failure_is_attributed_to_the_peer(monkeypatch):
    ...
    monkeypatch.setattr(
        listing_plan_1078,
        "observe",
        lambda *_: (_ for _ in ()).throw(ValueError("Invalid 1078 health candidate")),
    )
    with pytest.raises(
        listing_plan_1078.OwnedPeerUnavailable,
        match="Spiritual owned booth observation unavailable",
    ) as caught:
        listing_plan_1078._peer(
            None, SimpleNamespace(id="spiritual-profile", name="Spiritual")
        )
    assert caught.value.character == "Spiritual"


def test_only_ungranted_unsent_unavailable_peer_handoff_is_released(
    ...
    monkeypatch.setattr(
        "conquest.merchants.background_probe.probe_busy", lambda _: False
    )
    journal = Journal(tmp_path / "journal.sqlite3")
    runtime = SimpleNamespace(
        lock=threading.RLock(),
        journal=journal,
        handoff="merchant-refill:Spiritual:123456789",
    ...
    coordinator = SimpleNamespace(
        owner=None, stopped=False, manual_active=lambda: False
    )
...
def fresh_binding(x, booth_uid=38):
    ...
    request = {
        **deepcopy(x.request),
        "request_id": "booth-list1078-new-stall",
        "expected_own_booth_uid": booth_uid,
    }
    return current, request


def test_same_actor_new_owned_booth_retains_control_proof_and_records_current_binding(
    receipt,
):
    x = receipt
    capability.settle(x.j, x.key, x.first, x.second)
    with x.j.db() as db:
        original = tuple(
            db.execute(
                "SELECT before_json,result_json FROM transactions WHERE id=?", (x.key,)
            ).fetchone()
        )
    ...
    assert (
        binding["source_request_id"] == x.key
        and binding["source_proof_digest"] == saved["proof_digest"]
    )
    assert (
        binding["qualified_own_booth_uid"] == 18
        and binding["current_own_booth_uid"] == 38
    )
    ...
    with x.j.db() as db:
        assert (
            tuple(
                db.execute(
                    "SELECT before_json,result_json FROM transactions WHERE id=?",
                    (x.key,),
                ).fetchone()
            )
            == original
...
@pytest.mark.parametrize(
    "change",
    ["foreign_model", "unverified_model", "missing_grid", "stale", "closed_panel"],
)
def test_new_booth_requires_current_owned_model_and_live_grids(receipt, change):
    ...
    if change == "foreign_model":
        current["listing_preflight"]["owned_booth"]["owner_uid"] = 99
    elif change == "unverified_model":
        current["listing_preflight"]["owned_booth"]["model_owner_verified"] = False
    elif change == "missing_grid":
        current["listing_preflight"]["booth_grid"] = None
    elif change == "stale":
        current["timestamp"] -= 2
    else:
        current["booth_open"] = False
    with pytest.raises(ValueError):
        capability.bind_current(x.j, "Dutch", current, request)
...
def test_pending_old_request_cannot_be_rebound_or_used_on_new_booth(receipt):
    ...
    x.j.begin(
        old["request_id"], "Dutch", listing.KIND, {"request": old, "snapshot": x.before}
    )
    ...
    assert (
        json.loads(listing._row(x.j, old["request_id"])["before_json"])["request"][
