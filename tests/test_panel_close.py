...
def test_registry_race_before_close_is_retryable_without_press(monkeypatch,race_at):
    ...
    def windows():
        calls[0]+=1
        if calls[0]==race_at:raise GuiObservationChanged('GUI registry changed')
        return [{'name':'Warehouse','address':1,'geometry':[50,50,200,300]}]
    monkeypatch.setattr(p,'GuiReader',lambda a:NS(windows=windows,assert_hovered=lambda *args:None))
...
def test_post_press_failure_is_not_reclassified(monkeypatch):
    from conquest.merchants.memory import GuiObservationChanged
    monkeypatch.setattr(p,'GuiReader',lambda a:NS(windows=lambda:[
        {'name':'Warehouse','address':1,'geometry':[50,50,200,300]}],assert_hovered=lambda *args:None))
    monkeypatch.setattr(p,'wait_hover_validation',lambda guard,check:guard())
    pressed=[]
    def click(point,*,before_press):
        before_press();pressed.append(point)
