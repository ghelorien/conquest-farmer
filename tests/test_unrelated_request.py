...
def rig(tmp_path, before, after):
    ...
    reads = []
    gui = NS(
        viewport_size=lambda: (1000, 800), assert_hovered=lambda window, label: None
    )
...
def patches(monkeypatch, click):
    ...
    monkeypatch.setattr(
        "conquest.merchants.unrelated_request._cancel_control",
    ...
    monkeypatch.setattr(
        "conquest.focus_recovery.activate_client", lambda hwnd, identity: True
    )
    monkeypatch.setattr(
...
def test_lost_ack_modal_absent_restart_reconciles_without_second_click(
    ...
    assert (
        Journal(controller.journal.path).get("Dutch", "unrelated_request_decline")[
            "phase"
        ]
        == "submitted"
    )
    ...
    assert (
        restarted.journal.get("Dutch", "unrelated_request_decline")["phase"]
        == "verified"
    )
...
def test_pending_decline_rejects_changed_identity_or_booth(tmp_path, monkeypatch):
    ...
    restarted, _ = rig(tmp_path, changed, changed)
    restarted.journal = Journal(controller.journal.path)
...
def test_decline_requires_living_market_snapshot(tmp_path, monkeypatch, change):
    ...
    controller, _ = rig(tmp_path, before, before)
    patches(monkeypatch, lambda *a, **k: pytest.fail("No decline input is allowed"))
    assert (
        decline_unrelated_request(controller, before, operations_enabled=True) is False
    )
...
def test_reservation_created_during_hover_blocks_decline_press(tmp_path, monkeypatch):
    ...
    before = state()
    controller, _ = rig(tmp_path, before, state(False))
    checks = [0]
