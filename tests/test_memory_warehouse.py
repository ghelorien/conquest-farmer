...
def test_rich_warehouse_fences_first_row_gems_while_later_row_is_decoded(monkeypatch):
    from conquest import memory_warehouse as module
    base=0x140000000;shared=0x500000;owner=0x600000;table=0x700000
    entries=(0x800000,0x800100);items=(0x900000,0x900100);memory={}
    def put(address,data):
        for offset,value in enumerate(data):memory[address+offset]=value
    put(base+0x69c730,struct.pack('<Q',shared));put(shared,struct.pack('<Q',owner))
    put(owner+0x1008,struct.pack('<4Q',table,8,0,2));put(owner+0x1030,struct.pack('<I',20))
    put(table,struct.pack('<2Q',*entries)+bytes(48))
    for index,(entry,pointer) in enumerate(zip(entries,items),1):
        put(entry,struct.pack('<Q',pointer));raw=bytearray(0xa0)
        struct.pack_into('<Q',raw,0,base+0x5cf220);struct.pack_into('<I',raw,8,40+index)
        struct.pack_into('<I',raw,0x10,114643);struct.pack_into('<HH',raw,0x62,31,40)
        raw[0x67]=raw[0x68]=0;raw[0x6b]=1;put(pointer,raw)
    session=NS(read_block=lambda address,size:bytes(memory[address+i] for i in range(size)),
               assert_identity=lambda:None)
    monkeypatch.setattr(module,'MemoryGui',lambda session:NS(base=base,read=lambda name:'active'))
    def rich(pointer,slot):
        if slot==1:memory[items[0]+0x67]=1
        return NS(uid=41+slot,type_id=114643,plus=1,slot=slot,quantity=1,
                  gem1=0,gem2=0,bound=False)
...
def test_town_warehouse_items_rich_is_an_explicit_read_only_opt_in(monkeypatch):
    ...
    rich_item=object();calls=[]
    reader=NS(read=lambda **options:(calls.append(options) or WarehouseSnapshot((),20)))
    monkeypatch.setattr(module,'MemoryWarehouseReader',lambda adapter:reader)
    monkeypatch.setattr(merchant_memory,'MerchantMemory',lambda observer:NS(item=rich_item))
    trade=module.TownTrade.__new__(module.TownTrade)
    trade.observer=NS(adapter='adapter');trade.vendor=lambda kind:77
    assert trade({'action':'warehouse-items','rich':True})=={'items':(), 'capacity':20}
...
def test_deposit_drag_rechecks_control_layout_and_exact_state(monkeypatch):
    ...
    stash=WarehouseSnapshot((),20)
    inventory_grid=NS(size=(407.,175.),scroll=(0.,0.),position=(577.,435.))
    warehouse_grid=NS(size=(272.,326.),scroll=(0.,0.),position=(71.,195.))
    ...
    trade.vendor=lambda kind:123;trade.inventory=NS(read=lambda:bag)
    trade.shop=NS(gui=NS(read=lambda name:inventory_grid));trade.life=lambda **kwargs:None
    hovered=[];trade.require_warehouse_hover=lambda window:hovered.append(window)
    checks=[];trade.check_input=lambda:checks.append('control')
    layout=NS(assert_current=lambda revision:checks.append('layout'))
    revision=NS(client_size=(1416,1016),gui_size=(1036,793));trade.warehouse_layout=lambda:(layout,revision)
    endpoints=[]
...
def test_deposit_drag_refuses_release_after_warehouse_grid_moves(monkeypatch):
    ...
    stash=WarehouseSnapshot((),20)
    inventory_grid=NS(size=(407.,175.),scroll=(0.,0.),position=(577.,435.))
    original=NS(size=(272.,326.),scroll=(0.,0.),position=(71.,195.))
    moved=NS(size=(272.,326.),scroll=(0.,0.),position=(91.,195.))
    reads=[]
...
def test_deposit_refuses_grid_covered_by_another_panel(monkeypatch,covered):
    ...
    bag=NS(items=(item,),silver=100,equipped_ammo=None)
    stash=WarehouseSnapshot((),20)
    inventory_grid=NS(address=100,size=(407.,175.),scroll=(0.,0.),position=(577.,435.))
    warehouse_grid=NS(address=200,size=(272.,326.),scroll=(0.,0.),position=(71.,195.))
    reader=NS(read=lambda:stash,gui=NS(read=lambda name:warehouse_grid))
    monkeypatch.setattr(module,'MemoryWarehouseReader',lambda adapter:reader)
    monkeypatch.setattr(driver,'wait_hover_validation',lambda validate,check:validate())
    trade=module.TownTrade.__new__(module.TownTrade)
    trade.observer=NS(adapter=None,operations=NS(target='target'))
    trade.vendor=lambda kind:123;trade.inventory=NS(read=lambda:bag)
    trade.shop=NS(gui=NS(read=lambda name:inventory_grid));trade.life=lambda **kwargs:None
    ...
    def require_hover(window):
        ...
        if window is blocked:raise HoverNotReady(covered+' grid is covered')
    trade.require_warehouse_hover=require_hover
    pressed=[]
    def drag(*args,**kwargs):
        kwargs['before_press']();pressed.append(True);kwargs['before_release']()
    monkeypatch.setattr(module,'foreground_drag',drag)
