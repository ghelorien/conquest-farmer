...
def test_listing_preflight_allows_resize_but_rejects_changed_table_schema(changed_spacing):
    from conquest.merchants.driver import MerchantDriver
    controls={
        'inventory_item':{'window':'Inventory','size':[400,200],'table':'Items',
                          'columns':2,'stride':[40,40],'cell_offset':[20,20]},
        'remove_listing':{'window':'BoothChild','size':[400,200],'table':'BoothTable',
                          'columns':2,'stride':[80,64],'cell_offset':[60,39]},
        'booth_drop':{'window':'Booth','size':[800,466],'offset':[400,233]}}
    snapshot={'windows':[{'name':'Inventory','geometry':[0,0,620,300]},
                         {'name':'BoothChild','geometry':[0,0,700,350]},
                         {'name':'Booth','geometry':[0,0,1000,600]}]}
    def table(window,label):
        stride=(41 if changed_spacing and label=='Items' else
                40 if label=='Items' else 80)
        return {'columns':[{'content_x':10},{'content_x':10+stride}],
                'row_height':40 if label=='Items' else 64,
                'outer':(0,0,500,300),'clip':(0,0,500,300)}
...
def test_resized_booth_drop_uses_current_panel_center_and_current_scale(tmp_path):
    ...
    profile.write_text(__import__('json').dumps({'client_size':[1200,800],'gui_size':[1200,800],
        'controls':{'booth_drop':{'window':'Booth','size':[800,466],'offset':[400,233]}}}))
    driver=NS(qualification=profile,memory=NS(gui=NS(viewport_size=lambda:(1600,900))),
              target=NS(snapshot=lambda:{'client_size':[2400,1350]}))
    driver._control_layout=lambda snapshot,control,spec:MerchantDriver._control_layout(
        driver,snapshot,control,spec)
    snapshot={'windows':[{'name':'Booth','geometry':[66,27,1000,500],'scroll':[0,0]}]}
...
def test_booth_capability_survives_ordinary_client_resize(tmp_path):
    ...
    profile.write_text(__import__('json').dumps({
        'client_sha256':'build','character':'Dutch','server':'America',
        'client_size':[1200,800],'gui_size':[1200,800],
        'capabilities':{'booth_input':True},'evidence':'booth-control-evidence.json'}))
    driver=NS(qualification=profile,
    ...
    assert MerchantDriver.require_qualified(driver,'booth_input')['client_size']==[1200,800]
