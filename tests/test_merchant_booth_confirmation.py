...
@pytest.mark.parametrize(
    "change", ["none", "flag", "callback", "text", "handler", "window"]
)
def test_open_booth_control_pins_native_callback_and_flag(monkeypatch, change):
    ...
    window = 0x200000
    blocks = {
        model + 12: struct.pack("<B", 1),
        model + 0x100: struct.pack("<Q", model + 0xC8),
        model + 0xC8: struct.pack("<Q", base + 0x5C7CA0),
        model + 0xD0: struct.pack("<I", 12 if change == "flag" else 11),
        base + 0x5C7CB0: struct.pack(
            "<Q", base + (0xD8B91 if change == "callback" else 0xD8B90)
        ),
    }
    for rva, code in b.CODE.items():
        blocks[base + rva] = bytes.fromhex(code)
    if change == "handler":
        blocks[base + 0x95FD6] = bytes(5)
    raw = bytearray(0x250)
    ...
    if change == "none":
        assert b.control(driver, snapshot, {"uid": 11}) == (w, (944, 360))
    else:
        with pytest.raises(ValueError):
            b.control(driver, snapshot, {"uid": 11})


@pytest.mark.parametrize("owned", [True, False])
def test_submitted_claim_reconciles_without_second_confirmation(
    ...
    before = {
        ...
        "map_id": 1036,
        "position": [232, 193],
        "own_booth_uid": 90 if owned else 0,
        "booth_open": False,
    }
    record = {
        "phase": "submitted",
        "operation": "inspect_vacant_flag",
        "identity": {"pid": 1},
        "silver": 100,
        "inventory_before": [],
        "booth_before": [],
    ...
    monkeypatch.setattr(
        b, "submit", lambda *a: pytest.fail("Never repeat a submitted confirmation")
    )
    driver = NS(memory=NS(read=lambda: before), observer=NS(character="Dutch"))
    if owned:
        result = stall_probe.confirm_observed_flag(
            driver, None, Journal(tmp_path / "j.db"), record, lambda: None
        )
        assert result["claim_verified"] and not result["booth_open"]
        assert result["phase"] == "observed"
    else:
        with pytest.raises(ValueError, match="reconciliation"):
            stall_probe.confirm_observed_flag(
                driver, None, Journal(tmp_path / "j.db"), record, lambda: None
            )
