...
BASE = 0x140000000
REGISTRY = BASE + LAYOUT.ground_registry_rva
HOLDER = BASE + LAYOUT.ground_holder_vtable_rva


def fixture(monkeypatch):
    base, c, begin, obj = BASE, 0x100000, 0x200000, 0x300000
    owner = 0x400000
    record = bytearray(0x60)
    struct.pack_into("<Q", record, 0, base + LAYOUT.ground_actor_vtable_rva)
    ...
    layout = SimpleNamespace(
        begin_offset=0x58, end_offset=0x60, capacity_offset=0x68, max_objects=4096
    )
    entities = SimpleNamespace(
        session=session,
        layout=layout,
        _resolve=lambda: (base, c, [(0x500000, 0x600000)]),
    )
...
def test_ground_identity_type_and_position_are_read_without_images(monkeypatch):
    ...
    (drop,) = reader.read()
    assert (drop.uid, drop.type_id, drop.position, drop.silver) == (
        1996,
        1090020,
        (601, 585),
        True,
...
def test_disappearance_without_silver_increase_is_not_a_pickup(monkeypatch):
    ...
    (drop,) = reader.read()
    assert (
        memory_ground.pickup_delta(
            drop, SimpleNamespace(silver=100), SimpleNamespace(silver=100)
        )
        == 0
    )
    assert (
        memory_ground.pickup_delta(
            drop, SimpleNamespace(silver=100), SimpleNamespace(silver=113)
        )
        == 13
    )
...
def test_render_reference_counts_do_not_invalidate_stable_ground_identity(monkeypatch):
    ...
    def read(address, size):
        ...
        if address == 0x300000:
            struct.pack_into(
                "<I", record, 8, struct.unpack_from("<I", record, 8)[0] + 1
            )
        return value
...
def test_type_zero_registry_placeholder_does_not_block_stable_drops(monkeypatch):
    ...
    def fields(s, specs):
        return [
            0x200020
            if a in (REGISTRY + 8, REGISTRY + 16)
            else HOLDER
            if a == 0x410000
            else old_fields(s, [(a, k)])[0]
            for a, k in specs
        ]
...
@pytest.mark.parametrize(
    "offset,value",
    [(0, 0), (4, 123456), (8, 600), (12, 584), (16, 0x310010), (24, 0x310000)],
)
def test_registry_must_match_actor_identity_and_geometry(monkeypatch, offset, value):
    reader, _ = fixture(monkeypatch)
    struct.pack_into(
        "<Q" if offset >= 16 else "<I", reader.entities.test_registry, offset, value
    )
...
def test_creation_time_high_word_is_rechecked(monkeypatch):
    ...
    def read(a, n):
        value = old(a, n)
        if a == 0x300000:
            struct.pack_into("<I", record, 0x4C, 1)
        return value
...
def test_empty_ground_registry_is_valid(monkeypatch):
    reader, _ = fixture(monkeypatch)
    for address in (REGISTRY, REGISTRY + 8, REGISTRY + 16):
...
def test_ground_registry_bound_is_checked_before_reading_entries(monkeypatch):
    reader, _ = fixture(monkeypatch)
    for address in (REGISTRY + 8, REGISTRY + 16):
