...
def farmer(rig, monkeypatch):
    ...
    monkeypatch.setattr(
        manual_farmer,
        "controller",
        lambda runtime, observer: NS(driver=NS(observer=observer)),
    )
    return x


def test_farmer_request_is_observed_and_approved_through_same_exact_api(
    rig, monkeypatch
):
    ...
    with pytest.raises(CaptureUnavailable):
        with input_scope():
            pytest.fail("Pending farmer session must block farmer")
    with x.guard.lease("Dutch", purpose="trade"):
        pass
    x.now += 1
    x.runtime.approve_manual(pending["approval_binding"], operator="Floor")
...
def test_farmer_settlement_queues_only_farmer_replan_and_no_receipts(rig, monkeypatch):
    ...
    row = x.runtime.manual_status("Farmer")
    x.now += 1
    ...
    with x.journal.db() as db:
        ...
        assert not db.execute(
            "SELECT 1 FROM state WHERE character='Farmer' AND name='new_stock'"
        ).fetchone()


@pytest.mark.parametrize("phase", ["approval_pending", "needs_attention"])
def test_farmer_uuid_target_hold_blocks_all_farmer_input_not_other_targets(
    rig, monkeypatch, phase
):
    x = rig
    monkeypatch.setattr(
        "conquest.character_context.current",
        lambda: NS(profile=NS(id="farmer-uuid", role="Farmer")),
    )
    x.guard.set_manual_sessions(
        [
            dict(
                target_profile_id="farmer-uuid",
    ...
    with pytest.raises(CaptureUnavailable):
        with input_scope():
            pytest.fail("Farmer UUID is an input fence")
    with pytest.raises(CaptureUnavailable):
        with x.guard.lease("Farmer"):
            pytest.fail("Farmer lease is also fenced")
    with x.guard.lease("Dutch", purpose="trade"):
        pass
    assert not x.guard.manual_session_blocked("different-farmer-id")


def test_unsupported_farmer_full_memory_persists_qualification_blocker(
    rig, monkeypatch
):
    x = farmer(rig, monkeypatch)
    ...
    assert (
        status["session"]["phase"] == "needs_attention"
        and status["session"]["approval_binding"] is None
    )
...
def test_farmer_missing_decline_qualification_does_not_prevent_manual_approval(
    rig, monkeypatch
):
    x = farmer(rig, monkeypatch)

    def missing(*args):
        raise ValueError("trade_request input qualification pending")

    monkeypatch.setattr(manual_farmer, "controller", missing)
    x.runtime.observe_manual_farmer()
    assert (
        "qualification"
        in x.runtime.manual_farmer_status()["observation"]["decline_blocker"]
    )
...
def test_farmer_delivery_reservation_prevents_manual_admission(rig, monkeypatch):
    x = farmer(rig, monkeypatch)
    x.runtime.farmer_bot_owned = lambda: True
...
def test_native_farmer_boundary_detects_request_before_recovery_or_panels(
    rig, monkeypatch
):
    ...
    supervisor.recovery.step = lambda *a: pytest.fail(
        "Manual request must precede gameplay recovery"
    )
    before = control.snapshot()
    assert supervisor.observe()["manual_session"]
    assert (
        control.snapshot() == before
        and x.runtime.manual_status("Farmer")["phase"] == "approval_pending"
    )


def test_farmer_observer_keeps_reading_when_saved_farming_intent_is_off(
    rig, monkeypatch
):
    x = farmer(rig, monkeypatch)
...
def test_farmer_observer_closed_windows_needs_no_unqualified_full_reader(
    rig, monkeypatch
):
    x = farmer(rig, monkeypatch)
...
def test_farmer_loop_consumes_current_valuables_without_false_pickup_or_resume(
    ...
    supervisor.position = (301, 401)
    supervisor.pending_loot = ("old",)
    supervisor.patrol_chase = "old-route"
    supervisor.excluded_targets = {1: 9999}
    supervisor.movement_obstructions = {1: 9999}
    inventory = NS(
        started_at=time.monotonic(),
        items=[NS(uid=91, type_id=410009, plus=2, slot=0, amount=1)],
    )
    x.now += 1
    before = control.snapshot()
    ...
    with x.journal.db() as db:
        assert (
            db.execute("SELECT farmer_pending FROM manual_replans").fetchone()[0] == 0
        )
