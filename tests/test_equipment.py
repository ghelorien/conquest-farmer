...
@pytest.mark.parametrize(
    "change",
    [{"plus": 1}, {"plus": None}, {"type_id": 500037}, {"gem1": 1}, {"gem2": 1}],
)
def test_protected_equipped_gear_is_not_replaced_by_plain_shop_gear(change):
    s = state()
    s["equipment"]["bow"].update(change)
...
def test_equipping_keeps_old_item_and_all_unrelated_inventory():
    new = Item(2, 500045, 100, 100, 0, 0)
    keep = Item(3, 1088001, 1, 1, 1, 0)
    before = NS(items=(new, keep), silver=9000)
    old = Item(1, 500035, 100, 100, 0, 0)
    after = NS(items=(old, keep), silver=9000)
    newstate = state()
    newstate["equipment"]["bow"] = {"uid": 2}
    assert equip_receipt(2, "bow", before, state(), after, newstate)
    assert not equip_receipt(
        2, "bow", before, state(), NS(items=(keep,), silver=9000), newstate
    )
    assert not equip_receipt(
        2, "bow", before, state(), NS(items=(old, keep), silver=8999), newstate
    )
...
def test_every_slot_reviewed_and_headgear_is_in_armorer_review():
    ...
    head = bow(
        type_id=113413, name="CatHat", level=37, attack_min=0, attack_max=0, defense=20
    )
    s = state(37)
    ...
    assert (
        "Keeping silver for supplies"
        in review_slots([bow()], s, 6999)["bow"]["reasons"]
    )
    s["equipment"]["bow"]["plus"] = 1
    assert (
        "Keeping protected or unverified equipped gear"
        in review_slots([bow()], s, 10000)["bow"]["reasons"]
    )
