...
def trip(tmp_path):
    ...
    sample = {
        ...
        "session_id": 900,
        "kills": 20,
        "last_kill": {"rowid": 8, "time": 990, "count": 1},
    }
    visit = TownVisit(
        tmp_path / "trip.json",
        clock=lambda: now[0],
        probe=lambda: deepcopy(sample),
        profile="farmer-1",
    )
    health = {
...
def returned(trip):
    ...
    now[0] = 1020
    sample.update(observed_at=1020, cursor=15, kills=22)
...
def test_required_trip_reuses_identity_and_original_baseline_across_restart(trip):
    ...
    sample["kills"] = 22
    restart = TownVisit(
        visit.path, clock=visit.clock, probe=visit.probe, profile="farmer-1"
    )
...
def test_no_visit_is_created_by_hunting_or_unnecessary_town_request(trip):
    ...
    with pytest.raises(ValueError):
        visit.begin("periodic_merchant_trip", hunt_map_id=1000)
...
def test_trip_completes_only_after_return_and_a_new_verified_kill(trip):
    ...
    assert (
        completed["baseline"]["kills"] == 20
        and completed["return_baseline"]["kills"] == 22
    )
    assert completed["first_verified_resume_kill"] == {
        "rowid": 16,
        "time": 1021,
        "count": 1,
    }
    ...
    now[0] = 1030
    next_visit = visit.begin("restock", hunt_map_id=1000)
    assert next_visit["town_visit_id"] != completed["town_visit_id"]
    assert next_visit["history"][0]["town_visit_id"] == completed["town_visit_id"]
    assert (
        next_visit["history"][0]["phase"] == "complete"
        and "history" not in next_visit["history"][0]
    )
...
def test_unsafe_or_unverified_resume_never_completes_trip(trip, failure):
    ...
    if failure == "map":
        data["life"]["map_id"] = 1036
    if failure == "stale":
        data["observed_at"] = 1010
    if failure == "client":
        health["target"]["pid"] = 8
    if failure == "session":
        sample["session_id"] = 901
    if failure == "counter_reset":
        sample["kills"] = 1
    if failure == "counter_unchanged":
        sample["kills"] = 22
    if failure == "old_row":
        sample["last_kill"]["rowid"] = 15
    if failure == "old_time":
        sample["last_kill"]["time"] = 1019
    if failure == "metrics":
...
def test_unavailable_return_baseline_requires_a_later_kill_after_recovery(trip):
    ...
    sample.update(
        ...
        cursor=17,
        kills=24,
        last_kill={"rowid": 17, "time": 1022, "count": 1},
    )
    now[0] = 1022
    health["embedded_controls"]["observed_at"] = 1022
    assert visit.observe_hunting(health)["phase"] == "complete"


def test_stop_before_first_return_kill_rebases_new_session_then_requires_later_kill(
    trip, tmp_path
):
    ...
    stats = SessionKills(output, clock=lambda: now[0])
    stats.begin()

    def publish():
        write_json(
            output / "app-state.json",
            {"character": "Parasite", "updated_at": now[0], **stats.refresh()},
        )
    ...
    stats.begin()
    publish()
    health["embedded_controls"]["control"]["enabled"] = True
    assert visit.observe_hunting(health) is None
    row = visit.state()
    assert (
        row["phase"] == "returning_to_hunt"
        and visit.active_id() == row["town_visit_id"]
    )
    assert (
        row["return_baseline"]["session_id"] == 1030
        and row["return_baseline"]["kills"] == 0
    )
    assert row["return_baseline_history"][0]["previous_baseline"] == original
    assert (
        row["baseline"]["session_id"] == 1000
    )  # Historical town metrics are preserved.
    assert (
        visit.observe_hunting(health) is None
        and len(visit.state()["return_baseline_history"]) == 1
    )
    now[0] = 1031
    with sqlite3.connect(output / "trial.sqlite3") as db:
        db.execute(
            "INSERT INTO events VALUES(?,?,?)",
            (now[0], "kill_verified", json.dumps({"count": 1})),
        )
    publish()
    completed = visit.observe_hunting(health)
    assert (
        completed["phase"] == "complete"
        and completed["resumed_hunting"]["session_id"] == 1030
    )
    assert (
        completed["first_verified_resume_kill"]["time"] == 1031
        and visit.active_id() is None
    )
...
def test_kills_already_seen_in_the_new_session_cannot_complete_the_rebase(trip):
    visit, now, sample, health = returned(trip)
    now[0] = 1030
    health["embedded_controls"]["observed_at"] = 1030
    sample.update(
        session_id=1030,
        observed_at=1030,
        cursor=99,
    ...
    assert visit.observe_hunting(health) is None
    now[0] = 1031
    health["embedded_controls"]["observed_at"] = 1031
    sample.update(
        observed_at=1031,
        cursor=100,
        kills=51,
        last_kill={"rowid": 100, "time": 1031, "count": 1},
    )
...
def test_session_rollover_requires_same_live_unfenced_return_evidence(trip, failure):
    ...
    now[0] = 1030
    sample.update(session_id=1030, observed_at=1030, kills=0)
    data = health["embedded_controls"]
    data["observed_at"] = 1030
    if failure == "pid":
        health["target"]["pid"] = 8
    if failure == "creation":
        health["target"]["creation_time_100ns"] = 101
    if failure == "path":
        health["target"]["path"] = "another.exe"
    if failure == "missing_identity":
        health["target"].pop("creation_time_100ns")
    if failure == "map":
        data["life"]["map_id"] = 1036
    if failure == "stale":
        data["observed_at"] = 1020
    if failure == "dead":
        data["life"]["dead_candidate"] = True
    if failure == "hp":
        data["life"]["current_hp"] = 0
    if failure == "off":
        data["control"]["enabled"] = False
    if failure == "paused":
        data["control"]["paused"] = True
    if failure == "manual":
        data["manual_mouse"] = True
    if failure == "fence":
        data["manual_input_fence"] = True
    if failure == "missing_fence":
        data.pop("manual_input_fence")
    if failure == "metrics":
        sample["available"] = False
    if failure == "old_session":
        sample["session_id"] = 901
    assert visit.observe_hunting(health) is None
...
@pytest.mark.parametrize(
    "change", [{"pid": 8}, {"creation_time_100ns": 101}, {"path": "other.exe"}]
)
def test_hunt_reentry_cannot_replace_an_unfinished_return_identity(trip, change):
    visit, now, sample, health = returned(trip)
    before = visit.path.read_bytes()
    health["target"].update(change)
...
def test_readonly_checkpoint_uses_verified_rows_without_touching_counter_files(
    ...
    with sqlite3.connect(dbpath) as db:
        ...
        db.execute(
            "INSERT INTO events VALUES(?,?,?)",
            (999, "kill_verified", json.dumps({"count": 2})),
        )
        db.execute(
            "INSERT INTO events VALUES(?,?,?)",
            (1000, "attack", json.dumps({"count": 100})),
        )
    ...
    with sqlite3.connect(dbpath) as db:
        db.execute(
            "INSERT INTO events VALUES(?,?,?)",
            (1000, "kill_verified", json.dumps({"count": 1})),
        )
    checkpoint = kill_checkpoint(
        now=1000, output=tmp_path, after_cursor=0, after_time=998
    )
    assert (
        checkpoint["last_kill"]["rowid"] == 3
        and checkpoint["resume_kill"]["rowid"] == 1
    )


def test_market_visit_links_to_required_trip_without_renewing_budget(
    trip, tmp_path, monkeypatch
):
