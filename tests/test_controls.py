...
def observed_loot():
    return LootObservation(
        "test-client/map", 10, (), (), InventorySnapshot(10, 10, (), None, 0, 40)
    )
...
def test_focus_changes_never_change_user_switch_or_selection():
    ...
    assert (
        control.reconcile(focused=False, minimized=False, observed_ids=[77])[
            "execution_state"
        ]
        == "waiting_for_focus"
    )
    ...
    assert (
        control.reconcile(focused=True, minimized=False, observed_ids=[77])[
            "execution_state"
        ]
        == "ready"
    )
...
def test_group_selection_resolves_new_ids_without_turning_off(tmp_path):
    ...
    first = control.reconcile(
        focused=False,
        minimized=False,
        observed_ids=[77, 88],
        observed_types={77: 1, 88: 2},
    )
    assert first["resolved_target_ids"] == [77] and first["execution_state"] == "ready"
    second = control.reconcile(
        focused=False,
        minimized=False,
        observed_ids=[99, 100, 88],
        observed_types={99: 1, 100: 1, 88: 2},
    )
    assert (
        second["resolved_target_ids"] == [99, 100]
...
def test_group_off_rejects_pending_attack_and_other_species():
    ...
    assert control.dispatch(
        current["revision"], 99, lambda: calls.append(99), type_id=1
    )
    assert not control.dispatch(
        current["revision"], 88, lambda: calls.append(88), type_id=2
    )
...
def test_runtime_expands_group_but_does_not_abandon_attacked_id_without_kill_evidence():
    ...
    calls = []
    monsters = [
        {"entity_id": 77, "type_id": 1, "alive": True, "object_address": 177},
        {"entity_id": 88, "type_id": 2},
    ]
    runtime = ControlRuntime(
        ...
        observer=lambda: {
            ...
            "focused": False,
            "loot_observation": observed_loot(),
        },
        dispatcher=lambda monster, *args: calls.append(monster["entity_id"]),
        pickup_dispatcher=lambda *_: None,
        clock=lambda: 10,
    )
    runtime.step()
...
def test_no_ids_never_means_attack_everything():
    ...
    assert not control.dispatch(
        control.revision, 77, lambda: pytest.fail("Unselected attack")
    )


@pytest.mark.parametrize(
    "change", [{"enabled": False}, {"target_ids": [88]}, {"input_mode": "foreground"}]
)
def test_settings_change_invalidates_pending_attack(change):
    ...
    assert not control.dispatch(
        old["revision"], 77, lambda: pytest.fail("Stale attack")
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"target_ids": [2**32]},
        {"target_ids": ["77"]},
        {"input_mode": "steal_focus"},
        {"script": "execute"},
        {"target_ids": list(range(1, 130))},
    ],
)
...
def test_off_serializes_with_in_flight_input():
    ...
    attack = threading.Thread(
        target=lambda: control.dispatch(settings["revision"], 77, send)
    )
    attack.start()
    assert started.wait(2)
    off = threading.Thread(
        target=lambda: (control.update({"enabled": False}), stopped.set())
    )
    off.start()
    ...
    assert not control.dispatch(
        settings["revision"], 77, lambda: pytest.fail("Attack after off")
    )
...
def test_runtime_dispatches_only_selected_id_while_unfocused():
    ...
    observation = {
        "monsters": [
            {"entity_id": 66},
            {"entity_id": 77, "object_address": 177, "alive": True},
        ],
        "focused": False,
        "minimized": False,
        "blockers": [],
    ...
    runtime = ControlRuntime(
        ...
        dispatcher=lambda monster, data, mode: calls.append(
            (monster["entity_id"], mode)
        ),
        pickup_dispatcher=lambda *_: None,
        clock=lambda: 10,
    )
...
def test_validation_blocker_preserves_on_without_dispatching():
    ...
    runtime = ControlRuntime(
        ...
        "Parasite",
        observer=lambda: {
            "monsters": [{"entity_id": 77}],
            "focused": False,
            "blockers": ["HP unvalidated"],
        },
...
def test_external_runner_failure_preserves_real_reason_instead_of_generic_blockers():
    ...
    runtime = ControlRuntime(
        ...
        observer=lambda: {
            "monsters": [],
            "focused": True,
            "blockers": ["Old diagnostic blocker"],
        },
    )
...
def test_old_runner_cleanup_preserves_new_enabled_selection():
    ...
    assert control.snapshot()["enabled"] and control.snapshot()["target_type_ids"] == [
        7
    ]
    control.update({"enabled": False})
