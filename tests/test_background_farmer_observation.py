...
def ui(monkeypatch):
    ...
    session = SimpleNamespace(
        ...
        read_block=lambda address, size: struct.pack("<II", 130, 2),
    )
    ...
    monkeypatch.setattr(
        module,
        "BackgroundObservationReader",
        lambda s: SimpleNamespace(snapshot=lambda: {"frame": 1}),
    )
...
def test_optional_budget_never_starts_additional_scans(ui, monkeypatch):
    monkeypatch.setattr(
        module,
        "BackgroundObservationReader",
        lambda s: pytest.fail("optional scan started after deadline"),
    )
