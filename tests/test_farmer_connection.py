...
def test_exact_farmer_attachment_preserves_other_accounts_and_manual_control(
    monkeypatch, case
):
    calls = []
    ...
    app = NS(
        ...
        catalog=NS(windows=lambda: [candidate]),
    ...
    runtime = NS(
        connecting={},
        refilling={},
        observers={"Dutch": observer} if case == "merchant" else {},
        journal=NS(pending=lambda c: []),
    )
    ui = NS(
        app=app,
        closed=False,
        coordinator=NS(stopped=case == "stopped", owner=None),
    ...
    if case in ("login", "parasite", "layout_pending"):
        ...
        assert (
            result["attached"] == (case != "layout_pending")
            and not result["farming_enabled"]
        )
        if case == "layout_pending":
            assert result["pending"]
        assert ("identity" in calls) == (case != "login")
    else:
        with pytest.raises(ValueError):
            attach(ui, 10, 21 if case == "replaced" else 20, queued_at=queued)
