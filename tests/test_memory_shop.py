...
def test_ground_diagnostics_do_not_shadow_supply_snapshot_serializer():
    ...
    trade.life = lambda *args, **kwargs: SimpleNamespace(
        map_id=1011, position=(341, 441)
    )
    trade.inventory = SimpleNamespace(
        read=lambda: InventorySnapshot(1, 1, (), None, 200, 40)
    )
    assert trade.execute({"action": "supplies"})["silver"] == 200


def gui_fixture():
    base, context, array, window, name = (
        0x140000000,
        0x100000,
        0x200000,
        0x300000,
    ...
    blobs = {
        ...
        context + 0x3E58: struct.pack("<IIQ", 1, 1, array),
        context + 0x3E38: struct.pack("<I", 1000),
        array: struct.pack("<Q", window),
        window: record,
...
def test_shop_item_coordinates_follow_observed_grid_and_selected_memory_index():
    window = GuiWindow(1, "Shop", (51.0, 158.0), (288.0, 396.0), (0.0, 0.0))
    grid = GuiWindow(2, "Shop/grid", (71.0, 196.0), (248.0, 328.0), (0.0, 0.0))
...
def test_live_taller_pharmacist_preserves_qualified_item_pitch():
    window = GuiWindow(1, "Shop", (79.0, 172.0), (288.0, 430.0), (0.0, 0.0))
    grid = GuiWindow(2, "Shop/grid", (99.0, 210.0), (248.0, 362.0), (0.0, 0.0))
    potion = ShopProduct(2, 3, 1000020, "Painkiller", 60, 250)
...
@pytest.mark.parametrize(
    "size,position",
    [
        ((249.0, 362.0), (99.0, 210.0)),
        ((248.0, 363.0), (99.0, 210.0)),
        ((248.0, 362.0), (100.0, 210.0)),
        ((248.0, 362.0), (99.0, 211.0)),
    ],
)
def test_shop_width_padding_or_child_height_changes_still_block(size, position):
    window = GuiWindow(1, "Shop", (79.0, 172.0), (288.0, 430.0), (0.0, 0.0))
    grid = GuiWindow(2, "Shop/grid", position, size, (0.0, 0.0))
    potion = ShopProduct(2, 3, 1000020, "Painkiller", 60, 250)
