...
@pytest.mark.parametrize('phase,updated,enabled,expected',[
...
def test_dead_activity_only_claims_recovery_with_active_work(phase,updated,enabled,expected):
    from conquest.farm_telemetry import activity_text
    route={'phase':phase,'updated_at':updated,'detail':'reload failure'}
    text=activity_text(route,{}, {'enabled':enabled},{'dead_candidate':True},now=100)
    assert expected in text


def test_manual_stop_overrides_dead_route_recovery_activity():
    from conquest.farm_telemetry import activity_text
    text=activity_text({'phase':'recovering_route','updated_at':99},
        {'manual_stop_revision':4},{'enabled':False,'revision':4},{'dead_candidate':True},now=100)
