...
def test_new_root_private_acl_is_idempotent_and_children_are_same_user_writable(
    tmp_path,
):
    ...
    if os.name == "nt":
        ...
        descriptor = native.GetNamedSecurityInfo(
            str(root), native.SE_FILE_OBJECT, native.DACL_SECURITY_INFORMATION
        )
        ...
        assert [(native.ConvertSidToStringSid(row[2]), row[1]) for row in rows] == [
            (user, security.MODIFY),
            ("S-1-5-18", security.FULL),
            ("S-1-5-32-544", security.FULL),
        ]
...
def test_fresh_app_and_profile_roots_provision_before_payload(tmp_path):
    ...
    for path in (
        root / "app.lock",
        root / "profiles.lock",
        root.parent / ".managed.managed.lock",
    ):
...
def test_failed_acl_provision_leaves_no_stage_or_new_root(tmp_path, monkeypatch):
    ...
    root = tmp_path / "managed"
    with pytest.raises(ValueError, match="ACL unavailable"):
        security.ensure_managed_directory(root)
    assert not root.exists()
    with pytest.raises(ValueError, match="ACL unavailable"):
        profile_migration._create_stage(root)
    assert not list(tmp_path.iterdir())
    with pytest.raises(ValueError, match="ACL unavailable"):
        security.open_managed_lock(tmp_path / "lock")
    assert not list(tmp_path.iterdir())


def test_stage_acl_failure_preserves_diagnostics_and_legacy_source(
    tmp_path, monkeypatch
):
    source = tmp_path / "legacy"
    source.mkdir()
    (source / "keep").write_text("legacy")
    destination = tmp_path / "managed"
    (destination / "diagnostics").mkdir(parents=True)
    diagnostic = destination / "diagnostics" / "startup.txt"
    diagnostic.write_text("diagnostic")
    original = security.provision_new

    def provision(path, *, directory):
        if directory:
            raise ValueError("cannot protect stage")
    ...
    with pytest.raises(ValueError, match="cannot protect stage"):
        profile_migration.migrate_legacy(
            source, destination, check_offline=lambda _: None
        )
    assert (
        diagnostic.read_text() == "diagnostic"
        and (source / "keep").read_text() == "legacy"
    )
...
def test_missing_or_service_token_fails_closed_and_cleans_new_root(
    tmp_path, monkeypatch
):
    def unavailable(*args):
        raise ValueError("interactive user SID unavailable")
...
def test_existing_root_is_never_silently_repaired(tmp_path, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(security, "provision_new", lambda *a, **k: calls.append(a))
    monkeypatch.setattr(
        security,
        "verify_existing",
        lambda *a, **k: (_ for _ in ()).throw(ValueError("foreign or inaccessible")),
    )
