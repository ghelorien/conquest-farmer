...
def rig(tmp_path, monkeypatch):
    ...
    farmer_profile = profiles.add("Parasite")
    merchant_profile = profiles.add("Spiritual", role="Merchant")
    ...
    state["recipient"].update(
        address=1234,
        position=deepcopy(state["intent"]["merchant"]["position"]),
        point=[10, 10],
    )
    ...
    peer = {
        ...
        "controls": {"preserved": {"verified": True}},
        "evidence": "earlier-booth-proof",
    }
    ...
    source = observer("farmer")
    receiver = observer("merchant")
    owner = InputCoordinator(lambda: True, path=tmp_path / "input.lock")
    owner.on_acquire = lambda *_: pytest.fail(
    ...
    runtime.observers[character] = receiver
    runtime.controllers[character] = NS(driver=driver)
    ...
    ui = NS(
        ...
        app=NS(
            observer=source, control=NS(snapshot=lambda: dict(control)), closing=False
        ),
    )
    ...
    def fresh(*args, **kwargs):
        return tuple(
            {**deepcopy(snapshot), "timestamp": time.time()} for snapshot in snapshots
        )
    ...
    return NS(
        ...
        source=source,
        receiver=receiver,
        farmer_path=qualification_path(source, migrate=False),
        snapshots=snapshots,
        control=control,
        peer=peer,
        owner=owner,
...
def save(rig):
    rig.receipt.write_text(json.dumps(rig.state))
...
def listed_delivery(rig, *, uid=10, character=None, phase="verified", item=None):
    ...
    if phase == "verified":
        rig.runtime.journal.transition(
            key,
            "verified",
            {"uid": uid, "name": "MeteorScroll", "old_price": None, "price": 750000},
        )
...
def append_listing(rig, snapshot, item, price, suffix):
    ...
    after["inventory"] = [
        row for row in after["inventory"] if row["uid"] != item["uid"]
    ]
...
def chain(rig):
    ...
    return module._listing_chain_reconciles(
        rig.state, rig.runtime.journal, farmer, merchant, now=time.time()
    )
...
def test_promotion_accepts_only_a_complete_verified_native_listing_chain(rig):
    ...
    assert (
        manifest.is_file() and manifest.parent.name == "delivery-promotion-chain-audit"
    )
    raw = manifest.read_bytes()
    assert manifest.stem == hashlib.sha256(raw).hexdigest()
    farmer = json.loads(rig.farmer_path.read_text())
    merchant = json.loads(rig.peer_path.read_text())
    assert (
        farmer["listing_chain_evidence"]
        == merchant["listing_chain_evidence"]
        == str(manifest)
    )
...
def test_listing_chain_manifest_refuses_a_tampered_existing_audit(rig):
    listed_delivery(rig)
    farmer, merchant = module.pair(rig.ui, rig.character)
    manifest = module._chain_manifest(
        rig.state, rig.runtime.journal, farmer, merchant, now=time.time()
    )
...
def test_listing_chain_replays_every_verified_listing_before_the_delivered_scroll(rig):
    bamboo = {
        **rig.state["intent"]["items"][0],
        "uid": 11,
        "type_id": 410339,
        "plus": 0,
    }
    rig.state["intent"]["merchant"]["inventory"] = [deepcopy(bamboo)]
    rig.state["merchant_after"]["inventory"] = [
        deepcopy(bamboo),
        *rig.state["merchant_after"]["inventory"],
    ]
    rig.state["merchant_after"]["booth_open"] = False
    first = append_listing(
        rig, rig.state["merchant_after"], bamboo, 1_000_000, "bamboo"
    )
    second = append_listing(
        rig, first, rig.state["intent"]["items"][0], 750_000, "scroll"
    )
    rig.snapshots[0] = deepcopy(rig.state["farmer_after"])
    rig.snapshots[1] = second
    assert chain(rig)
...
def test_listing_chain_is_fail_closed_for_adversarial_evidence(rig, case):
    if case == "missing":
        ...
        with rig.runtime.journal.db() as db:
            db.execute(
                "UPDATE transactions SET character='wrong-merchant' WHERE id=?", (key,)
            )
        ...
        if case == "malformed":
            ...
            rig.runtime.journal.begin(
                "listing:Spiritual:unrelated", "Spiritual", "listing", extra
            )
            rig.runtime.journal.transition("listing:Spiritual:unrelated", "submitted")
            rig.runtime.journal.transition(
                "listing:Spiritual:unrelated",
                "verified",
                {"uid": 99, "name": "Other", "old_price": None, "price": 1},
            )
        elif case == "stale_current":
            farmer = deepcopy(rig.state["farmer_after"])
            merchant = deepcopy(rig.snapshots[1])
            farmer["timestamp"] = time.time()
            merchant["timestamp"] = 1
            assert not module._listing_chain_reconciles(
                rig.state, rig.runtime.journal, farmer, merchant, now=time.time()
            )
            return
            ...
            with rig.runtime.journal.db() as db:
                db.execute(
                    "UPDATE transaction_steps SET payload='{}' WHERE transaction_id=? AND status='submitted'",
                    (key,),
                )
            ...
            with rig.runtime.journal.db() as db:
                db.execute(
                    "UPDATE transactions SET updated=? WHERE id=?",
                    (time.time() + 3600, key),
                )
        elif case in (
            "historical_trade",
...
def test_listing_chain_rejects_overlapping_transaction_receipts(rig):
    ...
    with rig.runtime.journal.db() as db:
        prior = db.execute(
            "SELECT updated FROM transactions WHERE id='listing:Spiritual:bamboo'"
        ).fetchone()[0]
        db.execute(
            "UPDATE transactions SET created=? WHERE id='listing:Spiritual:scroll'",
            (prior - 0.001,),
        )
    assert not chain(rig)


def test_native_promotion_uses_canonical_archived_evidence_and_preserves_permissions(
    rig,
):
    ...
    assert (
        ...
        and not result["permissions_changed"]
    )
    ...
    farmer = json.loads(rig.farmer_path.read_text())
    peer = json.loads(rig.peer_path.read_text())
    assert (
        farmer["profile_id"] == rig.farmer_profile.id and farmer["character_uid"] == 1
    )
    assert farmer["capabilities"] == {"farmer_delivery": True}
    assert (
        peer["capabilities"]["trade"] is peer["capabilities"]["trade_request"] is True
    )
    assert peer["controls"]["preserved"] == {"verified": True}
    archive = Path(result["evidence"])
    assert (
        archive != rig.receipt
        and archive.parent == rig.receipt.parent / "delivery-request-probe-audit"
    )
    assert json.loads(archive.read_text()) == rig.state and farmer["evidence"] == str(
        archive
    )
    assert peer["trade_evidence"] == [str(archive)]
    assert not rig.runtime.enabled(rig.character) and not rig.runtime.refill_enabled(
        rig.character
    )
    assert (
        rig.control == {"enabled": False, "revision": 4, "paused": False}
        and rig.owner.owner is None
    )
...
def test_bridge_rejects_caller_paths_and_other_extra_fields_before_promotion(
    rig, extra
):
    value = (
        rig.character.profile_id
        if extra == "profile_id"
        else "Spiritual"
        if extra == "character"
        else "untrusted"
    ...
    with pytest.raises(ValueError):
        UnifiedUI.dispatch(rig.ui, {"action": "probe-delivery-promote", extra: value})
    assert (
        not rig.farmer_path.exists()
        and json.loads(rig.peer_path.read_text()) == rig.peer
    )
...
def test_invalid_or_changed_authority_never_promotes(rig, monkeypatch, change):
    if change == "phase":
        rig.state["phase"] = "merchant_confirm_submitted"
        save(rig)
    elif change == "receipt_farmer":
        rig.state["farmer_profile_id"] = "another"
        save(rig)
    elif change == "receipt_merchant":
        rig.state["target_profile_id"] = rig.farmer_profile.id
        save(rig)
    elif change == "source_process":
        rig.source.adapter.identity = {"pid": 987}
    elif change == "receiver_process":
        rig.receiver.adapter.identity = {"pid": 987}
    elif change == "source_build":
        rig.source.adapter.expected_sha256 = "a" * 64
    elif change == "receiver_build":
        rig.receiver.adapter.expected_sha256 = "a" * 64
    elif change == "candidate_build":
        rig.candidate.write_text(json.dumps({"client_sha256": "a" * 64}))
    elif change == "merchant_path":
        rig.runtime.controllers[
            rig.character
        ].driver.qualification = rig.peer_path.with_name("other.json")
    elif change in ("merchant_uid", "farmer_uid"):
        role = change.removesuffix("_uid")
        for snapshot in (rig.state["intent"][role], rig.state[role + "_after"]):
            snapshot["character_uid"] = 55
        rig.snapshots[role == "merchant"]["character_uid"] = 55
        save(rig)
    elif change == "live_uid":
        rig.snapshots[1]["character_uid"] = 55
    elif change == "live_item":
        rig.snapshots[1]["inventory"] = []
    elif change == "live_silver":
        rig.snapshots[1]["silver"] += 1
    elif change == "stop":
        rig.owner.stop()
    elif change == "manual":
        rig.owner.manual_sessions[rig.character.profile_id] = {"holds_automation": True}
    elif change == "farming":
        rig.control["enabled"] = True
    elif change == "revision":
        monkeypatch.setattr(
            module, "validate_candidate", lambda *args: rig.control.update(revision=5)
        )
    elif change == "pending":
        rig.runtime.journal.begin("pending", rig.character, "listing", {})
    elif change == "worker":
        rig.ui.delivery_probe_thread = NS(is_alive=lambda: True)
    else:

        def reject(*args):
            raise ValueError("Current recipient layout differs")
...
def test_replacement_during_validation_is_rejected_before_either_qualification_write(
    rig, monkeypatch, replacement
):
    def replace(*args):
        path = {
            "receipt": rig.receipt,
            "candidate": rig.candidate,
            "merchant": rig.peer_path,
        }[replacement]
...
def test_failed_second_file_write_is_safely_idempotent_and_does_not_enable_operations(
    rig, monkeypatch
):
    write = delivery_qualification.write_json

    def fail_peer(path, value):
        if Path(path) == rig.peer_path:
    ...
    with pytest.raises(OSError):
        module.promote_current(rig.ui)
    assert (
        rig.farmer_path.exists()
        and not json.loads(rig.peer_path.read_text())["capabilities"]["trade"]
    )
    assert not rig.runtime.enabled(rig.character) and not rig.runtime.refill_enabled(
        rig.character
    )
...
def test_status_and_readiness_observe_promoted_flags_without_resuming_any_permission(
    rig, monkeypatch
):
    ...
    monkeypatch.setattr(
        delivery_readiness,
        "read_json",
        lambda _: {"enabled": True, "parity_verified": True},
    )
    monkeypatch.setattr(
        delivery_readiness, "credential_path", lambda _: NS(is_file=lambda: True)
    )

    def farmer_driver(ui):
        driver = object.__new__(MerchantDriver)
        driver.observer = rig.source
        driver.qualification = rig.farmer_path
...
def test_candidate_validation_uses_live_memory_reader_and_pinned_native_mode(
    monkeypatch, change
):
    ...
    def read(source, candidate, merchant):
        ...
        if change == "actor":
            return {**actor, "address": 4321}
        if change == "uid":
            return {**actor, "uid": 3}
        if change == "name":
            return {**actor, "name": "Dutch"}
        return actor
    ...
    candidate = {
        ...
        "target_mode": {"rva": 123, "value": 18 if change == "mode" else 19},
    }
    state = {
        "recipient": actor,
        "intent": {
    ...
    if change in ("viewport", "uid", "name", "mode"):
        with pytest.raises(ValueError):
            module.validate_candidate(observer, candidate, {}, merchant, state)
    else:
