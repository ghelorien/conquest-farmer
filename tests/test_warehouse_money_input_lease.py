...
def bank(tmp_path, monkeypatch):
    ...
    state=NS(owner=owner,inventory=InventorySnapshot(1.,1.,(),None,1000,40),
        stored=500,amount='',clicks=[],typed=[],reads=[],requests=[],events=[],
        hook=lambda boundary:None,after_wait=lambda:None,credit=True)
    ...
    monkeypatch.setattr(warehouse_money,'WarehouseMoneyReader',
        lambda _:NS(read=lambda:read('bank',NS(silver=state.stored,amount=state.amount))))
    monkeypatch.setattr(warehouse_money,'money_points',lambda *args:read('geometry',points))
    ...
    def click(target,x,y,*args,**kwargs):
        ...
        if boundary!='amount' and state.credit:
            ...
            state.inventory=replace(state.inventory,silver=state.inventory.silver-delta)
            state.stored+=delta
    ...
    def verified(read,accept,failure,**kwargs):
        result=read()
        if not accept(result):raise ValueError(failure)
        return result
...
def test_contention_before_entry_retries_with_fresh_money_evidence(bank):
    ...
    assert receipt=={'direction':'deposit','amount':100,'silver':1100,
                     'stored_silver':800,'verified':True}
...
@pytest.mark.parametrize('direction,wallet,stored', [('deposit',900,600),('withdraw',1100,400)])
def test_one_shared_lease_covers_prechecks_entry_submission_and_receipt(bank,direction,wallet,stored):
    blocked=[]
    ...
    def observe(boundary):
        ...
        def peer():
            ...
            if acquired:bank.owner.lock.release()
    ...
    assert receipt['silver']==wallet and receipt['stored_silver']==stored and receipt['verified']
...
def test_manual_or_control_denial_between_steps_aborts_without_submission_or_retry(bank,boundary,denial):
    def intervene(current):
        if current!=boundary:return
        if denial=='stop':bank.owner.stop()
        elif denial=='mouse':bank.owner.manual_active=lambda:True
        elif denial=='manual_session':bank.owner.manual_sessions['Farmer']={'holds_automation':True}
        else:
            @contextmanager
            def invalidated():
    ...
    bank.hook=intervene
    with pytest.raises(CaptureUnavailable) as caught:transfer(bank)
    assert not isinstance(caught.value,TownObservationUnavailable)
    assert bank.clicks==['amount'] and bank.inventory.silver==1000 and bank.stored==500
    assert len(bank.requests)==1
    assert_released(bank)


@pytest.mark.parametrize('boundary', ['amount_after','typing_after','deposit_after'])
@pytest.mark.parametrize('error_type', [InputAcquisitionBusy,TownObservationUnavailable,CaptureUnavailable])
def test_inner_retry_shaped_errors_never_replay_the_whole_transfer(bank,boundary,error_type):
    def fail(current):
        if current==boundary:raise error_type('Waiting for the current input action')
    bank.hook=fail
    with pytest.raises(CaptureUnavailable) as caught:transfer(bank)
    assert type(caught.value) is CaptureUnavailable
    assert len(bank.requests)==1
    assert bank.clicks==(['amount','deposit'] if boundary=='deposit_after' else ['amount'])
...
def test_missing_bilateral_receipt_does_not_repeat_submission(bank):
    bank.credit=False
    with pytest.raises(ValueError,match='transfer unverified; no repeat input'):transfer(bank)
...
def test_stop_after_submission_allows_only_read_only_receipt_reconciliation(bank):
    bank.hook=lambda boundary:bank.owner.stop() if boundary=='deposit_after' else None
