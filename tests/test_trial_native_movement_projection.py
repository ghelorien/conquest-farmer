...
def test_scatter_landing_keeps_actual_anchor_and_refuses_unchecked_segment(
    ...
    monkeypatch.setattr(
        "conquest.scatter_movement.scatter_landing", lambda *a, **kw: landing
    )
    ...
    class Session:
        def request(self, operation, body=None):
            ...
            if operation == "sample":
                ...
                return {
                    "fields": [
                        {"name": key, "value": value} for key, value in fields.items()
                    ]
                }
    ...
    supervisor = SimpleNamespace(
        recovery=SimpleNamespace(terrain=ground),
        map_id=1002,
        movement_obstructions=blocked,
        scatter_plan=None,
        ...
        movement_failed=lambda source, destination: failures.append(
            (source, destination)
        ),
    )
