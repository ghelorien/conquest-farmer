...
def fixture(hp):
    layout=EntityLayout.model_validate(yaml.safe_load(open('profiles/classic-1074-entities-candidate.yaml')))
    ...
    h=bytearray(24);struct.pack_into('<IIQ',h,8,0,16,table)
    t=bytearray(64);struct.pack_into('<I',t,4,hp<<1)
    ...
    session=SimpleNamespace(assert_identity=lambda:None,read_block=read,
        modules=[{'name':layout.module,'base':0x400000}])
    monster=MonsterObservation(obj,25,'Turtledove',(680,570),(650,400),81,7,type_id=2)
...
def test_camera_scroll_does_not_invalidate_actor_health():
    ...
    struct.pack_into('<ii',blocks[monster.object_address],layout.draw_position_offset,750,450)
    original=session.read_block
    def read(address,size):
        value=original(address,size)
        if address==0x300000:
            struct.pack_into('<ii',blocks[monster.object_address],layout.draw_position_offset,850,500)
        return value
    session.read_block=read
...
def test_actor_identity_or_world_movement_still_invalidates_health(field):
    ...
    def read(address,size):
        value=original(address,size)
        if address==0x300000:
            struct.pack_into('<I',blocks[monster.object_address],getattr(layout,field),999)
        return value
    session.read_block=read
    with pytest.raises(ValueError,match='changed'):
        read_monster_health(session,layout,monster)
