...
@pytest.mark.skipif(sys.platform != 'win32', reason='Native workers use Windows file locks')
def test_managed_launcher_and_background_components_leave_release_unchanged(tmp_path):
    ...
    env = {key: value for key, value in os.environ.items() if not key.startswith('CONQUEST_')}
    env.update(PYTHONPATH=str(REPO/'src'), PYTHONDONTWRITEBYTECODE='1', PYTHONIOENCODING='utf-8')
    result = subprocess.run([sys.executable, '-B', str(Path(__file__).resolve()),
                             str(release), str(data)], cwd=release, env=env,
...
def _managed_probe(release, data):
    ...
    def main():
        ...
        def finish_tick(*_): raise EndTick()
        ...
        assert read_json(state_path('reports/merchants/market-guard.json'))['running'] is True
        ...
        with patch.object(alerts, 'request', return_value={'characters': {}}), \
        ...
        import queue
        resized = []
        app = NS(control=NS(snapshot=lambda: {'enabled': False, 'revision': 1}),
                 host=NS(api=NS(), resize=lambda *size: resized.append(size)),
                 pane=NS(winfo_width=lambda: 800, winfo_height=lambda: 600))
        ui = NS(app=app, ui_requests=queue.Queue(), runtime=runtime,
                safe_to_yield=lambda: True, coordinator=coordinator)
        ...
        with patch('conquest.worker.request', return_value={'read_only': True}) as health, \
                patch('subprocess.Popen', side_effect=AssertionError('Must reuse existing diagnostic')):
            result = UnifiedUI.dispatch(ui, {'action': 'start-account-diagnostic', 'profile_id': merchant.id})
        assert result == {'existing': True, 'read_only': True}
        assert health.call_args.args == (diagnostic, 'health')
        assert notification_health.describe() == 'Discord farmer: not configured | Discord shops: not configured'
        ...
        with patch.object(notification_health, 'process_alive', return_value=True):
            assert notification_health.service('.runtime/discord-webhook.dpapi',
                'reports/discord-status.json', now=read_json(discord_notify.STATUS)['updated_at']) == 'monitor running'
        ...
        from conquest.merchants import cancel_reserved_request as reserved, empty_delivery_cancel as empty
        from conquest.merchants.journal import Journal
        class InlineThread:
            def __init__(self, *, target, **kwargs): self.target = target
            def start(self): self.target()
            def is_alive(self): return False
        def no_input(*args, **kwargs): raise ValueError('offline test: no input')
        ...
        write_json(state_path('reports/banking/merchant-route.json'), {'active': {'request_id': 'test-request'}})
        with patch('threading.Thread', InlineThread), patch.object(reserved, 'pair', return_value=({}, {})), \
                patch.object(reserved, 'unchanged'), patch.object(reserved, 'run', side_effect=no_input) as cancel:
            reserved.start(ui)
        ...
        with patch('threading.Thread', InlineThread), patch.object(empty, 'pair', return_value=({}, {'trade': True})), \
                patch.object(empty, 'unchanged'), patch.object(empty, 'run', side_effect=no_input):
            empty.start(ui, merchant.id)
        assert read_json(shared/'reports/merchants/empty-delivery-cancel.json')['error'] == 'offline test: no input'
        ...
        with patch.dict(relist.__globals__, verify_rollout=lambda: None, run_cycle=lambda: {'test': True}), \
                patch.object(sys, 'argv', ['merchant_relist.py', '--watch']), \
                patch('time.sleep', side_effect=KeyboardInterrupt):
            relist()
        assert read_json(shared/'reports/merchants/script-worker.json')['test'] is True
        ...
        def offline_read(path, *args, **kwargs):
            if str(path) == r'C:\Program Files\Classic Conquer 2.0\ini\itemtype.json': return '{}'
            return original_read(path, *args, **kwargs)
        with patch.dict(scan.__globals__, browser_pages=lambda *_: {'test': True}, request=lambda _: {}), \
                patch.object(sys, 'argv', ['merchant_scan.py', '--browser-pages', str(pages)]), \
