...
def run(points,spacing=3):
    now = [0.]
    samples = iter(points)
    return record_route(lambda:next(samples),len(points),interval=1,spacing=spacing,
...
def test_stationary_samples_are_kept_as_evidence_without_duplicate_waypoints():
    result = run([(1002,(430,380)),(1002,(430,380)),(1002,(432,380)),(1002,(434,380)),(1002,(435,380))])
...
def test_map_changes_and_teleports_cannot_create_false_connected_routes():
    ...
    assert result["reason"] == "position_discontinuity" and result["route"] == [[430,380]]
