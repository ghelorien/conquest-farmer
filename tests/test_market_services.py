...
def test_dialog_choice_uses_exact_memory_records_and_table(monkeypatch):
    records = [
        {"kind": 0, "option": 0, "text": "Pack Meteors?"},
        {"kind": 1, "option": 1, "text": "Pack ten Meteors"},
        {"kind": 1, "option": 2, "text": "Leave"},
    ]
    d = {
        "records": records,
        "window": NS(position=(20.0, 30.0), size=(280.0, 160.0), scroll=(0.0, 0.0)),
        "table": (40.0, 100.0, 260.0, 22.0),
    }
    monkeypatch.setattr(m, "read_dialog", lambda observer: d)
    assert m.dialog_point(None, "Pack ten Meteors", records) == (95, 111)
    with pytest.raises(ValueError, match="changed"):
        m.dialog_point(None, "Pack ten Meteors", [])
    with pytest.raises(ValueError, match="absent"):
        m.dialog_point(None, "Unpack", records)
    d["table"] = (40.0, 100.0, 310.0, 22.0)
    with pytest.raises(ValueError, match="layout"):
        m.dialog_point(None, "Pack ten Meteors", records)


def test_resized_phoenix_dialog_uses_actual_equal_columns(monkeypatch):
    records = [
        {"kind": 1, "option": i, "text": text}
        for i, text in enumerate(("Twin City", "Market", "Just passing by."))
    ]
    d = {
        "records": records,
        "window": NS(position=(378.0, 20.0), size=(356.0, 162.0), scroll=(0.0, 0.0)),
        "table": (398.0, 130.0, 714.0, 44.0),
    }
    monkeypatch.setattr(m, "read_dialog", lambda observer: d)
    assert m.dialog_point(None, "Market", records) == (635, 141)
    assert m.dialog_point(None, "Just passing by.", records) == (477, 163)
    d["table"] = (398.0, 130.0, float("nan"), 44.0)
    with pytest.raises(ValueError, match="layout"):
        m.dialog_point(None, "Market", records)
...
def test_scrolled_packing_choices_must_be_fully_inside_dialog(monkeypatch):
    ...
    window = NS(position=(378.0, 20.0), size=(280.0, 198.0), scroll=(0.0, 46.0))
    data = {"records": records, "window": window, "table": (398.0, 188.0, 618.0, 22.0)}
    monkeypatch.setattr(m, "read_dialog", lambda observer: data)
    assert m.dialog_point(None, "Pack Meteors", records) == (453, 199)
    data["table"] = (398.0, 208.0, 618.0, 22.0)
    with pytest.raises(ValueError, match="layout"):
        m.dialog_point(None, "Pack Meteors", records)
...
def test_ten_meteor_exchange_rejects_partial_or_unrelated_item_loss():
    ...
    meteors = [
        dict(uid=i, type_id=1088001, amount=1, limit=1, plus=0) for i in range(10)
    ]
...
def test_service_discovery_tolerates_unrelated_scene_reordering(monkeypatch):
    layout = NS(
        ...
        max_objects=8,
        monster_vtable_rva=0x100,
        name_offset=0xA4,
        position_offset=0xE8,
        id_offset=0x78,
        kind_offset=0x80,
        draw_position_offset=0xF8,
    )
    base, collection, entries, a, b = 0x100000, 0x200000, 0x250000, 0x300000, 0x400000
    values = {
        collection: entries,
        collection + 8: entries + 32,
        collection + 16: entries + 32,
        a: base + 0x100,
        b: base + 0x100,
        a + 0x84: 87,
        b + 0x84: 9999,
    ...
    entities = NS(
        session=NS(assert_identity=lambda: None),
        layout=layout,
    ...
    with pytest.raises(ValueError, match="memory-identified"):
        m.discover(entities, 1036, "Warehouseman")
...
def test_unqualified_exchange_and_unfinished_batch_never_withdraw(monkeypatch):
    ...
    stored = {
        "items": [dict(uid=i, type_id=1088001, amount=1, limit=1) for i in range(10)]
    }
    monkeypatch.setattr(
        banking, "read_json", lambda path: {"enabled": False, "qualified": False}
    )
    ...
    monkeypatch.setattr(
        ...
        "read_json",
        lambda path: (
            {"enabled": True, "qualified": True, "exchange": {"fee": 0}}
            if path == banking.POLICY
            else {"phase": "exchange_pending"}
            if path == banking.JOURNAL
            else {}
        ),
    )
    with pytest.raises(ValueError, match="reconciliation"):
        banking.consolidate(None, stored)


@pytest.mark.parametrize(
    "type_id,plus", [(1088001, 0), (720027, 0), (1088000, 0), (150009, 0), (150006, 2)]
)
def test_market_return_refuses_carried_valuables_before_movement(type_id, plus):
    from conquest.meteor_banking import trip

    loop = NS(
        living=lambda: {"embedded_controls": {"life": {"map_id": 1036}}},
        town=lambda action: {
            "items": [{"uid": 42, "type_id": type_id, "plus": plus, "slot": 1}]
        },
    )
