...
def health():
    return {
        "target": {"pid": 1, "creation_time_100ns": 2},
        "embedded_controls": {
            "observations_available": True,
            "observed_at": s.time.time(),
            "monsters": [],
...
@pytest.mark.parametrize(
    "bad", ["nearby", "unknown_life", "stale", "injured", "dead", "mouse", "missing"]
)
def test_uncertain_or_unsafe_spot_never_qualifies(bad):
    h = health()
    d = h["embedded_controls"]
    if bad in ("nearby", "unknown_life"):
        d["monsters"] = [
            {"position": [110, 355], "alive": True if bad == "nearby" else None}
        ]
    if bad == "stale":
        d["observed_at"] -= 2
    if bad == "injured":
        d["life"]["current_hp"] = 100
    if bad == "dead":
        d["life"]["dead_candidate"] = True
    if bad == "mouse":
        d["manual_mouse"] = True
    if bad == "missing":
        d["observations_available"] = False
...
def test_handoff_rechecks_damage_and_new_monsters_before_release(monkeypatch):
    ...
    proof = {
        ...
        "position": [100, 350],
        "hp": 600,
        "verified_at": s.time.time(),
    }
    monkeypatch.setattr("conquest.worker.request", lambda *a: h)
    s.validate_handoff("worker", proof)
    life["current_hp"] = 599
    with pytest.raises(ValueError, match="Safe spot changed"):
        s.validate_handoff("worker", proof)
    life["current_hp"] = 600
    h["embedded_controls"]["monsters"] = [{"position": [101, 350]}]
...
def test_parking_requires_quiet_interval_and_keeps_care_running(monkeypatch):
    ...
    loop = NS(
        living=lambda: h,
        care=NS(check=lambda h: checks.append(now[0])),
...
def test_deployment_parking_reaches_verified_city_before_quiet_interval(
    monkeypatch, vendor_anchor
):
    now = [0.0]
    moves = []
    checks = []
    ...
    monkeypatch.setattr(
        "conquest.city_travel.service_role",
        lambda map_id, point: (
            3 if vendor_anchor and tuple(point) == (100, 350) else None
        ),
    )
    monkeypatch.setattr(
        "conquest.navigation.read_terrain",
        lambda *a: NS(map_id=1002, walkable=lambda point: True),
    )

    def travel(destination, **kwargs):
        moves.append((now[0], destination))
        life["position"] = list(destination)

    loop = NS(
        living=lambda: h,
        care=NS(check=lambda h: checks.append(now[0])),
        terrain=NS(map_id=1002),
        route=NS(restock_map_id=1002, restock_anchor=(100, 350)),
        travel=travel,
    )
    proof = s.park(loop, threading.Event(), lambda note: None, require_city=True)
    expected = (100, 349) if vendor_anchor else (100, 350)
    assert moves == [(0.0, expected)]
    assert now[0] >= 3 and len(checks) >= 15
    assert proof["position"] == list(expected) and proof["city_required"] is True
    assert (
        proof["city_anchor"] == list(expected)
        and proof["city_terrain_sha256"] == "verified-terrain"
    )
...
def test_deployment_parking_rejects_unknown_or_other_map(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda map_id: (_ for _ in ()).throw(ValueError("unknown")),
    )
...
def test_settings_label_error_does_not_starve_control_poll(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (_ for _ in ()).throw(TypeError("old schema")),
    )
    app = NS(
        session_note=NS(set=lambda text: None),
        _poll=lambda: calls.append("controls") or True,
        root=NS(after=lambda *args: calls.append("scheduled")),
        poll=lambda: None,
    )
    DesktopApp.poll(app)
...
def test_new_app_resumes_only_the_same_client_and_consumes_handoff(
    ...
    monkeypatch.setattr(s, "RESUME", tmp_path / "resume.json")
    h = health()
    calls = []
    monkeypatch.setattr("conquest.worker.request", lambda *a: h)
    s.write_json(
        s.RESUME,
        {
            ...
            "target": h["target"],
            "resume": True,
            "expires_at": s.time.time() + 60,
        },
    )
    app = NS(
        last={"worker_info_path": "worker"},
        runtime=True,
        update_control=lambda body: calls.append(body),
    )
    s.resume_after_embed(app)
    s.resume_after_embed(app)
...
def test_local_escape_moves_away_using_walkable_tiles():
    ...
    terrain = TerrainMap(
        1002, 100, 100, np.zeros((100, 100), dtype=bool), "0" * 64, (), ()
    )
...
def test_cancel_never_starts_safe_spot_movement():
    ...
    with pytest.raises(ValueError, match="canceled"):
        s.park(loop, cancelled, lambda note: None)
...
def test_dense_local_search_falls_back_to_fixed_town_route_with_care(monkeypatch):
    ...
    monkeypatch.setattr(
        "conquest.scene_input.memory_player_anchor", lambda *a: (710, 504)
    )

    def living():
        x, y = life["position"]
        h["embedded_controls"]["monsters"] = (
            [] if (x, y) == (10, 10) else [{"position": [x + 1, y], "alive": True}]
        )
        return h

    def step(target, **kw):
        moves.append((tuple(life["position"]), target))
        life["position"] = list(target)
        return {"reached": True}

    loop = NS(
        living=living,
        care=NS(check=lambda h: checks.append(now[0]), session=None),
        terrain=TerrainMap(
            1002, 100, 100, np.zeros((100, 100), dtype=bool), "", (), ()
        ),
        route=NS(restock_map_id=1002, restock_anchor=(10, 10)),
        stepper=NS(step_to=step),
    )
    proof = s.park(loop, threading.Event(), notes.append)
    assert proof["position"] == [10, 10] and 18 <= now[0] < 120
    assert len(checks) > len(moves) > 1
    assert all(
        max(abs(x - 10), abs(y - 10)) > max(abs(a - 10), abs(b - 10))
        for (x, y), (a, b) in moves
    )
...
def test_final_safe_check_failure_preserves_old_app(monkeypatch):
    ...
    app = NS(
        ...
        reload_proof={},
        reload_resume=True,
        release=lambda: pytest.fail("Must retain client"),
        state_text=NS(set=lambda note: calls.append(note)),
    )
    monkeypatch.setattr(
        "conquest.desktop_app.subprocess.run", lambda *a, **k: NS(returncode=0)
    )
    monkeypatch.setattr(
        "conquest.safe_reload.validate_handoff",
        lambda *a: (_ for _ in ()).throw(ValueError("monster approached")),
    )
...
def test_terminal_unqueryable_pid_still_requires_exclusive_controller_lock(
    monkeypatch, tmp_path, lock_available
):
    from contextlib import contextmanager
    ...
    def guard():
        calls.append("lock")
        yield lock_available

    monkeypatch.setattr("conquest.route_controller.controller_guard", guard)
    loop = NS(
        refresh=lambda: None, record=lambda *a, **k: None, check_stop=lambda: None
    )
    monkeypatch.setattr("conquest.overnight.OvernightLoop", lambda route: loop)
    monkeypatch.setattr(
        s, "park", lambda *a, **kw: calls.append(("park", kw)) or {"verified": True}
    )
    if lock_available:
        assert s.prepare("worker", "bandit", threading.Event(), lambda text: None) == {
            "verified": True
        }
