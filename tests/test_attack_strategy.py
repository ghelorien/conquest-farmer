...
def damaging_cast(strategy,before,after,now):
    ...
    strategy.observe([after],now+.8)


def test_surviving_third_damaging_cast_selects_left_and_persists(tmp_path):
    path=tmp_path/'decision.json';events=[]
    s=AttackStrategy(path,lambda *args:events.append(args));s.set_context('level27-bow')
    ...
    restored=AttackStrategy(path);restored.set_context('level27-bow')
...
def test_misses_missing_targets_recycled_ids_and_early_samples_do_not_count():
    s=AttackStrategy();s.set_context('gear')
    for index in range(6):
        s.issued(monster(),'right',index)
        s.observe([monster(50,address=2000)],index+.8)
        s.observe([],index+.9)
        s.observe([monster()],index+1.)
...
def test_kill_or_left_click_cannot_count_as_survived_scatter():
    ...
    for i in range(4):
        ...
        s.issued(monster(),'left',i+.1)
        s.observe([monster(20)],i+.9)
...
def test_gear_identity_ignores_consumption_but_detects_plus_gems_and_level():
    state={'level':27,'profession':41,'equipment':{
        'bow':{'type_id':500035,'uid':100,'plus':0,'gem1':0,'gem2':0,'attack_max':30},
    ...
    assert context()!=before
    upgraded=context();state['level']=28
    assert context()!=upgraded


def test_arrow_tier_change_retests_scatter_but_stack_consumption_does_not():
    state={'level':32,'profession':41,'equipment':{'arrows':{'type_id':1050000,'attack_min':10,'attack_max':10,'uid':1,'amount':10}}}
...
def test_group_size_excludes_dead_far_and_duplicate_entities():
    ...
    near=monster();second=monster(uid=11,address=2000)
