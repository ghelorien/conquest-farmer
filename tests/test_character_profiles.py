...
from conquest.profile_editor import (
    OPEN_SELECTED_CHARACTER,
    profile_choice_rows,
    selected_profile_id,
    new_character_role,
    open_on_profile_double_click,
...
def test_character_and_account_state_are_independent(tmp_path, monkeypatch):
    ...
    old = state_path(".runtime/native-controls.json")
    activate(monkeypatch, r, b)
    assert (
        old != state_path(".runtime/native-controls.json")
        and farmer_name() == "FreshArcher"
    )
    assert Path(state_path(".runtime/account.dpapi")) == cb.credentials
    assert (
        Path(state_path(".runtime/merchant-input.lock"))
        == tmp_path / "locks/input.lock"
    )


def test_profile_chooser_keeps_character_rows_visible_and_preselects_an_openable_choice(
    tmp_path,
):
    ...
    assert rows == [
        (parasite.id, ("Parasite", "America", "Farmer")),
    ...
    assert (
        selected_profile_id(rows, preferred="missing", previous=merchant.id)
        == merchant.id
    )
...
def test_profile_quick_actions_use_an_explicit_role_and_safe_double_click_and_toggle_wiring(
    tmp_path,
):
    registry = ProfileRegistry(tmp_path)
    registry.add("Existing seller", role="Merchant")
    ...
    section = Mock()
    button = Mock()
    section.winfo_manager.side_effect = (True, False)
...
def test_labels_do_not_key_records_or_credentials(tmp_path, monkeypatch):
    ...
    name = resolve_merchant(p.id)
    journal.set(name, "total", 90210)
    journal.begin("intent", name, "listing", {"uid": 123})
...
def test_duplicate_names_across_servers_require_bridge_id(tmp_path, monkeypatch):
    r = ProfileRegistry(tmp_path)
    a = r.add("Seller", role="Merchant")
    r.add("Seller", "Europe", "Merchant")
    activate(monkeypatch, r, a)
    from conquest.portable_ui import normalize_command

    with pytest.raises(ValueError):
        normalize_command({"action": "pause", "character": "Seller"})
    command = normalize_command({"action": "pause", "profile_id": a.id})
    assert command["character"].profile_id == a.id
    with pytest.raises(ValueError):
        r.add("seller", role="Farmer")
    with pytest.raises(ValueError):
        normalize_command({"action": "pause", "profile_id": "Seller"})


def test_owned_roster_and_explicit_trust(tmp_path, monkeypatch):
    r = ProfileRegistry(tmp_path)
    farmer = r.add("NewArcher")
    seller = r.add("MyShop", role="Merchant")
    r.add("OtherShop", role="Merchant", local_enabled=False)
    activate(monkeypatch, r, farmer)
    ...
    r.update(
        ...
        {
            "trusted_sources": [
                {"name": farmer.name, "server": "America", "character_uid": 42}
            ]
        },
        stopped=True,
        pending=False,
...
def test_binding_rejects_wrong_character_server_and_uid(tmp_path):
    r = ProfileRegistry(tmp_path)
    p = r.add("Archer")
    for name, server, uid in [
        ("Other", "America", 11),
        ("Archer", "Europe", 11),
        ("Archer", "America", 0),
    ]:
        with pytest.raises(ValueError):
            r.bind(p.id, name, server, uid)
    r.bind(p.id, "Archer", "America", 11)
    with pytest.raises(ValueError):
        r.bind(p.id, "Archer", "America", 12)
    with pytest.raises(ValueError):
        context_for(p.id, tmp_path).verify(
            {"character": "Archer", "server": "America", "character_uid": 12}
        )
...
def test_exports_reject_non_preferences(tmp_path, forbidden):
    r = ProfileRegistry(tmp_path)
    with pytest.raises(ValueError):
        r.add("Archer", overrides={forbidden: 1})


def test_export_import_new_identity_no_secrets_or_trust(tmp_path):
    r = ProfileRegistry(tmp_path / "a")
    p = r.add(
        "Old",
        overrides={"heal_below": 0.55},
    ...
    other = ProfileRegistry(tmp_path / "b")
    new = other.import_preferences(payload, "New")
    assert new.id != p.id and new.account_id != p.account_id
    assert new.character_uid is None and not new.trusted_sources
    assert new.overrides == {"heal_below": 0.55}
    assert not context_for(new.id, other.root).credentials.exists()
    with pytest.raises(ValueError):
        other.import_preferences({**payload, "secret": "no"}, "Third")


def test_template_overrides_only_explicit_settings(tmp_path):
    r = ProfileRegistry(tmp_path)
    key = r.save_template("Archer defaults", {"heal_below": 0.4, "jump_scatter": False})
    a = r.add("A", template=key, overrides={"heal_below": 0.6})
    b = r.add("B", template=key)
    assert r.effective(a) == {"heal_below": 0.6, "jump_scatter": False}
    assert r.effective(b) == {"heal_below": 0.4, "jump_scatter": False}
...
def test_role_change_requires_affected_profile_transaction_idle(tmp_path):
    ...
    other = r.add("B")
    path = (
        context_for(p.id, tmp_path).state_dir / "reports/banking/merchant-journey.json"
    )
    other_path = (
...
def test_ui_and_input_roles_do_not_collide_with_character_name(tmp_path):
    r = ProfileRegistry(tmp_path)
    p = r.add("Farmer", role="Merchant")
    name = ProfileName(p.name, p.id)
    mapping = ProfileMap()
    mapping["Farmer"] = "role"
...
def test_offline_migration_preserves_receipts_and_pause_intent(tmp_path, monkeypatch):
    ...
    old = Journal(source / "reports/merchants/journal.sqlite3")
    old.set("Dutch", "enabled", False)
    old.set("Dutch", "refill_enabled", True)
    old.set("Dutch", "silver_total", 123456)
    old.begin("pending", "Dutch", "listing", {"uid": 7})
    write_json(
        source / ".runtime/native-controls.json", {"enabled": False, "revision": 11}
    )
    write_json(
        source / ".runtime/merchants/dutch/qualification.json",
        {"unsafe": "old geometry"},
    )
    target = tmp_path / "portable"
    result = migrate_legacy(source, target, check_offline=lambda _: None)
    ...
    r = ProfileRegistry(target)
    activate(monkeypatch, r, r.resolve(result["farmer_profile_id"]))
...
def test_interrupted_migration_never_activates_partial_state(tmp_path):
    ...
    source = tmp_path / "old"
    write_json(source / ".runtime/test.json", {"value": 1})
    ...
    with pytest.raises(OSError):
        migrate_legacy(
            source, destination, check_offline=lambda _: None, copy_file=fail
        )
    assert not destination.exists() and (source / ".runtime/test.json").exists()
    assert (
        migrate_legacy(source, destination, check_offline=lambda _: None)["state"]
        == "complete"
    )
...
def test_app_lock_excludes_second_farmer_environment(tmp_path):
    ...
    with app_owner(tmp_path):
        with pytest.raises(ValueError):
            with app_owner(tmp_path, timeout=0):
                pass
    with app_owner(tmp_path, timeout=0):
        pass
...
def test_route_copies_are_saved_to_selected_character(tmp_path, monkeypatch):
    ...
    route = library.all()[0].model_dump()
    ...
    activate(monkeypatch, r, b)
    with pytest.raises(ValueError):
        RouteLibrary().load("private-route")
...
def test_surface_block_prevents_farmer_input_and_releases_lease(tmp_path):
    ...
    guard.surface_blocks["Farmer"] = True
    install(guard)
    try:
        with pytest.raises(CaptureUnavailable):
            with input_scope():
                pytest.fail("Invalid surface must not receive input")
...
def test_range_override_cannot_exceed_engine_capability(tmp_path, monkeypatch):
    ...
    r = ProfileRegistry(tmp_path)
    p = r.add("Fresh", overrides={"attack_range_tiles": 20})
    activate(monkeypatch, r, p)
    config = TrialConfig.model_validate(
        yaml.safe_load(Path("profiles/desktop-foreground.example.yaml").read_text())
    )
