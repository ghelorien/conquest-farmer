...
def fixture(*, fail_neutralize=False, change_identity=False, fail_down=False):
    ...
    class Scope:
        ...
        def __exit__(self, *args): calls.append('exit'); self.close()
        def check(self): pass
        def neutralize(self):
    ...
    def run():
        isolated_ctrl_probe(target, adapter, identity, SimpleNamespace(snapshot=lambda: state),
                            lambda: None, sample, samples, report, scope_factory=Scope)
...
def test_normal_release_stays_before_detach_and_does_not_duplicate_keyup():
    ...
    run()
    assert calls == ['enter', ('post', 0x100), 'neutralize', ('post', 0x101), 'exit', 'close']
    assert report['ctrl_verified'] and report['ctrl_release_verified'] and report['helper_detached']
