...
def incident(*, prior_booth):
    return dict(
        ...
        before={
            **stock(
                99,
                inventory=() if prior_booth else (ITEM,),
...
def live():
    return {
        **stock(102, silver=1450428),
        "profile_id": "Spiritual",
        "character_uid": 7,
        "client_sha256": "1078-test-build",
        "closed_modal": True,
        "map_id": 1036,
        "hp": 100,
        "own_booth_uid": 9,
...
def test_listing_plan_reconciles_prior_spiritual_booth_only_with_atomic_sale(
        ...
        if change in ("cash", "attributes"):
            with journal.db() as db:
                if change == "cash":
                    ...
                    altered = {**ITEM, "plus": 2}
                    db.execute(
                        "UPDATE sales SET items=? WHERE character='Spiritual'",
                        (json.dumps([altered]),),
                    )
    ...
    runtime = SimpleNamespace(
        journal=journal, market_path=Path(tmp_path / "market.json")
    )
    if change == "none":
        assert listing_plan_1078.plan(runtime, "Spiritual", live()) == []
        sold = seen[0]["prior_stock_sold_with_verified_receipts"]
        assert [
            (row["uid"], row["origin"], row["receipt"]["silver"]) for row in sold
...
def test_prior_inventory_sale_requires_native_listing_receipt_chain(tmp_path):
    ...
    snapshot = live()
    state = {
        "shop_return": incident(prior_booth=False),
    ...
    state["verified_listing_receipts_1078"] = [
        {
            ...
            "item": ITEM,
            "observed_at": 99.5,
            "request_id": "booth-list1078-proved",
        }
    ]
    sold = _preview(snapshot, state)["prior_stock_sold_with_verified_receipts"]
    assert [
        (row["uid"], row["origin"], row["listing_receipt"]["request_id"])
        for row in sold
