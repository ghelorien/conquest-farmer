...
def test_each_farmer_has_independent_speed_parameters(tmp_path):
    ...
    assert load_combat_speed("First", tmp_path).jump_arrival_seconds == 0.3
    assert load_combat_speed("Second", tmp_path).jump_arrival_seconds == 0.7
...
def test_receipt_cost_is_isolated_from_other_farmers_and_attack_stock(tmp_path):
    ...
    assert (
        ammunition_per_attack(
            SimpleNamespace(attack_button="right", combat_speed=speed)
        )
        == 3
    )
    with pytest.raises(ValueError):
        CombatSpeed(scatter_receipt_arrows=1)
...
def test_force_jump_scatter_precedes_isolated_and_is_disabled_with_missing_skill(
    ...
    assert (
        scatter_attack_mode(
            config, CombatSpeed(force_jump_scatter=True), strategy, isolated, "Bandit"
        )
        == expected
    )
...
def test_force_jump_scatter_applies_without_adaptive_strategy():
    ...
    assert (
        scatter_attack_mode(
            config, CombatSpeed(force_jump_scatter=True), None, True, "Bandit"
        )
        == "right"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "regional_search_expansion",
        "cluster_lookahead",
        "counter_gap_recovery",
    ],
)
