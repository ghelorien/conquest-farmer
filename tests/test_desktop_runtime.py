...
def test_elevated_start_preserves_profile_and_requested_action(tmp_path):
    ...
    assert args == [
        str(root / "scripts/start_desktop_app.py"),
        "--profile",
        str(profile),
        "--start",
    ]
    ...
    assert (
        start_arguments(root, profile, False, launch_client=True)[-1]
        == "--launch-client"
    )
...
def test_embed_restart_preserves_exact_process_creation_and_hwnd(tmp_path):
    args = start_arguments(
        tmp_path, tmp_path / "p.yaml", False, embed_client=(42, 987654321, 12345)
    )
    assert args[3:] == [
        "--embed-client",
        "--client-pid",
    ...
    with pytest.raises(ValueError):
        start_arguments(
            tmp_path, tmp_path / "p.yaml", False, embed_client=(42, 0, 12345)
        )
    with pytest.raises(ValueError):
        start_arguments(
            tmp_path,
...
def test_scaled_input_keeps_physical_origin_and_guard():
    ...
    session.operations = SimpleNamespace(
        dispatch=lambda op, body: calls.append((op, body))
    )
    body = {
        "point": [826, 751],
        "expected_size": [1536, 793],
        "expected_origin": [0, 29],
        "guard": {"name": "Parasite"},
...
class Host:
    ...
    def assert_owner(self, *args):
        pass

    def owns_window(self, *args):
        return True
...
def test_show_game_prepares_current_farmer_pane_and_uses_verified_activation(
    monkeypatch,
):
    calls = []
    identity = {"pid": 42, "creation_time_100ns": 123}
    saved = WindowState(identity, 20, 0, 0, 0, ())
    host = SimpleNamespace(saved=saved, api=object())
    pane = SimpleNamespace(
        winfo_ismapped=lambda: True, winfo_width=lambda: 1416, winfo_height=lambda: 1016
    )
    ...
    app = SimpleNamespace(
        host=host,
        client=(42, 20, identity),
        unified=SimpleNamespace(notebook=notebook, frames={"Farmer": "farmer"}),
        root=root,
        pane=pane,
    ...
    def activate(hwnd, current, *, api, focus):
        calls.append(("activate", hwnd, current, api))
        return focus()
...
def test_show_game_rejects_replaced_host_before_any_activation(monkeypatch):
    ...
    app = SimpleNamespace(
        host=SimpleNamespace(
            saved=WindowState({"pid": 99, "creation_time_100ns": 456}, 20, 0, 0, 0, ()),
            api=object(),
        ),
        client=(42, 20, identity),
        state_text=SimpleNamespace(set=lambda value: calls.append(value)),
    )
    monkeypatch.setattr(
        "conquest.desktop_app.activate_focused_client",
...
def test_show_game_denied_input_cannot_change_tab_or_host_geometry(monkeypatch):
    ...
    app = SimpleNamespace(
        ...
        unified=SimpleNamespace(
            notebook=SimpleNamespace(select=lambda frame: calls.append("tab")),
        ...
        pane=SimpleNamespace(
            ...
            winfo_width=lambda: 1416,
            winfo_height=lambda: 1016,
        ),
...
def focus_api(focused=40, foreground=10):
    ...
    api.attach_input = lambda first, second, attach: (
        calls.append(("attach", attach)) or True
    )
    ...
    api.gui = SimpleNamespace(
        GetAncestor=lambda *args: 10,
        GetForegroundWindow=lambda: foreground,
        IsChild=lambda parent, child: parent == 20 and child == 21,
        SetFocus=set_focus,
    )
...
def test_reload_releases_client_before_starting_pinned_replacement(
    monkeypatch, tmp_path, release_ok
):
    calls = []
    ...
    app = SimpleNamespace(
        ...
        release=release,
        host=SimpleNamespace(
            saved=WindowState({"pid": 42, "creation_time_100ns": 123}, 99, 0, 0, 0, ())
        ),
        root=SimpleNamespace(destroy=lambda: calls.append("destroy")),
        state_text=SimpleNamespace(set=lambda text: calls.append(text)),
    )
    monkeypatch.setattr(
        "conquest.desktop_app.subprocess.Popen",
        lambda args, **kwargs: calls.append(args),
    )
    monkeypatch.setattr(
        "conquest.desktop_app.subprocess.run",
        lambda *args, **kwargs: SimpleNamespace(returncode=0),
    )
    app.last = {"worker_info_path": "test"}
    app.reload_proof = {}
    app.reload_resume = True
    monkeypatch.setattr(
        "conquest.safe_reload.validate_handoff", lambda *args: calls.append("safe")
    )
...
def test_reload_invalid_new_source_preserves_running_app_and_client(monkeypatch):
    ...
    assert len(calls) == 1 and calls[0].startswith(
        "Reload canceled: startup check failed"
    )
