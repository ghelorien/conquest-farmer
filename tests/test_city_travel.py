...
def visit(monkeypatch,tmp_path):
    ...
    city=dict(map_id=1011,name='PhoenixCity',town_anchor=[210,195],town_boundary=[111,111,254,258],terrain_sha256='abc')
    ...
    health={'target':{'pid':1},'embedded_controls':{'observed_at':10,'life':dict(object_address=99,
        map_id=1011,position=[11,376],dead_candidate=False)}}
    calls=[]
    def travel(point,**kwargs):
        calls.append(('travel',point));health['embedded_controls']['life']['position']=list(point)
    loop=SimpleNamespace(route=SimpleNamespace(id='bandit'),living=lambda:health,phase='hunting',stop_farm=lambda:calls.append(('stop',)),
        record=lambda *a,**kw:None,travel=travel)
    return loop,health,calls


def test_explicit_prepared_hunt_skips_initial_visit_but_not_new_city_arrival(visit,monkeypatch):
    loop,health,calls=visit
    monkeypatch.setattr('conquest.session_plan.active_plan',lambda:{'route_id':'bandit',
...
def test_process_restart_rechecks_town_arrival(visit):
    loop,health,calls=visit
    c.ensure_city_visit(loop);health['target']={'pid':2}
    assert c.ensure_city_visit(loop)
    assert len(calls)==3
...
def test_town_arrival_needs_fresh_living_identity_and_town_position(visit,change):
    ...
    def travel(*args,**kwargs):
        life=health['embedded_controls']['life'];life['position']=[210,195]
        if change=='map':life['map_id']=1002
        if change=='dead':life['dead_candidate']=True
        if change=='stale':health['embedded_controls']['observed_at']=8
        if change=='outside':life['position']=[317,77]
        if change=='actor':life['object_address']=100
    loop.travel=travel
    with pytest.raises(ValueError,match='not confirmed'):c.ensure_city_visit(loop)
