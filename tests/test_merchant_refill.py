...
def setup(tmp_path):
    ...
    runtime = MerchantRuntime(
        catalog, guard, journal=j, market_path=tmp_path / "missing-market.json"
    )
    ...
    history.remember(
        MarketSnapshot(
            dict(
                ...
                total=2,
                listings=rows,
                equipment_categories={"130": "Trojan Armor", "120": "Necklace"},
            ),
            now=100,
        )
    )
    state = dict(
        ...
        character_uid=123,
        own_booth_uid=123,
        identity={"pid": 1, "creation_time_100ns": 2, "path": "C:/Game/ImConquer.exe"},
        timestamp=time.time(),
        map_id=1036,
        position=[264, 214],
        server="America",
        ...
        capacity=40,
        silver=100,
        inventory=[stock(1), stock(2, 120005)],
        booth=[],
        booth_open=True,
        request=None,
        trade=None,
    ...
    def read():
        return {**copy.deepcopy(state), "timestamp": time.time()}

    calls = []

    def post(before, item, price, check):
        check()
        calls.append((item["uid"], price))
        state["inventory"] = [i for i in state["inventory"] if i["uid"] != item["uid"]]
        state["booth"].append({**item, "price": price})

    driver = SimpleNamespace(
        read=read,
        require_qualified=lambda c: None,
        prepare_listing=lambda *args: None,
        list_item=post,
        wait_for=lambda predicate, check: read() if predicate(read()) else None,
    )
    ...
    now = [1000]
    refill = RefillSchedule("Dutch", j, clock=lambda: now[0])
    runtime.refills["Dutch"] = refill
    j.set(
        "Dutch",
        "refill",
        {
    ...
    def mark():
        j.set(
            ...
            "stock_marker",
            {
                "inventory": [i["uid"] for i in state["inventory"]],
...
def test_refill_posts_while_operations_remain_paused(setup):
    ...
    assert not x.runtime.input_allowed(
        "Dutch"
    )  # Permission exists only inside a refill operation.
    x.runtime.step("Dutch")
...
def test_refined_inventory_refills_from_normal_history_while_paused(setup):
    x = setup
    x.runtime.enable("Dutch", False)
    x.state["inventory"] = [stock(1, 130806), stock(2, 120006)]
    x.mark()
    x.runtime.step("Dutch")
    assert x.calls == [(2, 900000), (1, 130000)]
...
def test_explicit_refill_pause_and_global_stop_persist(setup):
    ...
    x.guard.resume()
    assert not x.runtime.refill_enabled("Dutch") and not x.runtime.refill_enabled(
        "Spiritual"
    )
    x.runtime.step("Dutch")
    assert not x.calls
    restarted = MerchantRuntime(
        SimpleNamespace(identities=lambda: []), InputCoordinator(), journal=x.j
    )
...
def test_refill_never_accepts_incoming_trades_while_operations_paused(setup, field):
    x = setup
    x.runtime.enable("Dutch", False)
    x.state[field] = {"participant": "Parasite"}
    x.runtime.controllers["Dutch"].accept_request = lambda *args: pytest.fail(
        "Trading is paused"
    )
    x.runtime.controllers["Dutch"].accept_delivery = lambda *args: pytest.fail(
        "Trading is paused"
    )
    x.runtime.step("Dutch")
...
def test_refill_controller_rejects_trades_and_reprices(setup):
    ...
    x = setup
    driver = x.runtime.controllers["Dutch"].driver
    c = RefillController("Dutch", x.j, driver, x.guard)
    with pytest.raises(ValueError, match="cannot reprice"):
        c.apply_price({"old_price": 100, "price": 99})
...
def test_farmer_handoff_wait_remains_pending_then_replans(setup):
    x = setup
    x.guard.safe_to_yield = lambda: False
    with pytest.raises(CaptureUnavailable):
        x.runtime.step("Dutch")
    ...
    x.guard.safe_to_yield = lambda: True
    x.runtime.step("Dutch")
...
def test_socket_mismatch_and_unknown_type_stay_deferred(setup):
    x = setup
    x.state["inventory"] = [stock(1, gem1=255), stock(2, 999999)]
    x.mark()
    x.runtime.step("Dutch")
...
def test_owned_shop_price_is_matched_without_repeated_undercutting(setup):
    x = setup
    x.state["inventory"] = [stock(1)]
    x.state["booth"] = [stock(50, price=100000)]
    x.mark()
    x.runtime.step("Dutch")
...
def test_refill_does_not_consume_pending_reprice_request(setup):
    x = setup
    x.j.request_scan("Dutch", "scheduled:one")
    x.runtime.step("Dutch")
...
def test_delivery_remainder_only_refills_and_preserves_one_time_scan(setup):
    x = setup
    x.runtime.refill_window = "delivery"
...
def test_unrelated_request_waits_for_approval_before_historical_held_stock_mode(
    setup, monkeypatch
):
    x = setup
    incident = {"phase": "needs_attention", "note": "Preserved historical incident"}
    x.j.set("Dutch", "shop_return", incident)
    x.state.update(
        request={
            "participant": "Stranger",
            "participant_uid": 44,
            "message": "Stranger wishes to trade with you.",
        },
        own_booth_uid=123,
    )
    x.runtime.returns["Dutch"] = SimpleNamespace(
        state=lambda: copy.deepcopy(incident),
        remember=lambda snapshot: None,
        step=lambda *a: pytest.fail("Historical return must remain stopped"),
    )
    x.runtime.controllers["Dutch"].driver.memory = SimpleNamespace(
        read=lambda recovery: x.read()
    )
    calls = []
    monkeypatch.setattr(
        "conquest.merchants.held_stock_refill.allowed",
...
def test_historical_request_decline_preserves_explicit_input_blocks(
    ...
    x.runtime.returns["Dutch"] = SimpleNamespace(
        state=lambda: copy.deepcopy(incident), remember=lambda snapshot: None
    )
    x.runtime.controllers["Dutch"].driver.memory = SimpleNamespace(
        read=lambda recovery: x.read()
    )
...
def test_delivery_reservation_blocks_unrelated_decline(setup, monkeypatch):
    ...
    monkeypatch.setattr(
        delivery_reservation, "active", lambda *a: {"request_id": "reserved"}
    )
    monkeypatch.setattr(
        "conquest.merchants.unrelated_request.decline_unrelated_request",
...
def test_preplanned_listing_cannot_take_focus_during_dedicated_window(
    setup, monkeypatch, window
):
    from conquest.merchants import delivery_reservation
    ...
    monkeypatch.setattr(
        delivery_reservation, "active", lambda *a: {"request_id": "reserved-delivery"}
    )
    focus = []
...
def test_delivery_permission_is_only_for_reserved_trade(setup, monkeypatch, purpose):
    ...
    x = setup
    x.runtime.delivery_window = "reserved-delivery"
    monkeypatch.setattr(
        delivery_reservation, "active", lambda *a: {"request_id": "reserved-delivery"}
    )
    if purpose == "trade":
...
def test_refill_permission_cannot_be_borrowed_by_another_thread(setup):
    ...
    def acquire(character):
        ...
        thread = threading.Thread(
            target=lambda: observed.append(x.runtime.input_allowed(character))
        )
        thread.start()
...
def test_old_timer_migrates_once_and_completion_has_no_catchup(tmp_path):
    ...
    j.set(
        "Dutch",
        "refill",
        dict(next_check=1200, last_checked=900, pending=True, status="checking"),
    )
...
def test_legacy_budget_expiry_is_pending_without_false_completion(tmp_path):
    ...
    timer.start()
    timer.complete("completed", listed=3)
...
def test_refill_at_town_resets_full_interval(tmp_path):
    ...
    now[0] = 1100
    timer.start()
    timer.complete("completed", listed=1)
    assert timer.state()["next_check"] == 2000


@pytest.mark.parametrize(
    "kind", [2000031, 2000032, 2000033, 2000034, 2000035, 2000036, 2000037, 2000038]
)
def test_rare_dragonball_never_refills_or_reprices(setup, kind):
    ...
    x = setup
    x.state["inventory"] = [
        {**stock(1, kind), "name": DRAGONBALL_NAMES[kind], "plus": 0}
    ]
    x.mark()
    x.runtime.step("Dutch")
    assert not x.calls
...
def test_delivery_readiness_distinguishes_handoff_wait_from_transaction_errors(
    setup, note, ready
):
    x = setup
    x.runtime.latest["Dutch"] = x.read()
