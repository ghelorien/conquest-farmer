...
def test_loose_meteors_are_deposited_before_merchant_departure(monkeypatch):
    ...
    def town(action, **fields):
        if action == "supplies":
            return {"items": list(bag)}
        if action == "warehouse-deposit":
            uid = fields["uid"]
            events.append(("deposit", uid))
            bag[:] = [i for i in bag if i["uid"] != uid]
            return {"verified_in_warehouse": True, "uid": uid}
        raise AssertionError(action)

    def start(loop):
        assert bag == [scroll]
        events.append(("merchant", 2))
        bag.clear()
...
def test_production_caller_routes_completed_banked_scroll_with_no_carried_eligible_loot(
    monkeypatch,
):
...
def test_unverified_or_user_resolved_consolidation_does_not_authorize_stored_scroll(
    monkeypatch,
):
    from conquest import meteor_banking
    from conquest.discord_notify import write_json
