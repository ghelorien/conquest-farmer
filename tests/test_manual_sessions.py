...
from conquest.merchants.manual_sessions import (
    BindingMismatch, ManualSessionError, ManualSessionStore, VisitorKey,
    canonical_ownership, ownership_digest,
)
...
def snapshot(at=100, *, request=True, **changes):
    value = dict(identity={'pid': 12, 'creation_time_100ns': 12345,
                          'path': 'C:/Game/ImConquer.exe', 'architecture': 'x64'},
                 character='Spiritual', character_uid=456, server='America',
                 timestamp=at, hp=100, position=[200, 201], map_id=1036,
                 capacity=40, silver=1000, inventory=[item()],
                 booth=[item(101, price=200)], booth_open=True, own_booth_uid=456,
                 request={'participant': 'Visitor', 'participant_uid': 789,
                          'message': 'Visitor wishes to trade with you.'} if request else None,
                 trade=None)
...
def test_allow_is_exact_machine_local_and_atomic(store):
    ...
    for field, value in [('target_profile_id', 'other'), ('visitor_name', 'visitor'),
                         ('visitor_server', 'Europe'), ('visitor_uid', 790)]:
    ...
    with store.db() as db:
        assert not db.execute("SELECT 1 FROM sqlite_master WHERE name='trusted_sources'").fetchone()
    assert ManualSessionStore(store.path.parent / 'other.sqlite3').permissions() == []


@pytest.mark.parametrize('field', ['session_id', 'request_id', 'target_profile_id', 'visitor',
                                  'game_process_identity', 'character_identity',
                                  'request_fingerprint', 'evidence_digest', 'ownership_digest'])
...
@pytest.mark.parametrize('change', ['visitor_name', 'visitor_uid', 'visitor_server', 'process',
                                   'character', 'character_uid', 'quantity', 'bound', 'booth_price',
                                   'silver', 'capacity', 'request_missing', 'trade_open'])
def test_live_approval_rejects_changed_identity_or_ownership(store, change):
    row = pending(store)
    current = snapshot(101)
    if change == 'visitor_name':
        current['request'].update(participant='Another', message='Another wishes to trade with you.')
    ...
    with pytest.raises(ManualSessionError):
        store.allow_and_activate(row['approval_binding'], current, operator='Floor', now=101)
    assert not store.permissions()


def test_activation_repeated_is_idempotent_and_requires_live_request(store):
    row = activate(store)
    result = store.allow_and_activate(row['approval_binding'], snapshot(102), operator='Floor', now=102)
    assert result['id'] == row['id']
    assert len([a for a in store.audit(row['id']) if a['event'] == 'request_approved']) == 1
    with pytest.raises(ManualSessionError):
        store.allow_and_activate(row['approval_binding'], snapshot(103, request=False), operator='Floor', now=103)
...
def test_permission_cannot_activate_wrong_visitor_and_revocation_is_audited(store):
    ...
    row = store.allow_and_activate(row['approval_binding'], snapshot(101), operator='Floor', now=101)
...
def test_decline_intent_and_claim_are_exactly_once_across_restart(store, decision):
    ...
    if decision == 'timeout':
        ...
        row = store.allow_and_activate(row['approval_binding'], snapshot(105), operator='Floor', now=105)
    assert row['request_state'] == 'decline_pending'
    assert not store.permissions()
    with pytest.raises(BindingMismatch):
        store.allow_and_activate(row['approval_binding'], snapshot(106), operator='Floor', now=106)
    ...
    with store.db() as db:
        ...
        assert db.execute('SELECT count(*) FROM manual_decline_claims').fetchone()[0] == 1
...
def test_timeout_does_not_expire_early_and_cannot_race_approval(store):
    ...
    def approve():
        barrier.wait()
        try:
            return store.allow_and_activate(row['approval_binding'], snapshot(110), operator='Floor', now=110)
...
def test_concurrent_allow_and_reject_have_one_winner(store):
    ...
    assert len([a for a in store.audit() if a['event'] in ('request_approved', 'decline_requested')]) == 1
...
@pytest.mark.parametrize('change', ['quantity', 'bound', 'gem1', 'gem2', 'plus', 'uid', 'type_id',
                                   'price', 'silver', 'capacity', 'own_booth_uid', 'booth_open'])
...
def test_two_distinct_observations_at_least_five_seconds_apart(store):
    ...
    for at in (102, 102, 106.999):
        assert store.observe(row['id'], snapshot(at, request=False), now=at)['phase'] == 'settlement_observed'
    assert store.observe(row['id'], snapshot(107, request=False), now=107)['phase'] == 'completed'


def test_new_approved_request_resets_stabilization(store):
    row = activate(store)
    store.observe(row['id'], snapshot(102, request=False), now=102)
    new = store.begin_request('merchant-id', snapshot(106), session_id=row['id'], now=106)
    ...
    assert store.observe(row['id'], snapshot(108, request=False), now=108)['phase'] == 'settlement_observed'
    assert store.observe(row['id'], snapshot(112, request=False), now=112)['phase'] == 'settlement_observed'
    assert store.observe(row['id'], snapshot(113, request=False), now=113)['phase'] == 'completed'
...
def test_same_process_restart_resumes_existing_stabilization(store):
    ...
    store.observe(row['id'], snapshot(102, request=False), now=102)
    restarted = ManualSessionStore(store.path)
    assert restarted.resume(row['id'], snapshot(107, request=False), now=107)['phase'] == 'completed'


@pytest.mark.parametrize('change', ['process', 'character_uid', 'missing_inventory', 'missing_request',
                                   'missing_bound', 'missing_capacity', 'missing_process_birth', 'stale', 'none'])
def test_missing_evidence_and_rollover_stay_needs_attention_across_restart(store, change):
    row = activate(store)
    ...
    assert ManualSessionStore(store.path).observe(row['id'], snapshot(110, request=False), now=110)['phase'] == 'needs_attention'


def test_unapproved_open_trade_never_becomes_manual_authority(store):
    row = pending(store)
    trade = {'participant': 'Visitor', 'participant_uid': 789}
    result = store.observe(row['id'], snapshot(101, request=False, trade=trade), now=101)
    assert result['phase'] == 'needs_attention'
    assert not store.permissions()


def test_unapproved_open_trade_without_session_is_quarantined(store):
    result = store.observe_target('merchant-id', snapshot(100, request=False,
        trade={'participant': 'Visitor', 'participant_uid': 789}), now=100)
    assert result['phase'] == 'needs_attention'
...
def test_approved_open_trade_must_match_exact_visitor(store):
    row = activate(store)
    good = snapshot(102, request=False, trade={'participant': 'Visitor', 'participant_uid': 789})
    assert store.observe(row['id'], good, now=102)['phase'] == 'manual_active'
    bad = snapshot(103, request=False, trade={'participant': 'Visitor', 'participant_uid': 790})
...
def test_withdrawal_requires_unchanged_ownership_and_two_observations(store):
    row = pending(store)
    assert store.observe(row['id'], snapshot(101, request=False), now=101)['phase'] == 'settlement_observed'
    assert store.observe(row['id'], snapshot(106, request=False), now=106)['phase'] == 'request_withdrawn'
...
def test_unapproved_ownership_change_is_not_a_verified_decline(store):
    ...
    assert store.observe(row['id'], snapshot(103, request=False, silver=2000), now=103)['phase'] == 'needs_attention'
...
def test_terminal_audit_and_evidence_are_immutable(store):
    ...
    for statement in ('UPDATE manual_evidence SET snapshot_json=\'{}\'', 'DELETE FROM manual_evidence',
                      "UPDATE manual_audit SET event='changed'", 'DELETE FROM manual_audit',
                      "UPDATE manual_sessions SET phase='manual_active'", 'DELETE FROM manual_sessions',
                      "UPDATE manual_requests SET binding_json='{}'", 'DELETE FROM manual_requests'):
...
def test_booth_stock_does_not_consume_bag_capacity():
    current = snapshot(request=False, inventory=[item(uid) for uid in range(1, 31)],
                       booth=[item(uid, price=100) for uid in range(101, 121)], capacity=40)
    assert len(canonical_ownership(current)['inventory']) == 30
    assert ownership_digest(current)


@pytest.mark.parametrize('trade', [{}, {'participant': 'Visitor'},
                                    {'participant': 'Visitor', 'participant_uid': None},
                                    {'participant_uid': 789}])
def test_unknown_open_trade_visitor_is_durably_quarantined(store, trade):
    result = store.observe_target('merchant-id', snapshot(100, request=False, trade=trade), now=100)
    assert result['phase'] == 'needs_attention'
...
def test_incomplete_ownership_during_unapproved_trade_is_quarantined(store):
    current = snapshot(100, request=False, trade={'participant': 'Visitor', 'participant_uid': 789})
...
def test_override_is_idempotent_disposition_and_never_sale(store):
    ...
    args = dict(confirmation_reference='incident-1', operator='Floor', reason='Reviewed exact inventory', now=103)
    ...
    with pytest.raises(ManualSessionError):
        store.operator_override(row['id'], **{**args, 'confirmation_reference': 'incident-2'})
...
def test_settlement_hook_commits_sales_state_and_audit_together(store):
    with store.db() as db:
        db.execute('CREATE TABLE integration_receipts(session_id TEXT PRIMARY KEY, digest TEXT)')
    calls = []
    ...
    assert calls == [row['id']]
    with store.db() as db:
        assert db.execute('SELECT session_id FROM integration_receipts').fetchone()[0] == row['id']
