...
def test_login_alone_does_not_confirm_shop_recovery_and_pause_is_quiet():
    ...
    state["enabled"] = True
    state["shop_return"]["phase"] = "complete"
    alerts.poll(status, 101)
    alerts.poll(status, 107)
    assert alerts.state["queue"][0]["kind"] == "recovery"


def test_persistent_failure_survives_restart_and_sends_once_then_recovers():
    alerts = Alerts()
    problem = ("Focus blocked", 60)
    alerts.observe("Dutch", problem, 0)
    alerts.observe("Dutch", problem, 59)
    assert not alerts.state["queue"]
    ...
    alerts.observe("Dutch", problem, 120)
    alerts.dispatch(120, load=lambda: "shops", send=send)
    assert len(sent) == 1 and sent[0][0] == "shops"
    alerts.observe("Dutch", None, 121)
    assert not alerts.state["queue"]
    alerts.observe("Dutch", None, 126)
    alerts.dispatch(126, load=lambda: "shops", send=send)
    assert len(sent) == 2 and "recovery confirmed" in sent[1][1]


def test_unsent_resolved_alert_cancels_without_fake_recovery():
    alerts = Alerts()
    alerts.observe("Dutch", ("failed", 0), 0)
...
def test_uncertain_transactions_and_auto_pause_are_urgent_but_normal_work_waits():
    ...
    assert (
        condition(
            {**healthy(), "needs_attention": {"note": "Unexpected failure; paused"}}
        )[1]
        == 0
    )
    assert (
        condition({**healthy(), "error": {"note": "Reconnect retries exhausted"}})[1]
        == 60
    )
...
def test_app_exit_and_reporting_failure_are_monitored_independently():
    ...
    other = Alerts()
    other.poll(None, 0, clean_shutdown=True)
    other.poll(None, 100, clean_shutdown=True)
    assert not other.state["queue"]
    status = {
        "characters": {"Dutch": healthy()},
        "sales_reporting": {"status": "needs_attention"},
    }
    other.poll(status, 0)
    other.poll(status, 60)
...
def test_delivery_is_durable_retries_safely_and_redacts_secrets():
    alerts = Alerts()
    alerts.observe(
        "Dutch", ("See https://discord.com/api/webhooks/1/secret token=secret", 0), 0
    )
    ...
    alerts.dispatch(
        0,
        load=lambda: "secret_url",
        send=failed,
        persist=lambda s: writes.append(copy.deepcopy(s)),
    )
    assert (
        len(writes) == 2
        and writes[0]["queue"]
        and alerts.state["queue"][0]["retry_at"] == 30
    )
    alerts = Alerts(json.loads(json.dumps(alerts.state)))
    calls = []
    ...
    alerts.dispatch(29, load=lambda: "shops", send=send)
    assert not calls
    alerts.dispatch(30, load=lambda: "shops", send=send)
    assert len(calls) == 1
...
def test_ui_and_reporting_worker_stalls_trigger_alerts():
    ...
    status = {
        "characters": {"Dutch": healthy()},
        "ui_health": {"tick_age_ms": 16000},
        "sales_reporting": {"status": "waiting", "last_checked_at": 0},
    }
    alerts.poll(status, 100)
    alerts.poll(status, 160)
    assert {r["subject"] for r in alerts.state["queue"]} == {
        "Conquest app",
...
def test_unavailable_owned_peer_alerts_after_sixty_seconds_and_requires_fresh_price_check():
    ...
    for now in (0, 59, 60):
        alerts.poll(status, now)
    assert [(row["subject"], row["kind"]) for row in alerts.state["queue"]] == [
        ("Dutch", "failure")
    ]
    ...
    dutch.update(
        snapshot={"timestamp": 62},
        foreground_refill_1078={
            "state": "waiting",
            "blocker": "waiting_farmer_handoff",
        },
    )
    dutch["enabled"] = False
    ...
    spiritual["snapshot"]["timestamp"] = 67
    dutch["snapshot"]["timestamp"] = 67
        ...
        "characters": {
            "Spiritual": {
                **healthy(),
                "connected": False,
                "refill": {"enabled": True},
                "error": {"note": "Invalid 1078 health candidate"},
            }
        }
    }
    alerts.poll(status, 0)
    alerts.poll(status, 120)
    assert not alerts.state["queue"] and not alerts.state["incidents"]
