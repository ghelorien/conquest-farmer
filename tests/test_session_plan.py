...
def test_level_42_does_not_leave_bandits_while_night_hold_is_active(monkeypatch):
    ...
    loop.health = lambda: {}
    events = []
    loop.record = lambda event, **kw: events.append(event)
    ...
    assert (
        loop.route.id == "bandit"
        and loop.last_level == 42
        and "route_hold_active" in events
    )


def test_pheasant_hold_prevents_level_progression_and_keeps_local_restock(
...
def test_route_hold_cannot_substitute_an_unrelated_restock_map(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="own restock town"):
        p.active_plan()
...
def test_circuit_checks_all_shops_in_both_cities_then_returns_to_bandits(
    tmp_path, monkeypatch, failed_vendor
):
    from conquest import world_travel, city_travel, equipment

    monkeypatch.setattr(p, "CIRCUIT", tmp_path / "circuit.json")
    monkeypatch.setattr(p, "active_plan", plan)
    calls = []
    current = [1011]
    services = {
    ...
    def review(self, v):
        calls.append(("shop", current[0], v))
        return v != failed_vendor

    monkeypatch.setattr(equipment.EquipmentReview, "visit", review)
    loop = NS(
        route=RouteLibrary().load("bandit"),
        phase="restocking",
        town=lambda action, **kw: {"level": 36} if action == "gear" else {},
        travel=lambda point: None,
        record=lambda *a, **k: None,
    )
    assert p.upgrade_circuit(loop)
    assert [c for c in calls if c[0] == "shop"] == [
        ("shop", 1002, 5),
        ("shop", 1002, 1),
        ("shop", 1002, 4),
        ("shop", 1011, 5),
...
def test_circuit_attempts_return_home_after_shop_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        city_travel,
        "city_for",
        lambda m: {"name": str(m), "services": {"blacksmith": [1, 1], "equipment": []}},
    )
    cities = []
    ...
    def town(action, **kw):
        if action == "gear":
            return {"level": 36}
        if cities:
            raise RuntimeError("Interrupted shop connection")
        return {}
    ...
    with pytest.raises(RuntimeError):
        p.upgrade_circuit(loop)
    assert cities[-1] == 1011
    assert not p.read_json(p.CIRCUIT)["completed"]


@pytest.mark.parametrize("finished", [False, True])
def test_restart_resumes_interrupted_upgrade_circuit_without_repeating_supply_trip(
    ...
    p.write_json(
        p.CIRCUIT,
        {
            "plan_started_at": 123,
            "completed": False,
            **({"finished_at": 124} if finished else {}),
        },
    )
    calls = []
    ...
    loop.town = lambda action, **kw: {
        "items": [
            {"type_id": 1050000, "amount": 1000},
    ...
    loop.restock = lambda: pytest.fail(
        "Already supplied; resume the interrupted crossing"
    )
...
def test_interrupted_circuit_continues_at_next_city(monkeypatch):
    ...
    p.write_json(
        p.CIRCUIT,
        {
            ...
            "tier": 35,
            "completed": False,
            "maps": [{"map_id": 1002, "reviewed": False}],
            "started_at": 124,
        },
    )
    monkeypatch.setattr(world_travel, "connection_path", lambda *a: [])
    ...
    monkeypatch.setattr(world_travel, "travel_to_map", lambda loop, m: cities.append(m))
    monkeypatch.setattr(
        equipment.EquipmentReview, "visit", lambda self, v: shops.append(v) or True
    )
    loop = NS(
        route=RouteLibrary().load("bandit"),
        phase="starting",
        town=lambda *a, **k: {"level": 36},
