...
from conquest.background_keyboard import BackgroundControlScope, _run_scope, validate_spec


SPEC = {'hwnd': 100, 'identity': {'pid': 10, 'creation_time_100ns': 42,
        'path': 'dummy.exe', 'architecture': 'x64'}, 'hold_seconds': .05}
...
class Api:
    def __init__(self):
        ...
        self.inspection = {'thread': 20, 'foreground': 300, 'foreground_thread': 30,
                           'active': 100, 'focus': 100}
    ...
    def thread_id(self): return 40

    def attach(self, a, b, value):
        self.attached = value
        self.calls.append(('attach', value))

    def keyboard(self): return bytes(self.state)
...
def test_restore_error_still_detaches():
    ...
    with pytest.raises(OSError):
        _run_scope(api, SPEC, lambda: True, lambda e: setattr(api, 'raise_restore', True))
...
def test_neutralized_ack_is_sent_only_after_restoration_while_attached():
    ...
    _run_scope(api, SPEC, lambda: 'neutralized' in events, emit,
               neutralize_requested=lambda: True, sleep=lambda seconds: None)
...
def test_public_neutralize_waits_for_ack_without_restarting_deadline():
    ...
    scope.process = SimpleNamespace(stdin=SimpleNamespace(write=write, flush=lambda: None),
                                    poll=lambda: None)
    scope.neutralize()
    scope.neutralize()
...
@pytest.mark.parametrize('value', [0, .01, 3, True, float('nan')])
