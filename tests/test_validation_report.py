...
def cycle():
    return [dict(time=1,event='kill_verified',count=2),
            ...
            dict(time=4,event='restock_complete',supplies=dict(arrows=5000,potions=5,free_slots=30)),
            dict(time=5,event='farming_area_reached'),dict(time=6,event='kill_verified',count=1)]
...
def test_forced_trip_unverified_storage_and_unrelated_events_never_qualify():
    rows=cycle();rows[1]['validation_cycle']=True
    assert not completed_cycles(rows)
    rows=cycle();rows[2]['verified_in_warehouse']=False
    assert not completed_cycles(rows)
    rows=cycle();rows[2]['time']=.5
    assert not completed_cycles(rows)
    rows=cycle();rows[4]['time']=3.5
    assert not completed_cycles(rows)
...
def test_extension_counts_later_downtime_and_preserves_original_baseline(tmp_path):
    ...
    write_json(config,{'started_at':100,'duration_seconds':120,'baseline_event_id':0})
    database=tmp_path/'reports/desktop-farming/trial.sqlite3';database.parent.mkdir(parents=True)
    with sqlite3.connect(database) as db:
        db.execute('create table events(time real,event text,payload text)')
        db.executemany('insert into events values(?,?,?)',[(110,'kill_verified',json.dumps({'count':120})),
    ...
    preserved=output.read_bytes();configuration=config.read_bytes()
