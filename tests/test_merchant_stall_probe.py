...
def test_stall_probe_journals_once_and_never_confirms(tmp_path, monkeypatch, case):
    ...
    clicked = False
    if case == "pending":
        journal.set("Dutch", "stall_probe", {"phase": "submitted"})
    flag = {"uid": 101503, "position": [230, 181], "draw_position": [1008, 364]}
    monkeypatch.setattr(
        "conquest.merchants.flag_target.flag_target", lambda *a: {"point": (1008, 332)}
    )
    monkeypatch.setattr(
        "conquest.scene_pointer.wait_scene_pointer", lambda s, p, check: check()
    )
    vacancy_reads = 0
    ...
    def vacant(observer, spec):
        ...
        vacancy_reads += 1
        if case == "occupied" or (
            case == "occupied_before_press" and vacancy_reads > 1
        ):
            return []
    ...
    def inventory():
        return NS(
            items=(NS(uid=9),) if not (clicked and case == "inventory_changed") else (),
            silver=100,
        )
    ...
    observer = NS(
        character="Dutch",
        health_layout=None,
        adapter=NS(read_block=lambda a, n: bytes(n)),
    )
    memory = NS(
        read=lambda: before,
        inventory=NS(read=inventory),
        gui=NS(
            viewport_size=lambda: (1888, 665),
            model=lambda *a: 0x200000,
            windows=lambda: [],
        ),
    )
    ...
    def qualified(capability):
        ...
        if case == "occupancy_unknown":
            raise ValueError("Stall occupancy qualification is pending")
        return {
            "shop_setup": {
                "vacancy_offset": 0x100,
                "vacancy_mask": 1,
                "vacancy_value": 0,
            }
        }
    ...
    def click(point, check, *, before_press):
        ...
        if case == "focus_denied":
            raise ValueError("No foreground focus; no button pressed")
        before_press()
        calls.append(point)
        clicked = True

    if case == "dialog":
        result = stall_probe.inspect_flag(
            driver, NS(click=click), journal, lambda: None
        )
...
def test_interrupted_stall_reconciliation_never_claims_unavailable_attribute_evidence(
    ...
    if case in ("exact", "changed"):
        record.update(inventory_before=[{"uid": 9, "plus": 1}], booth_before=[])
    ...
    if case in ("changed", "open_dialog"):
        with pytest.raises(ValueError):
            stall_probe.reconcile_interrupted_probe(driver, journal)
        assert journal.get("Dutch", "stall_probe")["phase"] == "submitted"
    else:
        result = stall_probe.reconcile_interrupted_probe(driver, journal)
        assert (
            result["phase"] == "reconciled_cancelled" and not result["input_qualified"]
        )
        assert result["exact_attributes_verified"] == (case == "exact")


@pytest.mark.parametrize(
    "case", ["opened", "changed_target", "pointer_timeout", "foreign_booth"]
)
def test_owned_booth_probe_uses_native_tile_and_verifies_receipt(
    tmp_path, monkeypatch, case
):
    import struct
    ...
    def targeting(*args):
        ...
        reads += 1
        return (
            {**target, "point": (980, 348)}
            if case == "changed_target" and reads > 1
            else target
        )
    ...
    def pointer(session, point, check):
        assert point == (976, 348)
        check()
        if case == "pointer_timeout":
            raise CaptureUnavailable("No pointer; no button pressed")

    monkeypatch.setattr("conquest.scene_pointer.wait_scene_pointer", pointer)

    def read(address, n):
        if n == 4:
            return struct.pack("<I", 103064)
        raw = bytearray(n)
        if pressed:
            raw[12] = 1
            struct.pack_into(
                "<I", raw, 0x4C, 999 if case == "foreign_booth" else 103064
            )
        return bytes(raw)
    ...
    observer = NS(
        character="Spiritual", health_layout=None, adapter=NS(read_block=read)
    )
    driver = NS(
        observer=observer,
        require_qualified=lambda *a: {"shop_setup": {}},
        memory=NS(
            read=lambda: state,
            inventory=NS(read=lambda: inv),
            gui=NS(
                viewport_size=lambda: (1888, 665),
                model=lambda *a: 0x200000,
                windows=lambda: [],
    ...
    if case == "opened":
        ...
        if case == "foreign_booth":
            ...
            assert (
                not pressed
                and record["phase"] == "cancelled_before_press"
                and record["retry_safe"]
            )
...
def test_recorded_manual_setup_supersedes_only_its_own_old_flag_probe(
    tmp_path, monkeypatch, case
):
    import time
    ...
    if case != "no_event":
        journal.event(
            "Spiritual",
            "manual_stall_setup_adopted",
            own_booth_uid=999 if case == "wrong_uid" else 103064,
            automatic_recovery_verified=False,
        )
    if case == "pending_trade":
        monkeypatch.setattr(journal, "pending", lambda *a: [{"id": "uncertain-trade"}])
    current = {
        "identity": {"pid": 1},
        "map_id": 1036,
        "position": [271, 174],
    ...
    def read():
        ...
        if case == "changed_stock" and reads > 1:
            fresh["silver"] += 1
        if case == "new_request" and reads > 1:
            fresh["request"] = {"uid": 123}
        return fresh

    booth = {
        "uid": 103064,
        "position": [274, 174] if case == "wrong_flag" else [272, 174],
    }
    ...
    if case == "adopted":
        ...
        assert (
            result["manual_setup_superseded_probe"]
            and result["own_booth_uid"] == 103064
        )
        assert (
            not result["input_qualified"]
            and not result["original_inventory_result_verified"]
        )
        assert (
            result["inventory_before"] == old["inventory_before"]
            and result["silver"] == 100
        )
    else:
        with pytest.raises(ValueError):
            stall_probe.reconcile_interrupted_probe(driver, journal)
