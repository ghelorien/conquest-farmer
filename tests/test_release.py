...
def _release(root, name="release"):
    ...
    (root / "scripts").mkdir()
    (root / "scripts" / "start_desktop_app.py").write_text(
        "print('launch')\n", encoding="utf-8"
    )
...
def test_manifest_and_release_roots_reject_reparse_points(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        release, "_reparse", lambda path: Path(path) == root or real(path)
    )
    ...
    release.write_manifest(root)
    monkeypatch.setattr(
        release, "_reparse", lambda path: Path(path) == linked or real(path)
    )
...
def test_live_state_and_local_profiles_are_not_release_content(tmp_path):
    ...
    (local / "profiles" / "desktop.local.yaml").write_text(
        "character: test", encoding="utf-8"
    )
    with pytest.raises(release.ReleaseError, match="Live machine state"):
...
def test_activation_never_places_managed_state_inside_release(tmp_path):
    ...
    with pytest.raises(release.ReleaseError, match="separate"):
        release.activate_release(
            root, state_root=root / "machine-state", lock=_unlocked
        )


def test_activation_rejects_release_nested_in_managed_state_but_allows_siblings(
    tmp_path,
):
    state = tmp_path / "state"
    ...
    release.write_json(
        state / release.ACTIVE, {**activated, "previous": nested_receipt}
    )
    with pytest.raises(release.ReleaseError, match="separate"):
...
def test_active_release_pins_manifest_and_launcher_uses_release_cwd(tmp_path):
    ...
    assert (
        options["env"]["CONQUEST_RELEASE_MANIFEST_SHA256"]
        == release.active_release(state_root=state)["manifest_sha256"]
    )
...
def test_build_selects_only_release_payload_and_excludes_worktree_state(tmp_path):
    ...
    (source / "reports" / "receipt.json").write_text("state", encoding="utf-8")
