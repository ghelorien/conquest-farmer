...
def test_pause_resume_retains_refill_only_permission():
    ...
    j=SimpleNamespace(get=lambda c,k,d=None:values.get((c,k),d),set=lambda c,k,v:values.__setitem__((c,k),v))
    r=SimpleNamespace(journal=j,enabled=lambda c:j.get(c,'enabled',False),refill_enabled=lambda c:j.get(c,'refill_enabled',False),set_refill_enabled=lambda c,v:j.set(c,'refill_enabled',v))
    ui=SimpleNamespace(runtime=r,pause=lambda c:j.set(c,'enabled',False),resume=lambda c:j.set(c,'enabled',True),resume_refill=lambda c:j.set(c,'refill_enabled',True))
    toggle(ui,'Dutch')
...
def test_refill_only_is_active_and_global_stop_is_clear():
    s={'connected':True,'enabled':False,'refill':{'enabled':True,'next_check':1000}}
