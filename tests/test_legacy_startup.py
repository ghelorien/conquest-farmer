...
def test_explicit_namespace_inherits_and_preserves_application_options(legacy):
    env = {}
    result = configure(
        ["--legacy-data-root", str(legacy), "--profile", "profiles/farmer.yaml"],
        environ=env,
    )
...
@pytest.mark.parametrize(
    "option",
    ["--data-root=x", "--profile-id=x", "--manage-profiles", "--migrate-from=x"],
)
...
def test_inherited_destination_cannot_be_replaced(legacy, tmp_path_factory):
    ...
    with pytest.raises(ValueError, match="differs"):
        configure(
            ["--legacy-data-root", str(other)], environ={ENVIRONMENT: str(legacy)}
        )


@pytest.mark.parametrize("check", [False, True])
def test_actual_launcher_bypasses_bootstrap_and_uses_live_cwd(
    legacy, monkeypatch, check
):
    for name in ("CONQUEST_DATA_ROOT", "CONQUEST_PROFILE_ID", ENVIRONMENT):
        monkeypatch.delenv(name, raising=False)
    calls = []
    app = ModuleType("conquest.desktop_app")
    app.main = lambda: calls.append(
        (Path.cwd(), list(sys.argv), os.environ.get(ENVIRONMENT))
    )
...
def test_elevation_carries_explicit_legacy_namespace(legacy, monkeypatch):
    for name in ("CONQUEST_DATA_ROOT", "CONQUEST_PROFILE_ID"):
        monkeypatch.delenv(name, raising=False)
