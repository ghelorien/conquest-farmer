...
def setup(monkeypatch, *, fail=None):
    ...
    win = dict(name=module.GRID, address=0x100000, geometry=[0., 0., 400., 400.], scroll=[0., 0.])
    before = dict(identity={'pid': 7}, inventory=[item], booth=[], silver=0,
                  position=[10, 10], windows=[win], request=None, trade=None)
    table = dict(columns=[dict(content_x=float(20 + i * 40), minimum=float(20 + i * 40),
                              maximum=float(60 + i * 40)) for i in range(5)],
                 outer=[20., 20., 220., 340.], clip=[20., 20., 220., 340.], row_height=40.)
    gui = SimpleNamespace(table=lambda *args: deepcopy(table), viewport_size=lambda: [400, 400])
    posts = []
    target = SimpleNamespace(snapshot=lambda: {'client_size': [400, 400]},
                             post=lambda *args: posts.append(args))
    driver = SimpleNamespace(target=target, memory=SimpleNamespace(gui=gui), read=lambda: deepcopy(before))
    neutral = dict(active={'id': 0}, drag={'active': False}, queue={'size': 0},
                   backend={'buttons_down': 0}, mouse_down=[False] * 5,
                   modifiers=dict(ctrl=False, shift=False, alt=False, super=False),
                   hover={'window': win['address'], 'id': 888}, want_capture_mouse=True,
                   mouse_position=[40., 40.])
    ...
    def sample(stage):
        ...
        if stage in ('drag-held', 'drag-return'):
            state['drag'] = dict(active=True, payload_type='CQITEM', source_item_uid=123,
                                 source_id=888, delivery=False)
...
def test_proves_exact_payload_then_returns_to_source_and_releases(monkeypatch):
    ...
    assert args[7] == ['drag-prime', 'drag-press', 'drag-held', 'drag-return', 'drag-release']
    ...
    assert all((row[2] & 0xffff) in (40, 41, 54, 55) for row in args[6] if row[0] == 0x200)
...
@pytest.mark.parametrize('invalid', ['bound', 'missing-slot', 'trade', 'dialog', 'clipped'])
def test_ineligible_initial_state_sends_nothing(monkeypatch, invalid):
    ...
    if invalid == 'bound':
        ...
        before['windows'][0]['geometry'] = [0., 0., 45., 45.]
    with pytest.raises(ValueError):
        module.run_drag_cancel(*args[:6])
    assert args[6] == []
