...
import pytest
from conquest.client_wrapper import (
    ClientWindow,
    LaunchWatch,
    ClientCatalog,
    pinned_client,
...
def client(pid=5, creation=10, title="Login", hwnd=99):
    return ClientWindow(
        {"pid": pid, "creation_time_100ns": creation, "path": "ImConquer.exe"},
        hwnd,
        title,
    )
...
def watcher():
    ...
    catalog = SimpleNamespace(
        identities=lambda: [c.identity for c in current], windows=lambda: list(current)
    )
...
@pytest.mark.parametrize(
    "exit_code,elapsed,state", [(7, 0, "failed"), (0, 181, "timed_out")]
)
...
def test_catalog_accepts_login_title_but_excludes_small_or_hidden_windows():
    ...
    backend = SimpleNamespace(
        processes=lambda exe: [{"pid": 5}],
        identity=lambda pid: identity,
        windows=lambda pid: [
            dict(hwnd=i, title="Login", visible=visible, client_size=size)
            for i, visible, size in [
                (1, True, [1024, 768]),
                (2, False, [1024, 768]),
                (3, True, [80, 50]),
            ]
    ...
    assert [w.hwnd for w in ClientCatalog(backend).windows(include_hidden=True)] == [
        1,
        2,
    ]
...
def test_catalog_waits_for_real_login_window_after_loading_splash():
    ...
    current = [
        dict(
            hwnd=1,
            title="ClassicConquer Loading",
            visible=True,
            client_size=[1400, 868],
        )
    ]
    backend = SimpleNamespace(
        processes=lambda exe: [{"pid": 5}],
    ...
    current[:] = [
        dict(hwnd=2, title="[ClassicConquer]", visible=True, client_size=[1400, 868])
    ]
