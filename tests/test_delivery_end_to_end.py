...
def item(uid):
    return {'uid':uid,'type_id':720027,'name':'MeteorScroll','plus':0,'gem1':0,
            'gem2':0,'quantity':1,'bound':False,'slot':uid,'price':None,'category':None}


def snapshot(name,uid,items=(),booth=()):
    return {'character':name,'character_uid':uid,'identity':{'pid':uid,'creation':1},
        'server':'America','timestamp':time.time(),'map_id':1036,'hp':100,'silver':100,
        'capacity':40,'inventory':copy.deepcopy(list(items)),'booth':copy.deepcopy(list(booth)),
        'booth_open':True,'trade':None,'request':None,'windows':[]}


class WorldDriver:
    def __init__(self,world,mode='delivered'):
        self.world,self.mode,self.operation=world,mode,None;self.input=[]
    def set_operation(self,operation):self.operation=operation
    def require_qualified(self,capability):
        if self.mode=='reject':raise ValueError('qualified preflight rejected')
    def read_pair(self,merchant):return copy.deepcopy(self.world['farmer']),copy.deepcopy(self.world['merchant'])
    def open_trade(self,intent):
        self.operation.before_action('trade_target_mode');self.input.append('target')
        self.operation.action_observed('trade_target_mode')
        self.operation.before_action('trade_request');self.input.append('request')
        for role,other in (('farmer','merchant'),('merchant','farmer')):
            mine=self.world[role];peer=self.world[other]
            mine['trade']={'participant':peer['character'],'participant_uid':peer['character_uid'],
    ...
    def place_item(self,intent,selected):
        self.operation.before_action('offer_item:'+str(selected['uid']));self.input.append('item')
    ...
    def confirm(self,intent):
        self.operation.before_action('farmer_confirm');self.input.append('confirm')
    def wait_pair(self,merchant):
        selected=self.world['farmer']['trade']['own_items']
        moved=selected if self.mode=='delivered' else selected[:1] if self.mode=='partial' else []
        ids={i['uid'] for i in moved}
        self.world['farmer']['inventory']=[i for i in self.world['farmer']['inventory'] if i['uid'] not in ids]
        self.world['merchant']['inventory']+=copy.deepcopy(moved)
        self.world['farmer']['trade']=self.world['merchant']['trade']=None
        for state in self.world.values():state['timestamp']=time.time()
...
def rig(tmp_path,monkeypatch,items):
    ...
    source=Journal(tmp_path/'source.sqlite3');receiver=Journal(tmp_path/'receiver.sqlite3')
    monkeypatch.setattr(delivery_operation,'JOURNAL',source.path)
    monkeypatch.setattr(delivery_bridge,'pair',lambda ui,character:
        (copy.deepcopy(world['farmer']),copy.deepcopy(world['merchant'])))
    qualified=NS(require_qualified=lambda capability:None)
    ui=NS(coordinator=NS(lock=threading.RLock(),owner=None,stopped=False),
        app=NS(control=NS(snapshot=lambda:{'enabled':False})),
        runtime=NS(enabled=lambda character:True,controllers={'Dutch':NS(driver=qualified)},
                   journal=receiver,lock=threading.RLock()))
...
def test_preinput_rejection_creates_no_durable_intent(tmp_path,monkeypatch):
    ...
    with pytest.raises(ValueError,match='preflight rejected'):
        DeliveryTransaction(source,driver,peer).run('Dutch',world['farmer']['inventory'],request_id='pre')
    assert not source.pending('Dutch') and receiver.get('Dutch','delivery_reservation') is None
    assert driver.input==[]
...
def test_lost_no_transfer_disposition_ack_restarts_read_only(tmp_path,monkeypatch):
        ...
        def __getattr__(self,name):
            ...
            def call(*args):
                ...
                if name=='disposition' and self.lost:
                    self.lost=False;raise OSError('lost disposition acknowledgement')
                return result
            return call
    clock=[time.time()];mono=[0.]
    def sleep(seconds):clock[0]+=seconds;mono[0]+=seconds
    tx=DeliveryTransaction(source,driver,LostPeer(),clock=lambda:clock[0],monotonic=lambda:mono[0],sleep=sleep)
    driver.open_trade=lambda intent:(_ for _ in ()).throw(OSError('request rejected before input'))
    ...
    assert receiver.get('Dutch','delivery_reservation')['phase']=='no_transfer_reconciled'
    calls=list(driver.input);clock[0]+=4;mono[0]+=4
    DeliveryTransaction(Journal(source.path),driver,real_peer,clock=lambda:clock[0],
                        monotonic=lambda:mono[0],sleep=sleep).recover('lost')
    assert driver.input==calls and not Journal(source.path).pending('Dutch')
    assert delivery_operation.status(Journal(source.path),'lost')['outcome']=='no_transfer'
...
def test_lost_finish_ack_restart_only_finalizes_receiver_receipt(tmp_path,monkeypatch):
        ...
        def __getattr__(self,name):
            ...
            def call(*args):
                ...
                if name=='finish' and self.lost:
                    self.lost=False;raise OSError('lost finish acknowledgement')
                return result
            return call
    ...
    assert delivery_operation.status(source,'finish')['next_action']=='finalize_receiver_receipt'
    ...
    assert delivery_operation.status(Journal(source.path),'finish')['next_action']=='release_route'


def test_journal_failure_before_action_trace_never_reaches_bridge_or_input(tmp_path,monkeypatch):
    world,source,receiver,peer=rig(tmp_path,monkeypatch,[item(10)])
    driver=WorldDriver(world)
...
def test_partial_confirm_restart_and_exact_verified_unrelated_sale(tmp_path,monkeypatch):
    rows=[item(10),item(11)];world,source,receiver,peer=rig(tmp_path,monkeypatch,rows)
    driver=WorldDriver(world,'partial');clock=[time.time()]
    tx=DeliveryTransaction(source,driver,peer,clock=lambda:clock[0],monotonic=lambda:clock[0],sleep=lambda s:None)
    with pytest.raises(ReconciliationBlocked,match='stable terminal settlement'):
        tx.run('Dutch',rows,request_id='partial')
    assert source.pending('Dutch')[0]['phase']=='uncertain' and driver.input.count('confirm')==1
    clock[0]+=4
    with pytest.raises(ReconciliationBlocked,match='no new input is authorized'):
        DeliveryTransaction(Journal(source.path),driver,peer,clock=lambda:clock[0],
                            monotonic=lambda:clock[0],sleep=lambda s:None).recover('partial')
    assert driver.input.count('confirm')==1 and source.pending('Dutch')[0]['phase']=='uncertain'
    ...
    listing=item(99);listing.update(name='Hat',price=100)
    ...
    changed=copy.deepcopy(world2['merchant']);changed['timestamp']+=.001
    changed['booth']=[];changed['silver']+=97
    ...
    DeliveryTransaction(source2,intent_driver,peer2,sale_receipts=receipt_source).recover('sale')
    result=delivery_operation.status(source2,'sale')
    assert result['outcome']=='no_transfer' and result['next_action']=='retry_delivery'
