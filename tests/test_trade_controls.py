...
@pytest.mark.parametrize('failure',[None,'idle','fingerprint','module','renderer','identity'])
def test_native_target_mode_requires_pinned_code_and_identity(failure):
    ...
    def identity():
        if failure=='identity':raise ValueError('identity changed')
    session=NS(expected_sha256='wrong' if failure=='fingerprint' else CLIENT_SHA256,
        modules=[] if failure=='module' else [{'name':'ImConquer.exe','base':base,'size':0x700000}],
        read_block=read,assert_identity=identity)
    if failure not in (None,'idle'):
        with pytest.raises(ValueError):controls.targeting_state(session)
...
def test_trade_button_tracks_the_verified_items_column(monkeypatch):
    ...
    def stale(gui):raise ValueError('Inventory button table is not current')
