...
def app(root):
    for name in ('scripts', 'src/conquest', 'profiles/routes', '.venv/Scripts', '.venv/bin'):
        (root/name).mkdir(parents=True, exist_ok=True)
    for name in ('pyproject.toml', 'scripts/start_desktop_app.py', 'scripts/run_overnight.py',
                 '.venv/Scripts/python.exe', '.venv/Scripts/pythonw.exe', '.venv/bin/python'):
        (root/name).touch()
    return root
...
def test_installed_package_never_guesses_from_cwd_or_site_packages(tmp_path, monkeypatch):
    root=app(tmp_path/'release')
    monkeypatch.setattr(layout, '__file__', str(tmp_path/'.venv/Lib/site-packages/conquest/application_layout.py'))
...
def test_immutable_controller_uses_pinned_script_interpreter_and_external_state(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(layout, '__file__', str(root/'.venv/Lib/site-packages/conquest/application_layout.py'))
    monkeypatch.setattr(route_controller, 'state_path', lambda value:str(state/value))
    from conquest import routes
    monkeypatch.setattr(routes, 'RouteLibrary', lambda directory:SimpleNamespace(load=lambda route:None))
    calls=[]
    monkeypatch.setattr(route_controller.subprocess,'Popen',lambda *a,**kw:calls.append((a,kw)) or SimpleNamespace(pid=8))
    assert route_controller.ensure_running('bandit')
    assert len(calls)==1
    argv=calls[0][0][0];options=calls[0][1]
    assert Path(argv[0])==root/('.venv/Scripts/pythonw.exe' if os.name=='nt' else '.venv/bin/python')
...
def test_invalid_launch_fails_before_stop_log_or_process_mutation(tmp_path, monkeypatch, fault):
    root=app(tmp_path/'release');state=tmp_path/'state';state.mkdir()
    release.write_manifest(root);digest=release.verify_release(root)['manifest_sha256']
    monkeypatch.setenv(layout.APP_ROOT, str(root));monkeypatch.setenv(layout.MANIFEST_PIN, digest)
    monkeypatch.setenv('CONQUEST_DATA_ROOT',str(state))
    monkeypatch.setattr(route_controller,'state_path',lambda value:str(state/value))
    calls=[];monkeypatch.setattr(route_controller.subprocess,'Popen',lambda *a,**kw:calls.append(a))
    if fault=='script':(root/'scripts/run_overnight.py').unlink()
    if fault=='python':(root/('.venv/Scripts/pythonw.exe' if os.name=='nt' else '.venv/bin/python')).unlink()
    if fault=='hash':monkeypatch.setenv(layout.MANIFEST_PIN, '0'*64)
    if fault=='reparse':
        original=layout.real_path
        def checked(value):
            if Path(value)==root:raise ValueError('reparse')
            return original(value)
        monkeypatch.setattr(layout,'real_path',checked)
    with pytest.raises(ValueError):
        route_controller.ensure_running('bandit',root=tmp_path/'other' if fault=='mismatch' else None)
    assert not calls and not list(state.iterdir())


def test_release_cannot_be_used_without_its_manifest_pin(tmp_path, monkeypatch):
    root=app(tmp_path/'release');release.write_manifest(root)
    with pytest.raises(ValueError,match='pin'):
        layout.application_root(root)


def test_live_controller_heartbeat_does_not_rehash_entire_release(tmp_path,monkeypatch):
    root=app(tmp_path/'release');state=tmp_path/'state';state.mkdir()
    release.write_manifest(root);digest=release.verify_release(root)['manifest_sha256']
    monkeypatch.setenv(layout.APP_ROOT,str(root));monkeypatch.setenv(layout.MANIFEST_PIN,digest)
    monkeypatch.setenv('CONQUEST_DATA_ROOT',str(state))
    monkeypatch.setattr(route_controller,'state_path',lambda value:str(state/value))
    monkeypatch.setattr(route_controller,'read_json',lambda path:
        {'phase':'hunting','pid':1,'updated_at':99} if Path(path).name=='status.json' else {})
    ...
    monkeypatch.setattr(release,'verify_release',lambda *a,**k:pytest.fail('full payload reread on heartbeat'))
...
def test_immutable_layout_requires_external_state_before_launch(tmp_path,monkeypatch):
    root=app(tmp_path/'release');release.write_manifest(root)
    monkeypatch.setenv(layout.APP_ROOT,str(root))
    monkeypatch.setenv(layout.MANIFEST_PIN,release.verify_release(root)['manifest_sha256'])
...
def test_entrypoint_rejects_wrong_root_before_conquest_imports(tmp_path):
    ...
    result=subprocess.run([sys.executable,'-B',str(root/'scripts/run_overnight.py'),'--help'],
