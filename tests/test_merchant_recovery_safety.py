...
def fixture(tmp_path):
    ...
    r = NS(
        journal=j,
        connect_cancel={},
        enable=lambda c, v: j.set(c, "enabled", v),
...
def test_normal_outside_market_never_authorizes_disconnect(tmp_path):
    ...
    assert (
        observe(
            r, "Dutch", i, life(), now=100, close=lambda i: pytest.fail("unauthorized")
        )
        is False
    )
...
def test_market_arrival_disarms_even_if_booth_work_is_blocked(tmp_path):
    ...
    assert not observe(
        ...
        life(world=1036),
        now=1000,
        close=lambda i: pytest.fail("safe Market"),
    )
...
def test_failed_close_preserves_pending_state_and_never_claims_success(tmp_path):
    ...
    with pytest.raises(ValueError):
        observe(r, "Dutch", i, now=105, close=fail)
    assert not r.journal.get("Dutch", KEY)["disconnected"]
    assert r.journal.get("Dutch", KEY)["phase"] == "disconnect_pending"


def test_explicit_hold_prevents_recovery_and_stale_restart_deadline_is_retained(
    tmp_path,
):
    r, i = fixture(tmp_path)
    r.journal.set("Dutch", "connect_hold", True)
    arm(r, "Dutch", now=100)
    assert r.journal.get("Dutch", KEY) is None
    r.journal.set("Dutch", "connect_hold", False)
    arm(r, "Dutch", now=100)
    submitted(r, "Dutch", now=100)
    arm(r, "Dutch", now=105)
    submitted(r, "Dutch", now=105)
    calls = []
    observe(r, "Dutch", i, now=105, close=lambda i: calls.append(i) or True)
    assert calls == [i]
