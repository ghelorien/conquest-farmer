...
def fixture(tmp_path, monkeypatch):
    ...
    runtime = NS(
        ...
        journal=journal,
        observers={"Dutch": observer, "Spiritual": observer},
        latest={
            c: {"identity": identity, "timestamp": time.time()}
        ...
        handoff="merchant-refill:Spiritual:123",
    )
    coordinator = NS(
        owner=None,
        purpose=None,
        ...
        manual_session_blocked=lambda c: False,
        surface_blocks={},
    )
    ui = NS(
        closed=False,
    ...
    monkeypatch.setattr(
        "conquest.merchants.listing_handoff_1078.parked",
        lambda ui: calls.append("fresh_park"),
    )
    monkeypatch.setattr(
        "conquest.merchants.booth_probe_1078._farmer_journals_clear", lambda r: None
    )
    monkeypatch.setattr(
        "conquest.merchants.coordination.input_scope",
        lambda **kw: nullcontext(calls.append(kw)),
    )
    ...
    class Host:
        ...
        def __init__(self):
            self.api = NS(
                assert_owner=lambda *a: calls.append("owner"),
                gui=NS(
                    IsIconic=lambda h: False,
                    GetClientRect=lambda h: (0, 0, 1420, 1009),
                    GetAncestor=lambda h, f: h if h == 2 else 10,
                    GetWindow=lambda h, f: 10,
                    IsWindowVisible=lambda h: False,
                ),
            )

        def attach(self, hwnd, identity, parent, w, h):
            calls.append(("attach", hwnd, parent, w, h))
            self.saved = NS(hwnd=hwnd, identity=identity)

        def resize(self, *a):
            calls.append(("resize", a))

    return ui, Host, calls


def test_hidden_hosts_attach_under_fresh_nonmarket_park_without_changing_controls(
    ...
    assert all(
        s["attached"] and not s["selected"] and not s["native_visible"]
        for s in ui.layout_status.values()
    )
...
def test_selected_identified_client_automatically_hosts_with_untouched_pending_meteor(
    tmp_path, monkeypatch
):
    ...
    monkeypatch.setattr(
        "conquest.merchants.booth_probe_1078._farmer_journals_clear",
        lambda runtime: pytest.fail(
            "Display must not interpret Farmer transaction journals"
        ),
    )
    ...
    ui.notebook = NS(select=lambda: "Dutch")
    ui.frames = {"Dutch": "Dutch", "Spiritual": "Spiritual"}
    ui.auto_embedding = False
    ui.auto_embed_retry = {}
    ui.client_panes["Dutch"] = NS(
    ...
    with ui.runtime.journal.db() as db:
        saved = list(
            db.execute("SELECT character,name,value FROM state ORDER BY character,name")
        )
    ...
    with ui.runtime.journal.db() as db:
        assert (
            list(
                db.execute(
                    "SELECT character,name,value FROM state ORDER BY character,name"
                )
            )
            == saved
        )
    assert ui.runtime.handoff == "merchant-refill:Spiritual:123"
    assert ui.app.control.snapshot() == {
        "enabled": False,
        "paused": False,
        "revision": 1,
    }
...
def test_pending_merchant_transaction_still_protects_layout(tmp_path, monkeypatch):
    ui, Host, calls = fixture(tmp_path, monkeypatch)
    ui.runtime.journal.begin(
        "unresolved-listing", "Dutch", "booth_listing_1078_once", {}
    )
...
@pytest.mark.parametrize(
    "hold", ["stop", "manual", "owner", "visitor", "handoff", "refilling"]
)
def test_hosting_holds_send_no_window_mutation(tmp_path, monkeypatch, hold):
    ui, Host, calls = fixture(tmp_path, monkeypatch)
    if hold == "stop":
        ui.coordinator.stopped = True
    elif hold == "manual":
        ui.coordinator.manual_active = lambda: True
    elif hold == "owner":
        ui.coordinator.owner = "Dutch"
    elif hold == "visitor":
        ui.coordinator.manual_session_blocked = lambda c: True
    elif hold == "handoff":
        ui.runtime.manual_handoff_status = lambda: {"phase": "ready"}
    else:
        ui.runtime.refilling = {"Dutch": 1}
    hosts.restore(ui, host_factory=Host)
    assert not any(isinstance(x, tuple) and x[0] == "attach" for x in calls)


def test_host_intent_independent_of_permissions_and_manual_release_survives_restart(
    tmp_path, monkeypatch
):
    ui, Host, calls = fixture(tmp_path, monkeypatch)
    for c in ("Dutch", "Spiritual"):
        ui.runtime.journal.set(c, "enabled", False)
        ui.runtime.journal.set(c, "refill_enabled", False)
    ...
    ui.released_clients = {
        c
        for c in ("Dutch", "Spiritual")
        if ui.runtime.journal.get(c, "host_released", False)
    }
...
def test_host_grant_cannot_yield_game_input_and_release_restores_queued_refill(
    tmp_path, monkeypatch
):
    from conquest.merchants.ui import UnifiedUI
    ...
    ui.runtime.finish_handoff = lambda: pytest.fail(
        "Host release must not pause refill or advance its timer"
    )
    ...
    assert UnifiedUI.dispatch(
        ui, {"action": "handoff-release", "request_id": request["request_id"]}
    ) == {"released": True}
    assert ui.runtime.handoff == "merchant-refill:Spiritual:123"
    assert ui.grant is None and ui.grant_fence.active is None


def test_stale_listing_request_cannot_gain_late_grant_after_validation(
    tmp_path, monkeypatch
):
    from conquest.merchants.ui import UnifiedUI
    from conquest.merchants.grant_fence import GrantFence
    ...
    monkeypatch.setattr(
        "conquest.merchants.listing_handoff_1078.validate_grant", slow_validation
    )
    with pytest.raises(
        ValueError, match="Handoff request, owner, or Farmer revision changed"
    ):
        UnifiedUI.dispatch(
            ui,
            {
...
def test_host_request_admission_requires_matching_connected_process_and_separate_timer(
    tmp_path,
):
...
def test_manual_session_defers_host_intent_without_erasing_preference(
    tmp_path, monkeypatch
):
    ui, Host, calls = fixture(tmp_path, monkeypatch)
...
def test_pending_transaction_defers_host_intent_until_exact_cleanup(
    tmp_path, monkeypatch
):
    ui, Host, calls = fixture(tmp_path, monkeypatch)
    previous = hosts.requested(ui)
...
def test_host_scope_denies_low_level_game_input(monkeypatch):
    ...
    with pytest.raises(CaptureUnavailable, match="does not authorize"):
        coordination.check_input()

    @coordination.coordinated_input
    def click():
        pytest.fail("Host-only authority reached game input")
...
def test_qualified_native_lease_uses_only_native_presentation_and_releases(tmp_path):
    ...
    guard = InputCoordinator(safe_to_yield=lambda: True, path=tmp_path / "input.lock")
    calls = []
    guard.native_trade1078_policy = lambda c: c == "Dutch"
    guard.surface_blocks["Dutch"] = True
    guard.on_acquire = lambda c: pytest.fail(
        "Legacy surface preparation must remain unavailable"
    )
    guard.on_native_acquire = lambda c: calls.append(
        ("present", c, guard.owner, guard.purpose)
    )
    guard.on_release = lambda c: calls.append(("release", c, guard.owner))
    with guard.lease("Dutch", purpose="trade"):
        calls.append("body")
    assert calls == [
        ("present", "Dutch", "Dutch", "trade"),
        "body",
        ("release", "Dutch", None),
    ]


def test_exact_native_lease_presents_hidden_host_without_qualifying_or_attaching(
    tmp_path, monkeypatch
):
    ui, Host, calls = fixture(tmp_path, monkeypatch)
    hosts.restore(ui, host_factory=Host)
    ...
    ui.coordinator.owner = "Dutch"
    ui.coordinator.purpose = "booth_listing_1078_once"
    ...
    ui.notebook = NS(select=select)
    ui.frames = {"Dutch": "Dutch"}
    ui.detail_tabs = {"Dutch": NS(select=lambda index: None)}
    ui.input_bookmarks = {}
    ui.root = NS(update_idletasks=lambda: None)
    ui.client_panes["Dutch"] = NS(
        winfo_ismapped=lambda: True, winfo_width=lambda: 1420, winfo_height=lambda: 1009
    )
    ui.hosts["Spiritual"].api.show_async = lambda h, flag: calls.append(
        ("hide", h, flag)
    )
    expected = (
        host,
        observer,
        2,
        dict(observer.adapter.identity),
...
def test_host_intent_does_not_starve_ordinary_work(tmp_path, monkeypatch, reason):
    ...
    status = {
        "host_request": {
            "request_id": "merchant-host:x",
            "identities": {"Dutch": identity},
        },
        "handoff_requested": "merchant-recovery:Dutch:1"
        if reason == "urgent"
        else "merchant-refill:Dutch:1",
        "characters": {
            "Dutch": {
                "connected": True,
    ...
    monkeypatch.setattr(
        handoff,
        "read_json",
        lambda path: {"parity_verified": True, "hunting_handoffs_enabled": True},
    )
    ordinary = NS(due=lambda: True)
    paths = []
    ...
    class ReachedHealth(Exception):
        pass

    def health():
        raise ReachedHealth

    with pytest.raises(ReachedHealth):
        handoff.service_window(NS(health=health), town=reason == "town")
...
def test_host_attachment_before_grant_does_not_release_nonexistent_request(
    ...
    status = {
        ...
        "characters": {
            "Dutch": {
                "connected": True,
                "snapshot": {"identity": identity, "timestamp": time.time()},
            }
        },
    }
    health = {
        "target": {"pid": 1},
        "embedded_controls": {"control": {"enabled": True, "revision": 3}},
    }
    original = handoff.WorkWindows
    ordinary = original(tmp_path / "ordinary.json")
    hosting = original(tmp_path / "hosting.json")
    monkeypatch.setattr(
        handoff, "WorkWindows", lambda path=None: ordinary if path is None else hosting
    )
    ...
    monkeypatch.setattr(
        ...
        "read_json",
        lambda path: (
            {"parity_verified": True, "hunting_handoffs_enabled": True}
            if path == handoff.POLICY
            else read(path)
        ),
    )
    ...
    loop = NS(
        ...
        focus=lambda h: calls.append("focus"),
    )
    if outcome == "attached":
        ...
        with pytest.raises(OSError, match="Lost acknowledgement"):
            handoff.service_window(loop)
        assert calls.index("handoff-release") < calls.index("resume")
    else:
        with pytest.raises(bridge.MerchantRejected):
            handoff.service_window(loop)
        assert "handoff-release" not in calls and "resume" not in calls


@pytest.mark.parametrize(
    "changed", ["owner", "grant", "manual", "identity", "stale", "attachment"]
)
def test_completed_host_race_never_ignores_conflicting_or_stale_proof(changed):
    ...
    status = {
        ...
        "characters": {
            "Dutch": {
                "connected": True,
                "snapshot": {"identity": {"pid": 2}, "timestamp": time.time()},
            }
        },
    }
    assert host_restored_before_grant(status, identities)
    if changed == "owner":
        status["input_owner"] = "Dutch"
    elif changed == "grant":
        status["handoff_active"] = True
    elif changed == "manual":
        status["manual_farmer"]["input_fenced"] = True
    elif changed == "identity":
        status["characters"]["Dutch"]["snapshot"]["identity"] = {"pid": 3}
    elif changed == "stale":
        status["characters"]["Dutch"]["snapshot"]["timestamp"] -= 3
    else:
        status["layout"]["Dutch"]["attached"] = False
