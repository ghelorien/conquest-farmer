...
def test_f1_priority_and_repeat_until_above_threshold(tmp_path, monkeypatch, initial_hp, expected_keys, level_up):
    import win32api
    now, keys = [10.], []
    if level_up and expected_keys:expected_keys=1
    profile = yaml.safe_load(open("profiles/pheasant-foreground-trial.yaml"))
    profile["observation_mode"] = "memory_only" if level_up else "legacy_visual"  # Exercise the historical loop with fakes.
    profile["route"] = []
    ...
    class Session:
        ...
        def request(self, name, args=None):
            ...
            if name == "sample":
                values = dict(name="Parasite", position=[423,455], max_hp=[110 if level_up and keys else 100],
    ...
    class Inventory:
        ...
        def read(self):
            now[0] += .1
            potions = tuple(Item(i,1000000,1,1,i) for i in range(3 if level_up else 3-len(keys)))
    ...
    monkeypatch.setattr(trial,"health_ratio",lambda frame, size: initial_hp if not keys else (.3995 if len(keys)==1 else .9))
    monkeypatch.setattr(trial,"targets",lambda *args: [])
    monkeypatch.setattr(trial,"nearby_drops",lambda *args: [])
    supervisor=None
    if level_up:
        def no_pixels(*args,**kwargs):pytest.fail('Level-up recovery must use memory only')
        ...
        supervisor=SimpleNamespace(recovery=SimpleNamespace(terrain=SimpleNamespace(width=1000,height=1000)),
            observe=lambda:{'health_ratio':1.0 if keys else initial_hp,'waiting':False},
            memory_targets=lambda *args:[],loot_step=lambda *args:False,
            dispatch=lambda callback,**kwargs:callback())
    result = trial.run_trial(path,"unused",tmp_path / "output",4,logging.getLogger("test"),supervisor=supervisor)
