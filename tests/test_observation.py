...
def report():
    return {
        "schema_version": 1,
        "process_identity": copy.deepcopy(Session.identity),
        "expected_sha256": Session.expected_sha256,
        "candidates": {
            "hp_u32": {"kind": "u32", "addresses": ["0x10000"], "truncated": False},
            "position_u32": {
                "kind": "xy_u32",
                "addresses": ["0x20000"],
                "truncated": False,
...
@pytest.mark.parametrize(
    "field,value", [("expected_sha256", "b" * 64), ("process_identity", {"pid": 1})]
)
...
@pytest.mark.parametrize(
    "addresses", [["0x10"], [], ["0x10000"] * 65, ["0x10000", "0x10000"]]
)
...
def test_recording_retains_changes_and_outcome(report, tmp_path):
    ...
    try:
        result = record(
            CandidateReader(session, report, clock=clock),
            recorder,
            {},
            seconds=1,
            interval=0.25,
            clock=clock,
            sleep=clock.sleep,
            on_sample=next_state,
        )
        assert result["samples"] == 4
        assert result["candidate_change_counts"] == {
            "hp_u32@0x10000": 3,
            "position_u32@0x20000": 3,
        }
        assert result["stop_reason"] == "duration_elapsed" and not result["qualified"]
        saved = recorder.db.execute(
            "SELECT sample_json FROM observation_samples ORDER BY sequence"
        ).fetchall()
        assert json.loads(saved[-1][0])["fields"][0]["value"] == [48]
        assert (
            recorder.db.execute(
                "SELECT stop_reason FROM observation_sessions"
            ).fetchone()[0]
            == "duration_elapsed"
        )
    finally:
...
def test_emergency_stop_responsive_during_long_interval(report, tmp_path):
    ...
    try:
        result = record(
            ...
            clock=clock,
            sleep=clock.sleep,
            should_stop=lambda: clock.now >= 0.1,
        )
...
def test_keyboard_interrupt_finalizes_recording(report, tmp_path):
    ...
    try:
        result = record(
            CandidateReader(Session(), report), recorder, {}, on_sample=interrupt
        )
...
def test_real_readonly_memory_recording_of_test_process(tmp_path):
    ...
    with MemorySession(os.getpid(), digest, executable=image_path.name) as session:
        report = {
            ...
            "candidates": {
                "hp_u32": {
                    "kind": "u32",
                    "addresses": [hex(ctypes.addressof(hp))],
                    "truncated": False,
                },
                "position_u32": {
                    "kind": "xy_u32",
                    "addresses": [hex(ctypes.addressof(position))],
                    "truncated": False,
                },
            },
        }
    ...
    with sqlite3.connect(tmp_path / "real.sqlite") as db:
        saved = json.loads(
            db.execute(
                "SELECT sample_json FROM observation_samples ORDER BY sequence LIMIT 1"
            ).fetchone()[0]
        )
