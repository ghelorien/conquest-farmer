...
def market(at, *, present=True):
    return MarketSnapshot(
        dict(
            ...
            observed_at=at,
            total=1,
            equipment_categories={"130": "Trojan Armor"} if present else {"500": "Bow"},
            listings=[
                dict(
                    name="Coat" if present else "Bow",
                    category="Trojan Armor" if present else "Bow",
                    quality="Normal",
                    plus=2,
...
def test_outliers_only_rejected_with_three_other_sellers():
    assert (
        price_item(
            KEY, [Listing(str(i), KEY, p) for i, p in enumerate([10, 100, 100])]
        ).price
        == 9
    )
    assert (
        price_item(
            KEY, [Listing(str(i), KEY, p) for i, p in enumerate([10, 100, 100, 100])]
        ).price
        == 99
    )
...
def test_plus_two_uses_three_times_raw_plus_one_value_only_without_exact_quote():
    ...
    assert (
        quote_item(KEY, [replace(base, key=replace(base.key, quality="Super"))]).price
        is None
    )
    assert (
        quote_item(
            KEY, [replace(base, key=replace(base.key, sockets=("Empty", "No socket")))]
        ).price
        is None
    )
...
def test_historical_fallback_persists_without_repeated_undercuts_or_age_reset(tmp_path):
    ...
    empty = market(200, present=False)
    history.remember(empty)
...
def test_history_retains_reference_instead_of_discounted_target(tmp_path):
    history = PriceHistory(tmp_path / "history.sqlite3")
    history.remember(market(100))
    assert history.quotes()[KEY]["unit_price"] == "130000"
    assert (
        quote_item(KEY, [Listing("Now", KEY, 120000)], history=history.quotes()).price
        == 118800
    )
...
def test_history_keeps_non_equipment_names_separate(tmp_path):
    ...
    assert (
        quote_item(
            replace(key, category="Stones:DragonBall"), [], history=history.quotes()
        ).price
        is None
    )
...
@pytest.mark.parametrize(
    "owned_quality", ["Fixed", "Normal", "Refined", "Unique", "Elite"]
)
@pytest.mark.parametrize(
    "market_quality", ["Fixed", "Normal", "Refined", "Unique", "Elite"]
)
def test_user_composition_quality_group(owned_quality, market_quality):
    key = replace(KEY, quality=owned_quality)
    assert (
        quote_item(
            key, [Listing("Outside", replace(KEY, quality=market_quality), 100)]
        ).price
        == 99
    )
    assert (
        quote_item(
            key, [Listing("Dutch", replace(KEY, quality=market_quality), 100)]
        ).price
        == 100
    )
...
@pytest.mark.parametrize(
    "sockets",
    [("Empty", "No socket"), ("Empty", "Empty"), ("Normal DragonGem", "No socket")],
)
...
def test_quality_group_does_not_include_super_or_unplussed_items():
    assert (
        quote_item(KEY, [Listing("Outside", replace(KEY, quality="Super"), 100)]).price
        is None
    )
    assert (
        quote_item(
            replace(KEY, plus=0),
            [Listing("Outside", replace(KEY, plus=0, quality="Elite"), 100)],
        ).price
        is None
    )
    ...
    assert (
        quote_item(
            KEY, [], history={row.key: {"unit_price": "100", "observed_at": 100}}
        ).price
        == 300
    )
