...
@pytest.mark.parametrize('message',['GUI registry changed','GUI geometry changed','GUI table changed during observation'])
def test_gui_race_before_any_input_is_retryable(message):
    trade=object.__new__(TownTrade)
    def execute(body):raise GuiObservationChanged(message)
    trade.execute=execute
    with pytest.raises(TownObservationUnavailable):trade({'action':'warehouse-money'})
...
def test_gui_race_after_input_is_not_retryable():
    ...
    def execute(body):
        ...
        raise error
    trade.execute=execute
    with pytest.raises(GuiObservationChanged) as caught:trade({'action':'warehouse-money-withdraw'})
    assert caught.value is error


def test_invalid_registry_is_not_a_transient_race():
    trade=object.__new__(TownTrade)
    def execute(body):raise ValueError('Invalid GUI registry')
    trade.execute=execute
    with pytest.raises(ValueError) as caught:trade({'action':'warehouse-money'})
