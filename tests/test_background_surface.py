...
class Native:
    ...
    def snapshot(self, hwnd, identity):
        ...
        return WindowState(
            dict(identity), hwnd, self.style, self.exstyle, self.owner, self.placement
        )
    ...
    def IsChild(self, hwnd, other):
        return other == 21

    def GetWindow(self, hwnd, flag):
        return self.owner if hwnd == 20 else 0
    ...
    def GetWindowPlacement(self, hwnd):
        return (
            (self.placement[0], 3, *self.placement[2:])
            if self.zoomed
            else self.placement
        )

    def GetWindowLong(self, hwnd, index):
        return {-16: self.style, -20: self.exstyle}[index]
...
def test_restore_preserves_style_owner_placement_fields_and_visibility(visible):
    ...
    surface.restore()
    assert (api.style, api.exstyle, api.owner) == (
        before.style,
        before.exstyle,
        before.owner,
    )
...
@pytest.mark.parametrize(
    "field,value",
    [("foreground", 20), ("foreground", 21), ("iconic", True), ("zoomed", True)],
)
...
def test_monitor_change_invalidates_surface_but_allows_nonactivating_restore():
    ...
    with pytest.raises(ValueError, match="Monitor layout"):
        surface.check()
    surface.restore()
    assert surface.saved is None
...
@pytest.mark.skipif(
    sys.platform != "win32", reason="Read-only installed Win32 API smoke check"
)
