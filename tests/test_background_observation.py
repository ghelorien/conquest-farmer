...
class Session:
    ...
    def __init__(self, events=()):
        ...
        self.memory[CONTEXT + 0x425C] = drag_state(
            active=0, frame=-1, payload_type=b"", size=0, address=0
        )
...
def drag_state(
    *,
    active=1,
    frame=12,
    payload_type=b"CQITEM",
    size=4,
    address=CONTEXT + 0x42F0,
    preview=0,
    delivery=0,
):
    raw = bytearray(0x50)
...
def test_mouse_queue_and_backend_are_read_only_and_serializable():
    session = Session(
        [
            event(1, payload=struct.pack("<ff", 120, 300)),
...
@pytest.mark.parametrize(
    "bad_event",
    [
        event(1, payload=struct.pack("<ff", float("nan"), 1)),
        event(2, payload=struct.pack("<ff", 1, float("inf"))),
        event(7),
        event(1, 999),
        event(3, payload=struct.pack("<IB", 5, 1)),
        event(3, payload=struct.pack("<IB", 0, 2)),
        event(6, 0, b"\x02"),
    ],
)
...
@pytest.mark.parametrize(
    "offset,fmt,values",
    [
        (0xD94, "<ff", (float("nan"), 1)),
        (0xD94, "<ff", (0, float("inf"))),
        (0xDAC, "<4B", (0, 2, 0, 0)),
        (0xE00, "<I", (16,)),
        (0xD9C, "<5B", (0, 0, 2, 0, 0)),
        (0xD0, "<B", (2,)),
...
def test_unknown_drag_payload_is_never_dereferenced_or_decoded_as_item():
    ...
    session.memory[CONTEXT + 0x425C] = drag_state(
        payload_type=b"CQITEMTYPEID", address=0x900000
    )
    result = BackgroundObservationReader(session).snapshot()["drag"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        dict(delivery=2),
        dict(active=2),
        dict(payload_type=b"A" * 33),
        dict(payload_type=b"BAD\x01TYPE"),
    ],
)
