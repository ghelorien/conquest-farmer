...
def test_full_stock_snapshot_rechecks_life_after_gui_sampling(
    monkeypatch, change, character
):
    reader = memory.MerchantMemory.__new__(memory.MerchantMemory)
    reader.base = 0x100000
    ...
    verified = []
    context = NS(
        verify=lambda snapshot: verified.append(snapshot["character"]),
    ...
    def windows():
        if change == "position_gui":
            live["position"] = (101, 100)
        if change == "death_gui":
            live["dead_candidate"] = True
        if change == "zero_hp_gui":
            live["current_hp"] = 0
        if change == "map_gui":
            live["map_id"] = 1002
        if change == "actor_gui":
            live["object_address"] = 0x600000
        return []
    ...
    if change:
        error = (
            memory.TransitObservationChanged
...
@pytest.mark.parametrize(
    "change", [None, "position", "death", "server", "silver", "dialog"]
)
def test_travel_snapshot_is_scoped_and_rechecks_mutable_state(monkeypatch, change):
    ...
    def block(address, size):
        ...
        if address == reader.base + 0x697860:
            return (
                b"wrong" if change == "server" and changed else b"Classic_US"
            ).ljust(64, b"\0")
        if address == 0x200020:
            return struct.pack("<I", 200 if change == "silver" and changed else 100)
        if address in (0x30000C, 0x40000C):
            return bytes([int(change == "dialog" and changed)])
    ...
    reader.gui = NS(
        model=lambda model, vtable: {14: 0x300000, 15: 0x400000}[model],
        windows=lambda: [],
    )
    calls = []
