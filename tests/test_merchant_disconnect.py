...
def fixture(tmp_path):
    ...
    identity = {"pid": 7, "path": "C:/game/ImConquer.exe", "creation_time_100ns": 99}
    journal.set("Dutch", "last_identity", identity)
    r = NS(
        journal=journal,
        coordinator=NS(owner=None, lock=threading.RLock()),
        enable=lambda n, on: journal.set(n, "enabled", on),
...
def test_disconnect_refuses_unsettled_input_or_missing_identity(tmp_path, case):
    r, _ = fixture(tmp_path)
    if case == "owner":
        r.coordinator.owner = "Spiritual"
    if case == "pending":
        r.journal.begin("t", "Dutch", "delivery", {})
    if case == "identity":
        r.journal.set("Dutch", "last_identity", None)
    calls = []
    with pytest.raises(ValueError):
        disconnect(r, "Dutch", close=lambda i: calls.append(i))
    assert not calls
