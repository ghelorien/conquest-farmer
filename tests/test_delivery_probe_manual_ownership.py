...
def supervised(rig, monkeypatch, tmp_path):
    ...
    x.probe = {
        ...
        "intent": {
            "farmer": deepcopy(x.farmer),
            "merchant": {**deepcopy(x.state), "request": None},
            "items": deepcopy(x.farmer["inventory"]),
        },
        "farmer_after": deepcopy(x.farmer),
        "merchant_after": deepcopy(x.state),
    }
...
def test_exact_probe_precedes_admission_without_granting_delivery_trust(
    supervised, monkeypatch
):
    x = supervised
    monkeypatch.setattr(
        "conquest.character_context.trusted_delivery", lambda *a, **kw: False
    )
    x.controller.reconcile = lambda *a: pytest.fail(
        "Supervised probe is not an ordinary transaction"
    )
...
def test_peer_read_completion_time_does_not_create_false_manual_admission(
    supervised, phase, entry
):
    x = supervised
    x.probe["phase"] = phase
    x.save()
...
def test_slow_peer_read_does_not_refresh_expired_local_evidence(supervised, read_fails):
    ...
    def read_source():
        x.now += 6
        if read_fails:
            raise OSError("slow failed read")
        return {**deepcopy(x.farmer), "timestamp": x.now}
...
def test_restart_rebinds_same_attached_observer_without_changing_farming_off(
    supervised,
):
    ...
    assert (
        intent == {"enabled": False, "paused": False, "revision": 7} and x.calls == []
    )
...
def test_observer_contention_retains_existing_fence_and_never_grants_bilateral_proof(
    supervised,
):
    x = supervised
...
def test_unresolved_exact_probe_structurally_suppresses_manual_admission_on_transient_peer_failure(
    ...
    x.save()
    assert x.runtime._structural_request_probe_owned(
        "Dutch", x.read(), probe.read_probe(), now=x.now
    )
...
def test_structural_probe_precedence_allows_unselected_farmer_inventory(supervised):
    ...
    for where in (
        x.farmer["inventory"],
        x.probe["intent"]["farmer"]["inventory"],
        x.probe["farmer_after"]["inventory"],
    ):
...
def test_structural_probe_precedence_rejects_rebound_farmer_before_peer_read(
    supervised,
):
    x = supervised
...
def test_structural_probe_precedence_rejects_malformed_durable_chronology(
    supervised, fault
):
    x = supervised
    x.source.lock = NS(acquire=lambda **_kwargs: False, release=lambda: None)
    if fault == "stale_intent":
        x.probe["intent"]["farmer"]["timestamp"] = 90
    else:
        x.probe["accepted_at"] = 101
    x.save()
    assert x.runtime.process_manual("Dutch", x.read(), now=x.now)
    assert x.runtime.manual_status("Dutch")["phase"] == "approval_pending"
...
def test_request_submitted_structural_precedence_requires_exact_live_merchant_ownership(
    supervised, change
):
    x = supervised
    x.probe["phase"] = "request_submitted"
    x.probe.pop("farmer_after")
    x.probe.pop("merchant_after")
    x.save()
    x.source.lock = NS(acquire=lambda **_kwargs: False, release=lambda: None)
    if change == "silver":
        x.state["silver"] += 1
    else:
        x.state["inventory"][0]["slot"] = 5
    assert x.runtime.process_manual("Dutch", x.read(), now=x.now)
    assert x.runtime.manual_status("Dutch")["phase"] == "approval_pending"
...
@pytest.mark.parametrize(
    "phase",
    ["request_submitted", "request_verified", "accept_submitted", "cancel_submitted"],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "merchant_process",
        "farmer_process",
        "merchant_character_uid",
        "farmer_character_uid",
        "merchant_position",
        "farmer_position",
        "recipient_uid",
        "recipient_name",
        ...
        "saved_process_changed",
        "saved_request_future",
    ],
)
def test_nonmatching_journal_never_suppresses_a_real_manual_request(
    ...
    if fault == "unknown_phase":
        x.probe["phase"] = "unknown"
    if fault == "prepared":
        x.probe["phase"] = "prepared"
    if fault == "wrong_character":
        x.probe["character"] = "Spiritual"
    if fault == "wrong_target_profile":
        x.probe["target_profile_id"] = "other-profile"
    if fault == "wrong_farmer_profile":
        x.probe["farmer_profile_id"] = "other-source"
    if fault == "wrong_uid":
        x.state["request"]["participant_uid"] += 1
    if fault == "live_bool_uid":
        x.state["request"]["participant_uid"] = True
    if fault == "wrong_name":
        x.state["request"].update(
            participant="Visitor", message="Visitor wishes to trade with you."
        )
    if fault == "wrong_message":
        x.probe["intent"]["farmer"]["character"] = "Visitor"
    if fault == "live_wrong_message":
        x.state["request"]["message"] = "Unrelated confirmation"
    if fault == "live_request_server":
        x.state["request"]["server"] = "other-server"
    if fault == "stale_merchant":
        x.read = lambda: {**deepcopy(x.state), "timestamp": 90}
    if fault == "wrong_server":
        x.probe["intent"]["merchant"]["server"] = "other-server"
    if fault == "merchant_process":
        x.probe["intent"]["merchant"]["identity"]["creation_time_100ns"] += 1
    if fault == "farmer_process":
        x.farmer["identity"]["creation_time_100ns"] += 1
    if fault == "merchant_character_uid":
        x.probe["intent"]["merchant"]["character_uid"] += 1
    if fault == "farmer_character_uid":
        x.farmer["character_uid"] += 1
    if fault == "merchant_position":
        x.probe["intent"]["merchant"]["position"][0] += 1
    if fault == "farmer_position":
        x.farmer["position"][0] += 1
    if fault == "recipient_uid":
        x.probe["recipient"]["uid"] += 1
    if fault == "recipient_name":
        x.probe["recipient"]["name"] = "Spiritual"
    if fault == "recipient_position":
        x.probe["recipient"]["position"][0] += 1
    if fault == "selected_uid":
        x.probe["selected_uids"] = [1000]
    if fault == "missing_intent":
        x.probe.pop("intent")
    if fault == "missing_start":
        x.probe.pop("started_at")
    if fault == "future_start":
        x.probe["started_at"] = 101
    if fault == "past_update":
        x.probe["updated_at"] = 99
    if fault == "future_update":
        x.probe["updated_at"] = 101
    if fault == "nan_update":
        x.probe["updated_at"] = float("nan")
    if fault == "stale_farmer":
        x.farmer_read = lambda: {**deepcopy(x.farmer), "timestamp": 90}
    if fault == "wrong_source_observer":
        x.source.character = "Different"
    if fault == "farmer_trade":
        x.farmer["trade"] = {"participant": "Dutch", "participant_uid": 123}
    if fault == "stock_changed":
        x.farmer["silver"] += 1
    if fault == "source_server":
        x.farmer["server"] = "other-server"
    if fault == "missing_saved_request":
        x.probe.pop("merchant_after")
    if fault == "saved_request_changed":
        x.probe["merchant_after"]["request"]["participant_uid"] += 1
    if fault == "saved_process_changed":
        x.probe["farmer_after"]["identity"]["pid"] += 1
    if fault == "saved_request_future":
        x.probe["merchant_after"]["timestamp"] = 101
    x.save()
    if fault == "missing":
        x.path.unlink()
    if fault == "corrupt":
        x.path.write_text("{invalid", encoding="utf-8")
    if fault == "list":
        x.path.write_text("[]", encoding="utf-8")
...
def test_exact_false_manual_admission_is_retracted_durably_without_input(
    supervised, phase, decline
):
    x = supervised
    ...
    if decline:
        store.reject(binding, operator="Floor", now=x.now)
    if phase == "needs_attention":
        store.observe(row["id"], {"reader_error": "temporary"}, now=x.now)
    x.now += 1
    x.runtime.step("Dutch")
    ...
    assert (
        receipt["original_phase"] == phase and receipt["request_still_visible"] is True
    )
    ...
    assert x.runtime.manual_status(
        "Dutch"
    ) is None and not x.guard.manual_session_blocked("Dutch")
    ...
    with store.db() as db:
        ...
        assert db.execute("SELECT COUNT(*) FROM manual_declines").fetchone()[0] == int(
            decline
        )
    events = [
        a
        for a in store.audit(row["id"])
        if a["event"] == "manual_admission_retracted_bot_owned"
    ]
    assert len(events) == 1
    x.now += 1
    x.runtime.step("Dutch")
    ...
    assert (
        "Game Request Still Visible" in message and "Request Withdrawn" not in message
    )
    assert "Input fence: released" in message


def test_full_preaccept_reconciliation_retracts_false_pending_after_recoverable_error(
    supervised,
):
    x = supervised
    store = x.runtime.manual_sessions
    ...
    assert x.runtime.process_probe_owned(
        "Dutch", x.read(), now=x.now, require_bilateral=True
    )
    final = store.get(row["id"])
    assert final["phase"] == "request_withdrawn" and not final["holds_automation"]
    assert final["terminal"]["disposition"] == "manual_admission_retracted_bot_owned"
    assert (
        final["terminal"]["gameplay_input"] is False
        and not store.permissions()
        and x.calls == []
    )


def test_supplied_preaccept_pair_reconciles_without_reacquiring_busy_farmer_observer(
    supervised,
):
    x = supervised
    store = x.runtime.manual_sessions
    ...
    x.source.lock = NS(
        acquire=lambda **_kwargs: pytest.fail(
            "pre-read pair must not reacquire farmer lock"
        ),
        release=lambda: None,
    )
...
def test_supplied_preaccept_pair_mismatch_cannot_retract_or_bypass_manual_hold(
    supervised,
):
    x = supervised
    store = x.runtime.manual_sessions
...
def test_supplied_preaccept_pair_preserves_genuine_manual_hold(supervised, protected):
    ...
    if protected == "approved":
        store.allow_and_activate(
            row["approval_binding"], x.read(), operator="Floor", now=x.now
        )
    else:
    ...
    x.runtime._sync_manual_fence()
    x.now += 1
    assert x.runtime.reconcile_probe_owned(
...
def test_existing_real_or_uncertain_manual_intervals_are_never_released(
    supervised, protected
):
    x = supervised
    store = x.runtime.manual_sessions
    original = x.read()
    if protected == "original_uid":
        original["request"]["participant_uid"] += 1
    if protected == "original_process":
        original["identity"]["pid"] += 1
    if protected == "original_stock":
        original["silver"] += 1
    if protected == "original_position":
        original["position"][0] += 1
    if protected == "original_map":
        original["map_id"] = 1002
    if protected == "original_slot":
        original["inventory"][0]["slot"] = 5
    ...
    if protected == "changed_probe":
        ...
        x.save()
    if protected == "other_decline_input":
        x.journal.set("Dutch", "unrelated_request_decline", {"phase": "submitted"})
    x.now += 1
    x.runtime.step("Dutch")
    final = store.get(row["id"])
    assert final["holds_automation"] and final["terminal"] is None
...
def open_trade(x, *, phase="trade_open_verified", offered=False):
    x.probe["phase"] = phase
    if phase == "trade_open_verified":
        x.probe["offered_uids"] = list(x.probe["selected_uids"]) if offered else []
    ...
    x.state.update(
        request=None,
        trade={**deepcopy(terms), "participant": "Parasite", "participant_uid": 55},
    )
    x.farmer["trade"] = {
        **deepcopy(terms),
        "participant": "Dutch",
    ...
    if phase not in ("accept_submitted", "cancel_submitted"):
        x.probe.update(
            accepted_at=x.now,
            updated_at=x.now,
            farmer_after=x.farmer_read(),
            merchant_after=x.read(),
        )
    x.save()
...
def test_exact_farmer_trade_has_same_bilateral_probe_precedence(
    supervised, monkeypatch, phase, offered
):
    ...
    monkeypatch.setattr(
        manual_farmer,
        "controller",
        lambda *a: pytest.fail("Probe must not create a decline controller"),
    )
...
def test_farmer_mismatch_is_quarantined_without_any_probe_privilege(supervised, fault):
    ...
    if fault == "merchant_uid":
        x.farmer["trade"]["participant_uid"] += 1
    if fault == "farmer_uid":
        x.state["trade"]["participant_uid"] += 1
    if fault == "merchant_name":
        x.farmer["trade"]["participant"] = "Spiritual"
    if fault == "farmer_name":
        x.state["trade"]["participant"] = "Visitor"
    if fault == "process":
        x.state["identity"]["creation_time_100ns"] += 1
    if fault == "farmer_process":
        x.farmer["identity"]["creation_time_100ns"] += 1
    if fault == "merchant_silver":
        x.state["silver"] += 1
    if fault == "farmer_silver":
        x.farmer["silver"] += 1
    if fault == "own_silver":
        x.farmer["trade"]["own_silver"] = 1
    if fault == "other_silver":
        x.state["trade"]["other_silver"] = 1
    if fault == "wrong_item":
        x.farmer["trade"]["own_items"][0]["plus"] = 2
    if fault == "missing_offer":
        x.farmer["trade"]["own_items"] = []
        x.state["trade"]["items"] = []
    if fault == "merchant_offer":
        x.state["trade"]["own_items"] = deepcopy(x.state["inventory"])
    if fault == "farmer_inventory":
        x.farmer["inventory"].append({**deepcopy(x.farmer["inventory"][0]), "uid": 999})
    if fault == "merchant_inventory":
        x.state["inventory"] = []
    if fault == "booth_price":
    ...
    if fault == "wrong_target":
        x.probe["target_profile_id"] = "other-target"
    if fault == "stale_peer":
        x.driver.read = lambda: {**deepcopy(x.state), "timestamp": 90}
    ...
    if fault == "missing_saved_peer":
        x.probe.pop("farmer_after")
    if fault == "saved_peer_identity":
        x.probe["merchant_after"]["identity"]["pid"] += 1
    if fault == "malformed_currency":
        x.farmer["trade"]["own_silver"] = False
    if fault == "missing_acceptance":
        x.farmer["trade"].pop("accepted")
    x.save()
    if fault == "corrupt":
        x.path.write_text("{bad", encoding="utf-8")
...
def test_probe_does_not_release_existing_farmer_interval(supervised, approved):
    ...
    if approved:
        x.runtime.manual_sessions.allow_and_activate(
            row["approval_binding"], incoming, operator="Floor", now=x.now
        )
    else:
        x.runtime.manual_sessions.reject(
            row["approval_binding"], operator="Floor", now=x.now
        )
...
def test_profile_scoped_probe_requires_explicit_exact_profile_binding(supervised):
    ...
    for key in ("target_profile_id", "farmer_profile_id"):
        x.probe.pop(key)
    assert ownership(
        x.probe, "Dutch", "Dutch", "Farmer", x.farmer_read(), x.read(), now=x.now
    )
    ...
    x.probe.update(
        target_profile_id="merchant-profile", farmer_profile_id="farmer-profile"
    )
    assert ownership(
        x.probe,
        "Dutch",
...
@pytest.mark.parametrize(
    "boundary",
    [
        "verified_after_admission",
        "missing_verified_time",
        "submitted_only",
        "evidence_predates_verified_time",
    ],
)
def test_retraction_requires_original_admission_after_durable_request_boundary(
    supervised, boundary
):
    x = supervised
    ...
    if boundary == "evidence_predates_verified_time":
        x.now = 101
    row = x.runtime.manual_sessions.begin_request("Dutch", original, now=x.now)
    if boundary == "missing_verified_time":
        x.probe.pop("updated_at")
    if boundary == "submitted_only":
        x.probe["phase"] = "request_submitted"
    x.save()
    x.now = 102
    x.runtime.step("Dutch")
...
@pytest.mark.parametrize(
    "phase,prior,placing,live,allowed",
    [
...
def test_open_trade_offer_is_bound_to_exact_durable_phase(
    supervised, phase, prior, placing, live, allowed
):
    x = supervised
    ...
    open_trade(
        x, phase=phase, offered=phase in ("offer_verified", "farmer_confirm_verified")
    )
    if prior is None:
        x.probe.pop("offered_uids", None)
    else:
        x.probe["offered_uids"] = prior
    if placing is None:
        x.probe.pop("placing_uid", None)
    else:
        x.probe["placing_uid"] = placing
    x.save()
