...
def progress_status():
    ...
    return {
        "handoff_requested": "merchant-refill:Dutch:123",
        "characters": {
            "Dutch": {
                "profile_id": "dutch-profile",
                "connected": True,
                "pending": [],
                ...
                "refill": {
                    ...
                    "attempt_started_at": 900,
                    "last_verified_listing": {
                        "request_id": "booth-list1078-refill-proof",
                        "verified_at": 940,
                        "character": "Dutch",
                        "profile_id": "dutch-profile",
                        "character_uid": 9,
...
def test_verified_progress_is_consumed_before_failed_parking_and_survives_restart(
    ...
    assert not windows.reserve(
        "repeat-failure", listing_progress=proof, continuation=True
    )
    next_proof = {
        **proof,
        "request_id": "booth-list1078-refill-next",
        "verified_at": 1001,
    }
    now[0] = 1061
    assert windows.reserve(
        "new-progress", listing_progress=next_proof, continuation=True
    )
...
def test_continuation_requires_current_priced_same_actor_receipt(monkeypatch, change):
    ...
    r = c["refill"]
    p = r["last_verified_listing"]
    assert handoff.pending_listing_progress(status)
    if change == "stale":
        c["snapshot"]["timestamp"] = 997.9
    elif change == "future_observation":
        c["snapshot"]["timestamp"] = 1001
    elif change == "future_receipt":
        p["verified_at"] = 1001
    elif change == "nan_receipt":
        p["verified_at"] = float("nan")
    elif change == "old_check":
        r["attempt_started_at"] = 950
    elif change == "unknown_queue":
        r["deferred"] = 2
    elif change == "unknown_status":
        c["foreground_refill_1078"] = {"state": "unknown_prices_deferred"}
    elif change == "manual":
        c["manual_input_fence"] = True
    elif change == "transaction":
        c["pending"] = [{"phase": "uncertain"}]
    elif change == "attention":
        c["needs_attention"] = {"reason": "uncertain"}
    elif change == "paused":
        r["enabled"] = False
    elif change == "process":
        p["identity"] = {**p["identity"], "creation_time_100ns": 99}
    elif change == "profile":
        p["profile_id"] = "another-profile"
    elif change == "character":
        p["character"] = "Spiritual"
    elif change == "uid":
        p["character_uid"] = 10
    elif change == "active_listing":
        r["listing1078_request"] = {"request_id": "new"}
...
def completed_ui(tmp_path, monkeypatch):
    ...
    runtime = NS(
        lock=threading.RLock(),
        handoff="merchant-refill:Dutch:123",
    ...
    ui = NS(
        ...
        grant=None,
        grant_fence=NS(active=None, requests={}),
        calibrating=set(),
        connect_threads={},
    )
    calls = []
...
def test_completed_refill_uses_existing_exact_release_without_changing_controls(
    completed_ui,
):
    ...
    assert calls == [
        {"action": "handoff-release", "request_id": "merchant-refill:Dutch:123"}
    ]
...
def test_completed_refill_never_releases_live_or_uncertain_work(
    completed_ui, monkeypatch, change
):
    ui, schedule, calls, finished = completed_ui
    if change == "other_request":
        ui.runtime.handoff = "merchant-refill:Spiritual:123"
    elif change == "grant":
        ui.grant = {"request_id": ui.runtime.handoff}
    elif change == "active_fence":
        ui.grant_fence.active = object()
    elif change == "known_fence":
        ui.grant_fence.requests[ui.runtime.handoff] = object()
    elif change == "owner":
        ui.coordinator.owner = "Dutch"
    elif change == "manual":
        ui.coordinator.manual_sessions = {"Dutch": {"holds_automation": True}}
    elif change == "manual_global":
        ui.runtime.manual_handoff_status = lambda: {"active": True}
    elif change == "stop":
        ui.runtime.stop_event.set()
    elif change == "pending_refill":
        schedule.start()
    elif change == "listing_worker":
        monkeypatch.setitem(listing.WORKERS, "running", NS(is_alive=lambda: True))
    elif change == "probe_worker":
        ui.delivery_probe_thread = NS(is_alive=lambda: True)
    elif change == "transaction":
        ui.runtime.journal.begin("pending", "Dutch", "delivery", {})
    elif change == "farmer_delivery":
        ui.runtime.farmer_bot_owned = lambda: True
    elif change == "calibrating":
        ui.calibrating.add("Dutch")
...
@pytest.mark.parametrize(
    "scope,scheduled,remaining,expected",
    [
        ("market_visit", True, 60, 35),
        ("market_visit", True, 25, 22),
...
def test_worker_budget_obeys_grant_and_keeps_unrelated_operations_at_twenty(
    ...
    before = {
        ...
        "farmer_grant": {"scope": scope, "expires_at": 1000 + remaining}
        if scope
        else None,
    }
    ...
    monkeypatch.setattr(
        input_probe, "MessageTarget", lambda pid, hwnd: NS(pid=pid, hwnd=hwnd)
    )
    ...
    monkeypatch.setattr(
        listing.GuiReader, "for_session", lambda session: NS(model=lambda *args: 1)
    )
    monkeypatch.setattr(listing, "open_read_only_1078", lambda *args: NS())
    monkeypatch.setattr(
        listing, "read_build_layout", lambda session: NS(merchant_booth_vtable_rva=2)
    )
    ...
    listing._run(
        NS(runtime=NS(journal=journal), coordinator=coordinator),
        "Dutch",
        NS(name="Dutch"),
        before,
        None,
...
def test_progress_proof_requires_verified_receipt_and_is_counted_once_after_restart(
    receipt,
):
    ...
    refill_1078._settle_cursor(x.j, "Dutch", x.key)
    reopened = Journal(x.j.path)
    state = reopened.get("Dutch", "refill")
    proof = state["last_verified_listing"]
    assert state["listed"] == 1 and state["cursor"] == [92]
    assert (
        proof["request_id"] == x.key and proof["profile_id"] == x.baseline["profile_id"]
    )
...
@pytest.mark.parametrize(
    "remaining,scan_cost,expected_reads", [(37.9, 0, 0), (38, 0, 0), (45, 8, 1)]
)
def test_scheduled_admission_rechecks_full_budget_after_slow_preflight(
    ...
    runtime = NS(
        journal=x.j,
        work_deadline=1000 + remaining,
        merchant_windows=lambda: [NS(identity=x.before["identity"], hwnd=11)],
    )
    runtime.can_start_work = lambda seconds: MerchantRuntime.can_start_work(
        runtime, seconds
    )
    ...
    monkeypatch.setattr(
        input_probe,
        "MessageTarget",
        lambda *args: NS(hwnd=11, snapshot=lambda: {"root_hwnd": 11}),
    )
    with pytest.raises(
        CaptureUnavailable, match="seconds remaining|preflight consumed"
    ):
