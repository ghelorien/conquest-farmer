...
def test_diagonal_travel_does_not_cut_blocked_corners_or_guarded_tiles():
    ...
    blocked=np.zeros((30,30),dtype=bool);blocked[:20,15]=True
    ...
    assert all(clear_segment(terrain,a,b,avoid={(14,20)}) for a,b in zip(path,path[1:]))
...
def test_installed_map_format_blocks_portals_and_solid_cells(tmp_path):
    (tmp_path/'ini').mkdir()
    (tmp_path/'ini/GameMap.json').write_text(json.dumps([{'DocumentId':1002,'FileName':'test.DMap'}]))
    header=struct.pack('<II',1003,0)+bytes(260)+struct.pack('<II',5,5)
    cells=b''.join(b''.join(struct.pack('<HHh',int(x==0),3,0) for x in range(5))+bytes(4) for y in range(5))
    tail=struct.pack('<I',1)+struct.pack('<III',4,4,7)+struct.pack('<II',0,0)
...
def test_return_boundary_includes_terrain_detours_and_departure():
    ...
    assert all(boundary[0]<=x<=boundary[2] and boundary[1]<=y<=boundary[3] for x,y in path)
...
def test_boundary_excursion_plans_traversable_return_to_saved_spot():
    ...
    waypoints,boundary=plan_hunting_return(terrain,(20,10),(10,10),(5,5,12,12))
    ...
    assert all(boundary[0]<=x<=boundary[2] and boundary[1]<=y<=boundary[3] for x,y in waypoints)
    assert max(max(abs(b[0]-a[0]),abs(b[1]-a[1])) for a,b in zip(((20,10),*waypoints),waypoints))<=12


def test_boundary_return_rejects_unreachable_or_wrong_hunting_spot():
    from conquest.navigation import TerrainMap,plan_hunting_return
    import numpy as np,pytest
    blocked=np.zeros((30,30),dtype=bool);blocked[:,15]=True
    terrain=TerrainMap(1002,30,30,blocked,'test',(),())
...
def test_scene_collision_connects_bridge_and_keeps_portals_blocked(tmp_path,portal):
    ...
    cells=b''.join(b''.join(struct.pack('<HHh',int(x==5),0,0) for x in range(10))+bytes(4) for y in range(10))
    part=bytearray(356)
    struct.pack_into('<II',part,332,3,2)
    struct.pack_into('<ii',part,344,-1,-2)
    # Anchor (7,7) plus (-1,-2) minus (width-1,height-1) => (4,4).
    scene=struct.pack('<I',1)+part+b''.join(struct.pack('<III',a,0,0) for a in [1,1,1,0,0,0])
    (tmp_path/'bridge.scene').write_bytes(scene)
    tail=struct.pack('<I',int(portal))+(struct.pack('<III',5,5,7) if portal else b'')
    tail+=struct.pack('<II',1,1)+b'bridge.scene'.ljust(260,b'\0')+struct.pack('<II',7,7)
    (tmp_path/'test.DMap').write_bytes(header+cells+tail)
    terrain=read_terrain(tmp_path,1002)
