...
def state(level=32, kind=1050000, attack=10):
    return {
        ...
        "equipment": {
            "arrows": dict(
                type_id=kind,
                level=1 if kind == 1050000 else 32,
                profession=40,
                attack_min=attack,
                attack_max=attack,
            )
...
def test_level_32_selects_iron_arrows_without_spending_supply_reserve():
    ...
    assert (
        choose_arrow_upgrade([product(kind=1050020, attack=100)], state(), 20000)
        is None
    )
...
def test_equipped_or_reserved_iron_is_not_treated_as_missing_lucky_arrows():
    ...
    assert (
        current_arrow({"level": 31, "equipment": {}}, reserves=[1050000, 1050001])
        == 1050000
    )
    ...
    bag = {
        "items": [
            {"type_id": 1050001, "amount": 1000},
            {"type_id": 1050000, "amount": 200},
        ],
        "equipped_ammo": {"type_id": 1050001, "amount": 836},
        "silver": 15983,
        "capacity": 40,
    }
...
def test_optional_iron_topup_preserves_money_but_essential_restock_can_spend_it():
    ...
    loop.route = loop.route.model_copy(
        update={
            "supplies": loop.route.supplies.model_copy(update={"arrow_type": 1050001})
        }
    )
    loop.record = lambda *a, **k: None
    bag = {
        "items": [],
        "equipped_ammo": {"type_id": 1050001, "amount": 1000},
        "silver": 7000,
        "capacity": 40,
    }
    calls = []
...
def test_tiny_iron_reserves_can_be_recycled_without_selling_last_ammo():
    ...
    assert not expendable_arrow(
        small, InventorySnapshot(0, 0, (small,), None, 9000, 40)
    )
...
def test_purchase_cap_counts_partial_packs_and_prevents_topups(count):
    ...
    bag = {
        "items": [
            {"uid": i, "type_id": 1050001 if i % 2 else 1050000, "amount": 24}
            for i in range(count - 1)
        ],
        "equipped_ammo": {"uid": 99, "type_id": 1050001, "amount": 23},
        "silver": 100000,
        "capacity": 40,
    }
    loop.town = lambda action, **kw: (
        bag if action == "supplies" else pytest.fail("No shop input at pack cap")
    )
...
def test_arrow_upgrade_does_not_bypass_pack_cap():
    ...
    bag = {
        "items": [{"uid": i, "type_id": 1050000, "amount": 24} for i in range(10)],
        "equipped_ammo": None,
    }
    loop = SimpleNamespace(
        town=lambda action, **kw: (
            bag if action == "supplies" else pytest.fail("Must not buy upgrade")
        ),
...
def test_spent_scatter_remnants_are_retired_before_refill_even_with_free_slots():
    ...
    loop.record = lambda *a, **kw: None
    bag = {
        "items": [{"uid": 1, "type_id": 1050001, "amount": 2}],
        "equipped_ammo": None,
        "silver": 200,
        "capacity": 40,
    }
...
def test_owned_speed_upgrade_does_not_require_purchase_money():
    ...
    assert (
        choose_arrow_upgrade([speed], state(73, 1050001, 50), 200, carried={1050002})
        == speed
    )
