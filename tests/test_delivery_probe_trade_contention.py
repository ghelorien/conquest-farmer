...
def observe(x, role):
    owner, snapshot = ('Dutch', x.read()) if role == 'merchant' else ('Farmer', x.farmer_read())
...
@pytest.mark.parametrize('phase,offered', [
    ...
    ('placement_submitted', False), ('placement_submitted', True),
    ('trade_open_verified', True), ('offer_verified', True),
    ('farmer_confirm_submitted', True), ('farmer_confirm_verified', True),
    ('merchant_confirm_submitted', True),
])
def test_peer_lock_defers_exact_trade_without_manual_admission(supervised, role, phase, offered):
    x = supervised
    open_trade(x, phase=phase, offered=offered)
    blocked_peer(x, role)
    for _ in range(3):assert observe(x, role)
    ...
    assert not x.runtime.process_probe_owned('Dutch' if role == 'merchant' else 'Farmer',
        x.read() if role == 'merchant' else x.farmer_read(), require_bilateral=True)


@pytest.mark.parametrize('role', ['farmer', 'merchant'])
@pytest.mark.parametrize('phase,offered', [('trade_open_verified', False), ('offer_verified', True)])
def test_actual_cross_thread_peer_lock_race_is_observation_only(supervised, role, phase, offered):
    x = supervised
    open_trade(x, phase=phase, offered=offered)
    peer = x.source if role == 'merchant' else x.runtime.observers['Dutch']
...
@pytest.mark.parametrize('fault', [
    'process', 'name', 'uid', 'server', 'position', 'map', 'silver', 'inventory', 'capacity',
    'booth', 'peer_uid', 'peer_name', 'peer_server', 'own_silver', 'other_silver',
    'acceptance', 'reverse_offer', 'offer', 'missing_offer', 'phase', 'saved_pair',
    'saved_time', 'saved_after_write', 'saved_before_start', 'saved_acceptance', 'rollover', 'digest',
])
def test_local_contention_mismatch_still_quarantines(supervised, monkeypatch, role, fault):
    x = supervised
    ...
    if fault == 'map':value['map_id'] = 1002
    if fault == 'silver':value['silver'] += 1
    if fault == 'inventory':value['inventory'].append({**deepcopy(value['inventory'][0]), 'uid': 901})
    if fault == 'capacity':value['capacity'] -= 1
    if fault == 'booth':value['own_booth_uid'] += 1
    if fault == 'peer_uid':value['trade']['participant_uid'] += 1
    if fault == 'peer_name':value['trade']['participant'] = 'Other'
    if fault == 'peer_server':value['trade']['server'] = 'Other'
    if fault == 'own_silver':value['trade']['own_silver'] = 1
    if fault == 'other_silver':value['trade']['other_silver'] = False
    if fault == 'acceptance':value['trade']['accepted'] = True
    offered_key = 'own_items' if role == 'farmer' else 'items'
    if fault == 'reverse_offer':value['trade']['items' if role == 'farmer' else 'own_items'] = deepcopy(x.probe['intent']['items'])
    if fault == 'offer':value['trade'][offered_key][0]['quantity'] += 1
    if fault == 'missing_offer':value['trade'][offered_key] = []
    if fault == 'phase':x.probe['phase'] = 'request_verified'
    if fault == 'saved_pair':x.probe['merchant_after']['trade']['items'][0]['uid'] += 1
    if fault == 'saved_time':x.probe['farmer_after']['timestamp'] = 101
    if fault == 'saved_after_write':
        x.now = 102
        x.probe['farmer_after']['timestamp'] = 101
    if fault == 'saved_before_start':x.probe['farmer_after']['timestamp'] = 99
    if fault == 'saved_acceptance':x.probe['merchant_after']['trade']['accepted'] = True
    if fault == 'rollover':
        peer = x.source if role == 'merchant' else x.runtime.observers['Dutch']
...
def test_restart_attachment_gap_defers_only_observation(supervised, role):
    x = supervised
    open_trade(x, phase='offer_verified', offered=True)
    if role == 'merchant':x.runtime.manual_farmer_provider = lambda: None
    else:x.runtime.observers.pop('Dutch')
    assert observe(x, role)
    assert x.runtime.manual_status() == [] and x.calls == []


@pytest.mark.parametrize('fault', [None, 'after_accept', 'visitor', 'silver', 'process', 'slot', 'peer_process'])
def test_request_read_before_accept_write_cannot_mint_false_manual_session(supervised, fault):
    x = supervised
    stale = x.read()
    x.now += .2
    open_trade(x)
    x.now += .2
    if fault == 'after_accept':stale['timestamp'] = x.now
    if fault == 'visitor':stale['request']['participant_uid'] += 1
    if fault == 'silver':stale['silver'] += 1
    if fault == 'process':stale['identity']['pid'] += 1
    if fault == 'slot':stale['inventory'][0]['slot'] = 20
    if fault == 'peer_process':x.source.adapter.identity = {**x.source.adapter.identity, 'pid': 999}
...
@pytest.mark.parametrize('reconciled,held', [(False,None),(True,None),(True,'Farmer'),(True,'Dutch')])
def test_trade_stage_reconciles_pair_before_first_lease_or_input(supervised, monkeypatch, stage, reconciled, held):
    from conquest.capture import CaptureUnavailable
    from conquest.merchants import delivery_offer_probe, delivery_confirm_probe
    x = supervised
    open_trade(x, phase='trade_open_verified' if stage == 'offer' else 'offer_verified', offered=stage == 'confirm')
    ...
    def reconcile(character, farmer, merchant):
        ...
        assert character == 'Dutch' and farmer == x.farmer_read() and merchant == x.read()
        return reconciled
...
def test_any_uncertain_side_rolls_back_both_retractions(supervised, monkeypatch, fault, observe_open):
    ...
    if fault in ('missing_evidence', 'rollover', 'stock_change'):
        evidence = {'reader_error': 'missing'} if fault == 'missing_evidence' else x.read()
        if fault == 'rollover':evidence['identity']['creation_time_100ns'] += 1
        if fault == 'stock_change':evidence['silver'] += 1
    ...
    if fault in ('decline_pending', 'decline_claimed', 'approved', 'multiple_request'):
        with store.db() as db:
            if fault == 'approved':
                db.execute("UPDATE manual_sessions SET ever_approved=1 WHERE id=?", (merchant['id'],))
                db.execute("UPDATE manual_requests SET state='approved' WHERE session_id=?", (merchant['id'],))
            elif fault == 'multiple_request':
                db.execute("INSERT INTO manual_requests SELECT id||'2',session_id,fingerprint,binding_json,before_json,'pending',created_at,expires_at FROM manual_requests WHERE session_id=?", (merchant['id'],))
            else:db.execute('UPDATE manual_requests SET state=? WHERE session_id=?', (fault, merchant['id']))
    if fault == 'journal_race':
        calls = [0]
        def read():
            calls[0] += 1
            return deepcopy(x.probe) if calls[0] < 3 else {**x.probe, 'phase': 'delivery_verified'}
    ...
    if fault == 'write_failure':
        ...
        def fail(db, session, event, payload, now):
            count[0] += 1
            if count[0] == 2:raise BindingMismatch('injected second-side write failure')
