...
def config():
    return TrialConfig(character="Parasite", player_profile="unused", inventory_profile="unused", template="unused",
                       client_size=(1584, 861), boundary=(405, 442, 435, 472))


@pytest.mark.parametrize("fraction", [0, .25, .5, .98, 1])
def test_health_fill_including_death(fraction):
    ...
    assert health_ratio(frame) == pytest.approx(fraction, abs=.002)
...
def test_stale_health_and_wrong_monsters_cannot_trigger_attack():
    target = Target("Pheasant", 830, 450, .99)
    c = config()
    assert choose_target([target], .99, (425, 460), c, .01) == target
    assert choose_target([target], .39, (425, 460), c, .01) is None
    assert choose_target([target], .99, (425, 460), c, .36) is None
    assert choose_target([target], .99, (100, 100), c, .01) is None
    assert choose_target([Target("Player", 830, 450, 1)], 1, (425, 460), c, .01) is None


def test_template_requires_health_bar_and_excludes_chat():
    template = cv2.imread(str(Path(__file__).parents[1] / "profiles/templates/pheasant-name.png"), 0)
    frame = np.zeros((861, 1584, 3), np.uint8)
...
def test_turtledove_calibration_filters_by_selected_monster():
    ...
    found = targets(frame, template, .94, "Turtledove")
    assert len(found) == 1 and found[0].name == "Turtledove"
    c = config()
    assert choose_target(found, .9, (425,460), c, .1) is None
    c.monster = "Turtledove"
    assert choose_target(found, .9, (425,460), c, .1) == found[0]
